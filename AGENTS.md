# Anakot Agent - Development Guide

Instructions for AI coding assistants and developers working on the anakot-agent codebase.
...
Anakot is a personal AI agent that runs the same agent core across a CLI, a messaging
...
Read the balance right: Anakot ships a **lot**. Most merges are bug fixes to reported
...
- **Expand reach at the edges.** New adapters, channels, providers, models, desktop/TUI/
  ...
  the existing setup/config UX (`anakot tools`, `anakot setup`, auto-install) rather than
...
- **E2E validation, not just green unit mocks.** Anything touching resolution chains, config
  ...
  real path with real imports against a temp `ANAKOT_HOME` — two of them (A→B→A) when the
...
- **New `ANAKOT_*` env vars for non-secret config.** `.env` is for secrets only. Behavioral
...
- **Outbound telemetry / usage attribution without opt-in gating.** No analytics,
  ...
  (config gate + setup prompt + `anakot tools` toggle) exists. Park behind a label.
...
- **Third-party products integrated into the core tree.** Observability backends, vendor
  ...
  backend we don't own. Ship as a **standalone plugin repo** (`~/.anakot/plugins/` or pip
...
2. **CLI command + skill** — config/state/infra expressible as shell commands; the agent runs
   `anakot <subcommand>` guided by a skill. Default for subscriptions, scheduled tasks,
   service setup (`anakot webhook`, `anakot cron`, `anakot tools`).
...
4. **Plugin** — third-party/niche/user-specific; lives in `~/.anakot/plugins/` or a pip
...
drive a locally spawned backend, one over SSH, one behind URL + token, or Anakot Cloud, and
only the first two carry `ANAKOT_DESKTOP=1`. An env-keyed gate is a silent no-op on the other
topologies — the tool is stripped from the schema while the platform hint tells the model it
is "inside the Anakot desktop app". The pattern:

- **The toolset is the surface gate.** Keep such tools off `_ANAKOT_CORE_TOOLS` and in a named
...
- **Ask which identity you mean.** `ANAKOT_DESKTOP=1` legitimately means "this backend was
  spawned by the app" (cron ticker, web-dist handling). It does NOT mean "a GUI is watching";
  the embedded terminal pane (`anakot --tui` against that backend) is the counterexample.
...
```
`scripts/run_tests.sh` probes `.venv`, then `venv`, then `$HOME/.anakot/anakot-agent/venv`
...
├── toolsets.py           # TOOLSETS dict, _ANAKOT_CORE_TOOLS
├── cli.py                # AnakotCLI (REPL, slash dispatch) + anakot_cli/cli_*_mixin.py
├── anakot_state.py       # SessionDB facade; anakot_state_*.py siblings
├── anakot_constants.py   # get_anakot_home(), display_anakot_home() — profile-aware paths
├── anakot_logging.py     # agent.log / errors.log / gateway.log (profile-aware)
├── batch_runner.py       # Parallel batch processing
├── agent/                # turn_*.py loop phases, providers, memory, compression, prompt builder
├── anakot_cli/           # CLI subcommands, setup, config, plugins loader, skins, updater
...
`~/.anakot/logs/` (`agent.log` INFO+, `errors.log` WARNING+, `gateway.log`); all
profile-aware via `get_anakot_home()`. Browse logs with `anakot logs [--follow] [--level] [--session]`.
...
- **Compat pointers are OFF LIMITS in-tree.** Old import paths kept alive for external plugins
  ...
  `-W error::anakot_cli.plugin_compat.AnakotPluginCompatWarning` catches them in the suite.
...
- **Never infer process identity from argv substrings** (`"serve" in cmdline`) — the bug class
  ...
  `anakot_cli.update_cmd._anakot_holder_subcommand`; flag sets are DERIVED from the parser
  (`_holder_value_flags()`), never hand-written; match FULL cmdlines and truncate only for
  display. Details: `anakot_cli/AGENTS.md`.
- **Never hardcode `~/.anakot`.** `get_anakot_home()` for code paths, `display_anakot_home()`
  for user-facing text (both from `anakot_constants`). Hardcoding breaks profiles (5 bugs in
  PR #3575). Profile operations themselves are HOME-anchored
  (`_get_profiles_root()` = `Path.home()/.anakot/profiles`) so `anakot -p x profile list`
...
- **One process may serve many profiles; code that runs outside a turn binds the owning
  ...
  module constants are a bug class — key slots by `anakot_home_key()` or resolve at call time.
  ...
  `ANAKOT_HOME`. Advisory lint: `scripts/check_profile_scope_patterns.py`.
- **Argparse alias dispatch:** `add_parser("list", aliases=["ls"])` sets `dest` to the literal
  the user typed (`"ls"`). Dispatch must accept both (caught PTY-testing `anakot webhook ls`).
- **Don't wire in dead code without E2E validation.** Unshipped code was dead for a reason;
  E2E the real resolution chain with real imports against a temp `ANAKOT_HOME` first.
...
vars unset, `TZ=UTC`, `LANG=C.UTF-8`, `ANAKOT_HOME` → temp dir, and per-file subprocess
...
- **Flake policy:** a failing FILE is retried once in a fresh subprocess (`--file-retries`;
  `ANAKOT_TEST_FILE_RETRIES=0` disables). Pass-on-retry is green but printed under `⚠ FLAKY`
...
- **Placement mirrors the source tree.** A test lives in `tests/<top-level source dir>/` (`tests/anakot_cli/`,
  `tests/agent/`, `tests/anakot_state/`, `tests/gateway/relay/`, ...); installer/updater script tests
  under `tests/scripts/{install,desktop_update}/`. Only tests of root-level modules (`batch_runner`,
  `utils`, `anakot_constants`, packaging) sit directly in `tests/`. No issue numbers in filenames —
...
- **Tests must not write to `~/.anakot/`.** The autouse `_isolate_anakot_home` fixture in
  `tests/conftest.py` redirects `ANAKOT_HOME`; never hardcode `~/.anakot/` in tests. Profile
  tests also mock `Path.home()` so `_get_profiles_root()` / `_get_default_anakot_home()` stay
  in the temp dir (pattern: `tests/anakot_cli/test_profiles.py`):
  ```python
  @pytest.fixture
  def profile_env(tmp_path, monkeypatch):
      home = tmp_path / ".anakot"; home.mkdir()
      monkeypatch.setattr(Path, "home", lambda: tmp_path)
      monkeypatch.setenv("ANAKOT_HOME", str(home))
      return home
  ```
  Tests that `patch.object(Path, "home", ...)` must ALSO set `ANAKOT_HOME` — code reads the
  env var, not `Path.home()/.anakot`.
...
`tests/anakot_cli/test_venv_holder_windows_live.py` on a real `windows-latest` runner (real
...
| `cli.py`, `anakot_cli/`, `main.py` | `anakot_cli/AGENTS.md` | CLI mixins, `_SLASH_DISPATCH`, slash registry, config system + loaders, skins, `anakot update` pipeline, profiles / multiplex |
...
| `plugins/`, `anakot_cli/plugins*.py` | `plugins/AGENTS.md` | Plugin kinds, native compat contract, in-tree policy, Sep-2026 compat window |
| `tui_gateway/`, `ui-tui/` | `tui_gateway/AGENTS.md` | Process model, JSON-RPC transport, key surfaces, slash flow, dev commands |
| `web/`, `anakot_cli/web_routers/` | `web/AGENTS.md` | Dashboard embeds the real TUI; what React may and may not rebuild |
...
| `cron/`, kanban (`anakot_cli/kanban*.py`, `tools/kanban_tools.py`, `plugins/kanban/`) | `cron/AGENTS.md` | Scheduler invariants, job fields, kanban board/dispatcher |
...
`anakot-agent-dev` skill, not here.
