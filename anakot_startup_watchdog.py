...
timeout. Config is env-only (``ANAKOT_STARTUP_WATCHDOG=0``,
``ANAKOT_STARTUP_WATCHDOG_TIMEOUT_S``) because config.yaml parsing is itself in scope.
...
ENV_STARTUP_WATCHDOG = "ANAKOT_STARTUP_WATCHDOG"
ENV_STARTUP_WATCHDOG_TIMEOUT_S = "ANAKOT_STARTUP_WATCHDOG_TIMEOUT_S"
...
# Module singleton: the arm sites (anakot_cli.main / anakot_cli.gateway /
...
def _process_anakot_home() -> Path:
    """ANAKOT_HOME for diagnostic files — stdlib-only replica of the anakot_constants default."""
...
def get_startup_watchdog_dump_path(home: Optional[Path] = None) -> Path:
    """Return ``<ANAKOT_HOME>/logs/gateway-startup-watchdog.log``."""
    base = home if home is not None else _process_anakot_home()
    return base.joinpath(*_DUMP_RELATIVE)


def startup_watchdog_disabled() -> bool:
    """True when ``ANAKOT_STARTUP_WATCHDOG`` opts out explicitly."""
...
def arm_startup_watchdog(
    ...
    *,
    ...
    ``ANAKOT_STARTUP_WATCHDOG=0`` or when the thread could not be started.
    """
    global _handle
    try:
