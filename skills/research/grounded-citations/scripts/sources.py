...
Ledger path resolution (first wins):
  --ledger PATH
  $ANAKOT_CITATION_LEDGER
  $ANAKOT_HOME/cache/citations/ledger.json
"""
...
from _anakot_home import get_anakot_home  # noqa: E402
...
def resolve_ledger_path(explicit: str | None = None) -> Path:
    ...
    env = os.environ.get("ANAKOT_CITATION_LEDGER", "").strip()
    if env:
        return Path(env).expanduser()
    return get_anakot_home() / "cache" / "citations" / "ledger.json"
