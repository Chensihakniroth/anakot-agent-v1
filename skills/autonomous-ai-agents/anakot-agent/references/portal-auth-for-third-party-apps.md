...
## Layer 1 — Is this thing a Anakot plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Anakot process**. Its LLM calls go through Anakot's already-configured provider. | Already uses Portal if user's Anakot is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Anakot. That LLM call is already authenticated
via Anakot's provider config. The plugin's own server auth (e.g.
...
What Anakot Agent has that *feels* like OAuth — `anakot login --provider nous`
opening a browser, user signs in, token lands in `~/.anakot/auth.json` — is a
**Anakot-specific browser flow**. Under the hood it produces a credential
Anakot uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Anakot's existing Portal credential out of `~/.anakot/auth.json`.
...
rides on the credential Anakot already holds.

Where this could live in Anakot:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Anakot itself.
...
1. First decide: Anakot plugin (runs inside Anakot) or separate app? If plugin,
   it already uses Portal via Anakot's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   anakot agent itself") is fully supported. Anakot treats agent
...
pitch in the Tool Gateway announcement is specifically about Anakot Agent's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Anakot is configured to use Portal as its provider. It is
