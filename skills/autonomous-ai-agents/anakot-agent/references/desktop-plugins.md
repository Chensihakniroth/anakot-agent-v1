...
Write plugins for the Anakot desktop app: statusbar items, layout panes,
...
system (`~/.anakot/plugins/`) is otherwise documented separately.

There are TWO on-disk doors, same contract and hot reload:

- `$ANAKOT_HOME/desktop-plugins/<id>/plugin.js` — standalone desktop plugin.
  Loads enabled by default.
- `$ANAKOT_HOME/plugins/<id>/desktop/plugin.js` — the desktop HALF of a
  ...
  `.anakot-package.json` marker) — the ONLY root the renderer loads from — so
...
- The Anakot desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$ANAKOT_HOME/desktop-plugins/` (usually
  `~/.anakot/desktop-plugins/`).

## How to Run

1. Create `$ANAKOT_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (in this skill directory) — that's
   `~/.anakot/...` by default, or `~/.anakot/profiles/<profile>/...` under a
...
The ONLY import surface is `@anakot/plugin-sdk` (plus `react` /
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Anakot
  ...
  (`anakot://index-network/intent/1`), a hash path (`/index-network/intent/1`),
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.anakot/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@anakot/plugin-sdk`, `react`, and
