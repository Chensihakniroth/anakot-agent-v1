...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger anakot agent runs by POSTing events to a URL.
...
All management is via the `anakot webhook` CLI command:

### Create a subscription
```bash
anakot webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.anakot/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.anakot/scripts/` (path traversal is blocked).

```bash
anakot webhook subscribe todoist-anakot \
  --prompt "Task changed: {payload.content}" \
  --script "todoist-anakot-label.py" \
...
```bash
anakot webhook remove <name>
```

### Test a subscription
```bash
anakot webhook test <name>
anakot webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: PR reviews
```bash
anakot webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
anakot webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
anakot webhook subscribe alerts \
...
- Subscriptions persist to `~/.anakot/webhook_subscriptions.json`

## How It Works

1. `anakot webhook subscribe` writes to `~/.anakot/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status anakot-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.anakot/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `anakot webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `anakot webhook test <name>` to verify the route works.
