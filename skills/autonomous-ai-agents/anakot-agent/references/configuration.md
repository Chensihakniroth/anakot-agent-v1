...
Edit with `anakot config edit` or `anakot config set section.key value`.
Full reference: https://anakot-agent.nousresearch.com/docs/user-guide/configuration
...
`anakot config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `anakot tools` (interactive) or `anakot tools enable/disable NAME`.
Full enumeration: `TOOLSETS` dict in `toolsets.py` (`_ANAKOT_CORE_TOOLS` is the default bundle most platforms inherit).
