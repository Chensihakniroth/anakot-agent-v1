# Themes / Skins — Author a Anakot Color Theme

Author a Anakot **skin** — one YAML file that themes the CLI, the TUI, and the
desktop GUI at once. The skin engine (`anakot_cli/skin_engine.py`) resolves the
active skin and the gateway pushes it to every surface, so a file dropped in
`~/.anakot/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Anakot itself.
...
- Write access to the Anakot home dir — `~/.anakot` by default, or `$ANAKOT_HOME`
  / the active profile's dir. Skins live in `<anakot-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `anakot config set`).
...
3. `write_file` it to `<anakot-home>/skins/<name>.yaml`.
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `anakot config set display.skin default` (or they can `/skin default`).
...
```
anakot skin set <key> <hex>      # e.g. anakot skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `anakot skin set background "#08201f"` changes
only the background; `anakot skin use <name>` / `anakot skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.anakot`** when a profile is active — resolve the real home
  from `$ANAKOT_HOME` first, falling back to `~/.anakot`.
...
- **Never hand-edit `config.yaml` to activate.** Use `anakot config set
...
- **You apply it, not the user.** `anakot config set display.skin <name>` is
...
- `read_file` the written `<anakot-home>/skins/<name>.yaml` and confirm valid
  YAML with the intended `name` and `colors`.
- Run `anakot config get display.skin` and confirm it reports `<name>`.
