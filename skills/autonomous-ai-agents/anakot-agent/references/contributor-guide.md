...
For occasional contributors and PR authors. Full developer docs: https://anakot-agent.nousresearch.com/docs/developer-guide/
...
Config: `~/.anakot/config.yaml` (settings), `~/.anakot/.env` (API keys) — both under `$ANAKOT_HOME` when it is set.
...
`_ANAKOT_CORE_TOOLS` (every platform) or to a specific toolset.

All handlers must return JSON strings. Use `get_anakot_home()` for paths,
never hardcode `~/.anakot`. For custom/local-only tools, write a plugin in
`~/.anakot/plugins/` instead of editing core — see the developer docs.

### Adding a Slash Command

1. Add `CommandDef` to `COMMAND_REGISTRY` in `anakot_cli/commands.py`
...
- Tests auto-redirect `ANAKOT_HOME` to temp dirs — never touch real `~/.anakot/`.
...
- POSIX file modes (0o600, etc.) → `@pytest.mark.skipif(sys.platform.startswith("win"), reason="POSIX mode bits not enforced on Windows")` (see `tests/anakot_cli/test_auth_toctou_file_modes.py`)
...
- Use `get_anakot_home()` from `anakot_constants` for all paths (profile-safe)
