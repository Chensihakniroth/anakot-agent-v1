...
Author widget apps for the Anakot TUI (`anakot --tui`): glanceable ambient
...
- The TUI must be in use (`anakot --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.anakot/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.anakot/logs/tui_gateway_crash.log` if a widget never appears.
