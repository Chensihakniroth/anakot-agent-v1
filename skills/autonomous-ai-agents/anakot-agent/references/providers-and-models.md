...
Set via `anakot model` (picker) or `anakot setup`. 35+ provider profiles ship as
plugins under `plugins/model-providers/`; user plugins of the same name override.
Full docs: https://anakot-agent.nousresearch.com/docs/integrations/providers
...
| nous | OAuth device code | `anakot auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `anakot auth add openai-codex` |
| qwen-oauth | OAuth | `anakot auth add qwen-oauth` |
| minimax-oauth | OAuth | `anakot auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`anakot auth`).
Fallback chain when the primary fails: `anakot fallback add|remove|list`.

### User-defined model aliases

Work with `/model <name>` in CLI and every gateway platform. Resolved by
`anakot_cli/model_switch.py::resolve_alias()`; user aliases are checked BEFORE
...
#   anakot config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
