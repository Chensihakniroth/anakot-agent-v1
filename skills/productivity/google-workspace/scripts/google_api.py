#!/usr/bin/env python3
"""Google Workspace API CLI for Anakot Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Anakot-facing JSON contract and falls back to the Python client
...
from _anakot_home import get_anakot_home
...
def _gws_binary() -> str | None:
    override = os.getenv("ANAKOT_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Anakot Agent")
