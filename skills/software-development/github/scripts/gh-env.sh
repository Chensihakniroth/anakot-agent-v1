#!/usr/bin/env bash
# GitHub environment detection helper for Anakot Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _anakot_env="${ANAKOT_HOME:-$HOME/.anakot}/.env"; [ -f "$_anakot_env" ] && grep -q "^GITHUB_TOKEN=" "$_anakot_env" 2>/dev/null; then
