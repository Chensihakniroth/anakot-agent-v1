"""GPT-Live voice chat mode: the full-duplex voice frontend that delegates to Anakot.
...
delegates every real request to Anakot as its *client-delegation* backend. Anakot stays the
...
* the renderer turns each ``session.delegation.created`` into a normal ``prompt.submit`` on the
  ...
  ``session.commentary.append`` — Anakot' answer is what the voice speaks.
...
# backend (Anakot) carries the real instructions, tools and memory.
LIVE_PERSONA = (
    "You are Anakot, a calm and friendly voice assistant. Speak naturally at an unhurried pace. "
    ...
    "Delegation policy:\n"
    "Backend tools:\n"
    "- anakot agent: a full AI agent with tools — it can run commands, read and edit files, "
