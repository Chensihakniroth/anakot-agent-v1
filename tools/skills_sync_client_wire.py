...
Independent of local skill state (no ``~/.anakot`` reads); ``tools/skills_sync_client.py``
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if ver.split(".", 1)[0] != WIRE_VERSION:
        raise SyncError(f"this server speaks sync version {ver!r}, but this Anakot speaks "
                        f"{WIRE_VERSION} — update Anakot to sync with it")
