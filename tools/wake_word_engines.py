...
class _OpenWakeWordEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        # Default (or explicit "hey_anakot") → the bundled model; built-in names / paths as-is.
...
# GigaSpeech), downloaded once under ANAKOT_HOME. Keywords are tokenized at RUNTIME.
...
def _sherpa_model_root() -> Path:
    ...
    return get_anakot_home() / "cache" / "wakewords"
...
class _SherpaKwsEngine(_Engine):
    ...
    def _build(self, cfg, sub, ww) -> None:
        ...
        phrase = str(ww._get(cfg, "phrase") or "hey anakot").strip()
        ...
        kw = tempfile.NamedTemporaryFile(mode="w", suffix=".txt", prefix="anakot-kws-", delete=False,
                                         encoding="utf-8")
