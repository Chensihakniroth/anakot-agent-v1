...
def _get_mcp_stderr_log() -> Any:
    ...
    from anakot_constants import get_anakot_home, anakot_home_key, mkdir_under_anakot_home
    ...
    with _mcp_stderr_log_lock:
        ...
        if fh is None or fh.closed:
            try:
                ...
                mkdir_under_anakot_home(log_dir)
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    from anakot_cli.env_loader import secret_source_names
    env = {
    ...
    for key in secret_source_names():
        ...
        if value is not None:
            env[key] = value
    for key in ("ANAKOT_KANBAN_DB", "ANAKOT_KANBAN_BOARD"):
...
def _node_fallback(command: str, *, windows: Optional[bool] = None) -> str:
    ...
    The managed tree comes from ``iter_anakot_node_dirs`` (Windows unpacks into ``<home>\\node``, POSIX into
    ``<home>/node/bin``) under the active profile's ``get_anakot_home()``; on Windows the real files are
    ...
    # /usr/local/bin: canonical Node location (from-source Linux, Anakot Docker image, Intel Homebrew),
    # needed when a hand-authored env.PATH omits it — npx's shebang re-execs /usr/bin/env node.
    directories = [*map(str, iter_anakot_node_dirs(get_anakot_home())), os.path.join(home, ".local", "bin"),
...
def _npx_bin_candidates(bin_dir: str, name: str, *, windows: Optional[bool] = None) -> list:
    ...
    extension (same precedence as ``anakot_constants._candidate_node_command_names``). ``windows``
...
def _npx_cached_bin(args: list) -> Optional[tuple]:
    ...
    for nothing (~48 MB private memory per MCP server, measured); Anakot already supervises the
