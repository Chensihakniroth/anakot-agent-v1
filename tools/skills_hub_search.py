"""Skills Hub discovery: the centralized Anakot index fetch (cached, stale-
...
from tools.skills_hub_official import AnakotIndexSource, OptionalSkillSource
...
def _anakot_index_cache_file() -> Path:
    from tools.skills_hub import _index_cache_dir
    return _index_cache_dir() / "anakot-index.json"


def _load_anakot_index() -> Optional[dict]:
    """Fetch the centralized skills index (docs site, rebuilt daily), cached
    locally for ANAKOT_INDEX_TTL; on any failure serve the stale cache.
    ...
    cache_file = _anakot_index_cache_file()
    cached = _read_json_if_fresh(cache_file, ANAKOT_INDEX_TTL)
    if cached is not None:
        return cached
    data = None
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            resp = httpx.get(ANAKOT_INDEX_URL, timeout=15, follow_redirects=True,
                             headers={"Accept-Encoding": accept_encoding})
            if resp.status_code != 200:
                logger.debug("Anakot index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
        except httpx.DecodingError as e:
            logger.debug("Anakot index decode failed (Accept-Encoding=%s): %s", accept_encoding, e)
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Anakot index fetch failed: %s", e)
            return _load_stale_index_cache()
...
def _load_stale_index_cache() -> Optional[dict]:
    ...
    from tools.skills_hub import _read_json_if_fresh
    return _read_json_if_fresh(_anakot_index_cache_file(), float("inf"))
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    return [
        ...
        AnakotIndexSource(auth=auth),     # centralized index (search + resolved install paths)
...
def _search_one_source(
    ...
    try:
        ...
        if provider_filter and isinstance(src, (AnakotIndexSource, GitHubSource)):
...
def _select_active_sources(sources: List[SkillSource], source_filter: str) -> List[SkillSource]:
    ...
    index_available = effective == "all" and any(
        src.source_id() == "anakot-index" and getattr(src, "is_available", False) for src in sources
    )
...
def _index_miss_fallback_sources(
    ...
    """
    if not query.strip() or provider_filter or not any(src.source_id() == "anakot-index" for src in active):
        return []
    if source_counts.get("anakot-index") != 0:
        return []
