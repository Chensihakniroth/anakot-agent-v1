...
def _check_frontmatter(frontmatter: Dict[str, Any], skill_dir: Optional[Path]) -> Iterator[LintFinding]:
    ...
    anakot_meta = meta.get("anakot") if isinstance(meta, dict) else None
    if not isinstance(anakot_meta, dict):
        yield _warn("missing-metadata", "frontmatter is missing metadata.anakot.{tags, related_skills}.")
    elif "tags" not in anakot_meta:
        yield _warn("missing-metadata", "metadata.anakot.tags is missing.")
    author = str(frontmatter.get("author", ""))
    if author and author.strip().lower() in ("anakot", "agent", "anakot agent") and (
        author != "Anakot Agent"):
        yield _warn("author-caps", f"author '{author}' should be 'Anakot Agent' (proper caps) "
