...
# resolve per call in production (below) because ``get_anakot_home()`` /
...
_anakot_config_resolved: str | None = None
_anakot_config_resolved_loaded = False
_real_anakot_home_cached: str | None = None
_real_anakot_home_loaded = False
...
def _anakot_home_real() -> str:
    from anakot_constants import get_anakot_home
    return os.path.realpath(str(get_anakot_home()))


def _get_anakot_config_resolved() -> str | None:
    """Resolved absolute path of the Anakot config file for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``ANAKOT_HOME`` scope (#107327);
    a test may pin it via ``_anakot_config_resolved`` + ``_anakot_config_resolved_loaded``."""
    if _anakot_config_resolved_loaded:
        return _anakot_config_resolved
    try:
        ...
        # home rather than the active multiplex ``ANAKOT_HOME`` — comparing
        ...
        # ``get_anakot_home()`` key the happy path uses, and substitute no
        # unrelated home if even that is gone (#107327 follow-up; PR #107335).
        try:
            from anakot_constants import get_anakot_home
            return str((Path(str(get_anakot_home())) / "config.yaml").resolve())
        except Exception:
            return None


def _get_real_anakot_home() -> str | None:
    """Realpath of the authoritative Anakot home for the ACTIVE profile.

    Resolved per call so it tracks the per-turn ``ANAKOT_HOME`` scope (#107327);
    a test may pin it via ``_real_anakot_home_cached`` + ``_real_anakot_home_loaded``.
    Consumers exempting a whole TREE want ``_anakot_exempt_homes()``: under a named
    profile this home is ``<root>/profiles/<name>`` and the root is exempt too."""
    if _real_anakot_home_loaded:
        return _real_anakot_home_cached
    try:
        return _anakot_home_real()
    except Exception:
        # Same active-profile binding on the exception path (see
        # ``_get_anakot_config_resolved``): re-derive from ``get_anakot_home()``
        # rather than ``_expand_tilde("~/.anakot")`` so the protected-instruction
        ...
        # the consumer: the ``~/.anakot`` exemption is skipped, so the gate runs.
        try:
            from anakot_constants import get_anakot_home
            return os.path.realpath(str(get_anakot_home()))
        except Exception:
...
def _anakot_exempt_homes() -> tuple[str, ...]:
    """Realpaths of the Anakot home tree(s) the protected-instruction gate must stay out of:
    the ACTIVE profile's home, plus the Anakot ROOT when that home is a named profile
    (``<root>/profiles/<name>``). Exempting only the profile dir left the root's DIRECT files
    (LEDGER.md / MEMORY.md / SOUL.md / AGENTS.md ...) to the ``.anakot`` component rule, which
    gated them like a project-local ``<repo>/.anakot/config.yaml`` — fail-closed headless
    (#110630). They are the agent's own store, governed by their own guards, exactly like
    ``~/.anakot`` under the default profile. The root is added only when the shape really is a
    named profile (``named_profile_home``), so a coincidental ``profiles/`` dir elsewhere never
    exempts its parent; the home comes from the ACTIVE scope, never ``ANAKOT_HOME`` alone."""
    home = _get_real_anakot_home()
    if not home:
        return ()
    try:
        from anakot_constants import named_profile_home
...
def _check_sensitive_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    anakot_config = _get_anakot_config_resolved()
    if anakot_config and anakot_config in candidates:
        return (
            f"Refusing to write to Anakot config file: {filepath}\n"
            "Agent cannot modify security-sensitive configuration. "
            "Edit ~/.anakot/config.yaml directly or use 'anakot config' instead.")
    return None
...
# vector (AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project .anakot tree).
...
def _protected_instruction_reason(filepath: str, task_id: str = "default",
    ...
    # ~/.anakot itself is governed by its own guards (config.yaml hard-block,
    # mirror guard, write_approval); this gate targets PROJECT-LOCAL files only.
    # Must run before the ``.anakot`` component rule, which would match the home.
    # ``_anakot_exempt_homes`` also covers the ROOT when the active home is a named
    # profile, so ~/.anakot/<file> cannot read as project-local ``.anakot`` config.
    for real_home in _anakot_exempt_homes():
    ...
    for candidate in (normalized, resolved):
        ...
        if base_lower in _PROTECTED_INSTRUCTION_BASENAMES or any(
            ...
            return base
        # Project-local .anakot config dirs (<repo>/.anakot/config.yaml) steer
        # behavior too. Only the IMMEDIATE parent counts — matching any ancestor
        # would gate every write inside a checkout living under ~/.anakot.
        parts = candidate.replace("\\", "/").rstrip("/").split("/")
        if len(parts) >= 2 and parts[-2] == ".anakot":
            return candidate
    return None
...
def _get_container_mirror_prefix_for_task(task_id: str = "default") -> str | None:
    """Return the container-side Anakot mirror prefix for persistent Docker file tools."""
    try:
        ...
        if env is not None:
            ...
            return "/root/.anakot" if persistent_docker else None
...
def _check_cross_profile_path(filepath: str, task_id: str = "default") -> str | None:
    ...
    Anakot state (a write the host never reads). Not profile isolation — that guard was
