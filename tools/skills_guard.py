...
_ANAKOT_CONFIG_FILES = r'.anakot/(?:config\.yaml|SOUL\.md)'
...
THREAT_PATTERNS = [
    ...
    (r'\$HOME/.anakot/\.env|\~/.anakot/\.env',
     "anakot_env_access", "critical", "exfiltration", "directly references Anakot secrets file"),
    ...
    # invocation: any client plugin that relays Anakot' secure prompts has to name them, and a bare
    ...
    # quarantine only acts on "dangerous") but high for Anakot/other config (setup docs routinely say
    ...
    (_prose_modify_re(_ANAKOT_CONFIG_FILES),
     "anakot_config_mod", "high", "persistence", "modification language aimed at Anakot configuration files (verify intent)"),
    (_shell_write_re(_ANAKOT_CONFIG_FILES),
     "anakot_config_mod_shell", "critical", "persistence", "shell write (redirect/sed -i/tee/cp/mv) targeting Anakot configuration files"),
    (r'.anakot/config\.yaml|.anakot/SOUL\.md',
     "anakot_config_ref", "low", "persistence", "references Anakot configuration files (informational; only modification intent is scored)"),
...
# `.skillignore` is Anakot-native; `.clawhubignore` is honored for skills published through ClawHub.
