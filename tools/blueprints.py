...
``metadata.anakot.blueprint`` (``schedule`` required; optional ``deliver`` [default "origin"],
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.anakot.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.anakot.blueprint`` is a mapping with a
    ...
    anakot = meta.get("anakot") if isinstance(meta, dict) else None
    blueprint = anakot.get("blueprint") if isinstance(anakot, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.anakot.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    ``metadata.anakot.blueprint`` block) ready for ``anakot skills publish``.
    ...
    frontmatter = {
        ...
        "metadata": {"anakot": {"tags": ["blueprint", "automation"], "blueprint": block}},
    }
