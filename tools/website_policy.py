...
Loads a user-managed website blocklist (``security.website_blocklist`` in ~/.anakot/config.yaml plus
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    default_path = get_anakot_home() / "config.yaml"
    ...
    for shared_file in shared_files:
        ...
        path = path if path.is_absolute() else (get_anakot_home() / path).resolve()
