...
def _anakot_tags(fm: dict) -> Any:
    """``metadata.anakot.tags`` from parsed frontmatter, or ``[]`` (unvalidated type)."""
    metadata = fm.get("metadata", {})
    anakot_meta = metadata.get("anakot", {}) if isinstance(metadata, dict) else None
    return anakot_meta.get("tags", []) if isinstance(anakot_meta, dict) else []
