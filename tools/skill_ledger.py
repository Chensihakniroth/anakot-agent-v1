...
``~/.anakot/skills/.curator_ledger.jsonl`` with before/after file manifests whose
contents are stored content-addressed (sha256-deduped) under
``~/.anakot/.curator_backups/blobs/``. JSONL, not the state DB: durable, greppable,
...
def blobs_dir() -> Path:
    return get_anakot_home() / ".curator_backups" / "blobs"
...
def fill_snapshot_from_curator_backup(
    ...
    for rel, data in extra.items():
        ...
        if not _is_within(skills, dest) or not _is_within(get_anakot_home(), dest):
            continue
...
def _read_ledger(what: str, *, quiet_missing: bool = False) -> Optional[bytes]:
    """Return valid UTF-8 ledger bytes, or warn and fail closed."""
    try:
...
def list_entries(skill: Optional[str] = None, limit: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    raw = _read_ledger("listing empty", quiet_missing=True)
    if raw is None:
        return []
...
def _validate_entry_paths(entry: Dict[str, Any]) -> Optional[str]:
    """Every entry path must be under ANAKOT_HOME — a hand-edited ledger must not
