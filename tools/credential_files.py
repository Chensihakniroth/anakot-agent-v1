...
from anakot_constants import get_anakot_dir, get_anakot_home
...
def _contained_host_path(rel: str, anakot_home: Path, abs_msg: str, traversal_msg: str) -> Optional[Path]:
    """Resolve *rel* under ANAKOT_HOME, refusing absolute paths and escapes."""
    if os.path.isabs(rel):
        logger.warning(abs_msg, rel)
        return None
    host_path = anakot_home / rel
    from tools.path_security import validate_within_dir  # resolves symlinks and ``..`` before checking

    if containment_error := validate_within_dir(host_path, anakot_home):
        logger.warning(traversal_msg, rel, containment_error)
        return None
    return host_path.resolve()


def register_credential_file(relative_path: str, container_base: str = "/root/.anakot") -> bool:
    """Register a ANAKOT_HOME-relative credential file for mounting; True if it exists and was registered.

    Rejects absolute paths and traversal out of ANAKOT_HOME. Containment alone is not
    enough: ANAKOT_HOME holds the MASTER stores (``.env``, ``auth.json``, ``mcp-tokens/``),
    ...
    resolved = _contained_host_path(
        relative_path, get_anakot_home(),
        "credential_files: rejected absolute path %r (must be relative to ANAKOT_HOME)",
    ...
    if not resolved.is_file():
        ...
        return False
    # Master credential stores are never mountable, even though they sit inside ANAKOT_HOME and therefore
...
def register_credential_files(entries: list, container_base: str = "/root/.anakot") -> List[str]:
    ...
    try:
        ...
        for item in cred_files if isinstance(cred_files, list) else []:
            ...
            if resolved_path is not None and resolved_path.is_file():
                result.append(_mount(resolved_path, f"/root/.anakot/{rel}"))
...
def _skill_dir_roots(container_base: str) -> Iterator[Tuple[Path, str]]:
    ...
    skills_dir = get_anakot_home() / "skills"
...
def get_skills_directory_mount(container_base: str = "/root/.anakot") -> list[Dict[str, str]]:
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = _safe_skills_tempdir = Path(tempfile.mkdtemp(prefix="anakot-skills-safe-"))
...
def iter_skills_files(container_base: str = "/root/.anakot") -> List[Dict[str, str]]:
...
# (new_subpath, old_name) pairs matching anakot_constants.get_anakot_dir().
...
def _cache_dir_roots(container_base: str, *, create_missing: bool) -> Iterator[Tuple[Path, str]]:
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_anakot_dir(new_subpath, old_name)
        if not host_dir.is_dir():
            ...
            # get_anakot_dir already picked new-vs-legacy, so this can't shadow a legacy dir.
            try:
                ...
                # bind-mounted dir costs nothing; a missing mount costs the feature. get_anakot_dir()
...
def get_cache_directory_mounts(container_base: str = "/root/.anakot") -> List[Dict[str, str]]:
    """Bind-mount entries for each cache directory (host layout via ``get_anakot_dir``)."""
...
def map_cache_path_to_container(host_path: str, container_base: str = "/root/.anakot") -> Optional[str]:
...
def from_agent_visible_cache_path(container_path: str, container_base: str = "/root/.anakot") -> str:
...
# Backends whose file-sync lands under the remote home: ``~/.anakot`` is
...
def to_agent_visible_cache_path(host_path: str, container_base: str = "/root/.anakot") -> str:
    ...
    ``/root/.anakot``; ssh/daytona/vercel_sandbox under ``~/.anakot``; plugin backends declare
    ...
    * docker / modal — bind-mounted (docker) or per-file-synced (modal) at ``/root/.anakot`` (the
    *container_base* default). * ssh / daytona / vercel_sandbox — file-synced under the remote user's home;
    ``~/.anakot`` is shell-expanded by the remote shell, so tool commands resolve it regardless of the
...
def iter_cache_files(container_base: str = "/root/.anakot") -> List[Dict[str, str]]:
