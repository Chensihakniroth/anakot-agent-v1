"""Central registry for all anakot-agent tools: each tool file calls ``registry.register()``
...
def _discovery_cache_path() -> Optional[Path]:
    ...
    try:
        ...
        return Path(get_anakot_home()) / "cache" / "tool_discovery_cache.json"
    except Exception:
        return None
...
# timescales, so results are cached ~30 s: env-var flips via ``anakot tools`` still land
...
_BROWSER_IDENTITY_KEYS = (
    "ANAKOT_SESSION_ID",
    "ANAKOT_BROWSER_CONTROL_PRINCIPAL",
    "ANAKOT_BROWSER_CONTROL_TRANSPORT_FAMILY")
...
def check_fn_cache_scope() -> Optional[str]:
    ...
    processes keep the process-wide cache; a multiplex gateway installs a Anakot-home override
...
def _core_tools_gated_by(fn: Callable) -> Set[str]:
    """Names of ``_ANAKOT_CORE_TOOLS`` members whose registered ``check_fn`` is *fn*."""
    try:
        from toolsets import _ANAKOT_CORE_TOOLS
        core = frozenset(_ANAKOT_CORE_TOOLS)
    except Exception:
        return set()
...
def invalidate_check_fn_cache() -> None:
    """Drop all cached ``check_fn`` results (after config changes like ``anakot tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        # Plugin overlays keyed by resolved ANAKOT_HOME; a profile sees its overlay first.
    ...
    def _plugin_namespace_of_module(self, module_namespace: str) -> Optional[str]:
        ...
        if module_namespace.startswith("anakot_plugins."):
    ...
    def deregister(self, name: str, *, scope: Optional[str] = None) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # Ownership binds to the plugin package root (``anakot_plugins.{name}``), not
                # the exact module: a submodule's handler is still the package's to remove.
                # A handler defined in ``anakot_plugins.pkg.handlers`` is still owned by the
                # ``anakot_plugins.pkg`` package — exact string equality would wrongly block root-module
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        absent). Probes use the ~30 s TTL cache so ``anakot tools enable`` lands quickly."""
        result = []
