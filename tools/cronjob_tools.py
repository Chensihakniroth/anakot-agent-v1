...
# `cronjob(action="run")` executes in-process (comfortably below ANAKOT_AGENT_TIMEOUT).
# Mirrors the 10s cadence of tools/environments/base.py::touch_activity_if_due (delegate_task's heartbeat
# uses 30s) — comfortably below the 1800s default ANAKOT_AGENT_TIMEOUT. See #76502.
_CRON_RUN_HEARTBEAT_INTERVAL = 10.0
# Hard ceiling: with ANAKOT_CRON_TIMEOUT=0 a truly hung run would otherwise mask the
# gateway watchdog forever; past this the heartbeat stops and the watchdog regains authority.
# The child cron run has its own inactivity watchdog (ANAKOT_CRON_TIMEOUT, default 600s) that bounds a
# wedged job, but with ANAKOT_CRON_TIMEOUT=0 (explicit "unlimited") a truly hung run_one_job would otherwise
...
def _reap_stale_executions(job_name: str) -> None:
    ...
    one-shot `anakot cron run` that died mid-run). The ticker does this at startup; one-shot
    ...
    try:
        ...
        # process -- e.g. a PRIOR one-shot `anakot cron run` invocation whose dispatched runner died with
...
def _try_dispatch_background_run(
    ...
    # Reap BEFORE the async/sync branch: the one-shot `anakot cron run` path returns early
    ...
    # completion, so don't claim-and-dispatch (direct callers like `anakot cron run` exit right after).
    ...
    try:
        ...
        origin_ui_session_id = get_session_env("ANAKOT_UI_SESSION_ID", "") or ""
    except Exception:
        pass
...
def _cronjob_schema_overrides() -> dict:
    ...
    params["properties"]["script"]["description"] = _script_description(display_anakot_home())
...
Jobs run in a fresh session with no current-chat context, so prompts must be self-contained, and the agent's FINAL RESPONSE is what gets delivered — cron runs are autonomous and cannot ask questions. Prefer updating an existing job over creating near-duplicates.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": _script_description("the profile ANAKOT_HOME")
            },
            "monitor": {
                "type": "string",
...
def check_cronjob_requirements() -> bool:
    ...
    return (
        env_var_enabled("ANAKOT_INTERACTIVE")
        or env_var_enabled("ANAKOT_GATEWAY_SESSION")
        or env_var_enabled("ANAKOT_EXEC_ASK")
        or is_truthy_value(get_session_env("ANAKOT_CRON_SESSION", ""))
    )


# Agent-facing arguments forwarded verbatim to cronjob(). model / provider / base_url are
# intentionally NOT here: per-job inference pins are user-owned (dashboard, `anakot cron
