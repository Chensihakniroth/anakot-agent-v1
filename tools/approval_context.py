...
# Anakot session id (observability identity, distinct from the gateway routing session_key), forwarded to approval
...
# os.environ["ANAKOT_INTERACTIVE"] races: one session's `finally` restore can clobber another's set mid-run, dropping
...
_anakot_interactive_ctx: contextvars.ContextVar[str | None] = _ctx("anakot_interactive", None)


def set_anakot_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context instead of mutating os.environ."""
    return _anakot_interactive_ctx.set("1" if interactive else "")


def reset_anakot_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_anakot_interactive_context`."""
    _anakot_interactive_ctx.reset(token)


def _is_interactive_cli() -> bool:
    """True for an interactive CLI/ACP session (contextvar first, env fallback)."""
    ctx_val = _anakot_interactive_ctx.get()
    return is_truthy_value(ctx_val) if ctx_val is not None else env_var_enabled("ANAKOT_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("ANAKOT_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    return _session_env("ANAKOT_SESSION_PLATFORM")


def _is_cron_approval_context() -> bool:
    """True when the current approval decision is running inside cron."""
    return is_truthy_value(_session_env("ANAKOT_CRON_SESSION"))
...
def _is_unattended_platform_approval_context() -> bool:
    ...
    Webhook, msgraph_webhook, and api_server sessions bind ``ANAKOT_SESSION_PLATFORM`` like chat gateways
...
def _is_single_query_approval_context() -> bool:
    """True for a single-query (-q) session: ``anakot chat -q`` exports
    ``ANAKOT_INTERACTIVE=1`` (so sudo password prompts work) but nobody is waiting
    ...
    return is_truthy_value(_session_env("ANAKOT_SINGLE_QUERY_SESSION"))


def _is_gateway_approval_context() -> bool:
    """True inside a gateway/API session that can answer an approval.

    Legacy integrations set ANAKOT_GATEWAY_SESSION; concurrent paths bind
    ANAKOT_SESSION_PLATFORM via contextvars. Cron is NEVER a gateway approval
    ...
    if _is_cron_approval_context() or _is_unattended_platform_approval_context():
        return False
    return env_var_enabled("ANAKOT_GATEWAY_SESSION") or bool(_get_session_platform())
...
def _should_fall_through_to_cli_approval(*, is_cli: bool, approval_callback, notify_cb) -> bool:
    ...
    ``ANAKOT_EXEC_ASK`` (or a platform marker) can leak into an interactive CLI
...
def approval_timeout_notice_kwargs() -> dict:
    ...
    ``90 seconds``) and a tripled ``approvals.timeout`` value the user can paste into ``anakot config set``."""
