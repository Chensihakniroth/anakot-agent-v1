...
under ``ANAKOT_HOME`` and new sandboxes are restored from them on task reuse.
"""
...
def _ensure_vercel_sdk() -> None:
    ...
    # The SDK (>=0.7) ships default-on telemetry; Anakot policy is opt-in only, so disable it
...
def _snapshot_store() -> Path:
    return get_anakot_home() / "vercel_sandbox_snapshots.json"
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _remote_anakot_dir(self) -> str:
        return f"{self._remote_home.rstrip('/')}/.anakot"
    ...
    def _attach_fresh_sandbox(self, requested_cwd: str) -> None:
        ...
        container_base = self._remote_anakot_dir()
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        archive_member = self._remote_anakot_dir().lstrip("/")
        remote_tar = f"/tmp/.anakot_sync.{os.getpid()}.tar"
        sandbox = self._require_sandbox()
        try:
            # --exclude: live sockets cannot be archived ("socket ignored") and must not fail the download.
            self._run_checked(f"tar cf {shlex.quote(remote_tar)} --exclude='*.sock' -C / {shlex.quote(archive_member)}",
                              "bulk download")
