...
_FORCE_SYNC_ENV = "ANAKOT_FORCE_FILE_SYNC"
...
_SYNC_BACK_TEMP_PREFIX = "anakot-sync-back-"
...
def iter_sync_files(container_base: str = "/root/.anakot") -> list[tuple[str, str]]:
    """Enumerate all (host_path, remote_path) pairs to sync to a remote. Credential paths are
    remapped from the hardcoded /root/.anakot to *container_base* (remote home may differ)."""
    ...
    files = [
        (entry["host_path"], entry["container_path"].replace("/root/.anakot", container_base, 1))
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        per ``sync_interval`` unless *force* or ``ANAKOT_FORCE_FILE_SYNC=1``. Transactional:
    ...
    def _sync_transaction(self, *, force: bool = False) -> None:
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="anakot-sync-push-") as staging:
                staged_files = []
                pushed_hashes = {}
    ...
    def sync_back(self, anakot_home: Path | None = None) -> None:
        """Pull remote changes back to the host: download the remote ``.anakot/`` as a tar and
        ...
        with self._transaction_lock:
            self._sync_back_transaction(anakot_home=anakot_home)

    def _sync_back_transaction(self, anakot_home: Path | None = None) -> None:
        ...
        # to avoid retry storms against an uninitialized remote .anakot/.
        if not self._pushed_hashes and not self._synced_files:
            logger.debug("sync_back: no prior push state — skipping")
            return

        lock_path = (anakot_home or get_anakot_home()) / ".sync.lock"
    ...
    def _infer_host_path(self, remote_path: str, file_mapping: list[tuple[str, str]] | None = None, *,
        ...
        substitution (``/root/.anakot/skills/b.md`` -> ``~/.anakot/skills/b.md``)."""
