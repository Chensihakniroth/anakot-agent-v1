...
# (`anakot update` re-runs it in the terminal) and only a short tail is quoted in chat.
_UPDATE_FAILED_NOTICE = (
    "❌ Anakot update failed; the previous version is still running. Run `anakot update` on the "
...
class GatewayNotificationsMixin:
    ...
    class _UpdatePaths:
        """Marker files ``anakot update --gateway`` and its watcher exchange under ANAKOT_HOME."""
    ...
    def _update_paths(cls) -> "GatewayNotificationsMixin._UpdatePaths":
        ...
        return cls._UpdatePaths(
            pending=_anakot_home / ".update_pending.json",
            claimed=_anakot_home / ".update_pending.claimed.json", output=_anakot_home / ".update_output.txt",
            exit_code=_anakot_home / ".update_exit_code",
            prompt=_anakot_home / ".update_prompt.json", response=_anakot_home / ".update_response",
        )
    ...
    async def _watch_update_progress(
        ...
        """Watch ``anakot update --gateway``, streaming output + forwarding prompts.
        ...
        while loop.time() < deadline:
            if paths.exit_code.exists():
                ...
                with _log_suppressed(logging.WARNING, "Update final notification failed: %s"):
                    ...
                    await target.send(
                        "✅ Anakot update finished." if exit_code == 0 else _UPDATE_FAILED_NOTICE
                    )
        ...
        if not paths.exit_code.exists():
            ...
            await _flush_buffer()
            with suppress(Exception):
                await target.send("❌ Anakot update timed out after 30 minutes.")
            self._clear_update_markers(paths, session_key)
    ...
    async def _send_update_notification(self) -> bool:
        ...
        try:
            ...
            if chat_id:
                ...
                if exit_code == 0:
                    msg = "✅ Anakot update finished successfully."
                    if output:
    ...
    async def _send_restart_notification(self) -> Optional[tuple[str, str, Optional[str]]]:
        ...
        from gateway.run import _anakot_home, _non_conversational_metadata
        notify_path = _anakot_home / ".restart_notify.json"
    ...
    def _free_tier_startup_line(self) -> Optional[str]:
        ...
        try:
            ...
            from anakot_cli.anon_auth import guest_carries_inference
    ...
    async def _send_home_channel_startup_notifications(
        ...
        message = "♻️ Gateway online — Anakot is back and ready."
    ...
    async def _send_session_db_warning_notifications(self) -> None:
        ...
        if getattr(self, "_session_db_handle_cache", None) is not None:
            ...
            if not error:
                ...
                return
        from anakot_constants import get_default_anakot_root, profile_cli_selector
        from anakot_state import _default_db_path, classify_persistence_error
        cause = classify_persistence_error(error)
        # Copy-pasteable, so name the real store and pin the profile: a bare `anakot` follows
        # active_profile, which may be a different database (#105887).
        profile_arg = profile_cli_selector()
        if cause == "corrupt":
            db_path = _default_db_path()
            backups_dir = get_default_anakot_root() / "backups"
            message = (
                "⚠️ Session database corruption detected. Messages may not be "
                "persisted. Recovery options:\n"
                f"1. Run `anakot {profile_arg}doctor --fix`\n"
                "2. Stop the gateway, then recover with:\n"
                f"   anakot {profile_arg}sessions recover --source {db_path} "
                "--inspect-only\n"
                f"   (if it reports recoverable) anakot {profile_arg}sessions recover "
                ...
                f"Run `anakot {profile_arg}doctor` for sanitized diagnostics."
            )
            ...
            message = (
                ...
                f"it is repaired: run `anakot {profile_arg}doctor --fix`, then restart the gateway. Do not run "
                f"recovery tools or restore a backup unless `anakot {profile_arg}doctor` confirms damage."
            )
        else:
            ...
            message = (
                ...
                f"empty. Cause: {failure.gloss}. Run `anakot {profile_arg}doctor --fix` on the "
                f"gateway machine, then `anakot {profile_arg}gateway restart`."
            )
    ...
    async def _inject_watch_notification(
        ...
        if not source:
            # API-server sessions bind the RAW X-Anakot-Session-Id key, not a structured ``agent:...`` key.
    ...
    def _completion_event_scope(self, evt: dict):
        ...
        (``tools.async_delegation`` → ``get_anakot_home()/state.db``) resolve from the ambient scope.
        ...
        if get_anakot_home_override() == str(profile_home):
            return contextlib.nullcontext()
