...
Defenses 1-2 (``_ANAKOT_GATEWAY`` guard on ``anakot gateway stop|restart`` /
...
persisted to ``<ANAKOT_HOME>/gateway/restart_loop.json`` and CHAIN while gaps stay
...
def _state_path():
    return get_anakot_home() / "gateway" / "restart_loop.json"
