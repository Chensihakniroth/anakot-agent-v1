...
``<ANAKOT_HOME>/state/gateway.heartbeat`` so supervisors can tell "process alive" from "loop
...
def _mark_exited_quietly(exit_code: int, reason: str) -> None:
    ...
    ``gateway_state.json`` (so ``anakot gateway status`` and every other reader of that file stop
...
def _process_anakot_home() -> Path:
    """ANAKOT_HOME for process-level identity files (ignore profile overrides)."""
    return get_process_anakot_home() if os.environ.get("ANAKOT_HOME", "").strip() else get_anakot_home()


def _home(home: Optional[Path]) -> Path:
    return home if home is not None else _process_anakot_home()
...
def get_loop_tick_socket_path(home: Optional[Path] = None, pid: Optional[int] = None) -> Path:
    """``<ANAKOT_HOME>/state/gateway.loop-tick.<pid>.sock`` — PID-suffixed so a stale node from a
