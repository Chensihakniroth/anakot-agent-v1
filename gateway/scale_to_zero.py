...
SCALE_TO_ZERO_ENV = "ANAKOT_SCALE_TO_ZERO"  # stamped by NAS when the Labs toggle is on
...
def dashboard_client_heartbeat_path(anakot_home: Optional[os.PathLike | str] = None):
    """Path of the dashboard-client liveness marker under ANAKOT_HOME."""
    if anakot_home is None:
        from anakot_constants import get_anakot_home
        anakot_home = get_anakot_home()
    return Path(anakot_home) / DASHBOARD_CLIENT_HEARTBEAT_REL
