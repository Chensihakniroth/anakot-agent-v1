...
# would otherwise each load the same pre-state and the later ``os.replace``
# drops the other key.
_LOCK = threading.Lock()


def _store_path() -> str:
    from anakot_constants import get_anakot_home  # honors the active profile override
    return os.path.join(str(get_anakot_home()), "state", "rich_sent_index.json")
...
async def record_media_async(chat_id, message_id, media: list[tuple[str, str]]) -> None:
    """``record_media`` for coroutine callers; see :func:`record_async`."""
