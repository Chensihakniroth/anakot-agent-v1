...
class PlatformEntry:
    ...
    required_env: list = field(default_factory=list)  # ``anakot setup`` display
    ...
    plugin_name: str = ""  # owning manifest so ``anakot gateway setup`` can auto-enable it
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # Plugin adapters are isolated per resolved ANAKOT_HOME and overlay the
        ...
        # `anakot` invocation, so the import happens only when a lookup asks for it.
