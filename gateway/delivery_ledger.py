...
def record_obligation(*, obligation_id: str, session_key: str, platform: str, chat_id: str,
    ...
    with _DB_LOCK, _transaction() as conn:
        ...
        # Prune on the caller's connection so the obligation and retention sweep commit or roll back together.
...
def _prune_unlocked(conn, now: float) -> None:
    """Run retention deletes on the caller's open transaction connection."""
    conn.execute(
