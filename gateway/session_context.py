"""Session-scoped context variables for the Anakot gateway.

Replaces the old ``os.environ``-based ``ANAKOT_SESSION_*`` state with task-local ``ContextVar``s
...
_SESSION_VARS = (
    ...
    "ANAKOT_SESSION_PLATFORM", "ANAKOT_SESSION_SOURCE", "ANAKOT_SESSION_CHAT_ID",
    "ANAKOT_SESSION_CHAT_TYPE", "ANAKOT_SESSION_CHAT_NAME", "ANAKOT_SESSION_THREAD_ID",
    "ANAKOT_SESSION_USER_ID", "ANAKOT_SESSION_USER_ID_ALT", "ANAKOT_SESSION_USER_NAME",
    "ANAKOT_SESSION_SCOPE_ID", "ANAKOT_SESSION_KEY", "ANAKOT_SESSION_ID",
    "ANAKOT_UI_SESSION_ID", "ANAKOT_SESSION_MESSAGE_ID", "ANAKOT_SESSION_PROFILE",
    "ANAKOT_BROWSER_CONTROL_PRINCIPAL", "ANAKOT_BROWSER_CONTROL_TRANSPORT_FAMILY",
    "ANAKOT_CRON_SESSION", "ANAKOT_SESSION_PARENT_CHAT_ID",
))
...
_SESSION_ASYNC_DELIVERY = ContextVar("ANAKOT_SESSION_ASYNC_DELIVERY", default=_UNSET)
...
_SESSION_HISTORY_DELIVERY = ContextVar("ANAKOT_SESSION_HISTORY_DELIVERY", default=_UNSET)

# Cron auto-delivery vars, set per-job in run_job() so concurrent jobs don't clobber.
_CRON_AUTO_DELIVER_PLATFORM = ContextVar("ANAKOT_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID = ContextVar("ANAKOT_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID = ContextVar("ANAKOT_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)
...
def set_current_session_id(session_id: str) -> None:
    """Synchronize ``ANAKOT_SESSION_ID`` across ContextVar and ``os.environ`` (tools read it
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session var by legacy ``ANAKOT_SESSION_*`` name; drop-in for os.getenv.  The
...
# Surfaces that are not a human chat channel (gateway binds ANAKOT_SESSION_PLATFORM, CLI/TUI/
# desktop bind ANAKOT_SESSION_SOURCE, so both are consulted).  Default-deny: an unrecognized
...
def session_is_messaging_surface() -> bool:
    ...
    ``ANAKOT_PLATFORM``, then the session platform, then the session source)."""
    platform = os.getenv("ANAKOT_PLATFORM") or get_session_env("ANAKOT_SESSION_PLATFORM", "")
    idents = (platform, get_session_env("ANAKOT_SESSION_SOURCE", ""))
...
def declare_stateless_channel() -> None:
    ...
    See Chensihakniroth/anakot-agent-v1#53027 and #63142.
    """
...
def async_delivery_supported() -> bool:
    ...
    (``ANAKOT_KANBAN_TASK``: one-shot subprocesses whose parent disappears after the turn)."""
