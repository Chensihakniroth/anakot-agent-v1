...
There are two ways to add a platform to the Anakot gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.anakot/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Anakot code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `anakot gateway status` or `get_connected_platforms()` until the SDK
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `anakot_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Anakot core.
...
```python
"anakot-your-platform": {
    "description": "Your Platform bot toolset",
    "tools": _ANAKOT_CORE_TOOLS,
    "includes": []
},
```

And add it to the `anakot-gateway` composite:

```python
"anakot-gateway": {
    "includes": [..., "anakot-your-platform"]
}
```
...
## 12. Status Display (`anakot_cli/status.py`)
...
## 13. Gateway Setup Wizard (`anakot_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ anakot_cli/ toolsets.py \
  --include="*.py" -l | sort -u
