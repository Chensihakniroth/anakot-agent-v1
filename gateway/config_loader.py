...
``gateway.<key>`` form (what ``anakot config set gateway.<key>`` produces) is consulted only when the
...
#
# Top-level settings are also accepted nested under ``gateway:`` (what ``anakot config set
...
def read_yaml_layers(home: Path) -> dict:
    ...
    # anakot_cli.config.load_config, so managed quick_commands / stt would otherwise be ignored).
