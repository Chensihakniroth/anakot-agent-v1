...
from anakot_state_errors import classify_persistence_error, is_disk_full_error
...
_DOCTOR = "Run `anakot {profile_arg}doctor --fix` to diagnose and repair."

# cause -> (code, gloss, action). "disk" is split by is_disk_full_error at lookup time.
_STORAGE_FAILURES: dict[str, tuple[str, str, str]] = {
    "locked": (
        "storage_locked",
        "the session database is locked by another Anakot process",
        "Wait a moment and try again; if it persists, stop the other Anakot process "
        "(`anakot {profile_arg}gateway stop`).",
    ),
    "disk_full": (
        "disk_full",
        "the disk is full",
    ...
    "corrupt": (
        ...
        _DOCTOR + " Recovery: `anakot {profile_arg}sessions recover --source <state.db> --inspect-only`.",
    ),
    "fts_index": (
        "storage_index_corrupt",
        "the session search index is damaged (the messages themselves are intact)",
        "Run `anakot {profile_arg}doctor --fix` (or `anakot {profile_arg}sessions repair`) to rebuild it.",
    ),
    "replaced": (
        "storage_replaced",
        "the session database file was replaced while Anakot was running",
        "Stop Anakot (`anakot {profile_arg}gateway stop`), run `anakot {profile_arg}doctor`, then start it again.",
    ),
    "deleted_wal": (
        "storage_replaced",
        "the session database file was changed or replaced while Anakot was running",
        "Stop Anakot (`anakot {profile_arg}gateway stop`), run `anakot {profile_arg}doctor`, then start it again.",
    ),
    "compression": (
        "storage_busy",
    ...
    "turn_lease": (
        "storage_busy",
        "another Anakot process took over this session",
