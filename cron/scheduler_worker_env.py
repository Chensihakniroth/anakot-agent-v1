...
``cron.scheduler``, not ``anakot_cli.main``, so nothing bootstraps the gateway's checkout
...
The shared subprocess sanitizer strips Anakot-owned PYTHONPATH entries because user
children must not see our tree. This child IS Anakot, so the pin is applied *after* the
...
def pin_anakot_tree_on_pythonpath(worker_env: dict, repo_root: Path) -> dict:
