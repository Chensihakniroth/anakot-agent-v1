...
Uses a throwaway ANAKOT_HOME so it never touches ~/.anakot. No real names.
"""
...
def _fresh_home():
    """Point ANAKOT_HOME at a throwaway dir BEFORE importing gateway modules
    (mirror.py binds _SESSIONS_INDEX from get_anakot_home() at import time)."""
    d = tempfile.mkdtemp(prefix="cron_inchannel_e2e_")
    os.environ["ANAKOT_HOME"] = d
    return Path(d)


HOME = _fresh_home()

# Import AFTER ANAKOT_HOME is set.
...
# a different get_anakot_home() at import if something imported it earlier).
...
def main():
    ...
    print(f"ANAKOT_HOME (throwaway): {HOME}")
