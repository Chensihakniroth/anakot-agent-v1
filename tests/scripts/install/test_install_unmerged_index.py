...
The ``anakot update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_parking_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m anakot_cli.main gateway run`` — image name
    ``pythonw``, not ``anakot.exe`` — so the ``taskkill /IM anakot.exe`` guard
    ...
    # The anakot.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM anakot.exe' in text
