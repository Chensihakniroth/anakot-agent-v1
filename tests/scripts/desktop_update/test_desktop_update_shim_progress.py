...
def progress(tmp_path):
    ...
    status = tmp_path / "anakot-update-status"
...
def test_every_state_carries_a_clock(progress):
    ...
    for state, message in [
        ...
        ("done", ""),
        ("manual", "Reopen Anakot to finish."),
        ("error", "Update failed."),
    ]:
...
# Stands in for `anakot update`, and reports the stage that was on screen
...
n="$(cat "$ANAKOT_TEST_CALLS" 2>/dev/null || echo 0)"; n=$((n + 1))
printf '%s' "$n" > "$ANAKOT_TEST_CALLS"
for f in "$TMPDIR"/anakot-update-status.[0-9]*; do
  case "$f" in *.tmp) continue ;; esac
  cp "$f" "$ANAKOT_TEST_CAPTURE.$n" 2>/dev/null
done
exit "$(cat "$ANAKOT_TEST_EXITS.$n" 2>/dev/null || echo 0)"
"""
...
def _run_handoff(tmp_path, exits: dict[int, int]) -> list[dict]:
    ...
    install_root = tmp_path / "anakot-agent"
    (install_root / "venv" / "bin").mkdir(parents=True)
    anakot = install_root / "venv" / "bin" / "anakot"
    anakot.write_text(FAKE_ANAKOT)
    anakot.chmod(0o755)
    ...
    env = {
        **os.environ,
        "TMPDIR": str(tmp_path),
        "ANAKOT_TEST_CAPTURE": str(capture),
        "ANAKOT_TEST_CALLS": str(calls),
        "ANAKOT_TEST_EXITS": str(tmp_path / "exits"),
    }
    ...
    result = tmp_path / ".anakot-update-result.json"
...
def test_update_gate_publishes_its_stage_before_running(tmp_path):
    """`anakot update` is the longest wait in the hand-off and the one the
