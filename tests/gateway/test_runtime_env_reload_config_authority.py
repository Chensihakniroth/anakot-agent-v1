...
ANAKOT_MAX_ITERATIONS, but a later per-turn load_dotenv(..., override=True)
can restore a stale .env ANAKOT_MAX_ITERATIONS value before the next turn.
"""
...
def test_reload_runtime_env_preserves_config_terminal_backend(
    ...
    ``anakot config get terminal.backend`` still says local.
    """
    anakot_home = tmp_path / ".anakot"
    anakot_home.mkdir()
    (anakot_home / "config.yaml").write_text(
    ...
    (anakot_home / ".env").write_text("TERMINAL_ENV=docker\n", encoding="utf-8")
