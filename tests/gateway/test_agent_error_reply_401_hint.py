...
sign-in (93889b770da: a bare ``anakot auth add <provider>`` from a named profile re-signs the
...
def test_gateway_401_reply_names_the_provider_and_the_profile(monkeypatch, tmp_path):
    ...
    assert "`anakot -p codex auth add openai-codex --type oauth`" in known, known
    ...
    assert "`anakot -p codex auth add <provider>`" in unknown, unknown
