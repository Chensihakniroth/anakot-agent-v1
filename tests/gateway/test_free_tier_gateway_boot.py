...
process that never runs the bootstrap can never have an identity. `cmd_chat` and `anakot serve` run it;
this file pins that `anakot gateway run` does too, and does it BEFORE any adapter connects, so a fast
...
def test_gateway_bootstrap_seam_calls_the_one_creator(monkeypatch):
    ...
    import anakot_cli.free_tier_bootstrap as ftb
    calls: list[dict] = []
