...
These run without Docker: the shim's ANAKOT_TINI_SHIM_TARGET /
ANAKOT_TINI_SHIM_WRAPPER hooks let us record the argv that would be
handed to /init.
"""
...
def _run_shim(
    ...
    env = os.environ.copy()
    env["ANAKOT_TINI_SHIM_TARGET"] = str(init)
    env["ANAKOT_TINI_SHIM_WRAPPER"] = str(wrapper)
...
def test_shim_script_is_executable_bit_friendly() -> None:
    ...
    assert "ANAKOT_TINI_SHIM_TARGET" in text
