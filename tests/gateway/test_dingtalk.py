...
class TestSend:
    ...
    async def test_send_posts_to_webhook(self):
        ...
        assert payload["markdown"]["title"] == "Anakot"
...
class TestMentionPatterns:
    ...
    def test_pattern_matches_text(self, monkeypatch):
        adapter = _make_gating_adapter(
            monkeypatch, extra={"mention_patterns": ["^anakot"]}
        )
        assert adapter._message_matches_mention_patterns("anakot please help") is True
        assert adapter._message_matches_mention_patterns("please anakot help") is False
