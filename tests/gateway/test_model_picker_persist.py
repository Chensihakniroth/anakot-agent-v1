...
closure the PR changed, against a real temp ``ANAKOT_HOME``.
"""
...
def _stub_picker_dependencies(monkeypatch):
    ...
    monkeypatch.setattr(
        "anakot_cli.model_switch.resolve_display_context_length",
        lambda *a, **k: 272000,
    )
...
def _setup_isolated_home(tmp_path, monkeypatch, model_yaml_value):
    ...
    # save_config writes to ``get_anakot_home() / config.yaml`` — point it here.
    monkeypatch.setattr("anakot_constants.get_anakot_home", lambda: anakot_home)
    monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: anakot_home)
    return cfg_path
...
async def test_multiplex_picker_global_persists_only_named_profile(
    ...
    monkeypatch.setattr(gateway_run, "_anakot_home", default_home)
    _stub_picker_dependencies(monkeypatch)
...
def _make_store_runner(adapter, sessions_dir, monkeypatch):
    ...
    monkeypatch.setattr(anakot_state, "SessionDB", _no_sqlite)
