...
from anakot_cli.heartbeat import HeartbeatManager, HeartbeatState, save_heartbeat, migrate_heartbeat_to_session
...
async def test_watches_read_and_admit_in_each_owner_profile(tmp_path, monkeypatch):
    ...
    class Adapter:
        ...
        async def handle_message(self, event):
            rows.append((event.source.profile, str(get_anakot_home()), event.text))
            self._active_sessions[event.metadata['gateway_session_key']] = True

    runner._delivery_adapter_for = lambda source: Adapter()
    watch = {}
    for name in ('alpha', 'beta'):
        home = tmp_path / '.anakot' / 'profiles' / name
    ...
    with _profile_runtime_scope(tmp_path / '.anakot' / 'profiles' / 'alpha'):
        await runner._heartbeat_poll_once(watch)
    assert {row[0] for row in rows} == {'alpha', 'beta'}
    assert all(row[1] == str(tmp_path / '.anakot' / 'profiles' / row[0]) for row in rows)
    assert len(watch) == 2
