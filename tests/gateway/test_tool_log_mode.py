...
lines to ~/.anakot/logs/tool_calls.log via write_tool_log's rotating handler.
...
async def test_write_tool_log_shares_one_logger_across_turns(tmp_path, monkeypatch):
    ...
    shared = logging.getLogger("anakot.tool_calls")
    ...
    try:
        ...
        assert set(logging.Logger.manager.loggerDict) - before <= {"anakot.tool_calls"}
