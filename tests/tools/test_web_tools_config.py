...
class TestBackendSelection:
    ...
    ``anakot tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from anakot_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.anakot/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.anakot/.env), not bare os.getenv. Parametrized over the four
    ...
    def test_keenable_search_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Keenable-Title"] == "anakot-agent"
    ...
    def test_tavily_request_reads_key_via_get_env_value(self, monkeypatch):
        ...
        with patch(
            ...
            assert headers["X-Client-Name"] == "anakot-agent"
