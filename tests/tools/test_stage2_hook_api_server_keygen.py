...
was gated on ``[ -f "$ANAKOT_HOME/.env" ]`` while the first-boot seed that
...
def _run_keygen(
    ...
    script = (
        ...
        f"{env_setup}"
        f'ANAKOT_HOME="{home}"\n'
        # In tests we run unprivileged; as_anakot is a passthrough then.
        'as_anakot() { "$@"; }\n'
...
def test_keygen_skips_when_container_env_provides_key(
    ...
    Anakot loads $ANAKOT_HOME/.env with override=True, so a key generated
...
def test_dockerignore_keeps_env_example_template() -> None:
    """The first-boot seed copies /opt/anakot/.env.example -> $ANAKOT_HOME/.env.
    ...
    assert verdict == "included", (
        ...
        "seeds $ANAKOT_HOME/.env from it on first boot"
    )
