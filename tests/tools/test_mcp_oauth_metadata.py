...
Covers:
- :class:`AnakotTokenStorage` ``.meta.json`` roundtrip (save / load / remove)
- The production manager provider
  (:class:`tools.mcp_oauth_manager.AnakotMCPOAuthProvider`) restoring metadata
...
from tools.mcp_oauth_manager import _ANAKOT_PROVIDER_CLS
...
# ---------------------------------------------------------------------------
# AnakotTokenStorage metadata roundtrip
# ---------------------------------------------------------------------------
...
class TestMetadataStorage:
    def test_save_and_load_roundtrip(self, tmp_path, monkeypatch):
        ...
        storage = AnakotTokenStorage("example-server")
    ...
    def test_remove_deletes_meta_file(self, tmp_path, monkeypatch):
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
        storage = AnakotTokenStorage("cleanup-server")
...
# ---------------------------------------------------------------------------
# Manager-path provider (AnakotMCPOAuthProvider) — production code path
# ---------------------------------------------------------------------------


def _manager_provider_with_context(storage: AnakotTokenStorage, **context_attrs):
    ...
    provider = _ANAKOT_PROVIDER_CLS.__new__(_ANAKOT_PROVIDER_CLS)
    provider._anakot_server_name = context_attrs.get("server_name", "srv")
    context = MagicMock()
...
class TestManagerOAuthProviderMetadata:
    def test_initialize_restores_metadata_from_disk(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
        storage = AnakotTokenStorage("mgr-srv")
        ...
        with patch.object(
            _ANAKOT_PROVIDER_CLS.__bases__[-1], "_initialize", new=AsyncMock()
        ):
    ...
    def test_async_auth_flow_persists_on_completion(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
        storage = AnakotTokenStorage("flow-srv")
        ...
        with patch.object(
            _ANAKOT_PROVIDER_CLS.__bases__[-1],
            "async_auth_flow",
