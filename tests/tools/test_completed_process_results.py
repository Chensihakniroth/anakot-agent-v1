...
def test_headless_terminal_result_survives_cli_exit(tmp_path):
    ...
    env = {**os.environ, "ANAKOT_HOME": str(home), "HOME": str(tmp_path),
    ...
    def read_result(profile):
        result = subprocess.run([sys.executable, "-c", consumer, process_id],
                                cwd=tmp_path, env={**env, "ANAKOT_HOME": str(profile)},
    ...
    env["ANAKOT_SESSION_ID"] = receipt["parent_session_id"]
...
def test_receipts_are_bounded_redacted_and_session_scoped(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_SESSION_ID", "owner-session")
    sessions = []
    ...
    paths = list((get_anakot_home() / "logs" / "process-results").glob("*.json"))
    assert len(paths) == 2
    ...
    assert not (get_anakot_home() / "logs" / "process-results" / f"{child.id}.json").exists()
