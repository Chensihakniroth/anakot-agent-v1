...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_env._scrub_child_env — broad ANAKOT_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: ANAKOT_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_anakot_allowlist_and_secret_blocks():
    from tools.code_execution_env import _scrub_child_env

    env = {
        # operational allowlist → kept
        "ANAKOT_HOME": "/h", "ANAKOT_PROFILE": "p",
        "ANAKOT_CONFIG": "/c.yaml", "ANAKOT_ENV": "/e",
        "ANAKOT_DELEGATED_CHILD_CONTEXT": "1",
        # other ANAKOT_* → dropped (broad prefix removed)
        "ANAKOT_BASE_URL": "https://x", "ANAKOT_INTERACTIVE": "1",
    ...
    for kept in (
        ...
        "ANAKOT_DELEGATED_CHILD_CONTEXT", "PATH",
    ):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "ANAKOT_BASE_URL", "ANAKOT_INTERACTIVE", "ANAKOT_KANBAN_TASK",
...
def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped ANAKOT_* vars."""
    import logging
