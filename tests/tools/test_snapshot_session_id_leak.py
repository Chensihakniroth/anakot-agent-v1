"""Cross-session ANAKOT_SESSION_ID leak via the shared bash snapshot.
...
``ANAKOT_SESSION_ID`` into the snapshot, so every LATER session ``source``d that
stale value and its ``echo $ANAKOT_SESSION_ID`` reported a FOREIGN session's id
— overriding the correct per-command Popen env injected by
``_inject_session_context_env``.

The fix strips the per-session bridged vars (ANAKOT_SESSION_* / UI /
...
run stamps per command (ANAKOT_DELEGATED_CHILD_CONTEXT, ANAKOT_CRON_SESSION), or
...
def test_export_snippet_shape():
    snippet = _export_dump_excluding_session_vars('"$__anakot_snap_tmp"')
    ...
    assert "${!ANAKOT_SESSION_*}" in snippet
    assert "${!ANAKOT_CRON_AUTO_DELIVER_*}" in snippet
    assert "${!ANAKOT_BROWSER_CONTROL_*}" in snippet
    assert "ANAKOT_UI_SESSION_ID" in snippet
    assert "grep -vE" not in snippet
    assert '"$__anakot_snap_tmp"' in snippet
    ...
    assert snippet.rstrip().endswith('> "$__anakot_snap_tmp"')
    ...
    try:
        def run_as(sid):
            ...
            def worker():
                ...
                out["r"] = env.execute('echo "[$ANAKOT_SESSION_ID]"')
        ...
        if os.path.exists(snap):
            with open(snap) as f:
                assert "ANAKOT_SESSION_ID" not in f.read()
    finally:
        env.cleanup()
...
def test_export_dump_drops_every_bridged_var_and_the_delegation_marker():
    ...
    exports = "; ".join([f'export {n}="x"' for n in scoped] + ['export ANAKOT_HOME="/h"', 'export MYVAR="keep"'])
    out = subprocess.run(
    ...
    assert 'declare -x ANAKOT_HOME="/h"' in out
