...
def test_no_scrub_inherit_profile_home_bridges_context_override(tmp_path):
    ...
    token = set_anakot_home_override(str(tmp_path))
    try:
        env = build_subprocess_env(
    ...
    assert env["ANAKOT_HOME"] == str(tmp_path)
...
def test_e2e_child_sees_anakot_home_and_no_planted_secret(tmp_path, monkeypatch):
    """A real child spawned with a factory-built env must see ANAKOT_HOME
    ...
    code = (
        "import os, json; "
        "print(json.dumps({'home': os.environ.get('ANAKOT_HOME'), "
    ...
    assert result["home"] == str(anakot_home)
...
# ---------------------------------------------------------------------------
# E2E regression (#93082): cron/no_agent children keep bare `anakot` on PATH
# ---------------------------------------------------------------------------


def test_e2e_scrubbed_env_resolves_bare_anakot_under_minimal_parent_path(monkeypatch):
    """Regression for #92998/#93082: a gateway launched by systemd/cron with a
    minimal PATH (no anakot console-script dir) must still hand cron job
    children an env whose PATH resolves bare ``anakot``.
    ...
    bin_dir = local_mod._resolve_anakot_bin_dir()
    if not bin_dir or not os.path.isfile(
        os.path.join(bin_dir, "anakot.exe" if os.name == "nt" else "anakot")
    ):
        pytest.skip("no real anakot console-script install available")

    # Simulate the service-manager minimal PATH: anakot dir absent.
    minimal_path = os.pathsep.join(["/usr/bin", "/bin"])
    monkeypatch.setenv("PATH", minimal_path)
    assert shutil.which("anakot", path=minimal_path) is None

    env = build_subprocess_env(scrub_secrets=True)  # cron _run_job_script path

    resolved = shutil.which("anakot", path=env.get("PATH", ""))
    assert resolved is not None, (
        f"bare 'anakot' must resolve from the child PATH {env.get('PATH')!r}"
    )
