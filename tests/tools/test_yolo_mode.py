"""Tests for --yolo (ANAKOT_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When ANAKOT_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_SESSION_KEY", "test-session")
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With ANAKOT_YOLO_MODE, dangerous commands are auto-approved."""
    ...
    def test_yolo_mode_not_set_by_default(self):
        """ANAKOT_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("ANAKOT_YOLO_MODE") is None or True  # no-op, documents intent
    ...
    def test_false_like_yolo_values_do_not_bypass_dangerous_command(self, monkeypatch, value):
        ...
        monkeypatch.setenv("ANAKOT_YOLO_MODE", value)
        monkeypatch.setenv("ANAKOT_INTERACTIVE", "1")
        monkeypatch.setenv("ANAKOT_SESSION_KEY", "test-session")
