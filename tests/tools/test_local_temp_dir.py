...
Anakot exposes ``terminal.temp_dir`` (mirrored to ``TERMINAL_TEMP_DIR``) so
...
def test_default_is_anakot_cache_not_tmp(tmp_path, monkeypatch):
    """With no overrides at all, the default temp root is real storage under
    ANAKOT_HOME (cache/terminal), NOT tmpfs /tmp."""
    import anakot_constants  # noqa: F401 — resolves ANAKOT_HOME per call
    ...
    assert result == str(tmp_path / ".anakot" / "cache" / "terminal")
...
def test_cleanup_terminal_temp_cache(tmp_path, monkeypatch):
    ...
    assert not (root / "anakot_bg_dead1.log").exists()
