...
Older images made /opt/anakot/ui-tui and /opt/anakot/node_modules writable so a
...
def test_dockerfile_uses_prebuilt_tui_instead_of_writable_node_modules() -> None:
    ...
    assert "ENV ANAKOT_TUI_DIR=/opt/anakot/ui-tui" in text
    assert "cd ../ui-tui && npm run build" in text
    assert "chown -R anakot:anakot /opt/anakot/ui-tui" not in text
    assert "chown -R anakot:anakot /opt/anakot/node_modules" not in text
