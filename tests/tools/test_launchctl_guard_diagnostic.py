...
def test_interpreter_kill_rejection_names_the_owned_process_route(tmp_path, monkeypatch):
    """Scheduled-Task topology (#113667): only ``ANAKOT_SUPERVISED_CHILD`` marks the launch, and an
    ...
    monkeypatch.setenv("ANAKOT_SUPERVISED_CHILD", "1")
    for marker in ("INVOCATION_ID", "XPC_SERVICE_NAME", "ANAKOT_S6_SUPERVISED_CHILD", "ANAKOT_GATEWAY_EXTERNAL_SUPERVISOR"):
    ...
    # Absence on the wrong side: a plain foreground `anakot gateway run` carries no launch marker.
    monkeypatch.delenv("ANAKOT_SUPERVISED_CHILD")
