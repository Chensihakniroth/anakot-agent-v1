...
def clean_env(monkeypatch):
    ...
    monkeypatch.delenv("ANAKOT_LOCAL_STT_COMMAND", raising=False)
    monkeypatch.delenv("ANAKOT_LOCAL_STT_LANGUAGE", raising=False)
...
class TestTranscribeLocalCommand:
    def test_command_provider_uses_sanitized_child_env(self, monkeypatch):
        """Salvage of #56332: command STT must not inherit Anakot secrets."""
    ...
    def test_command_fallback_with_template(self, monkeypatch, sample_ogg, tmp_path):
        ...
        monkeypatch.setenv("ANAKOT_LOCAL_STT_LANGUAGE", "en")
...
def mock_xai_http_module():
    ...
    fake_module = MagicMock()
    fake_module.anakot_xai_user_agent = MagicMock(return_value="anakot-xai/test")
...
class TestTranscribeXAI:
    ...
    def test_sends_language_and_format(self, monkeypatch, sample_ogg, mock_xai_http_module):
        ...
        monkeypatch.setenv("ANAKOT_LOCAL_STT_LANGUAGE", "fr")
...
class TestCafConversion:
    ...
    def test_caf_conversion_preserves_neighbors_and_removes_owned_output(
        ...
        """Cloud CAF conversion must not clobber a sibling WAV or leave output behind."""
...
class TestExplicitOpenaiSelectionError:
    ...
    with the ``anakot tools`` remediation for managed-Nous users), the old
    ...
    def test_dispatch_returns_selection_specific_error(self, monkeypatch):
        ...
        its anakot tools remediation instead of the all-provider install
        hint."""
        self._no_openai_credentials(monkeypatch)
        monkeypatch.setattr(
            "tools.tool_backend_helpers.managed_nous_tools_enabled", lambda: True
        ...
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
             ...
             patch(
                 ...
                 lambda what: f"managed route down for {what}; run `anakot tools`",
             ):
        ...
        assert "anakot tools" in result["error"]
