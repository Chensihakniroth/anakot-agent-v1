...
def test_output_path_rejects_anakot_oauth_store(tmp_path, monkeypatch):
    """TTS output_path must not bypass the shared protected-file write guard."""
    import agent.file_safety as file_safety

    anakot_home = tmp_path / "anakot-home"
    anakot_home.mkdir()
    monkeypatch.setattr(file_safety, "_anakot_home_path", lambda: anakot_home)
    monkeypatch.setattr(file_safety, "_anakot_root_path", lambda: anakot_home)

    target = anakot_home / ".anthropic_oauth.json"
    result = json.loads(text_to_speech_tool(
        text="hello",
...
def test_output_path_rejects_mcp_token_directory(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / "anakot-home"
    token_dir = anakot_home / "mcp-tokens"
