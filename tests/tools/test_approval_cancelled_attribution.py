...
def gateway_session(monkeypatch):
    ...
    for k in ("ANAKOT_CRON_SESSION", "ANAKOT_YOLO_MODE", "ANAKOT_INTERACTIVE"):
        monkeypatch.delenv(k, raising=False)
    monkeypatch.setenv("ANAKOT_GATEWAY_SESSION", "1")
    monkeypatch.setenv("ANAKOT_SESSION_KEY", SESSION_KEY)
    # ``--yolo`` is frozen at import from ANAKOT_YOLO_MODE; a host shell running yolo must not
...
def cli_session(monkeypatch):
    mod._session_approved.clear()
    for k in ("ANAKOT_CRON_SESSION", "ANAKOT_YOLO_MODE", "ANAKOT_GATEWAY_SESSION", "ANAKOT_EXEC_ASK"):
        monkeypatch.delenv(k, raising=False)
