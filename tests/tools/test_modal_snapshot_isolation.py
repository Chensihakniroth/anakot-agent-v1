...
def _restore_tool_modules():
    original_anakot_home = os.environ.get("ANAKOT_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_anakot_home is None:
            os.environ.pop("ANAKOT_HOME", None)
        else:
            os.environ["ANAKOT_HOME"] = original_anakot_home
        _reset_modules(("tools", "anakot_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    anakot_cli = types.ModuleType("anakot_cli")
    anakot_cli.__path__ = [str(REPO_ROOT / "anakot_cli")]  # type: ignore[attr-defined]
    sys.modules["anakot_cli"] = anakot_cli
    anakot_home = tmp_path / "anakot-home"
    os.environ["ANAKOT_HOME"] = str(anakot_home)
    sys.modules["anakot_cli.config"] = types.SimpleNamespace(
        get_anakot_home=lambda: anakot_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="anakot-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": anakot_home / "modal_snapshots.json",
