...
def _restore_tool_and_agent_modules():
    ...
    original_modules = {
        ...
        if name in {"tools", "agent", "anakot_cli"}
    ...
    try:
        ...
        _reset_modules(("tools", "agent", "anakot_cli"))
...
def _install_fake_tools_package(*, credential_mounts=None):
    ...
    anakot_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["anakot_cli"] = anakot_cli
    sys.modules["anakot_cli.config"] = types.SimpleNamespace(
        get_anakot_home=lambda: Path(tempfile.gettempdir()) / "anakot-home",
    )
...
def test_managed_modal_rejects_host_credential_passthrough():
    _install_fake_tools_package(
        credential_mounts=[{
            ...
            "container_path": "/root/.anakot/token.json",
        }]
    )
    managed_modal = _load_tool_module("tools.environments.managed_modal", "environments/managed_modal.py")
