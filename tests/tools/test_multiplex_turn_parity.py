...
(``ANAKOT_HOME=<profile>``) would — never the launch profile's values frozen into process caches or
...
def two_homes(tmp_path, monkeypatch):
    """Launch home A (ANAKOT_HOME) and routed profile B, differing in every setting under test."""
    a = tmp_path / ".anakot"
    b = a / "profiles" / "b"
...
def test_routed_local_profile_cwd_matches_standalone_gateway(tmp_path, two_homes):
    ...
    wherever ``anakot gateway`` happened to be launched from."""
...
def test_terminal_backend_consumers_read_the_routed_scope(two_homes):
    ...
    def observe():
        return (
            ...
            credential_files.to_agent_visible_cache_path("/host/.anakot/x.png", "/root/.anakot"),
            sorted(Path(m["host_path"]).name for m in credential_files.get_credential_file_mounts()),
        )

    with _profile_runtime_scope(a):
        assert observe() == (True, None, "/host/.anakot/x.png", ["cred_a.txt"])
    with _profile_runtime_scope(b):
        local, cache_base, translated, mounts = observe()
    assert local is False
    assert cache_base == "/root/.anakot"
    assert translated != "/host/.anakot/x.png" or mounts == ["cred_b.txt"]
