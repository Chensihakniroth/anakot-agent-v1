...
from tools.skills_hub_search import (
    ANAKOT_INDEX_TTL, _load_anakot_index, create_source_router, parallel_search_sources, unified_search,
)
...
class TestParseFrontmatterQuick:
    def test_valid_frontmatter_including_nested_yaml(self):
        ...
        nested = "---\nname: test\nmetadata:\n  anakot:\n    tags: [a, b]\n---\n\nBody.\n"
        assert GitHubSource._parse_frontmatter_quick(nested)["metadata"]["anakot"]["tags"] == ["a", "b"]
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `anakot skills browse`."
            )
...
def _make_index_source(skills):
    """Build a AnakotIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub_official import AnakotIndexSource
    src = AnakotIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestAnakotIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "anakot-index"
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "Chensihakniroth/anakot-agent-v1"
...
class TestOptionalSkillSourceLiveRepoFallback:
    ...
    searchable and installable without `anakot update` (live-repo fallback)."""
...
class TestInstallPathSafety:
    ...
    def test_install_from_quarantine_rejects_category_bucket_overwrite(self, tmp_path):
        ...
        Regression test for GitHub issue #75983: ``anakot skills install … --name
...
class TestIndexMissFallback:
    """An available anakot-index stands in for the external registries; when it
    ...
    def _sources(self, index_results):
        index = _FakeSource("anakot-index", results=index_results)
    ...
    def test_index_miss_consults_displaced_registries_but_not_github(self):
        ...
        assert source_counts == {"anakot-index": 0, "skills-sh": 1}
        assert timed_out == []
    ...
    def test_index_hit_leaves_registries_untouched(self):
        index, skills_sh, github = self._sources([self._meta("anakot-index")])

        results, source_counts, _ = parallel_search_sources(
            [index, skills_sh, github], query="humanizar", overall_timeout=5.0)

        assert [r.identifier for r in results] == ["anakot-index/humanizar"]
        assert source_counts == {"anakot-index": 1}
    ...
    def test_provider_filter_miss_skips_registries_without_provider_data(self):
        ...
        assert source_counts == {"anakot-index": 0}
...
# ---------------------------------------------------------------------------
# _load_anakot_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadAnakotIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "anakot-index.json"
        monkeypatch.setattr("tools.skills_hub_search._anakot_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (ANAKOT_INDEX_TTL + 100)
        import os
