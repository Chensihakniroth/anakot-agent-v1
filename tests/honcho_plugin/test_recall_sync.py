...
def test_two_queries_never_consume_previous_query_caches(tmp_path, monkeypatch):
    ...
    assert HonchoClientConfig.from_global_config(host="anakot", config_path=path).recall_sync
    config_provider.save_config({"hosts": {"anakot": {"recallSync": False}}}, str(tmp_path))
    assert not HonchoClientConfig.from_global_config(host="anakot", config_path=path).recall_sync
...
def test_missing_peer_notice_surfaces_once():
    ...
    assert "anakot honcho peer --user" in first
