"""Shared fixtures for the anakot-agent test suite.
...
2. **Isolated ANAKOT_HOME.** ANAKOT_HOME points to a per-test tempdir so
   code reading ``~/.anakot/*`` via ``get_anakot_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".anakot"`` instead of the canonical
   ``get_anakot_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No ANAKOT_SESSION_* inheritance** — the agent's current gateway
...
# ── Sandbox ANAKOT_HOME before ANY test module is imported ──────────────────
# `anakot_cli/main.py` calls `setup_logging()` at MODULE level, which resolves
# `get_anakot_home()` and attaches rotating file handlers to the ROOT logger.
...
# real `~/.anakot/logs/agent.log` and `errors.log`.
#
# The `_isolate_env` fixture below also sandboxes ANAKOT_HOME, but fixtures run
...
# the REAL Anakot root — capture it BEFORE the sandbox rewires ANAKOT_HOME,
# otherwise the deny-list would point at the throwaway tempdir and the guard
# would silently stop protecting the operator's actual ~/.anakot (#69385).
_PRE_SANDBOX_KANBAN_OVERRIDE = os.environ.get("ANAKOT_KANBAN_HOME", "").strip()
_PRE_SANDBOX_ANAKOT_HOME = os.environ.get("ANAKOT_HOME", "")


def _anakot_home_points_at_production(value: str) -> bool:
    """True when a pre-set ANAKOT_HOME resolves to the real production root.

    Gateway-launched shells (and developer shells that ``export
    ANAKOT_HOME=~/.anakot``) hand pytest the PRODUCTION home. Historically
    the session sandbox below honored any pre-set value, so collection-time
    imports (logging handlers, ``anakot_state.DEFAULT_DB_PATH``) froze paths
    inside the real ``~/.anakot`` — the escape vector that landed pytest
    ...
    ANAKOT_HOME is honored now.
    """
    if not value:
        return True
    try:
        # The platform-default root, not a hardcoded ``~/.anakot``: Windows installs live under
        # ``%LOCALAPPDATA%\anakot``, and a dev shell exporting that path used to be honored as
        # "custom", pinning import-time paths (``tui_gateway.server._anakot_home``) to the live
        # install so the state.db guard tripped on every store-touching test (#112692).
        from anakot_state_guard import _real_platform_state_root

        resolved = Path(value).expanduser().resolve()
        real_root = _real_platform_state_root() or (Path.home() / ".anakot").resolve()
    except Exception:
        return True
...
if _anakot_home_points_at_production(os.environ.get("ANAKOT_HOME", "")):
    _SESSION_ANAKOT_HOME = tempfile.mkdtemp(prefix="anakot-test-home-")
    os.environ["ANAKOT_HOME"] = _SESSION_ANAKOT_HOME
    atexit.register(shutil.rmtree, _SESSION_ANAKOT_HOME, True)
...
# like a real CLI") — which used to disarm anakot_state's live-DB guard in
# the child at the same moment the child lost the ANAKOT_HOME redirect.
# ANAKOT_TEST_ISOLATION is OUR marker: exported here (before any test module
# imports), inherited by every child by default, and honored by
# anakot_state_guard._running_under_pytest() as a test-context signal. A child
...
# open a real DB must export ANAKOT_STATE_DB_GUARD_BYPASS=1 in that child's
# env instead of stripping markers.
os.environ["ANAKOT_TEST_ISOLATION"] = os.environ.get("ANAKOT_HOME", "") or "1"

#: ANAKOT_HOME as it stood when conftest was imported - i.e. before any test
...
ANAKOT_HOME_AT_CONFTEST_IMPORT = os.environ.get("ANAKOT_HOME", "")
...
# ANAKOT_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_ANAKOT_BEHAVIORAL_VARS = frozenset({
    ...
    # final response text to ``anakot_cli.voice.speak_text`` — real synthesis,
    ...
    # any test run launched FROM a anakot agent session inherits them and
    # anakot_constants home-resolution helpers prefer them over monkeypatched
    # HOME (test_subprocess_home_isolation red locally, green on CI).
    "ANAKOT_REAL_HOME",
    "TERMINAL_HOME_MODE",
    "ANAKOT_INTERACTIVE",
    "ANAKOT_QUIET",
    "ANAKOT_TOOL_PROGRESS",
    "ANAKOT_TOOL_PROGRESS_MODE",
    "ANAKOT_MAX_ITERATIONS",
    "ANAKOT_SESSION_PLATFORM",
    "ANAKOT_SESSION_CHAT_ID",
    "ANAKOT_SESSION_CHAT_NAME",
    "ANAKOT_SESSION_CHAT_TYPE",
    "ANAKOT_SESSION_THREAD_ID",
    "ANAKOT_SESSION_SOURCE",
    "ANAKOT_SESSION_KEY",
    "ANAKOT_GATEWAY_SESSION",
    "ANAKOT_CRON_SESSION",
    "_ANAKOT_GATEWAY",
    "ANAKOT_PLATFORM",
    "ANAKOT_MODEL",
    "ANAKOT_INFERENCE_MODEL",
    "ANAKOT_INFERENCE_PROVIDER",
    "ANAKOT_TUI_PROVIDER",
    "ANAKOT_MANAGED",
    "ANAKOT_MANAGED_DIR",
    "ANAKOT_DEV",
    "ANAKOT_CONTAINER",
    "ANAKOT_EPHEMERAL_SYSTEM_PROMPT",
    "ANAKOT_TIMEZONE",
    "ANAKOT_REDACT_SECRETS",
    "ANAKOT_BACKGROUND_NOTIFICATIONS",
    "ANAKOT_EXEC_ASK",
    "ANAKOT_HOME_MODE",
    "ANAKOT_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.anakot/kanban.db instead of the per-test ANAKOT_HOME.
    "ANAKOT_KANBAN_DB",
    "ANAKOT_KANBAN_BOARD",
    "ANAKOT_KANBAN_HOME",
    "ANAKOT_KANBAN_WORKSPACES_ROOT",
    "ANAKOT_KANBAN_LOGS_ROOT",
    "ANAKOT_KANBAN_TASK",
    "ANAKOT_KANBAN_WORKSPACE",
    "ANAKOT_KANBAN_RUN_ID",
    "ANAKOT_KANBAN_CLAIM_LOCK",
    "ANAKOT_KANBAN_DISPATCH_IN_GATEWAY",
    ...
    "ANAKOT_DELEGATED_CHILD_CONTEXT",
    "ANAKOT_TENANT",
    ...
    # otherwise-unrelated config tests away from the default "anakot" host.
    "ANAKOT_HONCHO_HOST",
    ...
    "ANAKOT_DASHBOARD_OAUTH_CLIENT_ID",
    "ANAKOT_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and ANAKOT_HOME to per-test tempdirs so code that
    reads ``~/.anakot/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral ANAKOT_* vars that could change test semantics.
    for name in _ANAKOT_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)
    ...
    monkeypatch.setenv("ANAKOT_HONCHO_HOST", "anakot")

    # 3. Redirect ANAKOT_HOME to a per-test tempdir. Code that reads
    #    ``~/.anakot/*`` via ``get_anakot_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.anakot/*`` via
    #    ``Path.home() / ".anakot"`` instead of ``get_anakot_home()``
    #    is a bug to fix at the callsite.
    fake_anakot_home = tmp_path / "anakot_test"
    fake_anakot_home.mkdir()
    (fake_anakot_home / "sessions").mkdir()
    (fake_anakot_home / "cron").mkdir()
    (fake_anakot_home / "memories").mkdir()
    (fake_anakot_home / "skills").mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(fake_anakot_home))
    ...
    # anakot_state's live-DB guard stays armed in them even when the test
    # strips pytest's own PYTEST_* vars from the child env.
    monkeypatch.setenv("ANAKOT_TEST_ISOLATION", str(fake_anakot_home))
    # And never let a developer-shell (or leaked child) bypass disarm the
    # guard for in-process code under test.
    monkeypatch.delenv("ANAKOT_STATE_DB_GUARD_BYPASS", raising=False)

    # 3b. anakot_state computes ``DEFAULT_DB_PATH = get_anakot_home() / "state.db"``
    #     at import time. When the module is first imported at collection (any
    #     test file with a top-level ``from anakot_state import ...``) that
    ...
    anakot_state_mod = sys.modules.get("anakot_state")
    if anakot_state_mod is not None and hasattr(anakot_state_mod, "DEFAULT_DB_PATH"):
        monkeypatch.setattr(
            anakot_state_mod, "DEFAULT_DB_PATH", fake_anakot_home / "state.db"
        )
    ...
    monkeypatch.setenv("ANAKOT_DISABLE_LAZY_INSTALLS", "1")

    # 5. Reset plugin singleton so tests don't leak plugins from
    #    ~/.anakot/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import anakot_cli.plugins as _plugins_mod
        ...
        # previous test's tmp_path ANAKOT_HOME can't leak forward. Paths
...
def _neutralize_kanban_memory_guard(request, monkeypatch):
    ...
    if request.node.get_closest_marker("real_memory_guard"):
        return
    try:
        from anakot_cli import kanban_db_dispatch as _kbd_mod
    except Exception:
        return
...
def _close_leaked_session_dbs():
    ...
    Root cause of OOM incident 20260816: ~40 files under tests/anakot_cli/
    ...
    ``pytest tests/anakot_cli/`` accumulated 16-25 GB RSS and had to be
    OOM-killed three times in one day.

    Rather than editing every test file, ``SessionDB.__init__`` registers each
    instance in ``anakot_state_guard._test_instance_registry`` (a WeakSet,
    populated only when the ``ANAKOT_TEST_ISOLATION`` marker is set — i.e.
    ...
    Instances opened through ``anakot_state_registry.acquire()`` are skipped:
    ...
    yield
    try:
        from anakot_state_guard import _test_instance_registry as registry
    except Exception:
        return
    if not registry:
        return
...
# invocation), kanban writes silently pollute the real ~/.anakot. This autouse
...
# allow-list because test-level fixtures legitimately move ANAKOT_HOME to
...
def _capture_real_kanban_root() -> Path:
    ...
    file (before the session ANAKOT_HOME sandbox rewired the env), so the
    deny-list keeps pointing at the operator's actual root. Mirrors
    ``kanban_db.kanban_home()`` resolution order:
    1. ``ANAKOT_KANBAN_HOME`` env var when set and non-empty
    2. the real (pre-sandbox) Anakot root otherwise
    """
    if _PRE_SANDBOX_KANBAN_OVERRIDE:
        return Path(_PRE_SANDBOX_KANBAN_OVERRIDE).expanduser().resolve()
    if _PRE_SANDBOX_ANAKOT_HOME and not _anakot_home_points_at_production(
        _PRE_SANDBOX_ANAKOT_HOME
    ):
        # ANAKOT_HOME was genuinely set to a CUSTOM root before the sandbox
        ...
        return get_default_anakot_root().resolve()
    # No pre-existing ANAKOT_HOME: the real root is the platform default,
    # NOT the sandbox tempdir now sitting in the env.
    return (Path.home() / ".anakot").resolve()
...
def _kanban_write_guard(_hermetic_environment, monkeypatch):
    ...
    ``~/.anakot`` captured at import time. Hermetic tests that legitimately
    move ANAKOT_HOME to sibling tempdirs are unaffected.

    Only patches when ``anakot_cli.kanban_db_connect`` is *already imported*
    ...
    """
    _kdb = sys.modules.get("anakot_cli.kanban_db")
    _kdbc = sys.modules.get("anakot_cli.kanban_db_connect")
    if _kdb is None or _kdbc is None:
        return

    # The sys.modules probe can observe the module MID-IMPORT: a fixture
    # boundary firing while another test's lazy `import anakot_cli.kanban_db`
    ...
    def _guarded_connect(db_path=None, *args, **kwargs):
        ...
        raise RuntimeError(
            ...
            f"to the real ~/.anakot. See #69283."
        )
...
# ``anakot_state._ensure_test_isolation`` (the single choke point every
# ``SessionDB()`` construction goes through) refuses, under pytest, any DB
# path that resolves inside the REAL Anakot root. This fixture wires the
# test-side knobs:
...
#     installs where ANAKOT_HOME is not ~/.anakot) into the guard's
...
# so subprocess children that import anakot_state directly are covered even
# without this fixture.


@pytest.fixture(autouse=True)
def _state_db_write_guard(request, monkeypatch):
    _hs = sys.modules.get("anakot_state")
    ...
    if _PRE_SANDBOX_ANAKOT_HOME and not _anakot_home_points_at_production(
        ...
        extra_roots.append(
            Path(_PRE_SANDBOX_ANAKOT_HOME).expanduser().resolve()
        )
    monkeypatch.setattr(
...
def _reset_tui_gateway_server_state():
    ...
    # A leaked context-local Anakot home override redirects every later
    # ``get_anakot_home()`` call (active-session registry, config paths)
    # to a stale per-test tmpdir. Force the main-thread ContextVar back
    # to its default.
    try:
        from anakot_constants import get_anakot_home_override, set_anakot_home_override

        if get_anakot_home_override() is not None:
            set_anakot_home_override(None)
    except Exception:
        pass
...
@pytest.fixture()
def mock_config():
    """Return a minimal anakot config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``anakot-gateway`` process
...
# correlated with a ``tests/anakot_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> anakot-gateway``
...
def _wal_is_usable() -> bool:
    """True when Anakot will actually put a database into WAL mode here.

    Anakot refuses journal_mode=WAL on SQLite builds carrying the upstream
    ...
    running Anakot can link DIFFERENT SQLite versions: a repo ``.venv`` on
    3.50.4 (vulnerable → DELETE) alongside a Anakot managed runtime on 3.53.1
    (fixed → WAL). The same test then passes in one and fails in the other.

    IMPORTANT: this must NOT import ``anakot_state``. That module computes
    ``DEFAULT_DB_PATH`` from ``get_anakot_home()`` at import time, so importing
    it during collection — before the per-test ``_isolate_anakot_home`` fixture
    redirects ``ANAKOT_HOME`` — permanently caches the DEVELOPER'S REAL
    ``~/.anakot/state.db`` for the whole session. Tests then read live
    production sessions instead of a tempdir. The version predicate is
    duplicated from ``anakot_state._is_sqlite_wal_reset_vulnerable`` (upstream
...
#      process environment: ``os.environ["ANAKOT_VOICE_TTS"] = "1"``. The
...
#      ``anakot_cli.voice.speak_text(final_response)`` on a daemon thread.
...
# to mock it:
#
#  • ``anakot_cli.voice.speak_text`` — the synth+playback entry point both
#    gateway call sites late-import, so patching the module attribute catches
#    them wherever they import it from.
#  • ``anakot_cli.voice.play_audio_file`` — the module-level binding
#    ``speak_text`` actually plays through. Patching the binding inside
#    ``anakot_cli.voice`` (not ``tools.voice_mode``) keeps the real function
...
# OS gating
#
# Anakot runs on Linux, macOS and native Windows, and a lot of its behaviour
# genuinely differs per host: PTY vs pywinpty, taskkill vs SIGTERM, launchd
# vs systemd, Keychain vs libsecret, ``%LOCALAPPDATA%`` vs ``~/.anakot``.
#
...
def _relocate_basetemp_outside_operator_home(config) -> None:
    """Move pytest's basetemp out of the operator's platform-native Anakot home.

    Every per-test sandbox is ``<basetemp>/.../anakot_test``. ``get_default_anakot_root()``
    prefers the platform-native home whenever ``ANAKOT_HOME`` sits *under* it, so a basetemp
    inside ``~/.anakot`` (or ``%LOCALAPPDATA%\\anakot``, where ``TEMP`` commonly lives on
    ...
    """
    from anakot_constants import _get_platform_default_anakot_home

    native = _get_platform_default_anakot_home().resolve()
    ...
    # Anakot home). The repo is no escape either: the default install checks it
    # out *inside* the home (~/.anakot/anakot-agent). A sibling of the native
    ...
    safe = Path(tempfile.mkdtemp(prefix="anakot-pytest-basetemp-", dir=safe_root))
    assert not safe.resolve().is_relative_to(native), (
        f"pytest basetemp {safe} still resolves inside the operator's Anakot home {native}; "
...
def pytest_configure(config):  # noqa: D401 — pytest hook
    ...
    config.addinivalue_line(
        ...
        "SQLite WAL mode; skipped on builds where Anakot falls back to "
...
def pytest_collection_modifyitems(config, items):  # noqa: D401 — pytest hook
    ...
    reason = (
        f"SQLite {sqlite3.sqlite_version} has the WAL-reset bug — Anakot uses "
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart anakot-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting anakot/python patterns are also blocked.
    """
    ...
    def _matches_anakot_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _ANAKOT_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_anakot_gateway(cmd_str):
            return False
        try:
            tokens = _shlex.split(cmd_str)
        except ValueError:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in killer_tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch anakot-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m anakot_cli.main".
                if (
                    "anakot" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live anakot-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting anakot/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `anakot update` (or the
        # equivalent `python -m anakot_cli.main update`).  These commands
        ...
        if "update" in low and (
            # anakot update / anakot update --gateway / setsid bash -c ... anakot update
            ("anakot" in low and "update" in low.split())
            or
            # python -m anakot_cli.main update --gateway
            ("anakot_cli" in low and "update" in low.split())
            or
            # venv/bin/anakot update  (absolute path variant used in tests)
            (".venv/bin/anakot" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`anakot update` against the real checkout, fetching "
        ...
        # Block spawning a REAL gateway runtime (``python -m anakot_cli.main
        # gateway run|start|restart``). ``_spawn_anakot_action`` launches it
        # with start_new_session=True, so it outlives the pytest worker; the
        # child inherits the pytest-tmp ANAKOT_HOME, resolves the DEVELOPER's
        # ``anakot-gateway`` systemd unit (a tmp home hashes to no profile
        ...
        # A gateway launched INSIDE a container (`docker exec … anakot gateway start`) cannot
        ...
        if (
            ...
            raise RuntimeError(
                ...
                "anakot gateway runtime that outlives the test (it is "
                "detached), restarts the developer's live gateway, and "
                "holds the webhook port. Patch the spawn seam where "
                "production reads it (anakot_cli.web_server_gateway."
                "_spawn_anakot_action), or mark with "
...
def _audio_playback_guard(request, monkeypatch):
    ...
    depth behind ``_ANAKOT_BEHAVIORAL_VARS``: the env blanking stops the flag
    ...
    try:
        import anakot_cli.voice as _voice
    except Exception:
