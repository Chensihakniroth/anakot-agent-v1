...
#              install landed on OLD with a working `anakot`
#   update     advance served main to HEAD, apply ONE update method, assert
#              the checkout landed on HEAD with a working `anakot`
#
# Usage:
#   tests/install/installer-script-e2e.sh --update-method anakot-update|installer-script|installer-script+desktop
...
case "$UPDATE_METHOD" in
  anakot-update|installer-script|installer-script+desktop|anakot-desktop-app-update) ;;
  *) echo "error: --update-method must be anakot-update, installer-script, installer-script+desktop or anakot-desktop-app-update, got '$UPDATE_METHOD'" >&2; exit 1 ;;
esac
...
WORK_ROOT="${RUNNER_TEMP:-${TMPDIR:-/tmp}}/anakot-installer-script-e2e"
LOG_DIR="${ANAKOT_E2E_LOG_DIR:-$WORK_ROOT/logs}"
...
# Isolated HOME: the runner's real one may carry a preinstalled anakot or a
# developer config, and old installer scripts hardcode $HOME/.anakot (the
# ANAKOT_HOME env override is newer than tags we sample). GIT_CONFIG_GLOBAL
# above keeps working -- an explicit path wins over $HOME/.gitconfig.
export HOME="$WORK_ROOT/home"
mkdir -p "$HOME/.local/bin"
export PATH="$HOME/.local/bin:$PATH"
export ANAKOT_HOME="$HOME/.anakot"
mkdir -p "$ANAKOT_HOME"
...
assert_desktop_artifact() {
  ...
  local cand
  for cand in \
    "$release_dir/linux-unpacked/Anakot" \
    "$release_dir/linux-unpacked/anakot" \
    "$release_dir/mac-arm64/Anakot.app" \
    "$release_dir/mac/Anakot.app"; do
...
assert_checkout() {
  ...
  ok "checkout is $2 ($1)"
  local anakot="$INSTALL_DIR/venv/bin/anakot"
  [ -x "$anakot" ] || fail "no anakot console script at $anakot"
  "$anakot" --version 2>&1 | ts_prefix > "$LOG_DIR/version-$2.log" \
    || fail "anakot --version failed after $2; log in $LOG_DIR/version-$2.log"
  ok "anakot --version works: $(head -c 120 "$LOG_DIR/version-$2.log" | tr -d '\n')"
}

smoke_desktop() {
  # $1: label (old|head). Prove the installed CLI can produce the desktop
  # app: `anakot desktop --build-only` runs the full desktop pipeline
  # (workspace install, renderer build, stamp write) and stops before the
  # launch -- the same call `anakot update` itself makes. Probe the
  # INSTALLED anakot for the flag rather than assuming this checkout's
  # surface: sampled OLD releases may predate `anakot desktop` or
  # --build-only entirely, and for them the phase skips, loudly.
  local anakot="$INSTALL_DIR/venv/bin/anakot"
  if ! "$anakot" desktop --help 2>/dev/null | grep -qF -- --build-only; then
    ok "anakot desktop --build-only not supported at $1; skipping desktop smoke"
    return 0
  fi
  local rc=0
  (cd "$INSTALL_DIR" && "$anakot" desktop --build-only < /dev/null 2>&1 \
    | ts_prefix > "$LOG_DIR/desktop-smoke-$1.log") || rc=$?
  log_group "anakot desktop --build-only ($1) transcript" "$LOG_DIR/desktop-smoke-$1.log"
  [ "$rc" -eq 0 ] || fail "anakot desktop --build-only ($1) exited $rc; transcript above"
  ok "anakot desktop --build-only works at $1"
...
case "$UPDATE_METHOD" in
  anakot-update)
    ...
    # installed anakot; older ones read the prompt from stdin, so close it.
    ANAKOT="$INSTALL_DIR/venv/bin/anakot"
    if "$ANAKOT" update --help 2>&1 | grep -qF -- --yes; then
      update_cmd=("$ANAKOT" update --yes)
    else
      update_cmd=("$ANAKOT" update)
    fi
    rc=0
    (cd "$INSTALL_DIR" && "${update_cmd[@]}" < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/update.log") || rc=$?
    log_group "anakot update transcript" "$LOG_DIR/update.log"
    [ "$rc" -eq 0 ] || fail "anakot update exited $rc; transcript above, log at $LOG_DIR/update.log"
    ;;
  installer-script)
  ...
  anakot-desktop-app-update)
    # The real user surface: `anakot desktop` launches the app, the user
    ...
    # suite's own) is configured into ANAKOT_HOME exactly like the dev:mock
    ...
    step "capturing the anakot desktop launch spec (build runs for real)"
    rc=0
    (cd "$INSTALL_DIR" && \
      PYTHONPATH="$ASSETS/launch-capture${PYTHONPATH:+:$PYTHONPATH}" \
      ANAKOT_E2E_CAPTURE_LAUNCH="$SPEC" \
      "$ANAKOT" desktop < /dev/null 2>&1 | ts_prefix > "$LOG_DIR/desktop-launch-capture.log") || rc=$?
    log_group "anakot desktop (launch capture) transcript" "$LOG_DIR/desktop-launch-capture.log"
    [ "$rc" -eq 0 ] || fail "anakot desktop exited $rc during launch capture; transcript above"
    # Exit 0 without a capture means a version that never reached its
    # launch - that must fail loudly, not pass as a no-op.
    [ -f "$SPEC.captured" ] || fail "anakot desktop exited 0 but no launch was captured at $SPEC"
...
mkdir -p "$ildest"
cp -R "$ANAKOT_HOME/logs" "$ildest/anakot-logs" 2>/dev/null || true
if [ -n "${XDG_DATA_HOME:-}" ]; then
  cp -R "$XDG_DATA_HOME/anakot/logs" "$ildest/desktop-userdata-logs" 2>/dev/null || true
fi
cp "$ANAKOT_HOME/.anakot-update-result.json" "$ildest" 2>/dev/null || true
ls -la "$ANAKOT_HOME" > "$ildest/anakot-home-ls.txt" 2>/dev/null || true
