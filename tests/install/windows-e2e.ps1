...
# ============================================================================
# Proves, on a real Windows machine, that a user who installs Anakot the way
...
# actually touches:
#
#   INSTALL   - downloads the production Anakot-Setup.exe from the website,
#               launches it HEADED, and AutoHotkey clicks Install, waits,
#               then clicks Launch. The real Electron Anakot.exe must appear.
...
#   installer's `git clone` and `anakot update`'s `git fetch origin`
...
param(
    ...
    # open-app-update (from a desktop-installer install) and anakot-update /
    ...
    # they land.
    [ValidateSet("open-app-update", "anakot-desktop-app-update", "anakot-update", "desktop-installer@latest", "installer-script", "installer-script+desktop")]
    [string]$Route = "open-app-update",

    # The OLD version: the ref served as `main` while the installer runs,
    # i.e. what the user starts on. The published Anakot-Setup.exe carries
    ...
    [string]$WorkRoot = $(if ($env:ANAKOT_E2E_WORKROOT) { $env:ANAKOT_E2E_WORKROOT } else { Join-Path $env:TEMP "anakot-desktop-gui-e2e" }),

    [string]$SetupExeUrl = "https://anakot-assets.nousresearch.com/Anakot-Setup.exe",
...
$AnakotHome  = Join-Path $WorkRoot "anakot-home"
$InstallDir  = Join-Path $AnakotHome "anakot-agent"
...
function Invoke-Git([string[]]$GitArgs) {
    ...
    # if anakot ever adds a git command that calls something with ^ this will break, lol.
...
function Set-GitRedirect {
    # we redirect to our own repo so we can play around with what commit anakot thinks we're on.
...
function Get-DesktopExe {
    foreach ($c in @(
        (Join-Path $InstallDir "apps\desktop\release\win-unpacked\Anakot.exe"),
        (Join-Path $InstallDir "apps\desktop\release\win-arm64-unpacked\Anakot.exe")
    )) {
...
# Install-side state snapshot, taken BEFORE Test-AnakotRuns can throw: on
...
function Save-InstallSideState([string]$Label) {
    ...
    $logsDir = Join-Path $AnakotHome "logs"
    if (Test-Path -LiteralPath $logsDir) {
        Copy-Item $logsDir (Join-Path $dest "anakot-logs") -Recurse -Force -ErrorAction SilentlyContinue
    }
    $resultFile = Join-Path $AnakotHome ".anakot-update-result.json"
    ...
    Get-ChildItem -LiteralPath $AnakotHome -ErrorAction SilentlyContinue |
        Select-Object Name, Length, LastWriteTime |
        Format-Table -AutoSize | Out-String |
        Set-Content (Join-Path $dest "anakot-home-ls.txt")
}

function Test-AnakotRuns([string]$Label) {
    Save-InstallSideState $Label
    $anakotExe = Join-Path $InstallDir "venv\Scripts\anakot.exe"
    Assert-True (Test-Path -LiteralPath $anakotExe) "$Label -- venv\Scripts\anakot.exe exists"
    & $anakotExe --version 2>&1 | ForEach-Object { Write-Host "    anakot --version| $_" }
    Assert-True ($LASTEXITCODE -eq 0) "$Label -- anakot --version exits 0"
}
...
function Invoke-RefInstaller {
    ...
    $flags = @("-SkipSetup", "-AnakotHome", $AnakotHome, "-InstallDir", $InstallDir)
...
function Invoke-AnakotUpdate {
    # The venv updater. --yes reaches the update subcommand only in later
    # releases; ask the installed binary, never parse its source.
    $anakotExe = Join-Path $InstallDir "venv\Scripts\anakot.exe"
    $updateArgs = @("update")
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    $helpText = & $anakotExe update --help 2>&1 | Out-String
    if ($helpText -match '--yes') { $updateArgs += "--yes" }
    New-Item -ItemType Directory -Path (Join-Path $WorkRoot "logs") -Force | Out-Null
    $log = Join-Path $WorkRoot "logs\update.log"
    Push-Location $InstallDir
    try {
        & $anakotExe @updateArgs 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $updateExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
    }
    Write-LogGroup "anakot update transcript" $log
    Assert-True ($updateExit -eq 0) "anakot update exited $updateExit (expected 0)"
}

function Invoke-AnakotDesktopAppUpdate([string]$TargetSha) {
    # The anakot-desktop launch surface: `anakot desktop` runs its whole
    ...
    $prevCap = $env:ANAKOT_E2E_CAPTURE_LAUNCH
    $env:PYTHONPATH = if ($prevPy) { "$capDir;$prevPy" } else { $capDir }
    $env:ANAKOT_E2E_CAPTURE_LAUNCH = $spec
    $prevEap = $ErrorActionPreference; $ErrorActionPreference = "Continue"
    Push-Location $InstallDir
    try {
        & $anakotExe desktop 2>&1 | Add-TsPrefix | Out-File -Encoding UTF8 $log
        $capExit = $LASTEXITCODE
    } finally {
        Pop-Location
        $ErrorActionPreference = $prevEap
        $env:PYTHONPATH = $prevPy
        $env:ANAKOT_E2E_CAPTURE_LAUNCH = $prevCap
    }
    Write-LogGroup "anakot desktop (launch capture) transcript" $log
    Assert-True ($capExit -eq 0) "anakot desktop exited 0 during launch capture"
    ...
    try {
        & $node "launch-from-spec.mjs" --spec $spec `
            --result (Join-Path $AnakotHome ".anakot-update-result.json") `
    ...
    Assert-True ($driveExit -eq 0) "app driven via captured anakot desktop spec; update completed"
}
...
function Stop-AnakotAppProcesses([string]$Label) {
    # Close the desktop app the blunt way between phases (a user quitting).
    # Only Anakot.exe (Electron) -- never anakot.exe (the venv CLI shim).
    $procs = @(Get-Process -Name "Anakot" -ErrorAction SilentlyContinue)
    foreach ($p in $procs) {
        try { Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue } catch {}
    }
    if ($procs.Count -gt 0) {
        Write-Host "  [$Label] stopped $($procs.Count) Anakot.exe process(es)"
        Start-Sleep -Seconds 3
    }
}

function Get-ManagedNode {
    # `anakot update`/desktop builds use the Anakot-managed Node; use the same
    # one to run the Playwright driver so no system Node is required.
    $candidates = @(
        (Join-Path $AnakotHome "node\node.exe"),
        (Join-Path $AnakotHome "bin\node\node.exe"),
...
# ----------------------------------------------------------------------------
# Phase: install-gui -- website Anakot-Setup.exe, headed, AHK-driven
# ----------------------------------------------------------------------------
function Invoke-PhaseInstallGui {
    ...
    Write-Step "$($Mode.ToUpper()) (GUI): Anakot-Setup.exe from the website, headed, AHK clicks"
    ...
    $setupExe = Join-Path $WorkRoot "Anakot-Setup.exe"
    if (-not (Test-Path -LiteralPath $setupExe)) {
        Write-Host "  downloading $SetupExeUrl"
        Invoke-WebRequest -Uri $SetupExeUrl -OutFile $setupExe
    }
    Assert-True ((Get-Item $setupExe).Length -gt 1MB) "Anakot-Setup.exe downloaded ($([math]::Round((Get-Item $setupExe).Length / 1MB, 1)) MB)"
    ...
    Remove-Item Env:ANAKOT_SETUP_DEV_REPO_ROOT -ErrorAction SilentlyContinue
    ...
    try {
        ...
        Write-Host "  Anakot-Setup.exe launched (pid $($installer.Id))"

        # Drive it: Install click -> wait -> Launch click -> Anakot.exe window.
        ...
        $ahk = Start-Process -FilePath $ahkExe `
            -ArgumentList (Join-Path $AhkDir "install-and-launch.ahk"), $ahkLog, "Anakot-Setup.exe", (Join-Path $AnakotHome "logs\bootstrap-installer.log") `
            -PassThru
        ...
        # actually be running.
        Assert-True ($null -ne (Get-Process -Name "Anakot" -ErrorAction SilentlyContinue)) "Anakot.exe process is running (installer Launch hand-off worked)"

        # Installer should have exited after Launch.
        if (-not $installer.HasExited) {
            Start-Sleep -Seconds 10
        }
        Assert-True $installer.HasExited "Anakot-Setup.exe exited after Launch"
    }
    finally {
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        # Surface the installer's own log win or lose, full and folded.
        $bootLog = Join-Path $AnakotHome "logs\bootstrap-installer.log"
    ...
    Stop-AnakotAppProcesses "post-install"
    ...
    Test-AnakotRuns "post-$Mode-gui"
    Assert-True ($null -ne (Get-DesktopExe)) "packaged Desktop Anakot.exe exists"
...
function Invoke-GuiUpdateDesktopRoute([string]$TargetSha) {
    ...
    Assert-True ($null -ne $desktopExe) "packaged Anakot.exe present before update"
    ...
    try {
        ...
        # `anakot update` + desktop rebuild + relaunch. Which updater depends
        # on the installed checkout, and BOTH are production paths:
        #   * checkouts shipping scripts/desktop-update.ps1 -> that script,
        #     which writes .anakot-update-result.json on every exit;
        #   * older checkouts -> the staged anakot-setup.exe --update flow,
        #     which does NOT write the result JSON.
        # So: poll for COMPLETION = (result JSON) OR (checkout reached the
        # target sha AND the marker is gone). The sha/marker/anakot/relaunch
        ...
        $updateLog = Join-Path $AnakotHome "logs\update.log"
        $updateLogPos = 0
        ...
        Test-AnakotRuns "post-update"
        Assert-True ($null -ne (Get-DesktopExe)) "Anakot.exe still present after update"
        ...
        Write-Host "  waiting for the relaunched Anakot.exe ..."
        $rDeadline = (Get-Date).AddMinutes(5)
        $relaunched = $null
        while ((Get-Date) -lt $rDeadline) {
            $relaunched = Get-Process -Name "Anakot" -ErrorAction SilentlyContinue
            if ($relaunched) { break }
        ...
        # Foreground the relaunched Anakot window so the proof screenshot
        ...
        try {
            $mainProc = Get-Process -Name "Anakot" -ErrorAction SilentlyContinue |
...
'@ -ErrorAction SilentlyContinue
        ...
        Stop-DesktopRecorder $recorder (Join-Path $proof "desktop-frames")
        $handoffLog = Join-Path $AnakotHome "logs\desktop-update-handoff.log"
        ...
        Stop-AnakotAppProcesses "post-update"
    }
}

function Invoke-PhaseInstall {
    # Dispatch on the install axis. Each arm ends with the same contract:
    # checkout at OLD, anakot runs, and state carries how OLD landed so any
    ...
    switch ($InstallMethod) {
        ...
        "installer-script" {
            ...
            Test-AnakotRuns "post-install-script"
        }
        "installer-script+desktop" {
            ...
            Assert-True ((Get-InstalledHead) -eq $state.old) "installed checkout is at OLD"
            Test-AnakotRuns "post-install-script-desktop"
...
function Invoke-PhaseUpdate {
    ...
    New-Item -ItemType File -Path (Join-Path $AnakotHome ".skip_upstream_prompt") -Force | Out-Null
    ...
    switch ($Route) {
        ...
        "anakot-desktop-app-update" {
            Invoke-AnakotDesktopAppUpdate $state.current
        }
        "anakot-update" {
            Invoke-AnakotUpdate
        }
        ...
        "desktop-installer@latest" {
            # A user re-downloading Anakot-Setup.exe and clicking Install over
...
function Invoke-CheckedPhaseUpdate {
    ...
    foreach ($oldLog in @((Join-Path $WorkRoot "logs\update.log"), (Join-Path $AnakotHome "logs\desktop.log"))) {
