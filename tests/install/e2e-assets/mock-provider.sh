...
#   mock_start <workroot>      start, write config into $ANAKOT_HOME
#   mock_stop                  kill the background server
#
# Requires: ASSETS (e2e-assets dir), LOG_DIR, ANAKOT_HOME, ok/fail helpers.
...
mock_start() {
  ...
  cat > "$ANAKOT_HOME/config.yaml" <<EOF
model:
  default: mock-model
  provider: mock
providers:
...
EOF
  ...
  ok "provider 'mock' configured in $ANAKOT_HOME (api $url/v1)"
}
