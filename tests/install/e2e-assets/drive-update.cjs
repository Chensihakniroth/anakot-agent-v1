// drive-update.cjs — launch the INSTALLED Anakot.exe (real Electron desktop
...
//
//   node <this file> <path-to-Anakot.exe> <proof-dir>
//
...
if (!exePath || !proofDir) {
  console.error('usage: node drive-update.cjs <Anakot.exe> <proof-dir>')
  process.exit(1)
}
...
async function main() {
  ...
  const app = await _electron.launch({
    ...
    // Inherit the driver's env: ANAKOT_HOME (isolated install) and
  ...
  // The real backend (`anakot serve`) is booting underneath; give it time.
  ...
  // buttonless boot-progress card ("Starting Anakot... 86%"), then the
  ...
  if (!visible) {
    ...
    const status = await page.evaluate(() =>
      window.anakotDesktop?.updates?.check?.() ?? Promise.resolve('no updates.check bridge')
  ...
  // The "Updating Anakot — this window will close" overlay should appear,
  ...
  // the staged binary), which claims ANAKOT_HOME/.anakot-update-in-progress
  ...
  // proved the desktop had exited and `anakot update` was already running).
  //
  ...
  const anakotHome = process.env.ANAKOT_HOME
  const markerPath = anakotHome ? path.join(anakotHome, '.anakot-update-in-progress') : null
  const resultPath = anakotHome ? path.join(anakotHome, '.anakot-update-result.json') : null
