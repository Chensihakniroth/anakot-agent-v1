...
# (Anakot-Setup.dmg from the website) can reach HEAD.
#
...
#              anakot-desktop-app-update  capture `anakot desktop`'s spawn,
#                                         launch the spec under Playwright,
#                                         click Update now
#              anakot-update              CLI update from the installed venv
...
#     --update-method open-app-update|anakot-desktop-app-update
...
UPDATE_METHOD=""
INSTALL_REF=""
DMG_URL="https://anakot-assets.nousresearch.com/Anakot-Setup.dmg"
...
case "$UPDATE_METHOD" in
  open-app-update|anakot-desktop-app-update|anakot-update|installer-script|installer-script+desktop) ;;
...
WORK_ROOT="${ANAKOT_E2E_WORKROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}/anakot-macos-desktop-e2e}"
...
EOF
  ...
  export INSTALL_DIR="$ANAKOT_HOME/anakot-agent"
}
...
phase_stage() {
  ...
  arm_redirect
  mkdir -p "$ANAKOT_HOME"
  touch "$ANAKOT_HOME/.skip_upstream_prompt"
...
find_installed_app() {
  ...
  local cand
  for cand in \
    "$INSTALL_DIR/apps/desktop/release/mac-arm64/Anakot.app" \
    "$INSTALL_DIR/apps/desktop/release/mac/Anakot.app" \
    "/Applications/Anakot.app"; do
...
phase_install() {
  ...
  . "$STATE"
  arm_redirect
  step "installing OLD ($OLD_REF) via the published Anakot-Setup.dmg"

  local dmg="$WORK_ROOT/Anakot-Setup.dmg"
  ...
  # "Install Anakot" button with native input.
  local rc=0
  ...
  log_group "Anakot-Setup (dmg bootstrap) transcript" "$LOG_DIR/bootstrap-install.log"
  ...
  "$anakot" --version 2>&1 | ts_prefix > "$LOG_DIR/version-old.log" || fail "anakot --version failed after install"
  ok "anakot --version works: $(head -c 120 "$LOG_DIR/version-old.log" | tr -d '\n')"
  find_installed_app >/dev/null || fail "no installed Anakot.app after the dmg bootstrap"
...
phase_update() {
  ...
  case "$UPDATE_METHOD" in
    anakot-update)
      ...
      # update subcommand only in later releases; ask the installed anakot.
      local anakot="$INSTALL_DIR/venv/bin/anakot"
      local update_cmd=("$anakot" update)
      ...
      [ "$rc" -eq 0 ] || fail "anakot update exited $rc; transcript above"
      ;;
    installer-script)
    ...
    installer-script+desktop)
      ...
      for cand in \
        "$INSTALL_DIR/apps/desktop/release/mac-arm64/Anakot.app" \
        "$INSTALL_DIR/apps/desktop/release/mac/Anakot.app"; do
        [ -d "$cand" ] && { head_app="$cand"; break; }
      done
      [ -n "$head_app" ] || fail "no built Anakot.app under the checkout after the +desktop update"
    ...
    open-app-update)
      ...
      # which is exactly what the app's children (git, anakot update) need.
      local app app_bin
...
PYEOF
    ...
    anakot-desktop-app-update)
      ...
      [ "$rc" -eq 0 ] || fail "anakot desktop exited $rc during launch capture"
      [ -f "$spec.captured" ] || fail "anakot desktop exited 0 but no launch was captured"
  ...
  mkdir -p "$ildest"
  cp -R "$HOME_SANDBOX/.anakot/logs" "$ildest/anakot-logs" 2>/dev/null || true
  local ud="$HOME_SANDBOX/Library/Application Support/Anakot"
  ...
  "$INSTALL_DIR/venv/bin/anakot" --version 2>&1 | ts_prefix > "$LOG_DIR/version-head.log" \
    || fail "anakot --version failed after update"
  ok "anakot --version works post-update"
