    ...
    with patch.dict(
        "sys.modules",
        {
            "anakot_constants": MagicMock(
                get_anakot_home=MagicMock(return_value="/tmp/anakot_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'anakot-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'anakot-release' patched"
    }
