...
guard is covered in ``tests/anakot_cli/test_picker_prewarm.py``):

- ``main()`` invokes ``anakot_cli.model_switch_providers.prewarm_picker_cache_async``
...
import anakot_cli.model_switch_providers
...
def _run_main(monkeypatch, events, *, prewarm=None):
    ...
    # entry.main() imports the helper lazily from anakot_cli.model_switch,
