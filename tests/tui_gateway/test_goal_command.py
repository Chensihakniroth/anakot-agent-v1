...
def anakot_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves ANAKOT_HOME.
...
def server(anakot_home, monkeypatch):
    ...
    # Pin config resolution to the isolated ANAKOT_HOME. Sibling test
    ...
    # module-level ``_anakot_home = get_anakot_home()`` snapshot freezes
...
def test_moa_bare_returns_usage(server, session, anakot_home):
    _write_moa_config(anakot_home, """
moa:
...
def test_goal_draft_uses_session_profile_without_blocking_rpc_reader(
    ...
    def draft(objective):
        observed.append(get_anakot_home())
        started.set()
