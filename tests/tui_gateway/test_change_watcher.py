...
Behavior contracts, exercised against a real temp ANAKOT_HOME (no mocks on the
...
def watcher_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_anakot_home", str(tmp_path))
...
def test_served_profile_store_move_broadcasts_sessions_changed(watcher_home, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.profiles.get_profile_dir", lambda name: home / "profiles" / name)
...
def test_pairing_probe_reuses_live_profile_roots_until_the_profile_set_moves(watcher_home, monkeypatch):
    ...
    home, events = watcher_home
    live_calls = []
    real_live = anakot_constants.named_profile_is_live
    monkeypatch.setattr(anakot_constants, "named_profile_is_live",
