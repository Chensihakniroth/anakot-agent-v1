...
def local_route(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("anakot_cli.banner.get_available_skills", lambda: {})
...
def test_session_info_recovers_identity_from_the_owning_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_anakot_home", launch)
    monkeypatch.setattr(server, "_probe_credentials", lambda agent: None)
    monkeypatch.setattr("anakot_cli.banner.get_update_result", lambda **kw: None)
    monkeypatch.setattr("anakot_cli.banner.get_available_skills", lambda: {})
    ...
    with server._profile_build_scope(secondary):
        ...
        assert get_anakot_home() == secondary
    assert get_anakot_home() == launch
