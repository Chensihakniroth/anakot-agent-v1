...
def test_required_frontmatter_fields(p):
    ...
    anakot = (fm.get("metadata") or {}).get("anakot") or {}
    if not (anakot.get("tags") or fm.get("tags")) and not _grandfathered(p, "tags"):
        pytest.fail(f"{_rel(p)}: no tags (metadata.anakot.tags or top-level tags)")
...
def test_related_skills_resolve(p):
    ...
    anakot = (fm.get("metadata") or {}).get("anakot") or {}
    dangling = [
        rs for rs in (anakot.get("related_skills") or []) if rs not in _all_names()
    ]
