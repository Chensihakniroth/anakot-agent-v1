...
work on a file several Anakot processes share (gateway service, the Desktop
app's `anakot serve` backend, CLI sessions, the TUI slash worker):

* `anakot_state_common.fts_rebuild_admission` — full structural FTS rebuilds
* `anakot_state_repair._cross_process_repair_lock`   — writable_schema surgery / VACUUM
