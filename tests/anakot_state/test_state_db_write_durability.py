...
The defect: anakot_state already knows macOS `fsync()` does not guarantee
...
import anakot_state
from anakot_state import repair_state_db_schema
from anakot_state_repair import _connect_repair_durable
...
def test_repair_path_has_no_bare_connects() -> None:
    ...
    """
    # The repair/probe helpers live in anakot_state_repair; anakot_state only
    # re-imports them.
    import anakot_state_repair

    source = Path(anakot_state_repair.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source, filename=str(anakot_state_repair.__file__))
