...
#88532 made SessionStore follow ANAKOT_HOME; this covers the TUI/agent
...
def test_open_profile_session_db_does_not_fallback_on_open_failure(homes, monkeypatch):
    ...
    monkeypatch.setattr("anakot_state.SessionDB", boom_db)
...
def _stub_rebuild_env(monkeypatch, server, launch) -> list[str]:
    """Neutralize the rebuild's side paths; returns the ANAKOT_HOME seen by each _make_agent call."""
    homes_seen: list[str] = []

    def fake_make_agent(_sid, _key, *, session_db=None, **_kw):
        from anakot_constants import get_anakot_home

        homes_seen.append(str(get_anakot_home()))
...
def test_bot_capability_rebuild_stays_on_the_profile_store(homes, monkeypatch):
    ...
    ``_make_agent`` with neither the session's ``state.db`` handle nor its ANAKOT_HOME, so the replacement
    agent bound the launch ``_get_db()`` handle and built its prompt/skills from the launch profile: every
    later turn of a named-profile bot appended to ``~/.anakot/state.db`` under the same session id while the
