...
Anakot instances: a main gateway (user ``ubuntu``,
ANAKOT_HOME=/home/ubuntu/.anakot) and a demo gateway (user ``demo``,
ANAKOT_HOME=/home/demo/.anakot).  The demo gateway runs as another user, so
its ``/proc/<pid>/fd`` table is unreadable from the main instance and the
holder scan falls back to ``/proc/<pid>/cmdline`` + ``_looks_like_anakot``.
The demo process's argv matches the Anakot patterns exactly, so the fallback
...
rebuild in ``anakot_state_schema._recover_stale_fts`` was deferred 42 times
...
anakot); a genuine second instance with a DIFFERENT ANAKOT_HOME is still
misjudged on current main (issue #92401).

Behavior contract: an uninspectable holder identified only by argv must be
counted unless its own argv proves it is scoped to a *different* Anakot
...
# token lives under /home/demo/.anakot, the binary name matches the Anakot
# patterns, and nothing references the main instance's home or state.db.
DEMO_HOME_ARGV = [
    "/home/demo/.anakot/anakot-agent/anakot",
    "gateway",
    "run",
]

# Same, spelled through the venv interpreter + anakot launcher script.
DEMO_VENV_ARGV = [
    "/home/demo/.anakot/anakot-agent/venv/bin/python",
    "/home/demo/.anakot/anakot-agent/anakot_cli/main.py",
    "gateway",
]

# A Anakot-shaped argv with no absolute paths: cannot disprove that this
# process touches our state.db, so it must stay fail-closed.
AMBIGUOUS_ARGV = ["anakot", "gateway", "run"]
...
class TestUninspectableHolderInstanceScope:
    def test_other_instance_argv_is_not_a_holder_of_our_db(self, tmp_path, monkeypatch):
        ...
        DIFFERENT Anakot home → not a holder of our state.db."""
    ...
    def test_argv_referencing_our_db_stays_flagged(self, tmp_path, monkeypatch):
        ...
        for argv in (
            ["anakot", f"--db={db_path}", "gateway"],
            ["anakot", "checkpoint", f"{db_path}-wal"],
            ["anakot", "--home", our_home, "gateway"],
        ):
            assert anakot_state_holders._looks_like_anakot(argv) or argv[0] == "anakot"
            _install_fake_proc(monkeypatch, tmp_path, unreadable_pids=(222,))
