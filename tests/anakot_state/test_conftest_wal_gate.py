"""The conftest WAL gate must agree with anakot_state, and must not import it.

``tests/conftest.py::_wal_is_usable`` duplicates the SQLite WAL-reset version
predicate instead of importing ``anakot_state``. That is deliberate: importing
``anakot_state`` during collection caches ``DEFAULT_DB_PATH`` from the real
``~/.anakot`` before the per-test ``ANAKOT_HOME`` redirect, which makes tests
...
def test_conftest_gate_agrees_with_anakot_state(version_info, monkeypatch):
    ...
    assert _wal_is_usable() is not is_sqlite_wal_reset_vulnerable(version_info), (
        f"conftest gate and anakot_state disagree for SQLite {version_info}"
    )


def test_conftest_does_not_import_anakot_state_at_collection():
    """The gate must stay import-free of anakot_state.

    Importing it during collection caches DEFAULT_DB_PATH from the real
    ~/.anakot, so tests read live production sessions instead of a tempdir.
    Reading the source is not an option here (banned), so assert on behavior:
    the gate must work with ``anakot_state`` absent from ``sys.modules`` and
    ...
    def guard(name, *args, **kwargs):
        if name == "anakot_state" or name.startswith("anakot_state."):
            blocked.append(name)
            raise AssertionError(
                "conftest._wal_is_usable imported anakot_state — this caches "
                "DEFAULT_DB_PATH from the real ~/.anakot during collection"
            )
        return real_import(name, *args, **kwargs)

    saved = sys.modules.pop("anakot_state", None)
    ...
    try:
        ...
        if saved is not None:
            sys.modules["anakot_state"] = saved
    assert not blocked
