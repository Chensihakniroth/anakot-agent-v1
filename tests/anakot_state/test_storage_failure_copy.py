...
@pytest.mark.parametrize(
    ...
    [
        ...
        (sqlite3.OperationalError("attempt to write a readonly database"), "storage_readonly", "anakot doctor --fix"),
        (sqlite3.DatabaseError("database disk image is malformed"), "storage_corrupt", "anakot doctor --fix"),
        (OSError(28, "No space left on device"), "disk_full", "Free some disk space"),
        (None, "storage_unavailable", "anakot doctor --fix"),
    ],
)
...
def test_action_command_is_pinned_to_the_failing_profile(monkeypatch, tmp_path):
    ...
    whose state.db is not the process default; a bare ``anakot`` follows active_profile)."""
    ...
    for exc in (sqlite3.OperationalError("database is locked"), sqlite3.DatabaseError("malformed"), None):
        ...
        assert f"`anakot {selector}" in action and "`anakot doctor" not in action and "`anakot gateway" not in action
