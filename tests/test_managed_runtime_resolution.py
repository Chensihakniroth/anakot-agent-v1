"""Guard: Anakot-owned subprocesses must not resolve managed runtimes by bare PATH.

Anakot installs runtimes for itself — ``uv`` at ``$ANAKOT_HOME/bin/uv``, Node at
``$ANAKOT_HOME/node``. Neither directory is on the ambient PATH of an arbitrary
process, so ``shutil.which("uv")`` / ``shutil.which("node")`` in Anakot's own
...
* when a system copy also exists, the one Anakot does not own wins — which is
...
``iter_anakot_node_dirs()``, ``resolve_uv()``, or ``ensure_uv()``. This test is
...
# Runtimes Anakot provisions into ANAKOT_HOME and must therefore resolve
# through a managed-aware helper rather than PATH.
_MANAGED_COMMANDS = frozenset({"uv", "node", "npm", "npx"})

# Directories that are not Anakot-owned subprocess code: plugins ship their own
...
_ALLOWED: dict[tuple[str, str], str] = {
    ...
    ("anakot_cli/update_cmd_deps.py", "uv"): (
    ...
    ("anakot_cli/update_cmd_deps.py", "npm"): (
...
def _source_files() -> list[Path]:
    ...
    # (anakot_agent-<version>/) gets created and deleted concurrently, and
...
def test_no_unreviewed_bare_managed_runtime_lookups():
    ...
    assert not unexpected, (
        "Bare PATH lookup for a Anakot-managed runtime.\n\n"
        + "\n".join(f"  {rel}:{lineno}  which({cmd!r})" for rel, cmd, lineno in unexpected)
        + "\n\n$ANAKOT_HOME/bin (uv) and $ANAKOT_HOME/node are not on an "
        ...
        "  node/npm -> anakot_constants.find_node_executable()\n"
        "  PATH env -> anakot_constants.iter_anakot_node_dirs()\n"
...
def test_managed_node_helpers_exist(helper):
    ...
    assert callable(getattr(anakot_constants, helper))


def test_managed_uv_helpers_exist():
    from anakot_cli.managed_uv import ensure_uv, managed_uv_path, resolve_uv
