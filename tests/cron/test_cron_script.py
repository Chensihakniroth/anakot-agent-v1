...
- Path resolution (absolute, relative to ANAKOT_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp ANAKOT_HOME."""
    anakot_home = tmp_path / ".anakot"
    anakot_home.mkdir()
    (anakot_home / "cron").mkdir()
    (anakot_home / "cron" / "output").mkdir()
    (anakot_home / "scripts").mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "ANAKOT_DIR", anakot_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", anakot_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", anakot_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", anakot_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_missing_script_names_the_profile_folder(self, cron_env):
        ...
        assert "anakot cron edit" in output


    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Anakot provider env (SECURITY.md §2.3)."""
        from tools.environments.local_env_policy import _ANAKOT_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler_script import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_ANAKOT_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.anakot/scripts/ must be rejected."""
        from cron.scheduler_script import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("ANAKOT_SESSION_PLATFORM") is None
        assert os.environ.get("ANAKOT_SESSION_CHAT_ID") is None
        assert os.environ.get("ANAKOT_SESSION_CHAT_NAME") is None
...
class TestScriptTimeoutTreeKill:
    ...
    def test_timeout_leaves_no_setsid_grandchild(self, cron_env, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_CRON_SCRIPT_TIMEOUT", "2")
