...
def cron_store(tmp_path, monkeypatch):
    ...
    (anakot_home / "cron").mkdir(parents=True)
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.setattr(J, "ANAKOT_DIR", anakot_home)
    monkeypatch.setattr(J, "CRON_DIR", anakot_home / "cron")
    monkeypatch.setattr(J, "JOBS_FILE", anakot_home / "cron" / "jobs.json")
    monkeypatch.setattr(J, "OUTPUT_DIR", anakot_home / "cron" / "output")
...
class TestCronRearmRespectsScheduleLegality:
    def test_weekday_job_errored_friday_does_not_fire_on_saturday(self, cron_store):
        ...
        with mock.patch.object(J, "_anakot_now", lambda: now):
            due = J.get_due_jobs()
