...
normal ``anakot chat -q`` CLI agent (its default toolset includes ``cronjob``)
running with ``ANAKOT_KANBAN_TASK`` legitimately set in its own environment,
...
``$ANAKOT_KANBAN_TASK`` — letting an unrelated cron job close the worker's task
...
def worker_env(monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_KANBAN_TASK", "t_worker_real_task")
    monkeypatch.setenv("ANAKOT_KANBAN_WORKSPACE", "/tmp/ws")
    monkeypatch.setenv("ANAKOT_KANBAN_RUN_ID", "42")
    monkeypatch.setenv("ANAKOT_KANBAN_CLAIM_LOCK", "lock-abc")
    monkeypatch.setenv("ANAKOT_KANBAN_BOARD", "team-alpha")
...
class TestRunJobKanbanIsolation:
        ...
        class FakeAgent:
            def __init__(self, **kwargs):
                ...
                observed["kanban_env_during_init"] = {
                    ...
                    if k.startswith("ANAKOT_KANBAN_")
                }
    ...
    def test_environment_is_left_untouched(self, monkeypatch, worker_env):
        ...
        before = {
            k: v for k, v in os.environ.items() if k.startswith("ANAKOT_KANBAN_")
        }
    ...
    def test_context_reset_even_when_job_raises(self, monkeypatch, worker_env):
        ...
        assert os.environ.get("ANAKOT_KANBAN_BOARD") == "team-alpha"
...
def test_dispatcher_grants_only_the_assigned_worker_scope(tmp_path, monkeypatch):
    ...
    from anakot_cli.kanban_db_dispatch import _default_spawn

    monkeypatch.setenv("HOME", str(tmp_path))
    db = tmp_path / "board.db"
    monkeypatch.setenv("ANAKOT_KANBAN_DB", str(db))
    conn = connect(db)
    ...
    worker.chmod(0o700)
    monkeypatch.setenv("ANAKOT_BIN", str(worker))
    # Building a new worker under an existing task must replace, not inherit, its scope.
    monkeypatch.setenv("ANAKOT_KANBAN_TASK", "prior-task")
    ...
    monkeypatch.setenv("ANAKOT_DELEGATED_CHILD_CONTEXT", str(tmp_path))
    ...
    assert os.environ["ANAKOT_KANBAN_TASK"] == "prior-task"
    conn.close()
