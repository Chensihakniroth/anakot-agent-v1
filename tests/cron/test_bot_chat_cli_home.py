...
def test_cli_keeps_discovered_home_when_launch_selection_changes(tmp_path, monkeypatch, profile):
    ...
    token = set_anakot_home_override(str(root))
    ...
    def run(argv, env, report_path, timeout):
        ...
        code = ('import json,sys; sys.argv=["anakot"]+json.loads(sys.argv[1]); '
                'import anakot_cli.main; from anakot_constants import get_anakot_home; '
                'print(json.dumps(str(get_anakot_home())))')
        # Everything after the launcher (binary or ``python -m anakot_cli.main``) is the CLI argv.
        cli_argv = argv[3:] if argv[1:3] == ["-m", "anakot_cli.main"] else argv[1:]
    ...
    try:
        ...
        assert seen == [home]
        assert not (tmp_path / ".anakot").exists()
    finally:
