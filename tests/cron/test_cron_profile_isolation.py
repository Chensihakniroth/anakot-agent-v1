...
profile's ANAKOT_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.anakot/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active ANAKOT_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever ANAKOT_HOME the ticker happens to have —
...
def _set_profile_env(monkeypatch, root: Path, profile_home: Path) -> None:
    ...
    ANAKOT_HOME is a profile under it (``<root>/profiles/<name>``)."""
    import anakot_constants

    monkeypatch.setattr(
        anakot_constants, "_get_platform_default_anakot_home", lambda: root
    )
    monkeypatch.setenv("ANAKOT_HOME", str(profile_home))


def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile ANAKOT_HOME (<root>/profiles/<name>), the cron store
    ...
    assert anakot_constants.get_anakot_home().resolve() == profile_home.resolve()
    assert anakot_constants.get_default_anakot_root().resolve() == root.resolve()

    # cron/jobs.py computes ANAKOT_DIR from get_anakot_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.ANAKOT_DIR.resolve() == profile_home.resolve()
        assert (
