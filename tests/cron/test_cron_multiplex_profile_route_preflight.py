...
def multiplex_homes(tmp_path, monkeypatch):
    ...
    ``get_default_anakot_root`` is patched so the primary config, the profiles
    ...
    monkeypatch.setattr(
        "anakot_constants.get_default_anakot_root", lambda: root
    )
    token = set_anakot_home_override(str(grant_home))
