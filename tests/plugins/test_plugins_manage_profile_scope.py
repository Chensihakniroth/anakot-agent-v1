...
``set_anakot_home_override`` / ``reset_anakot_home_override``. Because
``_plugins_dir()`` keys off ``get_anakot_home()``, the list action must then
...
def test_plugins_manage_profile_reads_that_profiles_dir(tmp_path, monkeypatch):
    ...
    import anakot_cli.profiles as profiles
