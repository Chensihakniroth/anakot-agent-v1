"""Plugin data paths follow the active profile's ANAKOT_HOME, including the ContextVar override.

Several plugins carried a ``~/.anakot`` fallback (guarding an ImportError of ``anakot_constants``
...
def _openviking_log(home):
    ...
    return ov.get_anakot_home() / ov._OPENVIKING_SERVER_LOG_RELATIVE_PATH
