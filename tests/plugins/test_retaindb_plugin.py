...
def _isolate_env(tmp_path, monkeypatch):
    """Ensure ANAKOT_HOME and RETAINDB vars are isolated."""
...
class TestRetainDBMemoryProvider:
    ...
    def test_initialize_creates_client_and_queue(self, tmp_path, monkeypatch):
        ...
        p.initialize("test-session", anakot_home=str(tmp_path / ".anakot"))
...
class TestPrefetch:
    ...
    def _make_initialized_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        p = RetainDBMemoryProvider()
        p.initialize("test-session", anakot_home=str(anakot_home))
        return p
