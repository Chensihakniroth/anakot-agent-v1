...
plugin SDK (``window.__ANAKOT_PLUGIN_SDK__``): ``fetchJSON`` (JSON REST),
``authedFetch`` (uploads / blob downloads), and ``buildWsUrl`` /
``buildWsAuthParam`` (WebSockets). These handle BOTH dashboard auth modes —
loopback (``X-Anakot-Session-Token`` header) and gated OAuth
(``anakot_session_at`` cookie / single-use ``?ticket=``).
...
_FORBIDDEN = "__ANAKOT_SESSION_TOKEN__"
