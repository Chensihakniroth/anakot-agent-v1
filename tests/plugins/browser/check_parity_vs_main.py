...
Run from the PR worktree:
    ...
    cd ~/.anakot/anakot-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.anakot/anakot-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.anakot/anakot-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a anakot-agent checkout"
)
...
# Isolated ANAKOT_HOME for the config write.
