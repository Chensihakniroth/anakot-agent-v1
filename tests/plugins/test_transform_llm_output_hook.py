...
from anakot_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(anakot_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <anakot_home>/plugins/<name> and opt it in."""
    plugin_dir = anakot_home / "plugins" / name
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / "anakot_empty"))
