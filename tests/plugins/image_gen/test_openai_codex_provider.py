...
def _tmp_anakot_home(tmp_path, monkeypatch):
...
class TestMetadata:
    ...
    def test_setup_schema_has_no_required_env_vars(self, provider):
        ...
        assert "anakot auth add openai-codex" in schema["post_setup_hint"]
        assert "anakot auth codex`" not in schema["post_setup_hint"]
