...
OPENVIKING_* / HINDSIGHT_BANK_ID / ANAKOT_HONCHO_HOST, the provider must fall back to its own default
...
_DEFAULT_ENV = {
    ...
    "ANAKOT_HONCHO_HOST": "host-default", "HONCHO_BASE_URL": "https://honcho.default",
...
def secondary_profile(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(prof_b))
...
def test_secondary_profile_memory_identity_never_inherits_default_environ(secondary_profile):
    ...
    assert (cfg["agent_id"], cfg["host"], cfg["mode"]) == ("anakot", "", "platform")
    ...
    provider.initialize("s1", anakot_home=str(secondary_profile))
    assert provider._client.project == "anakot-b"
    ...
    assert (hcfg["banks"]["anakot"]["bankId"], hcfg["mode"]) == ("anakot", "cloud")
