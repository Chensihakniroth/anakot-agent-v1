...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", anakot_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Anakot"],
        search_results=[{"memory": "Uses Anakot", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Anakot") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Anakot memory provider", "similarity": 0.88}],
    }
...
def test_capture_custom_id_buckets_by_four_hours():
    ...
    assert _capture_custom_id("", datetime(2026, 9, 12, 23, 0, tzinfo=timezone.utc)) == "anakot_2026-09-12_b5"
...
def test_merge_metadata_stamps_sm_source():
    # sm_source routes Anakot writes into the "Anakot" Space in the Supermemory
    ...
    assert merged["sm_source"] == "anakot"
    ...
    assert merged2["sm_source"] == "anakot"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "anakot-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", anakot_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "anakot_coder"
...
def test_container_tag_env_var_override(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    p = SupermemoryMemoryProvider()
    p.initialize("s1", anakot_home=str(tmp_path), platform="cli")
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "anakot"
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("anakot_cli.config.save_config", fake_save_config)
