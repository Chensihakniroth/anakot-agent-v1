...
ANAKOT_HOME override (``anakot_constants.set_anakot_home_override``).
...
def homes(tmp_path, monkeypatch):
    """Profile A (launch home, ``ANAKOT_HOME``) and profile B with different config/.env values."""
...
def test_memory_provider_skill_prune_only_touches_the_active_home(homes, monkeypatch):
    ...
    from anakot_cli.plugins import _reset_plugin_managers_for_tests, get_plugin_manager
    ...
    try:
        for home in (a, b):
            with scoped(home):
                ...
                provider.initialize(session_id=f"s-{home.name}", anakot_home=str(home))
