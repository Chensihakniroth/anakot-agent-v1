...
CASES = {
    ...
    "product python → nix": (["anakot_cli/config.py"], _lanes(python=True, scan=True)),
    ...
    # files change — not on every anakot_state.py PR.
    ...
    "python source alone → no desktop_updater lane": (["anakot_state.py"], _lanes(python=True, scan=True)),
...
def test_pull_request_changed_files_skips_non_pr_events(monkeypatch):
    ...
    monkeypatch.setenv("REPO", "Chensihakniroth/anakot-agent-v1")
