...
def test_upload_evidence_accepts_only_attachment_urls(tmp_path, monkeypatch):
    ...
    result = _mod.upload_evidence(
        ...
        tmp_path,
        "Chensihakniroth/anakot-agent-v1",
        "bot-session-token",
    )

    assert result == {"shot.png": "https://github.com/user-attachments/assets/12345678-1234-1234-1234-123456789abc"}
    assert calls[0][0] == ["gh", "image", "--repo", "Chensihakniroth/anakot-agent-v1", str(shot)]
...
def test_publish_marks_evidence_upload_failure_in_pr_comment(tmp_path, monkeypatch):
    comment = {
        "id": 123,
        "body": "before\n<!-- anakot-e2e-evidence:start -->\npending\n<!-- anakot-e2e-evidence:end -->\nafter",
    }
    updates = []
    ...
    assert updates == [
        (
            "https://api.github.com/repos/Chensihakniroth/anakot-agent-v1/issues/comments/123",
            "github-token",
            "PATCH",
            {
...
def test_find_review_comment_requires_the_evidence_marker():
    pending = "<!-- anakot-ci-review-bot -->\n<!-- anakot-e2e-evidence:start -->\npending\n<!-- anakot-e2e-evidence:end -->"

    assert _mod._find_review_comment([{"body": "<!-- anakot-ci-review-bot --> no evidence"}]) is None
