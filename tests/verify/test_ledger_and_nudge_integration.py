...
- ``anakot verify`` records into the evidence ledger (pass and fail),
- a passing run satisfies the verify-on-stop guard,
- the verify-on-stop nudge names ``anakot verify --json`` when the workspace
...
def _ledger_on(monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_VERIFY_ON_STOP", "1")
...
def _workspace(tmp_path, *, scripts=None, manifest_recipe=None):
    ...
    if manifest_recipe is not None:
        anakot_dir = project / ".anakot"
        anakot_dir.mkdir()
        (anakot_dir / "environment.json").write_text(
...
def test_record_verify_run_marks_workspace_passed(anakot_home):
    project = _workspace(anakot_home)
    ...
    assert status["evidence"]["canonical_command"] == "anakot verify"


def test_record_verify_run_records_failure(anakot_home):
    project = _workspace(anakot_home)
...
def test_cli_passing_run_writes_ledger_evidence(anakot_home, capsys):
    project = _workspace(anakot_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
def test_cli_failing_run_writes_failed_evidence(anakot_home, capsys):
    project = _workspace(anakot_home, manifest_recipe={"name": "Fake", "test": ["false"]})
    code = run_verify_command(make_args(project))
    assert code == 1
    status = verification_status(session_id=None, cwd=project)
    assert status["status"] == "failed"
...
def test_cli_partial_run_records_targeted_scope(anakot_home, capsys):
...
def test_cli_run_uses_anakot_session_id_env(anakot_home, capsys, monkeypatch):
    monkeypatch.setenv("ANAKOT_SESSION_ID", "sess-42")
    project = _workspace(anakot_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# closed loop: edit -> stop guard nudge -> anakot verify -> guard satisfied
# ---------------------------------------------------------------------------


def test_passing_verify_run_satisfies_stop_guard(anakot_home, capsys):
    project = _workspace(anakot_home, manifest_recipe={"name": "Fake", "test": ["echo ok"]})
...
# ---------------------------------------------------------------------------
# nudge wording: recipe-aware `anakot verify --json` suggestion
# ---------------------------------------------------------------------------


def test_nudge_mentions_anakot_verify_when_recipe_has_start(anakot_home):
    project = _workspace(anakot_home, scripts={"test": "vitest", "dev": "vite"})
    changed = str(project / "src" / "app.ts")
    ...
    assert "anakot verify --json" in nudge
    # The cheap verify commands are still listed first.
    assert "npm run test" in nudge


def test_nudge_mentions_anakot_verify_when_manifest_exists(anakot_home):
    # No start script, but a saved .anakot/environment.json qualifies.
    project = _workspace(
        anakot_home,
        scripts={"test": "vitest"},
...
def test_nudge_keeps_plain_wording_without_recipe_start(anakot_home):
    # Verify commands but no start script and no manifest: today's wording.
    project = _workspace(anakot_home, scripts={"test": "vitest"})
    changed = str(project / "src" / "app.ts")
    mark_workspace_edited(session_id="s1", cwd=project, paths=[changed])
    nudge = build_verify_on_stop_nudge(session_id="s1", changed_paths=[changed])
    assert nudge is not None
    assert "anakot verify" not in nudge


def test_nudge_recipe_detection_failure_is_silent(anakot_home, monkeypatch):
...
def test_detect_path_merges_project_facts_commands(anakot_home, capsys):
    project = _workspace(anakot_home)  # package.json with no scripts
...
def test_manifest_recipe_is_not_merged(anakot_home, capsys):
...
def test_merge_skips_commands_recipe_already_has(anakot_home, capsys):
    project = _workspace(anakot_home, scripts={"test": "vitest"})
    code = run_verify_command(make_args(project, detect_only=True))
    assert code == 0
    payload = json.loads(capsys.readouterr().out)
