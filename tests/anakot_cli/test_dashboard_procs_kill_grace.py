...
``anakot update`` / ``anakot dashboard --stop`` fall back to ``_kill_pids_posix`` for a
...
# Chat TUI): `anakot dashboard --stop` / `anakot update` must not SIGTERM their own process.
_STOPPER_CHILD = textwrap.dedent(
    """
    import os, pathlib, sys
    from anakot_cli import dashboard_procs as dp
