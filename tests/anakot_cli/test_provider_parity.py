...
the SAME provider universe as ``anakot model`` (the CLI/TUI picker).
...
def test_every_anakot_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `anakot model` universe."""
    ...
    assert not missing, (
        "providers shown in `anakot model` but not configurable in the desktop "
