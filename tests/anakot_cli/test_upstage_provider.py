...
Regression guard for the bug where `anakot model` saved `provider: upstage`
correctly but, on re-entry, showed a different provider as active. Root cause:
`anakot_cli/providers.py` (the resolver behind `resolve_provider_full`) had no
...
class TestUpstageOverlay:
    def test_overlay_exists(self):
        ...
        assert "upstage" in ANAKOT_OVERLAYS
        overlay = ANAKOT_OVERLAYS["upstage"]
...
class TestUpstageConfigProviderWins:
    ...
    Mirrors the display logic in `anakot_cli/main.py` (cmd_model): read
