"""Tests for the `anakot memory reset` CLI command.
...
- Profile-scoped reset (uses ANAKOT_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake ANAKOT_HOME with memory files."""
    anakot_home = tmp_path / ".anakot"
    memories = anakot_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nAnakot repo is at ~/.anakot/anakot-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `anakot memory reset` is run.
    """
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        (anakot_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
