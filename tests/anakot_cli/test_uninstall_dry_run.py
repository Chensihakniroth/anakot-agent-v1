...
def test_dry_run_prints_plan_without_mutating(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(uninstall, "get_anakot_home", lambda: anakot_home)
    monkeypatch.setattr(uninstall, "_is_default_anakot_home", lambda home: False)
    ...
    assert str(anakot_home) in output
    assert project_root.exists()
    assert anakot_home.exists()
