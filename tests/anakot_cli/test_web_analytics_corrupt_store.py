...
from anakot_cli.web_routers import _common, analytics
...
def test_corrupt_store_polls_return_status_and_warn_once_per_interval(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level(logging.DEBUG, logger="anakot_cli.web_server"):
    ...
    for resp in (first, second, third):
        ...
        assert "anakot doctor" in resp.json()["detail"]["message"]
    # Only the dashboard's own warning counts: anakot_state logs an unrelated
    # once-per-process SQLite-version advisory on some interpreters (CI's 3.50.4).
    warnings = [
        r for r in caplog.records
        if r.levelno >= logging.WARNING and r.name.startswith("anakot_cli.web_server")
    ]
    ...
    caplog.clear()
    with caplog.at_level(logging.WARNING, logger="anakot_cli.web_server"):
        assert client.get("/api/analytics/usage?days=7").status_code == 503
    assert sum(
        r.levelno >= logging.WARNING and r.name.startswith("anakot_cli.web_server")
