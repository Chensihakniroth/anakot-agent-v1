"""Tests for anakot_cli.uninstall.remove_node_symlinks.

Regression for #34536: the POSIX installer drops node/npm/npx symlinks in
~/.local/bin pointing into $ANAKOT_HOME/node and prepends ~/.local/bin to
PATH, shadowing an existing nvm. Uninstall must remove those symlinks, but
only when they still resolve into the Anakot-managed node dir.
"""
...
def _make_anakot_node(anakot_home: Path) -> Path:
    """Create a fake $ANAKOT_HOME/node/bin/{node,npm,npx} tree."""
    node_bin = anakot_home / "node" / "bin"
...
def test_leaves_unrelated_symlinks_untouched(fake_home):
    ...
    anakot_home = fake_home / ".anakot"
    _make_anakot_node(anakot_home)
    ...
    removed = uninstall.remove_node_symlinks(anakot_home)
...
def test_removes_fhs_symlinks_in_usr_local_bin(fake_home, tmp_path, monkeypatch):
    ...
    """
    anakot_home = fake_home / ".anakot"
    node_bin = _make_anakot_node(anakot_home)
