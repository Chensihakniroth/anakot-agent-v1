"""Tests for anakot_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import anakot_cli.gui_uninstall as gu


def _make_agent(anakot_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = anakot_home / "anakot-agent"
    (agent_root / "anakot_cli").mkdir(parents=True)
    (agent_root / "anakot_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(anakot_home: Path) -> None:
    """Create the source-built GUI artifacts a `anakot desktop` run produces."""
    desktop = anakot_home / "anakot-agent" / "apps" / "desktop"
    ...
    (anakot_home / "anakot-agent" / "node_modules").mkdir(parents=True)
    (anakot_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(anakot_home: Path) -> None:
    (anakot_home / "config.yaml").write_text("x: 1\n")
    (anakot_home / ".env").write_text("KEY=secret\n")
    (anakot_home / "sessions").mkdir()
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    _make_agent(anakot_home)
    _make_gui_build(anakot_home)
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    summary = gu.gui_install_summary(anakot_home)
    ...
    assert summary["anakot_home"] == str(anakot_home)
...
def test_linux_discovery_includes_launcher_entry(tmp_path, monkeypatch):
    """The launcher entry that `anakot desktop` installs is removable."""
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / ".anakot"
    _make_agent(anakot_home)
    icon = lde.icon_path(anakot_home / "anakot-agent")
    icon.parent.mkdir(parents=True, exist_ok=True)
    icon.write_bytes(b"\x89PNG")
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "none")

    removed = gu.uninstall_gui(anakot_home)
    ...
    assert lde.icon_path(anakot_home / "anakot-agent").exists()
    # The agent itself survives a GUI uninstall.
    assert (anakot_home / "anakot-agent" / "anakot_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".anakot")
...
def test_uninstall_args_namespace_mode_mapping():
    ...
    import anakot_cli.uninstall as uninstall
