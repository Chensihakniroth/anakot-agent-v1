...
def test_codex_pool_honors_anakot_codex_base_url(monkeypatch):
...
def test_explicit_openrouter_config_mirror_bypasses_pool(monkeypatch):
    ...
    # The canonical URL that `anakot setup` persists is NOT a mirror: the pool must still serve it.
...
class TestProviderEntryApiKeyEnvAlias:
    ...
    def test_snake_case_api_key_env_normalizes_to_key_env(self):
        from anakot_cli.config import _normalize_custom_provider_entry
        entry = {
            "name": "vendor",
    ...
    def test_valid_fields_set_lists_key_env(self):
        ...
        from anakot_cli.config import _VALID_CUSTOM_PROVIDER_FIELDS
...
def test_minimax_oauth_runtime_returns_anthropic_messages_mode(monkeypatch):
    ...
    from anakot_cli.auth import MINIMAX_OAUTH_GLOBAL_INFERENCE
...
_CODEX_STORE_CREDS = {"base_url": "https://chatgpt.com/backend-api/codex", "api_key": "tok",
                      "source": "anakot-auth-store", "last_refresh": 1}
