"""Tests for ``anakot doctor --live`` — opt-in bounded real-call tool-backend probes.
...
from anakot_cli import doctor_live
from anakot_cli.doctor_live import (
    ProbeResult,
...
class TestBrowserAvailableNpxRung:
    ...
    with. It must fall through to the same cascade `anakot doctor` uses."""

    def _block_path_and_node_modules_checks(self, monkeypatch, tmp_path):
        monkeypatch.setattr("shutil.which", lambda *a, **k: None)
        monkeypatch.setattr("anakot_cli.doctor.ANAKOT_HOME", tmp_path / "home")
        monkeypatch.setattr("anakot_cli.doctor.PROJECT_ROOT", tmp_path / "root")
