"""A Desktop-pooled ``anakot serve`` child must PROVE it is idle before the Desktop may retire it
...
from anakot_cli.web_server_idle_proof import idle_proof, pending_human_input
...
def _spawn_desktop_child(tmp_path: Path, name: str, *, busy: bool) -> subprocess.Popen:
    """Mirror the Desktop pool spawn: ``ANAKOT_DESKTOP=1`` (in-process cron ticker), a per-child
    ANAKOT_HOME, an ephemeral port, the session token the Desktop probes with. The busy child
    ...
    for k in ("ANAKOT_PARENT_PID", "ANAKOT_PARENT_START_MARKER", "ANAKOT_PARENT_NONCE"):
        env.pop(k, None)
    env.update(
...
def _probe(port: int, token: str | None = TOKEN) -> tuple[int, dict]:
    req = urllib.request.Request(f"http://127.0.0.1:{port}/api/health/idle",
                                 headers={"X-Anakot-Session-Token": token} if token else {})
    try:
...
def test_live_pooled_children_prove_idle_or_busy_over_the_desktop_probe(tmp_path):
    ...
    try:
        ...
        verdicts["cron-busy"] = _probe_settled(ports["cron-busy"], lambda b: str(b.get("detail", "")).startswith("cron:"))
