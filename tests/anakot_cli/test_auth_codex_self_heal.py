...
Anakot keeps its OWN copy of the Codex OAuth token (per profile + top-level),
separate from the Codex CLI's ``~/.codex/auth.json``. OAuth refresh_tokens are
single-use, so when the Codex CLI (or another Anakot process) rotates the shared
...
from anakot_cli.auth import AuthError, _refresh_codex_auth_tokens, resolve_codex_runtime_credentials
...
def test_self_heals_on_stale_refresh_token(monkeypatch):
    ...
    # the recovered token was persisted to the Anakot auth store
...
def test_self_heals_missing_singleton_access_token_from_codex_cli(tmp_path, monkeypatch):
    """Exact cron failure path: Anakot auth has refresh_token but missing access_token."""
    ...
    assert resolved["source"] == "anakot-auth-store"
    stored = json.loads((anakot_home / "auth.json").read_text())
...
def test_opt_out_never_adopts_codex_cli_login(tmp_path, monkeypatch):
    ...
    family the user did not hand to Anakot. Both automatic recovery paths must leave it (and Anakot' own
    auth.json) untouched and surface the real error instead."""
    anakot_home = tmp_path / "anakot"
    codex_home = tmp_path / "codex"
    anakot_home.mkdir()
    codex_home.mkdir()
    (anakot_home / "config.yaml").write_text("auth:\n  adopt_external_logins: false\n")
    anakot_auth = {"version": 1, "providers": {"openai-codex": {
        "tokens": {"refresh_token": "stale-refresh"}, "auth_mode": "chatgpt"}}}
    (anakot_home / "auth.json").write_text(json.dumps(anakot_auth))
    (codex_home / "auth.json").write_text(json.dumps({
    ...
    assert json.loads((anakot_home / "auth.json").read_text()) == anakot_auth
