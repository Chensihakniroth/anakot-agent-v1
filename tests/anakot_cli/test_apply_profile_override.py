...
def _run_apply_profile_override(
    ...
    if active_profile and active_profile != "default":
        ...
        (anakot_root / "profiles" / active_profile / "config.yaml").write_text("{}\n")  # identity marker
    ...
    for var in (
        "ANAKOT_SUPERVISED_CHILD",
        "ANAKOT_S6_SUPERVISED_CHILD",
        "INVOCATION_ID",
...
class TestApplyProfileOverrideAnakotHomeGuard:
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        user_home = tmp_path / "home" / "anakot"
        profile_dir = user_home / ".anakot" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (profile_dir / "config.yaml").write_text("{}\n")  # identity marker: a bare dir does not resolve
        (root_home / ".anakot").mkdir(parents=True, exist_ok=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("SUDO_USER", "anakot")
        monkeypatch.delenv("ANAKOT_HOME", raising=False)
        monkeypatch.setattr(os, "geteuid", lambda: 0, raising=False)
        monkeypatch.setattr(sys, "argv", ["anakot", "-p", "elias", "gateway", "install", "--system"])

        import pwd

        monkeypatch.setattr(pwd, "getpwnam", lambda name: SimpleNamespace(pw_dir=str(user_home)))

        from anakot_cli.main import _apply_profile_override, _resolve_sudo_user_profile_env
        _apply_profile_override()

        assert os.environ.get("ANAKOT_HOME") == str(profile_dir)
        assert sys.argv == ["anakot", "gateway", "install", "--system"]
        # Same identity gate as ``-p`` without sudo: a marker-less shell is not a profile.
        (user_home / ".anakot" / "profiles" / "ghost" / "cron").mkdir(parents=True)
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``anakot gateway run`` (no ``-p``) to mean "the root ANAKOT_HOME
    profile". The run-script exports ``ANAKOT_S6_SUPERVISED_CHILD=1``.
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `anakot gateway run` still honors
    ...
    def test_supervised_named_profile_flag_still_wins(self, tmp_path, monkeypatch):
        ...
        anakot_root = tmp_path / ".anakot"
        anakot_root.mkdir(parents=True, exist_ok=True)
        (anakot_root / "active_profile").write_text("briefer")
        for name in ("briefer", "coder"):
            (anakot_root / "profiles" / name).mkdir(parents=True, exist_ok=True)
            (anakot_root / "profiles" / name / "config.yaml").write_text("{}\n")  # identity marker

        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.delenv("ANAKOT_HOME", raising=False)
        monkeypatch.setenv("ANAKOT_S6_SUPERVISED_CHILD", "1")
        monkeypatch.setattr(sys, "argv", ["anakot", "-p", "coder", "gateway", "run"])

        from anakot_cli.main import _apply_profile_override
        _apply_profile_override()

        result = os.environ.get("ANAKOT_HOME")
        assert result is not None
        assert result.endswith("coder")
...
class TestGeneralizedSupervisorMarkers:
    ...
    profile identity via the unit's ANAKOT_HOME (root home for the default
    ...
    token. Markers: ANAKOT_SUPERVISED_CHILD (generalized, exported by
    generated units), INVOCATION_ID (systemd, gateway commands only), and
    ANAKOT_GATEWAY_EXTERNAL_SUPERVISOR (explicit opt-in).
    """
    ...
    def test_supervised_child_marker_skips_active_profile(
        ...
        result = _run_apply_profile_override(
            ...
            argv=["anakot", "gateway", "run"],
            extra_env={"ANAKOT_SUPERVISED_CHILD": "1"},
        )
        assert result == str(anakot_root), (
    ...
    def test_systemd_invocation_id_skips_active_profile_for_gateway(
        ...
        ANAKOT_SUPERVISED_CHILD marker existed."""
        anakot_root = self._root_home(tmp_path)
        result = _run_apply_profile_override(
            tmp_path,
            monkeypatch,
    ...
    def test_external_supervisor_marker_skips_active_profile(
        ...
        result = _run_apply_profile_override(
            ...
            active_profile="telegram_nick",
            argv=["anakot", "gateway", "run"],
            extra_env={"ANAKOT_GATEWAY_EXTERNAL_SUPERVISOR": "1"},
        )
        assert result == str(anakot_root)
    ...
    def test_desktop_ssh_serve_child_skips_active_profile(self, tmp_path, monkeypatch):
        ...
        result = _run_apply_profile_override(
            ...
            anakot_home=str(anakot_root),
            active_profile="telegram_nick",
            argv=["anakot", "serve", "--isolated", "--host", "127.0.0.1", "--port", "0",
    ...
    def test_generated_systemd_unit_exports_supervised_marker(
        ...
        (tmp_path / "home").mkdir()
        from anakot_cli.gateway import generate_systemd_unit

        unit = generate_systemd_unit()
        assert 'Environment="ANAKOT_SUPERVISED_CHILD=1"' in unit
    ...
    def test_generated_launchd_plist_exports_supervised_marker(
        ...
        assert "<key>ANAKOT_SUPERVISED_CHILD</key>" in plist
