"""Orphan Desktop-local ``anakot serve`` reap at backend start.
...
def test_desktop_local_serve_shape_matches_ephemeral_loopback():
    assert _is_desktop_local_serve_cmdline(
        "python -m anakot_cli.main serve --host 127.0.0.1 --port 0"
    )
    assert _is_desktop_local_serve_cmdline(
        "anakot serve --isolated --host 127.0.0.1 --port 0 --ssh-owner-nonce abc"
    )
    assert _is_desktop_local_serve_cmdline(
        "/venv/bin/anakot serve --host=127.0.0.1 --port=0"
    )


def test_desktop_local_serve_shape_spares_fixed_port_and_non_serve():
    assert not _is_desktop_local_serve_cmdline(
        "anakot serve --host 100.106.105.2 --port 9119 --skip-build"
    )
    assert not _is_desktop_local_serve_cmdline(
        "anakot serve --host 127.0.0.1 --port 9119"
    )
    assert not _is_desktop_local_serve_cmdline("anakot gateway run --replace")
    # "serve" inside another token is not the serve subcommand (token matcher, not substring).
    assert not _is_desktop_local_serve_cmdline(
        "anakot kanban --preserve-cache --host 127.0.0.1 --port 0"
    )


def test_reap_only_kills_ppid1_local_serves():
    scanned = [
        (111, "anakot serve --host 127.0.0.1 --port 0"),  # orphan local
        (222, "anakot serve --host 127.0.0.1 --port 0"),  # still has parent
        (333, "anakot serve --host 100.1.2.3 --port 9119"),  # fixed remote
        (444, "anakot serve --isolated --host 127.0.0.1 --port 0"),  # orphan isolated
    ]
    ...
    with (
        patch(
            "anakot_cli.dashboard_procs._scan_dashboard_processes",
            return_value=scanned,
        ),
        patch(
            "anakot_cli.dashboard_procs._process_ppid",
            side_effect=lambda pid: ppids.get(pid),
        ),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        os.environ.pop("ANAKOT_DESKTOP_CHILD_PID", None)
...
def test_reap_passes_child_pid_exclude_to_scan():
    with (
        ...
        patch("sys.platform", "darwin"),
        patch.dict(os.environ, {"ANAKOT_DESKTOP_CHILD_PID": "999,111"}, clear=False),
    ):
...
def _valid_lock_payload(pid: int, ownership_id: str, spawn_nonce: str) -> dict:
    ...
    return {
        ...
        "anakotHome": "~/.anakot",
        "logPath": f"~/.anakot/desktop-ssh/{ownership_id}/{spawn_nonce}.log",
...
def test_lock_owned_serve_pids_sees_root_home_locks_from_a_profile_home(tmp_path, monkeypatch):
    """A profile backend (``ANAKOT_HOME=<root>/profiles/<name>``) must still see the Desktop's SSH
    ...
    monkeypatch.setattr(anakot_constants, "_get_platform_default_anakot_home", lambda: root)
    monkeypatch.setattr(anakot_constants, "_default_anakot_root_memo", None, raising=False)
...
def test_valid_lockfile_payload_rejects_wrong_owner_and_shape():
    ...
    bad_log["logPath"] = "~/.anakot/desktop-ssh/{oid}/other.log".format(oid=oid)
...
def test_reap_spare_lock_owned_ssh_remote_backend_of_foreign_client():
    ...
    scanned = [
        (555, "anakot serve --host 127.0.0.1 --port 0"),  # lock-owned remote
        (666, "anakot serve --host 127.0.0.1 --port 0"),  # genuine orphan
    ]
    ppids = {555: 1, 666: 1}
    terms: list[int] = []
    live = {555, 666}
...
def test_reap_spares_young_backend_until_desktop_can_write_lock():
    ...
    scanned = [(777, "anakot serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
    ...
    with (
        ...
        patch("anakot_cli.dashboard_procs._process_ppid", return_value=1),
        patch("os.kill", side_effect=fake_kill),
        patch("sys.platform", "darwin"),
    ):
        result = _reap_orphaned_desktop_local_serves(
...
def test_reap_spares_backend_when_process_age_is_unknown():
    scanned = [(778, "anakot serve --isolated --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_age_boundary_makes_180_second_orphan_eligible():
    scanned = [
        (779, "anakot serve --isolated --host 127.0.0.1 --port 0"),
        (780, "anakot serve --isolated --host 127.0.0.1 --port 0"),
    ]
    terms: list[int] = []
    live = {779, 780}
...
def test_reap_spare_lock_owned_backend_even_without_exclude_match(tmp_path):
    ...
    spares a matching orphaned serve even when ANAKOT_DESKTOP_CHILD_PID is
    ...
    scanned = [(4242, "anakot serve --host 127.0.0.1 --port 0")]
    terms: list[int] = []
...
def test_reap_kills_descendants_of_killed_roots_but_spares_a_failed_roots_subtree():
    ...
    scanned = [
        (111, "anakot serve --host 127.0.0.1 --port 0"),  # ours: killed, child 1111 swept
        (444, "anakot serve --host 127.0.0.1 --port 0"),  # not ours: EPERM, child 4444 spared
    ]
    ...
    with (
        patch("anakot_cli.dashboard_procs._scan_dashboard_processes", return_value=scanned),
        patch("anakot_cli.dashboard_procs._process_ppid", return_value=1),
        patch("anakot_cli.dashboard_procs._posix_descendants", return_value=descendants) as snap,
