...
def test_update_notice_offers_v1_trigram_tool_calls_rebuild(tmp_path, monkeypatch, capsys):
    ...
    class FakeSessionDB:
        ...
        _db_needs_fts_storage_upgrade = staticmethod(
            anakot_state.SessionDB._db_needs_fts_storage_upgrade
        )

    monkeypatch.setattr(anakot_constants, "get_anakot_home", lambda: tmp_path)
    monkeypatch.setattr(anakot_state, "SessionDB", FakeSessionDB)
    ...
    assert "anakot sessions optimize-storage" in capsys.readouterr().out
    conn.close()
