...
option absent from both sets, so every ``anakot --reasoning high <cmd> …``
invocation mis-classified ``high`` as the first positional and forced eager
plugin CLI discovery (~500-650ms startup tax).

Both sites now share ``anakot_cli._parser.top_level_value_flag_sets()``,
...
from anakot_cli.main import _first_positional_argv, _plugin_cli_discovery_needed
...
def test_first_positional_skips_reasoning_value(monkeypatch):
    monkeypatch.setattr("sys.argv", ["anakot", "--reasoning", "high", "chat", "hello"])
    assert _first_positional_argv() == "chat"
    assert _plugin_cli_discovery_needed() is False


@pytest.mark.parametrize("argv", [
    ["anakot", "--reasoning", "ultra", "--provider", "openai", "-z", "prompt here"],
    ["anakot", "-m", "x", "--reasoning=low", "chat"],
])
