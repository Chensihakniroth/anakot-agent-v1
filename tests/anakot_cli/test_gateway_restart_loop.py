...
Covers:
- Defense 1: gateway stop/restart refuse when _ANAKOT_GATEWAY=1
...
from anakot_cli.cron import cron_command
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `python*` carries no anakot/gateway token yet takes it down — on every platform.
        ...
        "pkill -9 python3",
        "pkill -f python",
        'pkill -f "python -m anakot_cli.main"',
        "pkill -f anakot_cli",
        "killall -9 python3.12",
        "sudo pkill -9 python3",
        "pgrep python | xargs kill -9",
        "kill $(pidof python3)",
        # Windows spellings of the anakot-gateway forms (salvaged from #94379).
        "anakot.exe gateway restart",
        "anakot.cmd gateway stop",
        r'"C:\Program Files\anakot.exe" gateway restart',
        "taskkill /F /IM anakot-gateway.exe",
        "Stop-Process -Name anakot-gateway -Force",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        # Unrelated scripts that merely contain "anakot" cannot match the gateway cmdline.
        "pkill -f 'anakot-polis/run.sh'",
        "pkill -f my_anakot_bot.py",
        "pgrep python",
        "anakot.exe gateway start",
        "my-anakot.exe gateway restart",
    ...
    @pytest.mark.parametrize("text", [
        ...
        "launchctl submit -l ai.anakot.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.anakot/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l anakot-gateway-restart-helper -- /bin/sh helper.sh",
        # bootstrap loads an arbitrary plist — same laundering shape.
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.anakot.gateway.restart-once.plist",
        "launchctl bootout gui/501/ai.anakot.gateway",
        ...
        (
            "launchctl submit \\\n"
            "  -l ai.anakot.gateway-hard-restart-no-photon-notice \\\n"
            "  -- /bin/sh ~/.anakot/scripts/hard_restart_gateway_no_photon_notice.sh"
        ),
    ])
    ...
    def test_label_defined_before_verb_is_caught(self):
        ...
        cmd = (
            "uid=$(id -u); for item in 'ai.anakot.gateway-apollo:/a.plist' "
            "'ai.anakot.gateway-cronus:/c.plist' 'ai.anakot.gateway-plutus:/p.plist' "
            "'ai.anakot.gateway:/Users/botuser/Library/LaunchAgents/ai.anakot.gateway.plist'; "
    ...
    def test_line_continuation_does_not_bridge_unrelated_lines(self):
        ...
        text = (
            ...
            "unrelated anakot note on the next line"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
    ...
    @pytest.mark.parametrize("text", [
        ...
        # `launchctl kick"start" ... ai.anakot.gateway` executes exactly as
        ...
        'launchctl kick"start" -k gui/501/ai.anakot.gateway',
        "launchctl kick'start' -k gui/501/ai.anakot.gateway",
        "launchctl kick\\start -k gui/501/ai.anakot.gateway",
        'launchctl "kickstart" -k gui/501/ai.anakot.gateway',
        # Splices on the newer/legacy unload spellings this PR added.
        'launchctl boot"out" gui/501/ai.anakot.gateway',
        "launchctl dis\\able gui/501/ai.anakot.gateway",
        # The gateway identifier itself can be spliced just as easily.
        'launchctl bootout gui/501/ai.anakot."gateway"',
        # Same class on the systemctl and anakot-CLI branches.
        'systemctl re"start" anakot-gateway',
        'anakot gateway re"start"',
    ])
    ...
    def test_spliced_verb_inside_shell_c_payload_is_blocked(self):
        ...
        command = 'sh -c \'launchctl kick"start" -k gui/501/ai.anakot.gateway\''
    ...
    @pytest.mark.parametrize("text", [
        ...
        'launchctl kick"start" -k gui/501/ai.anakot.update-checker',
        'systemctl re"start" anakot-meta.service',
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anakot cron list",
        "anakot update",
        "anakot config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `anakot gateway start` is benign — starting a gateway from inside a
        ...
        "anakot gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway anakot
        # services must not be falsely blocked (the old `.*anakot` matched any
        # anakot token).
        "launchctl unload ai.anakot.update-checker.plist",
        "launchctl restart ai.anakot.daemon",
        ...
        "systemctl restart anakot-meta.service",
        "systemctl restart anakot-cron-helper",
        ...
        "echo after the anakot gateway restarted cleanly",
        "the anakot gateway stopped responding, please investigate",
        # #92372 Branch D: `p?kill` without a leading \b matched the "kill"
        # tail of "skill".
        "anakot skill view gateway-notes && echo anakot gateway docs",
        # #77173/#77536: a file path with embedded spaces containing the
        # lifecycle words must not match — `anakot` is a path component
        # there, not a command.
        "cat '/docs/anakot gateway restart-notes.md'",
        "less /home/user/notes/anakot gateway restart runbook.txt",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "anakot gateway restart; echo done",
        "anakot gateway stop && echo stopped",
        # #77173 command-position anchor must not weaken separator/subshell
        # forms either.
        "true;anakot gateway restart",
        "true && anakot gateway stop",
        "echo $(anakot gateway restart)",
        "echo `anakot gateway restart`",
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'import subprocess\nsubprocess.run(["launchctl", "bootout", "gui/501/ai.anakot.gateway"])',
        'subprocess.run(["anakot", "gateway", "restart"])',
        'os.system("launchctl kickstart -k gui/501/ai.anakot.gateway")',
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        'data = ["anakot", "notes"]  # unrelated list',
    ])
    ...
    def test_inert_heredoc_body_prose_not_blocked(self):
        ...
        text = (
            ...
            "If the box is wedged, a human can run: anakot gateway restart\n"
            "EOF"
        )
        assert not _contains_gateway_lifecycle_command(text), f"Should NOT match: {text!r}"
...
class TestProfileFlagGatewayLifecycle:
    """#78028: `anakot -p <profile> gateway restart|stop` bypasses Branch A's
    ...
    def _pin_profile_identity(self, monkeypatch):
        # The ambient test env may carry ANAKOT_HOME/ANAKOT_PROFILE; pin the
        # profile identity explicitly so every assertion is deterministic.
        monkeypatch.setenv("ANAKOT_PROFILE", "zeus")
        monkeypatch.delenv("ANAKOT_PROFILE_NAME", raising=False)

    @pytest.mark.parametrize("text", [
        "anakot -p zeus gateway stop",
        "anakot -p zeus gateway restart",
        "anakot --profile zeus gateway restart",
        "anakot --profile zeus gateway stop",
        "anakot --profile=zeus gateway restart",
        # Global flags before/after the selector must not hide the shape.
        "anakot -v -p zeus gateway restart",
        "anakot -p zeus -v gateway restart",
        "anakot --debug --profile zeus gateway stop",
    ...
    @pytest.mark.parametrize("text", [
        "anakot -p venus gateway stop",
        "anakot -p venus gateway restart",
        "anakot --profile venus gateway restart",
        "anakot --profile=venus gateway stop",
        "anakot -p venus -v gateway restart",
    ])
    def test_sibling_allowed(self, text):
        assert not _contains_gateway_lifecycle_command(text), f"Should allow: {text!r}"

    @pytest.mark.parametrize("text", [
        "anakot -p zeus gateway start",
        "anakot -p zeus gateway start --all",
    ])
    ...
    def test_adjacent_form_still_blocked(self):
        ...
        assert _contains_gateway_lifecycle_command("anakot gateway restart")
        assert _contains_gateway_lifecycle_command("anakot gateway stop")

    def test_anakot_home_derived_profile(self, monkeypatch):
        # Without ANAKOT_PROFILE the guard falls back to the ANAKOT_HOME-
        # derived profile identity (get_active_profile_name) — the signal the
        # gateway process itself carries.
        monkeypatch.delenv("ANAKOT_PROFILE", raising=False)
        monkeypatch.delenv("ANAKOT_PROFILE_NAME", raising=False)
        import anakot_cli.profiles as profiles_mod

        monkeypatch.setattr(profiles_mod, "get_active_profile_name", lambda: "zeus")
        assert _contains_gateway_lifecycle_command("anakot -p zeus gateway restart")
        assert not _contains_gateway_lifecycle_command("anakot -p venus gateway restart")
    ...
    def test_no_profile_context_conservative_allow(self, monkeypatch):
        ...
        assert not _contains_gateway_lifecycle_command("anakot -p zeus gateway restart")
        assert _contains_gateway_lifecycle_command("anakot gateway restart")
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_anakot_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade anakot then run anakot gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_anakot_gateway_once.sh). The script must live under
        # ANAKOT_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / ".anakot"))
        scripts_dir = tmp_path / ".anakot" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_ANAKOT_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _ANAKOT_GATEWAY=1.

    Issue #37453: systemctl --user restart anakot-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart anakot-gateway",
        "systemctl --user restart anakot-gateway",
        "systemctl stop anakot-gateway.service",
        "anakot gateway restart",
        "anakot gateway uninstall",
        "launchctl kickstart gui/501/ai.anakot.gateway",
        "launchctl bootout gui/501/ai.anakot.gateway",
        # #62891 exact reported shape and its bootstrap sibling.
        "launchctl submit -l ai.anakot.gateway-hard-restart-no-photon-notice -- /bin/sh ~/.anakot/scripts/hard_restart_gateway_no_photon_notice.sh",
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl bootstrap gui/501 ~/Library/LaunchAgents/ai.anakot.gateway.restart-once.plist",
        "pkill -f anakot.*gateway",
    ])
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart anakot-gateway", force=True
        ))
    ...
    def test_blocks_launchctl_submit_inside_gateway(self, monkeypatch, tmp_path):
        ...
        result = json.loads(tt.terminal_tool(
            command=(
                "launchctl submit -l ai.anakot.delayed-ops -- "
                f"/bin/bash {script}"
            )
        ))
    ...
    @pytest.mark.parametrize("command", [
        # Neutral, non-anakot label: label-independent detection is the point
        # (#62891 second reproduction used `ai.anakot.svc-reload-tmp`).
        "launchctl submit -l com.foo -- /path/gateway",
        "launchctl submit -l ai.anakot.svc-reload-tmp -- /bin/sh /tmp/h-svc-reload.sh",
    ...
    def test_cli_agent_session_not_blocked_by_inherited_env(
        ...
        """#92560: CLI/TUI agent sessions inherit _ANAKOT_GATEWAY=1 from the
        ...
        # Simulate a CLI agent session: _ANAKOT_GATEWAY=1 is in the
        ...
        self._patch_env(monkeypatch, _FakeEnv(), inside_gateway=False)
        monkeypatch.setenv("_ANAKOT_GATEWAY", "1")
        monkeypatch.setattr(
            tt, "_check_all_guards", lambda cmd, env, **kwargs: {"approved": True}
        )

        result = json.loads(tt.terminal_tool(command="anakot gateway restart"))

        assert result["exit_code"] == 0
        assert calls == ["anakot gateway restart"]
    ...
    def test_blocks_launchctl_submit_hidden_in_referenced_script(
        ...
        script.write_text(
            "#!/bin/bash\nlaunchctl submit -l ai.anakot.loop -- /bin/true\n"
        )
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
    ...
    def test_launchctl_submit_parser_handles_shell_quoting(self, monkeypatch):
        ...
        self._patch_env(monkeypatch, self._make_fake_env(), inside_gateway=True)
        result = json.loads(tt.terminal_tool(
            command="launchctl sub\"\"mit -l ai.anakot.loop -- /bin/true"
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-anakot systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run anakot gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("anakot gateway stop\n", encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfeanakot gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("", str(script))


    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under ANAKOT_HOME/scripts (the
    ...
    def test_python_script_with_literal_lifecycle_command_still_blocked(
        ...
        script.write_text('import os\nos.system("anakot gateway restart")\n', encoding="utf-8")
        with pytest.raises(GatewayLifecycleBlocked):
            check_gateway_lifecycle("clean prompt", str(script))
    ...
    def test_read_referenced_script_tolerates_nul_in_path(self):
        ...
        text, unsafe = _read_referenced_script(Path("/tmp/anakot\x00binary"))
        assert text is None
    ...
    def test_binary_from_remote_callback_never_false_positives(self):
        ...
        def _remote_read(_path: str):
            return "MZ\x00\x00\x90\x00 anakot gateway restart \x00\x00junk"
    ...
    def test_cron_guard_total_when_home_unresolvable(self, monkeypatch):
        """`get_anakot_home()` falls back to Path.home(), which raises
        RuntimeError when neither ANAKOT_HOME nor HOME resolves
...
class TestRelativePathDoesNotDisableDataExemption:
    ...
    @pytest.mark.parametrize("command", [
        "grep -r 'systemctl restart anakot-gateway' .",
        "grep -rn 'anakot gateway restart' ./logs",
        "rg 'anakot gateway restart' ../archive",
        "grep -c 'systemctl stop anakot-gateway' ./var/log/syslog",
        "sqlite3 ./stats.db \"SELECT restart_reason FROM anakot_gateway_restarts\"",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 ./db ".shell anakot gateway restart"',
        'sqlite3 ./db ".system systemctl restart anakot-gateway"',
        'psql ./x -c "\\! systemctl restart anakot-gateway"',
        "grep -r 'anakot gateway restart' . | sh",
        "grep -r 'anakot gateway restart' ./logs | bash",
        "grep -r 'anakot gateway restart' . | sudo sh",
        "grep -r 'x' . ; anakot gateway restart",
        "grep -r 'x' . && systemctl restart anakot-gateway",
        'grep -r "$(anakot gateway restart)" .',
        "rg 'x' ./logs | xargs systemctl restart anakot-gateway",
    ])
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell anakot gateway restart"',
        'sqlite3 db ".system systemctl restart anakot-gateway"',
        'psql -c "\\! systemctl restart anakot-gateway"',
    ])
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    anakot_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run anakot gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / ".anakot"))
        (tmp_path / ".anakot").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run anakot gateway restart nightly",
        ))
...
class TestTerminalToolGatewayLifecycleGuardRemote:
    ...
    def test_unscannable_executed_script_names_the_reason(self, monkeypatch, tmp_path):
        ...
        import tools.terminal_tool as tt
        from anakot_cli.sqlite_safe_read import connect_tracked
...
class TestLifecycleGuardDataArgumentExemption:
    ...
    @pytest.mark.parametrize("command", [
        ...
        "'systemctl stop anakot-gateway'\"",
        # grep/rg pattern arguments hunting for the lifecycle string.
        "grep -c 'systemctl restart anakot-gateway' /var/log/syslog",
        "rg 'anakot gateway restart' /home/user/.anakot/logs/",
        "journalctl -u anakot-gateway --grep 'systemctl restart anakot-gateway'",
        # SQL with stop/restart column/value words but no command shape.
        'sqlite3 stats.db "SELECT stop_time, restart_reason FROM '
        'anakot_gateway_restarts"',
    ...
    @pytest.mark.parametrize("command", [
        ...
        'sqlite3 db ".shell anakot gateway restart"',
        'psql -c "\\! systemctl restart anakot-gateway"',
        "grep 'systemctl restart anakot-gateway' cmds.txt | sh",
        "grep gateway f | xargs systemctl restart anakot-gateway",
        'grep "$(systemctl restart anakot-gateway)" f',
        "grep 'restart' log; systemctl restart anakot-gateway",
        'sqlite3 db "SELECT 1"; anakot gateway stop',
        # Plain lifecycle commands are unaffected by the exemption.
        "anakot gateway restart",
        "sudo systemctl stop anakot-gateway",
    ])
    ...
    def test_python_script_branch_gets_the_same_exemption(self, tmp_path):
        ...
        prompt = (
            ...
            "WHERE msg LIKE '%systemctl restart anakot-gateway%'\""
        )
