"""Tests for the update check mechanism in anakot_cli.banner.
...
def git_repo(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "anakot-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    monkeypatch.delenv("ANAKOT_REVISION", raising=False)
    ...
    monkeypatch.setattr("anakot_cli.config.get_project_root", lambda: repo_dir)
    return repo_dir


def _stub_git(monkeypatch, *, head=SHA_A, origin="https://github.com/Chensihakniroth/anakot-agent-v1.git"):
    calls = []
...
def test_passive_check_uses_the_api_and_never_fetches(git_repo, monkeypatch):
    ...
    tip.assert_called_once_with("nousresearch/anakot-agent", "main")
...
def test_cache_is_daily_but_invalidated_when_head_moves(git_repo, monkeypatch):
    """A fresh cache answers without any network; ``anakot update`` moving HEAD busts it at once;
