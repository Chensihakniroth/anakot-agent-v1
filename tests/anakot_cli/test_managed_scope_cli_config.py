...
anakot_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `anakot config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_anakot_home = get_anakot_home()`` at import time (module
    singleton), so monkeypatching ANAKOT_HOME after import doesn't move it.
    ...
    this only matters for tests that swap ANAKOT_HOME.)
    """
    import cli

    cli._anakot_home = home
