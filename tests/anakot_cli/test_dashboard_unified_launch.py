...
class TestUnifiedDashboardRouting:
    ...
    def test_profile_launch_reexecs_machine_dashboard(self, main_mod, monkeypatch):
        ...
        monkeypatch.setattr(
            "anakot_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
        ...
        # ANAKOT_HOME.  For a standard install (ANAKOT_HOME unset) that root is
        # the platform-native default (~/.anakot), NOT dropped — see the Docker
        # test below for why we resolve explicitly instead of popping.
        from anakot_constants import get_default_anakot_root
        assert env.get("ANAKOT_HOME") == str(get_default_anakot_root())


    def test_desktop_profile_backend_skips_machine_dashboard_reroute(self, main_mod, monkeypatch):
        """A desktop-spawned named-profile backend (ANAKOT_DESKTOP=1) must NOT
