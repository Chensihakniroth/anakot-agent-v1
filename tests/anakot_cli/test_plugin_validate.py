"""Tests for ``anakot plugins validate`` (anakot_cli/plugin_validate.py).
...
def test_requires_anakot_spec_is_validated(tmp_path):
    manifest = dict(BASE_MANIFEST, requires_anakot=">=0.21")
    d = _make_plugin(tmp_path, manifest=manifest)

    report = validate_plugin_dir(d)

    assert report.ok, report.failures
    assert ("requires_anakot", True, "spec '>=0.21' parses") in report.checks
...
class TestRequiresAnakotSpec:
    """A typo'd ``requires_anakot`` clause must fail admission, not silently gate nothing."""

    def test_typoed_clause_fails_admission(self, tmp_path):
        d = _make_plugin(
            tmp_path, manifest={**BASE_MANIFEST, "requires_anakot": ">=0.21.1,<0.x"}
        )
        report = validate_plugin_dir(d)
        assert not report.ok
        assert any(
            "requires_anakot" in f and "does not parse" in f for f in report.failures
        ), report.failures
...
class TestDesktopSurface:
    ...
    def test_sdk_only_plugin_passes(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            "import { definePlugin } from '@anakot/plugin-sdk'\n"
    ...
    def test_prototype_patch_and_chunk_import_fail(self, tmp_path):
        d = self._desktop_plugin(tmp_path, (
            ...
            "const sdk = await import('@anakot/plugin-sdk')\n"
        ))
        report = validate_plugin_dir(d)
