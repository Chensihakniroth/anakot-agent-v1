"""Tests for the unified provider catalog (anakot_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``anakot model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from anakot_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
