"""Tests for anakot_cli.gateway_windows."""
...
import anakot_cli.gateway as gateway
import anakot_cli.gateway_windows as gateway_windows
import anakot_cli.setup as setup


_BREAKAWAY_MARKER = "_ANAKOT_GATEWAY_BREAKAWAY"
...
def test_build_gateway_argv_keeps_venv_console_python_for_uv_venv(monkeypatch, tmp_path):
    ...
    anakot_home = tmp_path / "anakot-home"
    ...
    monkeypatch.setattr(gateway, "_profile_arg", lambda anakot_home: "")
    monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: str(anakot_home))

    argv, cwd, env_overlay = gateway_windows._build_gateway_argv()

    assert argv[:3] == [str(venv_python), "-m", "anakot_cli.main"]
    assert cwd == str(anakot_home.resolve())
...
def test_spawn_detached_marks_primary_breakaway_success(monkeypatch, tmp_path, caplog):
    ...
    argv = ["python.exe", "-m", "anakot_cli.main", "gateway", "run"]
    cwd = str(tmp_path)
    calls = []
    ...
    monkeypatch.setattr(
        gateway_windows,
        "_build_gateway_argv",
        lambda home=None: (argv, cwd, {"ANAKOT_GATEWAY_DETACHED": "1"}),
    )
    monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: tmp_path)
...
def test_spawn_detached_warns_and_marks_no_breakaway_fallback(
    ...
    monkeypatch.setattr(
        ...
        lambda home=None: (
            argv,
            cwd,
            {"ANAKOT_GATEWAY_DETACHED": "1", "SECRET_SENTINEL": "do-not-log"},
        ),
    )
    monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: tmp_path)
    monkeypatch.setattr(gateway_windows.subprocess, "Popen", fake_popen)
...
class TestStableWindowsGatewayWorkingDir:
    def test_stable_gateway_working_dir_uses_anakot_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".anakot"
        home.mkdir()
        monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: home)
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".anakot"
        project = tmp_path / "checkout"
        monkeypatch.setattr("anakot_cli.config.get_anakot_home", lambda: missing)
...
def _arrange_startup_fallback(monkeypatch, tmp_path, running_pids):
    script_path = tmp_path / "Anakot_Gateway_alice.cmd"
    startup_entry = tmp_path / "Startup" / "Anakot_Gateway_alice.cmd"
    calls = []

    monkeypatch.setattr(gateway_windows, "_prompt_install_choices", lambda *args, **kwargs: (False, True))
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
    monkeypatch.setattr(gateway_windows, "get_task_name", lambda: "Anakot_Gateway_alice")
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Anakot\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Anakot\venv\Scripts\python.exe"
...
def test_install_scheduled_task_recreates_instead_of_change(monkeypatch, tmp_path):
    ...
    ok, detail = gateway_windows._install_scheduled_task("Anakot_Gateway_alice", script_path)

    assert ok is True
    assert "/Change" not in [arg for call in calls for arg in call]
    assert calls[0][:4] == ("/Delete", "/F", "/TN", "Anakot_Gateway_alice")
    ...
    assert "Anakot_Gateway_alice.vbs" in xml_seen["text"]
...
def test_gateway_vbs_script_is_console_less(monkeypatch):
    ...
    assert "anakot_cli.main" in content
    assert "gateway run" in content
    assert ", 0, False" in content  # hidden window, detached/async
    for var in ("ANAKOT_HOME", "PYTHONIOENCODING", "ANAKOT_GATEWAY_DETACHED", "VIRTUAL_ENV", "PYTHONPATH"):
...
def test_atomic_write_leaves_no_staging_file_when_swap_fails(monkeypatch, tmp_path):
    ...
    startup.mkdir()
    entry, staging = startup / "Anakot_Gateway.vbs", startup / "Anakot_Gateway.tmp"
...
def test_uninstall_and_reinstall_sweep_stale_startup_staging_file(monkeypatch, tmp_path):
    ...
    startup = tmp_path / "Startup"
    startup.mkdir()
    entry, staging = startup / "Anakot_Gateway_alice.vbs", startup / "Anakot_Gateway_alice.tmp"
    script = tmp_path / "task" / "Anakot_Gateway_alice.cmd"
    ...
    monkeypatch.setattr(gateway_windows, "_legacy_startup_entry_path", lambda: startup / "Anakot_Gateway_alice.cmd")
...
<Task version="1.3" xmlns="http://schemas.microsoft.com/windows/2004/02/mit/task">
  ...
  <Actions Context="Author">
    <Exec>
      ...
      <Arguments>"C:\\Users\\me\.anakot\\gateway-service\\Anakot_Gateway.vbs"</Arguments>
    </Exec>
  </Actions>
</Task>
"""
...
def test_scheduled_task_drift_names_missing_hardening_leaves(monkeypatch):
    ...
    ``anakot gateway status`` prints together with the ``anakot gateway install`` repair hint."""
    launcher = Path(r"C:\Users\me.anakot\gateway-service\Anakot_Gateway.vbs")
    template = gateway_windows._build_scheduled_task_xml("Anakot_Gateway", launcher, r"PC\me")
...
def test_reconcile_scheduled_task_reregisters_only_on_drift(monkeypatch, tmp_path):
    ...
    assert gateway_windows.reconcile_scheduled_task("Anakot_Gateway") is True
    ...
    calls.clear()
    assert gateway_windows.reconcile_scheduled_task("Anakot_Gateway") is False
...
def _arrange_uninstalled_start(monkeypatch):
    ...
    installs, spawns = [], []
    monkeypatch.delenv("ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN", raising=False)
    monkeypatch.delenv("ANAKOT_NONINTERACTIVE", raising=False)
    monkeypatch.setattr(gateway_windows, "_assert_windows", lambda: None)
...
def test_stdin_interactive_only_when_isatty_and_a_console_answers_get_console_mode():
    """Windows CRT isatty() is True for the NUL device (`anakot gateway start < NUL`, stdin=DEVNULL), so
...
def test_start_with_nul_stdin_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    ...
    assert "anakot gateway install" in capsys.readouterr().out


def test_start_without_tty_starts_the_gateway_but_never_installs_login_persistence(monkeypatch, capsys):
    """`anakot gateway start < /dev/null` must not answer the persistence question with a default Yes
    ...
    assert "anakot gateway install" in out and "did not complete" not in out
...
def test_start_on_tty_hands_both_answers_to_install_and_honours_the_env_opt_out(monkeypatch):
    ...
    again). ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN=0 → no question, no install, a plain start."""
    installs, spawns = _arrange_uninstalled_start(monkeypatch)
    monkeypatch.setattr(setup, "is_interactive_stdin", lambda: True)
    ...
    installs.clear()
    monkeypatch.setenv("ANAKOT_GATEWAY_INSTALL_START_ON_LOGIN", "0")
...
# resume_pending=True) never fires when `anakot gateway stop` kills the
...
def test_anakot_owns_windows_service_requires_name_or_binary_under_a_anakot_root():
    """Task Scheduler (``Schedule`` in svchost) above a task-launched gateway is never its supervisor;
    a service is Anakot-owned only by a ``anakot*`` name or a binary under the install (#97208)."""
    roots = (
        r"C:\Users\kaize\AppData\Local\anakot\anakot-agent",
        r"C:\Users\kaize\AppData\Local\anakot\anakot-agent\venv\Scripts",
        r"C:\Users\kaize\AppData\Local\anakot\gateway-service",
    )
    owns = gateway_windows.anakot_owns_windows_service
    ...
    assert not owns("Other", r"C:\Users\kaize\AppData\Local\anakot\anakot-agent-fork\run.exe", roots)

    assert owns("AnakotGateway", r"C:\nssm\nssm.exe", roots)
    assert owns("Anakot_Gateway_derek", "", roots)
    assert owns("gw", r'"C:\Users\KAIZE\AppData\Local\anakot\anakot-agent\venv\Scripts\anakot.exe" gateway run', roots)
    assert owns("gw", r"C:\Users\kaize\AppData\Local\anakot\gateway-service\Anakot_Gateway.cmd", roots)
