...
def _stale_plist(tmp_path, monkeypatch, *, registered: bool):
    plist_path = tmp_path / "com.anakot.plist"
    ...
    monkeypatch.setattr(gw, "get_launchd_label", lambda: "com.anakot.agent")
...
def test_install_repair_warns_instead_of_claiming_success(tmp_path, monkeypatch, capsys):
    ...
    monkeypatch.setattr("anakot_constants.display_anakot_home", lambda: "~/.anakot-work")
    ...
    assert "~/.anakot-work/logs/launchd-reload.log" in out
