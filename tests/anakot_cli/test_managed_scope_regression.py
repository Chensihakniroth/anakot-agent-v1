...
def anakot_home(tmp_path, monkeypatch):
    ...
    # /etc/anakot on the dev/CI box can't influence the test.
    monkeypatch.setenv("ANAKOT_MANAGED_DIR", str(tmp_path / "no_such_managed_dir"))
...
def test_user_config_overrides_default(anakot_home, monkeypatch):
...
def test_env_expansion_in_user_config(anakot_home, monkeypatch):
    from anakot_cli.config import load_config, cfg_get
...
def test_user_env_overrides_shell(tmp_path, monkeypatch):
    ...
    load_anakot_dotenv(anakot_home=str(home))
