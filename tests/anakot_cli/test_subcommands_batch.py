...
from anakot_cli.subcommands.dashboard import build_dashboard_parser
...
from anakot_cli.subcommands.whatsapp import build_whatsapp_parser
...
def test_config_get_unset_subcommands_parse():
    """`anakot config get/unset` parse key args (and --json for get)."""
    parser = argparse.ArgumentParser(prog="anakot")
...
# ── deprecated `anakot login` fails gracefully, not with argparse error ────
#
# `anakot login` is a removed command; its handler (`login_command` in
# `anakot_cli/auth.py`) prints a deprecation notice pointing at `anakot auth` /
# `anakot model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `anakot --help`.
