...
def container_env(tmp_path, monkeypatch):
    """Set up a fake ANAKOT_HOME with .container-mode file."""
    anakot_home = tmp_path / ".anakot"
    anakot_home.mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.delenv("ANAKOT_DEV", raising=False)

    container_mode = anakot_home / ".container-mode"
    container_mode.write_text(
        "# Written by NixOS activation script. Do not edit manually.\n"
        "backend=podman\n"
        "container_name=anakot-agent\n"
        "exec_user=anakot\n"
        "anakot_bin=/data/current-package/bin/anakot\n"
    )
    return anakot_home
...
def test_get_container_exec_info_returns_metadata(container_env):
    ...
    with patch("anakot_constants.is_container", return_value=False):
        info = get_container_exec_info()

    assert info is not None
    assert info["backend"] == "podman"
    assert info["container_name"] == "anakot-agent"
    assert info["exec_user"] == "anakot"
    assert info["anakot_bin"] == "/data/current-package/bin/anakot"
...
def test_exec_in_container_calls_execvp(docker_container_info):
    ...
    from anakot_cli.main import _exec_in_container
    ...
    assert "-it" in cmd
    idx_u = cmd.index("-u")
    assert cmd[idx_u + 1] == "anakot"
    e_indices = [i for i, v in enumerate(cmd) if v == "-e"]
    e_values = [cmd[i + 1] for i in e_indices]
    assert "TERM=xterm-256color" in e_values
    assert "LANG=en_US.UTF-8" in e_values
    assert "anakot-agent" in cmd
    assert "/data/current-package/bin/anakot" in cmd
    assert "chat" in cmd
