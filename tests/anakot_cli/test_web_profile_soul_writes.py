...
def client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_DASHBOARD_SESSION_TOKEN", "soul-test-token")
...
def profile_dir(tmp_path, monkeypatch) -> Path:
    """Create a real profile directory under the test ANAKOT_HOME."""
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
...
class TestSoulWriteDurability:
    ...
    def test_existing_soul_survives_an_interrupted_save(
        ...
        # ANAKOT_HOME patch the client/profile_dir fixtures installed.
...
class TestSoulIoIsOffTheEventLoop:
    ...
    ``tests/anakot_cli/test_web_profiles_off_loop.py``.
    """
