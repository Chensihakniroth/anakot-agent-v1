"""Guards for anakot_cli._startup_fast — the pre-import version fast path.
...
1. IMPORT WEIGHT: _startup_fast must stay stdlib-only. The whole point of
   ...
   ``from anakot_cli.config import ...`` silently makes `anakot --version`
...
def test_startup_fast_import_weight():
    ...
    probe = (
        "import sys, json\n"
        "import anakot_cli._startup_fast\n"
    ...
    assert not offenders, (
        f"anakot_cli._startup_fast imported heavy modules: {offenders} — "
...
def _run_version(env_overrides: dict) -> subprocess.CompletedProcess:
    ...
    env.pop("ANAKOT_DEV", None)
    return subprocess.run(
        [sys.executable, "-m", "anakot_cli.main", "--version"],
        capture_output=True,
        text=True,
        timeout=60,
        cwd=REPO_ROOT,
...
def test_fast_version_parity_off_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version({"ANAKOT_HOME": str(home), "TERMUX_VERSION": ""})
    assert result.returncode == 0, result.stderr
    out = result.stdout
    for field in ("Anakot Agent v", "Install directory:", "Python:", "OpenAI SDK:"):
...
def test_fast_version_parity_on_termux(tmp_path):
    ...
    home.mkdir()
    result = _run_version(
        {"ANAKOT_HOME": str(home), "TERMUX_VERSION": "0.118"}
    )
    assert result.returncode == 0, result.stderr
    assert "Anakot Agent v" in result.stdout
...
def test_literal_tilde_anakot_home_expands_before_any_reader(tmp_path):
    """A literal ``~`` in ANAKOT_HOME (fish, or any quoted value) is expanded at process entry.
    ...
    fake_home.mkdir()
    cwd.mkdir()
    env = {**os.environ, "HOME": str(fake_home), "USERPROFILE": str(fake_home), "ANAKOT_HOME": "~/.x"}
    env.pop("ANAKOT_DEV", None)
    env["PYTHONPATH"] = str(REPO_ROOT)
    result = subprocess.run(
        [sys.executable, "-m", "anakot_cli.main", "config", "path"],
    ...
    # Raw-reader observable: the ~30 ``os.environ["ANAKOT_HOME"]`` readers in anakot_cli/ never
    # call the resolver, so the entry-point hunk in main.py (not anakot_constants) must have
    # rewritten the env var by the time the module import finishes.
    probe = subprocess.run(
        [sys.executable, "-c", "import anakot_cli.main, os; print(os.environ['ANAKOT_HOME'])"],
        capture_output=True, text=True, timeout=120, cwd=cwd, env=env,
    )
...
def test_normalize_anakot_home_env_rewrites_tilde_and_leaves_absolute_alone(tmp_path, monkeypatch):
    from anakot_cli import _startup_fast

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("USERPROFILE", str(tmp_path))
    monkeypatch.setenv("ANAKOT_HOME", "~/.x")
    _startup_fast.normalize_anakot_home_env()
    assert os.environ["ANAKOT_HOME"] == str(tmp_path / ".x")

    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path / "abs"))
    _startup_fast.normalize_anakot_home_env()
    assert os.environ["ANAKOT_HOME"] == str(tmp_path / "abs")

    monkeypatch.delenv("ANAKOT_HOME")
    _startup_fast.normalize_anakot_home_env()
    assert "ANAKOT_HOME" not in os.environ
