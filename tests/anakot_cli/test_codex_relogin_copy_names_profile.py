...
Profiles are islands (93889b770da): a bare ``anakot auth`` / ``anakot model`` from a named
profile's error text re-signs the ROOT store, which is exactly the loop #114012 measured.
Every relogin hint raised by ``anakot_cli/auth_codex.py`` and appended by
``format_auth_error`` must carry the ``-p <profile>`` selector under a profile ANAKOT_HOME.
"""
...
from anakot_cli.auth import AuthError, _read_codex_tokens, format_auth_error
from anakot_cli.auth_codex import _codex_refresh_failure_error
...
def test_missing_and_reused_codex_credential_copy_names_the_profile(codex_profile_home):
    ...
    for err in (missing.value, reused):
        text = str(err)
        assert "`anakot -p codex auth add openai-codex --type oauth`" in text, text
        assert "`anakot auth`" not in text, text
...
def test_format_auth_error_relogin_suffix_names_the_profile(codex_profile_home):
    ...
    assert "`anakot -p codex model`" in rendered, rendered
    assert "`anakot model`" not in rendered, rendered
