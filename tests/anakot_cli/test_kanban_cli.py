"""Tests for the kanban CLI surface (anakot_cli.kanban)."""
...
def test_worker_link_preserves_foreign_child_rules(kanban_home, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_KANBAN_TASK", worker)
    monkeypatch.setenv("ANAKOT_KANBAN_RUN_ID", str(worker_run_id))
    ...
    # with ANAKOT_KANBAN_RUN_ID — the one path _cmd_link forwards a run id for.
    assert kc._cmd_link(argparse.Namespace(
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="anakot", add_help=False)
