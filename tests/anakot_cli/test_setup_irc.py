"""Tests for IRC gateway configuration via `anakot setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Anakot install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Anakot install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#anakot")
            monkeypatch.setenv("IRC_NICKNAME", "anakot-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `anakot setup gateway` experience with IRC present."""
