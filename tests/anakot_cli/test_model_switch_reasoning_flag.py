...
The parser is the single owner (anakot_cli.model_switch.parse_model_switch_args); the CLI and
...
def test_cli_commit_applies_effort_after_the_agent_swap(monkeypatch):
    ...
    cli._stage_and_swap_model = lambda result, old: cli_mod.AnakotCLI._stage_and_swap_model(cli, result, old)
    monkeypatch.setattr(mixin, "_print_switch_summary", lambda *_a, **_k: None)
    monkeypatch.setattr(cli_mod.AnakotCLI, "_persist_model_switch_to_session", lambda *_a: None)
    saved = {}
