"""Tests for anakot backup and import commands."""
...
def _advance_backup_clock(seconds: float = 1.1) -> None:
    """Skew anakot_cli.backup's datetime forward instead of sleeping.
    ...
    import anakot_cli.backup as _backup

    shim = getattr(_backup.datetime, "_anakot_test_shim", None)
    if shim is None:
        class _ShimDatetime(_dt.datetime):
            _anakot_test_shim = True
...
def _make_anakot_tree(root: Path) -> None:
    """Create a realistic ~/.anakot directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    for db_name in ("memory_store.db", "anakot_state.db"):
    ...
    # anakot-agent repo (should be EXCLUDED)
    (root / "anakot-agent").mkdir(exist_ok=True)
    (root / "anakot-agent" / "run_agent.py").write_text("# big file\n")
    (root / "anakot-agent" / ".git").mkdir()
    (root / "anakot-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_anakot_agent(self):
        from anakot_cli.backup import _should_exclude
        assert _should_exclude(Path("anakot-agent/run_agent.py"))
        assert _should_exclude(Path("anakot-agent/.git/HEAD"))
    ...
    def test_excludes_state_snapshots_dir(self):
        ...
        from anakot_cli.backup import _EXCLUDED_DIRS, _QUICK_SNAPSHOTS_DIR, _should_exclude
    ...
    def test_excludes_desktop_emergency_state_db_baks(self):
        ...
        state.db.pre-update-emergency-*.bak files at the ANAKOT_HOME root —
...
class TestIterBackupFiles:
    def test_manual_and_automatic_paths_share_one_walk(self, tmp_path):
        ...
        ``anakot-agent`` at ANY depth, silently dropping nested skill dirs
        like ``skills/autonomous-ai-agents/anakot-agent/`` that the manual
        path preserved. One shared iterator makes that drift impossible;
        this test pins the contract."""
        from anakot_cli.backup import _iter_backup_files

        root = tmp_path / ".anakot"
        root.mkdir()
        _make_anakot_tree(root)

        # The case the old automatic walk got wrong: a nested dir named
        # anakot-agent holding real skill content.
        nested = root / "skills" / "autonomous-ai-agents" / "anakot-agent"
        ...
        rel_nested = str(Path("skills/autonomous-ai-agents/anakot-agent/SKILL.md"))
        assert rel_nested in selected
        assert str(Path("models/big.gguf")) not in selected
        assert not any(s.startswith("anakot-agent") for s in selected)
    ...
    def test_skipped_dirs_collected_for_summary(self, tmp_path):
        ...
        assert "anakot-agent" in skipped
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import anakot_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = anakot_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, anakot_home)
    ...
    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside ANAKOT_HOME."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        _make_anakot_tree(anakot_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(anakot_home / "skills" / "outside-link.txt", outside)
        ...
        from anakot_cli.backup import run_backup
        run_backup(args)
    ...
    def test_state_snapshots_not_nested_into_backup(self, tmp_path, monkeypatch):
        ...
        anakot_home.mkdir()
        _make_anakot_tree(anakot_home)
        with sqlite3.connect(anakot_home / "state.db") as conn:
            conn.execute("CREATE TABLE sessions (id TEXT PRIMARY KEY)")
            conn.execute("INSERT INTO sessions VALUES ('s1')")

        from anakot_cli.backup import _QUICK_SNAPSHOTS_DIR, create_quick_snapshot, run_backup
        ...
        assert snap_id and (anakot_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Anakot backup."""
        from anakot_cli.backup import _validate_backup_zip
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Anakot configuration has been restored." in out
        assert "anakot gateway install" in out
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        (anakot_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (anakot_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (anakot_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            anakot_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (anakot_home / "gateway.pid").write_text("4242")
        (anakot_home / "processes.json").write_text('{"live": true}')
        ...
        assert (anakot_home / "gateway.pid").read_text() == "4242"
        assert (anakot_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (anakot_home / "cron.pid").exists()
        assert not (anakot_home / "gateway.lock").exists()
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (anakot_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".anakot"
        src_home.mkdir(parents=True)
        _make_anakot_tree(src_home)

        monkeypatch.setenv("ANAKOT_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from anakot_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".anakot"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("ANAKOT_HOME", str(dst_home))
        ...
        # anakot-agent should NOT be present
        assert not (dst_home / "anakot-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from anakot_cli.sizefmt import format_bytes as _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_only_dirs(self):
        ...
        import io
        from anakot_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            # Only directory entries (trailing slash)
            zf.writestr(".anakot/", "")
            zf.writestr(".anakot/skills/", "")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
...
class TestBackupEdgeCases:
    ...
    def test_incomplete_archive_is_kept_but_reported_as_failure(self, tmp_path, monkeypatch, capsys):
        ...
        anakot_home.mkdir()
        _make_anakot_tree(anakot_home)
        unreadable = anakot_home / "skills" / "locked.md"
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        from anakot_cli.backup import _RUN_BACKUP_PREFIX, run_backup
        from anakot_cli.main import cmd_backup
    ...
    def test_empty_anakot_home(self, tmp_path, monkeypatch):
        """Backup handles empty anakot home (no files to back up)."""
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        # Only excluded dirs, no actual files
        (anakot_home / "__pycache__").mkdir()
        (anakot_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        (anakot_home / "config.yaml").write_text("model: test\n")

        # Create a file with epoch timestamp (1970-01-01)
        old_file = anakot_home / "ancient.txt"
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        (anakot_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (anakot_home / "config.yaml").exists()
        assert (anakot_home / "sessions" / "s0599.json").exists()
...
class TestImportAtomicWrites:
    """`anakot import` must never leave a user's file truncated.
    ...
    tests pin the invariant for both restore branches: the ANAKOT_HOME branch
    ...
    def test_failed_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        ...
        (anakot_home / "config.yaml").write_text(original)
        monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (anakot_home / "config.yaml").read_text() == original
        # And the aborted write must not litter the directory it staged in.
        assert list(anakot_home.glob(".config.yaml.*")) == []

    def test_failed_external_member_leaves_existing_file_intact(self, tmp_path, monkeypatch):
        """Same invariant on the `_external/` branch, which writes outside ANAKOT_HOME."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        anakot_home = dst_home / ".anakot"
        anakot_home.mkdir()
    ...
    def test_restore_preserves_existing_file_mode(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir()
        target = anakot_home / "config.yaml"
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo anakot import`` onto a user-owned (or Docker/NAS volume-owned)
        ANAKOT_HOME would hand every restored file to root. The uid/gid is
        ...
        monkeypatch.setattr(
            "anakot_cli.backup._preserve_file_owner",
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``anakot import`` is the
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from anakot_cli.backup import _safe_copy_db
    ...
    def test_is_zeroed_sqlite_file_detects_nul_header(self, tmp_path):
        from anakot_cli.backup import is_zeroed_sqlite_file
        p = tmp_path / "state.db"
...
class TestQuickSnapshot:
    @pytest.fixture
    def anakot_home(self, tmp_path):
        """Create a fake ANAKOT_HOME with critical state files."""
    ...
    def test_state_db_safely_copied(self, anakot_home):
        from anakot_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(anakot_home=anakot_home)
        db_copy = anakot_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_failed_state_db_copy_is_loud(self, anakot_home, monkeypatch, capsys):
    ...
    def test_restore_refused_db_is_not_counted(self, anakot_home, monkeypatch):
        ...
        restore_quick_snapshot(snap_id, anakot_home=anakot_home)

        manifest = json.loads(
            (backup_mod._quick_snapshot_root(anakot_home) / snap_id / "manifest.json").read_text()
        )
    ...
    def test_restore_state_db_live_connection(self, anakot_home):
        ...
        live_conn = sqlite3.connect(str(anakot_home / "state.db"))
        ...
        result = restore_quick_snapshot(snap_id, anakot_home=anakot_home)
        assert result is True
    ...
    def test_snapshot_includes_pairing_directories(self, anakot_home):
        ...
        (anakot_home / "platforms" / "pairing").mkdir(parents=True)
        (anakot_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (anakot_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (anakot_home / "pairing").mkdir()
        (anakot_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (anakot_home / "feishu_comment_pairing.json").write_text(
        ...
        snap_dir = anakot_home / "state-snapshots" / snap_id
...
# ---------------------------------------------------------------------------
# Pre-update backup (anakot update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_oversized_db_suppresses_pruning(self, anakot_home, capsys):
        ...
        """
        import json
        from anakot_cli.backup import create_quick_snapshot, list_quick_snapshots

        # First snapshot: complete (state.db is small, under any cap)
        first_id = create_quick_snapshot(label="complete", anakot_home=anakot_home)
        assert first_id is not None
        first_dir = anakot_home / "state-snapshots" / first_id
        ...
        second_id = create_quick_snapshot(
            label="oversized", anakot_home=anakot_home, max_file_size=1024, keep=1
        )
        assert second_id is not None
        second_dir = anakot_home / "state-snapshots" / second_id
        ...
        all_snaps = list_quick_snapshots(limit=100, anakot_home=anakot_home)
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_non_default_kanban_board_snapshotted(self, anakot_home):
        ...
        board_dir = anakot_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(anakot_home=anakot_home)
        copy = (
            anakot_home / "state-snapshots" / snap_id
        ...
        assert restore_quick_snapshot(snap_id, anakot_home=anakot_home) is True
        conn = sqlite3.connect(str(board_dir / "kanban.db"))
    ...
    def test_board_db_copied_wal_safely(self, anakot_home, monkeypatch):
        ...
        import anakot_cli.backup as bk
        from anakot_cli.backup import create_quick_snapshot

        board = anakot_home / "kanban" / "boards" / "work"
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``anakot update``
    ...
    def test_backup_contents_match_full_backup(self, anakot_home):
        """Pre-update backup should include the same user data that
        ``anakot backup`` would, and should exclude the same directories."""
        from anakot_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(anakot_home=anakot_home)
        ...
        # anakot-agent repo excluded
        assert not any(n.startswith("anakot-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, anakot_home):
        """``anakot update`` in ``full`` mode takes the quick snapshot *before*
        ...
        snap_id = create_quick_snapshot(label="pre-update", anakot_home=anakot_home)
        assert snap_id and (anakot_home / _QUICK_SNAPSHOTS_DIR / snap_id / "state.db").exists()
    ...
    def test_rotation_keeps_only_n(self, anakot_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(anakot_home=anakot_home, keep=3)
            created.append(out)
            _advance_backup_clock()

        remaining = sorted(
            p.name for p in (anakot_home / "backups").iterdir()
    ...
    def test_skips_symlinked_files(self, anakot_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside ANAKOT_HOME."""
        from anakot_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def anakot_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".anakot"
        root.mkdir()
        _make_anakot_tree(root)
        # Point ANAKOT_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("ANAKOT_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Config reads resolve ANAKOT_HOME dynamically and their caches are
    ...
    @staticmethod
    def _set_mode(anakot_home, value):
        import yaml
    ...
    @staticmethod
    def _zips(anakot_home):
        d = anakot_home / "backups"
        return list(d.glob("pre-update-*.zip")) if d.exists() else []

    @staticmethod
    def _snaps(anakot_home):
        d = anakot_home / "state-snapshots"
        return [p for p in d.iterdir() if p.is_dir()] if d.exists() else []




    def test_config_off_disables_everything_silently(self, anakot_home, capsys):
        ...
        self._set_mode(anakot_home, "off")
        from anakot_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is None
        assert out == ""
        assert not self._snaps(anakot_home)
        assert not self._zips(anakot_home)



    def test_config_full_mode(self, anakot_home, capsys):
        self._set_mode(anakot_home, "full")
        from anakot_cli.update_cmd import _run_pre_update_backup
        snap_id = _run_pre_update_backup(Namespace(no_backup=False, backup=False))
        out = capsys.readouterr().out
        assert snap_id is not None
        ...
        assert len(self._zips(anakot_home)) == 1




# ---------------------------------------------------------------------------
# Pre-migration backup (anakot claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``anakot claw migrate`` runs before mutating ~/.anakot/."""
    ...
    def test_restorable_with_anakot_import(self, anakot_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Anakot
        backup — `anakot import` should accept it."""
        from anakot_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(anakot_home=anakot_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, anakot_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from anakot_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(anakot_home=anakot_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        for _ in range(3):
            out = create_pre_migration_backup(anakot_home=anakot_home, keep=1)
            assert out is not None
            _advance_backup_clock()
...
class TestRestoreCronJobsIfEmptied:
    """`anakot update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, anakot_home: Path, label="pre-update"):
        from anakot_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, anakot_home=anakot_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from anakot_cli.backup import restore_cron_jobs_if_emptied
        anakot_home = tmp_path / ".anakot"
        jobs_path = anakot_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(anakot_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, anakot_home=anakot_home)
        assert result is not None
...
class TestRestoreConfigModelSettingsIfRewritten:
    ...
    def _seed(self, anakot_home: Path) -> Path:
        anakot_home.mkdir(parents=True, exist_ok=True)
        cfg = anakot_home / "config.yaml"
        cfg.write_text(self.USER_CONFIG, encoding="utf-8")
        return cfg

    def test_restores_rewritten_provider_and_dropped_moa(self, tmp_path):
        import yaml
        from anakot_cli.backup import restore_config_model_settings_if_rewritten

        anakot_home = tmp_path / ".anakot"
        cfg = self._seed(anakot_home)
        snap_id = self._make_snapshot(anakot_home)
        assert snap_id
        ...
        result = restore_config_model_settings_if_rewritten(
            snap_id, anakot_home=anakot_home
        )
        assert result is not None
        assert result["restored"] is True
        assert result["snapshot_id"] == snap_id
    ...
    def test_noop_when_user_never_set_protected_keys(self, tmp_path):
        ...
        anakot_home = tmp_path / ".anakot"
        anakot_home.mkdir(parents=True)
        cfg = anakot_home / "config.yaml"
...
# location, NOT under ANAKOT_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, anakot_home: Path) -> None:
        anakot_home.mkdir(parents=True, exist_ok=True)
        (anakot_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (anakot_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (anakot_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_skips_external_paths_outside_home(self, tmp_path, monkeypatch):
        ...
        anakot_home = tmp_path / ".anakot"
        self._make_min_tree(anakot_home)
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under ANAKOT_HOME,
...
# ---------------------------------------------------------------------------
# run_import: ANAKOT_HOME override handling (issue #99839)
# ---------------------------------------------------------------------------


class TestImportHonorsAnakotHomeOverride:
    """`anakot import` must restore into the home the command runs under.

    Resolving the target through get_default_anakot_root() maps a profile
    ...
    def test_import_targets_named_profile_home(self, tmp_path, monkeypatch):
        """ANAKOT_HOME=<root>/profiles/<name> must restore INTO the profile,
        not into <root> (which would clobber the live root config)."""
        root = tmp_path / "anakot-root"
        profile = root / "profiles" / "coder"
        profile.mkdir(parents=True)
        # Live root config that must survive untouched.
        (root / "config.yaml").write_text("model:\n  provider: openai\n")

        monkeypatch.setenv("ANAKOT_HOME", str(profile))
        from anakot_constants import get_anakot_home

        assert get_anakot_home() == profile
    ...
    def test_import_skips_gateway_install_for_non_default_home(
        ...
        monkeypatch.setenv("ANAKOT_HOME", str(sandbox))
        ...
        monkeypatch.setattr(
            backup_mod,
            "_get_platform_default_anakot_home",
            lambda: native_default,
        )

        calls = []
        monkeypatch.setattr(
            "anakot_cli.gateway.ensure_gateway_service",
            lambda *a, **kw: calls.append(kw),
        )
        monkeypatch.setattr(
            "anakot_cli.gateway._is_service_running",
            lambda: False,
        )
    ...
    def test_import_installs_gateway_when_default_home_is_target(
        ...
        native_default = tmp_path / "native-default"
        native_default.mkdir()
        monkeypatch.setenv("ANAKOT_HOME", str(native_default))
...
def _write_session_db(path: Path, sessions: int, messages_per_session: int) -> None:
    """Create a minimal Anakot-shaped session database at *path*."""
...
class TestImportLiveSessionDatabase:
    """`anakot import` must not swap the inode of a database Anakot holds open.
...
def test_run_backup_prunes_older_default_named_zips_but_not_others(tmp_path, monkeypatch):
    """Hourly `anakot backup` callers accumulated 150+ zips; --keep bounds the default-named
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    for i in range(4):
        (tmp_path / f"anakot-backup-2026-01-0{i + 1}-000000.zip").write_bytes(b"old")
    (tmp_path / "my-archive.zip").write_bytes(b"mine")

    backup_mod.run_backup(Namespace(output=None, keep=2))

    kept = sorted(p.name for p in tmp_path.glob("anakot-backup-*.zip"))
    assert len(kept) == 2 and kept[0] == "anakot-backup-2026-01-04-000000.zip"
