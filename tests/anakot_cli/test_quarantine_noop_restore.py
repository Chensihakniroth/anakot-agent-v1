...
``anakot*.exe`` shims aside (``anakot.exe.old.<ms>``) before invoking the
...
quarantined shims stayed renamed aside and ``anakot`` vanished from PATH
...
def _make_scripts_dir(tmp_path: Path) -> Path:
    ...
    for name in ("anakot", "anakot-agent", "anakot-acp", "anakot-gateway"):
...
def test_main_noop_success_restores_shims(tmp_path):
    ...
    assert "anakot.exe" in names, "anakot.exe must be restored after a no-op install"
    assert "anakot-acp.exe" in names
    assert "anakot-gateway.exe" in names
    assert (scripts / "anakot.exe").read_bytes() == b"MZ-old-anakot"
...
def test_main_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    assert (scripts / "anakot.exe").read_bytes() == b"MZ-new-anakot"
...
# ---------------------------------------------------------------------------
# anakot_cli._install_repair._run_install_cmd (the deferred-recovery path)
# ---------------------------------------------------------------------------
...
def test_repair_noop_success_restores_shims(tmp_path):
    ...
    names = _shim_names(scripts)
    assert "anakot.exe" in names, "anakot.exe must be restored after a no-op recovery install"
    assert (scripts / "anakot.exe").read_bytes() == b"MZ-old-anakot"
...
def test_repair_rewriting_success_keeps_fresh_shims(tmp_path):
    ...
    def fake_run(cmd, cwd=None, check=None, env=None):
        (scripts / "anakot.exe").write_bytes(b"MZ-new-anakot")
