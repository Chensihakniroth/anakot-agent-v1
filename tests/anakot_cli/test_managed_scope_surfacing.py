"""Surfacing tests — managed scope shown in `config show` and `anakot doctor`."""
import pytest
...
def test_config_show_no_managed_scope_silent(tmp_path, monkeypatch, capsys):
    ...
    home.mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(home))
    monkeypatch.setenv("ANAKOT_MANAGED_DIR", str(tmp_path / "nope"))
    (home / "config.yaml").write_text("model:\n  default: user/model\n", encoding="utf-8")
    import anakot_cli.config as cfg
    from anakot_cli import managed_scope
    ...
    cfg._RAW_CONFIG_CACHE.clear()
    managed_scope.invalidate_managed_cache()
    from anakot_cli.config import show_config
...
def test_doctor_silent_with_no_managed_scope(tmp_path, monkeypatch, capsys):
    monkeypatch.setenv("ANAKOT_MANAGED_DIR", str(tmp_path / "nope"))
    from anakot_cli import managed_scope, doctor
