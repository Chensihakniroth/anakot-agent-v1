"""Regression tests for the OAuth dispatcher in anakot_cli.web_server.
...
Anakot must not mint subscription OAuth tokens from an unattended HTTP
...
import anakot_cli.web_routers.oauth as _rt_oauth
import anakot_cli.web_server_oauth as _web_server_oauth

client = TestClient(app)
HEADERS = {"X-Anakot-Session-Token": _SESSION_TOKEN}
...
def test_minimax_login_does_not_launch_anthropic_flow():
    ...
    with patch(
        "anakot_cli.auth._minimax_request_user_code",
        return_value=fake_user_code_resp,
    ), patch(
        "anakot_cli.auth._minimax_pkce_pair",
        return_value=("verifier-stub", "challenge-stub", "stub-state"),
    ), patch(
        "anakot_cli.web_server_oauth._minimax_poller",
        return_value=None,
    ):
        resp = client.post(
...
def test_minimax_start_route_honors_poller_mock_on_owning_module(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "anakot_cli.auth._minimax_poll_token",
        fail_poll_token,
    ), patch(
        "anakot_cli.web_server_oauth._minimax_poller",
        fake_poller,
...
def test_oauth_provider_status_uses_profile_query(tmp_path, monkeypatch):
    from anakot_cli import web_server as ws
    ...
    def fake_status():
        observed_homes.append(get_anakot_home())
    ...
    fake_catalog = ({
        ...
        "name": "Fake OAuth",
        "flow": "pkce",
        "cli_command": "anakot auth add fake-oauth",
...
def test_codex_dashboard_start_rewords_device_authorization_error(monkeypatch):
    ...
    try:
        ...
        assert "anakot auth" not in detail
    finally:
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "anakot auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `anakot model` universe) must be offered by /api/providers/oauth. This keeps
    ...
    for d in provider_catalog():
        if d.tab == "accounts":
            assert d.slug in offered, (
                f"{d.slug} is an accounts-tab provider in `anakot model` but is "
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Anakot."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``anakot auth remove
    xai-oauth``.
    """
    from anakot_cli import auth as auth_mod
    from anakot_cli import web_server as ws
    ...
    # Prior `anakot auth remove xai-oauth` left the source suppressed.
...
def test_status_falls_through_to_generic_dispatcher_for_catalog_only_provider():
    ...
    logged-out; now they dispatch to anakot_cli.auth.get_auth_status (the
    ...
    with patch("anakot_cli.auth.get_auth_status", return_value=fake_status):
