"""Bridge for pre-hand-off ``anakot update``: a stale root ``utils`` must not kill restart.
...
the restart phase's fresh ``anakot_cli.config`` import then died with
``cannot import name 'file_signature' from 'utils'``. Freshly imported anakot_cli code
drops the incomplete root cache first (``anakot_cli.stale_modules``).
"""
...
# anakot_cli/update_cmd_maint.py): package prefixes only, root-level modules survive.
_PRE_HANDOFF_PURGE_PREFIXES = ("anakot_cli", "gateway", "tools", "tui_gateway", "agent")
_PRE_HANDOFF_PURGE_PROTECTED = {"anakot_cli", "anakot_cli.main", "anakot_cli.anakot_logging"}
...
def pre_handoff_purge():
    ...
    def _purge() -> None:
        for name in list(sys.modules):
            if name in _PRE_HANDOFF_PURGE_PROTECTED or name.startswith("anakot_cli.update_"):
                continue
...
@pytest.mark.parametrize("consumer", ["anakot_cli.config", "anakot_cli.managed_scope"])
def test_fresh_anakot_cli_import_heals_stale_utils_missing_file_signature(
    monkeypatch, pre_handoff_purge, consumer
):
    """Restart-phase shape: anakot_cli.* purged, root utils stale, consumer freshly imported."""
    import utils
