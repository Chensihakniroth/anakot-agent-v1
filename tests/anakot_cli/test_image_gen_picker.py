...
class TestCodexOAuthBootstrapHook:
    ...
    def test_hook_starts_codex_oauth_only_when_credentials_missing(self, monkeypatch, logged_in):
        from anakot_cli import auth, tools_config_post_setup

        monkeypatch.setattr(auth, "get_codex_auth_status", lambda: {"logged_in": logged_in})
        monkeypatch.setattr("anakot_cli.setup.prompt_choice", lambda *a, **kw: 0)
        started, saved = [], []
    ...
    def test_hook_prints_auth_command_instead_of_device_login_when_noninteractive(self, monkeypatch, capsys):
        """Desktop's PostSetupRunner spawns `anakot tools post-setup openai_codex` with stdin=DEVNULL and
        ANAKOT_NONINTERACTIVE=1: nobody can complete a device-code login there, so the hook must name
        the real command and return instead of starting one."""
        from anakot_cli import auth, tools_config_post_setup

        monkeypatch.setenv("ANAKOT_NONINTERACTIVE", "1")
        ...
        assert "anakot auth add openai-codex" in capsys.readouterr().out

    def test_readiness_reports_codex_row_from_auth_store(self, monkeypatch):
        from anakot_cli import auth, tools_config
