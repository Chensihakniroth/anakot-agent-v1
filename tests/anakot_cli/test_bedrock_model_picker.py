...
Covers the three paths changed by fix/bedrock-provider-model-ids-live-discovery:
  ...
  2. list_authenticated_providers() Section 2 (ANAKOT_OVERLAYS) — bedrock
...
# ---------------------------------------------------------------------------
# 2. list_authenticated_providers() — bedrock via ANAKOT_OVERLAYS (Section 2)
# ---------------------------------------------------------------------------
...
class TestBedrockOverlayRegistration:
    """bedrock entry in ANAKOT_OVERLAYS is correctly configured."""

    def test_bedrock_overlay_exists(self):
        from anakot_cli.providers import ANAKOT_OVERLAYS
        assert "bedrock" in ANAKOT_OVERLAYS
