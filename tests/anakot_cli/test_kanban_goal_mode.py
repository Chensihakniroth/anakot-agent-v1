...
2. Spawn: _default_spawn sets the ANAKOT_KANBAN_GOAL_MODE env vars only
...
# ---------------------------------------------------------------------------
# CLI judge gate tests (anakot kanban complete bypass fix)
# ---------------------------------------------------------------------------

class TestCLIJudgeGate:
    """anakot kanban complete must apply the same goal_mode judge gate as the
    ...
    def _run(self, monkeypatch, *, goal_mode=True, judge_available=True,
        ...
        from anakot_cli.kanban import _cmd_complete
        ...
        monkeypatch.setattr("anakot_cli.kanban.kb.get_task", lambda conn, tid: fake_task)
        monkeypatch.setattr("anakot_cli.kanban.kb.complete_task", fake_complete_task)
        monkeypatch.setattr("anakot_cli.kanban.kbc.connect_closing", fake_connect_closing)
        monkeypatch.setattr("anakot_cli.kanban._worker_run_id_for", lambda _: None)
