"""ensure_anakot_home is memoized per home path (perf: it runs on every
...
def test_distinct_home_paths_each_get_the_skeleton(tmp_path, monkeypatch):
    first = tmp_path / "a" / ".anakot"
    second = tmp_path / "b" / ".anakot"

    monkeypatch.setenv("ANAKOT_HOME", str(first))
    cfg.ensure_anakot_home()

    # Profile switch: ANAKOT_HOME moves → the new path is ensured too.
