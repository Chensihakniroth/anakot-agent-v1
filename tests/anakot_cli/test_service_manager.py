"""Tests for anakot_cli.service_manager — the abstract ServiceManager
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `anakot_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import anakot_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("anakot_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
...
# unprivileged anakot user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from anakot_cli.service_manager import _seed_supervise_skeleton
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``anakot gateway run``, an agent action, the Open WebUI helper) holds
    the per-ANAKOT_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root ANAKOT_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `anakot gateway run --replace` (no -p flag).
    assert "anakot gateway run --replace" in default_text
    assert "anakot -p default" not in default_text
...
#
# `anakot gateway stop` inside a container dispatches through
...
def test_s6_log_run_creates_leaf_as_anakot_without_chown(
    s6_scandir, fake_subprocess_run,
) -> None:
    """log/run must not root-chown/unlink volume paths; create leaf as anakot.

    #45258 parent ownership is stage2's job (``logs/gateways`` seeded as
    anakot). Restartable log/run must not pathname-chown or pathname-rm a
    anakot-writable tree from root — that is a symlink TOCTOU hole.
    """
    ...
    assert not any(line.lstrip().startswith("chown ") for line in log_text.splitlines()), (
        "restartable log/run must not invoke chown on anakot-writable paths; "
        f"saw: {log_text!r}"
    )
    assert 's6-setuidgid anakot mkdir -p "$log_dir"' in log_text
    assert 's6-setuidgid anakot rm -f "$log_dir/lock"' in log_text
    assert 'else\n  mkdir -p "$log_dir"\n  rm -f "$log_dir/lock"\nfi\n' in log_text
    ...
    mkdir_as_anakot_idx = log_text.index('s6-setuidgid anakot mkdir -p "$log_dir"')
    rm_as_anakot_idx = log_text.index('s6-setuidgid anakot rm -f "$log_dir/lock"')
    exec_idx = log_text.index("s6-log 1 ")
    assert mkdir_as_anakot_idx < rm_as_anakot_idx < exec_idx
...
def test_s6_log_run_never_invokes_chown_with_symlinked_log_dir(tmp_path) -> None:
    ...
    gateways = anakot_home / "logs" / "gateways"
    ...
    # Mark the drop so fake rm can refuse unlink outside ANAKOT_HOME the way
    # a real anakot uid cannot delete a foreign root-owned lock.
    ...
    (bin_dir / "s6-setuidgid").write_text(
        "#!/bin/sh\n"
        "shift\n"
        'ANAKOT_TEST_DROPPED=1 exec "$@"\n',
        encoding="utf-8",
    )
    real_rm = "/bin/rm"
    (bin_dir / "rm").write_text(
        "#!/bin/sh\n"
        # Privilege-dropped: no-op. Models that anakot cannot unlink a foreign
    ...
    env = os.environ.copy()
    env["ANAKOT_HOME"] = str(anakot_home)
