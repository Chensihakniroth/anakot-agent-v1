"""Regression tests for #34107 — Docker UID/GID handling in ensure_anakot_home.

When Anakot runs in Docker with ``ANAKOT_UID=1000`` / ``ANAKOT_GID=911``,
the entrypoint chowns the top-level ``ANAKOT_HOME`` once at startup. But
subdirectories created at runtime by ``ensure_anakot_home()`` — especially
...
The fix is a ``_chown_to_anakot_uid`` helper that reads the env vars and
...
# ---------------------------------------------------------------------------
# _resolve_anakot_uid_gid
# ---------------------------------------------------------------------------


class TestResolveAnakotUidGid:
    def test_returns_parsed_values_when_both_set(self, monkeypatch):
        monkeypatch.setenv("ANAKOT_UID", "1000")
        monkeypatch.setenv("ANAKOT_GID", "911")
        from anakot_cli.config import _resolve_anakot_uid_gid
...
# ---------------------------------------------------------------------------
# _chown_to_anakot_uid
# ---------------------------------------------------------------------------


class TestChownToAnakotUid:
    ...
    def test_attributeerror_swallowed_for_windows_compat(self, tmp_path, monkeypatch):
        ...
        with patch.object(cfg.os, "chown", side_effect=AttributeError("no chown on this platform")):
            cfg._chown_to_anakot_uid(d)  # must not raise
...
class TestSecureDirChown:
    ...
    def test_secure_dir_no_chown_when_env_unset(self, tmp_path, monkeypatch):
        monkeypatch.delenv("ANAKOT_UID", raising=False)
        monkeypatch.delenv("ANAKOT_GID", raising=False)
        from anakot_cli import config as cfg
