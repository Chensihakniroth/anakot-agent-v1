...
def test_save_conversation_writes_under_anakot_home(anakot_home, tmp_path, monkeypatch, capsys):
    ...
    cli.AnakotCLI.save_conversation(stub, "/save json")
...
def test_save_conversation_bare_shows_usage(anakot_home, capsys):
    ...
    cli.AnakotCLI.save_conversation(stub, "/save")
...
def test_save_conversation_bad_format_shows_usage(anakot_home, capsys):
    for mod in [m for m in sys.modules if m.startswith("cli") or m == "anakot_constants"]:
        sys.modules.pop(mod, None)
    import cli

    stub = _make_stub_cli([{"role": "user", "content": "hi"}])
    cli.AnakotCLI.save_conversation(stub, "/save pdf")
