"""Tests for --ignore-user-config and --ignore-rules flags on `anakot chat`.
...
files. In Anakot the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.anakot/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → AnakotCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("ANAKOT_IGNORE_USER_CONFIG", "ANAKOT_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``ANAKOT_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<anakot_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With ANAKOT_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def _reload_cli(self, monkeypatch, tmp_path):
        """Point cli._anakot_home at tmp_path and return a fresh load_cli_config."""
        import cli
        monkeypatch.setattr(cli, "_anakot_home", tmp_path)
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("ANAKOT_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``AnakotCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting ANAKOT_IGNORE_RULES=1 flips AnakotCLI.ignore_rules True."""
        monkeypatch.setenv("ANAKOT_IGNORE_RULES", "1")

        # Import AnakotCLI lazily — cli.py has heavy module-init side effects
        # that we don't want to run at test collection time.
        import cli
        importlib.reload(cli)

        # Build only enough of AnakotCLI to reach the ignore_rules assignment.
        ...
        obj = object.__new__(cli.AnakotCLI)
        # Replicate the exact logic from cli.py AnakotCLI.__init__:
        ignore_rules = False  # constructor default
        obj.ignore_rules = ignore_rules or os.environ.get("ANAKOT_IGNORE_RULES") == "1"

        assert obj.ignore_rules is True


class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``anakot_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("ANAKOT_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("ANAKOT_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("ANAKOT_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("ANAKOT_IGNORE_RULES") == "1"
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "ANAKOT_IGNORE_USER_CONFIG" not in os.environ
        assert "ANAKOT_IGNORE_RULES" not in os.environ
