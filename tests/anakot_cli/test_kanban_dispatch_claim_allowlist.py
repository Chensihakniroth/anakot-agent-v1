...
On a board shared across Anakot homes (one kanban.db mounted in several
...
def test_present_blank_or_null_allowlist_skips_all_cards(
    ...
    with kbc.connect() as conn, caplog.at_level("WARNING", logger="anakot_cli.kanban_db"):
        tid = kb.create_task(conn, title="foreign card", assignee="default")
        res = kbd.dispatch_once(conn, dry_run=True)
    assert res.spawned == []
    assert res.skipped_nonspawnable == [tid]
...
def test_diagnostics_reports_resolved_allowlist(kanban_home, capsys, config, expected):
    """`anakot kanban diagnostics` (text and --json) shows what this home may claim."""
    import argparse
    import json
