"""Tests for `anakot fallback` — chain reading, add/remove/clear, legacy migration."""
...
# ---------------------------------------------------------------------------
# Shared fixture — isolate ANAKOT_HOME so save_config writes to tmp_path
# ---------------------------------------------------------------------------
...
def _write_config(home: Path, data: dict) -> None:
    config_path = home / ".anakot" / "config.yaml"
...
class TestReadChain:
    def test_returns_empty_list_when_unset(self):
        from anakot_cli.fallback_cmd import _read_chain
        assert _read_chain({}) == []
    ...
    def test_reads_new_list_format(self):
        ...
        cfg = {
            "fallback_providers": [
                ...
                {"provider": "nous", "model": "Anakot-4-Llama-3.1-405B"},
            ]
        }
...
class TestExtractFallback:
    def test_extracts_from_default_field(self):
        from anakot_cli.fallback_cmd import _extract_fallback_from_model_cfg
...
class TestAddCommand:
    def test_add_appends_new_entry(self, isolated_home, capsys):
        ...
        with patch("anakot_cli.main.select_provider_and_model", side_effect=fake_picker), \
                patch("anakot_cli.main._require_tty"):
            from anakot_cli.fallback_cmd import cmd_fallback_add
    ...
    def test_restore_preserves_absent_active_provider(self):
        ...
        from anakot_cli import auth, fallback_cmd
    ...
    def test_picker_failure_restores_persisted_primary_without_masking_error(
        ...
        from anakot_cli import fallback_cmd
        ...
        with patch(
            "anakot_cli.main.select_provider_and_model",
...
class TestRemoveCommand:
    ...
    def test_remove_selected_entry(self, isolated_home, capsys):
        ...
        # Picker returns index 1 (the middle entry, "nous / Anakot-4")
        with patch("anakot_cli.setup._curses_prompt_choice", return_value=1):
            from anakot_cli.fallback_cmd import cmd_fallback_remove
...
class TestClearCommand:
    ...
    def test_clear_with_confirmation(self, isolated_home, capsys, monkeypatch):
        ...
        from anakot_cli.fallback_cmd import cmd_fallback_clear
...
class TestDispatcher:
    ...
    def test_unknown_subcommand_exits(self, isolated_home):
        _write_config(isolated_home, {})
        from anakot_cli.fallback_cmd import cmd_fallback
...
class TestArgparseWiring:
    """Verify `anakot fallback` is wired into main.py's argparse tree.
    ...
    def test_fallback_help_lists_subcommands(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "anakot_cli.main", "fallback", "--help"],
            capture_output=True,
            text=True,
            timeout=30,
        )
