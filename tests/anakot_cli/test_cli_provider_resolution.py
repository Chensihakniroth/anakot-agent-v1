...
from anakot_cli import main as anakot_main
import anakot_cli.main_provider_setup as anakot_cli_main_provider_setup
from anakot_cli import model_setup_flows
from anakot_cli import model_switch
...
def test_provider_flag_uses_named_custom_default_model(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.config.load_config", lambda: config)
    monkeypatch.setattr("anakot_cli.runtime_provider.load_config", lambda: config)

    shell = cli.AnakotCLI(provider="gmk-lan", compact=True, max_turns=1)
...
def test_provider_flag_logs_when_custom_default_model_cannot_resolve(monkeypatch, caplog):
    ...
    monkeypatch.setattr(
        "anakot_cli.runtime_provider._get_named_custom_provider",
        _boom,
    )
...
def test_anakot_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in AnakotCLI.__init__")

    monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("anakot_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.AnakotCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("anakot_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_ensure_runtime_credentials_passes_cli_model_as_target_model(monkeypatch):
    """`anakot -m mimo-v2.5 --provider opencode-go` must resolve credentials for the model the
    ...
    monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.AnakotCLI(model="mimo-v2.5", provider="opencode-go", compact=True, max_turns=1)
...
def test_model_flow_nous_does_not_restore_stale_custom_api_key(tmp_path, monkeypatch):
    ...
    config_home = tmp_path / "anakot"
    config_home.mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(config_home))
    ...
    monkeypatch.setattr(
        "anakot_cli.auth.get_provider_auth_state",
        lambda provider: {
    ...
    monkeypatch.setattr(
        "anakot_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "anakot_cli.models.get_curated_nous_model_ids",
        lambda: [selected_model],
    )
    monkeypatch.setattr("anakot_cli.models_pricing.get_pricing_for_provider", lambda provider: {})
    monkeypatch.setattr("anakot_cli.models.check_nous_free_tier", lambda **kwargs: False)
    monkeypatch.setattr(
        "anakot_cli.models.union_with_portal_paid_recommendations",
        lambda model_ids, pricing, portal_url: (model_ids, pricing),
    )
    monkeypatch.setattr(
        "anakot_cli.auth._prompt_model_selection",
        lambda *args, **kwargs: selected_model,
    )
    monkeypatch.setattr(
        "anakot_cli.nous_subscription.prompt_enable_tool_gateway",
        lambda config: None,
    )

    anakot_main._model_flow_nous(stale_config, current_model="glm-5.2")
...
def test_codex_provider_uses_config_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "anakot_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex"],
    )

    shell = cli.AnakotCLI(compact=True, max_turns=1)
...
def test_startup_fallback_re_resolves_reasoning_for_the_fallback_model(monkeypatch, reasoning_flag, expected_effort):
    ...
    monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    shell = cli.AnakotCLI(compact=True, max_turns=1, reasoning=reasoning_flag)
...
def test_custom_entry_model_swap_re_resolves_reasoning(monkeypatch):
    """`anakot chat --model <custom-provider-name>`: the runtime's explicit `model` replaces the
    ...
    shell = cli.AnakotCLI(model="my-lan", compact=True, max_turns=1)
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "anakot_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("anakot_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("anakot_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("anakot_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("anakot_cli.main_provider_setup._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr("anakot_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": next(answers))

    anakot_main._model_flow_custom({})
...
def test_model_flow_custom_persists_selected_api_mode(monkeypatch):
    ...
    monkeypatch.setattr("anakot_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("anakot_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr(
        "anakot_cli.models.probe_api_models",
        lambda api_key, base_url: {
    ...
    saved_env = {}
    monkeypatch.setattr("anakot_cli.config.load_config", lambda: saved_cfg)
    monkeypatch.setattr("anakot_cli.config.save_config", lambda cfg: saved_cfg.update(cfg))
    monkeypatch.setattr(
        "anakot_cli.config.save_env_value",
        lambda key, value: saved_env.__setitem__(key, value),
    )
    monkeypatch.setattr(
        "anakot_cli.main_provider_setup._save_custom_provider",
    ...
    monkeypatch.setattr("builtins.input", lambda _prompt="": next(answers))
    monkeypatch.setattr("anakot_cli.secret_prompt.masked_secret_prompt", lambda _prompt="": "test-key")

    anakot_main._model_flow_custom({"model": {"provider": "custom"}})
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(anakot_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "anakot_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "nous"}},
    )
    monkeypatch.setattr("anakot_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("anakot_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("anakot_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("anakot_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("anakot_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(anakot_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    monkeypatch.setattr(anakot_cli_main_provider_setup, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("anakot_cli.auth._login_nous", _fake_login)

    anakot_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.nousresearch.com",
            inference_url="https://inference.nousresearch.com/v1",
            client_id="anakot-local",
...
def test_auto_provider_name_localhost():
    from anakot_cli.main_provider_setup import _auto_provider_name
...
def test_save_custom_provider_uses_provided_name(monkeypatch, tmp_path):
    ...
    import yaml
    from anakot_cli.main_provider_setup import _save_custom_provider

    cfg_path = tmp_path / "config.yaml"
    cfg_path.write_text(yaml.dump({}))

    monkeypatch.setattr(
        "anakot_cli.config.load_config", lambda: yaml.safe_load(cfg_path.read_text()) or {},
    )
    saved = {}
    def _save(cfg):
        saved.update(cfg)
    monkeypatch.setattr("anakot_cli.config.save_config", _save)
...
def test_save_custom_provider_references_the_key_instead_of_inlining_it(monkeypatch, tmp_path):
    ...
    saved = {}
    monkeypatch.setattr("anakot_cli.config.save_config", lambda cfg: saved.update(cfg))

    _save_custom_provider(
        "http://localhost:11434/v1",
        api_key="sk-secret",
        name="Ollama",
        key_env="ANAKOT_CUSTOM_LOCALHOST_11434_API_KEY",
    )

    entry = saved["custom_providers"][0]
    assert entry["key_env"] == "ANAKOT_CUSTOM_LOCALHOST_11434_API_KEY"
...
def test_custom_endpoint_key_env_is_a_valid_posix_name_for_ip_endpoints():
    ...
    from anakot_cli.config import _ENV_VAR_NAME_RE, custom_endpoint_key_env
