...
import anakot_cli.web_routers.files as _rt_files
...
def forced_files_client(monkeypatch, tmp_path):
    root = tmp_path / "data"
    monkeypatch.setenv("ANAKOT_DASHBOARD_FILES_ROOT", str(root))
...
def local_files_client(monkeypatch, tmp_path):
    home = tmp_path / "home"
    home.mkdir()
    monkeypatch.delenv("ANAKOT_DASHBOARD_FILES_ROOT", raising=False)
...
def test_download_resolves_paths_in_the_originating_profile_session(local_files_client, monkeypatch):
    ...
    anakot_home = home / "isolated-anakot"
    ...
    for profile, sid, cwd in [("default", "origin-session", str(session_cwd)),
        ...
        db_home = anakot_home if profile == "default" else anakot_home / "profiles" / profile
...
def test_other_credential_store_basenames_blocked(forced_files_client):
    ...
    point the managed root at ANAKOT_HOME itself (#57505), which contains
