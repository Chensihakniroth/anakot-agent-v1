...
The notice prints the most recent curator run summary on `anakot update`,
...
session start) so users normally never see the rename map. `anakot update`
...
def curator_env(tmp_path, monkeypatch, capsys):
    ...
    from anakot_cli import update_cmd_maint
