...
1. `anakot secrets bitwarden setup --help` works (dispatch path)
2. `anakot update --check` works (update path)
3. `anakot secrets bitwarden disable` works (handler execution)
4. `anakot secrets onepassword status` works (lazy backend loads on demand)
...
def _run_anakot(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    """Run anakot CLI as a subprocess from repo root.
    ...
    connectivity — but ``anakot update --check`` really does ``git fetch``
    ...
    env.update(
        {
            ...
            "GIT_CONFIG_KEY_0": "url.file:///nonexistent-anakot-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_0": "https://",
            "GIT_CONFIG_KEY_1": "url.file:///nonexistent-anakot-test-remote/.insteadOf",
            "GIT_CONFIG_VALUE_1": "git@",
        }
    )
    return subprocess.run(
        [sys.executable, "-m", "anakot_cli.main"] + args,
        capture_output=True,
        text=True,
        cwd=str(repo_root),
        env=env,
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_status(self) -> None:
        """`anakot secrets bitwarden status` must exit 0 (runs lazy backend)."""
        result = _run_anakot(["secrets", "bitwarden", "status"])
    ...
    def test_bitwarden_disable(self) -> None:
        """`anakot secrets bitwarden disable` must exit 0."""
        result = _run_anakot(["secrets", "bitwarden", "disable"])
        assert result.returncode == 0, (
    ...
    def test_onepassword_status(self) -> None:
        """`anakot secrets onepassword status` must exit 0 (1Password lazy backend)."""
        result = _run_anakot(["secrets", "onepassword", "status"])
        assert result.returncode in (0, 1), (
    ...
    def test_onepassword_setup_help(self) -> None:
        """`anakot secrets onepassword setup --help` must exit 0."""
        result = _run_anakot(["secrets", "onepassword", "setup", "--help"])
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `anakot update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`anakot update --check` must not load cryptography._rust."""
        result = _run_anakot(["update", "--check"])
    ...
    def test_main_update_check_crypto_absent_in_sys_modules(self) -> None:
        """Decisive invariant: invoking main() with argv=['anakot','update','--check']
...
sys.argv = ['anakot', 'update', '--check']

import anakot_cli.main as m
...
# resolves the anakot_cli.main module's lazily re-exported attribute —
# so the patch must land on update_cmd._cmd_update_check.
with patch('anakot_cli.update_cmd._cmd_update_check', capture_update_check):
    try:
        m.main()
