"""Multiplexed gateway: anakot_cli's process-wide caches must not hand profile A's value to profile B.

Every test builds two real profile homes (config.yaml / .env / cache files that differ), warms a
cache under ``set_anakot_home_override(A)`` and reads under B. Only the HTTP transport is canned —
...
def test_nous_reasoning_caps_follow_each_profiles_portal(homes, monkeypatch):
    ...
    import anakot_cli.models as models
    import anakot_cli.models_reasoning_caps as caps
...
def test_model_catalog_in_process_copy_is_bound_to_its_cache_file(homes, monkeypatch):
    a, b = homes
    import anakot_cli.model_catalog as mc
...
def test_failed_guest_mint_only_suppresses_that_profile(homes, monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("ANAKOT_SHARED_AUTH_DIR", str(tmp_path / "shared"))
    import anakot_cli.anon_auth as anon
    ...
    def client(timeout_seconds, verify):
        def handler(request):
            attempts.append(str(get_anakot_home()))
            if status["code"] == 429:
