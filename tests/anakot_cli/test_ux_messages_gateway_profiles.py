...
next command, and every ``anakot <sub> <cmd>`` it cites is a registered subcommand.
"""
...
from anakot_cli import gateway as gateway_cli
from anakot_cli import profiles
from anakot_cli.update_lock import UpdateHolder, describe_holder
...
def test_systemctl_failure_is_explained_not_tracebacked(monkeypatch, capsys):
    exc = subprocess.CalledProcessError(1, ["systemctl", "--user", "start", "anakot-gateway"])
    out = _run_gateway_command_raising(monkeypatch, capsys, exc)
    assert not out.lstrip().startswith("Command '")
    assert "anakot gateway status --deep" in out
    assert "anakot gateway install --force" in out
...
def test_missing_systemctl_points_at_foreground_run(monkeypatch, capsys):
    ...
    assert "anakot gateway run" in out
...
def test_existing_gateway_guard_says_bots_are_online_and_names_status(monkeypatch, capsys):
    ...
    for cmd in ("anakot gateway status", "anakot gateway restart", "anakot gateway stop"):
        assert cmd in out
...
def test_unsupported_platform_copy_says_what_is_unsupported(subcommand):
    ...
    assert "anakot gateway run" in text or subcommand == "uninstall"
...
def test_invalid_profile_name_explains_rule_in_words_with_example():
    ...
    assert "anakot profile create my-work" in msg
...
def test_existing_profile_error_offers_use_and_list(tmp_path, monkeypatch):
    ...
    assert "anakot profile use work" in msg
    assert "anakot profile list" in msg
...
def test_describe_holder_avoids_jargon_and_names_next_steps():
    ...
    assert "anakot logs" in message
    assert "anakot update" in message
