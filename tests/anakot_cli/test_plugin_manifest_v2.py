...
def anakot_home(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_ENABLE_PROJECT_PLUGINS", "0")
    monkeypatch.setenv(
        "ANAKOT_BUNDLED_PLUGINS", str(tmp_path / "empty-bundled")
    )
...
class TestV1Regression:
    def test_v1_manifest_parses_with_defaults(self, anakot_home):
        _write_plugin(anakot_home / "plugins", "oldie")
        _enable(anakot_home, ["oldie"])
        mgr = PluginManager()
    ...
    def test_v1_unknown_fields_do_not_warn_loudly(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "oldie",
...
class TestV2Parsing:
    def test_v2_fields_parse(self, anakot_home):
        _write_plugin(
            anakot_home / "plugins", "modern",
            manifest_extra={
                "manifest_version": 2,
                "api_version": 1,
                "license": "MIT",
    ...
    def test_unknown_field_in_v2_warns_but_loads(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "modern",
    ...
    def test_future_manifest_version_warns_but_loads(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "fromfuture",
            manifest_extra={
                "manifest_version": SUPPORTED_MANIFEST_VERSION + 5,
            },
        )
        _enable(anakot_home, ["fromfuture"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
        assert mgr._plugins["fromfuture"].enabled
        assert "newer than this Anakot" in caplog.text

    def test_malformed_v2_fields_warn_and_degrade(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "sloppy",
            manifest_extra={
                "manifest_version": 2,
                "api_version": "banana",
...
class TestDependencyOrder:
    def test_dep_registers_before_dependent(self, anakot_home):
        ...
        _write_plugin(
            anakot_home / "plugins", "aaa-consumer",
            manifest_extra={
                "manifest_version": 2,
        ...
        _write_plugin(
            anakot_home / "plugins", "zzz-base",
            register_body="import sys; sys._m2_order.append('zzz-base')",
        )
        _enable(anakot_home, ["aaa-consumer", "zzz-base"])
        import sys
    ...
    def test_missing_dep_warns_but_loads(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "needy",
            manifest_extra={
                "manifest_version": 2,
                "requires_plugins": [{"id": "ghost-plugin"}],
            },
        )
        _enable(anakot_home, ["needy"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
    ...
    def test_cycle_warns_and_falls_back_alpha(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "cyc-a",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(anakot_home, ["cyc-a", "cyc-b"])
        import sys
...
class TestConfigSchema:
    def test_type_mismatch_warns_but_loads(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "cfgd",
            manifest_extra={
                "manifest_version": 2,
                "config_schema": {
                    "api_url": {"type": "str"},
    ...
    def test_unknown_declared_type_skips_check(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "weird",
            manifest_extra={
                "manifest_version": 2,
...
class TestPythonDependenciesSeam:
    def test_missing_pip_dep_surfaced_with_hint_not_installed(
        self, anakot_home, caplog, monkeypatch
    ):
        calls = []
        import subprocess
        ...
        assert "anakot plugins enable pipful" in caplog.text
        assert calls == []

    def test_satisfied_pip_dep_is_quiet(self, anakot_home, caplog):
        _write_plugin(
            anakot_home / "plugins", "pipok",
            manifest_extra={
                "manifest_version": 2,
                "python_dependencies": ["pyyaml>=5,<7"],
            },
        )
        _enable(anakot_home, ["pipok"])
        with caplog.at_level(logging.WARNING):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestCtxHasPlugin:
    def test_has_plugin_probe(self, anakot_home):
        _write_plugin(anakot_home / "plugins", "probe-target")
        _write_plugin(
            anakot_home / "plugins", "prober",
            manifest_extra={
                "manifest_version": 2,
        ...
        _enable(anakot_home, ["probe-target", "prober"])
        import sys
...
class TestRequiresAnakot:
    def test_unsatisfied_requires_anakot_skips_without_importing(self, anakot_home, monkeypatch):
        """A too-new ``requires_anakot`` records an error and never runs register(); a satisfied one loads."""
        import sys
        from anakot_cli import plugins_manifest
        monkeypatch.setattr(plugins_manifest, "running_anakot_version", lambda: "1.2.3")
        _write_plugin(anakot_home / "plugins", "future", manifest_extra={"requires_anakot": ">=99.0"},
                      register_body="import sys; sys._rh_future = True")
        _write_plugin(anakot_home / "plugins", "current", manifest_extra={"requires_anakot": ">=1.2,<2"},
                      register_body="import sys; sys._rh_current = True")
        _enable(anakot_home, ["future", "current"])
        try:
            mgr = PluginManager()
            mgr.discover_and_load()
            assert not hasattr(sys, "_rh_future")
            assert "requires anakot >=99.0" in (mgr._plugins["future"].error or "")
...
class TestDirectoryPluginKeepsIdentityOverEntryPoint:
    """A pyproject-wrapper plugin depends on a pip package that ships a ``anakot_agent.plugins``
    ...
    def test_loader_and_listing_prefer_the_installed_directory(self, anakot_home, monkeypatch):
        from anakot_cli.plugins_manifest import PluginManifest
        _write_plugin(anakot_home / "plugins", "twin")
        _enable(anakot_home, ["twin"])
        ...
        monkeypatch.setattr("anakot_cli.plugins_cmd.discover_entrypoint_manifests", lambda: [twin_ep], raising=False)
        monkeypatch.setattr("anakot_cli.plugins.discover_entrypoint_manifests", lambda: [twin_ep])

        mgr = PluginManager()
        mgr.discover_and_load()
        assert mgr._plugins["twin"].manifest.source == "user"

        from anakot_cli.plugins_cmd import _discover_all_plugins
