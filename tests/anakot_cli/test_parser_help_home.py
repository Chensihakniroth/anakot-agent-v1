...
``get_anakot_home()`` printed ``[ANAKOT_HOME fallback] ... wrong profile`` on every ``anakot``
command for users of ``anakot profile use`` (#112319, also reported in #112839).
"""
...
def test_help_text_names_the_profile_config_once_the_process_is_re_homed(sticky_profile_home, monkeypatch):
    from anakot_cli._parser import _cfg_path

    monkeypatch.setenv("ANAKOT_HOME", str(sticky_profile_home / "profiles" / "coder"))

    assert _cfg_path() == "~/.anakot/profiles/coder/config.yaml"
