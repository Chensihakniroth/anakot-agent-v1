...
A long-lived multiplex process (the Desktop's ``anakot serve`` backend) routes records to every
...
def routed_profile(tmp_path, monkeypatch):
    ...
    anakot_logging.setup_logging(anakot_home=home, force=True)
    try:
        yield home, profile
    finally:
...
def _log_into(profile: Path, *, adopt: bool) -> None:
    ...
    ``setup_logging(anakot_home=<profile>)`` adoption path ``agent_init`` takes."""
    logger = logging.getLogger("agent.tests.profile-delete")
    token = set_anakot_home_override(profile)
    try:
        if adopt:
            anakot_logging.setup_logging(anakot_home=profile)
        else:
            assert anakot_logging.enable_profile_log_routing([Path(profile).parent.parent, profile]) is True
...
def _routers() -> list:
    return [h for h in anakot_logging._queued_file_handlers
            if isinstance(h, anakot_logging._ProfileRoutingFileHandler)]
