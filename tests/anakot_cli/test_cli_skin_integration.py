...
from cli import AnakotCLI, _build_compact_banner, _rich_text_from_ansi
from anakot_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_narrow_terminals_compact_voice_recording_prompt_fragments(self):
        ...
        with patch.object(AnakotCLI, "_get_tui_terminal_width", return_value=50):
...
class TestCompactBannerSkinIntegration:
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_anakot(self):
        set_active_skin("poseidon")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Anakot Agent v0.1.0 (test)"}):
