...
The Python launcher (`anakot --tui …`) has a ~100–300ms cold-start window
where stdin is still in cooked + echo mode. If a previous Anakot session
...
from anakot_cli.main import _suppress_mouse_residue_early
...
class TestEarlyMouseDisable:
    ...
    def test_respects_diagnostic_escape_hatch(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["anakot", "--tui"])
        monkeypatch.delenv("ANAKOT_TUI", raising=False)
        monkeypatch.setenv("ANAKOT_TUI_NO_EARLY_DISABLE", "1")
    ...
    def test_oserror_is_swallowed(self, monkeypatch):
        monkeypatch.setattr(sys, "argv", ["anakot", "--tui"])
        monkeypatch.delenv("ANAKOT_TUI", raising=False)
        monkeypatch.delenv("ANAKOT_TUI_NO_EARLY_DISABLE", raising=False)
