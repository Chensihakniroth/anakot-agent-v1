"""Regression for #74973 — `anakot update` must not leave the gateway down.
...
cron stayed dark until someone manually ran ``anakot gateway restart``.
...
def launchd(monkeypatch):
    """Stub anakot_cli.gateway so no real launchctl call is made."""
    calls: list[str] = []
    ...
    monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.anakot.gateway", raising=False)
...
class TestLaunchdRestartAfterUpdate:
    def test_plist_present_always_restarts_without_classifying(self, launchd, capsys):
        ...
        assert update_cmd._restart_launchd_gateway_after_update(supervision_verify=False) == (["ai.anakot.gateway"], [])
    ...
    def test_restart_failure_warns_that_gateway_is_down(self, launchd, capsys):
        ...
        assert "anakot gateway restart" in out
...
ai.anakot.gateway = {
\tactive count = 1
\tpath = /Users/u/Library/LaunchAgents/ai.anakot.gateway.plist
\tstate = running
\tpid = 59038
\tprogram = /Users/u/.anakot/bin/anakot
}
"""
...
class TestServicePidSweepExclusion:
    ...
    def macos_launchd(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_mod, "get_launchd_label", lambda: "ai.anakot.gateway")
...
class TestParseLaunchdPidFromPrintOutput:
    def test_running_service(self):
        from anakot_cli.gateway import _parse_launchd_pid_from_print_output
