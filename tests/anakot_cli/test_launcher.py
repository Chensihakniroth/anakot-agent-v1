"""Tests for the top-level `./anakot` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./anakot` should use `anakot_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "anakot"
    called = []

    fake_main_module = types.ModuleType("anakot_cli.main")

    def fake_main():
        called.append("anakot_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "anakot_cli.main", fake_main_module)
