...
from anakot_cli.main_agent_cmds import cmd_insights
...
def _run_show_insights(command: str):
    ...
    with patch("anakot_state.SessionDB", return_value=db), \
...
def test_insights_skips_open_when_store_missing(capsys):
    ...
    for run in (lambda: AnakotCLI.__new__(AnakotCLI)._show_insights("/insights 7"),
                lambda: cmd_insights(SimpleNamespace(days=30, source=None))):
        with patch("anakot_state.SessionDB") as ctor:
            run()
...
def test_show_insights_opens_read_only():
    cli_obj = AnakotCLI.__new__(AnakotCLI)
    db = MagicMock()
    with patch("anakot_state.SessionDB", return_value=db) as ctor, \
         patch("agent.insights.InsightsEngine", _InsightsEngineStub):
        _InsightsEngineStub.calls = []
