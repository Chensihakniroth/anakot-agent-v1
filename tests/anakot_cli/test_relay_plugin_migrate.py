"""``anakot update`` / ``anakot migrate relay``: legacy ``ANAKOT_NEMO_RELAY_ATIF_*``/``ATOF_*`` vars
...
from anakot_cli.relay_plugin_migrate import (
...
ANAKOT_NEMO_RELAY_ATOF_ENABLED=1
ANAKOT_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY={home}/telemetry/atof
ANAKOT_NEMO_RELAY_ATOF_FILENAME=anakot-atof.jsonl
ANAKOT_NEMO_RELAY_ATOF_MODE=append
ANAKOT_NEMO_RELAY_ATIF_ENABLED=1
ANAKOT_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY={home}/telemetry/atif
ANAKOT_NEMO_RELAY_ATIF_FILENAME_TEMPLATE=trajectory-{{session_id}}.json
ANAKOT_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
"""
...
def test_legacy_env_becomes_validated_toml_selected_from_env(profile_env):
    ...
    assert sink["type"] == "file" and sink["filename"] == "anakot-atof.jsonl"
    assert document["components"][0]["config"]["atif"]["filename_template"] == "trajectory-{session_id}.json"
    # Relay itself accepts the file Anakot will load at runtime.
    ...
    assert "# migrated to relay-plugins.toml: ANAKOT_NEMO_RELAY_ATOF_ENABLED=1" in (profile_env / ".env").read_text(encoding="utf-8")
