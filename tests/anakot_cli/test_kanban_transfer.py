"""Tests for kanban board export / import (``anakot_cli.kanban_transfer``).
...
from anakot_cli import kanban_transfer as kt
from anakot_cli.archive_safe import normalize_archive_parts, safe_extract_targz
...
def kanban_root(tmp_path, monkeypatch):
    ...
    def _use(name: str) -> Path:
        ...
        monkeypatch.setenv("ANAKOT_KANBAN_HOME", str(root))
        for var in ("ANAKOT_KANBAN_DB", "ANAKOT_KANBAN_WORKSPACES_ROOT",
                    "ANAKOT_KANBAN_ATTACHMENTS_ROOT", "ANAKOT_KANBAN_BOARD"):
...
def test_import_rejects_a_future_format_version(kanban_root, tmp_path):
    ...
    kanban_root("target")
    with pytest.raises(ValueError, match="newer than this Anakot"):
