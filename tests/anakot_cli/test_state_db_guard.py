...
from anakot_cli.backup import verify_sqlite_integrity
...
class TestPreUpdateBackupIntegrityGuard:
    ...
    ANAKOT_HOME whose state.db is corrupted mid-flight (#68474)."""
    ...
    def anakot_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(sys.modules.keys()):
            if mod.startswith("anakot_cli.config") or mod == "anakot_constants":
                del sys.modules[mod]
        return root

    def test_healthy_db_stays_quiet(self, anakot_home, capsys):
    ...
    def test_zeroed_db_after_snapshot_is_loud(self, anakot_home, capsys, monkeypatch):
    ...
    def test_failed_snapshot_is_loud_and_update_continues(self, anakot_home, capsys, monkeypatch):
