"""Tests for the per-profile Projects store (anakot_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Anakot Agent", folders=["/tmp/anakot"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "anakot-agent"
    assert proj.name == "Anakot Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/anakot"
    assert [f.path for f in proj.folders] == ["/tmp/anakot"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "anakot-agent").id == pid
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' ANAKOT_HOME.
