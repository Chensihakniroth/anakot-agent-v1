...
Override the image with ``ANAKOT_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("ANAKOT_TEST_IMAGE", "anakot-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``ANAKOT_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("ANAKOT_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged anakot user
# ---------------------------------------------------------------------------
#
# Background: every Anakot runtime path inside the container drops to UID
# 10000 (the ``anakot`` user) via ``s6-setuidgid anakot``. ``docker exec``
...
# fails (PermissionError swallowed) for anakot, so a test that ran as root
...
# helpers below run every probe under ``-u anakot`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: anakot).
