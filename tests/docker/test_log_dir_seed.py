"""Runtime smoke test for Docker $ANAKOT_HOME/logs/gateways seeding.

Build the real image and verify logs/ and logs/gateways/ exist and are
owned by the anakot user after container boot.
...
registered later runs its log service as the dropped anakot user and
...
def test_logs_gateways_seeded_and_anakot_owned(
    built_image: str, container_name: str,
) -> None:
    """logs/ and logs/gateways/ must exist and be owned by anakot after boot."""
    ...
    assert "logs=anakot" in r.stdout, (
        f"logs/ not owned by anakot: {r.stdout}"
    )
    assert "gateways=anakot" in r.stdout, (
        f"logs/gateways/ not owned by anakot: {r.stdout}"
    )
