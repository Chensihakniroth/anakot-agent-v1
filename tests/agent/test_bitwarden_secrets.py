...
binary download are exercised manually by `anakot secrets bitwarden
...
def anakot_home(tmp_path, monkeypatch):
    """Point Anakot at an isolated home directory."""
    home = tmp_path / ".anakot"
    home.mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(home))
    # Some modules cache get_anakot_home; clear if needed.
    import anakot_constants
    if hasattr(anakot_constants, "_ANAKOT_HOME_CACHE"):
        anakot_constants._ANAKOT_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_happy_path(anakot_home, monkeypatch):
...
def test_env_loader_calls_bsm_when_enabled(tmp_path, monkeypatch):
    ...
    from anakot_cli.env_loader import _apply_external_secret_sources
