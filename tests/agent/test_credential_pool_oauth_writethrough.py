...
from anakot_cli import auth as A
...
def test_codex_pool_refresh_holds_auth_store_lock_across_post(monkeypatch, tmp_path):
    ...
    Codex refresh tokens are single-use. If two Anakot processes both read the
...
def test_anakot_pkce_refresh_writes_back_to_singleton(tmp_path, monkeypatch):
    """A successful anakot_pkce refresh must update
    ~/.anakot/.anthropic_oauth.json, or ``_seed_from_singletons()`` on the
    ...
    oauth_file = anakot_home / ".anthropic_oauth.json"
    oauth_file.write_text(
        json.dumps({"accessToken": "sk-ant-oat-rt0", "refreshToken": "rt0", "expiresAt": 0}),
        encoding="utf-8",
    )
    _write_store(anakot_home / "auth.json", {"version": 1, "providers": {}})
    ...
    assert on_disk["refreshToken"] == "rt1", (
        "successful anakot_pkce refresh must write back to "
        "~/.anakot/.anthropic_oauth.json, or _seed_from_singletons() will "
        "revert the pool entry to the pre-refresh (spent) token on next load"
    )

    reloaded = load_pool("anthropic")
    reloaded_entries = [e for e in reloaded.entries() if e.source.endswith("anakot_pkce")]
    assert reloaded_entries, "anakot_pkce entry should still be present after reload"
...
def test_manual_anakot_pkce_refresh_does_not_create_duplicate_singleton(
    tmp_path, monkeypatch
):
    """A pool-owned manual:anakot_pkce entry must not create a second source."""
    anakot_home = tmp_path / "anakot"
    anakot_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.setattr("anakot_cli.auth.is_provider_explicitly_configured", lambda pid: True)
    ...
    assert not oauth_file.exists(), (
        "manual:anakot_pkce is already pool-owned; refreshing it must not "
        "create a second anakot_pkce singleton source"
    )

    reloaded = load_pool("anthropic")
    matching = [e for e in reloaded.entries() if e.id == "manual-entry"]
    assert len(matching) == 1
    assert matching[0].source == "manual:anakot_pkce"
