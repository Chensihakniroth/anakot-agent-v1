"""Tests for ANAKOT_HOME credential-file read blocking in file_safety.

Regression for https://github.com/Chensihakniroth/anakot-agent-v1/issues/17656 —
``read_file`` was previously only sandboxed against ``ANAKOT_HOME`` itself,
...
for the credential stores while leaving arbitrary ``ANAKOT_HOME`` files
...
def fake_home(tmp_path, monkeypatch):
    """Point ``_anakot_home_path()`` at a tmp dir for isolated checks."""
    import agent.file_safety as fs

    home = tmp_path / "anakot_home"
    home.mkdir()
    monkeypatch.setattr(fs, "_anakot_home_path", lambda: home)
    return home
...
def test_arbitrary_anakot_home_file_not_blocked(fake_home):
    """Non-credential files inside ANAKOT_HOME stay readable."""
...
def test_identically_named_anakot_files_outside_home_not_blocked(
    fake_home, tmp_path
):
    """Anakot-specific filenames (``auth.json``, ``mcp-tokens/``, ``google_oauth.json``)
    outside ANAKOT_HOME must remain readable — the gate is per-location for
    ...
    project.mkdir()
    # auth.json outside ANAKOT_HOME — readable (per-location gate).
    p = project / "auth.json"
    p.write_text("not secret here", encoding="utf-8")
    assert get_read_block_error(str(p)) is None, (
        "auth.json outside ANAKOT_HOME should NOT be blocked"
    )
...
def test_profile_mode_blocks_root_credentials(tmp_path, monkeypatch):
    """Under a profile, ANAKOT_HOME = <root>/profiles/<name>, but
    ...
    monkeypatch.setattr(fs, "_anakot_home_path", lambda: profile)
