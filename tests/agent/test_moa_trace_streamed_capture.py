...
path via ``anakot chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp ANAKOT_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point ANAKOT_HOME at a temp dir and turn moa.save_traces on."""
    anakot_home = tmp_path / ".anakot"
    anakot_home.mkdir()
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))

    # save_moa_turn reads config via anakot_cli.config.load_config; stub it to
