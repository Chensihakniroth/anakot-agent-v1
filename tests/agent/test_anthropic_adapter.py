...
class TestBuildAnthropicClient:
    ...
    def test_disables_sdk_retries_for_api_key(self):
        ...
        double-retries inside anakot's outer loop. We delegate retry entirely
...
class TestResolveAnthropicToken:
    ...
    def test_falls_back_to_anthropic_credential_pool_oauth(self, monkeypatch, tmp_path):
        ...
        # returns nothing, mirroring a Anakot-PKCE-only setup.
...
class TestConvertToolsToAnthropicDedup:
    ...
    https://github.com/Chensihakniroth/anakot-agent-v1/issues/18478
    """
...
def test_oauth_system_prompt_sanitizer_preserves_docs_url():
    ...
    assert "github.com/Chensihakniroth/anakot-agent-v1" in system_text
    assert "anakot-agent@example.com" in system_text
    assert "skill_view(name='anakot-agent')" in system_text  # a quoted slug is an identifier
