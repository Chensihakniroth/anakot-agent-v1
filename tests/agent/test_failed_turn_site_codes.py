...
def test_persistence_failure_default_copy_is_actionable_and_profile_aware(monkeypatch):
    monkeypatch.setenv("ANAKOT_HOME", "/srv/anakot-profile")
    text = TurnExplainersMixin._format_turn_completion_explanation("session_persistence_failed", "replaced")
    assert "anakot gateway stop" in text and "anakot doctor" in text
    assert "~/.anakot" not in text and "/srv/anakot-profile" in text
    assert "manifest" not in text  # the runbook stays in logger.error at anakot_state
