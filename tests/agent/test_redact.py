...
def _ensure_redaction_enabled(monkeypatch):
    """Ensure ANAKOT_REDACT_SECRETS is not disabled by prior test imports."""
...
class TestTerminalOutputRedaction:
    ...
    def test_command_reads_secret_file_detection(self):
        ...
        # With paths
        assert _command_reads_secret_file("cat ~/.anakot/.env")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("cat ~/.anakot/config.yaml", "api_key: anakotConfigSecret123", "anakotConfigSecret123"),
            (
                "head ~/.anakot/profiles/work/config.yaml",
            ...
            (
                "grep 'foo|bar' ~/.anakot/config.yaml",
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "cat ~/.anakot/config.example.yaml",
            "cat ~/.anakot/config.template.yaml",
            "cat ~/.bashrc.example",
...
class TestAnakotHomePathClassification:
    """``_is_secret_file_arg`` must see the RESOLVED Anakot home: a managed Windows home
    (``%LOCALAPPDATA%\\anakot``) has no ``.anakot`` segment and a resolved path never spells
    ``$ANAKOT_HOME``, so the literal test alone classified its ``config.yaml`` as ordinary YAML."""
    ...
    def test_resolved_home_config_is_secret_bearing_but_project_config_is_not(self, tmp_path, monkeypatch):
        ...
        home = tmp_path / "anakot"  # no ".anakot" segment
