...
def test_reconcile_absorbed_into_beats_everything_else(curator_env):
    ...
    out = curator_env._reconcile_classification(
        ...
        destinations={"anakot-agent-dev"},
        absorbed_declarations={
            "pr-review-format": {"into": "anakot-agent-dev", "declared": True},
        },
    )
    ...
    assert e["into"] == "anakot-agent-dev"
...
# `anakot curator status`). The full data has always been in REPORT.md on
...
# ---------------------------------------------------------------------------
# Pin hint — surfaces `anakot curator pin <umbrella>` in the rename block so
