...
class TestParseConfigStringList:
    """#86661: `anakot config set` and JSON-mode editor saves store lists as
    ...
    def test_python_literal_array_string_parses(self):
        # `anakot config set` can persist single-quoted Python-literal forms.
...
def test_skill_config_home_vars_use_subprocess_home(tmp_path, monkeypatch):
    ...
    anakot_home = tmp_path / "da\\ta"
    subprocess_home = anakot_home / "home"
    subprocess_home.mkdir(parents=True)
    (anakot_home / "config.yaml").write_text("", encoding="utf-8")
    monkeypatch.setenv("ANAKOT_HOME", str(anakot_home))
    monkeypatch.setenv("HOME", str(anakot_home))
