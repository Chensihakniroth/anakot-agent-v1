...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Anakot Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    attachments = anakot_home / "attachments"
    ...
    assert result.expanded
    # Default container base for the docker backend is /root/.anakot.
    assert "/root/.anakot/attachments/archive.zip" in result.message
...
def test_oversized_text_reference_maps_host_attachment_to_container_path(
    ...
    assert "/root/.anakot/attachments/large.txt" in attached_context
    assert "too large to inline safely" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert result.expanded
    assert str(payload) in result.message
    assert "/root/.anakot/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.anakot/auth.json` and read the
    ...
    oauth = anakot_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = anakot_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.anakot/auth.json and @file:.anakot/.anthropic_oauth.json "
        "and @file:.anakot/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Anakot/composer-images/a.png",
...
async def test_side_thread_expansion_guards_the_served_profile_home(tmp_path: Path, monkeypatch):
    ...
    token = set_anakot_home_override(served_home)
    try:
        result = preprocess_context_references(
    ...
    assert any("internal Anakot path" in w for w in result.warnings)
