...
2. ``anakot lsp status`` surfaces a ``Backend warnings`` section when
...
def test_install_npm_works_without_extras(tmp_path, monkeypatch):
    ...
    install_targets = [c for c in cmd if not c.startswith("-") and c not in {
        "install", "--prefix", str(install_mod.anakot_lsp_bin_dir().parent),
        "/usr/bin/npm",
    }]
...
def test_install_pip_finds_windows_scripts_launcher(tmp_path, monkeypatch):
    ...
    def fake_run(cmd, **kwargs):
        scripts_dir = install_mod.anakot_lsp_bin_dir().parent / "python-packages" / "Scripts"
    ...
    assert (install_mod.anakot_lsp_bin_dir() / "fake-language-server.exe").exists()


# ---------------------------------------------------------------------------
# Fix 2: ``anakot lsp status`` surfaces shellcheck-missing for bash
# ---------------------------------------------------------------------------
...
def test_lsp_package_manager_config_selects_installer_argv_and_never_falls_back_silently(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("ANAKOT_HOME", str(tmp_path))
    staging = str(install_mod.anakot_lsp_bin_dir().parent)
    cfg = {"lsp": {}}
