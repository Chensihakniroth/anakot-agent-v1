"""Secret stores under ANAKOT_HOME are write-denied; control files stay writable (#110464).
...
@pytest.fixture()
def anakot_layout(tmp_path, monkeypatch):
    """Profile ANAKOT_HOME plus a distinct global root, both patched."""
    root = tmp_path / "anakot_root"
...
def test_read_denied_secret_stores_are_write_denied_on_profile_and_root(anakot_layout):
    root, profile = anakot_layout
...
def test_control_files_and_lookalikes_outside_home_stay_writable(anakot_layout, tmp_path):
    root, profile = anakot_layout
    for base in (profile, root):
...
class TestProfileHomeProcessHome:
    """With the process HOME pinned to ``{ANAKOT_HOME}/home`` (TERMINAL_HOME_MODE=profile,
    ...
    def profile_home_env(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_anakot_home_path", lambda: profile)
        monkeypatch.setattr(fs, "_anakot_root_path", lambda: profile.parent)
        return profile
