...
Real imports, real temp ANAKOT_HOME root + named profile, real auth.json I/O.
...
def fleet(tmp_path, monkeypatch):
    """Root ANAKOT_HOME with an expired-but-refreshable Anthropic pool row."""
    ...
    anakot_constants._default_anakot_root_memo = None  # type: ignore[attr-defined]
    ...
    store = {
        ...
        "credential_pool": {
            "anthropic": [{
                ...
                "priority": 0, "source": "manual:anakot_pkce",
...
def test_strip_helper_fails_closed_when_root_store_cannot_be_resolved(fleet, monkeypatch):
    ...
    monkeypatch.setattr(
        anakot_constants, "get_default_anakot_root",
...
def test_profile_auth_add_owns_only_its_own_rows(fleet):
    ...
    pool.add_entry(PooledCredential(
        ...
        priority=0, source="manual:anakot_pkce", access_token="sk-ant-oat01-MINE",
