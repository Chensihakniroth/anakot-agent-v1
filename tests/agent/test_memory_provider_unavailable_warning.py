"""Regression tests for Chensihakniroth/anakot-agent-v1#2765.
...
service that didn't inherit ``~/.anakot/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "anakot memory status" in msg
