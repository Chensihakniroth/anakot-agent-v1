...
def isolated_home(monkeypatch):
    """Temp ANAKOT_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="anakot_test_31179_")
    anakot_home = os.path.join(test_home, ".anakot")
...
def _fresh_modules():
    """Drop cached anakot modules so each test reloads against current env."""
    _drop_reload_targets()
