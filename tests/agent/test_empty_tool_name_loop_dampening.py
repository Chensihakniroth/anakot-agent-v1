...
@pytest.fixture()
def agent_env():
    """Spin up the mock provider + an isolated ANAKOT_HOME, yield (agent, helpers)."""
    ...
    test_home = tempfile.mkdtemp(prefix="anakot_e2e_47967_")
    os.makedirs(os.path.join(test_home, ".anakot"))
    prev_home = os.environ.get("ANAKOT_HOME")
    os.environ["ANAKOT_HOME"] = os.path.join(test_home, ".anakot")
    ...
    for mod in list(sys.modules):
        if mod == "run_agent" or mod.startswith("agent.") or mod.startswith("tools.") or mod.startswith("anakot_"):
            del sys.modules[mod]
    ...
    try:
        ...
        # references into anakot_cli.*/tools.* and their monkeypatches would otherwise
        ...
        if prev_home is None:
            ...
            os.environ["ANAKOT_HOME"] = prev_home
