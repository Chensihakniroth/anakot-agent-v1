...
class TestCreateSession:
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("anakot_constants._wsl_detected", True)
        monkeypatch.setattr(
    ...
    def test_make_agent_prefers_passed_toolsets_over_config_servers(self, monkeypatch):
        ...
        monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {})
        ...
        manager._make_agent(
            session_id="rebuilt", cwd=".", enabled_toolsets=["anakot-acp", "mcp-acp-server"], disabled_toolsets=["browser"],
        )

        assert (seen[0]["enabled_toolsets"], seen[0]["disabled_toolsets"]) == (["anakot-acp", "mcp-cfg-server"], None)
        assert (seen[1]["enabled_toolsets"], seen[1]["disabled_toolsets"]) == (["anakot-acp", "mcp-acp-server"], ["browser"])
    ...
    def test_make_agent_forwards_resolved_credential_pool(self, monkeypatch):
        ...
        monkeypatch.setattr("run_agent.AIAgent", FakeAgent)
        monkeypatch.setattr("anakot_cli.config.load_config", lambda: {"model": {"default": "m", "provider": "openai-codex"}})
        monkeypatch.setattr("anakot_cli.runtime_provider.resolve_runtime_provider", lambda **_kw: {
...
class TestSymlinkAliasNormalization:
    ...
    def test_missing_path_keeps_lexical_normalization(self):
        ...
        assert acp_session._normalize_cwd_for_compare(
            "/nonexistent-anakot-test/x/../y"
        ) == "/nonexistent-anakot-test/y"
