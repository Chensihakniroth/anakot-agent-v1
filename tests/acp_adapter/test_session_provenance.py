...
def test_root_session_no_compression(db):
    ...
    assert prov["currentAnakotSessionId"] == "root1"
    assert prov["rootAnakotSessionId"] == "root1"
    assert prov["parentAnakotSessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_anakot_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentAnakotSessionId"] == "old"
    assert prov["rootAnakotSessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousAnakotSessionId"] == "old"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootAnakotSessionId"] == "p"  # lineage root still walked
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"anakot"}
    assert "sessionProvenance" in meta["anakot"]
    assert meta["anakot"]["sessionProvenance"]["currentAnakotSessionId"] == "root1"
