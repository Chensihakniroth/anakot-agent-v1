...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.anakot/anakot-agent/utils.py)
...
2. ``anakot update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``anakot_cli/main.py`` (the ``anakot update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``anakot_constants`` after the
