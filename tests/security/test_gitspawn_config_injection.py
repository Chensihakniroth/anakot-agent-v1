...
``[diff "x"] command=/textconv=`` driver. Anakot gathers workspace context by
...
context-gathering git path Anakot runs neutralizes every sink. They use a real
...
def test_web_git_diff_is_safe(malicious_repo):
    from anakot_cli import web_git
    repo, marker = malicious_repo
