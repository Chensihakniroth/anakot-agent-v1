...
# 5. Symlinks the 'anakot' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}☤ Anakot Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/anakot-uv-install.$$.log")"
...
if is_termux; then
    # Anakot currently declares requires-python >=3.11,<3.14. Termux can expose
    ...
    if [ -z "${PYTHON_PATH:-}" ]; then
        if command -v python >/dev/null 2>&1; then
            ...
            echo -e "${RED}✗${NC} Termux Python $PYTHON_FOUND_VERSION is not supported; Anakot requires Python >=3.11,<3.14"
...
# ============================================================================
# PATH setup — symlink anakot into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up anakot command..."

ANAKOT_BIN="$SCRIPT_DIR/venv/bin/anakot"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$ANAKOT_BIN" "$COMMAND_LINK_DIR/anakot"
echo -e "${GREEN}✓${NC} Symlinked anakot → $COMMAND_LINK_DISPLAY_DIR/anakot"
...
# ============================================================================
# Seed bundled skills into ~/.anakot/skills/
# ============================================================================

ANAKOT_SKILLS_DIR="${ANAKOT_HOME:-$HOME/.anakot}/skills"
mkdir -p "$ANAKOT_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.anakot/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$ANAKOT_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  anakot status        # Check configuration"
if is_termux; then
    echo "  anakot gateway       # Run gateway in foreground"
else
    echo "  anakot gateway install # Install gateway service (messaging + cron)"
fi
echo "  anakot cron list     # View scheduled jobs"
echo "  anakot doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m anakot_cli.main setup
fi
