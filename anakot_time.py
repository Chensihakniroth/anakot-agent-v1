"""Timezone-aware clock for Anakot.

``now()`` returns a tz-aware datetime in the user's configured IANA timezone. Resolution order:
``ANAKOT_TIMEZONE`` env var, then ``timezone`` in ``~/.anakot/config.yaml``, else server-local
...
# ANAKOT_HOME, so one unkeyed global would leak the first profile's timezone into later
...
def _env_timezone() -> str:
    """``ANAKOT_TIMEZONE`` when it may speak for the active profile. Under the multiplexed
    ...
    if is_multiplex_active():
        return ""
    return os.getenv("ANAKOT_TIMEZONE", "").strip()
...
def _resolve_timezone_name() -> str:
    ...
    try:
        ...
        # parse is the stdlib-safe fallback for bootstrap consumers without anakot_cli importable.
        try:
