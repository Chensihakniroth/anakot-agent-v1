...
IGNORED_PATTERNS = [
    ...
    re.compile(r"^Anakot\s+(Agent|Audit)$", re.IGNORECASE),
    ...
    re.compile(r".*\[bot\]$", re.IGNORECASE),             # any GitHub [bot] suffix (anakot-seaeye[bot] etc.)
...
IGNORED_EMAILS = {
    ...
    "anakot@nousresearch.com",
    "anakot-audit@example.com",
    "nousbot@nousresearch.com",
    "anakot@habibilabs.dev",
    "omx@oh-my-codex.dev",
    "codex@openai.com",
...
def gh_pr_list():
    ...
    try:
        result = subprocess.run(
            [
                "gh", "pr", "list",
                "--repo", "Chensihakniroth/anakot-agent-v1",
                "--state", "merged",
