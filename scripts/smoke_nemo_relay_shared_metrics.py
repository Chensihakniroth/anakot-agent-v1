"""Run a real Anakot CLI turn and validate the Relay shared-metrics output."""
...
def _resolve_anakot_executable(anakot_repo: Path) -> Path:
    for relative_path in (
        Path(".venv") / "bin" / "anakot",
        Path(".venv") / "Scripts" / "anakot.exe",
    ):
        candidate = anakot_repo / relative_path
        if candidate.is_file():
            return candidate
    discovered = shutil.which("anakot")
    if discovered:
        return Path(discovered)
    raise SystemExit(
        "Anakot executable not found in the repository virtual environment "
        "or on PATH"
    )
...
def _arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path.cwd(),
        help="Anakot source checkout containing .venv/bin/anakot",
    )
    parser.add_argument(
        "--relay-python",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Directory for the isolated ANAKOT_HOME and captured output",
    )
...
def _validate_store(database_path: Path) -> list[dict[str, Any]]:
    ...
    if by_name["anakot.client.active"] != [
        {
        ...
        raise AssertionError(
            f"Unexpected client-active counter: {by_name['anakot.client.active']}"
        )
    [model] = by_name["anakot.model_route.count"]
    expected_model = {
        "name": "anakot.model_route.count",
        "dimensions": {
            "model": MODEL_CANARY,
            "provider": "custom",
        },
    ...
    if model != expected_model:
        raise AssertionError(
            f"Unexpected model counter: {by_name['anakot.model_route.count']}"
        )
    expected_start = {
    ...
    if by_name["anakot.task_run.started"] != [expected_start]:
        raise AssertionError(
            f"Unexpected task start: {by_name['anakot.task_run.started']}"
        )
    [terminal] = by_name["anakot.task_run.finished"]
    ...
    [tool] = by_name["anakot.tool_call.count"]
    ...
    lifecycle = by_name["anakot.skill.lifecycle.count"]
    expected_actions = {
        "archived",
        "created",
        "edited",
    ...
    loads = by_name["anakot.skill.load.count"]
...
def _validate_packages(
    ...
    try:
        ...
        raise RuntimeError(
            "The Anakot development environment requires jsonschema"
        ) from exc
    ...
    if metrics["anakot.client.active"] != [
        {
            "name": "anakot.client.active",
            "type": "counter",
            "dimensions": {},
        ...
        raise AssertionError(
            f"Unexpected client-active metric: {metrics['anakot.client.active']}"
        )
    [model] = metrics["anakot.model_route.count"]
    if model["dimensions"] != {
        "model": MODEL_CANARY,
        "provider": "custom",
    } or model["value"] != 2:
        raise AssertionError(
            f"Unexpected model metric: {metrics['anakot.model_route.count']}"
        )
    [terminal] = metrics["anakot.task_run.finished"]
    ...
    [tool] = metrics["anakot.tool_call.count"]
    if (
        tool["dimensions"]
        != {
            "approval_outcome": "not_required",
    ...
    lifecycle = metrics["anakot.skill.lifecycle.count"]
    ...
    loads = metrics["anakot.skill.load.count"]
    if {
        (
...
def main() -> int:
    args = _arguments()
    anakot_repo = args.anakot_repo.resolve()
    relay_python = args.relay_python.resolve() if args.relay_python else None
    anakot = _resolve_anakot_executable(anakot_repo)
    ...
    if args.output_dir:
        ...
        root = Path(tempfile.mkdtemp(prefix="anakot-relay-shared-metrics-"))
    home = root / "anakot-home"
    ...
    try:
        ...
        env = os.environ.copy()
        env["ANAKOT_HOME"] = str(home)
        python_paths = [str(anakot_repo)]
    ...
    (root / "anakot.stdout.txt").write_text(result.stdout, encoding="utf-8")
    (root / "anakot.stderr.txt").write_text(result.stderr, encoding="utf-8")
    if result.returncode != 0:
        raise AssertionError(
            f"Anakot exited with {result.returncode}\n"
            f"stdout:\n{result.stdout}\nstderr:\n{result.stderr}"
        )
    ...
    if PROMPT_CANARY not in json.dumps(request.get("messages", [])):
        raise AssertionError("Anakot model request did not contain the prompt canary")
    ...
    if TOOL_CALL_CANARY not in follow_up or TOOL_RESULT_CANARY not in follow_up:
        raise AssertionError("Anakot did not return the tool result to the model")
    if RESPONSE_CANARY not in result.stdout:
        raise AssertionError("Anakot did not print the mock model response")
    ...
    package_paths, packages = _validate_packages(
        ...
        / "observability"
        / "schemas"
        / "anakot.shared_metrics.v2.schema.json",
    )

    print("Anakot -> NeMo Relay shared-metrics smoke test passed")
