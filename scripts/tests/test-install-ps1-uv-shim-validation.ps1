...
# trust any file at $AnakotHome\bin\uv.exe -- including the Chocolatey ShimGen
...
$testRoot = Join-Path $env:TEMP ("anakot-uv-shim-test-" + [Guid]::NewGuid().ToString('N'))
$AnakotHome = Join-Path $testRoot 'home'
$InstallDir = Join-Path $testRoot 'missing-checkout'
New-Item -ItemType Directory -Force -Path $testRoot | Out-Null
. $installScript -AnakotHome $AnakotHome -InstallDir $InstallDir
...
function Invoke-InstallUvScenario {
    ...
    if (Test-Path $AnakotHome) { Remove-Item -LiteralPath $AnakotHome -Recurse -Force }
    New-Item -ItemType Directory -Force -Path (Join-Path $AnakotHome 'bin') | Out-Null
