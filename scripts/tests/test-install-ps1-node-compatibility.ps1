...
# changing PATH, installing software, or touching the user's Anakot home.

$repoRoot = Split-Path -Parent (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path))
$installScript = Join-Path $repoRoot 'scripts\install.ps1'
$testRoot = Join-Path $env:TEMP ("anakot-node-compatibility-test-" + [Guid]::NewGuid().ToString('N'))
