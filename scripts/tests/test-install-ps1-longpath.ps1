...
function Invoke-Normalization {
    ...
    try {
        ...
        # its diagnostics are all `[anakot] `-prefixed, so the two separate
        ...
        $stderr = ($raw | Where-Object { $_ -like '`[anakot`]*' }) -join "`n"
        $stdout = ($raw | Where-Object { $_ -notlike '`[anakot`]*' }) -join "`n"
    } finally {
    ...
    return @{
        ...
        AnakotHome = $(if ($paths) { $paths.anakot_home } else { $null })
...
$expectedInstallDir = "$($longRoot)${sep}AppData${sep}Local" + '\anakot\anakot-agent'
...
$result = Invoke-Normalization -Environment @{ TEMP = $shortTemp; TMP = $shortTemp } `
    -ExtraArgs @('-InstallDir', (Join-Path $shortProfile 'custom-anakot'))
Assert-Equal -Expected (Join-Path $longRoot 'custom-anakot') -Actual $result.InstallDir -Label "explicit -InstallDir keeps the caller's directory, on the long root"
