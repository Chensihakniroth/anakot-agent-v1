...
When Anakot runs in TUI mode, the gateway child process communicates with
...
# ``get_anakot_home() / "plugins"`` (user) and ``./.anakot/plugins/`` (project,
# gated behind ``ANAKOT_ENABLE_PROJECT_PLUGINS``) — see
# ``anakot_cli/plugins.py:10-12``.  The guard only checked the bundled
...
# this script runs as a standalone subprocess — ``anakot_constants`` isn't
...
def main() -> int:
    ...
    # Add repo root to sys.path so we can import anakot_constants (this script
    ...
    # plugins in get_anakot_home()/plugins/ and project plugins in
    # ./.anakot/plugins/, where code like ori/hooks.py can spawn
    # subprocesses with inherited stdin — #67639).
    plugin_roots: list[Path] = [get_anakot_home() / "plugins"]
    if os.environ.get("ANAKOT_ENABLE_PROJECT_PLUGINS"):
        plugin_roots.append(Path.cwd() / ".anakot" / "plugins")
