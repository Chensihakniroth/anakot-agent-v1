...
# tree generates llms.txt — the index every LLM (Anakot included, via the
# anakot-agent skill) reads to learn what Anakot can do — and
...
_PY_RELEVANT_CONTRACT_FILES = {
    ...
    # tests/anakot_cli/test_desktop_slash_registry.py
...
_MCP_CATALOG_FILES = {"anakot_cli/mcp_catalog.py"}
...
# Rust crates — currently just the Tauri bootstrap installer (Anakot-Setup).
...
def _py_test_only(p: str) -> bool:
    ...
    Product jobs (Desktop E2E's ``anakot serve`` backend, the Docker image)
