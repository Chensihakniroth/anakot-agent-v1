 ...
 *   The artifact published on the website right now -- Anakot-Setup.exe has
 ...
 * @typedef {InstallMethod | 'anakot-update' | 'open-app-update' | 'anakot-desktop-app-update'} UpdateMethod
 ...
 *   desktop-installer install; anakot-desktop-app-update starts the app
 *   via `anakot desktop`, which every install method provides on every
...
export const SPEC = {
  windows: {
    install: [
      // irm https://anakot.nousresearch.com/install.ps1 | iex
      { method: 'installer-script' },
      // The same one-liner with -IncludeDesktop: builds Anakot.exe AND
      ...
      // Website Anakot-Setup.exe, clicked through the GUI.
    ...
    update: [
      ...
      // Same button, app launched via `anakot desktop`.
      { method: 'anakot-desktop-app-update' },
    ],
  },
  macos: {
    install: [
      { method: 'installer-script' },
      { method: 'installer-script+desktop' },
      // The published Anakot-Setup.dmg from the website, mounted and run.
      { method: 'desktop-installer', versions: ['latest'] },
    ],
    update: [
      { method: 'installer-script' },
  ...
  linux: {
    ...
    update: [
      ...
      // linux, so there is no open-app-update; `anakot desktop` is always
...
export function methodNeedsDesktop(m) {
  return m.startsWith('desktop-installer') || m === 'installer-script+desktop' ||
    m === 'open-app-update' || m === 'anakot-desktop-app-update';
}
