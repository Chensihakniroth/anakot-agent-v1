...
#
# The whole job: wait for the Desktop to exit, run `anakot update`, tell the
...
# the ANAKOT_DESKTOP_CHILD_PID reaper-exclusion dance dies with it.
#
# CONTRACT (keep in sync with apps/desktop/electron/main.ts):
#   bash scripts/desktop-update/posix.sh
#     --install-root <path>    repo checkout (ANAKOT_HOME/anakot-agent)
...
ANAKOT_HOME="${INSTALL_ROOT:+$(dirname "$INSTALL_ROOT")}"
ANAKOT_HOME="${ANAKOT_HOME:-${TMPDIR:-/tmp}}"
MARKER="$ANAKOT_HOME/.anakot-update-in-progress"
LOG_DIR="$ANAKOT_HOME/logs"; mkdir -p "$LOG_DIR" 2>/dev/null || true
LOG="$LOG_DIR/desktop-update-handoff.log"
RESULT="$ANAKOT_HOME/.anakot-update-result.json"
STATUS="${TMPDIR:-/tmp}/anakot-update-status.$$"
...
notify_fallback() { # status message — renderer-free recovery surface.
  ...
  if [ "$(uname)" = "Darwin" ]; then
    /usr/bin/osascript -e "display notification \"$(printf '%s' "$2" | sed 's/"/\\"/g')\" with title \"Anakot update\"" 2>/dev/null && return 0
  else
    if command -v notify-send >/dev/null 2>&1; then
      notify-send -u critical "Anakot update" "$2" 2>/dev/null && return 0
    fi
    local p
    if command -v zenity >/dev/null 2>&1; then
      zenity --warning --title="Anakot update" --text="$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
    if command -v kdialog >/dev/null 2>&1; then
      kdialog --title "Anakot update" --sorry "$2" 2>/dev/null &
      p=$!; sleep 1
      kill -0 "$p" 2>/dev/null && return 0
      wait "$p" 2>/dev/null
    fi
...
find_browser() {
  ...
  # email included — inside this window that is titled "Anakot" (#88410).
...
start_ui() {
  ...
  UI_PROFILE_DIR="$(mktemp -d "${TMPDIR:-/tmp}/anakot-update-ui-XXXXXXXX")" || {
    UI_PROFILE_DIR=""
  ...
  # teardown TERM killed the shim ~1s into `anakot update` (2026-08-14 16:44,
...
stop_ui() { # error/manual outcomes keep the window up briefly so a watching
  ...
  if [ "${1:-}" = "leave-window" ]; then
    sleep "${ANAKOT_UPDATE_SHIM_GRACE_SECONDS:-15}"
  fi
...
linux_gate() {
  ...
  # _desktop_linux_userns_sandbox_available() probe in anakot_cli/main.py).
  ...
  GATE=manual GATE_MSG="Update complete, but the rebuilt app can't relaunch itself (its sandbox helper needs root ownership). Reopen Anakot to finish."
}

mac_swap() {
  local rebuilt="" c
  for c in "$INSTALL_ROOT/apps/desktop/release/mac-arm64/Anakot.app" \
           "$INSTALL_ROOT/apps/desktop/release/mac/Anakot.app"; do
    [ -d "$c" ] && { rebuilt="$c"; break; }
  done
  ...
  if [ "$FINAL_CODE" -eq 0 ] && [ -n "$rebuilt" ] && [ -d "$RELAUNCH_TARGET" ] && [ "$rebuilt" != "$RELAUNCH_TARGET" ]; then
    ...
    if ! /usr/bin/ditto "$rebuilt" "$RELAUNCH_TARGET.new"; then
      ...
      if mv "$RELAUNCH_TARGET.old" "$RELAUNCH_TARGET"; then
        ...
        FINAL_CODE=7 FINAL_MSG="The update finished but installing the new app failed and the previous app could not be restored. Reinstall Anakot (the rebuilt app is at $rebuilt)."
  ...
  if [ -n "$DONE_NOTE" ]; then
    if [ "$(uname)" = "Darwin" ]; then
      ...
      if ! launch_app; then
        ...
        FINAL_MSG="$DONE_NOTE Anakot also could not reopen itself - open it manually."
        write_result
      fi
    fi
    ...
    FINAL_MSG="Update complete. Reopen Anakot to finish (it could not restart itself)."
    MANUAL=1
    write_result
    publish "manual" "$FINAL_MSG"; stop_ui leave-window
  fi
...
# (`venv/bin/anakot` has a `python3` shebang), so no Python-side heal —
...
# (anakot_cli/macos_tcc_anchor.ensure_tcc_anchor, which re-anchors whenever
...
# touched; and when it does restore symlinks, the very `anakot update` run it
...
tcc_pick_update_invoke() { # sets UPDATE_INVOKE; safety net past a failed heal
  ...
  # launchd gateway proves `venv/bin/python -m anakot_cli.main` works when
  # every alias entrypoint is bricked — drive the update the same way.
  local bin="$1"
  UPDATE_INVOKE=("$bin/anakot")
  if ! tcc_probe_python "$bin/python3" && tcc_probe_python "$bin/python"; then
    UPDATE_INVOKE=("$bin/python" -m anakot_cli.main)
  fi
}
...
if [ "$SELF_TEST_UI" -eq 1 ]; then
  ...
  sleep "${ANAKOT_SELFTEST_HOLD_SECONDS:-6}"
  RELAUNCH_TARGET=""
  if [ -n "${ANAKOT_SELFTEST_FAIL:-}" ]; then FINAL_MSG="self-test error state"
...
# died two seconds after starting `anakot update`. Re-exec through a one-shot
...
if [ "$HANDOFF_DAEMONIZED" -ne 1 ]; then
  ...
  # EXIT: that would publish a false failure and relaunch Anakot while the
...
# so the shim server and the later `anakot update` subprocess both inherit
...
# update_lock.py (the `anakot update` child adopts it via process ancestry).
# The Desktop supplies one acquisition time for the whole ownership chain.
NOW="$(date +%s)"
STARTED_AT="${ANAKOT_UPDATE_STARTED_AT:-$NOW}"
...
if [ "$DESKTOP_PID" -gt 0 ] 2>/dev/null; then
  ...
  if kill -0 "$DESKTOP_PID" 2>/dev/null; then
    FINAL_CODE=4 FINAL_MSG="Update aborted: the Anakot window (pid $DESKTOP_PID) did not exit within 30s. Nothing was changed. Close Anakot fully and try again."
...
ANAKOT_BIN="$INSTALL_ROOT/venv/bin/anakot"
[ -x "$ANAKOT_BIN" ] || { FINAL_CODE=3 FINAL_MSG="Update aborted: $ANAKOT_BIN is missing. The install needs repair (run the Anakot installer or anakot doctor)."; log "$FINAL_MSG"; exit 3; }

# Heal a venv the reverted TCC anchor left bricked BEFORE invoking the CLI:
# venv/bin/anakot execs venv/bin/python3, so a dead alias kills every attempt
...
tcc_pick_update_invoke "$INSTALL_ROOT/venv/bin"
if [ "${UPDATE_INVOKE[0]}" != "$ANAKOT_BIN" ]; then
  log "venv/bin/python3 still unbootable; invoking the update via ${UPDATE_INVOKE[*]}"
fi

# Run FROM the install root: `anakot update` resolves the tree it mutates
...
# "close all Anakot windows" sentinel.
KEEP_STASH=""
if "${UPDATE_INVOKE[@]}" update --help 2>/dev/null | grep -q -- '--keep-stash'; then
  KEEP_STASH="--keep-stash"
else
  log "installed anakot predates --keep-stash; running without it"
fi
...
printf '%s\n' "$OUT" >> "$LOG" 2>/dev/null
log "anakot update exit code: $CODE"

if [ "$CODE" -ne 0 ] && [ "$CODE" -ne 2 ]; then
  # Retry once: update-boundary class (fresh code on disk, stale in memory).
  # Exit 2 ("close all Anakot windows") is not retryable.
  #
  ...
  if printf '%s' "$OUT" | grep -q "CODE UPDATE SKIPPED"; then
    log "anakot update skipped (checkout parked on a non-target branch); not retrying"
    FINAL_CODE=8
...
# Truthful completion: `anakot update` calls a GUI build failure non-fatal
...
if [ "$CODE" -eq 0 ] && printf '%s' "$OUT" | grep -q "Desktop build failed"; then
  log "desktop build failed inside anakot update; retrying build"
  publish_stage "Rebuilding Desktop"
  "${UPDATE_INVOKE[@]}" desktop --force-build --build-only >> "$LOG" 2>&1 || {
    FINAL_CODE=6 FINAL_MSG="Code and dependencies updated, but the Desktop app rebuild failed - you are running the previous build. Run anakot desktop --force-build from a terminal to retry."
    exit 6
  }
fi
...
if [ "$CODE" -eq 0 ]; then FINAL_CODE=0 FINAL_MSG="Update complete."
  ...
  FINAL_CODE="$CODE" FINAL_MSG="Update failed (exit $CODE). Run anakot debug share in a terminal to send a report."
  ...
  if ! tcc_probe_python "$INSTALL_ROOT/venv/bin/python3" \
    ...
    FINAL_MSG="Update failed: the Python interpreter inside $INSTALL_ROOT/venv cannot start (heal state: $TCC_HEAL_STATE). Reinstall the runtime with the Anakot installer, or run anakot doctor --fix from a terminal if any anakot command still works."
  fi
fi
exit "$FINAL_CODE"
