# ============================================================================
# Anakot Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [switch]$ForceCommit,
    [string]$Tag = "",
    [string]$AnakotHome = $(if ($env:ANAKOT_HOME) { $env:ANAKOT_HOME } else { "$env:LOCALAPPDATA\anakot" }),
    [string]$InstallDir = $(if ($env:ANAKOT_HOME) { "$env:ANAKOT_HOME\anakot-agent" } else { "$env:LOCALAPPDATA\anakot\anakot-agent" }),
    ...
    # builds apps/desktop into a launchable Anakot.exe.
    #
    # Why opt-in:
    #   * Anakot-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Anakot.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Anakot.exe
    ...
    #     `anakot desktop` already builds on demand.
...
# ANAKOT_HOME and InstallDir -- in that short form:
...
function Write-PathDiag {
    ...
    [Console]::Error.WriteLine("[anakot] $Message")
}
...
function ConvertTo-LongPath {
    ...
    try {
        if (-not ([System.Management.Automation.PSTypeName]'AnakotInstall.LongPath').Type) {
            Add-Type -Namespace 'AnakotInstall' -Name 'LongPath' -MemberDefinition @'
...
'@
        ...
        $length = [AnakotInstall.LongPath]::GetLongPathNameW($Path, $buffer, $buffer.Capacity)
...
# long. An explicitly passed -AnakotHome / -InstallDir is normalized in place
...
if ($script:NormalizedProfilePaths) {
    ...
    Write-PathDiag "resolved install paths: AnakotHome=$AnakotHome InstallDir=$InstallDir"
}
...
$RepoUrlSsh = "git@github.com:Chensihakniroth/anakot-agent-v1.git"
$RepoUrlHttps = "https://github.com/Chensihakniroth/anakot-agent-v1.git"
$PythonVersion = "3.11"
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $AnakotHome)) {
        New-Item -ItemType Directory -Force -Path $AnakotHome | Out-Null
    }
    $envFile = Join-Path $AnakotHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $AnakotHome "node"
...
function Install-Uv {
    # Anakot owns its own uv at $AnakotHome\bin\uv.exe.  Always install there --
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (anakot_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `anakot update` stay in sync.
    $managedUv = Join-Path $AnakotHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $AnakotHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $AnakotHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $AnakotHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $AnakotHome "bin"
        ...
        # (anakot_cli/managed_uv.py looks only at $AnakotHome\bin\uv.exe).
...
# Put the Anakot-managed Node dir at the FRONT of the persisted User PATH.
#
...
# environment (a standalone anakot-setup.exe run, a user typing `npm`) silently
...
# Upgrade the Anakot-managed Node tree's bundled npm into $NpmRange when
...
function Update-ManagedNpm {
    ...
    if (Test-ManagedNodeInUse $NodeDir) {
        Write-Warn "Anakot-managed Node.js is in use by a running app; skipping the bundled npm upgrade (applies on a later update with the app closed)."
        return $false
    }

    Write-Info "Upgrading bundled npm to satisfy $range ..."

    $tmpCwd = Join-Path $env:TEMP ("anakot-npm-upgrade-" + [Guid]::NewGuid().ToString("N"))
...
function Resolve-UvCmd {
    ...
    # sibling process and ANAKOT_HOME wasn't propagated).
...
function Initialize-ManagedPythonEnvironment {
    # Python used by Anakot belongs to the checkout, never to another
    ...
    $managedRoot = Join-Path $InstallDir ".anakot-runtime\python"
...
function Resolve-AvailablePythonVersion {
    # Return the path and minor version of the first Anakot-managed interpreter
    ...
    #
    # Under Anakot-Setup.exe each stage runs in a fresh powershell.exe. The
...
function New-GitBashAslrFailureReason {
    ...
    return @(
        ...
        "Then rerun Anakot setup. If the override is blocked or later re-applied, ask your Windows administrator to allow this per-program exception."
...
function Install-Git {
    ...
    which Anakot uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\anakot\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Anakot needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Anakot copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\anakot\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``ANAKOT_GIT_BASH_PATH`` (User scope) so Anakot can find it in a fresh
    ...
    # Download PortableGit into $AnakotHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $AnakotHome\git\ ..."
    ...
    try {
        ...
        Write-Info "then re-run this installer.  Anakot needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``ANAKOT_GIT_BASH_PATH`` (User env scope) so Anakot can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $AnakotHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $AnakotHome\git\usr\bin\bash.exe
    $candidates += "$AnakotHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$AnakotHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("ANAKOT_GIT_BASH_PATH", $candidate, "User")
            $env:ANAKOT_GIT_BASH_PATH = $candidate
            $script:GitBashPath = $candidate
            Write-Info "Set ANAKOT_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Anakot may not find Git Bash."
    Write-Info "If needed, set ANAKOT_GIT_BASH_PATH manually to your bash.exe path."
}
...
# range used to provision the Anakot-managed tree. Keeping this probe separate
...
function Test-Node {
    ...
    Write-Info "Using a Anakot-managed Node.js installation instead..."

    # Prefer a Anakot-managed Node from a previous run over a too-old system one.
    $managedNode = "$AnakotHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$AnakotHome\node;$env:Path"
        Set-ManagedNodeFirstOnUserPath "$AnakotHome\node"
        Write-Success "Node.js $version found (Anakot-managed)"
        ...
        Update-ManagedNpm "$AnakotHome\node" | Out-Null
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Anakot-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $AnakotHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $AnakotHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\anakot-node-extract"
            ...
            if ($extractedDir) {
                ...
                $version = & "$AnakotHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $AnakotHome\node\ (portable, user-scoped)"
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\anakot-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `anakot
                    # update` path (#4735).
                    ...
                    $stashName = "anakot-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # BACKWARDS. anakot-setup.exe bakes its build-time commit
                ...
                elseif ($Tag) {
                    ...
                    # ff-only pull cannot succeed -- mirror ``anakot update`` and
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                $extractPath = "$env:TEMP\anakot-agent-extract"
                ...
                if ($extractedDir) {
                    ...
                    # `anakot update` (see the notes at the shared clone-path
    ...
    # next `anakot update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Anakot-Setup.exe
    ...
    if (-not $resolvedPython) {
        throw "Anakot-managed Python is unavailable. Run install.ps1 -Stage python first."
    }
    ...
    try {
        ...
        # the task fires normally on next logon and `anakot update` / the
...
function Restore-VenvBackup {
    ...
    # venv so Anakot (and the venv-blocker probe) stay usable (#83149).
    $backupName = Get-PendingVenvBackup
    if (-not $backupName) { return }
    try {
...
"@ 2>$null
    ...
    if (-not $NoVenv) {
        # uv on Windows can register anakot.exe in dist-info/RECORD but fail to
        ...
        # `anakot` command while anakot-agent.exe / anakot-acp.exe exist
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `anakot dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: anakot_cli/web_server.py"
        }
    }
...
function Install-AnakotCommandLaunchers {
    ...
    # Requiring anakot.exe before creating the destination keeps the PATH
    # stage from reporting success with an unusable command (PR #92092).
    $scriptsDir = Join-Path $Root "venv\Scripts"
    $requiredSource = Join-Path $scriptsDir "anakot.exe"
    if (-not (Test-Path -LiteralPath $requiredSource -PathType Leaf)) {
        throw "Cannot set up the anakot command: required launcher not found: $requiredSource"
    }

    New-Item -ItemType Directory -Force -Path $Destination | Out-Null

    # Launcher form depends on the venv (keep in lockstep with
    # anakot_cli/_install_repair.py): a normal venv's exe trampoline
    ...
    foreach ($launcher in @("anakot", "anakot-acp")) {
...
function Set-PathVariable {
    Write-Info "Setting up anakot command..."
    ...
    if ($NoVenv) {
        ...
        # installer staged under anakot-agent\bin. No git operation can ever
        # touch this dir. Staging and verification live in
        # Install-AnakotCommandLaunchers, which throws BEFORE any PATH
        # mutation when the launchers cannot be staged.
        $anakotBin = "$AnakotHome\bin"
        Install-AnakotCommandLaunchers -Root $InstallDir -Destination $anakotBin | Out-Null
    }
    ...
    #   anakot-agent\bin -- lived inside the git checkout, where the update
    #                       autostash could sweep the launchers off disk
    # The anakot-agent\bin FILES are left in place on purpose: editor/ACP
    ...
    if (-not $NoVenv) {
        ...
        if ($cleaned.Count -ne $items.Count) {
            ...
            Write-Info "Removed legacy launcher entries from user PATH (kept anakot via $anakotBin)"
        }
    }
    
    if ($currentPath -notlike "*$anakotBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$anakotBin;$currentPath",
            "User"
        )
    ...
    # Set ANAKOT_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\anakot instead
    # of the Unix default ~/.anakot
    $currentAnakotHome = [Environment]::GetEnvironmentVariable("ANAKOT_HOME", "User")
    if (-not $currentAnakotHome -or $currentAnakotHome -ne $AnakotHome) {
        [Environment]::SetEnvironmentVariable("ANAKOT_HOME", $AnakotHome, "User")
        Write-Success "Set ANAKOT_HOME=$AnakotHome"
    }
    $env:ANAKOT_HOME = $AnakotHome
    
    # Update current session
    $env:Path = "$anakotBin;$env:Path"
    
    Write-Success "anakot command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir.anakot-bootstrap-complete which tells the Anakot
    ...
    # Anakot-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `anakot update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".anakot-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the ANAKOT_HOME directory structure ($AnakotHome, default %LOCALAPPDATA%\anakot)
    New-Item -ItemType Directory -Force -Path "$AnakotHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$AnakotHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$AnakotHome\.env"
    ...
    # Create config.yaml
    $configPath = "$AnakotHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Anakot's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$AnakotHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in anakot_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $AnakotHome"
    
    # Seed bundled skills into $AnakotHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $AnakotHome\skills ..."
    ...
    if (Test-Path $pythonExe) {
        try {
            ...
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                ...
                Write-Success "Skills copied to $AnakotHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Anakot-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'anakot setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\anakot-npm-browser-$(Get-Random).log"
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\anakot-npm-tui-$(Get-Random).log"
...
# failure is non-fatal (browser_exec can still run via uvx, and `anakot tools`
...
function Install-BrowserUseCli {
    ...
    $managedBin = Join-Path $AnakotHome "bin"
    ...
    $ErrorActionPreference = "Continue"
    try {
        # UV_TOOL_BIN_DIR keeps the binary inside Anakot' managed bin dir,
...
# Provision it at install time so enabling the tool later -- via `anakot
...
# paths still lazy-install via install_cua_driver() (anakot_cli/tools_config)
...
function Install-CuaDriver {
    ...
    $ErrorActionPreference = "Continue"
    try {
        # Same upstream installer `anakot computer-use install` runs. Bounded
        ...
        # Anakot' _CUA_INSTALLER_TIMEOUT (660s).
...
# the final `electron` -> `Anakot` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Anakot.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_anakot_desktop command
    # resolves apps/desktop/release/win-unpacked/Anakot.exe directly,
    ...
    # enforces the supported Node lines and prepends the Anakot-managed Node to
    ...
    # The --dir mode produces an unpacked Anakot.exe in
    ...
    # are NOT trying to work around). The Anakot icon + product name are
    # stamped onto Anakot.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\anakot-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Anakot.exe",
        "$desktopDir\release\win-arm64-unpacked\Anakot.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Anakot.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Anakot icon + identity are stamped onto Anakot.exe by the
    ...
    #     SIDs under %LOCALAPPDATA% (electron/electron#51761, anakot-agent#38216).
    ...
    #    Anakot.exe. We deliberately do NOT point them at `anakot desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Anakot.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Anakot.lnk')
        )
        ...
        # with the Anakot icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.anakot/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'anakot setup'."
        return
    }
...
function Start-GatewayIfConfigured {
    ...
    $anakotCmd = "$InstallDir\venv\Scripts\anakot.exe"
    if (-not (Test-Path $anakotCmd)) {
        $anakotCmd = "anakot"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$AnakotHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'anakot whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: anakot gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$AnakotHome\logs\gateway.log"
            Start-Process -FilePath $anakotCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$AnakotHome\logs\gateway-error.log" `
                -WindowStyle Hidden
...
function Write-Completion {
    ...
    Write-Host "$AnakotHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnakotHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnakotHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$AnakotHome\anakot-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   anakot              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   anakot setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   anakot config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   anakot config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   anakot gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   anakot update       " -NoNewline -ForegroundColor Green
