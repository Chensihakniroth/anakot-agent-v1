...
Run after changing any ``desktop=`` value or alias in ``anakot_cli/commands.py``;
``tests/anakot_cli/test_desktop_slash_registry.py`` fails until the committed
...
def render() -> str:
    ...
    from anakot_cli.commands import desktop_surface_registry
