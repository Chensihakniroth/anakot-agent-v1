...
React + Ink terminal UI for Anakot. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `ANAKOT_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `anakot setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.anakot/.anakot_history` or under `ANAKOT_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `anakot setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/anakot-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external anakot setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.anakot/tui-parent.log
    ...
    types/
      anakot-ink.d.ts            type declarations for @anakot/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent AnakotCLI subprocess for slash commands
```
