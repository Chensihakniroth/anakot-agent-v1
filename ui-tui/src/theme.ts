import { contrastRatio, ensureContrast, mix, parseColor, relativeLuminance, toHex } from '@anakot/shared/color'
import type { SkinBranding, SkinColors } from '@anakot/shared/skin'
...
// ── Color math ───────────────────────────────────────────────────────
//
// Generic color computation lives in @anakot/shared/color (the primitives,
...
// Mirrors @anakot/ink's colorize.ts. Keep local: app code compiles from
// ui-tui/src, while @anakot/ink is bundled separately from packages/.
...
export const DARK_SEEDS: ThemeSeeds = {
  accent: '#FFBF00',
  // The classic Anakot navy surfaces are IDENTITY, not derivation drift —
...
// The classic light-mode Anakot look was never hand-authored: for years the
 ...
 *  answer when known (cached in ANAKOT_TUI_BACKGROUND), else the mode's
 *  assumed pole. */
function referenceBackground(isLight: boolean, env: NodeJS.ProcessEnv = process.env): string {
  const cached = (env.ANAKOT_TUI_BACKGROUND ?? '').trim()
...
// Terminal. Explicit ANAKOT_TUI_THEME / COLORFGBG signals above still win,
...
// `ANAKOT_TUI_BACKGROUND` is intentionally generic so a future OSC11
...
//
//   1. `ANAKOT_TUI_LIGHT` boolean — `1`/`true`/`yes`/`on` → light;
//      `0`/`false`/`no`/`off` → dark.  Either explicit value wins
//      regardless of any later signal.
//   2. `ANAKOT_TUI_THEME` named override — `light` / `dark` win over
//      every signal below.
//   3. `ANAKOT_TUI_BACKGROUND` hex hint (3- or 6-digit) — luminance
...
//
// Anything we can't decide stays dark — the default Anakot palette
// is the dark one.
export function detectLightMode(
  ...
  const lightFlag = (env.ANAKOT_TUI_LIGHT ?? '').trim().toLowerCase()
  ...
  const themeFlag = (env.ANAKOT_TUI_THEME ?? '').trim().toLowerCase()
  ...
  const bgHint = backgroundLuminance(env.ANAKOT_TUI_BACKGROUND ?? '')
 ...
 * the OSC-11 background answer cached into ANAKOT_TUI_BACKGROUND after
...
export function fromSkin(
  ...
  // itself into ANAKOT_TUI_BACKGROUND. See #applySkin / syncThemeToTerminalBackground).
  ...
  //    base seeds for this polarity. The base's IDENTITY FILLS (Anakot navy
  ...
  // the derived value already carries the identity seeds (e.g. Anakot navy).
  const activeRow =
