import type { InputEvent, Key } from '@anakot/ink'
import * as Ink from '@anakot/ink'
 ...
 * but that accepts `key.meta` on macOS — and anakot-ink reports Option as
...
export function supportsFastEchoTerminal(env: NodeJS.ProcessEnv = process.env): boolean {
  ...
  if (isTermuxTuiMode(env)) {
    const override = String(env.ANAKOT_TUI_TERMUX_FAST_ECHO ?? '')
      .trim()
      .toLowerCase()
  ...
  useInput(
    (inp: string, k: Key, event: InputEvent) => {
      ...
      if (actionHome) {
        ...
        // ESC d, which anakot-ink decodes as meta+'d'; without this branch it
