...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.ANAKOT_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Anakot on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via ANAKOT_TUI_TRUECOLOR=1.
