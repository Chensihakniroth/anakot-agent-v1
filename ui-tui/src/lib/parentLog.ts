...
const logDir = join(process.env.ANAKOT_HOME?.trim() || join(homedir(), '.anakot'), 'logs')
...
// a real ~/.anakot (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('anakot-tui: parent lifecycle log unavailable\n')
    }
  }
}
