 ...
 * `anakot-boot-background` / `anakot-boot-color-scheme` localStorage keys.
 *
...
// Profile-aware: the Python launcher exports ANAKOT_HOME (set by
// _apply_profile_override) before spawning the TUI. Falling back to
// ~/.anakot matches get_anakot_home()'s default.
const bootFilePath = () => join(process.env.ANAKOT_HOME ?? join(homedir(), '.anakot'), 'tui-theme-boot.json')

// Never touch the user's real ~/.anakot from test runs (the TS suite has no
// ANAKOT_HOME isolation fixture).
...
export interface BootSeedResult {
  ...
  /** True when the cached config pin was seeded into ANAKOT_TUI_THEME. */
  seededPin: boolean
}
...
export function seedBootEnvironment(boot: BootTheme | null, env: NodeJS.ProcessEnv): BootSeedResult {
  ...
  if (!boot || env.ANAKOT_TUI_THEME || env.ANAKOT_TUI_LIGHT) {
    return result
  }
  ...
  if (boot.mode) {
    env.ANAKOT_TUI_THEME = boot.mode
  ...
  if (
    ...
    !env.ANAKOT_TUI_BACKGROUND
  ) {
    env.ANAKOT_TUI_BACKGROUND = boot.background
...
export function invalidateBootBackground(env: NodeJS.ProcessEnv = process.env): boolean {
  if (!seeded.seededBackground || env.ANAKOT_TUI_BACKGROUND !== seeded.seededBackground) {
    return false
  }

  delete env.ANAKOT_TUI_BACKGROUND
...
/** True when this boot replayed a cached config pin into ANAKOT_TUI_THEME.
