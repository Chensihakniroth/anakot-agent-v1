...
import { terminalBackgroundHex } from '@anakot/ink'
...
export const debugCommands: SlashCommand[] = [
  ...
  {
    help: 'rescan $ANAKOT_HOME/tui-widgets and (re)register user widget apps',
    name: 'widgets-reload',
    run: (_arg, ctx) => {
  ...
  {
    help: 'write a V8 heap snapshot + memory diagnostics (see ANAKOT_HEAPDUMP_DIR)',
    name: 'heapdump',
    run: (_arg, ctx) => {
      ...
      ctx.transcript.panel('Theme', [
        {
          rows: [
            ...
            ['ANAKOT_TUI_BACKGROUND', process.env.ANAKOT_TUI_BACKGROUND ?? '(unset)'],
            ['ANAKOT_TUI_THEME', process.env.ANAKOT_TUI_THEME ?? '(unset)'],
