import type { RunExternalProcess } from '@anakot/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`anakot ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching anakot: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`anakot ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
