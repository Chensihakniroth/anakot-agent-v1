...
/**
 * User widget apps — Anakot authors its own TUI widgets, mirroring the
 * Python plugin contract: drop `<name>.mjs` into `$ANAKOT_HOME/tui-widgets/`,
 ...
 * React.createElement.
 *
 * Trust model matches `~/.anakot/plugins/`: files under ANAKOT_HOME execute
...
const widgetsDir = () => join(process.env.ANAKOT_HOME?.trim() || join(homedir(), '.anakot'), 'tui-widgets')
 ...
 *  every change, so a widget Anakot writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Anakot) ever writes must hot-load too — a 10s poll here read
