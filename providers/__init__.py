...
3. Pip-installed plugins: distributions exposing a ``anakot_agent.plugins``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$ANAKOT_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$ANAKOT_HOME/plugins/model-providers/`` if it exists."""
    try:
        from anakot_constants import get_anakot_home

        d = get_anakot_home() / "plugins" / "model-providers"
...
def _installed_plugins_dir() -> Path | None:
    """Return ``$ANAKOT_HOME/plugins/`` if it exists.

    This is where ``anakot plugins install`` clones a plugin — flat, one
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple ANAKOT_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_anakot_user_provider_{safe_name}"
...
def _discover_entry_point_providers() -> None:
    """Import pip-installed provider plugins via the ``anakot_agent.plugins``
    ...
    * **Provider targets only.** The ``anakot_agent.plugins`` group is shared
    ...
    first, so filesystem plugins (bundled + ``$ANAKOT_HOME``) keep their
    ...
    try:
        from anakot_cli.plugins import _get_disabled_plugins, _get_enabled_plugins
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$ANAKOT_HOME/plugins/model-providers/<name>/``
      2b. Plugins installed by ``anakot plugins install`` at
          ``$ANAKOT_HOME/plugins/<name>/`` that declare ``kind: model-provider``
    ...
    # 0. Pip-installed plugins — entry points in the ``anakot_agent.plugins``
    ...
    #    filesystem steps means a bundled or ``$ANAKOT_HOME`` profile of the
    ...
    # 1. Bundled plugins — shipped with anakot-agent.
    ...
    # 2. User plugins — under $ANAKOT_HOME/plugins/model-providers/<name>/.
    ...
    # 2b. Plugins installed by ``anakot plugins install`` / the plugin index.
    #     Those clone into $ANAKOT_HOME/plugins/<name>/ — flat, NOT under
