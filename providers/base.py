...
def _profile_user_agent() -> str:
    """Return a ``anakot-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from anakot_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"anakot-cli/{_ver}"
    except Exception:
        return "anakot-cli"
...
class ProviderProfile:
    ...
    # describe how to launch it; anakot_cli/auth.py's
    ...
    def create_client(self, **client_kwargs: Any) -> Any | None:
        ...
        a profile registered from ``~/.anakot/plugins/model-providers/`` or a
    ...
    def fetch_models(
        ...
        *,
        ...
        # anakot-cli UA so the catalog endpoint is reachable.
