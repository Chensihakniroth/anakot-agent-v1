...
from anakot_constants import display_anakot_home as _display_anakot_home
...
def _cfg_get_provider(params):
    from anakot_cli.models import list_available_providers, normalize_provider
...
def _cfg_get_mtime(params):
    cfg_path = _anakot_home / "config.yaml"
    try:
...
_CONFIG_GETTERS = {
    ...
    "profile": lambda params: {"home": str(_anakot_home), "display": _display_anakot_home()},
...
def _readiness_check(rid, params, probe):
    ...
    optional ``profile`` param's ANAKOT_HOME + ``.env`` secret scope (ContextVars: concurrent checks
    ...
    # (``ANAKOT_CODEX_BASE_URL`` for openai-codex) fails closed and the UI shows onboarding.
...
def _(rid, params: dict) -> dict:
    ...
    added after boot — the Models page, a picker key, ``anakot setup`` from a shell — flips it
    ...
    try:
        ...
        from anakot_cli.free_tier_bootstrap import wait_for_record
    ...
    try:
        ...
        def probe(profile, scoped):
            ...
            if (not provider_configured and provider == "bedrock"
                ...
                return fail("No Anakot provider is configured.", source)
            ...
            from anakot_cli.anon_auth import route_is_welcome_host
...
def _(rid, params: dict) -> dict:
    ...
    force-redaction pipeline as ``anakot debug share --nous``; redaction is NOT client-controllable
    ...
    try:
        from anakot_cli.debug import _redact_log_text, build_nous_bundle, collect_share_bundle
