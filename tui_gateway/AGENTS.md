...
activate with `anakot --tui` or `ANAKOT_TUI=1`. `tui_gateway` is ALSO the backend the Desktop app
...
(`4000` + key path) and, under `ANAKOT_TEST_ISOLATION=1`, raises `ContractViolation` when a handler's
...
the launch home, never `~/.anakot`. The first non-launch home hosted flips
...
**A method that sets only `get_anakot_home_override()` is half-bound**: config paths resolve to the
...
(`anakot -p X` workers, `key_cmd` helpers, browser drivers), never `dict(os.environ)`. Grep for
...
themselves come from `anakot_cli/commands.py` (`anakot_cli/AGENTS.md`).
...
npm run dev       # watch mode (rebuilds anakot-ink + tsx --watch)
npm start         # production
npm run build     # full build (anakot-ink + tsc)
