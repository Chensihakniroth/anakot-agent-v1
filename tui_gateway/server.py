...
from anakot_constants import (
    get_anakot_home, get_anakot_home_override, get_process_anakot_home, profile_name_for_home,
    reset_anakot_home_override, set_anakot_home_override)
...
from tools.environments.local import anakot_subprocess_env
...
_anakot_home = _ANAKOT_HOME_AT_IMPORT = get_anakot_home()
load_anakot_dotenv(anakot_home=_anakot_home, project_env=Path(__file__).parent.parent / ".env")
...
_CRASH_LOG = os.path.join(_anakot_home, "logs", "tui_gateway_crash.log")
...
with contextlib.suppress(Exception):
    from anakot_cli.banner import prefetch_update_check
...
_SLASH_WORKER_TIMEOUT_S = max(5.0, env_float("ANAKOT_TUI_SLASH_TIMEOUT_S", 45.0))
...
def _resolve_ws_orphan_reap_grace() -> float:
    ...
    return _ws_orphan_setting("ANAKOT_TUI_WS_ORPHAN_REAP_GRACE_S", "ws_orphan_reap_grace_s", 20.0)
...
_WS_ORPHAN_ACTIVITY_STALE_S = _ws_orphan_setting("ANAKOT_TUI_WS_ORPHAN_ACTIVITY_STALE_S", "ws_orphan_activity_stale_s", 600.0)
...
_rpc_pool_workers = max(2, env_int("ANAKOT_TUI_RPC_POOL_WORKERS", 8))
...
_current_runtime_session_record: contextvars.ContextVar[dict | None] = contextvars.ContextVar(
    "anakot_gateway_runtime_session_record", default=None)
...
_current_rpc_method: contextvars.ContextVar[str] = contextvars.ContextVar("anakot_gateway_rpc_method", default="")
...
def _prepend_tool_paths(env: dict[str, str]) -> dict[str, str]:
    ...
    ~/.local/bin to PATH so slash_worker children resolve Anakot-managed CLIs under the Desktop's minimal PATH."""
    managed_bin = ""
    with contextlib.suppress(Exception):
        managed_bin = str(Path(get_anakot_home()) / "bin")
...
class _SlashWorker:
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the anakot agent → needs provider credentials. Tier-1 secrets
        ...
        # Prepend the Anakot venv bin dir and the user-local bin dir to PATH so slash_worker child processes
        # can resolve Anakot-managed CLIs (browser-use, uvx) even when the parent gateway was launched with
...
_SESSION_TTL_S = max(0.0, env_float("ANAKOT_TUI_SESSION_TTL_S", float(6 * 3600)))
...
_EXIT_FLUSH_BUDGET_S = max(0.0, env_float("ANAKOT_TUI_EXIT_FLUSH_BUDGET_S", 5.0))
_INCREMENTAL_FLUSH_INTERVAL_S = max(0.0, env_float("ANAKOT_TUI_SESSION_FLUSH_INTERVAL_S", _REAPER_SCAN_S))
...
def _launch_state_db_path() -> Path:
    """Launch profile's ``state.db`` at call time: the patched ``_anakot_home`` when a test changed
    it, else the live process home — resolved through :func:`get_process_anakot_home`, which honours
    ``ANAKOT_HOME`` but ignores the context-local override. The desktop multiplex cron ticker sets
    ...
    import time lets a harness that redirects ``ANAKOT_HOME`` after import be honoured (#112692)."""
    home = _anakot_home if _anakot_home != _ANAKOT_HOME_AT_IMPORT else get_process_anakot_home()
...
def _profile_db(params: dict | None = None, *, writer: bool = False):
    ...
    anakot_cli.web_routers.profiles._read_profile_db."""
    ...
    if (profile_home := _profile_home(profile)) is None:
        ...
        try:
            if writer:
                ...
                from anakot_cli.web_server_sessions import _open_session_db_at_path
...
def _canonical_profile_request(name: str) -> str:
    ...
    installation details — unless a real named profile of that name exists (``anakot`` is a legal
    id), in which case it wins; other unknown names keep failing closed in ``_profile_home``.
    """
    if name.casefold() in {".anakot", "anakot"}:
        from anakot_cli import profiles as profiles_mod
        # Check the profiles root directly: get_profile_dir rejects "anakot" as a
...
# backend, so calls carry ``profile`` → open that profile's db and bind its ANAKOT_HOME (ContextVar
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_anakot_home).resolve():
...
def _profile_scoped(handler):
    """Bind ``params['profile']``'s full runtime scope (ANAKOT_HOME + secrets + terminal policy) around a
...
def _bind_build_profile_scopes(profile_home: "str | None") -> "_TurnScopes | None":
    """Bind a session profile's ANAKOT_HOME / secret / terminal scopes for an agent build. ``None`` is the
    ...
    if profile_home:  # secret/terminal helper failed: keep at least the home + terminal refusal scope
        scopes.home = set_anakot_home_override(profile_home)
        with contextlib.suppress(Exception):
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    def _build() -> None:
        ...
        try:
            ...
            # Global-remote: bind the session profile's ANAKOT_HOME and hand the agent that profile's db —
...
def _active_config_path() -> Path:
    ...
    return Path(override if isinstance(override, str) and override else _anakot_home) / "config.yaml"
...
def _set_session_context(session_key: str, cwd: str | None = None, *, ui_session_id: str = "") -> list:
    with contextlib.suppress(Exception):
        ...
        # Live conversation id for subprocess ANAKOT_SESSION_ID: an explicitly empty contextvar is authoritative
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ.update(ANAKOT_GATEWAY_SESSION="1", ANAKOT_EXEC_ASK="1", ANAKOT_INTERACTIVE="1")
...
_TOUR_BRIDGE_UNAVAILABLE = json.dumps({
    "success": False,
    "error": ("No Anakot Desktop window answered the tour request. The tour is driven by the desktop app's "
              "renderer, which updates separately from this backend, so an app build older than the tour tool "
              "has nothing listening. Update the Anakot Desktop app and start a new session. Do not retry tour "
              "in this session.")})
...
def _env_model_seed() -> str:
    """The launch-scoped model seed (``anakot --tui -m``, hosted provisioning); "" when unset."""
    return (os.environ.get("ANAKOT_MODEL", "") or os.environ.get("ANAKOT_INFERENCE_MODEL", "")).strip()
...
def _resolve_model() -> str:
    ...
    with contextlib.suppress(Exception):
        from anakot_cli.models import get_preferred_silent_default_model
        return get_preferred_silent_default_model()
    return "z-ai/glm-5.2"


def _resolve_session_platform() -> str:
    """``ANAKOT_DESKTOP=1`` without ``ANAKOT_DESKTOP_TERMINAL`` → "desktop" (chat panel; the agent then
    suggests TUI-only slash commands), else "tui" (embedded terminal pane or standalone ``anakot --tui``)."""
    desktop = is_truthy_value(os.environ.get("ANAKOT_DESKTOP"))
    return "desktop" if desktop and not is_truthy_value(os.environ.get("ANAKOT_DESKTOP_TERMINAL")) else "tui"
...
def _config_model_target() -> tuple[str, str]:
    """(model, provider) selected by config.yaml — and ONLY config: the ANAKOT_MODEL launch seed fed into
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    if explicit_provider := os.environ.get("ANAKOT_TUI_PROVIDER", "").strip():
        return model, explicit_provider
    if not (explicit_model := _env_model_seed()):
        return model, None
    with contextlib.suppress(Exception):
        from anakot_cli.model_switch import resolve_startup_model_route
        from anakot_cli.models import detect_static_provider_for_model
        full_cfg = _load_cfg()
        cfg = full_cfg.get("model") or {}
        current_provider = ((str(cfg.get("provider") or "").strip().lower() if isinstance(cfg, dict) else "")
                            or os.environ.get("ANAKOT_INFERENCE_PROVIDER", "").strip().lower() or "auto")
        # Same owner as AnakotCLI/oneshot: ``custom:<name>:<model>`` selects that provider (#73943).
...
from anakot_state import _BARE_BILLING_PROVIDERS


def _is_routable_provider(provider: str) -> bool:
    with contextlib.suppress(Exception):
        from anakot_cli.runtime_provider import is_routable_provider
...
def _stored_session_runtime_overrides(row: dict | None) -> dict:
    ...
    if provider and not _is_routable_provider(provider):
        healed = None
        try:
            from anakot_cli.runtime_provider import canonical_custom_identity
...
def _persist_live_session_system_prompt(session: dict | None) -> None:
    ...
    # persist). The full scope, not ANAKOT_HOME alone: the external memory provider's system_prompt_block()
...
def _load_reasoning_config(model: str = "") -> dict | None:
    """Via the shared chokepoint :func:`anakot_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("ANAKOT_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _gui_surface_toolsets(platform: str) -> set[str]:
    """Toolsets that exist because of the CLIENT (both off ``_ANAKOT_CORE_TOOLS``; this is the one gate).
    ``platform`` is the SESSION's source, never a process env var: the desktop may drive a URL/cloud
    backend where ``ANAKOT_DESKTOP`` is unset (AGENTS.md surface rule)."""
...
def _resolve_explicit_toolsets(explicit: list[str], validate_toolset) -> list[str] | None | bool:
    """Resolve a ANAKOT_TUI_TOOLSETS pin: list, None for "all", False when nothing was valid."""
    ...
    if any(name in {"all", "*"} for name in built_in):
        if ignored := [name for name in explicit if name not in {"all", "*"}]:
            _tui_notice(f"[tui] ANAKOT_TUI_TOOLSETS=all enables every toolset; ignoring additional entries: {', '.join(ignored)}")
        return None
    if not unresolved:
        return built_in
    try:  # (enabled, disabled) MCP server names from raw config; both empty on any failure
        ...
        from anakot_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        _tui_notice(f"[tui] ignoring unknown ANAKOT_TUI_TOOLSETS entries: {', '.join(unknown)}")
    if disabled:
        _tui_notice("[tui] ignoring disabled MCP servers in ANAKOT_TUI_TOOLSETS "
...
def _load_enabled_toolsets(platform: str | None = None) -> list[str] | None:
    """The agent's toolsets for this session (None = all): an explicit ANAKOT_TUI_TOOLSETS pin; else the
    ...
    explicit = [item.strip() for item in os.environ.get("ANAKOT_TUI_TOOLSETS", "").split(",") if item.strip()]
    ...
    if explicit and validate_toolset is not None:
        ...
        if resolved is not False:
            return resolved
        fallback_notice = "[tui] no valid ANAKOT_TUI_TOOLSETS entries; using configured CLI toolsets"
    try:
        from anakot_cli.config import load_config
        ...
        if fallback_notice is not None:
            _tui_notice("[tui] no valid ANAKOT_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets")
        return None
...
def _get_usage(agent) -> dict:
    ...
    # Dev-only live credits-spent readout, gated on ANAKOT_DEV_CREDITS so the payload stays clean otherwise.
    if is_truthy_value(os.environ.get("ANAKOT_DEV_CREDITS")):
        with contextlib.suppress(Exception):
...
def _probe_config_health(cfg: dict) -> str:
    ...
    if isinstance(display_cfg, dict):
        ...
        if personality and personality not in {"default", "none", "neutral"}:
            with contextlib.suppress(Exception):
                from anakot_cli.personality import available_personalities
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    if provider == "custom" and "provider" not in mirror and agent is not None:
        ...
        with _profile_build_scope(sess.get("profile_home") or _anakot_home):
    ...
    if live_agent:
        ...
        with contextlib.suppress(Exception):
            from anakot_cli.banner import get_available_skills
    ...
    with contextlib.suppress(Exception):
        from anakot_cli.banner import get_update_result
...
def _resolve_runtime_with_fallback(resolve_kwargs: dict | None = None) -> _RuntimeFallbackResolution:
    ...
    from anakot_cli.runtime_provider import resolve_runtime_provider
    try:
        ...
        for entry in _load_fallback_model() or []:
            ...
            if not fb_provider or not fb_model:
                continue
            try:
                from anakot_cli.fallback_config import effective_runtime_provider, resolve_entry_api_key
...
def _startup_system_prompt(cfg: dict, task_id: str) -> str:
    """Config ephemeral system prompt + ANAKOT_TUI_SKILLS preload block. Hard-fails only when EVERY requested
    skill is missing (cli.py parity): a typo'd name must not auto-block the Kanban task."""
    from anakot_cli.config import resolve_ephemeral_system_prompt_from_config
    ...
    if missing_skills:
        ...
        logger.warning("Unknown skill(s) requested, skipping: %s. Continuing with: %s. "
                       "List available skills with `anakot skills list`.", missing_display, ", ".join(loaded_skills))
    if skills_prompt:
...
def _make_agent(
    ...
    for _mod in ("anakot_cli.mcp_startup", "tui_gateway.entry"):
        with contextlib.suppress(Exception):
    ...
    ignore_rules = is_truthy_value(os.environ.get("ANAKOT_IGNORE_RULES"))
    with _sessions_lock:
        session = _sessions.get(sid)
    agent = AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("ANAKOT_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("ANAKOT_TUI_PASS_SESSION_ID")),
...
def _init_session(
    ...
    with _sessions_lock:
        _sessions[sid] = {
            ...
            # Profile-scoped ANAKOT_HOME (None = launch); SessionBranch copies the parent's (same state.db).
...
def _load_resume_transcript(db, stored_id: str, *, model_history_only: bool = False) -> tuple[list, list, list]:
    ...
    from anakot_state import SessionResumeTooLargeError
...
def _pet_gen_root():
    ...
    root = get_anakot_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(1, int(os.environ.get("ANAKOT_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)))
except (TypeError, ValueError):
...
def _spawn_trees_root():
    root = get_anakot_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
_CLI_EXEC_BLOCKED = {
    ("setup",): "`anakot setup` needs a full terminal — run it outside the TUI",
    ("gateway",): "`anakot gateway` is long-running — run it in another terminal",
    ("sessions", "browse"): "`anakot sessions browse` is interactive — use /resume here, or run browse in another terminal",
    ("config", "edit"): "`anakot config edit` needs $EDITOR in a real terminal",
}
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `anakot` is interactive — use `/anakot chat -q …` or run `anakot` in another terminal"
...
def _resolve_name(name: str) -> str:
    with contextlib.suppress(Exception):
        from anakot_cli.commands import resolve_command
