...
def _relay_root() -> Path:
    ...
    every ANAKOT_HOME, including non-``profiles/`` subdirs of ``~/.anakot``."""
    from tools.bot_mode_probe import _default_home, _anakot_root
    return _anakot_root(Path(_default_home()))
...
def _(rid, params: dict, _root=_relay_root, _run=_run_delivery,
    ...
    GATEWAY via the one-turn ``anakot -p <profile> chat -c "Bot Chat"`` transport local DMs use →
    ...
    try:
        ...
        resolved = "default" if profile.lower() == "anakot" else profile
        ...
        owner_home = live_home if live_home is not None else Path(_anakot_home)
        ...
        fd, tmp = tempfile.mkstemp(prefix="anakot-relay-dm-", suffix=".txt", text=True)
        try:
