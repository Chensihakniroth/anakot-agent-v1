...
def _is_repo_junk(root: str) -> bool:
    ...
    ANAKOT_HOME. User-created projects pointing there are still honored."""
    if not root:
        return True
    ...
    return (
        ...
        or real == anakot_home
        or real.startswith(anakot_home + os.sep))


def _is_session_cwd_junk(cwd: str) -> bool:
    """A non-git cwd that stays in flat Recents. A DESCENDANT of ANAKOT_HOME may be an
    intentional prose/data workspace, so only ANAKOT_HOME itself is excluded here."""
    if not cwd:
        return True
    from anakot_constants import get_anakot_home
    real = os.path.normcase(os.path.realpath(cwd))
    anakot_home = os.path.normcase(os.path.realpath(str(get_anakot_home())))
    return real in _non_workspace_dirs() or real == anakot_home
...
def _scan_discovered_repos_remote(conn, policy: dict) -> bool:
    ...
    host must scan its own disk so repos with zero Anakot sessions still appear in the sidebar (#81723).
