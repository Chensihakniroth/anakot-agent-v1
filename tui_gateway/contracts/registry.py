...
# name; under ``ANAKOT_TEST_ISOLATION`` (set by ``scripts/run_tests.sh`` / ``tests/conftest.py``) it
# raises, which is what makes the suite the gate.

STRICT = bool(os.environ.get("ANAKOT_TEST_ISOLATION"))
        ...
        for err in exc.errors():
            if err.get("type") == "extra_forbidden":
                ...
                return None, (f"invalid params for {contract.name}: {loc}: {err.get('msg')} — the client and "
                              "the Anakot backend are out of sync (different versions); run `anakot update` "
                              "and restart both")
    return params, None
