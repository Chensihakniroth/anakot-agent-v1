...
class ConfigProviderRef(OpenModel):
    """``anakot_cli/models.py::list_available_providers`` row."""
...
class ModelPricing(Result):
    """``anakot_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown,
...
class ModelCapabilities(Result):
    """``anakot_cli/inventory.py::_apply_capabilities``."""
...
class ModelOptionProvider(OpenModel):
    """One ``anakot_cli/inventory.py::build_models_payload`` provider row (the union of every field
...
class GoalContractSnapshot(Result):
    """``anakot_cli/goals.py::GoalContract.to_dict``."""
