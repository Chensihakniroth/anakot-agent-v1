...
method("reload.env", params=ReloadEnvParams, result=ReloadEnvResult,
       doc="Re-read ~/.anakot/.env (CLI /reload parity); built agents keep their pool until /new.")
...
class SkillBrowseItem(OpenModel):
    """``anakot_cli.skills_hub.browse_skills`` row."""
...
class SkillInspectInfo(OpenModel):
    """``anakot_cli.skills_hub.inspect_skill``; ``{}`` when the identifier resolves nowhere."""
