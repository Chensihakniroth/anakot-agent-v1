...
class OpenModel(Result):
    ...
    (the closed set is owned elsewhere: anakot_state rows, provider inventories)."""
...
class StoredSessionRow(OpenModel):
    """One ``sessions`` row as ``anakot_state`` lists it (``session.list`` / ``session.info`` rows /
    ``sessions.changed``)."""
...
class MessageReaction(OpenModel):
    """One persisted reaction row (``anakot_state_messages.set_message_reaction``); ``seen`` is
