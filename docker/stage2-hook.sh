...
#
# Wired into the image as /etc/cont-init.d/01-anakot-setup by the
...
ANAKOT_HOME="${ANAKOT_HOME:-/opt/data}"
INSTALL_DIR="/opt/anakot"

# Drop to anakot via s6-setuidgid, but skip it when already non-root.
as_anakot() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid anakot "$@"; }
...
# starts non-root. The baked install tree under /opt/anakot is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $ANAKOT_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass ANAKOT_UID/ANAKOT_GID — or the PUID/PGID aliases — which the
...
# `id -u anakot` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# anakot UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u anakot)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-anakot UID).
...
and the baked /opt/anakot install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e ANAKOT_UID=\$(id -u) -e ANAKOT_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the anakot user to that UID/GID at boot and chowns the data
...
# --- Bootstrap ANAKOT_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid anakot mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom ANAKOT_HOME paths whose parents only
# root can create (e.g. `ANAKOT_HOME=/home/anakot/.anakot` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for ANAKOT_UID/ANAKOT_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  ANAKOT_UID/
# ANAKOT_GID still win when both are set.  See #15290, salvages #25872.
ANAKOT_UID="${ANAKOT_UID:-${PUID:-}}"
ANAKOT_GID="${ANAKOT_GID:-${PGID:-}}"

if [ -n "${ANAKOT_UID:-}" ] && validate_uid_gid "$ANAKOT_UID" && [ "$ANAKOT_UID" != "$(id -u anakot)" ]; then
    echo "[stage2] Changing anakot UID to $ANAKOT_UID"
    usermod -u "$ANAKOT_UID" anakot
fi
if [ -n "${ANAKOT_GID:-}" ] && validate_uid_gid "$ANAKOT_GID" && [ "$ANAKOT_GID" != "$(id -g anakot)" ]; then
    echo "[stage2] Changing anakot GID to $ANAKOT_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$ANAKOT_GID" anakot 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised anakot user (UID 10000)
...
# anakot process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes anakot. Idempotent across container
...
#   - anakot is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G anakot 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] anakot already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" anakot 2>/dev/null; then
        echo "[stage2] Added anakot to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group anakot failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When ANAKOT_UID is remapped or the top-level $ANAKOT_HOME isn't owned by
# the runtime anakot UID, restore ownership to anakot — but ONLY for the
# directories anakot actually writes to. The full $ANAKOT_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of anakot-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_anakot_uid=$(id -u anakot)
...
chown_anakot_tree() {
    ...
    chown -R anakot:anakot "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

tree_has_non_anakot_owner() {
    target="$1"
    find "$target" \( ! -user anakot -o ! -group anakot \) -print -quit 2>/dev/null | grep -q .
}

needs_chown=false
if [ "$(stat -c %u "$ANAKOT_HOME" 2>/dev/null)" != "$actual_anakot_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $ANAKOT_HOME (targeted) to anakot ($actual_anakot_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $ANAKOT_HOME: chown the directory itself (not its contents)
    # so anakot can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$ANAKOT_HOME"; then
        :
    else
        chown anakot:anakot "$ANAKOT_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $ANAKOT_HOME failed (rootless container?) — continuing"
    fi
    # Anakot-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by anakot (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$ANAKOT_HOME/$sub" ] && tree_has_non_anakot_owner "$ANAKOT_HOME/$sub"; then
            chown_anakot_tree "$ANAKOT_HOME/$sub"
        fi
    done
fi
...
# anakot user. Hosted/container instances keep mutable state under
# $ANAKOT_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# ANAKOT_DISABLE_LAZY_INSTALLS=1. Keeping /opt/anakot root-owned and
...
# $ANAKOT_HOME/lazy-packages dir on the data volume (Dockerfile sets
# ANAKOT_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to anakot
...
# Always reset ownership of $ANAKOT_HOME/profiles to anakot on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> anakot …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as anakot and walks
...
if [ -d "$ANAKOT_HOME/profiles" ] && tree_has_non_anakot_owner "$ANAKOT_HOME/profiles"; then
    chown_anakot_tree "$ANAKOT_HOME/profiles"
fi

# Always reset ownership of $ANAKOT_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged anakot runtime even
...
# gate as profiles/).
if [ -d "$ANAKOT_HOME/cron" ] && tree_has_non_anakot_owner "$ANAKOT_HOME/cron"; then
    chown_anakot_tree "$ANAKOT_HOME/cron"
fi

# Always ensure logs/gateways is anakot-owned (#45258). Formerly healed by
...
# top-level $ANAKOT_HOME is mis-owned, so a warm volume with anakot-owned
# ANAKOT_HOME but root-owned logs/gateways would otherwise leave
# s6-setuidgid anakot mkdir failing with Permission denied. Non-recursive:
# profile leaf dirs are each created/owned by their own log/run as anakot.
if [ -d "$ANAKOT_HOME/logs/gateways" ]; then
    if refuse_symlinked_path "chown" "$ANAKOT_HOME/logs/gateways"; then
        :
    else
        chown anakot:anakot "$ANAKOT_HOME/logs/gateways" 2>/dev/null || true
    fi
fi
...
# anakot pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged anakot gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $ANAKOT_HOME is
...
# and cron/.
if [ -d "$ANAKOT_HOME/platforms/pairing" ] && tree_has_non_anakot_owner "$ANAKOT_HOME/platforms/pairing"; then
    chown_anakot_tree "$ANAKOT_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$ANAKOT_HOME/pairing" ] && tree_has_non_anakot_owner "$ANAKOT_HOME/pairing"; then
    chown_anakot_tree "$ANAKOT_HOME/pairing"
fi

# Reset ownership of anakot-owned top-level state files on every boot.
# The targeted data-volume chown above only covers anakot-owned
# *subdirectories*; loose state files living directly under $ANAKOT_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> anakot …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged anakot runtime then hits
...
# sweep so host-owned files in a bind-mounted $ANAKOT_HOME are never
...
# entries of anakot_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$ANAKOT_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$ANAKOT_HOME/$f"; then
            :
        else
            chown anakot:anakot "$ANAKOT_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the anakot runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$ANAKOT_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$ANAKOT_HOME/config.yaml"; then
        :
    else
        chown anakot:anakot "$ANAKOT_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$ANAKOT_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as anakot user ---
# Run as anakot via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $ANAKOT_HOME values
# containing shell metacharacters. PR #30136 review item O2.
as_anakot mkdir -p \
    "$ANAKOT_HOME/backups" \
    "$ANAKOT_HOME/cron" \
    "$ANAKOT_HOME/sessions" \
    "$ANAKOT_HOME/logs" \
    "$ANAKOT_HOME/logs/gateways" \
    "$ANAKOT_HOME/hooks" \
    "$ANAKOT_HOME/memories" \
    "$ANAKOT_HOME/skills" \
    "$ANAKOT_HOME/skins" \
    "$ANAKOT_HOME/plans" \
    "$ANAKOT_HOME/workspace" \
    "$ANAKOT_HOME/home" \
    "$ANAKOT_HOME/pairing" \
    "$ANAKOT_HOME/platforms/pairing" \
    "$ANAKOT_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/anakot/.install_method (see Dockerfile), NOT written here into
# $ANAKOT_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $ANAKOT_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.anakot:/opt/data) and sometimes shared with a
...
# 'anakot update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $ANAKOT_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/anakot stamp, so deleting the data-dir copy is safe).
if [ -f "$ANAKOT_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$ANAKOT_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$ANAKOT_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$ANAKOT_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$ANAKOT_HOME/$dest"; then
            :
        else
            as_anakot cp "$INSTALL_DIR/$src" "$ANAKOT_HOME/$dest"
        fi
    fi
}
...
# the first-boot seed above depends on /opt/anakot/.env.example being present
...
# not generate one. Anakot loads $ANAKOT_HOME/.env with override=True, so
...
if [ -n "${API_SERVER_KEY:-}" ]; then
    if [ -f "$ANAKOT_HOME/.env" ] && grep -q '^API_SERVER_KEY=..*' "$ANAKOT_HOME/.env" 2>/dev/null; then
        echo "[stage2] Warning: API_SERVER_KEY is set in both the container environment and $ANAKOT_HOME/.env — the .env value wins at runtime (loaded with override=True)"
    else
        ...
        if [ -f "$ANAKOT_HOME/.env" ] && ! refuse_symlinked_path "clean" "$ANAKOT_HOME/.env"; then
            sed -i '/^API_SERVER_KEY=$/d' "$ANAKOT_HOME/.env" 2>/dev/null || true
        fi
        echo "[stage2] API_SERVER_KEY provided via container environment — skipping generation"
    fi
elif ! grep -q '^API_SERVER_KEY=..*' "$ANAKOT_HOME/.env" 2>/dev/null; then
    if refuse_symlinked_path "append" "$ANAKOT_HOME/.env"; then
        :
    else
        if [ ! -f "$ANAKOT_HOME/.env" ]; then
            ...
            (umask 077 && as_anakot touch "$ANAKOT_HOME/.env") 2>/dev/null || true
        fi
        if [ -f "$ANAKOT_HOME/.env" ]; then
            ...
            if [ -n "$_gen_key" ]; then
                ...
                sed -i '/^API_SERVER_KEY=$/d' "$ANAKOT_HOME/.env" 2>/dev/null || true
                if printf 'API_SERVER_KEY=%s\n' "$_gen_key" >> "$ANAKOT_HOME/.env" 2>/dev/null; then
                    echo "[stage2] Generated API_SERVER_KEY for the loopback gateway api_server"
                else
                    echo "[stage2] Warning: could not write API_SERVER_KEY to $ANAKOT_HOME/.env (read-only volume?) — gateway api_server (cron fires) will be unavailable"
                fi
            fi
            unset _gen_key
        else
            echo "[stage2] Warning: could not create $ANAKOT_HOME/.env — gateway api_server (cron fires) will be unavailable"
        fi
    fi
fi

# --- Sync deploy-injected Nous routing overrides into every profile .env ---
# Under multiplex, anakot_cli.auth_nous reads ANAKOT_PORTAL_BASE_URL (or its
...
# carries the value into $ANAKOT_HOME/.env and every profiles/*/.env: the
...
# (/etc/anakot/.env) composition reverted by #111600 is restored.
...
sync_routing_overrides() {
    ...
    for _name in ANAKOT_PORTAL_BASE_URL NOUS_PORTAL_BASE_URL NOUS_INFERENCE_BASE_URL; do
        eval "_value=\${$_name:-}"
        ...
        if [ ! -f "$_file" ] && ! (umask 077 && as_anakot touch "$_file") 2>/dev/null; then
...
sync_routing_overrides "$ANAKOT_HOME/.env"
for _profile_dir in "$ANAKOT_HOME"/profiles/*/; do
...
if [ -f "$ANAKOT_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$ANAKOT_HOME/.env"; then
        :
    else
        chown anakot:anakot "$ANAKOT_HOME/.env" 2>/dev/null || true
        chmod 600 "$ANAKOT_HOME/.env" 2>/dev/null || true
    fi
fi
...
# as the unprivileged `anakot` user — without this it gets EACCES on every
...
# anakot group (any group member could e.g. stop/suspend this machine), not
...
if [ -S /.fly/api ]; then
    if refuse_symlinked_path "chgrp/chmod" /.fly/api; then
        :
    elif chgrp anakot /.fly/api 2>/dev/null && chmod g+w /.fly/api 2>/dev/null; then
        echo "[stage2] Granted anakot group access to the Fly Machines API socket"
    else
...
# $ANAKOT_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `anakot update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set ANAKOT_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$ANAKOT_HOME/config.yaml" ]; then
    s6-setuidgid anakot "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$ANAKOT_HOME/auth.json" ] && [ -n "${ANAKOT_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$ANAKOT_HOME/auth.json"; then
        :
    else
        printf '%s' "$ANAKOT_AUTH_JSON_BOOTSTRAP" > "$ANAKOT_HOME/auth.json"
        chown anakot:anakot "$ANAKOT_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$ANAKOT_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via ANAKOT_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$ANAKOT_HOME/auth.json" ] && [ -n "${ANAKOT_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$ANAKOT_HOME/auth.json"; then
        :
    else
        s6-setuidgid anakot "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$ANAKOT_HOME/auth.json" \
...
# can set ANAKOT_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# ANAKOT_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$ANAKOT_HOME/gateway_state.json" ] && \
        [ "${ANAKOT_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$ANAKOT_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$ANAKOT_HOME/gateway_state.json"
        chown anakot:anakot "$ANAKOT_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$ANAKOT_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_anakot "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/anakot/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Anakot spawns for the
...
# main-wrapper.sh propagates it into the supervised ``anakot`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-anakot, dashboard, etc.).
