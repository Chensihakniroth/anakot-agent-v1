#!/bin/sh
# shellcheck shell=sh
# /opt/anakot/docker/main-wrapper.sh — wraps the container's CMD with
...
# with-contenv before touching ANAKOT_HOME / PATH. On the non-PID-1
# fallback path the Dockerfile env is still intact, so we skip the
# re-exec and continue directly.
#
# Routing:
#   no args                       → exec `anakot` (the default)
#   first arg is an executable    → exec it directly (sleep, bash, sh, …)
#   first arg is anything else    → exec `anakot <args>` (subcommand passthrough)
#
# Drop to anakot via s6-setuidgid, but skip it when already non-root.
set -e

if [ -z "${ANAKOT_MAIN_WRAPPER_ENV_READY:-}" ] && \
   [ -z "${ANAKOT_HOME:-}" ] && \
   [ -x /command/with-contenv ]; then
    export ANAKOT_MAIN_WRAPPER_ENV_READY=1
    exec /command/with-contenv sh "$0" "$@"
fi
unset ANAKOT_MAIN_WRAPPER_ENV_READY

drop() { [ "$(id -u)" = 0 ] && set -- s6-setuidgid anakot "$@"; exec "$@"; }
...
# an arbitrary non-root, non-anakot UID, the bootstrap was skipped and the
# baked image dirs (owned by the anakot build UID) are unwritable, so fail
...
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u anakot)" ]; then
    cat >&2 <<EOF
[anakot] ERROR: container started with --user $cur_uid (an arbitrary, non-anakot UID) — not supported.
...
# to the anakot user's home before dropping privileges so libraries that
...
_anakot_orig_cwd="${ANAKOT_ORIG_CWD:-$PWD}"
...
# the user's command so `anakot chat` starts in the Docker -w
...
# Anakot subcommand pass-through.
drop anakot "$@"
