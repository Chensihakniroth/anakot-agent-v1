...
# Anakot WebUI, NAS "update" flows that preserve an old entrypoint, etc.)
...
#
# Test hook: ANAKOT_TINI_SHIM_TARGET overrides the `/init` path (and
# ANAKOT_TINI_SHIM_WRAPPER the main-wrapper path) so unit tests can
# record argv without a real s6 tree.

set -e

INIT_TARGET="${ANAKOT_TINI_SHIM_TARGET:-/init}"
WRAPPER="${ANAKOT_TINI_SHIM_WRAPPER:-/opt/anakot/docker/main-wrapper.sh}"
