...
# by /etc/cont-init.d/01-anakot-setup (installed by the Dockerfile). This
...
# on the pre-s6 contract "entrypoint.sh sets up state then execs anakot"
...
echo "[anakot] WARNING: docker/entrypoint.sh is a deprecated shim under " \
...
exec /opt/anakot/docker/stage2-hook.sh "$@"
