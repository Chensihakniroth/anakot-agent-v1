"""Image-authored deployment provenance for immutable Anakot runtimes.

The image bakes ``/etc/anakot/image-provenance.json`` outside ``$ANAKOT_HOME`` and the checkout, so a
...
IMAGE_PROVENANCE_PATH = Path("/etc/anakot/image-provenance.json")
