...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    values: Dict[str, Any] = {}
    for path in (anakot_home / f"{name}.json", anakot_home / name / "config.json"):
    ...
    # Holographic stores under plugins.anakot-memory-store.
    plugins_cfg = cfg.get("plugins")
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("anakot-memory-store")
