"""Git plumbing for ``anakot update``: fork/upstream sync, trampoline-git detection, lockfile/EOL churn cleanup, orphan rescue refs, parked-branch assessment, fetch-failure classification.
...
def _git_stdout(git_cmd, args, cwd, **kw) -> Optional[str]:
    ...
    from anakot_cli.update_cmd import _git_run
...
def _prune_orphan_rescue_refs(
    ...
    """Expire old orphan rescue refs (``refs/anakot-update-backups/orphan-<branch>-<ts>-<sha>``).
    ...
    with suppress(OSError):
        prefix = f"refs/anakot-update-backups/orphan-{branch}-"
...
def _print_parked_branch_skip_warning(git_cmd: list[str], cwd: Path, current_branch: str, target_branch: str, reason: str) -> None:
    ...
    print(
        ...
        f"    git -C {cwd} status\n"
        f"    git -C {cwd} checkout {target_branch} && anakot update\n"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_anakot_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _portable_git_candidates() -> list:
    ...
    profile-scoped ANAKOT_HOME), then profile home as a fallback for custom layouts.

    The Anakot-managed PortableGit tree lives under the SHARED root (``<root>/git/...``), not the
    profile-scoped ANAKOT_HOME (``<root>/profiles/<name>``), so a profile-scoped ``anakot update`` must look
    there (monerostar review, #87876).
    """
    from anakot_cli.update_cmd import get_default_anakot_root, get_anakot_home
    candidates = []
    with suppress(Exception):
        candidates += [root / "git" / "mingw64" / "libexec" / "git-core" / "git.exe" for root in (get_default_anakot_root(), Path(get_anakot_home()))]
    return candidates
...
def _ensure_non_trampoline_git(git_cmd: list) -> list:
    ...
    from anakot_cli.update_cmd import _locate_real_git
...
def _normalize_managed_eol(git_cmd, repo_root):
    ...
    on update; install.ps1 pins ``false`` but older checkouts never got it and only ``anakot update`` can
    ...
    reuses its build-pinned ``install.ps1`` forever — so ``anakot update``, which ships with the checkout
