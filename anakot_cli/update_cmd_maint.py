"""Post-update maintenance for ``anakot update``: pre-update backup snapshot, state-db verify/restore, curator/FTS notices, FHS path guard, completion summary.

Split out of ``update_cmd.py``, which re-imports every name so ``anakot_cli.update_cmd.<name>``
...
def _sqlite_partial_completion_lines(sqlite_version: str) -> list[str]:
    ...
    return [
        ...
        "corruption bug. Anakot works, but sessions could be damaged.",
        f"  Fix: run the installer again ({_REINSTALL_ONE_LINER[bool(_m()._is_windows())]}) "
        "which installs a safe Python, then run `anakot doctor` to confirm.",
    ]
...
def _print_fts_optimize_available_notice() -> None:
    ...
    if interrupted:
        ...
        print(
            "  A previous `anakot sessions optimize-storage` run was "
            "interrupted. Search still works; re-run the command to resume "
            "and finish reclaiming disk:"
        )
        print("    anakot sessions optimize-storage")
        return
    ...
    print("  Run when convenient:  anakot sessions optimize-storage")
    print(
...
def _print_curator_recent_run_notice() -> None:
    ...
    if "\n" in summary:
        ...
        for line in summary.splitlines():
            print(f"  {line}")
        print("  (This message shows once per curator run. View anytime: anakot curator status)")
    ...
    *already_restarted_units*: systemd unit names (no ``.service``) the fleet-restart loop
    ...
    try:
        ...
        stop_result = _m()._kill_stale_dashboard_processes(
            ...
            scope_home=str(get_anakot_home()),
        )
        ...
        print("    anakot dashboard --port <port>   (or: systemctl --user restart anakot-dashboard)")
        return
    ...
    print("    anakot dashboard --port <port>")
...
def _print_update_completion(message: str) -> None:
    ...
    See #47359, #58764.
    """
    from anakot_cli.update_cmd import _branch_head_suffix
    print(f"{message}{_branch_head_suffix()}")
    action_id = os.environ.get("ANAKOT_ACTION_ID", "")
    if len(action_id) == 32 and all(char in "0123456789abcdef" for char in action_id):
        print(f"=== anakot-update completed {action_id} ===")
...
def _post_update_sqlite_runtime_status():
    ...
    from anakot_cli.sqlite_runtime import probe_sqlite_runtime
...
def _print_verified_update_completion(message: str) -> bool:
    """Print a success completion only after probing the next Anakot runtime."""
    from anakot_cli.update_cmd import _post_update_sqlite_runtime_status
...
def _print_update_summary(*, node_failures: list, desktop_build_ok: bool, pre_update_version: str | None) -> bool:
    ...
    See #88251.
    """
    from anakot_cli.update_cmd import _post_update_sqlite_runtime_status, _update_complete_message
    sqlite_runtime_ok, sqlite_info = _post_update_sqlite_runtime_status()
    if sqlite_info is None:
    ...
    if node_failures or not desktop_build_ok or not sqlite_runtime_ok:
        ...
        if not desktop_build_ok:
            print("  Run `anakot desktop` to retry the desktop rebuild.")
...
def _restore_state_db_from_snapshot(state_path: Path, snap_state: Path) -> bool:
    ...
    """
    from anakot_cli.backup import _foreign_db_holder_pids, verify_sqlite_integrity
    from anakot_cli.sqlite_safe_read import LiveConnectionError, offline_file_access
    holders = _foreign_db_holder_pids(state_path)
    if holders:
        print(
            f"  ✗ Auto-restore refused: process(es) {holders} still hold "
            "state.db or its WAL open. Stop them (anakot gateway stop), "
    ...
    try:
        ...
        print(
            ...
            "handles (or restart Anakot) and retry."
        )
        return False
...
def _verify_and_restore_one_state_db(home: Path, *, label: str) -> None:
    ...
    try:
        from anakot_cli.backup import _quick_snapshot_root, verify_sqlite_integrity
...
def _verify_and_restore_state_dbs_post_update() -> None:
    ...
    with _best_effort('Sibling-profile state.db guard sweep failed: %s'):
        from anakot_cli.backup import _sibling_profile_homes
...
def _print_bundled_skills_sync_report() -> None:
    ...
    if result.get("user_modified"):
        ...
        print("    → see them: anakot skills list-modified  (diff/reset to resume updates)")
...
def _ensure_fhs_path_guard() -> None:
    ...
    no-op on non-Linux/non-root/non-FHS or when ``bash -i -c 'command -v anakot'`` resolves."""
    from anakot_cli.update_cmd import _m
    ...
    try:
        ...
        return
    # Only for FHS-layout installs (link at /usr/local/bin/anakot).
    fhs_link = Path("/usr/local/bin/anakot")
    ...
    try:
        probe = subprocess.run(
            ...
            # recovery path in `anakot gateway restart` (`systemd_restart()`) as of PR #20949.
            capture_output=True,
...
def _ensure_acp_launcher() -> None:
    r"""Self-heal a ``anakot-acp`` launcher next to ``anakot`` (mirrors install.sh): ACP hosts
    resolve it on the login-shell PATH but the console script lives in the venv. The shim
    delegates to the sibling ``anakot acp``, correct for every layout.

    No-op on Windows (install.ps1 stages launchers into ``$AnakotHome\bin``, never
    ...
    for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin")):
        anakot_cmd = bin_dir / "anakot"
        acp_cmd = bin_dir / "anakot-acp"
        try:
            if not (anakot_cmd.is_file() or anakot_cmd.is_symlink()):
                continue
...
def _verify_state_db_after_snapshot(snapshot_id: str) -> None:
    ...
    _src_path = get_anakot_home() / "state.db"
    ...
    _snap_state = _quick_snapshot_root(get_anakot_home()) / snapshot_id / "state.db"
...
def _run_quick_snapshots() -> Optional[str]:
    ...
    from anakot_cli.update_cmd import _record_update_step
    ...
    with _best_effort('Sibling profile snapshots failed: %s'):
        from anakot_cli.backup import create_pre_update_snapshots_all_profiles
...
def _run_full_backup() -> None:
    """Zip ANAKOT_HOME under ``backups/`` (restorable via ``anakot import``). Never raises."""
    try:
    ...
    from anakot_cli.sizefmt import format_bytes
    # display_anakot_home so the user sees ~/.anakot/...
    try:
        from anakot_constants import get_anakot_home, display_anakot_home
        display_path = f"{display_anakot_home()}/{out_path.relative_to(get_anakot_home())}"
    except Exception:
        display_path = str(out_path)

    print(f"  Saved:    {display_path} ({format_bytes(size_bytes)}, {elapsed:.1f}s)")
    print(f"  Restore:  anakot import {out_path}")
...
def _run_pre_update_backup(args) -> Optional[str]:
    ...
    ``full`` — quick snapshot PLUS a zip of ANAKOT_HOME under ``backups/`` (``anakot import``).
...
def _sweep_bytecode_after_update(branch: str) -> None:
    ...
    # symbols in anakot_constants or lazy_deps. The dependency install above may also have regenerated
...
def _sync_profiles_after_update() -> None:
    ...
    # ANAKOT_HOME, so sync_skills()'s module-level ANAKOT_HOME cache can't skew it.
    with suppress(Exception):
        from anakot_cli.profiles import list_profiles, seed_profile_skills
    ...
    with suppress(Exception):
        # See #44792.
        from anakot_cli.profiles import backfill_profile_envs
...
def _refresh_cua_driver_after_update() -> None:
    ...
    if (
        ...
        from anakot_cli.tools_config import install_cua_driver
        print()
...
def _print_plugin_compat_notice() -> None:
    ...
    from anakot_cli.plugin_compat import compat_report, removal_in_effect, summary_lines
...
def _print_profiles_without_credentials_notice() -> None:
    from anakot_cli.profile_credential_audit import print_profiles_without_credentials_notice
...
def _print_post_update_notices_and_self_heals() -> None:
    ...
    from anakot_cli.update_cmd import _m, _print_curator_first_run_notice, _print_curator_recent_run_notice
    ...
    def _migrate_windows_bin_path() -> None:
        ...
        from anakot_cli._install_repair import migrate_windows_bin_path
    ...
    for message, step in (
        ...
        ('anakot-acp launcher self-heal failed: %s', _ensure_acp_launcher),
        ...
        # Legacy ANAKOT_NEMO_RELAY_ATIF_*/ATOF_* vars produce no traces since the Relay cutover;
...
def _migrate_relay_exporter_env() -> None:
    from anakot_cli.relay_plugin_migrate import run_relay_migration_after_update
...
def _run_post_update_maintenance(
    *, assume_yes, gateway_mode, pre_update_snapshot_id, had_desktop_app_before_update, node_failures, desktop_build_ok,
    ...
    from anakot_cli.update_cmd import _check_and_apply_config_migration, _m
    ...
    if sys.platform == "darwin" and had_desktop_app_before_update:
        print()
        print(
            "  ℹ macOS: if Anakot re-prompts for permissions you already "
            "granted (toggle shows ON), the stored grant is stale — run "
            "`tccutil reset ScreenCapture com.nousresearch.anakot` (repeat "
    ...
    try:
        # See #95596.
        from anakot_cli.macos_tcc_anchor import ensure_tcc_anchor
        ensure_tcc_anchor()
    except Exception:
    ...
    with _best_effort('Model catalog seed during update failed: %s'):
        from anakot_cli.model_catalog import seed_cache_from_checkout
