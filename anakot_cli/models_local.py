...
disk cache). Split out of ``anakot_cli.models``; helpers still defined there are looked up on
``anakot_cli.models`` at call time so ``patch("anakot_cli.models.<name>")`` mocks keep intercepting.
"""
...
from anakot_cli.urllib_security import url_origin

# Log-record parity with the origin module.
logger = logging.getLogger("anakot_cli.models")
...
def probe_ollama_local_models(
    ...
    from anakot_cli.models import _ANAKOT_USER_AGENT, _get_ollama_base_url, _urlopen_model_catalog_request
    ...
    if failed_at is not None:
        ...
        _OLLAMA_LOCAL_PROBE_FAILURE_CACHE.pop(failure_key, None)
    try:
        request_headers = {"User-Agent": _ANAKOT_USER_AGENT, **(headers or {})}
...
def _ollama_local_catalog(force_refresh: bool) -> list[str]:
    ...
    Cloud)."""
    from anakot_cli.models import _get_provider_config_dict, fetch_api_models
    if force_refresh:
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    return {"User-Agent": _ANAKOT_USER_AGENT, **({"Authorization": f"Bearer {token}"} if token else {})}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_anakot_home() / "ollama_cloud_models_cache.json"
...
def _load_ollama_cloud_cache(*, ignore_ttl: bool = False) -> Optional[dict]:
    ...
    from anakot_cli.models import _read_json_cache
