...
Mixin split out of ``cli.py``; bound onto ``AnakotCLI`` via the MRO. cli.py-internal symbols are
...
from anakot_constants import is_termux as _is_termux_environment
from rich.markup import escape as _escape
from utils import base_url_hostname, file_signature

from anakot_cli.cli_modal_mixin import _gated_confirm
...
class CLIInfoMixin:
    ...
    def show_banner(self):
        ...
        from anakot_cli.banner import build_welcome_banner
        self.console.clear()
        ctx_len = None
        ...
        if is_nous_anakot_non_agentic(getattr(self, "model", "") or ""):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Nous Research Anakot 3 & 4 models are NOT agentic and are not "
        ...
        # `anakot skills trust`. Never raises.
        try:
            ...
            if _proj_dirs:
                ...
                if _untrusted is not None:
                    ...
                    self._console_print(
                        ...
                        f"loaded — run `anakot skills trust` to enable them.[/]")
        except Exception:
    ...
    def show_help(self, arg: str = ""):
        ...
        from anakot_cli.commands import COMMANDS_BY_CATEGORY, HELP_SESSION_SUBGROUPS
        ...
        try:
            from anakot_cli.skin_engine import get_active_help_header
    ...
    def handle_bang_shell(self, text: str) -> bool:
        ...
        (tests/anakot_cli/test_bang_shell_mode.py). Returns False when the text is not a bang command or
        bang mode is disabled for this context (gateway/cron), so the caller routes normally.
        """
        from cli import _rich_text_from_ansi
        from anakot_cli.bang_shell import (
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_anakot_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_anakot_home()}/config.yaml")
            print()

    def _print_random_tip(self) -> None:
        """Best-effort discovery tip (startup + /clear); never raises."""
        try:
            from anakot_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
    ...
    def _toggle_verbose(self):
        ...
        if getattr(self, "_focus_view_enabled", False):
            ...
            try:
                from anakot_cli.focus_view import FOCUS_CONFIG_KEY
    ...
    def _usage_reset(self, force: bool = False):
        ...
        if str(self._agent_or_self("provider") or "").strip().lower() != "openai-codex":
            ...
            print("  Switch with `/model` or `anakot auth` first.")
            return
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.anakot/skills/ and queue a note for the next user turn.
