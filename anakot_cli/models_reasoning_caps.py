...
Split out of ``anakot_cli.models``. OpenRouter and
...
``anakot_cli.models`` — tests reset them there — and are read/written by attribute name.
...
# that must never block on HTTP — so without a disk copy, `anakot -p`, a cron job, or a freshly
...
def _reasoning_caps_disk_path() -> Path:
    ...
    return get_anakot_home() / "cache" / "reasoning_caps.json"
...
def _fetch_reasoning_caps_catalog(url: str, timeout: float) -> Optional[Caps]:
    ...
    m = _origin()
    try:
        req = urllib.request.Request(url, headers={"Accept": "application/json", "User-Agent": m._ANAKOT_USER_AGENT})
...
class _CapsSource:
    """One catalog's cache slots on ``anakot_cli.models`` plus how to name its URL.
    ...
    def get(self, slot: str):
        ...
        from anakot_cli.models_profile_cache import profile_slot_get
    ...
    def set(self, slot: str, value) -> None:
        if not self.per_profile:
            ...
            return
        from anakot_cli.models_profile_cache import profile_slot_set
