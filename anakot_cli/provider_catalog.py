...
The provider list shown by ``anakot model`` (CLI/TUI) and the desktop Settings → Providers tabs
...
come from :data:`anakot_cli.auth.PROVIDER_REGISTRY` (credential truth); ``display_name`` /
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `anakot model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """One descriptor per provider in the ``anakot model`` universe (:data:`CANONICAL_PROVIDERS`,
    ...
    PROVIDER_REGISTRY = _safe_import("anakot_cli.auth", "PROVIDER_REGISTRY", {})
    OPTIONAL_ENV_VARS = _safe_import("anakot_cli.config", "OPTIONAL_ENV_VARS", {})
    ...
    ANAKOT_OVERLAYS = _safe_import("anakot_cli.providers", "ANAKOT_OVERLAYS", {})
    try:
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        overlay = ANAKOT_OVERLAYS.get(slug)
