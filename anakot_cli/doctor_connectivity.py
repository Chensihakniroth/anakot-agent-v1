"""API connectivity probes for ``anakot doctor`` (split out of ``doctor.py``).
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from anakot_cli.providers import normalize_provider as _normalize_provider
...
_OPENROUTER_STATUS = {
    ...
    402: ("(out of credits — payment required)",
          ...
          "Fix: run 'anakot config set model.provider <provider>' "
...
def _anthropic_messages_probe(base: str, key: str):
    ...
    headers = {"anthropic-version": "2023-06-01", "User-Agent": _ANAKOT_USER_AGENT, **auth}
...
def _apikey_request(key: str, base_env, default_url) -> tuple:
    ...
    headers = {"Authorization": f"Bearer {key}", "User-Agent": _ANAKOT_USER_AGENT}
