...
Mixed into :class:`anakot_cli.plugins.PluginManager`. ``_resolve_hook_callback_timeout`` stays on
...
from anakot_cli.middleware import OBSERVER_SCHEMA_VERSION
...
PLUGIN_SECTIONS_START = "<!-- anakot-plugin-sections:start -->"
PLUGIN_SECTIONS_END = "<!-- anakot-plugin-sections:end -->"
...
def format_system_prompt_section(section_id: str, content: str) -> str:
    ...
    return (
        ...
        f"<!-- anakot-plugin-section-chars:{len(content)} -->\n\n{content}")
...
# Reserved event namespace prefix — only core may publish ``anakot:<event>``.
ANAKOT_EVENT_NAMESPACE = "anakot"
...
class PluginDispatchMixin:
    ...
    def _invoke_hook_callback(callback: Callable, payload: Dict[str, Any]) -> Any:
        ...
        """
        from anakot_cli.plugins import resolve_plugin_command_result
        try:
    ...
    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        ...
        """
        from anakot_cli.plugins import _resolve_hook_callback_timeout
    ...
    def _run_hook_callback_bounded(
        ...
        thread = threading.Thread(target=_runner, name=f"anakot-hook-{callback_name}"[:40], daemon=True)
        try:
            thread.start()
    ...
    def _ensure_event_worker_locked(self) -> None:
        ...
        worker = threading.Thread(
            target=self._event_worker_loop, args=(self._event_queue,), name="anakot-plugin-events",
            daemon=True,
        )
