...
Split out of ``anakot_cli.kanban_db``; origin-resident helpers are reached
...
from anakot_cli.quiet_single_query import KANBAN_WORKER_EXIT_TRAILER

if TYPE_CHECKING:
    from anakot_cli.kanban_db import Task
...
# ``ANAKOT_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``.
...
class DispatchResult:
    ...
    Code terminal like ``orion-cc``), not a Anakot profile. Expected steady-state
...
def describe_suppression(results: Iterable[Optional["DispatchResult"]]) -> str:
    ...
    bare zero-spawn count while ``anakot kanban tail`` is the only place the
...
# children): a per-tick ``anakot kanban dispatch`` process finds it empty, so
...
def _worker_log_exit_code(task_id: str, board: Optional[str] = None) -> Optional[int]:
    ...
    (``anakot_cli.quiet_single_query.exit_single_query``), so it is there whether
...
_LOG_CHROME = re.compile(r"[─━═╭╮╰╯│┃┌┐└┘]+|☤\s*Anakot")
    ...
    if sweep.exited_hook_payloads and _kb._kanban_observer_consumed("on_kanban_worker_exited"):
        ...
        for hook_fields in sweep.exited_hook_payloads:
            ...
            _kb._fire_kanban_lifecycle_hook(
                ...
                # fire in the DISPATCHER process (gateway-embedded dispatcher or ``anakot kanban
...
def _profile_exists_fn() -> Optional[Callable[[str], bool]]:
    """``anakot_cli.profiles.profile_exists``, or ``None`` when it cannot be
    ...
    """
    try:
        from anakot_cli.profiles import normalize_profile_name, profile_exists
    except Exception:
        return None
...
def _dispatch_profile_allowlist(normalize_profile_name) -> Optional[frozenset]:
    ...
    On a shared board (one ``kanban.db`` mounted across several Anakot homes),
...
def dispatch_profile_allowlist_summary() -> str:
    ...
    Surfaced by ``anakot kanban diagnostics`` so an operator on a shared board
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    """True iff a ready+assigned+unclaimed task maps to a real Anakot profile.
...
def review_dispatch_enabled() -> bool:
    """Whether review tasks dispatch automatically. Default true (Anakot ships
...
def count_running_tasks_other_boards(board: Optional[str] = None) -> int:
    ...
    Boards are matched by resolved DB path, so ``ANAKOT_KANBAN_DB`` (pins every
...
def _dispatch_lane_task(
    ...
    *,
    ...
    # would fail ``anakot -p <assignee>`` at startup and loop ready→crash→ready
    ...
    if guard_reason is not None:
        ...
        # Event so ``anakot kanban tail`` shows why the task looks stuck.
...
def _module_anakot_argv() -> list[str]:
    """Interpreter-bound Anakot CLI invocation (``anakot_cli.main`` is the
    console-script target — there is no top-level ``anakot`` package)."""
    return [sys.executable, "-m", "anakot_cli.main"]


def _absolute_anakot_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Anakot shim."""
...
def _anakot_path_argv(path: str) -> list[str]:
    """argv for a resolved Anakot executable path. Windows batch shims
    ...
    if _kb._IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_anakot_argv()
    return [_absolute_anakot_path(path)]


def _resolve_anakot_argv() -> list[str]:
    """Resolve the ``anakot`` invocation as argv for ``Popen``: ``$ANAKOT_BIN``
    ...
    anakot_cli.main`` (exactly this install; also covers shim-less cron,
    systemd ``User=``, launchd), then ``which("anakot")`` (Windows: safe PATH
    search, batch shims fall back to the module form) only when ``anakot_cli``
    is not importable. The module argv must win over PATH: a PATH-first lookup
    lets an attacker-planted ``anakot`` shadow the running install (#111569).
    Mirrors ``gateway.run._resolve_anakot_bin``; local because ``anakot_cli``
    sits below ``gateway`` in the dependency order.
    """
    import importlib.util
    import shutil

    env_bin = os.environ.get("ANAKOT_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _anakot_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _anakot_path_argv(resolved_env_bin)
        return _module_anakot_argv()
    ...
    anakot_bin = _safe_which_no_cwd("anakot") if _kb._IS_WINDOWS else shutil.which("anakot")
    if anakot_bin:
        return _anakot_path_argv(anakot_bin)
    return _module_anakot_argv()
...
def _resolve_worker_cli_toolsets(anakot_home: Optional[str]) -> Optional[list[str]]:
    ...
    tools when ``ANAKOT_KANBAN_TASK`` is set.
    """
    if not anakot_home:
        return None
    try:
        ...
        token = set_anakot_home_override(anakot_home)
        ...
        secret_token = (
            set_secret_scope(build_profile_secret_scope(Path(anakot_home)))
        ...
        _kb._log.debug(
            "kanban worker: could not resolve CLI toolsets for ANAKOT_HOME=%r (%s)",
            anakot_home,
            exc,
        )
        return None
...
def _worker_argv(task: Task, profile_arg: str, anakot_home: Optional[str]) -> list[str]:
    """Build the ``anakot -p <profile> --cli ... chat -q ...`` worker command."""
    cmd = [
        *_resolve_anakot_argv(),
        "-p", profile_arg,
        ...
        "--cli",
        # Workers run under a profile-scoped ANAKOT_HOME and so see that
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(anakot_home)
    if worker_toolsets:
...
def _open_worker_log(task: Task, board: Optional[str]):
    ...
    `anakot kanban log` reads its own file and boards sharing task ids don't
    collide."""
...
def _default_spawn(task: Task, workspace: str, *, board: Optional[str] = None) -> Optional[int]:
    """Fire-and-forget ``anakot -p <profile> chat -q ...`` subprocess.
    ...
    ``ANAKOT_KANBAN_DB`` / ``ANAKOT_KANBAN_BOARD`` / workspaces_root to the
    board the task was claimed from, so workers cannot see other boards.
    """
    if not task.assignee:
        raise ValueError(f"task {task.id} has no assignee")

    from anakot_cli.profiles import normalize_profile_name, resolve_profile_env
    ...
    try:
        ...
        # ANAKOT_PROFILE (set below) instead.
        profile_home = None
    ...
    # Inject ANAKOT_HOME so the worker reads the profile-scoped config.yaml:
    # without it the child's get_anakot_home() falls back to the DEFAULT
    # profile root because `anakot -p` applies its override before
    # anakot_constants is imported.
    if profile_home:
        env["ANAKOT_HOME"] = profile_home
    ...
    if task.tenant:
        env["ANAKOT_TENANT"] = task.tenant
    env["ANAKOT_KANBAN_TASK"] = task.id
    env["ANAKOT_KANBAN_WORKSPACE"] = workspace
    ...
    env["ANAKOT_SESSION_SOURCE"] = "kanban"
    ...
    if task.branch_name:
        env["ANAKOT_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["ANAKOT_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["ANAKOT_KANBAN_CLAIM_LOCK"] = task.claim_lock
    ...
    if task.goal_mode:
        env["ANAKOT_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["ANAKOT_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # match after `anakot -p` rewrites ANAKOT_HOME (symlink / Docker layouts).
    env["ANAKOT_KANBAN_DB"] = str(_kb.kanban_db_path(board=board))
    env["ANAKOT_KANBAN_WORKSPACES_ROOT"] = str(_kb.workspaces_root(board=board))
    _retag_legacy_worker_sessions(env["ANAKOT_KANBAN_WORKSPACES_ROOT"])
    # Board slug — defense-in-depth pin if a path is resolved without the
    # DB / workspaces env vars.
    env["ANAKOT_KANBAN_BOARD"] = _kb._normalize_board_slug(board) or _kb.get_current_board()
    # kanban_comment reads ANAKOT_PROFILE for its default author; `-p` alone
    # doesn't set the env var.
    env["ANAKOT_PROFILE"] = profile_arg
    ...
    # `--cli` is the highest-precedence TUI override; dropping ANAKOT_TUI covers
    # older anakot builds on PATH that predate the flag's precedence.
    env.pop("ANAKOT_TUI", None)

    cmd = _worker_argv(task, profile_arg, env.get("ANAKOT_HOME"))
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`anakot` executable not found on PATH. "
            "Install Anakot Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    the gateway dispatcher and ``anakot kanban dispatch`` — the standalone
...
from anakot_cli import kanban_db as _kb  # noqa: E402
from anakot_cli import kanban_db_connect as _kbc  # noqa: E402
from anakot_cli import kanban_db_workspace as _kbw  # noqa: E402
