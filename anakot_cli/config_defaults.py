"""Default configuration data for Anakot Agent: DEFAULT_CONFIG and OPTIONAL_ENV_VARS.

Pure-data leaf module — must not import from anakot_cli.config. Comments are the user-facing
docs of config.yaml.
"""
...
DEFAULT_CONFIG = {
    ...
    "toolsets": ["anakot-cli"],
    # journal_mode: SQLite journal mode for every Anakot DB. "wal" default; use "delete" on
    ...
    "session": {
        # Per-terminal `anakot -c`: each CLI session writes a breadcrumb under
        # $ANAKOT_HOME/terminal-sessions/<terminal-id>, so bare -c/--continue resumes THIS
    ...
    "agent": {
        ...
        # Default 30 min is a safety valve for wedged agents, not a target latency — an interactive `anakot
        ...
        # Anakot-level retry attempts for API errors (connection drops, timeouts, 5xx) wrapping the
        ...
        # wrapping Anakot (sandbox runner, managed platform) can describe proxy/credential/ mount
        # layout without editing SOUL.md. Env ANAKOT_ENVIRONMENT_HINT overrides it.
        "environment_hint": "",
        ...
        # share or `anakot doctor` (status channel only; prompt cache untouched). 0 = no escalation
        ...
        # gateway status (`anakot status` / fleet monitoring). Retries never stop — a signal, not a
        ...
        # ANAKOT_LOCAL_STREAM_STALE_TIMEOUT overrides.
    ...
    "terminal": {
        ...
        # sandboxes). Empty = TMPDIR/TMP/TEMP if set, else ANAKOT_HOME/cache/terminal (auto-pruned
        ...
        # (bytes); raise it for a ~/.anakot tree that legitimately exceeds 2 GiB.
        "sync_back_max_bytes": 2 * 1024 * 1024 * 1024,
        # HOME for host tool subprocesses: "auto" = host keeps the real OS-user HOME, containers use
        # ANAKOT_HOME/home; "real" = force real HOME; "profile" = force ANAKOT_HOME/home when it
        ...
        # ["/home/user/.anakot/cache/documents:/output"]. For gateway MEDIA delivery, write to
        ...
        # default for images whose entrypoints must start as root (e.g. the bundled Anakot image,
        # which drops to `anakot` via s6-setuidgid). When on, SETUID/SETGID caps are omitted.
    ...
    "web": {
        ...
        # parallel, firecrawl, keenable; tavily is opt-in keyless via `anakot tools`, not a ring
        # member). Set by the `anakot tools` picker. "free" = always anonymous endpoint even with a
    ...
    "browser": {
        ...
        # Consent to browse with the user's REAL logins locally: runs on a Anakot-managed SNAPSHOT
        # of the ACTIVE default-Chromium profile (Local State -> profile.last_used; cookies, logins,
        # prefs copied and re-synced per fresh session) driven by Anakot' packaged Chromium. The
        # snapshot dir sidesteps Chrome 136+'s default-profile debugging block and never contends
        # with the running browser. Turning off deletes ~/.anakot/browser-profile/ so credentials
        ...
        # runs `anakot browser close-profile` (kills that profile's browser tree, unsaved tabs lost)
    ...
    # write_file/patch call); restore with /rollback. Opt-in via `anakot chat --checkpoints` or
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ~/.anakot/checkpoints/ size (MB); the oldest checkpoint per project
        ...
        # must not guess. Orphans: `anakot checkpoints prune` (`--keep-orphans` to skip).
        "auto_prune": True,
        "retention_days": 7,
        "min_interval_hours": 24,
    },
    # Hard cap (chars) for one auto-loaded context file (SOUL.md, AGENTS.md, CLAUDE.md, .anakot.md,
    ...
    # Same bound for single-query mode (``anakot -q/-z``). With only ONE turn there is no
    ...
    "compression": {
        ...
        # Anakot' summarizer cannot shrink it. native = codex decides; anakot = Anakot' threshold
    ...
    "auxiliary": {
        ...
        # of skills); route cheaper via `anakot model` → auxiliary → Curator.
        "curator": _aux(600),
    ...
    "display": {
        ...
        # Interface bare `anakot`/`anakot chat` launches: "cli" (prompt_toolkit REPL) | "tui" (Ink).
        # Flags win: `--cli` forces the REPL, `--tui` / ANAKOT_TUI=1 forces the TUI.
        "interface": "cli",
        # `anakot --tui` auto-resumes the most recent human-facing session (like `anakot -c`).
        # ANAKOT_TUI_RESUME=<id> always wins.
        ...
        # `path`/`paths` = ANAKOT_HOME-relative YAML files/dirs (or conventional status_phrases.yaml
        ...
        # Gateway runtime footer on the FINAL message, e.g. `model · 68% · ~/projects/anakot`.
        ...
        # CLI/TUI/desktop, managed with `anakot pets`. No effect on prompt caching.
        "pet": {
            "enabled": False,
            "slug": "",   # active pet slug in get_anakot_home()/pets/; empty → first installed
    ...
    "dashboard": {
        ...
        # client disconnects. 0 = park forever. Env: ANAKOT_TUI_WS_ORPHAN_REAP_GRACE_S.
        ...
        # newest message older than ANAKOT_TUI_SESSION_TTL_S, default 6h) with
        ...
        # plugin. Env ANAKOT_DASHBOARD_OAUTH_CLIENT_ID / ANAKOT_DASHBOARD_PORTAL_URL win when
        ...
        # no-op. Env ANAKOT_DASHBOARD_BASIC_AUTH_USERNAME / _PASSWORD_HASH / _PASSWORD / _SECRET /
        ...
        # ANAKOT_DASHBOARD_DRAIN_SECRET; no-op unless >=256-bit, weak secrets rejected
        # (fail-closed). scope = capability label; min_secret_chars in url-safe-b64 chars.
        "drain_auth": {"scope": "drain", "min_secret_chars": 43},
        # Public URL (env ANAKOT_DASHBOARD_PUBLIC_URL): full authority (scheme + host + optional
        # prefix, e.g. https://example.com/anakot) for the OAuth redirect_uri; its hostname is
    ...
    "tts": {
        ...
        "piper": {
            ...
            # (~/.anakot/cache/piper-voices/), use_cuda, length_scale (2.0 = twice as slow),
    ...
    "voice": {
        ...
        #   chained  — STT → Anakot turn → TTS (the stt.* / tts.* providers below)
        #   gpt-live — one full-duplex voice model (OpenAI GPT-Live) owns the mic and speaker and
        #              DELEGATES every real request to Anakot (any model / provider you have
    ...
    # "Hey Anakot" hands-free wake word: always-on, on-device hotword detection that starts a fresh
    ...
    "wake_word": {
        ...
        "openwakeword": {
            # "hey_anakot" | built-in openWakeWord name ("hey_jarvis", "alexa", ...) | path to a
    ...
    # plugins/context_engine/<name>/ or ~/.anakot/plugins/.
    "context": {
        "engine": "compressor",
    ...
    "delegation": {
        ...
        # ~/.anakot/cache/delegation/ with a head+tail window + read_file offset footer, nothing
        ...
        # Total subagents a finite one-shot run (anakot chat -q / --oneshot) may spawn; 0 = unlimited.
    ...
    "moa": {
        ...
        # <anakot_home>/moa-traces/<session_id>.jsonl (or trace_dir) for auditing.
        "save_traces": False,
        "trace_dir": "",
    ...
    # and resolved; read-only — creation goes to ~/.anakot/skills/ unless create_dir redirects it.
    "skills": {
        ...
        # ANAKOT_HOME, scanned alongside the local dir.
        "create_dir": "",
        # In a git checkout, <root>/.anakot/skills/ and <root>/.agents/skills/ load as the
        # highest-precedence tier — ONLY if the root is in trusted_project_dirs. false = no scan, no
        # untrusted-skills notice.
        "project_discovery": True,
        # Trusted project roots; managed by `anakot skills trust` / `untrust`.
        ...
        # skip; ANAKOT_IGNORE_RULES suppresses the list like the other auto-injected context.
        "auto_load": [],
        # Substitute ${ANAKOT_SKILL_DIR} / ${ANAKOT_SESSION_ID} in SKILL.md content.
        "template_vars": True,
        ...
        # Advisory NVIDIA SkillEvaluator Tier 1 scan on `anakot skills install` (alongside the
        ...
        "write_approval": False,
        # Audit ledger: every skill mutation appends to ~/.anakot/skills/.curator_ledger.jsonl with
        # before/after hashes (blobs under ~/.anakot/.curator_backups/blobs/); powers `anakot
    ...
    # daemon. `anakot curator status` shows the last run.
    "curator": {
        "enabled": True,
        ...
        # aux-model cost. `anakot curator run --consolidate` overrides once.
        "consolidate": False,
        # Also prune bundled built-ins (a suppression list stops `anakot update` restoring them);
        ...
        # TTL purge of skills/.archive/: 0 = never; > 0 lets the explicit `anakot curator purge`
        ...
        # ~/.anakot/skills/ to ~/.anakot/skills/.curator_backups/<utc-iso>/skills.tar.gz (`anakot curator
    ...
    # peerName, sessions, enabled); anakot-specific overrides only here.
    "honcho": {},
    ...
    "whatsapp": {
        # reply_prefix: None = built-in "☤ *Anakot Agent*" header; "" disables; \n allowed.
    },
    ...
    #   cron job, a -q session (ANAKOT_INTERACTIVE=1 but nobody to answer), or an unattended
    ...
    "approvals": {
        ...
        # export ANAKOT_INTERACTIVE=1 (for interactive sudo prompts) but have NO user waiting to answer
        ...
        # Approve" → false. ANAKOT_TUI_NO_CONFIRM=1 skips the TUI modal.
    ...
    # are given.
    "platform_hints": {},
    # Plugin system. `enabled`/`disabled` lists are written by `anakot plugins enable|disable` and
    ...
    "plugins": {
        ...
        # 2026-09-14 removal date (see COMPAT_MANIFEST.md, `anakot plugins compat`). Stopgap only: the
    ...
    # approvals persist in ~/.anakot/shell-hooks-allowlist.json. Schema + examples:
    # website/docs/user-guide/features/hooks.md.
    "hooks": {},
    # Auto-accept shell-hook registrations without a TTY prompt (also --accept-hooks or
    # ANAKOT_ACCEPT_HOOKS=1). Gateway/cron/non-interactive runs need one of these to pick up
    # newly-added hooks.
    ...
    "auth": {  # Login policy (credentials themselves live in auth.json / .env).
        ...
        # logins automatically when Anakot has no usable login of its own. Their refresh tokens are single-use
        # and rotate, so two programs on one login can log each other out; set false to make Anakot use only
        # its own logins (`anakot auth add <provider>`). `anakot auth add openai-codex` still offers the import
        # interactively.
    ...
    "security": {  # Security: pre-exec scanning via tirith plus related guards.
        ...
        # .anakot config) always need human approval, even under yolo. Extra patterns are fnmatch
        ...
        # banner. Add via `anakot doctor --ack <id>`; remove by editing the list. Catalog:
        # anakot_cli/security_advisories.py.
        "acked_advisories": [],
    ...
    "cron": {
        ...
        # an installed provider (plugins/cron_providers/<name>/ or $ANAKOT_HOME/plugins/ <name>/),
        ...
        # Env override: ANAKOT_CRON_MAX_PARALLEL.
        ...
        "output_retention": 50,
        # Timeout (seconds) for a no-agent cron script. Env: ANAKOT_CRON_SCRIPT_TIMEOUT. Keep in
        ...
        # job's dispatch guard forever. Env: ANAKOT_CRON_SESSION_DB_TIMEOUT. 0 = unlimited.
        "session_db_timeout_seconds": 10,
        # Timeout (seconds) per media attachment send during gateway delivery; large attachments
        # (long TTS audio, big exports) need more than 30s. Env: ANAKOT_CRON_MEDIA_SEND_TIMEOUT.
        ...
        # before one reminder ping (the run is still recorded; `anakot cron incidents` shows it).
        # A green run or a different error alerts again immediately; `anakot cron incidents ack`
    ...
    # promotes dependency-satisfied todos to ready, and fires `anakot -p <assignee> chat -q ...` per
    ...
    "kanban": {
        ...
        # Per-home claim allowlist for boards shared across Anakot homes (#110995): profile names
        ...
        # Auto-run the decomposer on Triage tasks every tick. False = manual via `anakot kanban
    ...
    "code_execution": {  # execute_code settings (programmatic tool calls).
        ...
        # relative paths resolve. strict = isolated temp dir with anakot-agent's own python
    ...
    # by tool_search / tool_describe / tool_call bridges and surfaced on demand. Core Anakot tools
    ...
    "logging": {  # File logging to ~/.anakot/logs/: agent.log captures INFO+, errors.log WARNING+.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://anakot-agent.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in minutes. The gateway refreshes in the background on this cadence; the
        # CLI refetches on the next /model or `anakot model` once the cache is older. Network
    ...
    # into "text-only" and hide vision_analyze / reasoning controls (#112649). Provider keys: Anakot
    ...
    "monitoring": {
        ...
        "gateway_health_export": {  # Gateway health & diagnostics export.
            ...
            "resource_attributes": {
                "service.name": "anakot-gateway", "deployment.environment.name": "production"
            },
        },
    ...
    "gateway": {  # Gateway settings (messaging platforms: Telegram, Discord, Slack, ...).
        ...
        # to ANAKOT_GATEWAY_PLATFORM_CONNECT_TIMEOUT, which wins if set explicitly.
        ...
        # run_gateway() bridges these to ANAKOT_STARTUP_WATCHDOG / ANAKOT_STARTUP_WATCHDOG_TIMEOUT_S
        # and re-arms the live handle; explicit env wins.
        "startup_watchdog": True,
        "startup_watchdog_timeout_seconds": 300,
        # Keep writing the legacy ~/.anakot/sessions/sessions.json mirror of the routing index
        ...
        # `false` keeps per-profile gateways for good. `anakot gateway migrate --multiplex` folds a
        ...
        # adapter is parked; `anakot profile create --clone` therefore leaves messaging channels
        # behind unless --clone-channels is passed.
        "multiplex_profiles": True,
        # May `anakot update` fold this install onto a multiplexed default gateway by itself?
        ...
        # `anakot gateway migrate --multiplex` is an explicit request and always proceeds.
        ...
        # (ANAKOT_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent AND a wakeUrl is
        ...
        # ANAKOT_GATEWAY_MAX_STARTS / ANAKOT_GATEWAY_START_WINDOW_S.
        ...
        # credential/system denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.anakot/.env, auth.json). True:
        # files must be under the Anakot cache, media_delivery_allow_dirs, or fresher than
        # trust_recent_files_seconds — recommended for public-facing gateways so prompt injection
        # can't exfiltrate host secrets. Bridged to ANAKOT_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra roots (project/scratch dirs, mounted shares) from which bare file paths may be
        # uploaded; the Anakot cache is always trusted. List of absolute paths or one
        # os.pathsep-separated string; tildes expanded. Bridged to ANAKOT_MEDIA_ALLOW_DIRS. Honored
        # in both modes.
        ...
        # pure-allowlist mode. Bridged to ANAKOT_MEDIA_TRUST_RECENT_FILES. Only consulted when
        # strict is true.
        "trust_recent_files": True,
        # Recency window in seconds; 600 covers a multi-tool turn. Bridged to
        # ANAKOT_MEDIA_TRUST_RECENT_SECONDS. Only consulted when strict is true.
    ...
    # Automatic cleanup of ~/.anakot/state.db, which otherwise grows without bound and slows FTS5
    ...
    "sessions": {
        ...
        "auto_prune": True,
        # Inactive days of ended-session history to keep (= `anakot sessions prune`).
        ...
        # stay until `anakot sessions optimize-storage` runs, since the rebuild is disk-heavy on
        # large DBs. advise = `anakot update` prints a one-line notice with reclaimable size when a
        ...
        # (native/fts5_cjk/build.sh → ~/.anakot/lib/libfts5_cjk.so), 1-2 char CJK terms get exact
        # index matches instead of LIKE scans. True = use when present (inert otherwise); False =
        # never load/serve it. Bridged to ANAKOT_CJK_FTS.
        "cjk_fts": True,
        ...
        # Max active messages per session for in-memory export (`anakot sessions export`); checked
    ...
    "doctor": {
        # Per-probe timeout (seconds) for `anakot doctor --live` real-call probes.
        "live_probe_timeout": 10,
    },

    "updates": {
        # Passive version/banner checks only; explicit `anakot update --check` remains enabled.
        "check": True,
        # Pre-update backup. quick = snapshot small critical state (pairing JSONs, cron jobs,
        # config.yaml, .env, auth.json, profile DBs) into <ANAKOT_HOME>/state-snapshots/, skipping
        # files >1 GiB; restore via ``/snapshot``. full = quick PLUS a ``anakot backup`` zip in
        # <ANAKOT_HOME>/backups/ (``anakot import`` restores; slow on large homes; ``--backup``
        ...
        # update cleanly. `anakot update --switch-branch` overrides to switch for one run.
        "parked_branch_strategy": "switch",
        # Refresh an installed cua-driver during `anakot update` (best-effort, macOS only). Turn off
    ...
    "lsp": {
        ...
        "wait_mode": "document",
        "wait_timeout": 5.0,
        # Missing server binaries: auto = install via npm/go/pip into <ANAKOT_HOME>/lsp/bin/ on
        ...
        # <ANAKOT_HOME>/lsp/node_modules; a configured manager that is not installed, or an unknown
    ...
    # (SuperGrok OAuth or XAI_API_KEY) AND the toolset is enabled in `anakot tools`.
    "x_search": {
    ...
    # encrypted vault (`anakot vault add`, Desktop → Settings → Credential Vault) is always on.
    ...
    "secrets": {
        ...
        "bitwarden": {
            ...
            # ~/.anakot/.env (or the shell), never in config.yaml.
            ...
            # Last-good fallback for NETWORK/TIMEOUT outages: AES-GCM cache under ~/.anakot/cache/,
            ...
            # Auto-download bws into ~/.anakot/bin/ on first use; False = bws must be on PATH.
            "auto_install": True,
    ...
    "computer_use": {
        # cua-driver's upstream PostHog telemetry defaults ON; Anakot sets
        ...
        # #47032; Linux/WSL2 idle spin #28152). cua-driver ≥ 0.6.x supports --no-overlay; Anakot also calls
    ...
    "proxy": {
        ...
        "tunnel_port": 9090,
        # Auto-download the pinned binary into ~/.anakot/bin/; False = iron-proxy on PATH.
        "auto_install": True,
    ...
    "desktop": {  # Anakot Desktop (Electron) launch options; only affect `anakot desktop`.
        ...
        # Bridged to ANAKOT_DESKTOP_DISABLE_GPU: auto = disable GPU only on remote displays
        ...
        # to ANAKOT_DESKTOP_PASSWORD_STORE; ignored off-Linux.
        "password_store": "auto",
    ...
    "nous": {
        ...
        "anthropic_wire": "chat",
        # Nous free tier: with no other provider configured, Anakot sets up a free Nous identity on
        # first use (inference on nous/welcome + connectors) and offers `/login` (terminal:
        # `anakot auth upgrade`) to sign in. false turns the free tier off entirely: nothing is set
    ...
    "local_runtime": {
        # Off = detection-only (Anakot still finds an external llama-server you run).
        "enabled": False,
        # Pinned llama.cpp release tag; bumped by Anakot releases after validation.
        "tag": "b10964",
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": _prov(
        ...
        "OAuth2, not a static API key — this points at the credentials Anakot mints short-lived "
    ...
    "AZURE_FOUNDRY_BASE_URL": _prov(
        "Azure Foundry base URL (set via 'anakot model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": _tool(
        ...
        "read from the Anakot auth store)", "Tool-gateway user token", None, advanced=True),
    ...
    "PORCUPINE_ACCESS_KEY": _tool(
        "Picovoice access key for the Porcupine 'Hey Anakot' wake word engine (optional; "
    ...
    "ANAKOT_LANGFUSE_PUBLIC_KEY": _tool("Langfuse project public key (pk-lf-...)",
        "Langfuse public key", "https://cloud.langfuse.com", password=False),
    "ANAKOT_LANGFUSE_SECRET_KEY": _tool("Langfuse project secret key (sk-lf-...)",
        "Langfuse secret key", "https://cloud.langfuse.com"),
    "ANAKOT_LANGFUSE_BASE_URL": _tool("Langfuse server URL (default: https://cloud.langfuse.com)",
    ...
    "SLACK_ALLOWED_USERS": _msg(
        "Comma-separated Slack member IDs allowed to use Anakot, e.g. U01ABC2DEF3. Without "
    ...
    "MATRIX_USER_ID": _msg("Matrix user ID (e.g. @anakot:example.org)",
    ...
    "MATRIX_DEVICE_ID": _msg(
        "Stable Matrix device ID for E2EE persistence across restarts (e.g. ANAKOT_BOT)",
    ...
    "IRC_CHANNEL": _msg("IRC channel to join (e.g. #anakot)", "IRC channel", None),
    "IRC_NICKNAME": _msg("Bot nickname on IRC (default: anakot-bot)", "IRC nickname", None),
    ...
    "API_SERVER_MODEL_NAME": _msg(
        "Model name advertised on /v1/models. Defaults to the profile name (or 'anakot-agent' "
    ...
    "GATEWAY_PROXY_URL": _msg(
        "URL of a remote Anakot API server to forward messages to (proxy mode). When set, the "
        ...
        "Remote Anakot API server URL (e.g. http://192.168.1.100:8642)", None, advanced=True),
    "GATEWAY_PROXY_KEY": _msg(
        "Bearer token for authenticating with the remote Anakot API server (proxy mode). Must "
    ...
    # ANAKOT_TOOL_PROGRESS_MODE (deprecated; use display.tool_progress) is intentionally NOT listed:
    # this dict feeds user-facing surfaces (dashboard keys page, setup checklists), so deprecated
    # knobs stay in config._EXTRA_ENV_KEYS only. ANAKOT_TOOL_PROGRESS is unsupported.
    "ANAKOT_PREFILL_MESSAGES_FILE": _setting(
        "Path to JSON file with ephemeral prefill messages for few-shot priming",
        "Prefill messages file path", None),
    "ANAKOT_EPHEMERAL_SYSTEM_PROMPT": _setting(
