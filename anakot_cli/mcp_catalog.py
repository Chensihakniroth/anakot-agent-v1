...
Entries are added only by merging a PR into anakot-agent; presence in ``optional-mcps/`` = Nous
...
from anakot_constants import get_anakot_home, get_optional_mcps_dir
from anakot_cli._subprocess_compat import noninteractive_git_env
from anakot_cli.colors import Colors, color
from anakot_cli.config import load_config, save_config, get_env_value, save_env_value
from anakot_cli.cli_output import prompt as _prompt_input
...
class TransportSpec:
    ...
    # go through auth.env so they are prompted for and land in ~/.anakot/.env.
...
def _parse_suggest(path: Path, suggest_raw: Any) -> Optional[SuggestSpec]:
    ...
    from anakot_cli.mcp_app_detection import validate_applications
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Anakot understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    skipped too but surfaced via :func:`catalog_diagnostics` so UIs can say "update Anakot".
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    ``kind`` is ``future_manifest`` (newer than this Anakot) or ``invalid`` (malformed)."""
...
def _install_root() -> Path:
    ...
    root = get_anakot_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.anakot/mcp-installs/<name>`` and run bootstrap. Returns the dir."""
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    """Prompt for each env spec; secrets and non-secrets alike go to ~/.anakot/.env."""
...
def _build_server_config(entry: CatalogEntry, install_dir: Optional[Path]) -> dict:
    """Translate a manifest into the ``mcp_servers.<name>`` block format used by anakot_cli/mcp_config.py."""
    cfg: dict = {}
    t = entry.transport
    if t.type == "stdio":
        ...
        if entry.auth.type == "oauth":
            ...
            from anakot_cli.mcp_config import _bearer_auth_headers
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    if not server_cfg:
        return None
    try:
        from anakot_cli.mcp_config import _probe_single_server  # lazy: keep this module cheap
...
def _apply_tool_selection(
    ...
    *,
    ...
    else apply ``default_enabled``, else no filter; point the user at ``anakot mcp configure``.
    """
    print()
    name = entry.name
    configure_hint = f"`anakot mcp configure {name}`"
    ...
    if len(chosen_indices) == len(probed):
        ...
        # re-run `anakot mcp configure <name>` and unselect a tool (switches to include-mode).
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "oauth" and entry.auth.provider:
        # Provider-mediated OAuth relies on the existing `anakot auth <provider>` flow; surface
        ...
        _say(
            ...
            f"`anakot auth {entry.auth.provider}` if you have not "
    ...
    from anakot_cli.mcp_config import _save_mcp_server
    ...
    _say(
        ...
        f"Start a new Anakot session to load its tools."
    )
