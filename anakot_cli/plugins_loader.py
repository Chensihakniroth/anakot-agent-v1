...
dependency/config-schema warnings. Mixed into :class:`anakot_cli.plugins.PluginManager`.

Origin-internal names (``PluginContext``, ``LoadedPlugin``, ``_PLUGINS_DEBUG`` …) are imported lazily
through ``anakot_cli.plugins`` so tests that patch them on the origin keep working.
"""
...
from anakot_cli.plugins_discovery import ENTRY_POINTS_GROUP, _select_entry_point_group
from anakot_cli.plugins_manifest import PluginManifest, manifest_key, validate_config_schema
from anakot_cli.plugins_state import _plugin_settings_entry

if TYPE_CHECKING:  # pragma: no cover
    from anakot_cli.plugins import LoadedPlugin

logger = logging.getLogger("anakot_cli.plugins")
...
def _plugin_home_scope(home: Path):
    """Bind discovery and loading to the manager's immutable Anakot home."""
...
class PluginLoaderMixin:
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``anakot plugins list`` until then."""
        from anakot_cli.plugins import LoadedPlugin
    ...
    def _register_deferred_platform_tools(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        would miss them in ``anakot tools`` / ``platform_toolsets``. Opt-in is explicit via ``provides_tools``;
        ...
        returns ``[]``, the toolset is missing from the ``anakot tools`` checklist, and even an explicit
        ...
        """
        from anakot_cli.plugins import PluginContext, _PLUGINS_DEBUG
        ...
        try:
            ...
            # Tools registered before the raise are live: credit them or `anakot plugins list` under-reports
    ...
    def _warn_python_dependencies(self, manifest: PluginManifest) -> None:
        ...
        ``anakot plugins install``/``enable`` and after ``anakot update`` (``anakot_cli.plugin_python_deps``)
        ...
        if missing:
            logger.warning(
                ...
                "installed: %s. Run `anakot plugins enable %s` to install them, "
    ...
    def _load_plugin_scoped(self, manifest: PluginManifest) -> None:
        ...
        from anakot_cli.plugins import LoadedPlugin, PluginContext, _PLUGINS_DEBUG
        ...
        if manifest.portable:
            ...
            return
        # requires_anakot gate: skip cleanly (no import, no traceback) on a version mismatch.
        from anakot_cli.plugins_manifest import requires_anakot_error
        reason = requires_anakot_error(manifest)
        if reason:
            loaded.error = reason
        ...
        # skipped with a clear reason instead of dying on ImportError mid-register (anakot_cli.plugin_compat).
        from anakot_cli.plugin_compat import disable_reason
    ...
    def _track_tool_override_policy(self, manifest: PluginManifest, module_name: str) -> None:
        ...
        from anakot_cli.plugins import PluginContext
    ...
    def _attribute_registrations(
        ...
        # Discovery-time tools predate registration_start; credit them back or `anakot plugins list`
    ...
    def _load_portable_plugin(self, manifest: PluginManifest, loaded: LoadedPlugin) -> None:
        ...
        lookup_key = manifest_key(manifest)
        try:
            from anakot_cli.agent_plugins import load_agent_plugin
            package = load_agent_plugin(
                Path(manifest.path), get_anakot_home() / "plugin-data" / manifest.skill_namespace)
    ...
    def _directory_module_name(self, manifest: PluginManifest) -> str:
        """Profile-safe import namespace for a directory plugin: the bare ``anakot_plugins.<slug>`` for the
    ...
    def _load_directory_module(
        ...
        """Import a directory plugin as ``anakot_plugins.<slug>`` (slug from ``manifest.key`` so
        ...
        # Evict stale entries for this slug (same slug cached from another Anakot home, or an earlier force
