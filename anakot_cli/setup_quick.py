"""Streamlined setup flows: the Nous Portal one-shot (`anakot portal`), first-time quick setup,
Blank Slate setup and the `--quick` missing-items pass. Names from setup.py are imported lazily
per function so test patches on ``anakot_cli.setup`` take effect."""
...
logger = logging.getLogger("anakot_cli.setup")
...
def _blank_slate_done(config: dict, anakot_home, tools_line: str, *extra: str, intro: str | None = None) -> None:
    """Shared Blank Slate epilogue: success banner, the "enable later" hints, then the summary."""
    from anakot_cli.setup import _info, _print_setup_summary, print_success
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    _info(*([intro] if intro else []), tools_line, "  Seed skills:         anakot skills opt-in --sync",
          "  Add MCP servers:     anakot mcp add", *extra, "  Tune agent settings: anakot setup agent", None)
...
def _reload_config_into(config: dict, *, dict_only: bool = False) -> None:
    ...
    from anakot_cli.setup import load_config
...
def _run_nous_flow(config: dict, *, context: str, cancel_exc: tuple, cancel_lines: tuple, print_error) -> bool:
    ...
    single source of truth shared with ``anakot model``. False when cancelled or failed (the
    message is already printed)."""
    from anakot_cli.setup import _info
    try:
        from anakot_cli.model_setup_flows import _model_flow_nous
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Nous Portal setup (``anakot setup --portal`` / ``anakot portal``)."""
    from anakot_cli.setup import _info, _print_banner, print_error, print_info, print_success
    _print_banner("│     ☤ Anakot Setup — Nous Portal (one-shot)             │")
    ...
    def _on_error(exc: Exception) -> None:
        ...
        print()
        lead, *rest = provider_setup_failure_lines(exc, retry_command="anakot portal")
        print_error(f"  {lead}")
        for line in rest:
            print_info(f"  {line}")

    if not _run_nous_flow(config, context="`anakot portal`", cancel_exc=(KeyboardInterrupt, EOFError, SystemExit),
                          cancel_lines=(None, "  Setup cancelled.", "  You can retry later with `anakot portal`."),
    ...
    _info("  Run `anakot portal info` to inspect routing.", "  Run `anakot` to start chatting.")


def _run_first_time_quick_setup(config: dict, anakot_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice("Connect a messaging platform? (Telegram, Discord, etc.)", [
        "Set up messaging now (recommended)", "Skip — set up later with 'anakot setup gateway'",
    ], 0)
    if gateway_choice == 0:
        ...
        # platforms come alive as soon as tokens are added later (e.g. via `anakot import`).
        from anakot_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    print()
    print_success("Setup complete! You're ready to go.")
    _info(None, "  Configure all settings:    anakot setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  anakot setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    _info(None, "  macOS tip: silence ALL folder permission prompts with one switch —",
          ...
          "  your terminal (and Anakot.app if you use Desktop), or run:",
          "    open \"x-apple.systempreferences:com.apple.preference.security?Privacy_AllFiles\"",
          "  The grant is permanent — it survives every Anakot update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    ``vision`` (``read_file`` can't read images) and ``skills`` (the seeded ``anakot-agent`` skill
    ...
    try:
        ...
        from anakot_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
        all_keys = {k for k, _, _ in CONFIGURABLE_TOOLSETS}
        all_keys.update(_get_plugin_toolset_keys())
        # Plain TOOLSETS entries catch recovered toolsets like ``kanban``. Skip "anakot-*" platform
        ...
        for k, tdef in TOOLSETS.items():
            if k.startswith("anakot-") or (isinstance(tdef, dict) and (tdef.get("includes") or tdef.get("posture"))):
                continue
...
def _blank_slate_minimize_config(config: dict):
    """Turn OFF every optional config feature; all opt back in via ``anakot setup agent``."""
...
def _run_blank_slate_setup(config: dict, anakot_home, is_existing: bool):
    ...
    _info("Everything starts OFF. First we force-enable only what's required",
          ...
          "essential `anakot-agent` skill is always kept so the agent",
          "can help you drive and configure Anakot itself.", None)
    ...
    if path != 0:
        _blank_slate_walkthrough(config, anakot_home)
        return
    save_config(config)
    # Blank Slate means no bundled skills; record the opt-out so future `anakot update` runs
    # don't re-inject them.
    _set_bundled_skills_opt_out(True, "skill opt-out")
    _blank_slate_done(config, anakot_home, "  Enable tools:        anakot tools", "  Enable plugins:      anakot plugins",
                      intro="Enable anything later, on demand:")


def _blank_slate_walkthrough(config: dict, anakot_home):
    ...
    def _opted_out(_result) -> None:
        _info("No skills seeded (except the essential `anakot-agent`",
              "skill). A .no-bundled-skills marker keeps future",
              "`anakot update` runs from re-injecting them. Opt back in any",
              "time with `anakot skills opt-in --sync`.")
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `anakot tools`.")
    ...
    for header, question, yes_msg, no_msg in (
        ("Plugins", "Review and enable built-in plugins now?",
         "Manage plugins with `anakot plugins list` / `anakot plugins install`.",
         "No plugins enabled. Add later with `anakot plugins`."),
        ("MCP Servers", "Add an MCP server now?",
         "Add servers with `anakot mcp add <name> --url ... | --command ...`.",
         "No MCP servers configured. Add later with `anakot mcp add`."),
    ):
    ...
    if prompt_yes_no("Connect a messaging platform (Telegram, Discord, …)?", default=False):
        setup_gateway(config)
    save_config(config)
    _blank_slate_done(config, anakot_home, "  Enable more tools:   anakot tools")


def _run_quick_setup(config: dict, anakot_home):
    ...
    from anakot_cli.config import (get_missing_env_vars, get_missing_config_fields, check_config_version)
    ...
    if not (missing_required or missing_optional or missing_config or current_ver < latest_ver):
        ...
        _info(None, "Run 'anakot setup' and choose 'Full Setup' to reconfigure,",
    ...
    if missing_messaging:  # checklist, then prompt for each selected platform's vars
        ...
        _info("Connect Anakot to messaging apps to chat from anywhere.",
              "You can configure these later with 'anakot setup gateway'.")
