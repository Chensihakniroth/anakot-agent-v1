...
Every Anakot process that may need the free tier runs this once at boot (``anakot serve`` on a
...
(:func:`anakot_cli.anon_auth.guest_enabled`), resolves which provider carries inference, records
...
def reconcile_record() -> Optional[SetupRecord]:
    ...
    writes this process never saw (``anakot setup`` / ``anakot model`` from a shell, a hand edit).
    ...
    if anakot_home_key() != anakot_home_key(get_process_anakot_home()) or _inventory_stamp == _config_stamp():
        return record
...
def start_background_bootstrap() -> threading.Thread:
    """``anakot serve`` entry: run on a daemon thread so a slow portal never delays the socket."""
