"""Cron subcommand for anakot CLI."""
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     If no gateway is running: anakot gateway install\n"
                "                    sudo anakot gateway install --system  # Linux servers\n"
                "     Check status:  anakot cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        print(color("No scheduled jobs.\nCreate one with 'anakot cron create ...' "
...
def _delivery_fix_hint(job: Dict[str, Any]) -> str:
    return (f"Check the target with `anakot cron status` or change it with "
            f"`anakot cron edit {job.get('id', '<id>')} --deliver <target>`.")
...
def _missed_fire_issue(job: Dict[str, Any], fire_err: Dict[str, Any]) -> str:
    return (f"missed scheduled fire at {fire_err.get('at', '?')}: {_short_reason(fire_err['detail'])}. "
            "The messaging gateway was unreachable. Run `anakot gateway restart`, then "
            f"`anakot cron run {job.get('id', '<id>')}` to run it now.")
...
def cron_tick():
    ...
    try:
        ...
        print("  Check `anakot cron status` and the gateway log for details.")
        return 1
    return 0
...
def cron_incidents(args) -> int:
    ...
    if action == "ack":
        ...
        if not incident_id:
            print(color("✗ Incident ID required: anakot cron incidents ack <incident_id>", Colors.RED))
            return 1
    ...
    for inc in incidents:
        ...
        print()
    print(color(f"  {len(incidents)} incident(s)  |  ack one with: anakot cron incidents ack <id>",
                Colors.DIM))
    return 0


_PERMISSION_HINT = ("  Hint: jobs.json may be owned by another user (e.g. rewritten by a root "
                    "`docker exec anakot anakot cron ...`). Fix ownership to match the gateway "
...
def _print_ticker_health(pids: list, restart_command: str = "anakot gateway restart") -> None:
    ...
    if hb_age is None:
        ...
        print("  Cron jobs will NOT fire until the ticker writes its first heartbeat.\n"
              "  If the gateway just started, wait ~60s and re-run `anakot cron status`.\n"
...
def cron_status():
    ...
    from anakot_cli.gateway import find_gateway_pids, named_profile_served_by_running_multiplexer
    ...
    if provider != "builtin":
        ...
        if pids or gateway_alive_via_lock or served_by_multiplexer:
            if served_by_multiplexer:
                ...
                _print_ticker_health([], restart_command="anakot --profile default gateway restart")
            else:
            ...
            print("\n  To enable automatic execution for this profile:\n"
                  ...
                  "    sudo anakot gateway install --system  # Linux servers: boot-time system service\n"
                  "    anakot gateway run        # Or run in foreground")
            if active not in ("default", "custom"):
                print("\n  Alternatives for this named profile:\n"
                      ...
                      "      anakot --profile default config set gateway.multiplex_profiles true\n"
                      "      anakot --profile default gateway restart\n"
                      "    To migrate existing per-profile services with preflight checks:\n"
                      "      anakot --profile default gateway migrate --multiplex\n"
                      "  Check: anakot cron status from this profile should show its ticker heartbeat.\n")
...
def cron_doctor() -> int:
    ...
    for job, issues in findings:
        ...
        for issue in issues:
            print(f"    - {issue}")
    print()
    print(color("Review the findings above, then run `anakot cron doctor` again.", Colors.DIM))
    return 1
...
def _job_action(action: str, job_id: str, success_verb: str) -> int:
    ...
    if action == "run":
        ...
        # inherits ANAKOT_SESSION_KEY) would be orphaned mid-LLM-call, leaving the execution row
        ...
        with contextlib.suppress(Exception):
            ...
            # gateway/desktop session env (ANAKOT_SESSION_KEY); declare the channel stateless so
...
def cron_resume(args) -> int:
    ...
    from cron.jobs import AmbiguousJobReference, _anakot_now, rearm_oneshot
    try:
        job = rearm_oneshot(args.job_id, _anakot_now().isoformat() if run_now else run_at)
...
def cron_notepad(args) -> int:
    """Handle ``anakot cron notepad <job_id> [get|set|delete|list]`` (per-job durable KV).
    ...
    try:
        ...
        if key is None or (action == "set" and value is None):
            print(color(f"Usage: anakot cron notepad <job_id> {usage_args}", Colors.RED))
            return 1
        if action == "set":
...
def cron_command(args):
    ...
    print(f"Unknown cron command: {subcmd}\n"
          "Usage: anakot cron [list|create|edit|pause|resume|run|remove|resnap|status|runs|doctor|tick]")
    sys.exit(1)


def _cron_resnap(args) -> int:
    """Handle `anakot cron resnap [job_id] [--all]`."""
    ...
    if not job_id:
        ...
        print("Usage: anakot cron resnap <job_id> | anakot cron resnap --all")
        return 1
