"""``anakot debug`` debug tools for Anakot Agent."""
...
_REDACTION_BANNER = (
    "[anakot debug share: log content redacted at upload time. "
...
_USER_AGENT = "anakot-agent/debug-share"
...
# ``anakot debug`` sweeps on entry (CLI-only users). Replaced a fork-and-sleep subprocess that
# leaked ~20 MB per share.


def _pending_file() -> Path:
    return get_anakot_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    try:
        ...
        pass  # non-fatal — worst case the user runs ``anakot debug delete`` manually
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `anakot debug share` "
...
def _upload_dpaste_com(content: str, expiry_days: int = 1) -> str:
    boundary = "----AnakotDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from anakot_cli.logs import LOG_FILES
    filename = LOG_FILES.get(log_name)
    return (get_anakot_home() / "logs" / filename) if filename else None
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Anakot Desktop on the machine running the app, not by this "
...
def _capture_dump() -> str:
    """Run ``anakot dump`` and return its stdout as a string."""
    from anakot_cli.dump import run_dump
    capture = io.StringIO()
...
def collect_debug_report(
    *, log_lines: int = 200, dump_text: str = "",
    ...
    ``dump_text`` is pre-captured dump output; when empty, ``anakot dump`` is run internally.
    """
    buf = io.StringIO()
...
_NOUS_BUNDLE_FORMAT = "anakot-debug-share/1"
...
def build_debug_share(
    ...
    Shared by ``anakot debug share`` and the dashboard ``POST /api/ops/debug-share``. Blocking
    ...
    if redact:
        logger.info(
            "anakot debug share: applied force-mode redaction to log snapshots before upload")
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `anakot debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    if dpaste_urls:
        ...
        print(f"⚠️  {len(dpaste_urls)} of {len(result.urls)} upload(s) fell back to "
              ...
              "deleted with `anakot debug delete`.\n"
              "\nShare these links with the Anakot team for support.")
    else:
        print(f"\n⏱  Pastes will auto-delete in {result.auto_delete_seconds // 3600} hours.\n"
              "To delete now:  anakot debug delete <url>\n"
              "\nShare these links with the Anakot team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Anakot version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """``anakot debug share --nous``: gzip the same bundle into the Nous envelope → Nous-S3."""
    from anakot_cli.diagnostics_upload import share_to_nous
    ...
    bundle = collect_share_bundle(log_lines=log_lines, redact=redact)
    if redact:
        logger.info("anakot debug share --nous: applied force-mode redaction before upload")
    ...
    try:
        ...
        print(f"\nNous upload failed: {exc}\n"
              ...
              "Run `anakot debug share --local` to print the report instead, "
              "or `anakot debug share` to upload to a public paste service.\n", file=sys.stderr)
        sys.exit(1)
    ...
    print("\nShare this private link with the Nous team — only Nous staff "
          ...
          "  GitHub Issues        https://github.com/Chensihakniroth/anakot-agent-v1/issues\n"
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: anakot debug delete <url> [<url> ...]\n"
              "  Deletes paste.rs pastes uploaded by 'anakot debug share'.")
        return
    for url in urls:
        try:
            if delete_paste(url):
