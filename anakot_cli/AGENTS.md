# anakot_cli/ + cli.py — CLI, slash commands, config, skins, updater, profiles

Applies on top of the root `AGENTS.md`. Long-form: `website/docs/developer-guide/cli-internals.md`.

## CLI architecture

`cli.py` holds `AnakotCLI` (REPL loop, config, slash dispatch); behaviour lives in mixins
`anakot_cli/cli_commands_mixin.py`, `cli_stream_mixin.py`, `cli_status_bar_mixin.py`,
...
`AnakotCLI._SLASH_DISPATCH` (`canonical -> (method name, pass_arg)`), falling back to a
`_handle_<name>_command` method by naming convention. **There is no `elif` ladder — do not add one.**
Skill slash commands (`agent/skill_commands.py`) scan `~/.anakot/skills/` and inject as a **user
message**, never into the system prompt (prompt caching).

Rules: all interactive menu-pickers use curses (`anakot_cli/curses_ui.py`; example
`anakot_cli/tools_config.py`). Never emit `\033[K` (ANSI erase-to-EOL) in spinner/display code —
...
## Slash command registry (`anakot_cli/commands.py`)
...
`anakot_cli/goal_command.py::dispatch_goal_command` owns `/goal` parsing and manager
...
## Config system (`anakot_cli/config.py`)
...
- **.env = SECRETS ONLY** (keys, tokens, passwords): add to `OPTIONAL_ENV_VARS` with
  ...
  in `.env` is deprecated — the loader warns; canonical is `terminal.cwd`. `anakot config
...
- **Three loaders — know which you're in:** `load_cli_config()` (CLI, `cli.py`); `load_config()`
  (`anakot tools/setup`, most subcommands, `anakot_cli/config.py`, merges `DEFAULT_CONFIG`);
  `anakot_cli/config_effective.py::load_user_config_effective()` (gateway runtime via
  `gateway/run.py::_load_gateway_config`, TUI gateway `_load_cfg`, cron, `anakot send`, doctor,
  `anakot_time`/`anakot_logging`: user file + managed overlay + `${VAR}` expansion + model-key
...
- **Every `DEFAULT_CONFIG` key has a runtime reader, and every reader a registry entry.** Both drift
  ...
  key: `rg -n '"<key>"' anakot_cli/config_defaults.py` AND `rg -n '<key>' --glob '!tests' .` both hit,
...
## Skin engine (`anakot_cli/skin_engine.py`)
...
`load_skin(name)` (user `~/.anakot/skins/*.yaml` → built-ins → default; missing values inherit
...
## Update pipeline (`anakot update`) — transactional; every stage guards a real field failure
...
- **Plan** (`update_inventory.py`, `anakot update --plan`): read-only inventory — install kind, all
...
- **Apply**: git pull, or the Windows ZIP fallback — which fires ONLY when git itself failed
  ...
  root entry the ZIP does not ship (`.bytecode-fingerprint`, `.anakot-bootstrap-complete`,
  `anakot_agent.egg-info/`; tracked root entries stand in for the ZIP set before the download, the
  ...
  `anakot_cli/web_dist`, `ui-tui/{dist,node_modules,packages/anakot-ink/dist}`, `web/node_modules`,
  ...
  rebuild decision also trusts the build stamp under ANAKOT_HOME, so an install that already lost
  its artifacts in an earlier update is rebuilt instead of "forgotten" (#90495).
- **Restart-per-kind**: systemd and launchd restarts are FLEET-WIDE (every `anakot-gateway*` unit /
  `ai.anakot.gateway*` LaunchAgent), drain-first (SIGUSR1), with per-unit/per-label failure
...
- **Report**: every run writes a machine-readable receipt to `~/.anakot/logs/update_receipts/`
...
- **Nothing runs pulled code in the pre-pull interpreter** (`update_handoff.py`). The process that
  started `anakot update` imported the PRE-pull tree; once git (or the ZIP swap) has replaced the
  ...
  `anakot update <same flags> --post-swap <file>` under the venv interpreter, which imports only the
...
`_apply_profile_override()` in `anakot_cli/main.py` sets `ANAKOT_HOME` before any module import for
single-profile commands (`anakot -p x <cmd>`), so there `get_anakot_home()` scopes to the active
profile. The multiplex gateway and the Desktop/dashboard `serve` backend instead bind the active
profile per activity via a contextvar override while `os.environ["ANAKOT_HOME"]` keeps the launch
...
carries an identity marker (`anakot_constants.named_profile_has_identity`: `config.yaml`/`.env`/`SOUL.md`/
...
cron all go through `mkdir_under_anakot_home` / `_ensure_cron_dir`, which refuse a deleted or
missing named profile, #94590). Process-global per-profile slots (MCP discovery in `mcp_startup.py`,
tool registry overlays) key on `anakot_constants.anakot_home_key()`, never a single flag.
...
Migration from per-profile gateways: `anakot_cli/gateway_migrate.py` (`anakot gateway migrate
...
other service domain / UNIX user / ANAKOT_HOME outside `profiles/` — notices for the explicit command,
...
`anakot gateway restart` for a gateway Anakot did not install (custom launchd agent / unit running
...
profile's `ANAKOT_HOME` (and `HOME` for the service user) explicitly — a supervisor starts with an
...
## Nous free tier (`anakot_cli/anon_auth.py`)
...
which never contains a raw exception, a URL or a `anakot` verb) and `.copy_terminal`, so no caller
...
contextvars. `upgrade_guest` (`anakot auth upgrade`), the CLI `/login` handler and the desktop
