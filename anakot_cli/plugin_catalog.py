"""Plugin catalog — curated, Nous-approved Anakot plugins shipped with the repo.
...
an installed Anakot sees new entries and removals without updating. Any fetch failure falls back to the
...
LIVE_CATALOG_URL = "https://anakot-agent.nousresearch.com/docs/api/plugin-catalog.json"
...
class PluginCatalogEntry:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "requires_anakot": self.requires_anakot,
...
def entry_from_mapping(data: Any, label: str) -> Optional[PluginCatalogEntry]:
    ...
    return PluginCatalogEntry(
        ...
        requires_anakot=str(data.get("requires_anakot") or "").strip(),
...
def _live_cache_path() -> Path:
    ...
    return get_anakot_home() / "cache" / "plugin-catalog.json"
...
def fetch_live_catalog(*, force: bool = False) -> Optional[Dict[str, Any]]:
    ...
    ``ANAKOT_HOME/cache`` for :data:`LIVE_CATALOG_TTL_SECONDS`. ``None`` on ANY failure — callers fall
    ...
    try:
        ...
        mkdir_under_anakot_home(cache.parent)
...
def entry_capability_summary(entry: PluginCatalogEntry) -> str:
    ...
    if entry.requires_anakot:
        bits.append(f"Requires Anakot {entry.requires_anakot}.")
    return " ".join(bits)
