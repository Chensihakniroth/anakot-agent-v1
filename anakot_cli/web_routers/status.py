...
Extracted from ``anakot_cli.web_server``; app state and helpers are late-bound through
:mod:`anakot_cli.web_deps` (cycle-safe, monkeypatch-friendly).
"""
...
from anakot_cli.web_deps import LateState, late
from anakot_cli.web_server_gateway import _display_system_platform
...
from anakot_cli.config import get_config_path, get_env_path
from anakot_constants import get_process_anakot_home, profile_name_for_home
from anakot_cli.web_models import CuratorPause, LearningNodeRef, LearningNodeEdit, DebugShareRequest
from anakot_cli.web_routers._common import scoped_to_thread
...
_collect_profile_gateway_topology_cached = late("_collect_profile_gateway_topology_cached", "anakot_cli.web_server_gateway")
_config_profile_scope = late("_config_profile_scope", "anakot_cli.web_server_profiles")
_dashboard_local_update_managed_externally = late("_dashboard_local_update_managed_externally", "anakot_cli.web_server_files")
_load_configured_gateway_platforms = late("_load_configured_gateway_platforms", "anakot_cli.web_server_gateway")
_probe_gateway_health = late("_probe_gateway_health", "anakot_cli.web_server_gateway")
_require_token = late("_require_token")
_resolve_profile_dir = late("_resolve_profile_dir", "anakot_cli.web_server_profiles")
_resolve_restart_drain_timeout = late("_resolve_restart_drain_timeout", "anakot_cli.web_server_lifecycle")
_spawn_anakot_action = late("_spawn_anakot_action", "anakot_cli.web_server_gateway")
_ssh_runtime_intact = late("_ssh_runtime_intact")
app = LateState("app")  # the FastAPI instance (app.state.*)
check_config_version = late("check_config_version", "anakot_cli.config")
get_anakot_home = late("get_anakot_home", "anakot_cli.config")
...
load_config = late("load_config", "anakot_cli.config")
read_runtime_status = late("read_runtime_status", "gateway.status")
_open_session_db_for_profile = late("_open_session_db_for_profile", "anakot_cli.web_server_sessions")
...
def _count_status_active_sessions() -> int:
    ...
    /api/status never routinely writes to state.db while another Anakot process uses it."""
    from anakot_state import _default_db_path
...
async def get_ssh_ownership(request: Request):
    from anakot_cli.web_server import _SSH_OWNER_NONCE
...
async def get_health_idle(request: Request):
    ...
    from anakot_cli.web_server_idle_proof import idle_proof
    _require_token(request)
...
# Profile segment mirrors anakot_cli.profiles._PROFILE_ID_RE. Platform segment mirrors the
...
# status) are written to the per-profile home, not the process-level ANAKOT_HOME (see issue #69143). Plain
...
async def _resolve_gateway_status(profile_dir: Optional[Path], health_url) -> Dict[str, Any]:
    ...
    if liveness.runtime is not None:
        ...
        # profile is the process's own home (a pooled ``anakot --profile X serve``).
        served_name = profile_dir.name if profile_dir is not None else profile_name_for_home(get_process_anakot_home())
    ...
    if runtime:
        ...
        if not gateway_running:
            ...
            # as ``anakot gateway status`` so the sidebar strip and the CLI agree.
...
def _nous_session_validity() -> str:
    ...
    try:
        from anakot_cli.auth import get_nous_session_validity
...
async def _component_health(gateway: Dict[str, Any]) -> Dict[str, Any]:
    ...
    from anakot_cli.web_server import DASHBOARD_HEALTH
    ...
    try:
        ...
        storage_check = await run_in_threadpool(_probe_state_db, get_anakot_home())
...
async def get_status(profile: Optional[str] = None):
    ...
    """
    from anakot_cli.web_server import _GATEWAY_HEALTH_URL
    status_scope = None
    ...
    try:
        ...
        # Off-loop: on a cold Windows install the first import of anakot_cli.gateway blocks
        ...
        status = {
            ...
            "can_update_anakot": not _dashboard_local_update_managed_externally(),
        ...
        await _advisory_pressure(status, profile_dir if profile_dir else get_anakot_home())

        # Profile NAMES and ``gateway_mode`` are low-sensitivity product surface (Anakot Cloud
        ...
        if not auth["auth_required"]:
            status.update({
                "anakot_home": str(get_anakot_home()), "config_path": str(get_config_path()),
...
async def get_system_stats():
    ...
    disk/uptime when available). Non-sensitive: no env values, no paths beyond anakot home."""
    ...
    info: Dict[str, Any] = {
        **_display_system_platform(
        ...
        "anakot_version": __version__, "cpu_count": os.cpu_count()}

    def _disk():
        du = psutil.disk_usage(str(get_anakot_home()))
...
def _spawn_action(argv: list, name: str, prefix: str) -> dict:
    """Spawn a background ``anakot <argv>`` action; a spawn failure is ``500 "<prefix>: <exc>"``."""
    try:
        proc = _spawn_anakot_action(argv, name)
    except Exception as exc:
...
def _get_portal_status_sync():
    ...
    auth: Dict[str, Any] = {}
    try:
        from anakot_cli.auth import get_nous_auth_status_local
    ...
    features = []
    try:
        from anakot_cli.nous_subscription import get_nous_subscription_features
...
async def get_logs(
    ...
    from anakot_cli.logs import _read_tail, LOG_FILES
    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_anakot_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from anakot_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
