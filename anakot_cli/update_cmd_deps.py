"""Post-``anakot update`` dependency sync: venv preflight, editable reinstall, lazy refresh,
npm/Desktop rebuilds, self-lock deferral. Names are re-imported by ``update_cmd`` (so
``anakot_cli.update_cmd.<name>`` resolves/monkeypatches); origin helpers are imported lazily."""
...
from anakot_constants import project_venv_dir, venv_python_path
...
def _editable_install_is_current(git_cmd, cwd, pre_pull_sha: str | None) -> bool:
    ...
    -e .`` always rewrites console-script shims (Windows: ``anakot.exe`` quarantine, ``os error 32``
...
_UPDATE_CRITICAL_MODULES = "anakot_cli.main", "run_agent", "model_tools", "toolsets"
...
def _critical_module_import_failures(
    ...
    """
    from anakot_cli.update_cmd import _UPDATE_CRITICAL_MODULES, _m
    from anakot_constants import FIRST_PARTY_MODULE_ROOTS
    import secrets
    marker = f"__ANAKOT_IMPORT_HEALTH_{secrets.token_hex(16)}__"
    probe = (
        "import importlib, json, sys\n"
        # Importing anakot_cli.main runs the startup dotenv load, which pulls external secret
        ...
        # only our own packages count. Roots come from anakot_constants so the user hint can't drift.
        "        missing = (getattr(exc, 'name', '') or '').split('.')[0]\n"
        "        if missing in %r or missing.startswith('anakot_') or %r:\n"
...
def _capture_active_tool_dependencies() -> list[str]:
    """Snapshot Python dependencies installed explicitly through ``anakot tools``."""
    try:
        from anakot_cli import tools_config
        return tools_config.active_restorable_python_tool_dependencies()
    except Exception as exc:
        logger.debug("Could not snapshot active Anakot Tools dependencies: %s", exc)
        return []
...
def _restore_active_tool_dependencies(
    ...
    """Restore allowlisted ``anakot tools`` dependencies (from a pre-rebuild probe) into a rebuilt
    ...
    try:
        ...
        logger.debug("Anakot Tools dependency restore skipped (import failed): %s", exc)
        return
    ...
    print()
    print(f"→ Restoring {len(missing)} Anakot Tools dependency set(s)...")
    restored: list[str] = []
...
def _refresh_active_lazy_features(
    ...
    if install_cmd_prefix is None:
        print("  ⚠ Lazy refresh failed; rerun `anakot update` once resolved.")
        return False
    ...
    if status == "healthy":
        ...
        print("  Rerun `anakot update` once the upstream issue is resolved.")
        return True
...
def _refresh_active_memory_provider_dependencies() -> None:
    ...
    ``plugin.yaml`` (not Anakot extras / ``LAZY_DEPS``), so the core reinstall can strip them;
    ...
    try:
        from anakot_cli.memory_setup import _install_dependencies
    except Exception as exc:
...
def _reapply_plugin_python_dependencies() -> None:
    ...
    ``uv sync``/reinstall strips anything Anakot' own lock does not know). Non-memory plugins whose
    deps no longer resolve are disabled loudly, memory providers last. Never raises."""
    from anakot_cli.plugin_python_deps import reapply_all
    from anakot_cli.update_cmd import _m
    ...
    for name, reason in report.dropped:
        print(f"  ✗ Plugin '{name}' DISABLED: {reason}. Fix the plugin's declared dependencies, "
              f"then `anakot plugins enable {name}`.")
    if report.failed:
...
def _migrate_removed_memory_providers() -> None:
    ...
    try:
        from anakot_cli.memory_provider_migration import migrate_all_homes
        migrate_all_homes()
...
def _install_psutil_android_compat(
    ...
    from anakot_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    from anakot_cli.update_cmd import _m
    from anakot_cli.managed_uv import resolve_uv
    existing = resolve_uv()
    if existing:
        return existing
...
def _npm_lockfile_changed(anakot_root: Path) -> bool:
    from anakot_cli.update_cmd import _m
    ...
    if (web_dir / "package.json").is_file() and not _web_build_toolchain_ready(
        *_web_toolchain_roots(web_dir)):
        return True
    try:
        cache_file = _npm_lock_cache_file(anakot_root)
...
def _npm_lock_cache_file(anakot_root: Path) -> Path:
    ...
    return anakot_root / f".npm_lock_hash_{cache_key}"


def _record_npm_lockfile_hash(anakot_root: Path) -> None:
    digest = _npm_manifests_digest()
    if digest is None:
        return
    try:
        _npm_lock_cache_file(anakot_root).write_text(digest, encoding="utf-8")
    except OSError:
...
def _repair_node_deps_on_current_checkout(
    ...
    *,
    ...
    """Repair Node deps on the ``commit_count == 0`` path: a failed npm install says "re-run anakot
    ...
    if node_failures:
        ...
        print("    Fix npm and re-run `anakot update`.")
...
def _update_node_dependencies() -> list[str]:
    ...
    if not npm:
        ...
        if is_wsl() and path_npm and _m()._is_windows_npm_path(path_npm):
            ...
            print("    package manager), then re-run `anakot update`.")
            has_workspace = any(
    ...
    shared_anakot_root = get_default_anakot_root()
    ...
    # Runs before the lockfile-unchanged early return below since that's the common `anakot update` case.
    # See #43564.
    ...
    if not _m()._npm_lockfile_changed(shared_anakot_root):
    ...
    nixos_env = with_anakot_node_path(_m()._nixos_build_env())
    ...
    # The chatty npm-deprecation noise during `anakot update` comes from the *desktop* build, not this step;
    ...
    if result.returncode == 0:
        _record_npm_lockfile_hash(shared_anakot_root)
    ...
    print("    deps). Fix npm and re-run `anakot update`.")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    if not venv_python.exists():
        ...
        managed_markers = (_m().PROJECT_ROOT / ".anakot-bootstrap-complete", _m()._update_marker_path())
...
def _venv_dependency_set_stale() -> tuple[bool, str]:
    """Whether the venv's ``anakot-agent`` distribution was installed from an OLDER checkout than
    ...
    from anakot_cli.update_cmd import _m, _read_project_version
    ...
    try:
        result = subprocess.run(
            [str(venv_python), "-c", "import importlib.metadata as m; print(m.version('anakot-agent'))"],
    ...
    return stale, f"installed anakot-agent {installed}, checkout is {expected}" if stale else ""
...
# canonical case: ``anakot_cli.main`` used to import it at startup while resolving external secret sources;
...
def _abort_dependency_sync_if_self_locked(gateway_resume=None) -> None:
    ...
    a mapped ``.pyd`` -> exit 2, next launch's marker recovery finishes; the ``anakot.exe`` shim
    ...
    if _m()._reexec_dependency_sync_off_windows_shim(gateway_resume):
        ...
        # (and anakot.exe locked) exactly while the child needs it gone (#101600).
        sys.exit(0)
...
def _defer_update_for_self_lock(loaded: list[str]) -> None:
    ...
    print("  sync has been deferred: the next `anakot` launch will complete it")
...
def _rebuild_desktop_after_update(
    ...
    # build stamp under ANAKOT_HOME: it outlives a swap that lost the artifacts in an earlier run, and
    ...
    desktop_build_cmd = [sys.executable, "-m", "anakot_cli.main", "desktop", "--build-only"]
    # Capture the loud build output into update.log; retry once on failure (still-settling
    # rebuild window), then surface the tail. Put Anakot-managed Node on PATH: the desktop
    # updater chain loses shell PATH customizations, so a bare-PATH child hits `node: not found`.
    from anakot_constants import with_anakot_node_path
    build_env = with_anakot_node_path()
    for _attempt in range(2):
        ...
        if build_result.returncode == 0:
            break
    if build_result.returncode != 0:
        print("  ⚠ Desktop build failed (run `anakot desktop` to retry)")
...
def _venv_foreign_owned_paths(venv_root, limit: int = 5) -> list:
    ...
    A venv touched by ``sudo pip``/``sudo anakot`` dies mid-update with ``venv/bin/anakot`` already
    ...
    See #83529.
    A later normal ``anakot update`` then dies mid-mutation inside ``uv pip install -e .`` ("Permission
    denied (os error 13)") with ``venv/bin/anakot`` already deleted — the CLI is bricked. Same philosophy as
    the contended-venv gate (#87331): a venv we cannot safely mutate is never mutated at all.
    """
    from anakot_cli.update_cmd import _path_uid
    try:
...
def _refuse_update_if_venv_foreign_owned(project_root) -> None:
    ...
    print("  Updating now would fail midway (Permission denied) and leave Anakot broken.")
    print("  This usually happens after running anakot or pip with sudo. Offending paths:")
...
def _sync_python_dependencies_after_pull(
    ...
    if _m()._refresh_active_lazy_features(install_prefix, env=lazy_env, features=active_lazy_features):
        ...
        print(
            "  ⚠ Lazy-refresh recovery incomplete — run `anakot` again "
