...
Imported by ``anakot_cli/main.py`` BEFORE its heavy import wall (config, argparse tree, logging,
providers). Everything here must stay **stdlib-only and cheap** (os/sys file probes; no yaml, no
anakot_cli.config, no argparse). Exists so version-printing stops being reimplemented as
...
__all__ = [
    "project_root_str", "normalize_anakot_home_env", "ensure_project_root_on_path", "is_termux_env",
...
def normalize_anakot_home_env() -> None:
    """Expand ``~``/``$VAR`` in ``ANAKOT_HOME`` once, at process entry, and write it back.

    fish does not expand ``~`` inside ``VAR=~/...`` and every shell passes a quoted value
    through verbatim, so a literal tilde reaches the process. ``Path("~/.anakot")`` is
    *relative*: the many raw ``os.environ["ANAKOT_HOME"]`` readers (this fast path, the
    active_profile probe, profile re-home, the dotenv loader) would each resolve it against
    cwd and scaffold a full home under ``<cwd>/~/.anakot``. One expansion here gives every
    reader the same absolute spelling; ``anakot_constants`` expands as well for non-CLI
    entry points. A relative value that is not tilde/variable-shaped is left alone.
    """
    raw = os.environ.get("ANAKOT_HOME", "")
    if not raw.strip():
        return
    expanded = os.path.expanduser(os.path.expandvars(raw.strip()))
    if expanded != raw:
        os.environ["ANAKOT_HOME"] = expanded
...
def active_profile_may_override_home(anakot_root: str) -> bool:
    """Cheap probe: does an active non-default profile redirect ANAKOT_HOME?"""
    active = (_read_text(os.path.join(anakot_root, "active_profile")) or "").strip()
    return bool(active and active != "default")


def _default_home() -> str:
    return os.path.join(os.path.expanduser("~"), ".anakot")


def _resolved_home() -> str:
    return os.environ.get("ANAKOT_HOME", "").strip() or _default_home()
...
def container_mode_may_be_active() -> bool:
    ...
    be active".
    """
    if os.environ.get("ANAKOT_DEV") == "1" or is_container_startup_environment():
        return False
    anakot_home = os.environ.get("ANAKOT_HOME", "").strip()
    if anakot_home:
        if os.path.exists(os.path.join(anakot_home, ".container-mode")):
            return True
        parent_name = os.path.basename(os.path.dirname(os.path.normpath(anakot_home)))
        return parent_name != "profiles" and active_profile_may_override_home(anakot_home)
...
def print_fast_version_info(*, check_updates: bool = True) -> None:
    """THE canonical ``anakot --version`` output (also used by /version).
    ...
    try:
        ...
        from anakot_cli import __release_date__, __version__

        print(f"Anakot Agent v{__version__} ({__release_date__})")
    ...
    try:
        from anakot_cli.banner import UPDATE_AVAILABLE_NO_COUNT, check_for_updates
...
def try_fast_version(argv: list[str] | None = None) -> bool:
    """Handle ``anakot --version`` before the heavy import wall.
    ...
    ANAKOT_TERMUX_DISABLE_FAST_CLI escape hatch.
    """
    if argv is None:
        argv = sys.argv[1:]
    is_termux = is_termux_env()
    if is_termux and os.environ.get("ANAKOT_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
    if is_termux:
