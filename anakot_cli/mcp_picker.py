"""MCP picker — interactive `anakot mcp picker` (also the default `anakot mcp`)."""
...
from anakot_cli.colors import Colors
from anakot_cli.cli_output import prompt_yes_no
from anakot_cli.curses_ui import curses_single_select
from anakot_cli.mcp_catalog import (
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    _say(
        ...
        "Start a new Anakot session for changes to take effect."
    )
...
def _configure_tools(name: str) -> None:
    ...
    from anakot_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    """Plain-text catalog dump: `anakot mcp catalog` output and the non-curses fallback."""
    print()
    if not rows:
    ...
    print()
    _say("  Install: anakot mcp install <name>    Picker: anakot mcp", Colors.DIM)
    # Manifest-version warnings: the user's Anakot is too old to install everything listed.
    future = [d for d in catalog_diagnostics() if d[1] == "future_manifest"]
    if future:
        print()
        for name, _, _msg in future:
            _say(f"  ⚠ '{name}' requires a newer Anakot — run `anakot update` to install this entry.", Colors.YELLOW)
        print()
    print()


def show_catalog() -> None:
    """`anakot mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`anakot mcp picker` (and default `anakot mcp`) — interactive selector; re-renders after each
    action until ESC/q."""
    while True:
        rows = _build_rows()
...
def install_by_name(identifier: str) -> int:
    """`anakot mcp install <name>` — non-interactive entry-point."""
    from anakot_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        _say(
            f"  ✗ '{identifier}' is not in the catalog. Run `anakot mcp catalog` to see available entries.",
            Colors.RED,
        )
        return 1
