"""``anakot plugins validate`` — admission checks for a plugin directory.
...
scratch child process (with a throwaway ``ANAKOT_HOME``), so a crashing or
...
from anakot_cli.plugin_validate_desktop import check_desktop_surface
...
_PROBE_TIMEOUT = 30
_PROBE_SENTINEL = "ANAKOT_VALIDATE_JSON:"
...
def _requires_anakot_spec_valid(spec: str) -> bool:
    """Strictly validate a ``requires_anakot`` spec.

    Unlike :func:`anakot_cli.plugins_manifest.version_satisfies` (permissive at load
    ...
    """
    from anakot_cli.plugins_manifest import _VERSION_COMPARATOR_RE, _version_tuple
...
def _check_requires_anakot(report: ValidationReport, manifest: dict) -> None:
    spec = str(manifest.get("requires_anakot") or "").strip()
    if not spec:
        report.add("requires_anakot", True, "not declared")
        return
    if _requires_anakot_spec_valid(spec):
        report.add("requires_anakot", True, f"spec {spec!r} parses")
    else:
        report.add(
            "requires_anakot",
            False,
            f"requires_anakot spec {spec!r} does not parse "
...
# from anakot so a hostile plugin only sees a bare interpreter — the one
...
class RecordingContext:
    ...
    plugin_id = "anakot_validate_probe_plugin"
...
def _run_capability_probe(plugin_dir: Path, manifest: dict) -> Tuple[Optional[dict], str]:
    ...
    """
    with tempfile.TemporaryDirectory(prefix="anakot-validate-") as scratch:
        env = dict(os.environ)
        env["ANAKOT_HOME"] = scratch
        try:
...
def validate_plugin_dir(plugin_dir: Path) -> ValidationReport:
    ...
    _check_requires_anakot(report, manifest)
...
def _check_loadable(report: ValidationReport, plugin_dir: Path) -> None:
    """A plugin.yaml with nothing beside it that Anakot can load (no ``register()`` module, no
...
def _check_python_dependencies(report: ValidationReport, plugin_dir: Path) -> None:
    ...
    from anakot_cli.plugin_python_deps import read_declaration
    ...
    if decl.external:
        ...
        return
    from anakot_cli.plugin_python_deps import applicable_specs, unsupported_specs
