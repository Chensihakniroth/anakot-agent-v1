...
_WINDOWS_GATEWAY_BREAKAWAY_ENV = "_ANAKOT_GATEWAY_BREAKAWAY"
...
def noninteractive_git_env(base: "Mapping[str, str] | None" = None) -> dict[str, str]:
    ...
    Anakot's plumbing calls. ``core.sshCommand`` is pinned to ``ssh -o BatchMode=yes`` so the ssh
    ...
    Anakot shells out to git from many non-interactive contexts — MCP catalog installs, plugin
...
def _text_names_anakot(text: str) -> bool:
    r"""True when *text* names Anakot at a path-segment / token boundary.

    A bare ``"anakot" in text`` substring test would also match unrelated processes whose paths
    merely contain the letters (``...\sanakota\...``) — the false-positive class this prevents.
    """
    return any(token.startswith(("anakot", ".anakot"))
               for token in re.split(r"[\\/\s=,;\"']+", text.lower()))


def _process_command_is_anakot(pid: int) -> bool:
    """Best-effort check that *pid* currently runs Anakot code."""
    try:
        import psutil
        ...
        return _text_names_anakot(f"{command} {executable}")
    except Exception:
        return False


def pid_is_anakot(pid: int, *, expected_start_time: int | None = None) -> bool:
    """Whether it is safe to use ``taskkill`` for *pid*.

    The PID must be valid, currently exist, and identify a Anakot process. When the caller captured
    ...
    if expected_start_time is not None and current_start_time != expected_start_time:
        return False
    try:
        return _process_command_is_anakot(pid)
    except Exception:
        return False
...
def _legacy_kill_process_tree(proc: "subprocess.Popen") -> None:
    ...
    if IS_WINDOWS:
        ...
        # be recycled while we hold it. The fail-closed ``pid_is_anakot`` guard is for BARE pids.
        try:
...
def bounded_probe_run(
    ...
    ``Get-CimInstance Win32_Process`` gateway scan hit exactly this during ``anakot update`` on slow-WMI
    ...
    try:
        ...
        from anakot_cli.local_runtime.processes import spawn_server
