...
from anakot_cli._subprocess_compat import windows_detach_flags
...
def _probe_gateway_health() -> tuple[bool, dict | None]:
    ...
    """
    from anakot_cli.web_server import _GATEWAY_HEALTH_TIMEOUT, _GATEWAY_HEALTH_URL
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    """
    try:
        from anakot_cli.profiles import _check_gateway_running, profiles_to_serve
...
# ~/.anakot/logs/<action>.log which the dashboard tails.

_ACTION_LOG_DIR: Path = get_anakot_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "anakot-update": "anakot-update.log",
...
def _dashboard_spawn_executable() -> str:
    ...
    """
    from anakot_cli.web_server import PROJECT_ROOT
...
def _profile_action_environment(
    ...
    """Environment for a detached ``anakot <subcommand>`` action.

    The dashboard loads its own profile's ``.env`` into process-global ``os.environ``. Copying
    that mapping verbatim into ``anakot -p <other> ...`` lets the named child see the dashboard
    profile's platform credentials and ports *before* its own dotenv loads (``load_anakot_dotenv``
    ...
    Named-profile actions therefore start from Anakot' standard scrubbed subprocess env, then drop
    the profile-managed keys plus every key declared by the dashboard/default profile dotenv files
    and their hydrated secret sources, and pin ``ANAKOT_HOME`` to the target profile. The child's
    ...
    if profile is None:
        ...
        from anakot_cli.env_loader import (
            _PROFILE_MANAGED_ENV_KEYS, _env_keys_defined_in_dotenv, get_secret_source_values,
        )
        from anakot_cli.web_server_profiles import _resolve_profile_dir
        from anakot_constants import apply_subprocess_home_env, get_default_anakot_root
        ...
        try:
            source_homes = {str(get_default_anakot_root()), str(get_anakot_home())}
        except Exception:
            source_homes = set()
        ...
        action_env["ANAKOT_HOME"] = str(target_home)
        apply_subprocess_home_env(action_env)

    action_env["ANAKOT_NONINTERACTIVE"] = "1"
    ...
    # The dashboard runs inside the gateway process, so os.environ carries _ANAKOT_GATEWAY=1;
    ...
    action_env.pop("_ANAKOT_GATEWAY", None)
    if env_overrides:
...
def _action_targets_system_gateway(subcommand: List[str]) -> bool:
    ...
    ``anakot update`` fleet restart (``update_cmd_fleet._needs_sudo`` / ``_sudo_noninteractive_ok``).
    """
    from anakot_cli.update_cmd_fleet import _needs_sudo
    ...
    from anakot_cli.gateway import _select_systemd_scope
    from anakot_constants import reset_anakot_home_override, set_anakot_home_override

    profile = _named_profile_from_action(subcommand)
    if profile is None:
        return _select_systemd_scope(False)
    # Unit names are derived from ANAKOT_HOME, so a selector-bearing action must be resolved
    ...
    from anakot_cli.web_server_profiles import _resolve_profile_dir
    token = set_anakot_home_override(_resolve_profile_dir(profile))
    try:
        return _select_systemd_scope(False)
    finally:
...
def _spawn_anakot_action(
    subcommand: List[str], name: str, *, env_overrides: Optional[Dict[str, str]] = None
) -> subprocess.Popen:
    """Spawn ``anakot <subcommand>`` detached (via ``anakot_cli.main``) and record the handle."""
    from anakot_cli.web_server import PROJECT_ROOT
    _ACTION_LOG_DIR.mkdir(parents=True, exist_ok=True)
    log_file = open(_ACTION_LOG_DIR / _ACTION_LOG_FILES[name], "ab", buffering=0)
    log_file.write(f"\n=== {name} started {time.strftime('%Y-%m-%d %H:%M:%S')} ===\n".encode())

    cmd = [_dashboard_spawn_executable(), "-m", "anakot_cli.main", *subcommand]
    if _action_targets_system_gateway(subcommand):
        ...
        # ANAKOT_HOME past sudo's env_reset and reads SUDO_USER for the service identity.
        ...
        # two-step gate as the ``anakot update`` fleet restart: a refused blanket probe falls
        # back to ``sudo -l`` on the exact argv, so a command-scoped NOPASSWD entry qualifies.
        from anakot_cli.update_cmd_fleet import _sudo_noninteractive_ok
        ...
        if not _sudo_noninteractive_ok(["-l", "--", *cmd]):
            message = (
                ...
                f"'sudo anakot {' '.join(subcommand)}' on the host, or grant that user NOPASSWD sudo."
            )
    ...
    # dashboard profile's credentials; see _profile_action_environment (also drops _ANAKOT_GATEWAY).
    ...
    action_id = (env_overrides or {}).get("ANAKOT_ACTION_ID")
    if action_id:
...
def _own_profile_selector(profile: Optional[str]) -> Optional[str]:
    ...
    profile (a pooled Desktop ``anakot --profile X serve`` answers ``/api/gateway/*`` without
    ...
    own = profile_name_for_home(get_process_anakot_home())
    return own if own and own != "default" else None


def _gateway_subcommand(profile: Optional[str], verb: str) -> List[str]:
    """``anakot [-p X] gateway <verb>`` argv for a dashboard lifecycle action. A profile served by the
    ...
    pooled ``--profile X serve`` would inherit X's ``ANAKOT_HOME`` and hit the same exit-78 refusal."""
    from anakot_cli.web_server_profiles import _profile_cli_args
    ...
    if profile and verb == "restart" and multiplexed_profile_refusal(profile, verb) is not None:
        from anakot_constants import get_process_anakot_home, profile_name_for_home
        args = [] if profile_name_for_home(get_process_anakot_home()) == "default" else ["-p", "default"]
...
def multiplexed_profile_refusal(profile: Optional[str], verb: str) -> Optional[str]:
    ...
    The spawned ``anakot -p X gateway <verb>`` would only print exit-78 / "no gateway running for this
...
def _restart_gateway_after(profile: Optional[str], *, what: str, label: str) -> dict[str, Any]:
    ...
    from anakot_cli.web_server import _spawn_gateway_restart
    try:
