...
from anakot_cli.commands import (
    COMMAND_REGISTRY, _is_gateway_available, _iter_plugin_command_entries, _resolve_config_gates)

# Logger name parity with the origin module (tests capture "anakot_cli.commands").
logger = logging.getLogger("anakot_cli.commands")
...
def discord_skill_commands_by_category(
    ...
    parity, external-dir skills are visible via ``anakot skills list`` and the agent's ``/skill-name``
...
# Canonical commands deliberately routed through ``/anakot <command>`` on Slack only: the registry
...
_SLACK_VIA_ANAKOT_ONLY = frozenset({
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    _SLACK_VIA_ANAKOT_ONLY are skipped. ``/anakot`` is always first for anything dropped."""
...
def slack_app_manifest(
    request_url: str = "https://anakot-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """name/alias -> "/command" for the Slack ``/anakot`` handler, plugin commands included."""
