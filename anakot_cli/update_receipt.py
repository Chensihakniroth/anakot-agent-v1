...
# ``anakot update`` is a single-threaded CLI command; a module singleton lets the 7k-line updater
...
class UpdateReceipt:
    """Collects the observable facts of one ``anakot update`` run."""
    ...
    def gateway_restart_result(
        ...
        if fresh_recovery is not None:
            ...
            # ``anakot serve`` hosts tui_gateway and is not a gateway profile, so neither the
...
def _receipt_dir() -> Path:
    # ``anakot_constants`` (stdlib-only), never ``anakot_cli.config``: the receipt must be
    ...
    return get_anakot_home() / "logs" / "update_receipts"
...
def finalize_pending_update_receipt(exit_code: Optional[int] = None, stop_reason: str = "") -> Optional[Path]:
    ...
    ``anakot update`` has many early ``sys.exit`` paths (preflight refusals, venv-holder refusal,
...
def _profile_homes() -> list[tuple[str, Path]]:
    ...
    from anakot_cli.profiles import _get_default_anakot_home, _get_profiles_root, _PROFILE_ID_RE
...
_FLEET_ROW_IDENTITY_PENDING = (
    ...
    " — re-check with `anakot gateway status`"
)
...
def print_fleet_version_matrix(fleet: list[dict[str, Any]]) -> bool:
    ...
    if stale_or_down:
        ...
        print("  Run `anakot gateway restart` (or `anakot -p <profile> gateway restart` for a named")
        print("  profile), then `anakot gateway status` to confirm.")
