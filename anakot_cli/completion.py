"""Shell completion script generation for anakot CLI. Walks the live argparse parser tree, so
    ...
    for cmd, info in subcommands:
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_anakot_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;")
            ...
            cases.append(
                ...
                f"            ;;")
    cases_str = "\n".join(cases)
    return f"""# Anakot Agent bash completion
# Add to ~/.bashrc:
#   eval "$(anakot completion bash)"

_anakot_profiles() {{
    local profiles_dir="$HOME/.anakot/profiles"
...
_anakot_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_anakot_profiles)" -- "$cur"))
        return
    fi
...
complete -F _anakot_completion anakot
"""
    ...
    for cmd, info in subcommands:
        ...
        if cmd == "profile":
            ...
            sub_cases.append(
                ...
                f"                    ;;")
    sub_cases_str = "\n".join(sub_cases)
    return f"""#compdef anakot
# Anakot Agent zsh completion
# Add to ~/.zshrc:
...
_anakot_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.anakot/profiles" ]]; then
        profiles+=($HOME/.anakot/profiles/*(N/:t))
    fi
...
_anakot() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_anakot_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'anakot command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Anakot Agent fish completion",
        "# Add to your config:",
        "#   anakot completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __anakot_profiles",
        "    echo default",
        "    if test -d $HOME/.anakot/profiles",
        "        for d in $HOME/.anakot/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c anakot -f -s p -l profile"
        " -d 'Profile name' -xa '(__anakot_profiles)'",
        "",
        "# Top-level subcommands"]
    for cmd, info in subcommands:
        lines.append(
            f"complete -c anakot -f "
        ...
        if cmd == "profile":  # profile names for the actions that take one
            for action in sorted(_PROFILE_NAME_ACTIONS):
                lines.append(
                    ...
                    f"-a '(__anakot_profiles)' -d 'Profile name'")
    lines.append("")
    return "\n".join(lines)
