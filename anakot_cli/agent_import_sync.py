"""``anakot import-agent --sync`` — keep previously imported Claude Code / Codex setups current.

Every successful ``anakot import-agent`` run records its source in ``ANAKOT_HOME/import-sync.json``
...
def _iter_sync_files(agent: str, source_root: Path) -> Iterator[Path]:
    ...
    :class:`~anakot_cli.agent_import.AgentImporter` reads for ``agent``."""
...
def sync_imported_agents(args) -> None:
    """Handle ``anakot import-agent --sync``: re-import every registered source whose digest
    changed. Prompt-free (cron-friendly); ``--dry-run`` previews without touching the manifest."""
    from anakot_cli.agent_import import AgentImporter, print_import_report
    from anakot_cli.setup import print_error, print_header, print_info, print_success
    from anakot_constants import get_anakot_home

    dry_run = bool(getattr(args, "dry_run", False))
    anakot_home = get_anakot_home().resolve()
    agents: Dict[str, Any] = load_sync_manifest(anakot_home).get("agents", {})
    if not agents:
        print()
        print_info("No import sources registered yet.")
        print_info("Run 'anakot import-agent' first — successful imports are "
    ...
    for agent_name in sorted(agents):
        ...
        try:
            report = AgentImporter(agent_name, source_dir, anakot_home, execute=not dry_run,
        ...
        if not dry_run:
            try:
                ...
                update_sync_manifest(agent_name, source_dir, anakot_home, overwrite, report,
