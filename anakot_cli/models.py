...
``anakot_cli.models.<name>`` stays the stable import/monkeypatch surface:
...
from anakot_cli.urllib_security import open_credentialed_url
from anakot_cli.models_catalog_static import (
    CANONICAL_PROVIDERS,
    OPENROUTER_MODELS,
...
from anakot_cli.models_reasoning_caps import (
...
def _custom_provider_ssl_context(base_url: str):
    ...
    if not base_url:
        return None
    try:
        from anakot_cli.config import get_custom_provider_tls_settings
...
def _openrouter_catalog_disk_ttl() -> float:
    ...
    from anakot_cli.model_catalog import refresh_interval_seconds
...
def _openrouter_catalog_disk_path() -> Path:
    ...
    return get_anakot_home() / "cache" / "openrouter_curated_catalog.json"
...
def _nous_recommended_disk_path() -> "Path":
    from anakot_constants import get_anakot_home
    return get_anakot_home() / "cache" / "nous_recommended_cache.json"
...
def _resolve_nous_portal_url() -> str:
    ...
    try:
        from anakot_cli.auth import DEFAULT_NOUS_PORTAL_URL, get_provider_auth_state
...
def get_preferred_silent_default_model(provider: str = "openrouter") -> str:
    ...
    try:
        from anakot_cli.model_catalog import get_default_model_from_cache
...
def recommended_nous_default_model() -> dict[str, Any]:
    ...
    sign-in completion in ``anakot_cli.anon_auth`` so both land on the same model.
    """
    from anakot_cli import models_pricing as mp
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    """True when ``supported_parameters`` advertises ``tools`` (anakot-agent is tool-calling-first).
...
def fetch_openrouter_models(
    ...
    from anakot_cli.models_profile_cache import profile_slot_get, profile_slot_set
    ...
    try:
        from anakot_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id, _ in fallback:
        ...
        if live_item is None or not _openrouter_model_supports_tools(live_item):
            continue
        # Hide models that don't advertise tool-calling support — anakot-agent requires it and surfacing
...
def fetch_ai_gateway_models(
    ...
    from anakot_constants import AI_GATEWAY_BASE_URL
...
def _configured_custom_provider_ids() -> set[str]:
    ...
    try:
        ...
        from anakot_cli.providers import custom_provider_slug
...
def _provider_has_credentials(pid: str) -> bool:
    try:
        from anakot_cli.auth import get_auth_status, has_usable_secret
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    derived from :data:`CANONICAL_PROVIDERS` (shared with ``anakot model`` and ``/model``)."""
...
def _resolve_static_model_alias(
    ...
    try:
        from anakot_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def detect_provider_for_model(
    ...
    from anakot_cli.models_detect import (
...
def normalize_provider(provider: Optional[str]) -> str:
    ...
    ``anakot_cli.auth.resolve_provider()`` to resolve it from credentials."""
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Anakot should expose the /fast toggle for this model."""
...
def _first_exchangeable_copilot_token(raw_tokens) -> str:
    ...
    from anakot_cli.copilot_auth import exchange_copilot_token, validate_copilot_token
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    def _pool_token() -> str:
        from anakot_cli.auth import read_credential_pool
...
def _model_dedup_key(model_id: str) -> str:
    ...
    try:
        from anakot_cli.model_search import model_alias_canonical
...
def _codex_catalog(normalized: str, force_refresh: bool) -> list[str]:
    from anakot_cli.codex_models import get_codex_model_ids
    ...
    try:
        from anakot_cli.auth import _codex_access_token_is_expiring, resolve_codex_runtime_credentials
...
def _nous_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    try:
        from anakot_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
    ...
    # Portal models still surface without a Anakot release.
...
def _openai_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    # entries, so intersect with the curated agentic catalog so ``/model`` matches ``anakot model``.
    ...
    # pure-catalog helper (no extra live fetch) so we only accept models Anakot actually ships. (#46850)
    ...
    from anakot_cli.providers import is_official_openai_host
...
def _azure_foundry_catalog(normalized: str, force_refresh: bool) -> Optional[list[str]]:
    ...
    """
    try:
        from anakot_cli.azure_detect import _probe_openai_models
...
# every authed provider's /v1/models). One JSON file at $ANAKOT_HOME/provider_models_cache.json;
...
def _spawn_swr_refresh(cache_key: str, refresh_fn=None) -> None:
    ...
    from anakot_constants import get_anakot_home_override, anakot_home_key
    inflight_key = cache_key if get_anakot_home_override() is None else (anakot_home_key(), cache_key)
...
def _provider_models_cache_path() -> Path:
    from anakot_constants import get_anakot_home
    return get_anakot_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    if provider == "azure-foundry":
        try:
            from anakot_cli.runtime_provider import _config_base_url_for_provider
    ...
    try:
        ...
        for rel in ("auth.json", "credentials.json"):
            _mtime_part(rel, get_anakot_home() / rel)
    except Exception:
        pass
...
def clear_provider_models_cache(provider: Optional[str] = None) -> None:
    ...
    ``/model --refresh`` and ``anakot model --refresh``."""
    try:
...
def probe_api_models(
    ...
    headers: dict[str, str] = {"User-Agent": _ANAKOT_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"anakot-agent/{_ANAKOT_VERSION}"
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=anakot"
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    headers = {"Authorization": f"Bearer {api_key}", "User-Agent": _ANAKOT_USER_AGENT}
    try:
...
def cached_fetch_api_models(
    ...
    from anakot_cli.model_switch_providers import _NativePickerModelList
...
_PLUGIN_COMPAT_LAZY = {
    'LMStudioLoadResult': ('anakot_cli.models_local', 'LMStudioLoadResult'),
    'PROVIDER_GROUPS': ('anakot_cli.models_catalog_static', 'PROVIDER_GROUPS'),
    'ProviderEntry': ('anakot_cli.models_catalog_static', 'ProviderEntry'),
    'atomic_json_write': ('utils', 'atomic_json_write'),
    'base_url_host_matches': ('utils', 'base_url_host_matches'),
    'compute_sale_discount': ('anakot_cli.models_pricing', 'compute_sale_discount'),
    'ensure_lmstudio_model_loaded': ('anakot_cli.models_local', 'ensure_lmstudio_model_loaded'),
    'fetch_ai_gateway_pricing': ('anakot_cli.models_pricing', 'fetch_ai_gateway_pricing'),
    'fetch_lmstudio_models': ('anakot_cli.models_local', 'fetch_lmstudio_models'),
    'fetch_models_with_pricing': ('anakot_cli.models_pricing', 'fetch_models_with_pricing'),
    'fetch_ollama_local_models': ('anakot_cli.models_local', 'fetch_ollama_local_models'),
    'get_cached_nous_inference_base_url': ('anakot_cli.models_pricing', 'get_cached_nous_inference_base_url'),
    'get_pricing_for_provider': ('anakot_cli.models_pricing', 'get_pricing_for_provider'),
    'group_providers': ('anakot_cli.models_catalog_static', 'group_providers'),
    'lmstudio_model_reasoning_options': ('anakot_cli.models_local', 'lmstudio_model_reasoning_options'),
    'nous_catalog_url': ('anakot_cli.models_reasoning_caps', 'nous_catalog_url'),
    'nous_model_reasoning_capabilities': ('anakot_cli.models_reasoning_caps', 'nous_model_reasoning_capabilities'),
    'nous_policy_allowed_ids': ('anakot_cli.models_pricing', 'nous_policy_allowed_ids'),
    'ollama_model_supports_thinking': ('anakot_cli.models_local', 'ollama_model_supports_thinking'),
    'openrouter_model_reasoning_capabilities': ('anakot_cli.models_reasoning_caps', 'openrouter_model_reasoning_capabilities'),
    'parse_openrouter_reasoning_capabilities': ('anakot_cli.models_reasoning_caps', 'parse_openrouter_reasoning_capabilities'),
    'peek_cached_pricing': ('anakot_cli.models_pricing', 'peek_cached_pricing'),
    'pricing_cache_scope': ('anakot_cli.models_pricing', 'pricing_cache_scope'),
    'probe_lmstudio_models': ('anakot_cli.models_local', 'probe_lmstudio_models'),
    'probe_ollama_local_models': ('anakot_cli.models_local', 'probe_ollama_local_models'),
    'provider_group_for_slug': ('anakot_cli.models_catalog_static', 'provider_group_for_slug'),
    'refresh_reasoning_caps_async': ('anakot_cli.models_reasoning_caps', 'refresh_reasoning_caps_async'),
    'restrict_to_nous_policy': ('anakot_cli.models_pricing', 'restrict_to_nous_policy'),
    'should_use_ollama_native_catalog': ('anakot_cli.models_local', 'should_use_ollama_native_catalog'),
    'url_origin': ('anakot_cli.urllib_security', 'url_origin'),
    'validate_requested_model': ('anakot_cli.models_validate', 'validate_requested_model'),
    'warm_nous_reasoning_caps_async': ('anakot_cli.models_reasoning_caps', 'warm_nous_reasoning_caps_async'),
    'warm_openrouter_reasoning_caps_async': ('anakot_cli.models_reasoning_caps', 'warm_openrouter_reasoning_caps_async'),
}
