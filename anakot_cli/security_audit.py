"""On-demand supply-chain audit for Anakot Agent installs.
...
def _discover_plugins(anakot_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.anakot/plugins``. Plugins typically don't install
...
def _discover_components(
    *, skip_venv: bool = False, skip_plugins: bool = False, skip_mcp: bool = False, anakot_home: Optional[Path] = None
) -> list[Component]:
    """Discover all scannable components across the enabled sources."""
    home = anakot_home or Path(get_anakot_home())
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `anakot security audit`."""
    ...
    components = _discover_components(anakot_home=home, **skips)
    ...
    try:
        findings = run_audit(anakot_home=home, components=components)
