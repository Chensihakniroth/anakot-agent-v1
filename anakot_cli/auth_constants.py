...
Pure leaf: imports nothing from ``anakot_cli.auth`` so the per-provider modules
...
# httpx is imported lazily (~30ms) because anakot_cli.auth is on the interactive-CLI startup path
...
if TYPE_CHECKING:
    ...
    class _LazyHttpx:
        ...
        # set/del forward to the real module so monkeypatch.setattr("anakot_cli.auth.httpx.Client")
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from anakot_cli import __version__ as _ANAKOT_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _ANAKOT_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"anakot-cli/{_ANAKOT_CLI_VERSION}"
...
SPOTIFY_DOCS_URL = "https://anakot-agent.nousresearch.com/docs/user-guide/features/spotify"
...
OAUTH_OVER_SSH_DOCS_URL = "https://anakot-agent.nousresearch.com/docs/guides/oauth-over-ssh"
