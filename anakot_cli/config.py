"""Configuration management for Anakot Agent: config.yaml / .env loading, saving,
validation, migration, and the ``anakot config`` command."""
...
# dies with ImportError before any later heal point is reached. See anakot_cli.stale_modules.
...
from anakot_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
from anakot_cli.secret_prompt import masked_secret_prompt
# Re-export from anakot_constants — canonical definition lives there.
from anakot_constants import get_anakot_home, get_process_anakot_home  # noqa: F401
...
_PARSE_FAILURE_FALLBACK_MSG = {
    "last-known-good": "Anakot is running on the settings it loaded before the edit until it is fixed, so recent changes are not applied.",
    "last-known-good-backup": "Anakot is running on your last good settings until it is fixed, so recent changes are not applied.",
    "refuse-write": "Nothing was written, so the existing file is preserved."}
_PARSE_FAILURE_DEFAULTS_MSG = (
    "Anakot is running on default settings until it is fixed, so none of your saved settings are applied.")
_PARSE_FAILURE_REPAIR_MSG = "Open it with `anakot config edit`, fix {where}, then run `anakot config check`."
...
def format_config_parse_failure(config_path: Path, exc: Exception, *, fallback: str = "defaults") -> str:
    """User copy for an unparseable config.yaml: what happened, what Anakot is doing, how to fix.
...
def _warn_config_parse_failure(
    ...
    from anakot_cli.config_backups import backup_config
    ...
    try:
        sys.stderr.write(f"⚠️  anakot config: {msg}\n    Details: {_yaml_error_details(exc)}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# interpreter init (PYTHON*, NODE_*: Anakot restarts through them), PATH (fix tool lookup
...
# and Anakot runtime-location / security-policy flags (config.yaml is the supported surface).
#
# ``ANAKOT_*`` overall is NOT blocked — many integration credentials use that prefix
# (ANAKOT_LANGFUSE_PUBLIC_KEY, ANAKOT_SPOTIFY_CLIENT_ID, ...). The denylist is name-by-name so
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Anakot runtime location
    "ANAKOT_HOME", "ANAKOT_PROFILE", "ANAKOT_CONFIG", "ANAKOT_ENV",
    "ANAKOT_CONFIG_PATH", "ANAKOT_ENV_PATH",
    # MCP catalog trust root; package-manager wrappers may still set it in the process env.
    "ANAKOT_OPTIONAL_MCPS",
    # Local ACP subprocess selection (executable/argv authority).
    "ANAKOT_COPILOT_ACP_COMMAND", "ANAKOT_COPILOT_ACP_ARGS",
    # Security policy / approval-routing context — set via their dedicated controls only.
    "ANAKOT_YOLO_MODE", "ANAKOT_ACCEPT_HOOKS", "ANAKOT_REDACT_SECRETS",
    "ANAKOT_INTERACTIVE", "ANAKOT_EXEC_ASK", "ANAKOT_GATEWAY_SESSION",
    "ANAKOT_CRON_SESSION", "ANAKOT_SINGLE_QUERY_SESSION",
    "ANAKOT_SESSION_KEY", "ANAKOT_SESSION_PLATFORM"})
...
def validate_env_var_name_for_write(key: str) -> None:
    ...
    if _env_var_policy_name(key) in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "or Anakot runtime location and security policy (ANAKOT_HOME, ANAKOT_YOLO_MODE, ...) "
            "cannot be persisted via the env writer. If you really need this, edit ~/.anakot/.env "
            "directly.")
...
# acquires it again (anakot_cli/plugins.py: `with ..., config_mod._CONFIG_LOCK:` then
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # back-compat fallback. The boolean ANAKOT_TOOL_PROGRESS variant is unsupported (its only
    # consumer, the v3->4 migration, is below the v12 support floor); doctor flags it as ignored.
    "ANAKOT_TOOL_PROGRESS_MODE",
    ...
    "ANAKOT_LANGFUSE_ENV", "ANAKOT_LANGFUSE_RELEASE", "ANAKOT_LANGFUSE_SAMPLE_RATE",
    "ANAKOT_LANGFUSE_MAX_CHARS", "ANAKOT_LANGFUSE_CAPTURE", "ANAKOT_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY", "LANGFUSE_SECRET_KEY", "LANGFUSE_BASE_URL",
    # ACP (Agent Client Protocol) keys — profile-isolable so profiles can use different backends.
    "ANAKOT_ACP_AUTH_METHOD", "ANAKOT_ACP_AUTO_APPROVE", "ANAKOT_COPILOT_ACP_COMMAND",
    "ANAKOT_COPILOT_ACP_ARGS", "COPILOT_CLI_PATH", "COPILOT_ACP_BASE_URL"})
...
# ANAKOT_MANAGED). Module-level so tests can patch it without touching /nix/store.
...
# Explicit opt-out (``ANAKOT_MANAGED=false``): without this a bool-shaped value became a package
# manager literally named "false" and is_managed() blocked `anakot update` (#12864).
_MANAGED_FALSE_VALUES = frozenset({"false", "0", "no", "off"})


def get_managed_system() -> Optional[str]:
    """Return the package manager owning this install, if any.
    Signals: ANAKOT_MANAGED env var (systemd service) or a ``.managed`` marker file in
    ANAKOT_HOME (NixOS activation script — interactive shells don't see the service env)."""
    marker = os.getenv("ANAKOT_MANAGED", "").strip().lower() or None
    managed_marker = get_anakot_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Anakot is running in package-manager-managed mode."""
...
_NIX_UPDATE_MSG = (
    "Update Anakot through the Nix source that installed it "
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Anakot was installed: apt/docker/nix/nixos/home-manager/git/unknown.
    Order: code-scoped ``<install tree>/.install_method`` stamp (authoritative) -> legacy
    ``$ANAKOT_HOME/.install_method`` -> managed marker -> /nix/store path -> .git dir -> unknown.
    The stamp lives next to the code because ANAKOT_HOME is shared data: a container and a host
    install can bind-mount the same home, so a home-scoped ``docker`` stamp would make the host
    ``anakot update`` refuse to run. A legacy ``docker`` value is therefore ignored unless we are
    ...
    the code; - the published ``nousresearch/anakot-agent`` image bakes a ``docker`` stamp into
    ``/opt/anakot`` at build time. An unsupported manual install dropped into a container (no stamp) falls
    through to the ``.git`` checks and behaves like any off-path install. See issue #34397.
    """
    # The stamp is a property of the running code tree (parent of anakot_cli/), NOT of $ANAKOT_HOME,
    ...
    method = _install_method_stamp(get_anakot_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Import-safe wrapper around ``anakot_constants.is_container``."""
    try:
...
_UPDATE_COMMAND_BY_METHOD = {
    "docker": "docker pull nousresearch/anakot-agent:latest",
    "apt": "pkg upgrade anakot-agent",  # "apt" == Termux APT by contract; uses Termux's `pkg`.
}
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    return _UPDATE_COMMAND_BY_METHOD.get(method, "anakot update")
...
# Shared by ``cmd_update`` and ``_cmd_update_check`` (anakot_cli/main.py) so the wording never
# forks. The published image excludes ``.git``, so the git update path can never succeed there
# and the generic "reinstall via install.sh" fallback would install a NEW host-side Anakot.
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``anakot update`` inside Docker."""
    return _DOCKER_UPDATE_MESSAGE


def format_managed_message(action: str = "modify this Anakot installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    return (
        f"Cannot {action}: this Anakot installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Anakot.")
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from ANAKOT_HOME/.container-mode.
    ...
    already inside the container, or when ANAKOT_DEV=1 is set. Only FileNotFoundError is
    swallowed; other errors (permissions, malformed data) propagate."""
    if os.environ.get("ANAKOT_DEV") == "1":
        return None
    ...
    try:
        info = {}
        with open(get_anakot_home() / ".container-mode", "r", encoding="utf-8") as f:
            for line in f:
                line = line.strip()
    ...
    return {
        ...
        "container_name": info.get("container_name", "anakot-agent"),
        "exec_user": info.get("exec_user", "anakot"),
        "anakot_bin": info.get("anakot_bin", "/data/current-package/bin/anakot")}


# ---- Config paths / ANAKOT_HOME skeleton ----
...
def require_parseable_user_config(*, ignore_user_config: bool = False) -> None:
    ...
    states; ``--ignore-user-config`` / ANAKOT_IGNORE_USER_CONFIG=1 remain authoritative."""
    if ignore_user_config or os.environ.get("ANAKOT_IGNORE_USER_CONFIG") == "1":
        return
    ...
    message = (
        f"Anakot stopped because your settings file ({config_path}) has a formatting error"
        f"{f' at {where}' if where else ''}. Fix it with `anakot config edit` and check with "
        "`anakot config check`, or add --ignore-user-config to run once with default settings.")
    if backup_path is not None:
...
def _resolve_anakot_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read ANAKOT_UID / ANAKOT_GID (set by Docker deployments); (None, None) if unset/invalid/Windows.
    The entrypoint chowns ANAKOT_HOME once, but subdirs created at runtime (``profiles/<name>/``)
    need the same chown or they land root:root and block later uid-mapped workers.

    Docker containers running Anakot commonly set these to map the in-container user to a host user so
    ...
    return _env_int("ANAKOT_UID"), _env_int("ANAKOT_GID")


def _chown_to_anakot_uid(path) -> None:
    """Chown ``path`` to ``ANAKOT_UID:ANAKOT_GID`` when set; EPERM/ENOENT are non-fatal (the
    ...
    :func:`ensure_anakot_home` on Docker deployments. See #34107.
    """
    uid, gid = _resolve_anakot_uid_gid()
...
def _secure_dir(path):
    """chmod a directory owner-only (0700) and apply ANAKOT_UID/GID ownership. No-op when managed;
    in a container only an explicit ANAKOT_HOME_MODE is applied. ANAKOT_HOME_MODE (e.g. 0701)
    overrides the mode so a web server can traverse ANAKOT_HOME to a served subdirectory without
    directory listings.

    Also applies ``ANAKOT_UID``/``ANAKOT_GID``-based ownership when those env vars are set (#34107 — Docker
    ...
    if is_managed():
        return
    explicit_mode = os.environ.get("ANAKOT_HOME_MODE", "").strip()
    ...
    # start (#10757). An explicit ANAKOT_HOME_MODE is the operator's choice and is still applied.
    if _is_container() and not explicit_mode:
        _chown_to_anakot_uid(path)
        return
    try:
...
def _is_container() -> bool:
    """Detect Docker/Podman/LXC (or ANAKOT_CONTAINER / ANAKOT_SKIP_CHMOD opt-out).
    ...
    if (os.environ.get("ANAKOT_CONTAINER") or os.environ.get("ANAKOT_SKIP_CHMOD")
...
_ANAKOT_HOME_ENSURED: set = set()
_ANAKOT_HOME_SUBDIRS = (
    "cron", "sessions", "logs", "logs/curator", "memories",
    "pairing", "hooks", "image_cache", "audio_cache", "skills")


def ensure_anakot_home():
    """Ensure the ~/.anakot directory skeleton exists with secure permissions.
    ...
    from anakot_constants import assert_named_profile_home_live
    assert_named_profile_home_live(home)
    if key in _ANAKOT_HOME_ENSURED and home.is_dir():
        return
    from anakot_cli.config_home import initialize_home
    initialize_home(home, _ANAKOT_HOME_SUBDIRS, _ANAKOT_HOME_ENSURED)


# ---- Config loading/saving ----

from anakot_cli.config_defaults import DEFAULT_CONFIG, OPTIONAL_ENV_VARS  # noqa: E402,F401
from anakot_cli.config_providers import (  # noqa: E402,F401  (re-exported; callers/tests use anakot_cli.config.<name>)
...
# Back-compat re-exports — :mod:`anakot_cli.personality` owns personality/overlay semantics.
from anakot_cli.personality import (  # noqa: E402,F401
...
def _split_key_path(key: str) -> list[str]:
    ...
    ``anakot config set`` uses ``.`` as the nesting separator, so a key that itself contains a literal dot
...
def _is_env_config_key(key: str) -> bool:
    """Return whether `anakot config set` routes this credential-shaped key to .env through the
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # A malformed SKILL.md must never break `anakot update`; this prompting is a nicety.
...
_EXTRA_KNOWN_ROOT_KEYS = {
    ...
    "plugins",           # plugin enable/disable lists (anakot_cli/plugins_cmd.py)
    "smart_model_routing",   # written by the setup wizard
    "platform_toolsets",     # written by the setup wizard
    "known_plugin_toolsets", # anakot_cli/tools_config.py toolset-save flow
...
def _validate_timezone(config: Dict[str, Any], issues: List[ConfigIssue]) -> None:
    ...
    ``anakot_time._get_zoneinfo()`` swallows an invalid name behind a single WARNING in the
    ...
    hint = ("Use an IANA zone name such as America/New_York or Asia/Tokyo (see "
            ...
            "schedules silently fall back to server-local time. ANAKOT_TIMEZONE overrides "
            "this key when set.")
    ...
    for _key in ("backend", "search_backend", "extract_backend"):
        ...
        if note:
            _issue(issues, "warning",
                   ...
                   "Run 'anakot tools' and pick a different Web Search & Extract provider")
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            from anakot_cli.config_home import config_load_issue
    ...
    if cp and not config.get("model"):
        _issue(issues, "warning",
               "custom_providers defined but no 'model' section — Anakot won't know which provider to use",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'anakot doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    from anakot_cli.config_migrations import (
    ...
    if floor_refused:
        ...
        sys.stderr.write(f"⚠ anakot config: {msg}\n")
        if not quiet:
            print(f"  ⚠ {msg}")
    else:
...
def _disable_suspicious_mcp_servers(results: Dict[str, Any], quiet: bool) -> None:
    ...
    if not isinstance(raw_mcp_servers, dict):
        return
    try:
        from anakot_cli.mcp_security import validate_mcp_server_entry
    except Exception:
        return
    mcp_touched = False
...
def _warn_invalid_platform_toolsets(results: Dict[str, Any], quiet: bool) -> None:
    ...
    try:
        ...
        from anakot_cli.toolset_validation import validate_platform_toolsets
        from anakot_cli.toolset_scope import toolset_allowed_for_platform
...
def _offer_list(heading: str, items: List[str], question: str) -> bool:
    ...
    if not _ask_yes_no(question):
        print("  Set later with: anakot config set <key> <value>")
        return False
    print()
    return True
...
def _env_expand_match(m: re.Match) -> str:
    ...
    if inner.startswith("env:"):
        logger.warning(
            "Config ref %r: %s is not set (check ~/.anakot/.env); "
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    made against a different environment (load before ``load_anakot_dotenv()``, in-process
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    LiteLLM users reach for, and ``anakot config set`` blindly accepts any dotted key — so
    ...
    from OpenAI-compatible backends) — while display paths (``anakot status``/``dump``) read ``name`` and
...
# the ANAKOT_MAX_ITERATIONS env bridge, works in every ``<``/``>=``/``max - used`` comparison in
...
def _backups_dir_display() -> str:
    from anakot_constants import display_anakot_home
    return f"{display_anakot_home()}/backups/config/"


_FIX_PERMS = "Fix the file permissions or move it aside first."
_FIX_YAML = (
    "Fix it with `anakot config edit` and check with `anakot config check`, or copy the newest good "
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    """Whether the remote SSH shell must expand *cwd* itself: ``~`` expanded on the Anakot host
...
def _last_known_good_fallback(config_path: Path, path_key: str, cache_sig, exc: Exception) -> Optional[Dict[str, Any]]:
    ...
    if lkg is None:
        # Fresh process (CLI restart, `anakot config get`): nothing loaded yet in this process, so
        ...
        from anakot_cli.config_backups import load_newest_good_backup
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cached is not None and cache_sig is not None and cached[:8] == cache_sig:
            ...
            # load_anakot_dotenv() pins unexpanded literals for the process lifetime.
            # Without this, a load_config() that ran before load_anakot_dotenv() pins unexpanded literals
...
#   openai-codex (OAuth — anakot auth) — OpenAI Codex
#   nous         (OAuth — anakot auth) — Nous Portal
...
def save_config(
    ...
    """Save configuration to ~/.anakot/config.yaml.
...
def load_env() -> Dict[str, str]:
    """Load ~/.anakot/.env as a dict. Memoised inside ``load_env_file`` (``get_env_value()`` runs
...
def sanitize_env_file() -> int:
    """Rewrite ~/.anakot/.env with normalized line formatting; returns the number of changed lines."""
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.anakot/.env (also matching ``export KEY=`` lines, so a save
...
def custom_endpoint_key_env(identity: str) -> str:
    ...
    so two endpoints on one host get separate slots. The fixed ``ANAKOT_CUSTOM_`` prefix keeps the
    ...
    return f"ANAKOT_CUSTOM_{slug}_API_KEY" if slug else "ANAKOT_CUSTOM_API_KEY"


def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.anakot/.env and os.environ; True if it was found and removed."""
...
def save_env_value_secure(key: str, value: str) -> Dict[str, Any]:
    ...
    from anakot_cli.credential_lifecycle import save_provider_env_credential
...
def reload_env() -> int:
    """Re-read ~/.anakot/.env into os.environ; returns count of vars changed.
    Removes deleted vars only when known to Anakot (OPTIONAL_ENV_VARS and _EXTRA_ENV_KEYS) so
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ``os.environ`` (scope-aware) or ``~/.anakot/.env``.
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a Anakot-managed credential preferring ``~/.anakot/.env`` over ``os.environ``, so a
...
def _show_model_section(config: Dict[str, Any]) -> None:
    ...
    # Read the .env FILE directly so a stale ANAKOT_MAX_ITERATIONS ghost is caught even when the
    # gateway bridge already overrode os.environ.
    try:
        env_ghost = load_env().get("ANAKOT_MAX_ITERATIONS")
    except Exception:
        env_ghost = None
    if env_ghost is not None and str(env_ghost).strip() != str(cfg_max_turns).strip():
        print(color(f"                ⚠ .env has stale ANAKOT_MAX_ITERATIONS={env_ghost} "
                    f"(run 'anakot doctor --fix' to remove)", Colors.YELLOW))
...
def _show_display_section(config: Dict[str, Any]) -> None:
    ...
    try:
        from anakot_cli.personality import active_personality_name
...
def show_config():
    ...
    from anakot_cli.auth import get_anthropic_key
    ...
    print(color("  anakot config edit     # Edit config file", Colors.DIM))
    print(color("  anakot config set <key> <value>", Colors.DIM))
    print(color("  anakot setup           # Run setup wizard", Colors.DIM))
    print()
...
def resolve_cron_model_drift_defaults(
    ...
    Mirrors the scheduler's precedence: a truthy configured model wins over ``ANAKOT_MODEL``; the
    ...
    return provider, configured_model or _model_assignment_text(env.get("ANAKOT_MODEL", ""))
...
def warn_unpinned_cron_jobs_after_model_config_change(
    ...
    print(
        ...
        "`anakot cron edit <job_id> --provider <provider> --model <model>` or set a fleet default "
        "with `anakot config set cron.model <model>`.")
...
def _coerce_config_set_value(key: str, value: str) -> Any:
    """Auto-coerce a ``anakot config set`` string to bool/None/int/float/list/dict.
...
def _redirect_platform_display_key(key: str) -> tuple[str, Optional[str]]:
    ...
    Before #71047 a write such as ``anakot config set platforms.telegram.streaming false`` landed on a key
...
def _guard_section_overwrite(key: str, value: Any, user_config: Dict[str, Any], force: bool) -> str:
    ...
    err += [
        ...
        f"    anakot config set {key}.<sub-key> <value>",
        "  Or use --force to replace the entire section:",
        f"    anakot config set --force {key} {value!r}"]
...
def _touch_skin_file(key: str, value: Any) -> None:
    ...
    if key == "display.skin" and isinstance(value, str) and value:
        try:
            skin_file = get_anakot_home() / "skins" / f"{value}.yaml"
            if skin_file.exists():
                skin_file.touch()
        except Exception:
            pass
...
def _print_unknown_key_notice(key: str, suggestion: Optional[str]) -> None:
    print(color(
        ...
        "but Anakot may not read it.", Colors.YELLOW))
    if suggestion:
...
def set_config_value(key: str, value: str, force: bool = False):
    ...
    if _is_env_config_key(key):
        ...
        return
    from anakot_cli.config_env_routing import is_env_setting_key, save_env_setting
    ...
    if is_env_setting_key(key):
        ...
        # here, --force included (#111848). The env writer's denylist (ANAKOT_YOLO_MODE, PATH, ...)
    ...
    if key == "model.provider" and _old_provider.lower() != str(value).strip().lower():
        from anakot_cli.route_identity import drop_stale_model_route
        ...
        if _popped:
            ...
            _route_notice = color(
                ...
                "`anakot config unset model.base_url` (and model.api_mode).", Colors.YELLOW)
    # api_base -> base_url alias at set-time too (mirrors _normalize_root_model_keys).
    if key.strip().lower() in ("model.api_base", "api_base"):
        # Normalize the api_base → base_url alias at set-time too (issue #8919), so a fresh `anakot config
...
def get_config_value(key: str, *, as_json: bool = False, raw: bool = False):
    ...
    from anakot_cli.config_env_routing import is_env_setting_key, read_env_setting
    ...
    if _split_key_path(key)[0] in _known_top_level_keys():
        ...
        if not is_known:
            print(color(
                f"⚠ '{key}' is not a recognized config key — Anakot may not read it; the value "
...
def unset_config_value(key: str):
    ...
    if _is_env_config_key(key):
        ...
        # See #51071.
        from anakot_cli.credential_lifecycle import remove_provider_env_credential

        if not remove_provider_env_credential(key.upper()).get("found"):
            _exit_invalid(f"Config key not set: {key}")
        print(f"✓ Unset {key} from {get_env_path()}")
        return
    from anakot_cli.config_env_routing import is_env_setting_key, remove_env_setting
...
_USAGE_GET = ("Usage: anakot config get <key> [--json] [--raw]", [
    "anakot config get model", "anakot config get terminal.backend",
    "anakot config get skills.config --json"], None)
_USAGE_SET = ("Usage: anakot config set [--force] <key> <value>", [
    "anakot config set model anthropic/claude-sonnet-4", "anakot config set terminal.backend docker",
    "anakot config set OPENROUTER_API_KEY sk-or-..."], [
    "", "  --force: skip the unknown-key notice for unrecognized keys,",
    "           and allow a scalar to replace a whole mapping section"])
_USAGE_UNSET = ("Usage: anakot config unset <key>", [
    "anakot config unset model", "anakot config unset terminal.backend",
    "anakot config unset OPENROUTER_API_KEY"], None)
...
def _cmd_config_check(args):
    ...
    if missing_config:
        ...
        print("    Run 'anakot config migrate' to add them")
...
_CONFIG_USAGE = """Available commands:
  anakot config           Show current configuration
  anakot config edit      Open config in editor
  anakot config get <key>          Print a resolved config value
  anakot config set <key> <value>   Set a config value
  anakot config unset <key>        Remove a config value
  anakot config check     Check for missing/outdated config
  anakot config migrate   Update config with new options
  anakot config path      Show config file path
  anakot config env-path  Show .env file path"""
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    Chat etc. are configurable in ``anakot config`` UI without the core knowing they exist.
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``anakot_cli/`` — i.e. the git checkout for source
    installs, ``/opt/anakot`` inside the published image. It is a property of
    the running interpreter, NOT of ``$ANAKOT_HOME``, which is why a
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$ANAKOT_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/anakot`` in the published image, which instead bakes the stamp at
...
_PLUGIN_COMPAT_LAZY = {
    'normalize_route_base_url': ('anakot_cli.route_identity', 'normalize_route_base_url'),
}
