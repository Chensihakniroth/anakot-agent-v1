"""Bounded product contract for the first Anakot shared-metrics slice."""
...
SCHEMA_KEY = "anakot.metrics.schema_version"
SCHEMA_VERSION = "anakot.metrics.event.v2"
MODEL_CALL_SCOPE = "anakot.model_call"
MODEL_CALL_PROFILE_MODEL = "unknown"
TASK_SCOPE = "anakot.task_run"
TOOL_CALL_SCOPE = "anakot.tool_call"
CLIENT_ACTIVE_MARK = "anakot.client.active"
TOOL_APPROVAL_MARK = "anakot.tool_approval"
SKILL_LIFECYCLE_MARK = "anakot.skill.lifecycle"
SKILL_LOAD_MARK = "anakot.skill.load"
SUBSCRIBER_NAME = "anakot.nemo_relay.shared_metrics"
CLIENT_ACTIVE_METRIC = "anakot.client.active"
LEGACY_MODEL_CALL_METRIC = "anakot.model_call.count"
MODEL_ROUTE_METRIC = "anakot.model_route.count"
TASK_STARTED_METRIC = "anakot.task_run.started"
TASK_FINISHED_METRIC = "anakot.task_run.finished"
TOOL_CALL_METRIC = "anakot.tool_call.count"
TOOL_APPROVAL_METRIC = "anakot.tool_approval.count"
SKILL_LIFECYCLE_METRIC = "anakot.skill.lifecycle.count"
SKILL_LOAD_METRIC = "anakot.skill.load.count"
...
CLIENT_RESOURCE_KEYS = frozenset({"architecture", "anakot_version", "install_method", "os_family"})
...
def client_install_method(value: Any) -> str:
    """Return an allowlisted Anakot installation method."""
    normalized = _norm(value)
    return _allowlisted("nixos" if normalized == "nix" else normalized, CLIENT_INSTALL_METHODS)


def client_resource(
    anakot_version: Any, *, os_name: Any, architecture: Any, install_method: Any
) -> dict[str, str]:
    """Build the bounded client resource attached to aggregate packages."""
    version = str(anakot_version or "").strip()
    return {
        "architecture": client_architecture(architecture),
        "anakot_version": version if 0 < len(version) <= 64 else "unknown",
...
def client_resource_is_valid(resource: Any) -> bool:
    ...
    if not isinstance(resource, dict) or set(resource) != CLIENT_RESOURCE_KEYS:
        return False
    version = resource.get("anakot_version")
    return (
...
def _auxiliary_model_call_dimensions(event: Any) -> dict[str, str] | None:
    """Project a terminal auxiliary route from its Anakot logical scope."""
    metadata = _relay_metadata(
        event, RUNTIME_SCHEMA_KEY, RUNTIME_SCHEMA_VERSION, "anakot.call_role"
    )
    call_role = (metadata or {}).get("anakot.call_role")
...
def execution_surface(kwargs: dict[str, Any]) -> str:
    ...
    try:
        from anakot_cli.platforms import get_all_platforms
...
def task_terminal_state(kwargs: dict[str, Any]) -> tuple[str, str, str]:
    """Map Anakot terminal state to bounded (outcome, end_reason, termination)."""
...
_TOOL_CATEGORY_PREFIXES = (
    ...
    (("discord", "email", "feishu", "anakot-yuanbao", "slack", "sms"), "communication"),
)


def tool_category(kwargs: dict[str, Any]) -> str:
    """Map Anakot registry toolset metadata to a low-cardinality category."""
...
def tool_outcome(kwargs: dict[str, Any]) -> str:
    """Normalize the terminal Anakot tool status without inspecting its result."""
...
def model_call_fields(kwargs: dict[str, Any]) -> dict[str, str]:
    """Return the terminal model identity and provider route known to Anakot."""
