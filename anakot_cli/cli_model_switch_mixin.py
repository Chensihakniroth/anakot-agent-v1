...
Mixin split out of ``cli.py``; bound onto ``AnakotCLI`` via the MRO. cli.py-internal
...
module-level functions taking ``cli`` and siblings are called as ``AnakotCLI.<name>(self, ...)``.
"""
...
def _merge_preflight_warning(cli, result, custom_providers) -> None:
    ...
    if cli.agent is None:
        return
    try:
        from anakot_cli.context_switch_guard import merge_preflight_compression_warning
...
def _print_switch_summary(cli, result, old_model, *, one_turn: bool, strict_context: bool) -> None:
    ...
    from anakot_cli.model_switch import format_model_for_display, resolve_display_context_length
...
def _commit_model_switch(
    ...
    from cli import AnakotCLI, _cprint
    old_model = cli.model
    ...
    if persist_global:
        from anakot_cli.model_switch import persist_model_selection
    ...
    if not one_turn:
        AnakotCLI._persist_model_switch_to_session(cli, result)
...
def _show_model_picker(cli, ctx, force_refresh: bool) -> None:
    ...
    from cli import _cprint
    from anakot_cli.inventory import build_models_payload
...
class CLIModelSwitchMixin:
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider == "copilot":
            from anakot_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        from anakot_cli.models import opencode_provider_family
        if opencode_provider_family(resolved_provider) is not None:
            from anakot_cli.models import normalize_opencode_model_id, opencode_model_api_mode
            return _adopt_with_mode(
        ...
        if self._model_is_default:
            from anakot_cli.codex_models import DEFAULT_CODEX_MODELS, get_codex_model_ids
    ...
    def _restore_session_model(self, session_meta: dict, *, quiet: bool = False) -> None:
        ...
        from anakot_cli.local_runtime.endpoint import LLAMACPP_ALIASES
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        if warning is None:
            return True
        choices = [
            ("once", "Switch anyway", "Use this model for the current Anakot session."),
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # Curated list (same as `anakot model` / gateway pickers); live catalog only when
            ...
            if not model_list:
                try:
                    from anakot_cli.models import cached_provider_model_ids
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from cli import _cprint
        from anakot_cli.model_switch import parse_model_switch_args, resolve_persist_behavior
    ...
    def _cmd_moa(self, cmd_original: str):
        ...
        from anakot_cli.moa_config import moa_usage, normalize_moa_config
