...
def _strip_anakot_path_lines(content: str) -> str:
    """Drop the ``# Anakot Agent`` marker (+ its PATH line) and any anakot PATH line; squash blank runs."""
    new_lines = []
    skip_next = False
    for line in content.split('\n'):
        if '# Anakot Agent' in line or '# anakot-agent' in line:
            skip_next = True
            continue
        if skip_next and ('anakot' in line.lower() and 'PATH' in line):
            skip_next = False
            continue
        skip_next = False
        if 'anakot' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
            continue
...
def remove_path_from_shell_configs():
    """Remove Anakot PATH entries from shell configuration files."""
    removed_from = []
    for config_path in (c for c in (Path.home() / n for n in _SHELL_RC_NAMES) if c.exists()):
        try:
            content = config_path.read_text(encoding="utf-8")
            new_content = _strip_anakot_path_lines(content)
...
def remove_wrapper_script():
    """Remove the anakot wrapper script if it exists."""
    def _unlink_ours(wrapper: Path) -> bool:
        # Only our wrapper (contains a anakot_cli / anakot-agent reference).
        content = wrapper.read_text(encoding="utf-8")
        return _unlink_if('anakot_cli' in content or 'anakot-agent' in content, wrapper)

    candidates = (
        bin_dir / name
        for bin_dir in (Path.home() / ".local" / "bin", Path("/usr/local/bin"))
        for name in ("anakot", "anakot-acp", "anakot-agent"))
...
def remove_node_symlinks(anakot_home: Path) -> list:
    ...
    node_dir = (anakot_home / "node").resolve()
...
def uninstall_gateway_service():
    ...
    try:
        from anakot_cli.gateway import kill_gateway_processes, find_gateway_pids
...
def _remove_launchd_gateway() -> bool:
    ...
    from anakot_cli.gateway import get_launchd_plist_path
...
def _remove_windows_gateway() -> bool:
    ...
    from anakot_cli import gateway_windows as gw
...
# ANAKOT_HOME / ANAKOT_GIT_BASH_PATH (HKCU\Environment), User-scope PATH entries
# (%LOCALAPPDATA%\anakot\git\{cmd,bin,usr\bin}, ...\anakot\node), PortableGit + Node copies
...
def _anakot_path_markers(anakot_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Prefixes identifying Anakot-owned User-PATH entries (prefix match sweeps git\cmd, git\bin,
    ...
    root = str(anakot_home).rstrip("\\/")
    subs = ("anakot-agent", "git", "node", "venv") + (("bin",) if include_managed_bin else ())
    return [f"{root}\\{sub}" for sub in subs]


def remove_path_from_windows_registry(anakot_home: Path, *, include_managed_bin: bool = False) -> list[str]:
    """Strip Anakot-owned entries from User-scope PATH in the registry (see ``_anakot_path_markers``)."""
    markers = tuple(m.lower() for m in _anakot_path_markers(anakot_home, include_managed_bin=include_managed_bin))
...
def remove_anakot_env_vars_windows() -> list[str]:
    """Delete ANAKOT_HOME and ANAKOT_GIT_BASH_PATH from User-scope env vars."""
    def edit(winreg, key, removed):
        for name in ("ANAKOT_HOME", "ANAKOT_GIT_BASH_PATH"):
            try:
...
def remove_portable_tooling_windows(anakot_home: Path) -> list[Path]:
    """Delete the PortableGit / Node / gateway-service dirs the Windows installer created under
    ``anakot_home`` (isolated from any system Git / Node, so nothing else breaks)."""
    targets = (anakot_home / sub for sub in ("git", "node", "gateway-service"))
...
def remove_windows_bin_launchers(*, windows: bool | None = None) -> list[Path]:
    """Delete the ``anakot`` launchers install.ps1 staged in the managed ``bin`` dir: every mode
    ...
    try:
        ...
        from anakot_cli._install_repair import _WINDOWS_BIN_LAUNCHERS
        from anakot_constants import get_default_anakot_root
        bin_dir = get_default_anakot_root() / "bin"
    except Exception as e:
...
def _is_default_anakot_home(anakot_home: Path) -> bool:
    """Return True when ``anakot_home`` points at the default (non-profile) root."""
    try:
        from anakot_constants import get_default_anakot_root
        return anakot_home.resolve() == get_default_anakot_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    out to ``anakot -p <name> gateway stop|uninstall`` because service names / unit paths derive
    from the current ANAKOT_HOME and can't be switched in-process."""
    name = profile.name
    log_info(f"Uninstalling profile '{name}'...")

    # 1. Gateway service, via `python -m anakot_cli.main` (the `anakot` wrapper may be half-gone).
    anakot_invocation = [sys.executable, "-m", "anakot_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                anakot_invocation + ["gateway", subcmd], capture_output=True, text=True,
    ...
    # 3. Wipe the profile's ANAKOT_HOME directory.
    _rmtree_step(profile.path, indent="  ", fully=False)


def run_gui_uninstall(args):
    """``anakot uninstall --gui``: remove the desktop app's built artifacts, packaged bundle
    (best-effort) and Electron userData — never config/sessions/.env, the agent or its venv."""
    from anakot_cli.gui_uninstall import agent_is_installed, gui_install_summary, uninstall_gui
    anakot_home = get_anakot_home()
    summary = gui_install_summary(anakot_home)
    skip_confirm = bool(getattr(args, "yes", False))

    print()
    _print_box("│         ☤ Anakot Chat GUI Uninstaller                  │", Colors.MAGENTA)
    print()

    if not summary["gui_installed"]:
        print("No Anakot Chat GUI installation was found.")
        print(f"  Checked: {anakot_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The anakot agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(anakot_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The anakot agent at {anakot_home / 'anakot-agent'}")
        print(f"  • Your config, sessions, and secrets under {anakot_home}")
        print()
    ...
    print()
    uninstall_gui(anakot_home)

    print()
    _print_box("│            ✓ Chat GUI Uninstalled!                      │", Colors.GREEN)
    print()
    print("The anakot agent is still installed. Run 'anakot' to use the CLI,")
    print("or 'anakot uninstall' to remove the agent too.")
    print()


def run_uninstall(args):
    """Interactive/``--yes`` uninstall: full removes code and ``~/.anakot/``; keep-data removes only
    ...
    if bool(getattr(args, "dry_run", False)):
        _print_uninstall_dry_run(
            project_root=project_root, anakot_home=anakot_home, full_uninstall=full_flag)
        return

    # Named profiles (only when uninstalling from the default root) are offered for cleanup too,
    # instead of leaving zombie ANAKOT_HOMEs and systemd units behind.
    is_default_profile = _is_default_anakot_home(anakot_home)
    ...
    print()
    _print_box("│            ☤ Anakot Agent Uninstaller                  │", Colors.MAGENTA)
    print()

    # Show what will be affected
    print(color("Current Installation:", Colors.CYAN, Colors.BOLD))
    print(f"  Code:    {project_root}")
    print(f"  Config:  {anakot_home / 'config.yaml'}")
    print(f"  Secrets: {anakot_home / '.env'}")
    print(f"  Data:    {anakot_home / 'cron/'}, {anakot_home / 'sessions/'}, {anakot_home / 'logs/'}")
    print()
    ...
    # services, alias wrappers, ANAKOT_HOME dirs) — otherwise they leave zombie services behind.
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Anakot data!", Colors.RED, Colors.BOLD))
        print(color("   Including: configs, API keys, sessions, scheduled jobs, logs", Colors.RED))
        if remove_profiles:
            print(color(f"   Plus {n_profiles} profile(s): {profile_names}", Colors.RED))
    else:
        print("This will remove the Anakot code but keep your configuration and data.")
    ...
    _perform_uninstall(
        project_root=project_root, anakot_home=anakot_home, full_uninstall=full_uninstall,
        remove_profiles=remove_profiles, named_profiles=named_profiles)


def _print_uninstall_dry_run(*, project_root: Path, anakot_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Anakot PATH entries from shell configs / Windows User PATH")
    print("  • Anakot wrapper scripts and Anakot-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if not full_uninstall:
        print(f"  • Keep Anakot config/data: {anakot_home}")
    else:
        print(f"  • Anakot config/data: {anakot_home}")
        profiles = _discover_named_profiles() if _is_default_anakot_home(anakot_home) else []
        if profiles:
...
def _perform_uninstall(
    *,
    ...
    the checkout -> (Windows) PortableGit/Node -> optionally ``$ANAKOT_HOME`` and named profiles."""
    print()
    ...
    # 2-3b. PATH entries, wrapper, Windows launchers, node symlinks. Windows: anakot_home is
    #    %VAR%-expanded because install.ps1 writes literal C:\Users\<u>\...; anakot\bin (launchers +
    ...
    windows = _is_windows()
    sweep_managed_bin = windows and full_uninstall and _is_default_anakot_home(anakot_home)
    for on_this_platform, label, remove, success_fmt, none_msg in (
        ...
        (windows, "Removing PATH entries from Windows User environment...",
         lambda: remove_path_from_windows_registry(
             Path(os.path.expandvars(str(anakot_home))), include_managed_bin=sweep_managed_bin),
         "Removed from User PATH: {}", "No Anakot-owned PATH entries in User environment"),
        (windows, "Removing ANAKOT_HOME / ANAKOT_GIT_BASH_PATH User env vars...",
         remove_anakot_env_vars_windows, "Removed User env var: {}", "No Anakot-set User env vars to remove"),
        (True, "Removing anakot command...", remove_wrapper_script, "Removed {}", "No wrapper script found"),
        (windows, "Removing Windows anakot launchers...",
         remove_windows_bin_launchers, "Removed {}", "No Windows anakot launchers found"),
        (True, "Removing Anakot-managed node/npm/npx symlinks...",
         lambda: remove_node_symlinks(anakot_home), "Removed {}", "No Anakot-managed node/npm/npx symlinks found"),
    ):
        if on_this_platform:
    ...
    #     .env (safe in keep-data mode); the packaged app + Electron userData live OUTSIDE ANAKOT_HOME.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from anakot_cli.gui_uninstall import uninstall_gui
        if not uninstall_gui(anakot_home):
    ...
    if windows:
        _remove_step(
            ...
            lambda: remove_portable_tooling_windows(anakot_home), "Removed {}",
            "No Windows installer artifacts to remove")

    # 5. Optionally remove ~/.anakot/ data directory (and named profiles)
    if full_uninstall:
        ...
        _rmtree_step(anakot_home)
    else:
        log_info(f"Keeping configuration and data in {anakot_home}")
    ...
    print()
    print("Thank you for using Anakot Agent! ☤")
    print()
...
def main(argv=None) -> int:
    """``python -m anakot_cli.uninstall --mode <gui|lite|full>``. Imports only stdlib +
    ``anakot_constants`` + ``anakot_cli.colors``, so it runs under a bare system Python (no venv)."""
    import argparse
    parser = argparse.ArgumentParser(prog="python -m anakot_cli.uninstall")
    parser.add_argument(
