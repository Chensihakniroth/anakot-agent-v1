"""``anakot approvals suggest`` — mine approval history into allowlist proposals.

Anakot has no dedicated approval-decision ledger: ``always`` answers land in ``command_allowlist``
...
def normalize_command(command: str) -> str:
    """Fold user/anakot home prefixes and collapse whitespace."""
    from tools.approval_detection import _rewrite_resolved_anakot_home, _rewrite_resolved_user_home
    return " ".join(_rewrite_resolved_user_home(_rewrite_resolved_anakot_home(command)).split())
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  anakot approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.anakot/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``anakot approvals suggest``."""
    ...
    if apply_spec:
        ...
        if as_json:
            ...
            for pattern in applied:
                print(f"  + {pattern}")
            print(f"\ncommand_allowlist now has {len(merged)} entries (~/.anakot/config.yaml).")
        return 0
...
def approvals_command(args) -> int:
    """Dispatch ``anakot approvals <subcommand>``."""
    sub = getattr(args, "approvals_command", None)
    if sub == "suggest":
        return suggest_command(args)
    if sub == "test":
        from anakot_cli.approvals_test import approvals_test_command
        return approvals_test_command(args)
    print(
        "usage: anakot approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `anakot approvals <subcommand> -h` for details."
    )
    return 1
