...
Tokens live in ~/.anakot/auth.json, NOT ~/.codex/: Anakot keeps its own Codex OAuth session
...
# ``anakot auth`` from a named profile re-signs the ROOT store (93889b770da, #114012).
...
def _clear_pool_entry_status(entry: Dict[str, Any]) -> None:
    ...
    from anakot_cli.auth import _POOL_STATUS_FIELDS
...
def _codex_base_url() -> str:
    return os.getenv("ANAKOT_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
...
def _load_auth_store_maybe_locked(lock: bool) -> Dict[str, Any]:
    ...
    from anakot_cli.auth import _auth_store_lock, _load_auth_store
    if lock:
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Anakot auth store (~/.anakot/auth.json)."""
    from anakot_cli.auth import _load_provider_state, _nonempty_str
...
def _sync_codex_pool_entries(
    ...
    ``device_code`` (the singleton-seeded entry from ``anakot setup`` / the model picker) is always
    synced. ``manual:device_code`` (``anakot auth add openai-codex``) is synced only when its
...
def _save_codex_tokens(
    ...
    """
    from anakot_cli.auth import _provider_state_transaction, _save_auth_store, _store_provider_state, _utc_now_z
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid Codex CLI token pair into Anakot auth, if available.
    ...
    from anakot_cli.auth import _import_codex_cli_tokens, _save_codex_tokens
...
def refresh_codex_oauth_pure(
    ...
    """Refresh Codex OAuth tokens without mutating Anakot auth state."""
    from anakot_cli.auth import _nonempty_str, _utc_now_z
...
def _refresh_codex_auth_tokens(tokens: Dict[str, str], timeout_seconds: float) -> Dict[str, str]:
    ...
    """
    from anakot_cli.auth import _provider_state_transaction, _save_codex_tokens, refresh_codex_oauth_pure
    ...
    with _provider_state_transaction("openai-codex", lock_timeout) as (_store, state, _source):
        ...
        try:
            ...
            # (or another Anakot process) rotates the shared token this frozen copy fails with a
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    ...
    from anakot_cli.auth import _codex_access_token_is_expiring
...
def resolve_codex_runtime_credentials(
    *, force_refresh: bool = False, refresh_if_expiring: bool = True,
    ...
    """Resolve runtime credentials from Anakot's own Codex token store.
    ...
    refresh_timeout_seconds = env_float("ANAKOT_CODEX_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    if _should_refresh(access_token):
        # Re-read under lock to avoid racing with other Anakot processes
        ...
        with _auth_store_lock(timeout_seconds=lock_timeout):
            ...
            access_token = _stripped(tokens.get("access_token"))
    return _codex_runtime_result(
        access_token, source="anakot-auth-store", last_refresh=data.get("last_refresh"))
...
def _probe_codex_quota_restored(
    ...
    this freely.
    """
    from anakot_cli.auth import _codex_quota_probe_cache, _nonempty_str
...
def _refresh_expired_codex_probe_token(
    ...
    """
    from anakot_cli.auth import _codex_quota_probe_cache
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    ...
    from anakot_cli.auth import _nonempty_str, read_credential_pool
...
def _login_openai_codex(args, pconfig: ProviderConfig, *, force_new_login: bool = False) -> None:
    """OpenAI Codex login via device code flow. Tokens stored in ~/.anakot/auth.json."""
    from anakot_cli.auth import (
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("Anakot will create its own session to avoid conflicts with Codex CLI / VS Code.")
            if _prompt_yes_no(
                ...
                print("Anakot will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Anakot gets its own OAuth session
    print()
    print("Signing in to OpenAI Codex...")
    print("(Anakot creates its own session — won't affect Codex CLI or VS Code)")
    print()
...
def _codex_login_rate_limited_error(response: "httpx.Response", *, during: str = "") -> AuthError:
    ...
    # callers surface a "retry later" notice instead of a misleading "run anakot auth" prompt (see issue
    # #32790).
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    from anakot_cli.auth import _utc_now_z
