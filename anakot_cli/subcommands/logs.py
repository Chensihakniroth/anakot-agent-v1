...
def build_logs_parser(subparsers, *, cmd_logs: Callable) -> None:
    ...
    logs_parser = subparsers.add_parser(
        "logs", help="View and filter Anakot log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log / gui.log / desktop.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    anakot logs                    Show last 50 lines of agent.log
    anakot logs -f                 Follow agent.log in real time
    anakot logs errors             Show last 50 lines of errors.log
    anakot logs gateway -n 100     Show last 100 lines of gateway.log
    anakot logs gui -f             Follow gui.log in real time
    anakot logs desktop -f         Follow desktop.log (Electron app boot/backend)
    anakot logs --level WARNING    Only show WARNING and above
    anakot logs --session abc123   Filter by session ID
    anakot logs --component tools  Only show tool-related lines
    anakot logs --since 1h         Lines from the last hour
    anakot logs --since 30m -f     Follow, starting from 30 min ago
    anakot logs list               List available log files with sizes
""")
