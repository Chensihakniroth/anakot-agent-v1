"""Top-level argparse construction for the anakot CLI.

Only the top-level parser and the ``chat`` subparser live here. Every other subparser (model,
gateway, sessions, …) is built by ``anakot_cli/subcommands/<group>.py`` and wired in
...
# (it sets ``ANAKOT_HOME`` and strips itself from ``sys.argv``), so it isn't on the parser.
...
# live parser fails. The derived path is authoritative; tests/anakot_cli/
...
def _cfg_path() -> str:
    """``~/.anakot/config.yaml`` spelled for the active profile, for help text.

    ``main._apply_profile_override`` builds this parser (via ``top_level_value_flag_sets``) BEFORE
    it re-homes the process to the sticky ``active_profile``; ``get_anakot_home()`` would emit the
    "[ANAKOT_HOME fallback] ... wrong profile" warning on every ``anakot`` command for that
    throwaway help string. Read the process home directly: after the override it IS the profile home.
    """
    from anakot_constants import display_anakot_home, get_process_anakot_home

    return f"{display_anakot_home(get_process_anakot_home())}/config.yaml"
...
def top_level_value_flag_sets() -> tuple[frozenset[str], frozenset[str]]:
    ...
    argparse surface — the drift that made ``anakot --reasoning high chat …`` misread ``high`` as
...
def _inherited_flag(parser, *args, **kwargs):
    """``parser.add_argument`` + tag the Action ``inherit_on_relaunch`` for ``anakot_cli.relaunch``."""
...
_EPILOGUE = """
Examples:
    anakot                        Start interactive chat
    anakot chat -q "Hello"        Single query mode
    anakot --tui                  Launch the modern TUI (or set display.interface: tui)
    anakot --cli                  Force the classic REPL (overrides display.interface: tui)
    anakot -c                     Resume the most recent session
    anakot -c "my project"        Resume a session by name (latest in lineage)
    anakot --resume <session_id>  Resume a specific session by ID
    anakot --resume latest        Resume the most recent session (same as -c)
    anakot --tui --resume latest --in ./dir   Resume ./dir's latest session in the TUI
    anakot setup                  Run setup wizard
    anakot logout                 Clear stored authentication
    anakot auth add <provider>    Add a pooled credential
    anakot auth list              List pooled credentials
    anakot auth remove <p> <t>    Remove pooled credential by index, id, or label
    anakot auth reset <p> [t]     Clear exhaustion status for a provider, or one credential
    anakot auth priority <p> <t> <n>  Move a pooled credential to priority n (0 = tried first)
    anakot auth refresh <p> [t]   Refresh a pooled OAuth credential and clear its cooldown
    anakot model                  Select default model
    anakot fallback [list]        Show fallback provider chain
    anakot fallback add           Add a fallback provider (same picker as `anakot model`)
    anakot fallback remove        Remove a fallback provider from the chain
    anakot config                 View configuration
    anakot config edit            Edit config in $EDITOR
    anakot config set model gpt-4 Set a config value
    anakot gateway                Run messaging gateway
    anakot gateway install        Install gateway background service
    anakot gateway start          Start the installed gateway service
    anakot gateway stop           Stop the gateway service
    anakot gateway status         Show gateway status
    anakot -p <profile> <cmd>     Run any command against a named profile's
                                  home (also --profile) — e.g. anakot -p coder gateway stop
    anakot -s anakot-agent-dev,github-auth
    anakot -w                     Start in isolated git worktree
    anakot sessions list          List past sessions
    anakot sessions browse        Interactive session picker
    anakot sessions rename ID T   Rename/title a session
    anakot logs                   View agent.log (last 50 lines)
    anakot logs -f                Follow agent.log in real time
    anakot logs errors            View errors.log
    anakot logs --since 1h        Lines from the last hour
    anakot debug share             Upload debug report for support
    anakot console                Open the safe Anakot command console
    anakot update                 Update to latest version
    anakot dashboard              Start web UI dashboard (port 9119)
    anakot dashboard --stop       Stop running dashboard processes
    anakot dashboard --status     List running dashboard processes
...
def _add_top_level_flags(parser: argparse.ArgumentParser) -> None:
    ...
    inherited(parser, "-m", "--model", default=None, help=(
        ...
        "Applies to -z/--oneshot and --tui. Also settable via ANAKOT_INFERENCE_MODEL env var."))
    inherited(parser, "--provider", default=None, help=(
        "Provider override for this invocation (e.g. openrouter, anthropic). "
        "Applies to -z/--oneshot and --tui. The persistent provider lives in config.yaml "
        "under model.provider — use `anakot setup` or edit the file to change it."))
    ...
    inherited(parser, "--accept-hooks", action="store_true", default=False, help=(
        ...
        "without a TTY prompt.  Equivalent to ANAKOT_ACCEPT_HOOKS=1 or "
...
def _build_chat_parser(subparsers) -> argparse.ArgumentParser:
    ...
    ``anakot -m foo chat`` argparse first sets ``args.model`` from the top-level parser, then
    ...
    unless the flag is actually passed after the subcommand (tests/anakot_cli/
    test_argparse_flag_propagation.py).
    """
    chat_parser = subparsers.add_parser(
        "chat", help="Interactive chat with the agent",
        description="Start an interactive chat session with Anakot Agent")
    ...
    inherited(chat_parser, "--accept-hooks", action="store_true", default=SUPPRESS, help=(
        "Auto-approve any unseen shell hooks declared in config.yaml "
        "without a TTY prompt (see also ANAKOT_ACCEPT_HOOKS env var and "
    ...
    inherited(chat_parser, "--safe-mode", action="store_true", default=SUPPRESS,
              help="Troubleshooting mode: disable ALL customizations — user config, AGENTS.md/memory injection, plugins, and MCP servers (implies --ignore-user-config and --ignore-rules). Use to isolate whether a problem comes from your setup or from Anakot itself.")
...
class AnakotArgumentParser(argparse.ArgumentParser):
    ...
    def _check_value(self, action, value):
        if isinstance(action, argparse._SubParsersAction) and value not in action.choices:
            # ``self.prog`` is "anakot" at the top level and "anakot gateway" for a nested group
...
def build_top_level_parser():
    ...
    """
    parser = AnakotArgumentParser(
        prog="anakot", description="Anakot Agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter, epilog=_EPILOGUE)
    _add_top_level_flags(parser)
    # metavar keeps the usage line to ``anakot [...] <command>`` instead of the brace list of
    # every subcommand name; ``anakot --help`` still lists each command with its help row.
