...
# Install-scoped breadcrumbs live next to the venv (not under $ANAKOT_HOME)
...
def _stdout_to_stderr():
    ...
    a protocol stream (``anakot acp`` speaks JSON-RPC on stdout) must never get install noise."""
...
def _recover_from_interrupted_install() -> None:
    """Finish update work left half-done by a prior ``anakot update``.
...
def _recover_core_update_marker_locked() -> None:
    ...
    from anakot_cli.main import PROJECT_ROOT
    print(
        "⚠ A previous `anakot update` was interrupted mid-install — "
        "finishing dependency installation now...")

    # Windows: a ``anakot.exe`` launch has the launcher as an ancestor; the quarantined full
    ...
    self_locked = _windows_running_anakot_launcher_locked()
    if self_locked:
        install_prefix, install_env = _default_venv_install_target()
        print(
            "  → Running from anakot.exe; applying package-only first aid, "
    ...
    try:
        from anakot_cli import _install_repair as _ir
        ...
        manual = (
            "  Anakot is still running from the launcher that needs "
            "replacing. Close other Anakot windows, restart from a "
...
def _windows_shim_in_process_chain() -> Path | None:
    ...
    ``venv\\Scripts\\anakot.exe`` holds itself open (no ``FILE_SHARE_DELETE``) for the whole
    ...
    with the project venv's own shims so a foreign ``anakot.exe`` never matches.
...
def _venv_shim_matcher():
    ...
    if scripts_dir is None:
        return None
    shims = {_norm_exe_path(shim): shim for shim in _anakot_exe_shims(scripts_dir)}
    if not shims:
        return None
...
def _windows_shim_holder_pid() -> int:
    """Pid a detached child must outwait before touching the venv: the ``anakot.exe`` launcher
...
def _windows_running_anakot_launcher_locked() -> bool:
    """True when a venv ``anakot*.exe`` shim is this process or an ancestor (best-effort)."""
    return _windows_shim_in_process_chain() is not None


# Set on the re-exec'd child so it can never spawn another one.
_UPDATE_REEXEC_ENV = "ANAKOT_UPDATE_REEXEC"
...
def _reexec_dependency_sync_off_windows_shim(gateway_resume: dict | None = None) -> bool:
    ...
    ``.update_exit_code``. The child re-runs ``anakot update`` so the sync and its tail happen
    ...
    ``venv\\Scripts\\anakot.exe`` is a launcher that runs the interpreter with the shim as its script and
    ...
    from anakot_cli.update_handoff import detached_shim_child_env
    python_exe = venv_python_path(shim.parent.parent, windows=True)
    cmd = [str(python_exe), "-m", "anakot_cli.main", *sys.argv[1:]]
...
def _run_install_with_heartbeat(
    ...
    try:
        ...
        # is imported when `anakot update` starts, so an update running
...
def _venv_scripts_dir() -> Path | None:
    ...
    from anakot_cli.main import PROJECT_ROOT
    from anakot_constants import project_venv_dir, venv_bin_dir
...
def _anakot_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _quarantine_running_anakot_exe(
    scripts_dir: Path, *, max_attempts: int = 4, failed_out: list[str] | None = None
) -> list[tuple[Path, Path]]:
    """Pre-empt the Windows file lock on the running ``anakot.exe``.
    ...
    transient; Anakot Desktop backend child: until closed) — retry with backoff, then warn
    ...
    for shim in _anakot_exe_shims(scripts_dir):
        if not shim.exists():
            continue
        ...
        print(
            "    Close Anakot Desktop, exit other `anakot` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `anakot update`.")
...
def _cleanup_pending_shim_renames(scripts_dir: Path) -> int:
    """Drop reboot renames older Anakot versions queued for our shims: ``MOVEFILE_DELAY_UNTIL_REBOOT``
    ...
    try:
        ...
        with winreg.OpenKey(
            ...
            if value_type != winreg.REG_MULTI_SZ or not isinstance(entries, list):
                return 0
            kept, removed = _filter_pending_shim_renames(entries, _anakot_exe_shims(scripts_dir))
            if not removed:
                return 0
            if kept:
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_anakot_exe`` if uv didn't write replacements. Safety-
    critical: a failed quarantine only aborts an update; a failed restore leaves no ``anakot``
...
class ShimQuarantineError(RuntimeError):
    """A live ``anakot*.exe`` shim could not be renamed aside. Raised by
...
def _run_quarantined_install(
    ...
    """Run an editable install, quarantining the running ``anakot.exe`` first.

    Every editable install rewrites the entry-point shims; on Windows the live ``anakot.exe``
    can be neither deleted nor overwritten, so without quarantine ``anakot`` drops off PATH.
    ...
    if scripts_dir is not None:
        moved = _quarantine_running_anakot_exe(scripts_dir, failed_out=failed)
...
# rewrites no entry points, which would otherwise leave the shims quarantined aside and `anakot` missing
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep — and where necessary RESCUE — ``anakot.exe.old.*`` from updates.

    Called early on every invocation. Two cases an unconditional ``unlink()`` gets wrong:
    (1) orphan rescue — ``anakot.exe`` missing while ``anakot.exe.old.*`` exists means the .old
...
def _run_package_only_install(cmd: list[str], *, env: dict[str, str] | None = None) -> None:
    ...
    ``anakot.exe``, and the quarantine path would rename shims uv then never recreates.
...
def _repair_venv_via_import_probes(
    ...
    ``anakot.exe``. Never raises. Returns ``"healthy"``, ``"repaired"``, ``"failed"``
    ...
    print("  ⚠ Venv repair incomplete. Run manually, then `anakot update`:")
...
def _interpreter_scripts_dir() -> Path | None:
    ...
    See #76105.
    """
    from anakot_constants import venv_bin_dir
...
def _install_python_dependencies_with_optional_fallback(
    ...
    attempt quarantines the live ``anakot*.exe`` shims first. When ``env`` carries a
    ...
    if pin_python:
        ...
        # dir, not PROJECT_ROOT/venv; quarantining the wrong dir leaves anakot.exe locked.
...
def _verify_console_scripts_installed(
    ...
    On Windows ``uv pip install -e .`` can register ``anakot.exe`` in the wheel RECORD while the
    file never lands (live shim locked, launcher write skipped), so ``anakot`` drops off PATH
    after a "successful" install. Missing shims get ``--reinstall -e .`` under quarantine.

    The symptom is ``anakot-agent.exe`` and ``anakot-acp.exe`` present but ``anakot.exe`` missing, so
    ``anakot`` drops off PATH even though the install reported success (issue #52931).
    """
    if not _is_windows():
        return
    scripts_dir = _venv_scripts_dir()
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            "  ⚠ Entry point repair failed; try `anakot update --force` after "
            "closing other anakot processes.")):
        return
    _report_still_missing(
        _missing(), "Workaround: python -m anakot_cli.main <command>",
...
def _verify_core_dependencies_installed(
    ...
    # ``anakot update``; the install prefix/env encode which environment we targeted.
    venv_python = _resolve_install_target_python(install_cmd_prefix, env)
    if venv_python is None:
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg="  ⚠ Repair install failed; check `anakot update` output above."):
        return
    ...
    if not _run_repair_step(
        ...
        fail_msg=(
            ...
            "Run `anakot update --force` after closing other anakot processes.")):
        return
    _report_still_missing(
        _missing_deps(), "Run `anakot update --force` after closing other anakot processes.",
