"""Anakot Agent — Web UI server: FastAPI app assembly, auth/host middleware, ``start_server``.
...
Usage: ``python -m anakot_cli.main web [--port 8080]``.
"""
...
from anakot_cli.install_identity import get_install_id as _shared_get_install_id
from anakot_cli.pty_session import run_reaper
...
try:
    ...
    # running `anakot dashboard` needs fastapi+uvicorn; lazy install keeps
...
from anakot_cli.web_server_lifecycle import (  # noqa: E402
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The desktop spawns a ``anakot dashboard`` backend, not a gateway, so without
    ...
    if isinstance(provider, InProcessCronScheduler):
        try:
            from anakot_cli.profiles import (
            ...
            if initial_profile_homes:
                ...
                from anakot_logging import enable_profile_log_routing
...
async def _lifespan(app: "FastAPI"):
    ...
    # (#79531/#80037): a store left behind by `anakot update` otherwise 500s
    ...
    # Import anakot_cli.gateway *before* the yield: on Windows + 3.11 the
    ...
    # refuse with "restart required" after `anakot update` replaced the code
    ...
    # Desktop-spawned backends (ANAKOT_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `anakot
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("ANAKOT_DESKTOP") == "1":
        ...
        try:
            from anakot_cli.gateway import _reap_unsupervised_gateway_orphans
    ...
    def _boot_local_runtime():
        try:
            from anakot_cli.config import load_config
            from anakot_cli.local_runtime.bootstrap import ensure_local_runtime
    ...
    # when ANAKOT_GUEST_ONBOARDING=1, records the answer for setup.status / free_tier.status and
    ...
    from anakot_cli.free_tier_bootstrap import start_background_bootstrap
    ...
    try:
        ...
        try:
            from anakot_cli.local_runtime.bootstrap import shutdown_local_runtime
...
app = FastAPI(title="Anakot Agent", version=__version__, lifespan=_lifespan)


# Memory-provider OAuth connect routes live in the memory layer, not here.
from anakot_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402

app.include_router(_memory_oauth_router)

# Session token for sensitive endpoints. The desktop shell mints it via
# ANAKOT_DASHBOARD_SESSION_TOKEN; otherwise fresh per server start. It dies with
# the process and is injected into the SPA HTML so only the web UI can use it.
def _resolve_session_token() -> str:
    return os.environ.get("ANAKOT_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)


_SESSION_TOKEN = _resolve_session_token()
_SESSION_HEADER_NAME = "X-Anakot-Session-Token"
...
def _apply_ssh_owner_nonce(nonce: Optional[str]) -> None:
    ...
    if nonce:
        ...
        try:
            marker = os.path.join(purelib, f".anakot-ssh-runtime-{nonce}")
...
from anakot_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS as _PUBLIC_API_PATHS
...
def _dashboard_public_hosts() -> frozenset[str]:
    ...
    """
    from anakot_cli.dashboard_auth.prefix import resolve_public_url
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    old launch scripts but IGNORED since the June 2026 anakot-0day campaign.
    """
...
def _desktop_loopback_auth_exempt(
    ...
    ``ANAKOT_DESKTOP=1``, and an operator-minted credential (env token, SSH
    session token, or owner nonce).
    """
    return (
        host in _LOOPBACK_HOST_VALUES
        and os.environ.get("ANAKOT_DESKTOP") == "1"
        and bool(os.environ.get("ANAKOT_DASHBOARD_SESSION_TOKEN") or ssh_session_token or ssh_owner_nonce)
    )
...
async def _plugin_api_runtime_gate(request: Request, call_next):
    ...
    if plugin_name and (
        ...
        try:
            ...
            # (#46435)
            from anakot_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
...
async def _dashboard_auth_gate(request: Request, call_next):
    ...
    """
    from anakot_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    """
    from anakot_cli.dashboard_auth.token_auth import token_auth_middleware
...
from anakot_cli import web_server_gateway as _gateway_mod  # noqa: E402
from anakot_cli.web_server_gateway import _ACTION_LOG_FILES, _terminate_desktop_managed_gateway  # noqa: E402
from anakot_cli.web_server_sessions import _auto_archive_ticker_loop  # noqa: E402
from anakot_cli.web_server_chat import PTY_REGISTRY  # noqa: E402
from anakot_cli.web_server_dashboard import (  # noqa: E402
...
# persisted under the ROOT Anakot home (not the profile ANAKOT_HOME) so every
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``anakot gateway restart``, reusing an in-flight or recent restart.
    ...
    proc = _gateway_mod._spawn_anakot_action(subcommand, "gateway-restart")
...
from anakot_cli.web_routers import (  # noqa: E402
...
def _no_auth_provider_message(host: str) -> str:
    ...
    if host in _LOOPBACK_HOST_VALUES:
        ...
        fix_hint = (
            ...
            "(and unset ANAKOT_DASHBOARD_PUBLIC_URL) to restore the "
    ...
    fix_hint += (
        ...
        "  • OAuth: run `anakot dashboard register` (Nous Portal) or "
    ...
    try:
        from anakot_cli.config import load_config as _load_cfg
        from anakot_cli.plugins_cmd import _BASIC_AUTH_PLUGIN_KEYS
        ...
        if has_creds and (set(disabled) & _BASIC_AUTH_PLUGIN_KEYS):
            fix_hint = (
                ...
                "`anakot plugins enable basic`), then restart the "
                "dashboard.\n\n"
            ) + fix_hint
    except Exception:
        pass
...
def _configure_auth_gate(
    ...
    # ``--insecure`` no longer disables the gate (June 2026 anakot-0day
    ...
    if app.state.auth_required:
        ...
        from anakot_cli.dashboard_auth import list_providers
...
def _build_uvicorn_server(host: str, port: int, *, ssh_isolated: bool = False):
    ...
    if ssh_isolated:
        from anakot_cli.web_server_idle_exit import (
...
def _on_server_started(
    ...
    *,
    ...
    def _reap_desktop_serves() -> None:
        from anakot_cli.dashboard_procs import _reap_orphaned_desktop_local_serves

        _reap_orphaned_desktop_local_serves()

    def _reap_mcp_helpers() -> None:
        from anakot_cli.process_identity import reap_orphaned_mcp_helpers
    ...
    # No-op for standalone `anakot serve` (no ANAKOT_PARENT_PID).
    _start_parent_death_watchdog()
    ...
    if getattr(app.state, "ssh_isolated_clients", None) is not None:
        from anakot_cli.web_server_idle_exit import DEFAULT_IDLE_GRACE_S, start_idle_watchdog
        try:
    ...
    # ACTUAL port — what lets `anakot update` relaunch a manually-started serve
    # on its real endpoint (#63206).
    def _register_identity() -> None:
        from anakot_cli.process_identity import attach_self_to_kill_on_close_job, register_self
    ...
    ready_token = "ANAKOT_BACKEND_READY" if headless else "ANAKOT_DASHBOARD_READY"
    ...
    if headless:
        ...
        print(f"  Anakot backend listening on {host}:{actual_port}", flush=True)
    else:
        print(f"  Anakot Web UI → http://{host}:{actual_port}")
    ...
    if start_mcp_discovery_after_bind:
        ...
        try:
            from anakot_cli.mcp_startup import defer_background_mcp_discovery
...
def start_server(
    ...
    JSON-RPC/WS backend, no UI build, no SPA mount (``ANAKOT_SERVE_HEADLESS``).
    ...
    from anakot_cli.resource_limits import apply_nofile_soft_limit
    ...
    try:
        from anakot_cli.nous_auth_keepalive import start_nous_auth_keepalive
...
_PLUGIN_COMPAT_LAZY = {
    'AudioTranscriptionRequest': ('anakot_cli.web_models', 'AudioTranscriptionRequest'),
    'AutomationBlueprintInstantiate': ('anakot_cli.web_models', 'AutomationBlueprintInstantiate'),
    'BackupRequest': ('anakot_cli.web_models', 'BackupRequest'),
    'BulkDeleteSessions': ('anakot_cli.web_models', 'BulkDeleteSessions'),
    'CONFIG_SCHEMA': ('anakot_cli.web_server_config', 'CONFIG_SCHEMA'),
    'ChatImageUpload': ('anakot_cli.web_models', 'ChatImageUpload'),
    'ConfigUpdate': ('anakot_cli.web_models', 'ConfigUpdate'),
    'CredentialPoolAdd': ('anakot_cli.web_models', 'CredentialPoolAdd'),
    'CronJobCreate': ('anakot_cli.web_models', 'CronJobCreate'),
    'CronJobUpdate': ('anakot_cli.web_models', 'CronJobUpdate'),
    'CuratorPause': ('anakot_cli.web_models', 'CuratorPause'),
    'CustomEndpointUpdate': ('anakot_cli.web_models', 'CustomEndpointUpdate'),
    'DEFAULT_CONFIG': ('anakot_cli.config', 'DEFAULT_CONFIG'),
    'DebugShareRequest': ('anakot_cli.web_models', 'DebugShareRequest'),
    'EnvVarDelete': ('anakot_cli.web_models', 'EnvVarDelete'),
    'EnvVarReveal': ('anakot_cli.web_models', 'EnvVarReveal'),
    'EnvVarUpdate': ('anakot_cli.web_models', 'EnvVarUpdate'),
    'FontSetBody': ('anakot_cli.web_models', 'FontSetBody'),
    'FsWriteText': ('anakot_cli.web_models', 'FsWriteText'),
    'GitBranchSwitchBody': ('anakot_cli.web_models', 'GitBranchSwitchBody'),
    'GitCommitBody': ('anakot_cli.web_models', 'GitCommitBody'),
    'GitFileBody': ('anakot_cli.web_models', 'GitFileBody'),
    'GitPathBody': ('anakot_cli.web_models', 'GitPathBody'),
    'GitWorktreeAddBody': ('anakot_cli.web_models', 'GitWorktreeAddBody'),
    'GitWorktreeRemoveBody': ('anakot_cli.web_models', 'GitWorktreeRemoveBody'),
    'HookCreate': ('anakot_cli.web_models', 'HookCreate'),
    'HookDelete': ('anakot_cli.web_models', 'HookDelete'),
    'ImportRequest': ('anakot_cli.web_models', 'ImportRequest'),
    'LearningNodeEdit': ('anakot_cli.web_models', 'LearningNodeEdit'),
    'LearningNodeRef': ('anakot_cli.web_models', 'LearningNodeRef'),
    'MCPCatalogInstall': ('anakot_cli.web_models', 'MCPCatalogInstall'),
    'MCPEnabledToggle': ('anakot_cli.web_models', 'MCPEnabledToggle'),
    'MCPServerCreate': ('anakot_cli.web_models', 'MCPServerCreate'),
    'MCPServersReplace': ('anakot_cli.web_models', 'MCPServersReplace'),
    'ManagedDirectoryCreate': ('anakot_cli.web_models', 'ManagedDirectoryCreate'),
    'ManagedFileDelete': ('anakot_cli.web_models', 'ManagedFileDelete'),
    'ManagedFileUpload': ('anakot_cli.web_models', 'ManagedFileUpload'),
    'ManagedFilesPolicy': ('anakot_cli.web_server_files', 'ManagedFilesPolicy'),
    'MemoryProviderConfigUpdate': ('anakot_cli.web_models', 'MemoryProviderConfigUpdate'),
    'MemoryProviderSelect': ('anakot_cli.web_models', 'MemoryProviderSelect'),
    'MemoryProviderSetupRequest': ('anakot_cli.web_models', 'MemoryProviderSetupRequest'),
    'MemoryReset': ('anakot_cli.web_models', 'MemoryReset'),
    'MessagingPlatformUpdate': ('anakot_cli.web_models', 'MessagingPlatformUpdate'),
    'MoaConfigPayload': ('anakot_cli.web_models', 'MoaConfigPayload'),
    'MoaModelSlot': ('anakot_cli.web_models', 'MoaModelSlot'),
    'MoaPresetPayload': ('anakot_cli.web_models', 'MoaPresetPayload'),
    'ModelAssignment': ('anakot_cli.web_models', 'ModelAssignment'),
    'OAuthSubmitBody': ('anakot_cli.web_models', 'OAuthSubmitBody'),
    'OPTIONAL_ENV_VARS': ('anakot_cli.config', 'OPTIONAL_ENV_VARS'),
    'PairingApprove': ('anakot_cli.web_models', 'PairingApprove'),
    'PairingRevoke': ('anakot_cli.web_models', 'PairingRevoke'),
    'ProfileActiveUpdate': ('anakot_cli.web_models', 'ProfileActiveUpdate'),
    'ProfileCreate': ('anakot_cli.web_models', 'ProfileCreate'),
    'ProfileDescribeAuto': ('anakot_cli.web_models', 'ProfileDescribeAuto'),
    'ProfileDescriptionUpdate': ('anakot_cli.web_models', 'ProfileDescriptionUpdate'),
    'ProfileModelUpdate': ('anakot_cli.web_models', 'ProfileModelUpdate'),
    'ProfileRename': ('anakot_cli.web_models', 'ProfileRename'),
    'ProfileSoulUpdate': ('anakot_cli.web_models', 'ProfileSoulUpdate'),
    ...
    'PtyBridge': ('anakot_cli.pty_bridge', 'PtyBridge'),
    'PtySessionRegistry': ('anakot_cli.pty_session', 'PtySessionRegistry'),
    'PtyUnavailableError': ('anakot_cli.pty_bridge', 'PtyUnavailableError'),
    'RawConfigUpdate': ('anakot_cli.web_models', 'RawConfigUpdate'),
    'RegistryFull': ('anakot_cli.pty_session', 'RegistryFull'),
    'STORAGE_HONCHO_HOST_BLOCK': ('plugins.memory.config_schema', 'STORAGE_HONCHO_HOST_BLOCK'),
    'SessionImport': ('anakot_cli.web_models', 'SessionImport'),
    'SessionPrune': ('anakot_cli.web_models', 'SessionPrune'),
    'SessionRename': ('anakot_cli.web_models', 'SessionRename'),
    'SkillContentUpdate': ('anakot_cli.web_models', 'SkillContentUpdate'),
    'SkillCreate': ('anakot_cli.web_models', 'SkillCreate'),
    'SkillInstallRequest': ('anakot_cli.web_models', 'SkillInstallRequest'),
    'SkillToggle': ('anakot_cli.web_models', 'SkillToggle'),
    'SkillUninstallRequest': ('anakot_cli.web_models', 'SkillUninstallRequest'),
    'SkillsUpdateRequest': ('anakot_cli.web_models', 'SkillsUpdateRequest'),
    'TTSLeaseRequest': ('anakot_cli.web_models', 'TTSLeaseRequest'),
    'TTSSpeakRequest': ('anakot_cli.web_models', 'TTSSpeakRequest'),
    'TelegramOnboardingApply': ('anakot_cli.web_models', 'TelegramOnboardingApply'),
    'TelegramOnboardingStart': ('anakot_cli.web_models', 'TelegramOnboardingStart'),
    'TerminalBackendSelect': ('anakot_cli.web_models', 'TerminalBackendSelect'),
    'ThemeSetBody': ('anakot_cli.web_models', 'ThemeSetBody'),
    'ToolsetEnvUpdate': ('anakot_cli.web_models', 'ToolsetEnvUpdate'),
    'ToolsetModelSelect': ('anakot_cli.web_models', 'ToolsetModelSelect'),
    'ToolsetPostSetup': ('anakot_cli.web_models', 'ToolsetPostSetup'),
    'ToolsetProviderSelect': ('anakot_cli.web_models', 'ToolsetProviderSelect'),
    'ToolsetToggle': ('anakot_cli.web_models', 'ToolsetToggle'),
    'WebhookCreate': ('anakot_cli.web_models', 'WebhookCreate'),
    'WebhookEnabledToggle': ('anakot_cli.web_models', 'WebhookEnabledToggle'),
    'WhatsAppOnboardingApply': ('anakot_cli.web_models', 'WhatsAppOnboardingApply'),
    'WhatsAppOnboardingStart': ('anakot_cli.web_models', 'WhatsAppOnboardingStart'),
    'activate_custom_endpoint': ('anakot_cli.web_routers.config_env', 'activate_custom_endpoint'),
    'add_credential_pool_entry': ('anakot_cli.web_routers.ops', 'add_credential_pool_entry'),
    'add_mcp_server': ('anakot_cli.web_routers.mcp', 'add_mcp_server'),
    'apply_telegram_onboarding': ('anakot_cli.web_routers.messaging', 'apply_telegram_onboarding'),
    'apply_whatsapp_onboarding': ('anakot_cli.web_routers.messaging', 'apply_whatsapp_onboarding'),
    'approve_pairing': ('anakot_cli.web_routers.ops', 'approve_pairing'),
    'auth_mcp_server': ('anakot_cli.web_routers.mcp', 'auth_mcp_server'),
    'build_cron_model_impact': ('anakot_cli.config', 'build_cron_model_impact'),
    'bulk_delete_sessions_endpoint': ('anakot_cli.web_routers.sessions', 'bulk_delete_sessions_endpoint'),
    'cancel_oauth_session': ('anakot_cli.web_routers.oauth', 'cancel_oauth_session'),
    'cancel_telegram_onboarding': ('anakot_cli.web_routers.messaging', 'cancel_telegram_onboarding'),
    'cancel_whatsapp_onboarding': ('anakot_cli.web_routers.messaging', 'cancel_whatsapp_onboarding'),
    'cfg_get': ('anakot_cli.config', 'cfg_get'),
    'check_config_version': ('anakot_cli.config', 'check_config_version'),
    'check_anakot_update': ('anakot_cli.web_routers.actions', 'check_anakot_update'),
    'clear_model_endpoint_credentials': ('anakot_cli.config', 'clear_model_endpoint_credentials'),
    'clear_pending_pairing': ('anakot_cli.web_routers.ops', 'clear_pending_pairing'),
    'coerce_provider_id': ('anakot_cli.config', 'coerce_provider_id'),
    'console_ws': ('anakot_cli.web_routers.chat_ws', 'console_ws'),
    'count_empty_sessions_endpoint': ('anakot_cli.web_routers.sessions', 'count_empty_sessions_endpoint'),
    'create_cron_job': ('anakot_cli.web_routers.cron', 'create_cron_job'),
    'create_hook': ('anakot_cli.web_routers.ops', 'create_hook'),
    'create_managed_directory': ('anakot_cli.web_routers.files', 'create_managed_directory'),
    'create_profile_endpoint': ('anakot_cli.web_routers.profiles', 'create_profile_endpoint'),
    'create_skill': ('anakot_cli.web_routers.skills', 'create_skill'),
    'create_webhook': ('anakot_cli.web_routers.ops', 'create_webhook'),
    'cron_fire_webhook': ('anakot_cli.web_routers.cron', 'cron_fire_webhook'),
    'custom_endpoint_key_env': ('anakot_cli.config', 'custom_endpoint_key_env'),
    'delete_agent_plugin': ('anakot_cli.web_routers.dashboard_ui', 'delete_agent_plugin'),
    'delete_cron_job': ('anakot_cli.web_routers.cron', 'delete_cron_job'),
    'delete_custom_endpoint': ('anakot_cli.web_routers.config_env', 'delete_custom_endpoint'),
    'delete_empty_sessions_endpoint': ('anakot_cli.web_routers.sessions', 'delete_empty_sessions_endpoint'),
    'delete_hook': ('anakot_cli.web_routers.ops', 'delete_hook'),
    'delete_learning_node': ('anakot_cli.web_routers.status', 'delete_learning_node'),
    'delete_managed_file': ('anakot_cli.web_routers.files', 'delete_managed_file'),
    'delete_profile_endpoint': ('anakot_cli.web_routers.profiles', 'delete_profile_endpoint'),
    'delete_session_endpoint': ('anakot_cli.web_routers.sessions', 'delete_session_endpoint'),
    'delete_webhook': ('anakot_cli.web_routers.ops', 'delete_webhook'),
    'derive_gateway_busy': ('gateway.status', 'derive_gateway_busy'),
    'derive_gateway_drainable': ('gateway.status', 'derive_gateway_drainable'),
    'describe_profile_auto_endpoint': ('anakot_cli.web_routers.profiles', 'describe_profile_auto_endpoint'),
    'detect_install_method': ('anakot_cli.config', 'detect_install_method'),
    'disconnect_oauth_provider': ('anakot_cli.web_routers.oauth', 'disconnect_oauth_provider'),
    'download_dashboard_backup': ('anakot_cli.web_routers.ops', 'download_dashboard_backup'),
    'download_managed_file': ('anakot_cli.web_routers.files', 'download_managed_file'),
    'enable_webhooks': ('anakot_cli.web_routers.ops', 'enable_webhooks'),
    'env_var_enabled': ('utils', 'env_var_enabled'),
    'events_ws': ('anakot_cli.web_routers.chat_ws', 'events_ws'),
    'export_session_endpoint': ('anakot_cli.web_routers.sessions', 'export_session_endpoint'),
    'find_provider_entry': ('anakot_cli.config', 'find_provider_entry'),
    'format_docker_update_message': ('anakot_cli.config', 'format_docker_update_message'),
    'fs_default_cwd': ('anakot_cli.web_routers.files', 'fs_default_cwd'),
    'fs_download': ('anakot_cli.web_routers.files', 'fs_download'),
    'fs_git_root': ('anakot_cli.web_routers.files', 'fs_git_root'),
    'fs_list': ('anakot_cli.web_routers.files', 'fs_list'),
    'fs_read_data_url': ('anakot_cli.web_routers.files', 'fs_read_data_url'),
    'fs_read_text': ('anakot_cli.web_routers.files', 'fs_read_text'),
    'fs_write_text': ('anakot_cli.web_routers.files', 'fs_write_text'),
    'gateway_drain': ('anakot_cli.web_routers.actions', 'gateway_drain'),
    'gateway_ws': ('anakot_cli.web_routers.chat_ws', 'gateway_ws'),
    'get_action_status': ('anakot_cli.web_routers.actions', 'get_action_status'),
    'get_active_profile_endpoint': ('anakot_cli.web_routers.profiles', 'get_active_profile_endpoint'),
    'get_auxiliary_models': ('anakot_cli.web_routers.models', 'get_auxiliary_models'),
    'get_client_voice_config': ('anakot_cli.web_routers.audio', 'get_client_voice_config'),
    'get_computer_use_status': ('anakot_cli.web_routers.tools', 'get_computer_use_status'),
    'get_config': ('anakot_cli.web_routers.config_env', 'get_config'),
    'get_config_path': ('anakot_cli.config', 'get_config_path'),
    'get_config_raw': ('anakot_cli.web_routers.analytics', 'get_config_raw'),
    'get_cron_delivery_targets': ('anakot_cli.web_routers.cron', 'get_cron_delivery_targets'),
    'get_cron_job': ('anakot_cli.web_routers.cron', 'get_cron_job'),
    'get_curator_status': ('anakot_cli.web_routers.status', 'get_curator_status'),
    'get_dashboard_font': ('anakot_cli.web_routers.dashboard_ui', 'get_dashboard_font'),
    'get_dashboard_plugins': ('anakot_cli.web_routers.dashboard_ui', 'get_dashboard_plugins'),
    'get_dashboard_themes': ('anakot_cli.web_routers.dashboard_ui', 'get_dashboard_themes'),
    'get_defaults': ('anakot_cli.web_routers.config_env', 'get_defaults'),
    'get_egress_status': ('anakot_cli.web_routers.config_env', 'get_egress_status'),
    'get_elevenlabs_voices': ('anakot_cli.web_routers.audio', 'get_elevenlabs_voices'),
    'get_env_path': ('anakot_cli.config', 'get_env_path'),
    'get_env_vars': ('anakot_cli.web_routers.config_env', 'get_env_vars'),
    'get_health': ('anakot_cli.web_routers.status', 'get_health'),
    'get_anakot_home': ('anakot_cli.config', 'get_anakot_home'),
    'get_learning_graph': ('anakot_cli.web_routers.status', 'get_learning_graph'),
    'get_learning_node': ('anakot_cli.web_routers.status', 'get_learning_node'),
    'get_logs': ('anakot_cli.web_routers.status', 'get_logs'),
    'get_media': ('anakot_cli.web_routers.files', 'get_media'),
    'get_memory_provider_config': ('anakot_cli.web_routers.memory_providers', 'get_memory_provider_config'),
    'get_memory_status': ('anakot_cli.web_routers.ops', 'get_memory_status'),
    'get_messaging_platforms': ('anakot_cli.web_routers.messaging', 'get_messaging_platforms'),
    'get_moa_models': ('anakot_cli.web_routers.models', 'get_moa_models'),
    'get_model_info': ('anakot_cli.web_routers.models', 'get_model_info'),
    'get_model_options': ('anakot_cli.web_routers.models', 'get_model_options'),
    'get_models_analytics': ('anakot_cli.web_routers.analytics', 'get_models_analytics'),
    'get_plugins_hub': ('anakot_cli.web_routers.dashboard_ui', 'get_plugins_hub'),
    'get_portal_status': ('anakot_cli.web_routers.status', 'get_portal_status'),
    'get_process_anakot_home': ('anakot_cli.config', 'get_process_anakot_home'),
    'get_profile_setup_command': ('anakot_cli.web_routers.profiles', 'get_profile_setup_command'),
    'get_profile_soul': ('anakot_cli.web_routers.profiles', 'get_profile_soul'),
    'get_profiles_sessions': ('anakot_cli.web_routers.profiles', 'get_profiles_sessions'),
    'get_profiles_sessions_sidebar': ('anakot_cli.web_routers.profiles', 'get_profiles_sessions_sidebar'),
    'get_provider_config_schema': ('plugins.memory.config_schema', 'get_provider_config_schema'),
    'get_recommended_default_model': ('anakot_cli.web_routers.models', 'get_recommended_default_model'),
    ...
    'get_schema': ('anakot_cli.web_routers.config_env', 'get_schema'),
    'get_session_detail': ('anakot_cli.web_routers.sessions', 'get_session_detail'),
    'get_session_latest_descendant': ('anakot_cli.web_routers.sessions', 'get_session_latest_descendant'),
    'get_session_messages': ('anakot_cli.web_routers.sessions', 'get_session_messages'),
    'get_session_stats': ('anakot_cli.web_routers.sessions', 'get_session_stats'),
    'get_sessions': ('anakot_cli.web_routers.sessions', 'get_sessions'),
    'get_skill_content': ('anakot_cli.web_routers.skills', 'get_skill_content'),
    'get_skills': ('anakot_cli.web_routers.skills', 'get_skills'),
    'get_ssh_ownership': ('anakot_cli.web_routers.status', 'get_ssh_ownership'),
    'get_status': ('anakot_cli.web_routers.status', 'get_status'),
    'get_system_stats': ('anakot_cli.web_routers.status', 'get_system_stats'),
    'get_telegram_onboarding_status': ('anakot_cli.web_routers.messaging', 'get_telegram_onboarding_status'),
    'get_terminal_backends': ('anakot_cli.web_routers.tools', 'get_terminal_backends'),
    'get_toolset_config': ('anakot_cli.web_routers.tools', 'get_toolset_config'),
    'get_toolset_models': ('anakot_cli.web_routers.tools', 'get_toolset_models'),
    'get_toolsets': ('anakot_cli.web_routers.tools', 'get_toolsets'),
    'get_update_receipt': ('anakot_cli.web_routers.actions', 'get_update_receipt'),
    'get_usage_analytics': ('anakot_cli.web_routers.analytics', 'get_usage_analytics'),
    'get_whatsapp_onboarding_status': ('anakot_cli.web_routers.messaging', 'get_whatsapp_onboarding_status'),
    'git_base_branches_route': ('anakot_cli.web_routers.git', 'git_base_branches_route'),
    'git_branch_switch_route': ('anakot_cli.web_routers.git', 'git_branch_switch_route'),
    'git_branches_route': ('anakot_cli.web_routers.git', 'git_branches_route'),
    'git_commit_context_route': ('anakot_cli.web_routers.git', 'git_commit_context_route'),
    'git_commit_route': ('anakot_cli.web_routers.git', 'git_commit_route'),
    'git_create_pr_route': ('anakot_cli.web_routers.git', 'git_create_pr_route'),
    'git_file_diff_route': ('anakot_cli.web_routers.git', 'git_file_diff_route'),
    'git_push_route': ('anakot_cli.web_routers.git', 'git_push_route'),
    'git_rev_parse_route': ('anakot_cli.web_routers.git', 'git_rev_parse_route'),
    'git_revert_route': ('anakot_cli.web_routers.git', 'git_revert_route'),
    'git_review_diff_route': ('anakot_cli.web_routers.git', 'git_review_diff_route'),
    'git_review_list_route': ('anakot_cli.web_routers.git', 'git_review_list_route'),
    'git_ship_info_route': ('anakot_cli.web_routers.git', 'git_ship_info_route'),
    'git_stage_route': ('anakot_cli.web_routers.git', 'git_stage_route'),
    'git_status_route': ('anakot_cli.web_routers.git', 'git_status_route'),
    'git_unstage_route': ('anakot_cli.web_routers.git', 'git_unstage_route'),
    'git_worktree_add_route': ('anakot_cli.web_routers.git', 'git_worktree_add_route'),
    'git_worktree_remove_route': ('anakot_cli.web_routers.git', 'git_worktree_remove_route'),
    'git_worktrees_route': ('anakot_cli.web_routers.git', 'git_worktrees_route'),
    'grant_computer_use_permissions': ('anakot_cli.web_routers.tools', 'grant_computer_use_permissions'),
    'import_sessions_endpoint': ('anakot_cli.web_routers.sessions', 'import_sessions_endpoint'),
    'install_mcp_catalog_entry': ('anakot_cli.web_routers.mcp', 'install_mcp_catalog_entry'),
    'install_skill_hub': ('anakot_cli.web_routers.skills', 'install_skill_hub'),
    'instantiate_blueprint': ('anakot_cli.web_routers.cron', 'instantiate_blueprint'),
    'is_nix_install_method': ('anakot_cli.config', 'is_nix_install_method'),
    'list_checkpoints': ('anakot_cli.web_routers.ops', 'list_checkpoints'),
    'list_credential_pool': ('anakot_cli.web_routers.ops', 'list_credential_pool'),
    'list_cron_blueprints': ('anakot_cli.web_routers.cron', 'list_cron_blueprints'),
    'list_cron_job_runs': ('anakot_cli.web_routers.cron', 'list_cron_job_runs'),
    'list_cron_jobs': ('anakot_cli.web_routers.cron', 'list_cron_jobs'),
    'list_custom_endpoints': ('anakot_cli.web_routers.config_env', 'list_custom_endpoints'),
    'list_hooks': ('anakot_cli.web_routers.ops', 'list_hooks'),
    'list_managed_files': ('anakot_cli.web_routers.files', 'list_managed_files'),
    'list_mcp_catalog': ('anakot_cli.web_routers.mcp', 'list_mcp_catalog'),
    'list_mcp_servers': ('anakot_cli.web_routers.mcp', 'list_mcp_servers'),
    'list_oauth_providers': ('anakot_cli.web_routers.oauth', 'list_oauth_providers'),
    'list_pairing': ('anakot_cli.web_routers.ops', 'list_pairing'),
    'list_profiles_endpoint': ('anakot_cli.web_routers.profiles', 'list_profiles_endpoint'),
    'list_skills_hub_sources': ('anakot_cli.web_routers.skills', 'list_skills_hub_sources'),
    'list_webhooks': ('anakot_cli.web_routers.ops', 'list_webhooks'),
    'load_env': ('anakot_cli.config', 'load_env'),
    'mcp_oauth_callback': ('anakot_cli.web_routers.mcp', 'mcp_oauth_callback'),
    'mcp_oauth_flow_status': ('anakot_cli.web_routers.mcp', 'mcp_oauth_flow_status'),
    'normalize_updated_at': ('gateway.status', 'normalize_updated_at'),
    'open_profile_terminal_endpoint': ('anakot_cli.web_routers.profiles', 'open_profile_terminal_endpoint'),
    'parse_active_agents': ('gateway.status', 'parse_active_agents'),
    'pause_cron_job': ('anakot_cli.web_routers.cron', 'pause_cron_job'),
    'poll_oauth_session': ('anakot_cli.web_routers.oauth', 'poll_oauth_session'),
    'post_agent_plugin_disable': ('anakot_cli.web_routers.dashboard_ui', 'post_agent_plugin_disable'),
    'post_agent_plugin_enable': ('anakot_cli.web_routers.dashboard_ui', 'post_agent_plugin_enable'),
    'post_agent_plugin_install': ('anakot_cli.web_routers.dashboard_ui', 'post_agent_plugin_install'),
    'post_agent_plugin_update': ('anakot_cli.web_routers.dashboard_ui', 'post_agent_plugin_update'),
    'post_plugin_visibility': ('anakot_cli.web_routers.dashboard_ui', 'post_plugin_visibility'),
    'preview_skill_hub': ('anakot_cli.web_routers.skills', 'preview_skill_hub'),
    'prune_checkpoints': ('anakot_cli.web_routers.ops', 'prune_checkpoints'),
    'prune_sessions_endpoint': ('anakot_cli.web_routers.sessions', 'prune_sessions_endpoint'),
    'pty_ws': ('anakot_cli.web_routers.chat_ws', 'pty_ws'),
    'pub_ws': ('anakot_cli.web_routers.chat_ws', 'pub_ws'),
    'put_plugin_providers': ('anakot_cli.web_routers.dashboard_ui', 'put_plugin_providers'),
    'read_managed_file': ('anakot_cli.web_routers.files', 'read_managed_file'),
    'read_raw_config': ('anakot_cli.config', 'read_raw_config'),
    'read_runtime_status': ('gateway.status', 'read_runtime_status'),
    'recommended_update_command_for_method': ('anakot_cli.config', 'recommended_update_command_for_method'),
    'redact_key': ('anakot_cli.config', 'redact_key'),
    'remove_credential_pool_entry': ('anakot_cli.web_routers.ops', 'remove_credential_pool_entry'),
    'remove_env_value': ('anakot_cli.config', 'remove_env_value'),
    'remove_env_var': ('anakot_cli.web_routers.config_env', 'remove_env_var'),
    'remove_mcp_server': ('anakot_cli.web_routers.mcp', 'remove_mcp_server'),
    'rename_profile_endpoint': ('anakot_cli.web_routers.profiles', 'rename_profile_endpoint'),
    'rename_session_endpoint': ('anakot_cli.web_routers.sessions', 'rename_session_endpoint'),
    'replace_mcp_servers': ('anakot_cli.web_routers.mcp', 'replace_mcp_servers'),
    'rescan_dashboard_plugins': ('anakot_cli.web_routers.dashboard_ui', 'rescan_dashboard_plugins'),
    'reset_memory': ('anakot_cli.web_routers.ops', 'reset_memory'),
    'resolve_cron_model_drift_defaults': ('anakot_cli.config', 'resolve_cron_model_drift_defaults'),
    'resolve_gateway_liveness': ('gateway.status', 'resolve_gateway_liveness'),
    'restart_gateway': ('anakot_cli.web_routers.actions', 'restart_gateway'),
    'resume_cron_job': ('anakot_cli.web_routers.cron', 'resume_cron_job'),
    'reveal_env_var': ('anakot_cli.web_routers.config_env', 'reveal_env_var'),
    'revoke_pairing': ('anakot_cli.web_routers.ops', 'revoke_pairing'),
    'run_backup': ('anakot_cli.web_routers.ops', 'run_backup'),
    'run_config_migrate': ('anakot_cli.web_routers.status', 'run_config_migrate'),
    'run_curator': ('anakot_cli.web_routers.status', 'run_curator'),
    'run_debug_share_endpoint': ('anakot_cli.web_routers.status', 'run_debug_share_endpoint'),
    'run_doctor': ('anakot_cli.doctor', 'run_doctor'),
    'run_dump': ('anakot_cli.dump', 'run_dump'),
    'run_import': ('anakot_cli.web_routers.ops', 'run_import'),
    'run_import_upload': ('anakot_cli.web_routers.ops', 'run_import_upload'),
    'run_prompt_size': ('anakot_cli.web_routers.status', 'run_prompt_size'),
    'run_security_audit': ('anakot_cli.web_routers.ops', 'run_security_audit'),
    'run_toolset_post_setup': ('anakot_cli.web_routers.tools', 'run_toolset_post_setup'),
    'save_config': ('anakot_cli.config', 'save_config'),
    'save_env_value': ('anakot_cli.config', 'save_env_value'),
    'save_toolset_env': ('anakot_cli.web_routers.tools', 'save_toolset_env'),
    'scan_skill_hub': ('anakot_cli.web_routers.skills', 'scan_skill_hub'),
    'search_sessions': ('anakot_cli.web_routers.sessions', 'search_sessions'),
    'search_skills_hub': ('anakot_cli.web_routers.skills', 'search_skills_hub'),
    'select_terminal_backend': ('anakot_cli.web_routers.tools', 'select_terminal_backend'),
    'select_toolset_model': ('anakot_cli.web_routers.tools', 'select_toolset_model'),
    'select_toolset_provider': ('anakot_cli.web_routers.tools', 'select_toolset_provider'),
    'serve_plugin_asset': ('anakot_cli.web_routers.dashboard_ui', 'serve_plugin_asset'),
    'set_active_profile_endpoint': ('anakot_cli.web_routers.profiles', 'set_active_profile_endpoint'),
    'set_curator_paused': ('anakot_cli.web_routers.status', 'set_curator_paused'),
    'set_dashboard_font': ('anakot_cli.web_routers.dashboard_ui', 'set_dashboard_font'),
    'set_dashboard_theme': ('anakot_cli.web_routers.dashboard_ui', 'set_dashboard_theme'),
    'set_env_var': ('anakot_cli.web_routers.config_env', 'set_env_var'),
    'set_mcp_server_enabled': ('anakot_cli.web_routers.mcp', 'set_mcp_server_enabled'),
    'set_memory_provider': ('anakot_cli.web_routers.ops', 'set_memory_provider'),
    'set_moa_models': ('anakot_cli.web_routers.models', 'set_moa_models'),
    'set_model_assignment': ('anakot_cli.web_routers.models', 'set_model_assignment'),
    'set_webhook_enabled': ('anakot_cli.web_routers.ops', 'set_webhook_enabled'),
    'setup_memory_provider': ('anakot_cli.web_routers.memory_providers', 'setup_memory_provider'),
    'speak_stream_ws': ('anakot_cli.web_routers.audio', 'speak_stream_ws'),
    'speak_text': ('anakot_cli.web_routers.audio', 'speak_text'),
    'start_gateway': ('anakot_cli.web_routers.ops', 'start_gateway'),
    'start_oauth_login': ('anakot_cli.web_routers.oauth', 'start_oauth_login'),
    'start_telegram_onboarding': ('anakot_cli.web_routers.messaging', 'start_telegram_onboarding'),
    'start_whatsapp_onboarding': ('anakot_cli.web_routers.messaging', 'start_whatsapp_onboarding'),
    'stop_gateway': ('anakot_cli.web_routers.ops', 'stop_gateway'),
    'stream_managed_file': ('anakot_cli.web_routers.files', 'stream_managed_file'),
    'submit_oauth_code': ('anakot_cli.web_routers.oauth', 'submit_oauth_code'),
    'test_mcp_server': ('anakot_cli.web_routers.mcp', 'test_mcp_server'),
    'test_messaging_platform': ('anakot_cli.web_routers.messaging', 'test_messaging_platform'),
    'toggle_skill': ('anakot_cli.web_routers.skills', 'toggle_skill'),
    'toggle_toolset': ('anakot_cli.web_routers.tools', 'toggle_toolset'),
    'transcribe_audio_upload': ('anakot_cli.web_routers.audio', 'transcribe_audio_upload'),
    'trigger_cron_job': ('anakot_cli.web_routers.cron', 'trigger_cron_job'),
    'tts_lease': ('anakot_cli.web_routers.audio', 'tts_lease'),
    'uninstall_skill_hub': ('anakot_cli.web_routers.skills', 'uninstall_skill_hub'),
    'update_config': ('anakot_cli.web_routers.config_env', 'update_config'),
    'update_config_raw': ('anakot_cli.web_routers.analytics', 'update_config_raw'),
    'update_cron_job': ('anakot_cli.web_routers.cron', 'update_cron_job'),
    'update_anakot': ('anakot_cli.web_routers.actions', 'update_anakot'),
    'update_learning_node': ('anakot_cli.web_routers.status', 'update_learning_node'),
    'update_memory_provider_config': ('anakot_cli.web_routers.memory_providers', 'update_memory_provider_config'),
    'update_messaging_platform': ('anakot_cli.web_routers.messaging', 'update_messaging_platform'),
    'update_profile_description_endpoint': ('anakot_cli.web_routers.profiles', 'update_profile_description_endpoint'),
    'update_profile_model_endpoint': ('anakot_cli.web_routers.profiles', 'update_profile_model_endpoint'),
    'update_profile_soul': ('anakot_cli.web_routers.profiles', 'update_profile_soul'),
    'update_skill_content': ('anakot_cli.web_routers.skills', 'update_skill_content'),
    'update_skills_hub': ('anakot_cli.web_routers.skills', 'update_skills_hub'),
    'upload_chat_image': ('anakot_cli.web_routers.files', 'upload_chat_image'),
    'upload_managed_file': ('anakot_cli.web_routers.files', 'upload_managed_file'),
    'upload_managed_file_stream': ('anakot_cli.web_routers.files', 'upload_managed_file_stream'),
    'upsert_custom_endpoint': ('anakot_cli.web_routers.config_env', 'upsert_custom_endpoint'),
    'validate_custom_endpoint': ('anakot_cli.web_routers.config_env', 'validate_custom_endpoint'),
    'validate_provider_credential': ('anakot_cli.web_routers.config_env', 'validate_provider_credential'),
    'windows_detach_flags': ('anakot_cli._subprocess_compat', 'windows_detach_flags'),
    'windows_hide_flags': ('anakot_cli._subprocess_compat', 'windows_hide_flags'),
    'write_platform_config_field': ('anakot_cli.config', 'write_platform_config_field'),
}
...
def __getattr__(name):  # PEP 562 — lazy so no import cycles
    ...
    import importlib
    from anakot_cli.plugin_compat import warn_once
