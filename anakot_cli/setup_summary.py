...
resolved through the module object so test patches on ``anakot_cli.setup.<name>`` take effect."""

import logging
from tools import tool_backend_helpers
from anakot_cli import nous_subscription
...
_TTS_SUMMARY_ROWS = {
    ...
    "neutts": ("NeuTTS", "neutts", "run 'anakot setup tts'"),
    "kittentts": ("KittenTTS", "kittentts", "run 'anakot setup tts'")}
...
_STT_SUMMARY_DEFAULT = ("Local Whisper", "faster_whisper", "run 'anakot tools' → Speech-to-Text")
...
_EDIT_WIZARD_ROWS = (
    ("anakot setup", "          Re-run the full wizard"), ("anakot setup model", "    Change model/provider"),
    ("anakot setup terminal", " Change terminal backend"), ("anakot setup gateway", "  Configure messaging"),
    ("anakot setup tools", "    Configure tool providers"))
_EDIT_CONFIG_ROWS = (
    ("anakot config", "         View current settings"), ("anakot config edit", "    Open config in your editor"),
    ("anakot config set <key> <value>", ""))
_READY_ROWS = (
    ("anakot", "              Start chatting"), ("anakot gateway", "      Start messaging gateway"),
    ("anakot doctor", "       Check for issues"))
...
def _vision_row(config, feats):
    ...
    try:
        ...
        ok = False
    return ("Vision (image analysis)", ok, None if ok else "run 'anakot setup' to configure")
...
def _video_gen_row(config, feats):
    # Opt-in via `anakot tools` → Video Generation. Only show the row when a plugin reports
...
def _modal_row(config, feats):
    ...
    if _setup.cfg_get(config, "terminal", "backend") == "modal":
        ...
        return ("Modal Execution", False, "run 'anakot setup terminal'")
...
def _spotify_row(config, feats):
    # OAuth via anakot auth spotify — check auth.json, not env vars
    try:
...
def _print_setup_summary(config: dict, anakot_home):
    ...
    try:
        ...
        print()
        _setup.print_warning("No inference provider is configured — Anakot cannot chat yet.")
        _setup._info("  Finish this one step with either of:",
              "    anakot model            (pick any provider/model)",
              "    anakot setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if available_count < len(tool_status):
        _setup.print_warning("Some tools are disabled. Run 'anakot setup tools' to configure them,")
    ...
    for label, value in (("Settings:", f"  {_setup.get_config_path()}"), ("API Keys:", f"  {_setup.get_env_path()}"),
                         ("Data:", f"      {anakot_home}/cron/, sessions/, logs/")):
...
import anakot_cli.setup as _setup  # noqa: E402  (bottom: anakot_cli.setup imports this module)
