...
Split out of :mod:`anakot_cli.plugins`; validation warns and never fails a load.
"""
...
from anakot_cli.plugin_capabilities import parse_declared_capabilities as _parse_declared_capabilities
...
# and continue loading. ``capabilities``/``emits``/``listens``/``anakot``/``depends`` are reserved.
...
_KNOWN_MANIFEST_FIELDS: Set[str] = {
    ...
    "license", "homepage", "tags", "capabilities", "emits", "listens", "anakot", "depends",
    "requires_anakot", "python_runtime",
}

# Highest manifest schema version this Anakot understands.
...
def _plugins_debug() -> bool:
    from anakot_cli import plugins as _origin
...
def _parse_manifest_v2_fields(data: Mapping, key: str) -> Dict[str, Any]:
    ...
    if mv > SUPPORTED_MANIFEST_VERSION:
        logger.warning(
            "Plugin %s: manifest_version %d is newer than this Anakot "
        ...
        for dep in manifests[k].requires_plugins:
            ...
            if resolved is None:
                logger.warning(
                    ...
                    "via ctx.has_plugin). Run `anakot plugins enable %s` if it is installed.",
                    k, dep_id, dep_id,
                )
            elif resolved == k:
...
class PluginManifest:
    ...
    kind: str = "standalone"
    # Path-derived registry key used by plugins.enabled/disabled and `anakot plugins list`: ``disk-cleanup``
    # for a flat plugin, ``image_gen/openai`` for a category plugin. Empty -> name.
    key: str = ""
    # Anakot version requirement (``">=0.19"``, comma-separated clauses allowed). Unsatisfied plugins are
    # recorded with an error and skipped before import — see ``requires_anakot_error``.
    requires_anakot: str = ""
    portable: bool = False
    skill_namespace: str = ""
    ...
    # VALIDATED AND SURFACED ONLY — Anakot never auto-installs these (isolation design for the install seam
...
# ── requires_anakot version gate ─────────────────────────────────────────────
_VERSION_COMPARATOR_RE = re.compile(r"^\s*(>=|<=|==|!=|>|<)\s*(.+?)\s*$")


def running_anakot_version() -> str:
    """Installed ``anakot-agent`` distribution version, else ``anakot_cli.__version__`` (source checkout)."""
    try:
        return importlib.metadata.version("anakot-agent")
    except Exception:
...
def requires_anakot_error(manifest: "PluginManifest") -> Optional[str]:
    """Load-blocking reason when the manifest's ``requires_anakot`` rejects the running version."""
    if not manifest.requires_anakot:
        return None
    current = running_anakot_version()
    if version_satisfies(manifest.requires_anakot, current):
        return None
    return f"requires anakot {manifest.requires_anakot}, running {current}"
...
def parse_manifest_file(
    ...
    try:
        ...
        return PluginManifest(
            ...
            kind=kind, key=key, requires_anakot=str(data.get("requires_anakot") or "").strip(),
