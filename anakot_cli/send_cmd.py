"""CLI subcommand: ``anakot send`` — pipe text from shell scripts to any configured messaging platform
...
def _read_message_body(positional: Optional[str], file_path: Optional[str]) -> Optional[str]:
    ...
    if file_path:
        ...
        try:
            ...
            _fail(
                f"anakot send: {file_path} is not a text file. --file reads the "
                ...
                f'  anakot send --to telegram "MEDIA:{file_path}"\n'
                f'  anakot send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  anakot send --to telegram "[[as_document]] MEDIA:{file_path}"',
                _USAGE_EXIT)
        except OSError as exc:
            _fail(f"anakot send: cannot read {file_path}: {exc}", _USAGE_EXIT)
...
def _emit_result(result_json: str, *, json_mode: bool, quiet: bool) -> int:
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"anakot send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        return _fail(f"anakot send: failed to load channel directory: {exc}")
    try:
        raw = load_directory()
    except Exception as exc:
        return _fail(f"anakot send: failed to read channel directory: {exc}")
    ...
    if platform_filter:
        ...
        if not filtered:
            return _fail(
                f"anakot send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `anakot gateway setup`, or run the gateway once so")
        from anakot_constants import get_default_anakot_root, get_anakot_home, anakot_home_key
        home, root = get_anakot_home(), get_default_anakot_root()
        ...
        if anakot_home_key(root) != anakot_home_key(home) and (root / "channel_directory.json").exists():
...
def _load_anakot_env() -> None:
    """Populate the credential environment from ``<ANAKOT_HOME>/.env`` AND bridge top-level ``config.yaml``
    ...
    if isinstance(scope, dict):
        ...
        if env_path.exists():
            try:
                from anakot_cli.env_loader import _load_dotenv_with_fallback
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    _load_anakot_env()  # the downstream gateway config loader reads credentials from os.environ
    if getattr(args, "list_targets", False):  # --list short-circuits everything else
        # `anakot send --list telegram` lands "telegram" in the `message` positional.
        exit_code = _list_targets(getattr(args, "message", None), json_mode=getattr(args, "json", False))
        sys.exit(exit_code)
    target = (getattr(args, "to", None) or "").strip()
    if not target:
        _fail(
            "anakot send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  anakot send --to telegram \"hello\"\n"
            "  anakot send --to discord:#ops --file report.md\n"
            "  anakot send --list      # list available targets",
            _USAGE_EXIT)
    mentions = list(getattr(args, "mentions", None) or [])
    if mentions and target.split(":", 1)[0].strip().lower() != "whatsapp":
        _fail("anakot send: --mention is only supported for WhatsApp targets.", _USAGE_EXIT)
    invalid_mentions = _invalid_whatsapp_mentions(mentions)
    if invalid_mentions:
        _fail(
            "anakot send: invalid --mention value(s): "
    ...
    if message is None or not message.strip():
        _fail(
            "anakot send: no message provided. Pass text as a positional "
    ...
    # Lazy import keeps `anakot send --help` fast (no tool registry / gateway config stack).
...
_SEND_ARGUMENTS = (
    ...
    (("-l", "--list"), dict(dest="list_targets", action="store_true", default=False,
                            help="List available targets. Optional positional filter: `anakot send --list telegram`.")),
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Anakot "
            "is already configured for. Reuses the gateway's platform "
            f"credentials ({anakot_home / '.env'} + "
            f"{anakot_home / 'config.yaml'}) — no LLM, "
        ...
        epilog=(
            ...
            "  anakot send --to whatsapp:GROUP@g.us --mention 15551234567 \"@15551234567 hello\"\n"
            "  anakot send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
