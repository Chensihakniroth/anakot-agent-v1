...
The decomposition (PR #102117) moved most of Anakot's internals into ``<stem>_<topic>`` sibling modules.
...
Surfaces that read from here: the CLI banner, ``anakot plugins compat``, ``anakot doctor``, the post-update
...
class AnakotPluginCompatWarning(FutureWarning):
...
def report_file_path() -> Path:
    ...
    return get_anakot_home() / REPORT_FILE
...
def disable_reason(manifest, *, today: Optional[_dt.date] = None) -> Optional[str]:
    ...
    if not hits:
        return None
    return (f"uses {len(hits)} import path(s) removed on {COMPAT_REMOVAL}; run `anakot plugins compat` "
...
def summary_lines(report: Dict[str, List[Hit]], *, today: Optional[_dt.date] = None) -> List[str]:
    ...
    if removal_in_effect(today) and allow_deprecated_imports():
        ...
        tail = "Update the plugin(s); the old paths no longer exist. Details: anakot plugins compat"
    elif removal_in_effect(today):
        head = (f"{n} plugin{'s' if n != 1 else ''} DISABLED: they import paths removed on {COMPAT_REMOVAL}: {names}")
        tail = f"Update the plugin(s) or set plugins.{ALLOW_KEY}: true to force-load. Details: anakot plugins compat"
    else:
        ...
        tail = "Check for plugin updates or notify the author before then. Details: anakot plugins compat"
    return [head, tail]
...
def warn_once(facade: str, name: str, target_module: str, target_name: str) -> None:
    """Per-name record that a moved name was resolved through its old path: a ``AnakotPluginCompatWarning``
    ...
    msg = (f"anakot plugin compat: `{facade}.{name}` moved to `{new}`. The old path is kept only for external "
           f"plugins and is removed on {COMPAT_REMOVAL}; update your import.")
    _log.warning(msg)
    warnings.warn(msg, AnakotPluginCompatWarning, stacklevel=3)
...
def quiet_for_interactive() -> None:
    ...
    Appends (does not override) so an explicit ``-W error::...AnakotPluginCompatWarning`` still wins."""
    if not any(a == "error" and c is not None and issubclass(AnakotPluginCompatWarning, c)
               for a, _m, c, _mod, _l in warnings.filters):
        warnings.filterwarnings("ignore", category=AnakotPluginCompatWarning, append=True)
