"""Persistent session goals — the Ralph loop for Anakot.
...
# (schema DDL, FTS tables, first anakot_cli.config import) measures ~300ms warm and more on slow
...
def _get_session_db() -> Optional[Any]:
    """Cached SessionDB per ANAKOT_HOME (profile switches pick the right DB); None on any failure.
    ...
    if cached is not None and _registry_tore_down(cached):
        # ``anakot profile delete`` force-closes every handle under the profile home
        # (``anakot_state_registry.close_all_under``) before rmtree; a same-name recreate in this
