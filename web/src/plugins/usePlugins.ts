...
import { api, ANAKOT_BASE_PATH } from "@/lib/api";
...
export const MANIFEST_CACHE_KEY = "anakot:plugin-manifests";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${ANAKOT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${ANAKOT_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?anakot_dv=${Date.now()}`
        : baseUrl;
