...
// (e.g. https://mission-control.tilos.com/anakot/). The Python backend
// injects ``window.__ANAKOT_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__ANAKOT_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const ANAKOT_BASE_PATH = readBasePath();
const BASE = ANAKOT_BASE_PATH;
...
declare global {
  interface Window {
    __ANAKOT_SESSION_TOKEN__?: string;
    __ANAKOT_BASE_PATH__?: string;
    ...
    __ANAKOT_AUTH_REQUIRED__?: boolean;
  }
}
...
export async function fetchJSON<T>(
  ...
  const token = window.__ANAKOT_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``anakot update``, ``anakot gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__ANAKOT_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__ANAKOT_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
  ...
  if (res.ok) {
    ...
    // current ``window.__ANAKOT_SESSION_TOKEN__`` is valid, so the next
 ...
 *
 * The dashboard's gated-mode WS auth (``anakot_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__ANAKOT_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Anakot-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``anakot_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__ANAKOT_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export async function buildWsUrl(
  ...
  return buildAnakotWebSocketUrl({
...
export const api = {
  ...
  updateAnakot: () =>
    fetchJSON<ActionResponse>("/api/anakot/update", { method: "POST" }),
  checkAnakotUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/anakot/update/check${force ? "?force=true" : ""}`,
    ),
...
/** Preflight for `anakot gateway migrate --multiplex` (mirrors the CLI plan JSON). */
...
export interface SkillHubSource {
  ...
  /** anakot-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``anakot update``. */
  can_update_anakot?: boolean;
  config_path: string;
  ...
  /** NS-656: disk-usage rollup for the ANAKOT_HOME volume. Absent on
   * older gateways. */
 ...
 * sample of the ANAKOT_HOME volume — no staleness dimension, so no
 * sampled_at. */
