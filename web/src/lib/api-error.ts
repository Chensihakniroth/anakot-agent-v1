...
export const API_UNREACHABLE_MESSAGE =
  "Anakot dashboard cannot reach the Anakot service. Is `anakot dashboard` still running?";
...
const STATUS_COPY: Record<number, string> = {
  ...
  500: "The Anakot service hit an internal error.",
  502: "The dashboard proxy could not reach the Anakot service.",
  503: "The Anakot service is not ready yet. Try again in a moment.",
  504: "The Anakot service took too long to respond.",
};

export function humanizeStatus(status: number): string {
  return STATUS_COPY[status] ?? `The Anakot service answered with an unexpected error (${status}).`;
}
