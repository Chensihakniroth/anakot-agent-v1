...
const SIDEBAR_COLLAPSED_KEY = "anakot-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="anakot-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="anakot-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateAnakot = status?.can_update_anakot === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'anakot update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "anakot update";
    return (
      t.status.updateAnakotConfirmMessage ??
      `This will run 'anakot update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateAnakotConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateAnakot) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateAnakot,
      runningLabel: t.status.updatingAnakot,
      spin: false,
    });
  }
  ...
  return (
      ...
      description={
        sharedGateway
          ...
          : (t.status.restartGatewayConfirmMessage ??
            "This restarts the Anakot gateway process. Connected channels and active sessions will reconnect afterward.")
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateAnakotConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateAnakotConfirmTitle ?? `${t.status.updateAnakot}?`}
    />
    </>
  );
}
