...
const BACKEND = process.env.ANAKOT_DASHBOARD_URL ?? "http://127.0.0.1:9119";

/**
 * In production the Python `anakot dashboard` server injects a one-shot
 * session token into `index.html` (see `anakot_cli/web_server.py`). The
...
function anakotDevToken(): Plugin {
  ...
  const EMBEDDED_RE =
    /window\.__ANAKOT_DASHBOARD_EMBEDDED_CHAT__\s*=\s*(true|false)/;
  const INITIAL_PROFILE_RE =
    /window\.__ANAKOT_INITIAL_PROFILE__\s*=\s*("(?:\\.|[^"\\])*")/;
  ...
  return {
    ...
    async transformIndexHtml() {
      try {
        ...
        if (!match) {
          console.warn(
            `[anakot] Could not find session token in ${BACKEND} — ` +
        ...
        return [
          {
            ...
            injectTo: "head",
            children:
              `window.__ANAKOT_SESSION_TOKEN__="${match[1]}";` +
              `window.__ANAKOT_DASHBOARD_EMBEDDED_CHAT__=${embeddedJs};` +
              `window.__ANAKOT_INITIAL_PROFILE__=${initialProfileJs};`,
          },
        ];
      } catch (err) {
        console.warn(
          `[anakot] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`anakot dashboard\` or set ANAKOT_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `anakot dashboard` must serve these; Vite has no
