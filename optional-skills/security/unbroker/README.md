...
The design is **Anakot-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
anakot skills install official/security/unbroker
```

Then start a new Anakot session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Anakot session:
...
- **PII handling.** Dossiers live under the Anakot home directory (`0600`, optionally
