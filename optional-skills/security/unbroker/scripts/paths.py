...
All per-subject data lives under PDD_DATA_DIR (default: $ANAKOT_HOME/unbroker),
which is the same trust boundary Anakot uses for .env and OAuth tokens.
"""
...
def anakot_home() -> Path:
    return Path(os.environ.get("ANAKOT_HOME") or (Path.home() / ".anakot"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else anakot_home() / "unbroker"
