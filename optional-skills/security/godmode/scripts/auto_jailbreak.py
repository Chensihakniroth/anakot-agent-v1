...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("ANAKOT_HOME", os.path.expanduser("~/.anakot")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("ANAKOT_HOME", Path.home() / ".anakot")) / "skills" / "red-teaming" / "godmode"
...
ANAKOT_HOME = Path(os.getenv("ANAKOT_HOME", Path.home() / ".anakot"))
CONFIG_PATH = ANAKOT_HOME / "config.yaml"
PREFILL_PATH = ANAKOT_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Anakot models — already uncensored, just needs clean prompt
    "anakot": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "anakot" in model_lower or "nous" in model_lower:
        return "anakot"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Anakot config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.anakot/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Anakot for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Anakot for changes to take effect.")
