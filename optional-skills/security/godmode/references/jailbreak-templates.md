...
## 5. GODMODE FAST — Anakot 4 405B (Zero Refusal)

**Model:** `nousresearch/anakot-4-405b`
**Strategy:** Anakot models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Anakot Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.anakot/config.yaml`:
