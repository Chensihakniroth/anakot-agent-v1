#!/usr/bin/env python3
"""Canvas LMS API CLI for Anakot Agent.
...
def _check_config():
    ...
    if missing:
        anakot_env = os.path.join(
            os.environ.get("ANAKOT_HOME", os.path.expanduser("~/.anakot")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {anakot_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Anakot Agent"
    )
