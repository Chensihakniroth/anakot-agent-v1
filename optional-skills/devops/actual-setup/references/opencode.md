...
is typically only in `~/.anakot/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Anakot,
...
`anakot-custom-providers` skill, pitfall 2). Anakot hits this because it forwards
