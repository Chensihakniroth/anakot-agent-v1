...
Usage (via cron with --no-agent):
    ...
    anakot cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$ANAKOT_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Anakot-Watcher/1.0"})
