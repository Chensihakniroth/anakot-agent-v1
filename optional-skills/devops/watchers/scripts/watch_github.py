...
Usage (via cron with --no-agent):
    ...
    anakot cron create anakot-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$ANAKOT_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name anakot-issues --repo Chensihakniroth/anakot-agent-v1 --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Anakot .env file
(``${ANAKOT_HOME:-~/.anakot}/.env``) to avoid the 60 req/hr
