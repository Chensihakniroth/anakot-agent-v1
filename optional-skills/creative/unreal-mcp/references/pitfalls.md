...
### 1. Start order: editor first, Anakot second

Anakot probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Anakot session.
...
restart the editor, restart the Anakot session.
...
server fails to bind (Output Log) or Anakot' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.anakot/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Anakot

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Anakot' connection
lives in `~/.anakot/config.yaml` via `anakot mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Anakot — just don't
mistake it for the Anakot setup step.
...
explicitly warns against overlapping calls. Anakot executes same-turn tool
...
### 9. Timeouts: Anakot gives up before Unreal does

Anakot' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Anakot runs elsewhere (SSH backend,
