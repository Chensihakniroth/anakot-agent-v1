...
the actual `setup.sh` that creates the project workspace, configures Anakot
...
2. **Create profiles** — `anakot profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.anakot/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `anakot kanban create` assigned to the director
...
```bash
anakot profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.anakot/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.anakot/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Anakot
...
Each profile gets a `SOUL.md` at `~/.anakot/profiles/<name>/SOUL.md` that
...
```bash
anakot kanban create "Direct production of <video title>" \
    --assignee director \
...
the Anakot `.env` (`${ANAKOT_HOME:-$HOME/.anakot}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _anakot_env="${ANAKOT_HOME:-$HOME/.anakot}/.env"
    if grep -q "^${var}=" "$_anakot_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_anakot_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_anakot_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY anakot ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY anakot OPENROUTER_API_KEY || exit 1
# ...
```
