...
anakot kanban watch --tenant <project-slug>

# Snapshot of the board
anakot kanban list --tenant <project-slug>
anakot kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
anakot kanban stats --tenant <project-slug>

# Visual dashboard (browser)
anakot dashboard

# Inspect a specific task (includes comments + events)
anakot kanban show <task-id>

# Follow a single task's event stream
anakot kanban tail <task-id>
```

Verify available subcommands with `anakot kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `anakot kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `anakot kanban block <id>` then `anakot kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `anakot kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `anakot kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `anakot profile list` (profile exists?), `anakot status` (gateway/dispatcher up?), and verify tenant. |
...
anakot kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
anakot kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(anakot kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
anakot kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
anakot kanban block <task-id>
anakot kanban archive <task-id>

# Diagnose what happened
anakot kanban show <task-id>      # task body, comments, recent events
anakot kanban tail <task-id>      # follow the live event stream
anakot kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `anakot kanban create` for the director
...
while true; do
    clear
    anakot kanban list --tenant <slug>
    echo "---"
    anakot kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `anakot kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `anakot profile list` and start
