...
Maps each role archetype to the Anakot skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public anakot-agent
...
## Anakot skills relevant to video production

### Visual / rendering skills (`anakot-agent/skills/creative/`)
...
| `touchdesigner-mcp` (via the `touchdesigner` plugin: `anakot plugins install touchdesigner`) | Control a running TouchDesigner instance — real-time visuals, audio-reactive installation art, VJ | Renderer for real-time/audio-reactive content; installation art; live performance |
...
### Design / pre-production skills (`anakot-agent/skills/creative/`)
...
### Audio / media skills (`anakot-agent/skills/creative/` + `skills/media/`)
...
These are **not** Anakot skills but external CLIs / APIs that profiles invoke.
...
## Built-in Anakot tools for media review

These are native Anakot tools — not invoked via terminal but through their own
...
key is present in `${ANAKOT_HOME:-~/.anakot}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Anakot profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${ANAKOT_HOME:-~/.anakot}/.env` → environment variable.
...
When a new Anakot-public video skill ships:
