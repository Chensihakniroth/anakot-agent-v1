#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `anakot kanban list` and
...
def anakot_available() -> bool:
    return shutil.which("anakot") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed anakot CLI."""
    try:
        out = subprocess.run(
            ["anakot", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `anakot kanban list`
    out = subprocess.run(
        ["anakot", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["anakot", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, encoding='utf-8', errors='replace', check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not anakot_available():
        print("ERROR: 'anakot' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)
