...
Usage:
  ...
  <server_name> matches the files in $ANAKOT_HOME/mcp-tokens/<server>.json etc.
...
def _anakot_home():
    # Prefer Anakot' own resolver (profile-safe); fall back to env then ~/.anakot.
    try:
        from anakot_constants import get_anakot_home
        return str(get_anakot_home())
    except Exception:
        return os.environ.get("ANAKOT_HOME") or os.path.expanduser("~/.anakot")


def _tokens_dir():
    return os.path.join(_anakot_home(), "mcp-tokens")
    ...
    status, hdrs, body = _post(
        ...
        data={
            ...
            "params": {"protocolVersion": "2025-06-18", "capabilities": {},
                       "clientInfo": {"name": "anakot-diag", "version": "1.0"}},
        },
        headers={
