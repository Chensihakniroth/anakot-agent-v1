...
Anakot Agent (see website/docs/getting-started/platform-support.md). The
...
build sandbox. ``nix/python.nix`` sets ``ANAKOT_NIX_BUILD=1`` on the
Anakot package derivation, so only that build may create an artifact.
...
_IN_NIX_BUILD = os.environ.get("ANAKOT_NIX_BUILD") == "1"
...
_BLOCK_MESSAGE = (
    ...
    "the Anakot Nix derivation sets ANAKOT_NIX_BUILD=1. If it does, file a bug."
)
...
# Root single-file modules (``run_agent``, ``anakot_state``, ``toolsets``...)
