...
an unlinked SQLite DB/WAL/SHM generation.  ``anakot_state`` supplies only the
...
_ANAKOT_EXECUTABLES = frozenset({"anakot", "anakot-agent", "anakot-acp"})
_ANAKOT_PYTHON_MODULES = frozenset({"acp_adapter", "anakot_cli.main"})
_ANAKOT_PYTHON_SCRIPTS = frozenset({"anakot_cli/main.py", "run_agent.py"})
...
def _looks_like_anakot(argv: Sequence[str]) -> bool:
    """Return whether argv identifies a supported Anakot execution target."""
    if not argv:
        return False
    program = os.path.basename(argv[0]).lower().removesuffix(".exe")
    if program in _ANAKOT_EXECUTABLES:
        return True
    ...
    if kind == "module":
        return normalized in _ANAKOT_PYTHON_MODULES
    return any(
        normalized == script or normalized.endswith(f"/{script}")
        for script in _ANAKOT_PYTHON_SCRIPTS
    )
...
def _argv_scoped_to_other_home(argv: Sequence[str], db_path: Path) -> bool:
    ...
    ``state.db`` lives at the ANAKOT_HOME root, so an absolute-path token
    containing a ``/.anakot`` segment (or naming a ``state.db``/WAL/SHM under
    some other parent) identifies that token's own Anakot home.  When at least
    ...
    of ours (issue #92401: a second gateway under /home/demo/.anakot deferred
    ...
    for token in argv:
        ...
        if path_token is not None:
            ...
            if normalized in ours or normalized.startswith(this_home + os.sep):
                return False
            if "/.anakot" in normalized or normalized.endswith("/.anakot"):
...
def foreign_state_db_holders(db_path: Path) -> List[Tuple[int, str]]:
    ...
    # ANAKOT_HOME would otherwise make every holder invisible and let maintenance proceed.
