"""ACP permission bridging for Anakot dangerous-command approvals."""
...
# ACP permission option id -> Anakot approval result. Ids are stable across the
# ``allow_permanent=True`` and ``False`` paths even though the option list differs.
_OPTION_ID_TO_ANAKOT = {
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True, smart_denied: bool = False,
    ...
    """Return ACP options that match Anakot approval semantics."""
    ...
    # DENY override — offering a scope Anakot discards would re-prompt every write.
    # See #81887.
    once_only = smart_denied or not allow_session
    options = [PermissionOption(option_id="allow_once", kind="allow_once", name="Allow once")]
    if not once_only:
        # ACP has no session-scoped kind: closest persistent hint, Anakot semantics in the id.
...
def _map_outcome_to_anakot(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Anakot approval strings."""
    ...
    if outcome.option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_ANAKOT.get(outcome.option_id, "deny")
...
def resolve_permission_timeout(timeout: float | None) -> float:
    ...
    waiting saw Anakot self-deny under it (#73403)."""
...
def make_approval_callback(request_permission_fn: Callable, loop: asyncio.AbstractEventLoop,
    ...
    """Return a Anakot approval callback (``command, description, **kw`` as used by
    ...
    def _callback(command: str, description: str, *, allow_permanent: bool = True,
        ...
        if response is None:
            return "deny"
        return _map_outcome_to_anakot(response.outcome, allowed_option_ids={option.option_id for option in options})
