"""ACP agent server — exposes Anakot Agent via the Agent Client Protocol."""
...
from acp_adapter.commands import ANAKOT_VERSION, SlashCommandsMixin, _estimate_tokens
...
from tools.approval_context import reset_anakot_interactive_context, set_anakot_interactive_context
...
def _history_summary_meta(message: dict[str, Any], text: str) -> dict[str, Any] | None:
    ...
    if kind is None and message.get(COMPRESSED_SUMMARY_METADATA_KEY):
        ...
        kind = "standalone"
    if kind == "standalone":
        return {"anakot": {"compactionSummary": True}}
    if kind == "merged":
        return {"anakot": {"containsCompactionSummary": True}}
    return None
...
class AnakotACPAgent(SlashCommandsMixin, acp.Agent):
    """ACP Agent implementation wrapping Anakot AIAgent."""
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        """Authenticated providers + models, from the shared Anakot inventory (same substrate
        as ``anakot model``/TUI/dashboard) so the selector isn't just the current curated list."""
    ...
    def _switch_model(
        ...
        Resolution goes through ``anakot_cli.model_switch.switch_model`` seeded with the live
        ...
        from anakot_cli.config import get_compatible_custom_providers, load_config
    ...
    def _provenance_meta(
        self, acp_session_id: str, current_anakot_session_id: str, previous_anakot_session_id: Optional[str] = None
    ) -> Optional[dict]:
        """Best-effort ``_meta.anakot.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(), acp_session_id, current_anakot_session_id,
                previous_anakot_session_id=previous_anakot_session_id,
            )
        except Exception:
    ...
    async def _send_session_info_update(
        self, session_id: str, *,
        current_anakot_session_id: Optional[str] = None, previous_anakot_session_id: Optional[str] = None,
    ) -> None:
        """Session metadata update; pass ``previous_anakot_session_id`` when the internal head
        ...
        update = SessionInfoUpdate(
            ...
            field_meta=self._provenance_meta(
                session_id, current_anakot_session_id or session_id, previous_anakot_session_id
            ),
        )
        await self._send(
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            agent.enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(agent, "enabled_toolsets", None) or ["anakot-acp"],
    ...
    def _schedule_mcp_late_refresh(self, state: SessionState) -> None:
        ...
        try:
            from anakot_cli.mcp_startup import mcp_discovery_in_flight
        except Exception:
            return
        ...
        def _wait_then_refresh() -> None:
            try:
                from anakot_cli.mcp_startup import join_mcp_discovery
    ...
    async def initialize(
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="anakot-agent", version=ANAKOT_VERSION),
    ...
    def _run_agent_turn(
        ...
        Interactive routing is a ``tools.approval`` contextvar, not ``ANAKOT_INTERACTIVE`` in
        ...
        with contextlib.ExitStack() as stack:
            # ANAKOT_SESSION_KEY scopes per-session caches (interactive sudo password) to this
            # session, not the reused thread. ``cwd`` pins what the system prompt reports as the
            # working directory — otherwise it advertises the Anakot workspace while tools are
            # rooted at the client's project and edits land outside it. ``cron_session=""`` masks
            # any leaked process-global ANAKOT_CRON_SESSION.
            ...
            stack.callback(reset_anakot_interactive_context, set_anakot_interactive_context(True))
            # Tools tag side-effects with the ACP session (``kanban_create``); save/restore it.
            stack.callback(_restore_env, "ANAKOT_SESSION_ID", os.environ.get("ANAKOT_SESSION_ID"))
    ...
    async def prompt(self, prompt: list[PromptBlock], session_id: str, **kwargs: Any) -> PromptResponse:
        """Run Anakot on the user's prompt and stream events back to the editor."""
        state = await asyncio.to_thread(self.session_manager.get_session, session_id)
        if state is None:
        ...
        try:
            ...
            pre_turn_anakot_id = getattr(state.agent, "session_id", None)
        ...
        return await self._finish_turn(state, session_id, conn, result, pre_turn_anakot_id, cbs.streamed)
    ...
    async def _finish_turn(
        self, state: SessionState, session_id: str, conn: Any, result: dict, pre_turn_anakot_id: Any,
        ...
        post_turn_anakot_id = getattr(state.agent, "session_id", None)
        if conn and post_turn_anakot_id and pre_turn_anakot_id and post_turn_anakot_id != pre_turn_anakot_id:
            try:
                await self._send_session_info_update(
                    session_id, current_anakot_session_id=post_turn_anakot_id,
                    previous_anakot_session_id=pre_turn_anakot_id,
                )
            except Exception:
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Anakot has no typed ACP config surface yet."""
        state = await asyncio.to_thread(self.session_manager.get_session, session_id)
        if state is None:
