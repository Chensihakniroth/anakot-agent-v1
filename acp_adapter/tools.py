"""ACP tool-call helpers for mapping anakot tools to ACP ToolKind and building content."""
...
# Anakot tool name -> ACP ToolKind (anything unlisted is "other").
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a anakot tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    Some Anakot tools append a human hint after the payload (``{...}\\n\\n[Hint: ...]``),
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Anakot tool result clearly failed.
...
def build_tool_title(tool_name: str, args: Args) -> str:
    ...
    every other Anakot surface, so ACP clients never show a different summary than the CLI/TUI;
...
def build_tool_start(tool_call_id: str, tool_name: str, arguments: Args, *, edit_diff: Any = None) -> ToolCallStart:
    """Create a ToolCallStart event for the given anakot tool invocation.
