"""ACP auth helpers — detect and advertise Anakot authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "anakot-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Anakot runtime provider, or None if unavailable.
...
def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Anakot.

    The ACP registry requires at least one usable auth method in the initial
    handshake. A fresh Zed install may have no Anakot credentials yet, so the
    ...
    if provider:
        methods.append(AuthMethodAgent(
            ...
            description=f"Authenticate Anakot using the currently configured {provider} runtime credentials.",
        ))
    methods.append(TerminalAuthMethod(
        id=TERMINAL_SETUP_AUTH_METHOD_ID, name="Configure Anakot provider", type="terminal", args=["--setup"],
        description=("Open Anakot' interactive model/provider setup in a terminal. "
                     "Use this when Anakot has not been configured on this machine yet."),
    ))
    return methods
...
def has_provider() -> bool:
    """Return True if Anakot can resolve any runtime provider credentials."""
