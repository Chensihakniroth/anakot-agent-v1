# Anakot Agent Security Policy

This document describes Anakot Agent's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/Chensihakniroth/anakot-agent-v1/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Anakot Agent does not operate a bug
bounty program.**
...
- Environment details (`anakot --version`, commit SHA, OS, Python
  version).
...
Anakot Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Anakot Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Anakot Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Anakot Agent's documentation
...
Anakot Agent supports two OS-level isolation postures. They address
...
Anakot Agent supports this in two ways:

- **Anakot Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Anakot Agent's in-process heuristics
...
Anakot Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Anakot Agent
itself. Bugs in Anakot Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Anakot Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Anakot Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Anakot Agent has documented a stance about how its output should
...
- **Documented break-glass settings.** Operator-selected trade-offs
  ...
  anakot-home security, and similar. Reports against those
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Anakot Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Anakot Agent. Bugs in Anakot
...
- Anakot Agent includes supply-chain guards for MCP server
