"""
Anakot MCP Server — expose messaging conversations as MCP tools (`anakot mcp serve`).
...
Matches OpenClaw's 9-tool channel bridge surface plus the Anakot-specific
channels_list. Client config: {"mcpServers": {"anakot": {"command": "anakot", "args": ["mcp", "serve"]}}}
"""
...
logger = logging.getLogger("anakot.mcp_serve")
...
def _anakot_home() -> Path:
    try:
        ...
        return Path(os.environ.get("ANAKOT_HOME", Path.home() / ".anakot"))


def _get_sessions_dir() -> Path:
    return _anakot_home() / "sessions"


def _read_state_db_mtime() -> float:
    try:
        return (_anakot_home() / "state.db").stat().st_mtime
...
def _load_channel_directory() -> dict:
    ...
    return _read_json(_anakot_home() / "channel_directory.json")
...
class EventBridge:
    ...
    event queue with waiter support (the Anakot analogue of OpenClaw's WebSocket
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "MCPServer":
    """Create and return the Anakot MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(f"MCP server requires the 'mcp' package. Install with: {sys.executable} -m pip install 'mcp'")
    mcp = MCPServer("anakot", instructions=(
        "Anakot Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Anakot MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
