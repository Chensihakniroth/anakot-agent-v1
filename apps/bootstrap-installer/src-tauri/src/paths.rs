...
//!
//! Mirrors `anakot_constants.get_anakot_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\anakot
//!   macOS:   ~/.anakot
//!   Linux:   ~/.anakot  (override via $ANAKOT_HOME)
//!
//! NOTE (macOS): Python's get_anakot_home(), scripts/install.sh, and the
//! Electron desktop's resolveAnakotHome() ALL use ~/.anakot on macOS — there
...
/// Returns the canonical Anakot home directory, respecting $ANAKOT_HOME if set.
pub fn anakot_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("ANAKOT_HOME") {
    ...
    {
        // %LOCALAPPDATA%\anakot — matches scripts/install.ps1's $AnakotHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("anakot");
        }
    }

    // macOS + Linux + fallback: ~/.anakot (matches Python get_anakot_home(),
    // install.sh, and the Electron desktop's resolveAnakotHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".anakot");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    anakot_home().join("bootstrap-cache")
}
...
/// ANAKOT_HOME so it survives repo checkout deletion (unlike anything under
/// anakot-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\anakot\anakot-setup.exe`; on other
...
pub fn copy_self_to_anakot_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to ANAKOT_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_ANAKOT_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".anakot-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under ANAKOT_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[anakot-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("ANAKOT_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_anakot_home() -> String {
    anakot_home().to_string_lossy().into_owned()
}
