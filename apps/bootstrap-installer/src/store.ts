...
export const $anakotHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.anakot/logs/bootstrap-installer.log')
    $anakotHome.set('~/.anakot')
  ...
  try {
    const [logPath, anakotHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_anakot_home'),
      invoke<AppMode>('get_mode')
    ])

    $logPath.set(logPath)
    $anakotHome.set(anakotHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Anakot" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Anakot-Setup.exe --update);
...
export async function launchAnakotDesktop(): Promise<void> {
  ...
  await invoke('launch_anakot_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Anakot repository' },
