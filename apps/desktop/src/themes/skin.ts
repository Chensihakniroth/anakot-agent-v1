/**
 * Anakot skin → DesktopTheme converter.
 *
 * A "skin" is the CLI/TUI theme unit: a YAML file in `$ANAKOT_HOME/skins/` (or a
 * built-in) resolved by `anakot_cli/skin_engine.py` and pushed to every surface
 ...
 * skin Anakot authors from a prompt lights up all three surfaces from one file.
 *
...
import { ensureContrast, mix } from '@anakot/shared/color'
import type { AnakotSkin, SkinColors } from '@anakot/shared/skin'
 ...
 */
export function skinToDesktopTheme(skin: AnakotSkin): DesktopTheme | null {
