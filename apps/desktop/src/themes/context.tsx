...
import { ensureContrast, mix, parseColor } from '@anakot/shared/color'
...
const PROFILE_SKINS_KEY = 'anakot-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'anakot-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'anakot-desktop-active-profile-v1'
...
// mix / ensureContrast live in @anakot/shared/color (shared with the TUI);
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark', chatFontFamily = $chatFontFamily.get()) {
  ...
  root.dataset.anakotTheme = skinName
  root.dataset.anakotMode = rendered
  ...
  window.anakotDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('anakot-boot-background', chromeBg)
    window.localStorage.setItem('anakot-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.anakotThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.anakotDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  // Drain a backend-driven skin switch (Anakot authoring/activating a skin from a
