/**
 * Live skin sync from the Anakot backend.
 *
 * The backend resolves the active skin (built-in or `$ANAKOT_HOME/skins/*.yaml`)
 ...
 * stomps the user's persisted desktop theme; only a genuine name change (Anakot
 * authoring/activating a skin from a prompt, or `/skin` elsewhere) repaints.
 */

import type { AnakotSkin } from '@anakot/shared/skin'
...
const BACKEND_THEMES_KEY = 'anakot-desktop-backend-themes-v1'
 ...
 */
export function ingestBackendSkin(skin: AnakotSkin | undefined | null, { apply }: { apply: boolean }): void {
  ...
  if (name !== 'default' && !BUILTIN_THEMES[name]) {
    const theme = skinToDesktopTheme(skin as AnakotSkin)
