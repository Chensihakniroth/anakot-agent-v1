...
import { host, LruCache } from '@anakot/plugin-sdk'
...
// to (local, remote URL, SSH, Anakot Cloud, docker) must be able to find
...
// anakot_cli/config_defaults.py and tools/bot_relay.py and fails if the
  ...
  try {
    ...
    return profiles
      .map(profile => ({
        ...
        title: String(profile?.ui_meta?.['anakot-bots']?.title || profile?.display_name || ''),
