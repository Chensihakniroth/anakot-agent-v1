...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Anakot". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `anakot profile rename
  ...
  // never generic "Anakot" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Anakot (the agent it is)
...
/** The profile id the backend accepts (`anakot_cli.profiles._PROFILE_ID_RE`
