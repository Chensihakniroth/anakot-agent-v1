...
import { atom, host } from '@anakot/plugin-sdk'
...
const GROUP_CHAT_SYNC_META_KEY = 'anakot-bots-groups'
...
/** The v3 envelope stored under the default profile's `anakot-bots-groups`
 ...
 *  clients (Anakot Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Anakot"
 *  (community report, Aug 21 2026: renamed default still read "Anakot is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Anakot (matching displayName and the
 *  @anakot handle) so the main agent never loses its name in rooms.
 *
 ...
 *  failing `default`s are never one anonymous "Anakot" — judged against the
...
export function groupSpeakerLabel(name?: null | string, group?: null | string) {
  ...
  if (boundary !== -1) {
    ...
    const label = title || (profile.toLowerCase() === 'default' ? 'Anakot' : profile)
  ...
  // title, then the local row's display_name, then default → Anakot.
  ...
  return isDefault ? 'Anakot' : trimmed
}
