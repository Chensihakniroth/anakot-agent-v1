...
import { forgetSessionUnread, host, queryClient } from '@anakot/plugin-sdk'
...
export function mergeServerMeta(roster: RosterRow[], fetchedAt = 0) {
  ...
  for (const bot of roster) {
    const server = bot.ui_meta?.['anakot-bots']
...
/** Permanently delete a bot's Anakot profile, then remove plugin-local state
 ...
 * directory (anakot-agent#52279). That is the "can't delete a bot" error. */
