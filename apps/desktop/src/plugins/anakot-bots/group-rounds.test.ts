...
describe('routing', () => {
  ...
  it('resolves @anakot to the default member', async () => {
    const { rounds } = await loadRoom()
    ...
    const parsed = rounds.parseGroupChatMentions('@anakot take a look', members)
...
describe('turn prompt', () => {
  it('addresses the default profile as @anakot', async () => {
    const { rounds } = await loadRoom()
    ...
    expect(own).toMatch(/You are @anakot,/)
    ...
    expect(peer).toMatch(/group chat with @anakot/)
  })

  // #89720: a renamed primary is @bobby to the roster, autocomplete and the
  // mention resolver; introducing it to itself as @anakot made it treat
