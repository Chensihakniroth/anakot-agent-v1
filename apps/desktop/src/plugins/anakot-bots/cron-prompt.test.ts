 ...
 * job for a different profile keeps the `anakot -p <bot> chat` delegation
...
/** Run the delegation command under a `anakot` stub that prints its argv, so
 *  the assertion is what the SHELL passed — not what the string looks like. */
function argvOf(prompt: string): string[] {
  const command = prompt.slice(prompt.indexOf('anakot '), prompt.lastIndexOf('\n\nIf the command'))
  const result = spawnSync('sh', ['-c', `anakot() { printf '%s\\037' "$@"; }\n${command}`], { encoding: 'utf8' })
...
describe('direct vs delegated execution', () => {
  ...
  it('keeps the delegation wrapper for a different active profile', () => {
    ...
    expect(prompt).toMatch(/anakot -p 'research' chat/)
