...
Bundled Anakot Desktop plugin, **off by default**. Enable **Radio** in
...
`plugin.js` imports only `@anakot/plugin-sdk`, `react`, and
...
`$ANAKOT_HOME/desktop-plugins/radio/plugin.js` for development. Do not install
that duplicate alongside the bundled version. Layout styles use Anakot theme
...
Stream availability, regional restrictions, and metadata vary. Anakot is not
