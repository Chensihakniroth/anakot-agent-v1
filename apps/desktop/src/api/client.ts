import { JsonRpcGatewayClient } from '@anakot/shared'

import type { AnakotApiRequest } from '@/global'
...
// "Timed out connecting to Anakot backend" that hangs the UI (#48504).
//
...
export class AnakotGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Anakot gateway connection closed',
      connectErrorMessage: 'Could not connect to Anakot gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Anakot gateway is not connected',
      ...
      onUnhandledRequest: request =>
        console.warn(`[gateway] Anakot Desktop has no server-request registry for ${request.method} (${request.id})`),
...
// process to route them to that host (anakot:api's registry branch). Null
...
export function anakotApi<T>(request: AnakotApiRequest): Promise<T> {
  return window.anakotDesktop.api<T>({ ...connectionScoped(), ...request })
}
