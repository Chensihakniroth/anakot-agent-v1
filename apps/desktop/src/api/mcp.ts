...
export function testMcpServer(name: string, profile?: ProfileScope): Promise<McpTestResult> {
  return window.anakotDesktop.api<McpTestResult>({
 ...
 *  `saveAnakotConfig`, this REPLACES rather than deep-merges, so deletes,
...
// `anakot mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveAnakotConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return anakotApi<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function addMcpServer(
  ...
  return window.anakotDesktop.api<McpServerSummary>({
    ...capabilityScoped(profile),
    path: '/api/mcp/servers',
    method: 'POST',
    body
...
export function getMcpCatalog(profile?: ProfileScope, detectApps = false): Promise<McpCatalogResponse> {
  return window.anakotDesktop.api<McpCatalogResponse>({
    ...capabilityScoped(profile),
...
export function installMcpCatalogEntry(
  ...
  return window.anakotDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...capabilityScoped(profile),
