import { reconnectBackoffDelayMs } from '@anakot/shared'
 ...
 *
 *  Exported for tests. */
export async function activeConnection(): Promise<AnakotConnection> {
  ...
  return withTimeout(
    window.anakotDesktop.getConnection(profile),
    RECONNECT_ATTEMPT_TIMEOUT_MS,
    `Timed out connecting to profile "${profile}"`
  )
}

/** Options for a plugin REST call — mirrors the app's own `anakotDesktop.api`
 *  shape, minus the path (which is namespace-derived). */
export interface PluginRestOptions {
  method?: string
  body?: unknown
  /** Single-file multipart upload (see AnakotApiRequest.upload). */
...
export async function pluginRest<T>(pluginId: string, path: string, opts: PluginRestOptions = {}): Promise<T> {
  if (!window.anakotDesktop?.api) {
    throw new Error('Anakot desktop bridge unavailable')
  }
