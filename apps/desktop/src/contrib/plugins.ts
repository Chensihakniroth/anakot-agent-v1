 ...
 *    a `AnakotPlugin` registers automatically (vite glob — drop a folder in).
 *    `anakot-bots` (Bot Mode) ships in-tree and is ON by default; other
 *    reference/demo plugins live in the companion `anakot-example-plugins`
 ...
 *  - RUNTIME: the on-disk doors (`<anakot home>/desktop-plugins/<name>/plugin.js`
 *    and the unified-package half `<anakot home>/plugins/<name>/desktop/plugin.js`)
...
const modules = import.meta.glob<{ default: AnakotPlugin }>('../plugins/*/plugin.{js,ts,tsx}', { eager: true })
...
export function discoverBundledPlugins(): void {
  ...
  for (const [path, mod] of Object.entries(modules)) {
    ...
    if (!plugin?.id || typeof plugin.register !== 'function') {
      console.warn(`[plugins] ${path} has no valid default AnakotPlugin export — skipped`)
