...
// getStatus would supply the connected backend's anakot_home — a REMOTE path in
// remote mode. The disk scanner must NOT derive the plugin root from it (#66899).
const getStatus = vi.fn(async () => ({ anakot_home: '/remote/box/.anakot' }))

vi.mock('@/anakot', async importActual => ({
  ...(await importActual<typeof AnakotModule>()),
  getStatus: () => getStatus()
}))

const desktopPluginsRoot = vi.fn<() => Promise<string>>()
const readDir = vi.fn<(path: string) => Promise<AnakotReadDirResult>>()
...
beforeEach(() => {
  ...
  getStatus.mockClear()
  ;(window as unknown as { anakotDesktop: unknown }).anakotDesktop = {
    desktopPluginsRoot,
...
afterEach(() => {
  delete (window as unknown as { anakotDesktop?: unknown }).anakotDesktop
})

describe('scanDiskPlugins (#66899)', () => {
  it('scans the Electron-resolved local roots, never the backend anakot_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.anakot/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })

    await discoverRuntimePlugins()

    expect(desktopPluginsRoot).toHaveBeenCalled()
    expect(readDir).toHaveBeenCalledWith('/local/.anakot/desktop-plugins')
    ...
    expect(readDir).not.toHaveBeenCalledWith('/local/.anakot/plugins')
    // The remote backend's anakot_home must never feed the local plugin scan.
    expect(getStatus).not.toHaveBeenCalled()
    expect(readDir).not.toHaveBeenCalledWith('/remote/box/.anakot/desktop-plugins')
  })
  ...
  it('treats a folder without plugin.js as metadata, not a throwing file read', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.anakot/desktop-plugins') {
        return {
          entries: [{ isDirectory: true, name: 'my-feature', path: '/local/.anakot/desktop-plugins/my-feature' }]
        }
      }

      if (dir === '/local/.anakot/desktop-plugins/my-feature') {
        return {
          entries: [
            { isDirectory: false, name: 'README.md', path: '/local/.anakot/desktop-plugins/my-feature/README.md' }
          ]
        }
      }
    ...
    expect(readDir).toHaveBeenCalledWith('/local/.anakot/desktop-plugins/my-feature')
  ...
  it('a DIRECTORY named plugin.js is not a plugin entry (metadata walk rejects it)', async () => {
    ...
    readDir.mockImplementation(async dir => {
      if (dir === '/local/.anakot/desktop-plugins') {
        return { entries: [{ isDirectory: true, name: 'odd', path: '/local/.anakot/desktop-plugins/odd' }] }
      }

      if (dir === '/local/.anakot/desktop-plugins/odd') {
        // A folder literally named plugin.js — must resolve to "no entry".
        return {
          entries: [{ isDirectory: true, name: 'plugin.js', path: '/local/.anakot/desktop-plugins/odd/plugin.js' }]
        }
      }
  ...
  it('loads a unified desktop half (app-root copy + package marker) OPT-IN and tags it with its package', async () => {
    ...
    const root = '/local/.anakot/desktop-plugins'
    ...
    readDir.mockImplementation(async dir => {
      ...
      if (dir === `${root}/uni`) {
        return {
          entries: [
            { isDirectory: false, name: '.anakot-package.json', path: `${root}/uni/.anakot-package.json` },
    ...
    readFileText.mockImplementation(async file =>
      file.endsWith('.anakot-package.json')
...
describe('watchRuntimePlugins dir watch (#66899)', () => {
  it('watches the Electron-resolved app root, never the backend anakot_home', async () => {
    desktopPluginsRoot.mockResolvedValue('/local/.anakot/desktop-plugins')
    readDir.mockResolvedValue({ entries: [] })
    ...
    expect(watchDirectory).toHaveBeenCalledWith('/local/.anakot/desktop-plugins')
    expect(watchDirectory).not.toHaveBeenCalledWith('/remote/box/.anakot/desktop-plugins')
    expect(getStatus).not.toHaveBeenCalled()
  })
})
...
describe('plugin source reads (512 KiB preview-cap bug)', () => {
  ...
  const standaloneRootWith = (name: string) => {
    const folder = `/local/.anakot/desktop-plugins/${name}`
  ...
  it('loads the full source via readPluginSource when the shell offers it', async () => {
    ;(window.anakotDesktop as unknown as { readPluginSource: unknown }).readPluginSource = readPluginSource
    desktopPluginsRoot.mockResolvedValue('/local/.anakot/desktop-plugins')
    ...
    try {
      ...
      expect(readPluginSource).toHaveBeenCalledWith('/local/.anakot/desktop-plugins/big/plugin.js')
      expect(register).toHaveBeenCalledTimes(1)
  ...
  it('older shell without readPluginSource: a truncated preview read fails LOUDLY, never evaluates', async () => {
    ...
    try {
      ...
      expect($pluginRecords.get().huge).toMatchObject({
        kind: 'disk',
        status: 'error',
        file: '/local/.anakot/desktop-plugins/huge/plugin.js'
      })
  ...
  it('disposes runtime host event subscriptions before a hot reload (#112366)', async () => {
    ...
    try {
      const source = `
        import { host } from '@anakot/plugin-sdk'
        export default {
...
describe('bundled-shadowed disk copies', () => {
  it('skips a disk copy of a bundled plugin but publishes a visible inventory row', async () => {
    ...
    publishPlugin({ id: 'anakot-bots', name: 'Bot Mode', kind: 'bundled', status: 'loaded' })
    ...
    try {
      const id = await loadRuntimePlugin(
        'export default { id: "anakot-bots", name: "Bot Mode", register() {} }',
        'anakot-bots',
        { file: '/local/.anakot/desktop-plugins/anakot-bots/plugin.js' }
      )

      // Skipped — the bundled copy stays the only live registration...
      expect(id).toBeNull()
      expect($pluginRecords.get()['anakot-bots']).toMatchObject({ kind: 'bundled', status: 'loaded' })
      ...
      expect($pluginRecords.get()['anakot-bots:disk-shadowed']).toMatchObject({
        kind: 'disk',
        status: 'disabled',
        file: '/local/.anakot/desktop-plugins/anakot-bots/plugin.js'
      })
