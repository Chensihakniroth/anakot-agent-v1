...
import { searchSessions, type SessionInfo, type SessionSearchResult } from '@/anakot'
...
// highlight markers (sqlite snippet() delimiters — see anakot_state_search.py).
...
export function ChatSidebar({
  ...
  useEffect(() => {
    ...
    window.addEventListener('anakot:new-session-shortcut', onShortcut)

    return () => {
      window.removeEventListener('anakot:new-session-shortcut', onShortcut)
      clearTimeout(timeout)
    }
  }, [])
  ...
  // A turn can `git worktree add/remove` in the terminal (e.g. you ask Anakot to
