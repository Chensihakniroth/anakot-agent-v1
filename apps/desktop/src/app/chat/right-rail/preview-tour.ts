...
function buildTourScript(action: TourAction): string {
  return `(function () {
  var w = window;
  if (!w.__anakotTourEngine) {
    ${driverIife}
    w.__anakotTourHolder = {};
    w.__anakotTourCollect = (${collectTourTargets.toString()});
    w.__anakotTourEngine = (${runTourEngine.toString()});
  }
  if (!document.getElementById('__anakot-tour-style')) {
    var style = document.createElement('style');
    style.id = '__anakot-tour-style';
    style.textContent = ${JSON.stringify(driverCss)};
    (document.head || document.documentElement).appendChild(style);
  }
  return JSON.stringify(w.__anakotTourEngine(
    w.driver.js.driver,
    w.__anakotTourHolder,
    ${JSON.stringify(action)},
    w.__anakotTourCollect,
    document
  ));
})()`
}
