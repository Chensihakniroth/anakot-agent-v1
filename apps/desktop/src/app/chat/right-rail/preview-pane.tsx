        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.anakotDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({ embedded = false, onRestartServer, reloadRequest = 0, tabId, target }: PreviewPaneProps) {
  ...
  const annotateGuest = useCallback((): null | PreviewAnnotateGuest => {
    ...
    return {
      capture: async rect => {
        ...
        const dataUrl = await window.anakotDesktop.capturePreview?.({ rect, viewport, webContentsId })
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Anakot to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.anakotDesktop?.watchPreviewFile ||
      !window.anakotDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.anakotDesktop.onPreviewFileChanged(payload => {
    ...
    void window.anakotDesktop
      ...
      .then(watch => {
        if (!active) {
          void window.anakotDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.anakotDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:anakot-preview')
    ...
    // `anakot:openExternal` channel, never a popup side effect.
    const onGuestExternal = (event: Event) => {
      ...
      if (admitPreviewExternalUrl(url)) {
        void window.anakotDesktop?.openExternal?.(url)
      }
    }
    ...
    const onGuestContextMenu = (event: Event) => {
      ...
      const zoom = window.anakotDesktop?.zoom?.factor?.() || 1
      ...
      openGuestContextMenu(
        ...
        },
        {
          addToDictionary: (word: string) => {
            ...
            if (typeof webContentsId === 'number') {
              void window.anakotDesktop?.contextMenuGuestAddWord?.({ webContentsId, word })
            }
          },
          copyImage: () => void window.anakotDesktop?.contextMenuCopyImage?.(),
          ...
          <PreviewBrowserBar
            ...
            onOpenExternal={
              !isBrowserWindow() && !canOpenBrowserWindow()
                ? () => void window.anakotDesktop?.openExternal(currentUrl)
                : undefined
            }
