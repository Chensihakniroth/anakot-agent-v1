...
const preamble = () => `  var w = window;
  ...
  w.__anakotActHolder = w.__anakotActHolder || {};
  w.__anakotAct = ${actEngineSource()};
  w.__anakotWatch_fn = (${watchInPage.toString()});
  w.__anakotWatchTag = ${WATCH_TAG};
  var holder = w.__anakotActHolder;
  var act = function (a) { return w.__anakotAct(document, holder, a); };
  var watch = function (stage, label) {
    try { w.__anakotWatch_fn(document, holder, stage, label); } catch (err) {}
  };
...
${preamble()}
  ...
  document.addEventListener('pointerdown', function (e) {
    w.__anakotHit = { tag: e.target ? e.target.tagName : '?', trusted: e.isTrusted === true };
...
function hitNote(hit?: { tag: string; trusted: boolean } | null): string | undefined {
  return hit && hit.tag === 'ANAKOT-WATCH' ? 'The action overlay intercepted the click instead of the page.' : undefined
}
