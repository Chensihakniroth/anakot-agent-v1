...
import { getProfiles, type ProfileScope, profileScopeKey } from '@/anakot'
...
export function useCapabilityScope({
  ...
  const registryBridge = window.anakotDesktop?.connections
  const rosterBridge = window.anakotDesktop?.getAgentRoster
  ...
  const options: ScopeOption[] = useMemo(() => {
    ...
    return (profilesData?.profiles ?? []).map(p => ({
      key: p.name,
      label: p.is_default ? 'Anakot (default)' : p.name,
      value: p.name
    }))
