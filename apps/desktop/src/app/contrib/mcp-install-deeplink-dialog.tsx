...
import { getAnakotConfigRecord, saveMcpServers } from '@/anakot'
...
import { setAnakotConfigCache } from '../hooks/use-config-record'

/**
 * Explicit-confirm gate for `anakot://mcp/install` deep links. The payload is
 ...
 * confirming lets Anakot spawn that local process. An existing server name is
...
export function McpInstallDeepLinkDialog() {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      const current = getServers(await getAnakotConfigRecord())
      ...
      setAnakotConfigCache(previous => (previous ? { ...previous, mcp_servers: nextServers } : previous))
