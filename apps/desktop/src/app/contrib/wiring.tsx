...
import { getLatestSessionMessages } from '@/anakot'
...
import { useAnakotConfigRecord } from '../hooks/use-config-record'
...
export function ContribWiring({ children }: { children: ReactNode }) {
  ...
  // "Restart Anakot" from a toast: recycle the local backend the user is
  ...
  useEffect(() => {
    ...
    if (backendRestartRequest > 0) {
      if ($connection.get()?.mode === 'remote') {
        void reconnectGateway().catch(err => notifyError(err, translateNow('notifications.errors.restartAnakotFailed')))

        return
      }

      void window.anakotDesktop?.recycleBackend?.(normalizeProfileKey($activeGatewayProfile.get())).catch(err =>
        notifyError(err, translateNow('notifications.errors.restartAnakotFailed'))
      )
    }
  ...
  // Without this pin, the owner ladder sends session.create to anakot-setup
  ...
  const { refreshAnakotConfig, sttEnabled, voiceMaxRecordingSeconds } = useAnakotConfig({ activeSessionIdRef })
  ...
  useEffect(() => {
    ...
    window.addEventListener('anakot:open-keybinds', onOpenKeybinds)

    return () => window.removeEventListener('anakot:open-keybinds', onOpenKeybinds)
  }, [navigate])
  ...
  useEffect(() => {
    ...
    void refreshAnakotConfig(true)
    void refreshActiveProfile()
    resetProjectTreeState()
  }, [gatewayScope, refreshCurrentModel, refreshAnakotConfig])
  ...
  const configRecord = useAnakotConfigRecord()
  ...
  const customWindowControls = connection?.customWindowControls ?? window.anakotDesktop?.windowControls?.custom ?? false
