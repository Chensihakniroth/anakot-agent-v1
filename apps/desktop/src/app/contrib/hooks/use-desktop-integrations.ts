...
import { pathFromAnakotDeepLink, resolveAnakotOpenPath } from '@/lib/anakot-open-target'
...
export function useDesktopIntegrations({
  ...
  useEffect(() => {
    ...
    // "About Anakot" — it is the OS-standard affordance for updating THIS app,
    ...
    const unsubscribe = window.anakotDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow('client'))
  ...
  useEffect(() => {
    window.anakotDesktop?.setPreviewShortcutActive?.(true)
  }, [])
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // anakot://index-network/intent/1 vocabulary with deep links.
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onNotificationActivate?.(payload => {
      if (!payload) {
        return
      }
      ...
      if (payload.activate) {
        ...
        // the pre-IPC validation — any future anakotDesktop.notify caller gets
        // funneled through the same resolver.
        const path = resolveAnakotOpenPath(payload.activate)
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onDeepLink?.(payload => {
      if (!payload?.kind) {
        return
      }
      ...
      // link (anakot://index-network/intent/1, anakot://open/…). The resolver
      // rejects reserved kinds and unsafe paths.
      const path = pathFromAnakotDeepLink(payload.kind, payload.name || '', payload.params || {})
    ...
    void window.anakotDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onClosePreviewRequested?.(
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onPreviewNav?.(command => {
  ...
  useEffect(() => {
    const unsubscribe = window.anakotDesktop?.onOpenFolderRequested?.(() => void openFolderAsProject())
