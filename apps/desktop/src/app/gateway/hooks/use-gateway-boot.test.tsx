...
// bug that drives the REAL useGatewayBoot hook + REAL AnakotGateway through a
...
function Harness({
  ...
  refreshAnakotConfig = async () => undefined,
  refreshSessions
}: {
  beforeConnectionSwitch?: () => void
  refreshAnakotConfig?: (force?: boolean, shouldPublish?: () => boolean) => Promise<void>
...
beforeEach(() => {
  ...
  ;(window as { anakotDesktop?: unknown }).anakotDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { anakotDesktop?: unknown }).anakotDesktop
  window.localStorage.removeItem('anakot.desktop.workspace-cwd')
  $currentCwd.set('')
  $busy.set(false)
  $awaitingResponse.set(false)
})
...
describe('default-route profile adoption', () => {
  it.each([null, 'coder-remote'])(
    ...
    async connectionId => {
      ...
      ;(window as { anakotDesktop?: unknown }).anakotDesktop = desktop
      render(<Harness />)
      await flushAsync()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  ...
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForAnakot) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startAnakot()'s remote branch awaits waitForAnakot() for 45s before it
    ...
    // After ~45s waitForAnakot gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Anakot backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
  ...
  it('a superseded Settings switch cannot publish delayed cwd or config work after the winner', async () => {
    ...
    const refreshAnakotConfig = async (_force = false, shouldPublish?: () => boolean) => {
      if (!shouldPublish) {
        return
      }
    ...
    render(<Harness refreshAnakotConfig={refreshAnakotConfig} />)
    await flushAsync()
    expect($gatewayState.get()).toBe('open')
  ...
  it('a getConnection() that hangs on INITIAL boot rejects on its own after the reconnect-attempt timeout, not only when main eventually gives up (#93454)', async () => {
    ...
    // eventual timeout (e.g. waitForAnakot, ~45s) ever settled it. A wedge
    // that main never resolves (not even a rejection) must not hang
    // "Starting Anakot…" forever; the renderer needs to own its own bound
  ...
  it('manual reconnect replaces only the active secondary route', async () => {
    ;(window as { anakotDesktop?: unknown }).anakotDesktop = {
      ...fakeDesktop(),
      getConnectionFor: vi.fn(async () => coderConn),
      getGatewayWsUrlFor: vi.fn(async () => coderConn.wsUrl)
    }
  ...
  it('FIX: post-boot ticket-mint boot-progress errors do not lock the UI', async () => {
    ...
    act(() => {
      desktop.emitBootProgress({
        error: 'Could not reach the remote Anakot gateway while refreshing its WebSocket ticket. Try reconnecting.',
  ...
  it('FIX: a failed session-list fetch during boot is non-fatal — the app still boots', async () => {
    ...
    // failDesktopBoot's "Anakot couldn't start" overlay — the socket is open
  ...
  it('a backend exit while the boot overlay is up fails the overlay and does not add a dead-button toast', async () => {
    ...
    // Anakot" toast here would do nothing when clicked; the overlay's own
  ...
  it('seeds the configured default project dir pre-connect — no route-resume race (#71873)', async () => {
    ...
    expect(cwdAtConnect).toBe('C:\\Anakot')
    expect($currentCwd.get()).toBe('C:\\Anakot')
  })
  ...
  it('FIX #82679: a transient remote boot failure self-heals — the next attempt rebuilds the dropped connection', async () => {
    ...
    // SSH/HTTP remote whose transport dropped. startAnakot() rejects with a
  ...
  it('RETRY CONTRACT: a post-connect failure stays terminal even when its socket closes before boot catches it — a closed socket after a good dial is not a dial failure', async () => {
    ...
    const refreshAnakotConfig = vi.fn(async () => {
    ...
    expect(refreshAnakotConfig).toHaveBeenCalledTimes(1)
    expect($desktopBoot.get().error).toBeTruthy()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
    await advanceBackoff()
    expect(desktop.getConnection).toHaveBeenCalledTimes(1)
  ...
  it('a failed cold boot keeps its recovery surface while main replays cold-boot progress behind it (#112899)', async () => {
    // Main keeps startAnakot() available after the renderer's boot concluded
    ...
    const desktop = fakeDesktop()
    desktop.getConnection = vi.fn(async () => {
      throw new Error('Anakot backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example')
    })
    desktop.getBootProgress = vi.fn(async () => ({
      error: 'Anakot backend did not become ready: getaddrinfo ENOTFOUND gateway.tailnet.example',
      fakeMode: false,
      message: 'Desktop boot failed',
      phase: 'backend.error',
      progress: 24,
    ...
    for (let replay = 0; replay < 2; replay += 1) {
      act(() => {
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Resolving Anakot backend',
        ...
        desktop.emitBootProgress({
          error: null,
          fakeMode: false,
          message: 'Connecting to remote Anakot backend at https://gateway.tailnet.example:8443',
          phase: 'backend.remote',
          progress: 24,
          running: true,
          timestamp: Date.now()
