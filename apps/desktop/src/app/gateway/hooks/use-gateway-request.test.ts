...
vi.mock('@/anakot', async importOriginal => {
  const actual = await importOriginal<typeof AnakotModule>()

  class FakeAnakotGateway {
  ...
  return { ...actual, AnakotGateway: FakeAnakotGateway }
})
...
const fakeGateway = { connectionState: 'open' } as unknown as AnakotGateway
...
async function activateRemoteGateway() {
  ...
  setPrimaryGateway(primary as unknown as AnakotGateway, 'default')
  $gateway.set(primary as unknown as AnakotGateway)
...
describe('useGatewayRequest', () => {
  ...
  it('surfaces a missing optional scoped mint bridge without falling back to a stale ticket or local lookup', async () => {
    const { desktop, gateway } = await activateRemoteGateway()
    Reflect.deleteProperty(window.anakotDesktop, 'getGatewayWsUrlFor')
  ...
  it('rejects instead of hanging forever when the reconnect getConnection() wedges (#93454)', async () => {
    ...
    } as unknown as AnakotGateway

    const getConnection = vi.fn(() => new Promise(() => undefined))

    ;(window as unknown as { anakotDesktop: unknown }).anakotDesktop = { getConnection }
    $gateway.set(dropped)
