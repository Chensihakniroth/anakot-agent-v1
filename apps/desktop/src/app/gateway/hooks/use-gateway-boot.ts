...
export function primaryRuntimeConnectionId(connection: Pick<AnakotConnection, 'connectionId' | 'mode'>): null | string {
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.anakotDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: AnakotGateway | null) => void
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    let pendingWindowState: AnakotWindowState | null = null

    const publish = (next: AnakotConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Anakot...". Once the
    ...
    // The other way a cold boot concludes. Main keeps startAnakot() available
    ...
    const attemptReconnect = async (manual?: { profile: string; activationEpoch: number }) => {
      ...
      try {
        ...
        // "Starting Anakot…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Anakot backend'
        )
        ...
        // as an opaque "Could not connect to Anakot gateway". resolveGatewayWsUrl
        ...
        if (!manual || (isActivePrimary() && gatewayActivationEpoch() === manual.activationEpoch)) {
          reconcileBusyStatesOnReconnect()
          await callbacksRef.current.refreshAnakotConfig().catch(() => undefined)
    ...
    const softSwitch = async () => {
      ...
      try {
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAnakotConfig(false, ownsSwitch).catch(() => undefined),
    ...
    const onBootProgress = (payload: DesktopBootProgress) => {
      ...
      // Soft switch / post-boot startAnakot re-emits progress — ignore so the
    ...
    const gateway = adoptedFromHmr ? survivor!.gateway : new AnakotGateway()
    ...
    configureGatewayRegistry({
      onServerRequest: request => {
        if (!callbacksRef.current.handleServerRequest(request)) {
          request.fail(JSON_RPC_METHOD_NOT_FOUND, `Anakot Desktop cannot answer ${request.method}`)
        }
      },
    ...
    const offGatewayReconnect = registerGatewayReconnect(async () => {
      ...
      if (!isActivePrimary()) {
        ...
        if (!(await ensureActiveGatewayOpen({ explicit: true }))) {
          throw new Error('Anakot gateway is not connected')
        }
    ...
    const offExit = desktop.onBackendExit(() => {
      ...
      notify({
        ...
        durationMs: 0,
        action: {
          label: translateNow('boot.errors.restartAnakot'),
    ...
    async function boot() {
      ...
      try {
        ...
        // round-trip must not hang "Starting Anakot…" forever. Initial boot
        ...
        const conn = await withTimeout(
          ...
          BACKEND_BOOT_WAIT_TIMEOUT_MS,
          'Timed out connecting to Anakot backend'
        )
        ...
        // reaches the recovery affordance instead of hanging "Starting Anakot…".
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          callbacksRef.current.refreshAnakotConfig(),
          ...
          // retry it), not brick boot behind the "Anakot couldn't start"
