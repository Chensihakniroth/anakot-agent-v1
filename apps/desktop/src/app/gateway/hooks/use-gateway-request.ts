import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@anakot/shared'
...
export function useGatewayRequest() {
  ...
  const gateway = useStore($gateway) as AnakotGateway | null
  const gatewayRef = useRef<AnakotGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.anakotDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<AnakotGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as AnakotGateway | null
      }),
    []
  )
