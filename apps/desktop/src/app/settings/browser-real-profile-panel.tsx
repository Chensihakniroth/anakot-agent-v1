...
import { type ProfileScope, saveAnakotConfigRecord } from '@/anakot'
 ...
 * the default browser's profile (cookies/logins) into a Anakot-owned copy;
...
export function BrowserRealProfilePanel({ profile }: BrowserRealProfilePanelProps) {
  ...
  const { data: config } = useAnakotConfigRecord(profile)
  ...
  const toggle = useCallback(
    async (on: boolean) => {
      ...
      try {
        ...
        await saveAnakotConfigRecord({ browser: { use_real_profile: on } }, profile)
        notify({
          kind: 'info',
