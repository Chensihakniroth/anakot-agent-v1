import { REASONING_EFFORTS } from '@anakot/shared'
...
// lib/personalities (mirrors anakot_cli/personality.py BUILTIN_PERSONALITIES).
...
export const EMPTY_SELECT_VALUE = '__anakot_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Anakot & Nous-trained models',
  ...
  { prefix: 'ANAKOT_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // anakot_cli/config.py (local/groq/openai/mistral/elevenlabs).
  'stt.provider': ['local', 'groq', 'openai', 'mistral', 'xai', 'elevenlabs'],
  // How the desktop voice conversation is wired — tools/voice_live.py owns the
  // gpt-live branch (one full-duplex voice model delegating to Anakot).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  browser: {
    useRealProfile:
      "Local browsing uses your real logins. Anakot copies your default browser's profile (cookies, logins, preferences) into a managed snapshot and drives it with its packaged Chromium — your live profile is never opened directly, and the copy is refreshed from it on each run. Also lets the agent open a local real-profile session on request even when a cloud browser backend is configured. Only Chromium browsers (Chrome, Edge, Brave, Brave Origin, Chromium) are supported; a non-Chromium default fails with a clear message. Off by default."
  },
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Anakot stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Anakot can read from one file request.',
  approvals: {
    mode: 'How Anakot handles commands that need explicit approval.',
  ...
  voice: {
    ...
    voiceChatMode:
      'chained: speech-to-text → Anakot → text-to-speech with the providers below. gpt-live: one full-duplex OpenAI voice model (gpt-live-1) listens and talks, and hands every real request to Anakot — any model you have selected answers with the full toolset. Needs an OpenAI API key; the voice layer bills $0.05 per minute.',
    gptLive: {
      voice: 'Voice for GPT-Live mode. Custom voice IDs are accepted.',
      instructions:
        'Extra sentences for the live voice persona (tone, pace, language). Anakot keeps its own system prompt.'
    }
  },
  tts: {
    xai: {
