...
export function VoiceProviderFields({
  ...
  const { data: loadedConfig } = useAnakotConfigRecord(profile)
  ...
  const writeConfigCache = useMemo(() => anakotConfigCacheWriter(profile), [scopeKey])

  const { data: schemaResponse } = useQuery({
    queryKey: ['anakot-config-schema'],
    queryFn: () => getAnakotConfigSchema(),
    staleTime: 5 * 60 * 1000
  })
  ...
  const [baseline, setBaseline] = useState<AnakotConfigRecord | null>(null)
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      void saveAnakotConfigRecord(diffConfig(baseline ?? {}, config), profile)
        .then(() => {
          setBaseline(config)
