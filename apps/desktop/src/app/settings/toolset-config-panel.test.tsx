...
import type { ToolsetConfig } from '@/types/anakot'
...
const getAnakotConfigSchema = vi.fn()
const saveAnakotConfig = vi.fn()
const saveAnakotConfigRecord = vi.fn()
...
vi.mock('@/anakot', () => ({
  ...
  getAnakotConfigSchema: () => getAnakotConfigSchema(),
  saveAnakotConfig: (config: unknown) => saveAnakotConfig(config),
  saveAnakotConfigRecord: (config: unknown, profile?: unknown) => saveAnakotConfigRecord(config, profile),
...
beforeEach(() => {
  ...
  getAnakotConfigSchema.mockResolvedValue({ fields: {}, category_order: [] })
  saveAnakotConfig.mockResolvedValue({ ok: true })
  saveAnakotConfigRecord.mockResolvedValue({ ok: true })
...
describe('ToolsetConfigPanel', () => {
  it('renders inline voice/model fields for a TTS provider row carrying tts_provider', async () => {
    ...
    await waitFor(() => expect(saveAnakotConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const saved = saveAnakotConfigRecord.mock.calls.at(-1)?.[0] as Record<string, Record<string, Record<string, string>>>
    ...
    expect(saveAnakotConfigRecord.mock.calls.at(-1)?.[1]).toBeUndefined()
  })
  ...
  it('autosaves the inline voice fields into the profile the panel is scoped to', async () => {
    ...
    await waitFor(() => expect(saveAnakotConfigRecord).toHaveBeenCalled(), { timeout: 3000 })
    const [saved, forwarded] = saveAnakotConfigRecord.mock.calls.at(-1) as [
