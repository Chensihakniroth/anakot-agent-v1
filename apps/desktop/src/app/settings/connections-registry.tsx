...
function editorFromConnection(conn: DesktopRegistryConnection): EditorState {
  return {
    ...
    remoteAnakotPath: conn.remoteAnakotPath || '',
 ...
 * agent sources (local runtime + any number of remote gateways / Anakot Cloud
...
export function ConnectionsRegistrySection() {
  ...
  useEffect(() => {
    if (!editorWantsOauth || !editorUrl || !window.anakotDesktop?.probeConnectionConfig) {
      setAuthProbe(null)
  ...
  const signInOauth = useCallback(async () => {
    ...
    try {
      const result = await window.anakotDesktop.oauthLoginConnectionConfig(editorUrl)
    ...
    async (allowPlainTextToken = false) => {
      ...
      try {
        ...
        if (editor.kind === 'remote' || editor.kind === 'cloud') {
          ...
          payload.remoteAnakotPath = editor.remoteAnakotPath.trim()
        }
  ...
  // Fan out `anakot update` to every eligible source; per-connection results
          ...
          {editor.kind === 'ssh' && (
            <>
              ...
              <ListRow
                action={
                  <Input
                    onChange={e => setEditor({ ...editor, remoteAnakotPath: e.target.value })}
                    placeholder={t.settings.gateway.sshAnakotPathPlaceholder}
                    value={editor.remoteAnakotPath}
                  />
                }
                description={t.settings.gateway.sshAnakotPathDesc}
                title={t.settings.gateway.sshAnakotPathTitle}
              />
            </>
          )}
