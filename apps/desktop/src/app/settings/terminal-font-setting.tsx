...
import { setAnakotConfigCache, useAnakotConfigRecord } from '../hooks/use-config-record'
...
function fontFamilyFromConfig(config: AnakotConfigRecord): string {
...
export function TerminalFontSetting() {
  ...
  const { data: loadedConfig, dataUpdatedAt } = useAnakotConfigRecord()
  ...
  useEffect(() => {
    ...
    // holds — successful saves write it back via setAnakotConfigCache, so
    ...
    const timeout = window.setTimeout(() => {
      ...
      void saveAnakotConfig(setNested({}, 'terminal.font_family', value))
        .then(result => {
          if (!result.ok) {
        ...
        <div className="mt-3 space-y-2">
          <div className="flex items-center gap-3">
            <Input
              ...
              className="flex-1"
              disabled={draft === null}
              list="anakot-terminal-font-families"
          ...
          <datalist id="anakot-terminal-font-families">
