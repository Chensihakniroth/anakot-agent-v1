...
function ResumeLastSessionSetting() {
  ...
  const configQuery = useAnakotConfigRecord()
  ...
  const update = (on: boolean) => {
    ...
    void saveAnakotConfig(setNested({}, 'display.resume_last_session', on))
      .then(result => {
        if (!result.ok) {
...
function MarketplaceThemeResults({
  ...
  const search = useQuery({
    ...
    queryFn: () => window.anakotDesktop?.themes?.searchMarketplace(debounced) ?? Promise.resolve([]),
