...
// Anakot Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const setKeychainEncryption = async (on: boolean) => {
    ...
    try {
      const res = await window.anakotDesktop.setSecretStorageEncryption(on)
  ...
  useEffect(() => {
    if (state.mode !== 'ssh' || !window.anakotDesktop?.sshConfigHosts) {
      return
    }
  ...
  const payload = (allowPlainTextToken?: boolean) => ({
    ...
    sshRemoteAnakotPath: state.sshRemoteAnakotPath.trim(),
  ...
  const performSave = async (apply: boolean, allowPlainTextToken: boolean) => {
    ...
    try {
      const next = apply
        ? await window.anakotDesktop.applyConnectionConfig(payload(allowPlainTextToken))
        : await window.anakotDesktop.saveConnectionConfig(payload(allowPlainTextToken))
      ...
      const errors = {
        ...
        'anakot-not-found': g.sshErrNotInstalled,
  ...
  const signIn = async () => {
    ...
    try {
      ...
      if (result.connected) {
        const refreshed = await window.anakotDesktop.getConnectionConfig(null)
  ...
  const signOut = async () => {
    ...
    try {
      await window.anakotDesktop.oauthLogoutConnectionConfig(trimmedUrl)
      const refreshed = await window.anakotDesktop.getConnectionConfig(null)
  ...
  const resolveSshHost = async (host: string) => {
    if (!host || !window.anakotDesktop?.sshResolveHost) {
      return
    }
    ...
    try {
      const resolved = await window.anakotDesktop.sshResolveHost(host)
  ...
  const testSsh = async () => {
    ...
    try {
      const result = await window.anakotDesktop.testConnectionConfig(payload())
  ...
  return (
    <SettingsContent bare={embedded}>
      ...
      {/* Anakot Cloud panel: one portal sign-in, then a discovered-agent picker
        ...
        <div className="mt-5 grid gap-1">
          ...
          <ListRow
            action={
              <Input
                className={cn('h-8 font-mono', CONTROL_TEXT)}
                onChange={event => setState(current => ({ ...current, sshRemoteAnakotPath: event.target.value }))}
                placeholder={g.sshAnakotPathPlaceholder}
                value={state.sshRemoteAnakotPath}
              />
            }
            description={g.sshAnakotPathDesc}
            title={g.sshAnakotPathTitle}
          />
        </div>
      ) : null}
      ...
      {embedded ? null : (
        <div className="mt-6 grid gap-1">
          ...
          <ListRow
            action={
              <Button onClick={() => void window.anakotDesktop?.revealLogs()} size="sm" variant="textStrong">
                <FileText />
                {g.openLogs}
              </Button>
            }
