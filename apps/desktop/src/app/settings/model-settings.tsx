...
import { DEFAULT_REASONING_EFFORT, REASONING_EFFORT_VALUES } from '@anakot/shared'
...
import { anakotConfigCacheWriter, invalidateAnakotConfig, useAnakotConfigRecord } from '../hooks/use-config-record'
...
// backend now surfaces the full `anakot model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in anakot_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged, scopeProfile }: ModelSettingsProps) {
  ...
  const { data: config } = useAnakotConfigRecord(scopeProfile)
  const setConfig = useMemo(() => anakotConfigCacheWriter(scopeProfile), [scopeProfile])
  ...
  const refresh = useCallback(
    async ({ replaceSelection = false }: { replaceSelection?: boolean } = {}) => {
      ...
      try {
        ...
        void invalidateAnakotConfig(scopeProfile)
      } catch (err) {
        if (profileEpoch.current === epoch) {
  ...
  const writeAgentDefault = useCallback(
    async (key: string, value: string) => {
      ...
      try {
        await saveAnakotConfig(setNested({}, key, value), scopeProfile)
      } catch (err) {
        setConfig(prev)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `anakot model` curation). Best-effort — fall through to the refreshed
  ...
  const recycleStaleBackend = useCallback(async () => {
    ...
    try {
      await window.anakotDesktop?.recycleBackend?.(scopeProfile)
        ...
        {needsSetup && !setupIsApiKey && selectedProviderRow && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Anakot runs the flow for you.`}
          </p>
        )}
