...
import { disconnectOAuthProvider, listOAuthProviders } from '@/anakot'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/anakot'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.anakotDesktop?.terminal)
...
//      in anakot_cli/provider_catalog.py) — the SAME provider identity
//      `anakot model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Anakot can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Anakot can't clear via the API, but ships a
...
export function ProvidersSettings({
  ...
  // Anakot never deletes creds another tool owns behind a silent API call.
