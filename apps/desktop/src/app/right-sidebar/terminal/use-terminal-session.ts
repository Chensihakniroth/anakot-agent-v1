...
export function useTerminalSession({
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // Full-screen TUIs (anakot --tui, vim) grab the mouse, so a plain drag
    ...
    term.attachCustomKeyEventHandler(event => {
      ...
      void (async () => {
        const text = (await window.anakotDesktop?.readClipboard?.()) ?? ''
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      hasSessionActivityRef.current = true
      void window.anakotDesktop?.terminal?.write(sessionId, `${command}\r`)
