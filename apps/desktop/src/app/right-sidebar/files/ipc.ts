...
import type { AnakotReadDirEntry, AnakotReadDirResult } from '@/global'
...
export type ProjectTreeEntry = AnakotReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: AnakotReadDirEntry) {
...
async function filterIgnored(entries: AnakotReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<AnakotReadDirResult> {
