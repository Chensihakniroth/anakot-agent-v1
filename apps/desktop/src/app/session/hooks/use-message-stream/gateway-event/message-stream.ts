import type { BillingBlock } from '@anakot/shared'
 ...
 * billing-specific toast — never the generic "Anakot error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Anakot reported an error',
