...
export function useGatewayEventHandler(deps: GatewayEventDeps) {
  const { activeSessionIdRef, compactedTurnRef, refreshAnakotConfig, sessionStateByRuntimeIdRef } = deps
  ...
  // refreshAnakotConfig — two REST calls (config + defaults) per event, per
