...
import { useAnakotConfig } from './use-anakot-config'

vi.mock('@/anakot', () => ({
  getAnakotConfig: vi.fn(),
  getAnakotConfigDefaults: vi.fn().mockResolvedValue({})
}))

const WORKSPACE_CWD_KEY = 'anakot.desktop.workspace-cwd'

const mockConfig = (config: Record<string, unknown>) =>
  vi.mocked(getAnakotConfig).mockResolvedValue(config as Awaited<ReturnType<typeof getAnakotConfig>>)

describe('useAnakotConfig refreshAnakotConfig', () => {
  beforeEach(() => {
  ...
  it('mirrors display.show_reasoning and resets a missing key to the default', async () => {
    ...
    const { result } = renderHook(() => useAnakotConfig({ activeSessionIdRef: { current: null } }))

    await act(async () => {
      await result.current.refreshAnakotConfig()
    })
  ...
  // `agent.reasoning_effort: high` to Anakot' built-in medium.
  ...
  it('does not let terminal.cwd replace an active session workspace', async () => {
    ...
    mockConfig({ terminal: { cwd: '/Users/example/new-workspace' } })
    const { result } = renderHook(() => useAnakotConfig({ activeSessionIdRef: { current: 'session-1' } }))
  ...
  it('does not let a stale forced config refresh overwrite newer draft selector intent', async () => {
    const profileConfig = deferred<Awaited<ReturnType<typeof getAnakotConfig>>>()
    vi.mocked(getAnakotConfig).mockReturnValueOnce(profileConfig.promise)

    const { result } = renderHook(() => useAnakotConfig({ activeSessionIdRef: { current: null } }))

    let pendingRefresh!: Promise<void>
    act(() => {
      pendingRefresh = result.current.refreshAnakotConfig(true)
    })
    expect(getAnakotConfig).toHaveBeenCalled()
    ...
    } as Awaited<ReturnType<typeof getAnakotConfig>>)
  ...
  it('does not publish config after its switch loses ownership', async () => {
    const staleConfig = deferred<Awaited<ReturnType<typeof getAnakotConfig>>>()
    vi.mocked(getAnakotConfig).mockReturnValueOnce(staleConfig.promise)
    const { result } = renderHook(() => useAnakotConfig({ activeSessionIdRef: { current: null } }))
    let ownsSwitch = true

    let refresh!: Promise<void>
    act(() => {
      refresh = result.current.refreshAnakotConfig(false, () => ownsSwitch)
    })
  ...
  it('does not let an older profile config overwrite a newer profile', async () => {
    const profileB = deferred<Awaited<ReturnType<typeof getAnakotConfig>>>()
    const profileC = deferred<Awaited<ReturnType<typeof getAnakotConfig>>>()
    vi.mocked(getAnakotConfig).mockReturnValueOnce(profileB.promise).mockReturnValueOnce(profileC.promise)
    ...
    act(() => {
      refreshB = result.current.refreshAnakotConfig(true)
      refreshC = result.current.refreshAnakotConfig(true)
    })
