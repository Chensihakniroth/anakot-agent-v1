...
import { getAnakotConfig, getAnakotConfigDefaults } from '@/anakot'
...
interface AnakotConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
}

export function useAnakotConfig({ activeSessionIdRef }: AnakotConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)
  const profileRefreshEpochRef = useRef(0)

  const refreshAnakotConfig = useCallback(
    async (force = false, shouldPublish: () => boolean = () => true) => {
      ...
      try {
        const [config, defaults] = await Promise.all([getAnakotConfig(), getAnakotConfigDefaults().catch(() => ({}))])
        ...
        // rendering/applying Anakot' built-in medium over the user's config.
        if (!canPublish()) {
          return
        }
  ...
  return { refreshAnakotConfig, sttEnabled, voiceMaxRecordingSeconds }
}
