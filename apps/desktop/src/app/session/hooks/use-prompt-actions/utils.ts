...
export async function readImageForRemoteAttach(
  ...
  const dataUrl = await window.anakotDesktop?.readFileDataUrl(filePath)
...
export async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.anakotDesktop?.readFileDataUrlForAttach ?? window.anakotDesktop?.readFileDataUrl
