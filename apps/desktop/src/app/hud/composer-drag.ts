...
function setWorkspaceTransfer(transferring: boolean): void {
  window.anakotDesktop?.hud?.setWorkspaceTransfer?.(transferring)
}

function moveHud(state: PressState): void {
  window.anakotDesktop?.hud?.moveBy?.({
    width: state.originW,
...
function armGrab(state: PressState, workspaceTransfer: boolean): void {
  ...
  window.anakotDesktop?.hud?.beginMove?.()
}
 ...
 * (see anakot:hud:move-by — a transparent frameless window drifts wider on
...
export function useHudComposerDrag(
  ...
  const reset = useCallback(() => {
    ...
    if (state) {
      if (state.armed) {
        window.anakotDesktop?.hud?.endMove?.()
      }
