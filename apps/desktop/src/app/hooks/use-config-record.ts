...
import { getAnakotConfigRecord, type ProfileScope, profileScopeKey } from '@/anakot'
...
//
// Distinct from session/hooks/use-anakot-config.ts, which is side-effecting —
// it pushes personality/cwd/voice/… into the session stores for live chat.
export const ANAKOT_CONFIG_KEY = ['anakot-config-record'] as const
...
// share a cache row.
export const anakotConfigKey = (profile?: ProfileScope) =>
  profile == null ? ANAKOT_CONFIG_KEY : ([...ANAKOT_CONFIG_KEY, profileScopeKey(profile)] as const)
...
export const useAnakotConfigRecord = (profile?: ProfileScope) =>
  useQuery({
    queryKey: anakotConfigKey(profile),
    ...
    queryFn: () => getAnakotConfigRecord(profile ?? undefined),
    staleTime: 0
  })

// setAnakotConfigCache writes the app-wide (base-key) record. Pass a profile to
...
export const setAnakotConfigCache = writeCache<AnakotConfigRecord>(ANAKOT_CONFIG_KEY)
export const anakotConfigCacheWriter = (profile?: ProfileScope) =>
  writeCache<AnakotConfigRecord>(anakotConfigKey(profile))

export const invalidateAnakotConfig = (profile?: ProfileScope) =>
  queryClient.invalidateQueries({ queryKey: anakotConfigKey(profile) })
