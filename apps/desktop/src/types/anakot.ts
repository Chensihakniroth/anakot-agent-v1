import type { ConnectionRequestPayload } from '@anakot/shared'
...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Anakot doesn't know how to remove it. */
...
export interface FreeTierStatus {
   ...
   *  (`anakot_cli/anon_auth.py`), `error` its sentence, `retryable` whether a
...
export interface EnvVarInfo {
  ...
  // in anakot_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `anakot model` uses — instead of
...
export type AnakotConfigRecord = Record<string, unknown>
...
export interface ProfileInfo {
  ...
  display_name?: string
  /** Bot Mode title (profile.yaml ui_meta['anakot-bots'].title) — the name the
...
// non-desktop) Anakot simply carries none of it. See store/profile-share.ts.
export interface ProfileDesktopOverlay {
  ...
  /** Layout tree (anakot.desktop.layoutTree.v2 shape). */
  layoutTree?: unknown
...
// folders. Mirrors anakot_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Anakot, 'hub' = installed. */
  provenance?: 'agent' | 'bundled' | 'hub'
}

/** One entry of the built-in optional-skills catalog (optional-skills/ in the
 *  repo) — official skills that ship with Anakot but install on demand. */
...
export interface ToolProvider {
   ...
   *  CLI `anakot tools` active-provider detection). */
  is_active: boolean
 ...
 *  not Anakot); elsewhere, driver health from `cua-driver doctor`. `null`
...
export interface ActionStatusResponse {
  ...
  pid: number | null
  running: boolean
  /** anakot-update only: durable completion identity recovered from update.log. */
  action_id?: string
  /** anakot-update only: summary of the durable update receipt (#91277 bullet 3) —
...
/** Shape of `GET /api/anakot/update/check` — the backend's own update state.
