...
vi.mock('@/anakot', () => ({ deleteProfile: vi.fn(), getLogs: vi.fn(), getStatus: vi.fn(), anakotApi: vi.fn() }))
...
const { deleteProfile, anakotApi } = await import('@/anakot')
...
describe('connection-aware plugin host APIs', () => {
  it('retires a profile gateway before deleting it', async () => {
    ...
    // profile's backend, re-creating its ANAKOT_HOME (#94235).
  ...
  it('refreshes the profile inventory before asking Electron for routes', async () => {
    ...
    ;(window as unknown as { anakotDesktop: unknown }).anakotDesktop = { getProfileRoutes }
  ...
  it('reads and hides persisted sessions through the source primary without activating the profile', async () => {
    ...
    expect(anakotApi).toHaveBeenNthCalledWith(
      1,
    ...
    expect(anakotApi).toHaveBeenNthCalledWith(2, {
      connectionId: 'source-a',
      path: '/api/sessions/bot-chat',
      method: 'PATCH',
      body: { hidden: true, profile: 'backend-worker' }
    })
    expect(vi.mocked(anakotApi).mock.calls.every(([request]) => !('profile' in request))).toBe(true)
