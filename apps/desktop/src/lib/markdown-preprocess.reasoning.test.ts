...
describe('reasoning blocks in streamed markdown', () => {
  it('does not fuse the words a stripped block separated', () => {
    expect(preprocessMarkdown('no<thinking>hmm</thinking> Anakot Desktop')).toBe('no Anakot Desktop')
    ...
    expect(preprocessMarkdown('no<thinking>a</thinking><think>b</think>Anakot')).toBe('no Anakot')
  })
