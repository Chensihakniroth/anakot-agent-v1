...
/** Why a known Anakot command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Anakot profile', surface: action('profile') },
  {
    name: '/skin',
 ...
 * `anakot_cli/commands.py::desktop_surface_registry` by
...
function isKnownAnakotSlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Anakot' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownAnakotSlashCommand(normalized)
}
 ...
 * activity — the ones that ship with Anakot and were never asked for. It is
