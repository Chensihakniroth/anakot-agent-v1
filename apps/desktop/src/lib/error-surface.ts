...
export interface ErrorRecoveryPlan {
  ...
  editMessage: boolean
  /** Reveal the Anakot data folder so the user can free space (disk_full). */
  openAnakotFolder: boolean
...
const CODE_PLANS: Partial<Record<ErrorCodeKey, Partial<ErrorRecoveryPlan>>> = {
  ...
  disk_full: { openAnakotFolder: true, retry: true },
