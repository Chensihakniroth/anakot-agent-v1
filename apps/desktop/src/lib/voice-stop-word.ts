...
//
// When someone is in a hands-free "Hey Anakot" voice chat, the natural way to
...
// phrase (optionally addressed to Anakot), so a real turn that merely contains
...
// Optional address prefixes so "anakot stop" / "ok stop" / "hey anakot, stop"
// still count. Stripped before matching the core phrase.
const ADDRESS_PREFIXES: readonly string[] = ['hey anakot', 'hey anakot,', 'anakot', 'anakot,', 'ok', 'okay', 'hey']
...
function stripAddress(text: string): string {
  for (const prefix of ADDRESS_PREFIXES) {
    if (text === prefix) {
      // Bare address ("anakot") is not a stop command on its own.
      continue
    }
 ...
 * to Anakot). Returns false for anything that merely contains "stop" as part of
