 ...
 * `anakot://` deep links and `host.navigate('/path?…')`.
 *
 ...
 *  - `anakot://index-network/intent/1` → `/index-network/intent/1` (plugin-scoped)
 *  - `anakot://open/my-page?item=x` → `/my-page?item=x` (generic open)
 *  - `/my-page?item=x` / `#/my-page?item=x` (hash-router paths)
 */

export type AnakotOpenTarget = string | { href: string } | { path: string; params?: Record<string, string> }
...
export function normalizeAnakotOpenString(raw: string): string | null {
  ...
  if (trimmed.startsWith('anakot://') || trimmed.startsWith(`${ANAKOT_PROTOCOL}//`)) {
    try {
      const url = new URL(trimmed)
      const host = url.hostname || ''
      const rest = decodeURIComponent((url.pathname || '').replace(/^\//, ''))

      // anakot://open/<path>?… → /<path>?…
      if (host === 'open') {
        if (!rest) {
          return null
        }
      ...
      // anakot://index-network/intent/1 → /index-network/intent/1
...
export function resolveAnakotOpenPath(target: AnakotOpenTarget | null | undefined): string | null {
  if (target == null) {
    return null
  }

  if (typeof target === 'string') {
    return normalizeAnakotOpenString(target)
  }
  ...
  if ('href' in target && typeof target.href === 'string') {
    return normalizeAnakotOpenString(target.href)
  }

  if ('path' in target && typeof target.path === 'string') {
    const base = normalizeAnakotOpenString(target.path)
 ...
 * (`anakot://<kind>/<name>?…` → kind/name/params).
 */
export function pathFromAnakotDeepLink(kind: string, name: string, params: Record<string, string> = {}): string | null {
  if (!kind || !name) {
    return null
  }

  if (kind === 'open') {
    return resolveAnakotOpenPath({ path: `/${name.replace(/^\//, '')}`, params })
  }
  ...
  return resolveAnakotOpenPath({ path: `/${kind}/${name.replace(/^\//, '')}`, params })
}

/** Convenience for `anakot://open/<name>?…` payloads. */
export function pathFromOpenDeepLink(name: string, params: Record<string, string> = {}): string | null {
  return pathFromAnakotDeepLink('open', name, params)
}
