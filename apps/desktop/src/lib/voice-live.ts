 ...
 * that into an ordinary Anakot turn on the open session and streams the reply
 * back with `session.commentary.append`, which the voice paraphrases aloud.
 * Anakot keeps every capability — model choice, tools, memory, approvals.
 *
...
export interface VoiceLiveHandlers {
  /** GPT-Live asked the backend (Anakot) for help. `context` is the recent
...
export async function fetchVoiceLiveStatus(): Promise<null | VoiceLiveStatus> {
  try {
    const response = await anakotApi<{ ok: boolean } & VoiceLiveStatus>({
      ...profileScoped(),
...
export class VoiceLiveSession {
  ...
  /** The delegation currently being answered by Anakot; late results for an
  ...
  async start(history: LiveHistoryMessage[]): Promise<void> {
    ...
    const response = await anakotApi<{
      ok: boolean
      session?: { id: string }
  ...
  /** Quiet progress for the live model ("Anakot is running the tests…"). */
