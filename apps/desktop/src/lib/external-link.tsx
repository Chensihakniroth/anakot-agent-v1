...
const SKIP_PROTO_RE = /^(?:file|data|mailto|javascript|blob|chrome|about|anakot):/i
...
export function fetchLinkTitle(url: string): Promise<string> {
  ...
  const bridge = typeof window === 'undefined' ? undefined : window.anakotDesktop?.fetchLinkTitle
...
export function openExternalLink(href: string): void {
  if (href) {
    void window.anakotDesktop?.openExternal?.(href)
  }
}
 ...
 * doesn't cost a context switch out of Anakot, and it is the surface the agent
