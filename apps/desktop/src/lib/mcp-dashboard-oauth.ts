...
const UPDATE_BACKEND = 'Update the Anakot backend to support Desktop MCP OAuth callbacks.'
...
export async function completeMcpDesktopOAuth({
  ...
  const bridge = window.anakotDesktop.mcpOauth

  // A legacy null connection can resolve to a remote registry primary.
  if (!bridge && scope.connectionId !== 'local') {
    throw new Error('Update Anakot Desktop to support MCP OAuth callbacks.')
  }
  ...
  try {
    ...
    await window.anakotDesktop.openExternal(authUrl)
    let pollFailures = 0
