import { anakotApi } from '@/api/client'
import type {
  AnakotConnection,
  AnakotReadDirResult,
  AnakotReadFileTextResult,
  AnakotSelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: AnakotConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export function desktopFsCacheKey(connection: AnakotConnection | null = $connection.get()) {
...
export async function readDesktopDir(path: string): Promise<AnakotReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<AnakotReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<AnakotReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<AnakotReadFileTextResult>(fsPath('read-text', path))
}
...
export async function readDesktopFileDataUrlLocalFirst(path: string): Promise<string> {
  try {
    const local = await window.anakotDesktop?.readFileDataUrl?.(path)
...
export async function selectDesktopPaths(options?: AnakotSelectPathsOptions): Promise<string[]> {
  const desktop = bridge()
