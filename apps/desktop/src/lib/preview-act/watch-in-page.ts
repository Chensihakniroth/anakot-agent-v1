...
export function watchInPage(doc: Document, holder: WatchHolder, stage: WatchStage, label?: string): void {
  const win = doc.defaultView as (Window & { __anakotWatch?: Record<string, unknown> }) | null
  ...
  const busy = 25
  const state = win.__anakotWatch || (win.__anakotWatch = {})
  ...
  const build = () => {
    ...
    const stale = doc.querySelectorAll('anakot-watch')

    for (let i = 0; i < stale.length; i++) {
      stale[i].remove()
    }

    const host = doc.createElement('anakot-watch')
  ...
  // this up" in dev.
  const stamp = (win as unknown as { __anakotWatchTag?: number }).__anakotWatchTag
