...
describe('selectProfile startup preference (#79886)', () => {
  ...
  it('does not replace the local startup preference for a profile SSH override', async () => {
    ...
    selectProfile('macmini-anakot')

    await vi.waitFor(() => expect(ensureGatewayForProfile).toHaveBeenCalledWith('macmini-anakot'))
    await vi.waitFor(() => expect(getConnection).toHaveBeenCalledWith('macmini-anakot'))
