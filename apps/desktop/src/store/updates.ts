...
import type { BackendUpdateCheckResponse } from '@/types/anakot'
...
const UPDATE_TOAST_SNOOZE_KEY = 'anakot:update-toast-snooze-until'
...
const SKEW_TOAST_SNOOZE_KEY = 'anakot:backend-skew-toast-snooze-until'
...
const INSTALL_METHOD_TOAST_SNOOZE_KEY = 'anakot:install-method-toast-snooze-until'
...
export function reportBackendContract(contract: number | undefined): void {
  ...
  notify({
    action: {
      label: translateNow('notifications.updateAnakot'),
      onClick: () => {
        snoozeSkewToast()
...
export async function refreshDesktopVersion(): Promise<DesktopVersionInfo | null> {
  ...
  try {
    const next = await window.anakotDesktop?.getVersion?.()
...
export async function checkBackendUpdates({
  ...
  try {
    const status = mapBackendCheck(await checkAnakotUpdate(force))
...
export async function checkUpdates({ force = false }: UpdateCheckOptions = {}): Promise<DesktopUpdateStatus | null> {
  const bridge = window.anakotDesktop?.updates
...
export async function applyUpdates(opts: DesktopUpdateApplyOptions = {}): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    // `anakot update` themselves. Land on a dedicated manual state so the
    ...
    if (result?.manual) {
      $updateApply.set({
        ...
        applying: false,
        stage: 'manual',
        message: result.command ?? 'anakot update',
        command: result.command ?? 'anakot update'
      })
...
function completedAfterRestart(
  ...
  return !!actionId && status.lines.some(line => line === `=== anakot-update completed ${actionId} ===`)
}
...
async function runBackendUpdate(): Promise<DesktopUpdateApplyResult> {
  ...
  try {
    ...
    const started = await updateAnakot()
    ...
    if (!started.ok) {
      ...
      const command = (started as { update_command?: string }).update_command || 'anakot update'
    ...
    while (Date.now() < deadline) {
      ...
      if (!started.action_id && last.exit_code === null) {
        try {
          const status = await checkAnakotUpdate(true)
...
const FOCUS_RECHECK_KEY = 'anakot.updates.last-passive-check'
