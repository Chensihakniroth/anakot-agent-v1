 ...
 *  Working in `~/www/anakot-agent`, the draft says "anakot-agent" constantly
 *  — a "Use skill: anakot-agent" pill on every mention is noise (the reported
 *  annoyance that prompted this guard). Boundary containment, not exact
 *  segment equality, so worktree dirs (`anakot-agent-suggest`) suppress too. */
 ...
 *  segment of a qualified form (`github/anakot-agent-dev`, `plugin:skill`). */
