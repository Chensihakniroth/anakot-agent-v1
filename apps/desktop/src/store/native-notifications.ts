...
import { type AnakotOpenTarget, resolveAnakotOpenPath } from '@/lib/anakot-open-target'
...
export interface NativeNotificationInput {
   ...
   * `sessionId` (plugins). Same vocabulary as `anakot://index-network/intent/1`.
   */
  activate?: string
...
export interface PluginNotificationAction {
  id: string
  label: string
  /** Navigate here on button press (path or `anakot://index-network/intent/1`). */
  activate?: AnakotOpenTarget
 ...
 *  user is away from Anakot — the in-app toast (`host.notify`) covers the
 *  foreground case. */
export function dispatchPluginNativeNotification(pluginId: string, input: PluginNativeNotificationInput): void {
  const activate = resolveAnakotOpenPath(input.activate) ?? undefined
  ...
  const actions: NativeNotificationAction[] | undefined = input.actions?.map(action => ({
    activate: resolveAnakotOpenPath(action.activate) ?? undefined,
    id: action.id,
    text: action.label
  }))
