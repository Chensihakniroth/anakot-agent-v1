 ...
 * app's bounds and stays visible while Anakot is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'anakot.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'anakot.desktop.pet-overlay-bounds.v1'
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.anakotDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.anakotDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.anakotDesktop?.petOverlay?.close()
}
