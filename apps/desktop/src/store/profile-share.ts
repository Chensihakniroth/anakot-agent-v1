 ...
 *
 * The archive is the CLI's own `anakot profile export` tar.gz (config, skills,
...
import { exportProfileArchive, importProfileArchive } from '@/anakot'
...
import type { ProfileDesktopOverlay } from '@/types/anakot'
...
const ARCHIVE_FILTERS = [{ extensions: ['tar.gz', 'tgz'], name: 'Anakot profile' }]
...
export async function runExportProfileFlow(profile?: string): Promise<null | string> {
  ...
  const pick = window.anakotDesktop?.selectSavePath
