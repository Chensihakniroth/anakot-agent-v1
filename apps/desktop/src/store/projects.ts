...
import type { ProjectInfo, ProjectsPayload } from '@/types/anakot'
...
async function activeProjectsContext(profile = projectProfile()): Promise<ActiveProjectsContext> {
  ...
  if (!gateway || gateway !== activeGateway() || profile !== normalizeProfileKey($activeGatewayProfile.get())) {
    throw new Error('Active Anakot profile changed while connecting')
  }
...
async function refreshProjectTreeAcrossProfiles(): Promise<void> {
  ...
  try {
    const res = await anakotApi<ProjectTreePayload>({
...
const repoScanStates = new WeakMap<AnakotGateway, RepoScanState>()
const scanningGatewayGenerations = new WeakMap<AnakotGateway, number>()
...
export async function scanAndRecordRepos(force = false): Promise<void> {
  if (isDesktopFsRemoteMode()) {
    ...
    // with `scan: true` — added in #81723) so repos with zero Anakot
  ...
  try {
    const policy = repoDiscoveryPolicyFromConfig(await getAnakotConfig(context.profile))
...
// repo, returning where Anakot should start working. Git is the source of
