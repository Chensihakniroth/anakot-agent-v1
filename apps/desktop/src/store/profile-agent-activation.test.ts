...
const agentConn = (over: Partial<AnakotConnection> = {}): AnakotConnection =>
  ({ baseUrl: 'https://homelab.invalid', mode: 'remote', profile: 'research', ...over }) as AnakotConnection

const localConn = (over: Partial<AnakotConnection> = {}): AnakotConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as AnakotConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<AnakotConnection>>()

const getConnectionFor =
  vi.fn<(payload: { connectionId?: null | string; profile?: null | string }) => Promise<AnakotConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { anakotDesktop: { getConnection, getConnectionFor }, localStorage })
...
describe('ensureGatewayAgent commit hook (beforeActivate) — the Sessions switcher door (#93937)', () => {
  ...
  it('a descriptor lookup that never answers is bounded, fails open, and releases the mutex', async () => {
    ...
    try {
      getConnectionFor.mockImplementationOnce(() => new Promise<AnakotConnection>(() => undefined))
