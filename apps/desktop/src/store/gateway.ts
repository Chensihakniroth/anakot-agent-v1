...
import { AnakotGateway, setApiRequestConnection } from '@/anakot'
...
function dialProfile(
  desktop: NonNullable<typeof window.anakotDesktop>,
  profile: string,
...
const isOpen = (gateway: AnakotGateway | null): boolean => gateway?.connectionState === 'open'
...
interface RegistryConfig {
  ...
  onActiveConnectionChanged?: (connection: AnakotConnection) => void
  /**
...
interface Secondary {
  ...
  connection: AnakotConnection | null
...
interface GatewayRegistryState {
  ...
  primaryGateway: AnakotGateway | null
   ...
   *  `anakot serve --profile <primary>` child and can never stand in for a
  ...
  $gateway: ReturnType<typeof atom<AnakotGateway | null>>
  $activeProfile: ReturnType<typeof atom<string>>
}

const STATE_KEY = Symbol.for('anakot.desktop.gatewayRegistryState')
...
function createRegistryState(): GatewayRegistryState {
  return {
    ...
    $gateway: atom<AnakotGateway | null>(null),
...
export function setPrimaryGateway(gateway: AnakotGateway | null, profile = 'default'): void {
...
export function setPrimaryGatewayConnection(connection: Pick<AnakotConnection, 'connectionId' | 'mode'> | null): void {
...
async function isAttachedSharedRemote(
  ...
  // A local primary is one `anakot serve --profile <primary>` child; every other
  ...
  try {
    ...
    // (early return above): it is one `anakot serve --profile <primary>` child
...
export function activeGateway(): AnakotGateway | null {
  if (g.activeKey === g.primaryProfile) {
...
function applyActive(profile: string, activationEpoch: number): boolean {
  ...
  // Push the active scope's registry connection into the anakot module (null
...
function publishActiveConnection(connection: AnakotConnection): void {
...
function createSecondary(profile: string, connectionId: null | string = null): Secondary {
  const gateway = new AnakotGateway()
...
async function gatewayForProfile(
  ...
  leaseRequest = false,
  spawnPriority: SpawnPriority = 'background'
): Promise<{ gateway: AnakotGateway | null; key: string; release: () => void; scopeProfile: boolean }> {
  const key = normKey(profile)
...
export async function requestGatewayForProfile<T>(
  ...
  try {
    if (!route.gateway) {
      throw new Error(`Anakot gateway unavailable for profile "${route.key}"`)
    }
...
export async function requestGatewayForAgent<T>(
  ...
  if (!window.anakotDesktop?.getConnectionFor) {
    throw new Error('This Desktop build cannot dial registry connections. Update Anakot Desktop.')
  }
...
function publishTurnLease(scope: string, activeTurn: boolean): void {
  void window.anakotDesktop?.touchBackend?.(scope, { activeTurn }).catch(() => undefined)
}
...
export async function ensureActiveGatewayOpen({ explicit = false }: { explicit?: boolean } = {}): Promise<AnakotGateway | null> {
  if (g.activeKey === g.primaryProfile) {
    return g.primaryGateway
  }
  ...
  if (!isOpen(entry.gateway)) {
    ...
    // "Anakot gateway is not connected" on the Sessions `+` action (#88880).
