 ...
 * `@anakot/shared/translucency`, which owns the mapping both this store and the
 * main process read.
 *
 ...
 * material underneath the web contents to read (see the `[data-anakot-glass]`
 * block in styles.css).
 */
...
} from '@anakot/shared/translucency'
 ...
 * sets `anakotDesktop.glassSupported` from `os.release()` so Win10 cannot
...
export const GLASS_SUPPORTED =
  typeof window !== 'undefined' && typeof window.anakotDesktop?.glassSupported === 'boolean'
    ? window.anakotDesktop.glassSupported
...
export const TRANSLUCENCY_SUPPORTED =
  typeof window !== 'undefined' && typeof window.anakotDesktop?.translucencySupported === 'boolean'
    ? window.anakotDesktop.translucencySupported
    : isMacPlatform() || isWindowsPlatform()
...
/* Peek: while the user is actively adjusting translucency from Settings, the
   ...
   [data-anakot-translucency-peek] rules in styles.css). A counter rather
   than a boolean: a held slider drag and a timed pulse from a picker click
   can overlap. */
const PEEK_ATTR = 'data-anakot-translucency-peek'
...
const applyGlassSurfaces = ({ intensity, mode, scope }: TranslucencyState): void => {
  ...
  root.toggleAttribute('data-anakot-glass-on', glassLive)
  root.toggleAttribute('data-anakot-glass', glassOn)
  root.toggleAttribute('data-anakot-clear', clearOn)
  ...
  if (glassOn) {
    root.setAttribute('data-anakot-glass-scope', scope)
  } else {
    root.removeAttribute('data-anakot-glass-scope')
  }
...
if (typeof window !== 'undefined') {
  ...
  $translucency.subscribe(state => {
    ...
    window.anakotDesktop?.setTranslucency?.(state)
  })
