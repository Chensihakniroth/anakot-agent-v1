...
const { getProfiles } = await import('@/anakot')
...
const profile = (name: string, isDefault = false): ProfileInfo => ({
  ...
  model: null,
  name,
  path: `/tmp/anakot/${name}`,
  provider: null,
  skill_count: 0
})

const remoteConn = (over: Partial<AnakotConnection> = {}): AnakotConnection =>
  ({ baseUrl: 'https://anakot-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as AnakotConnection
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { anakotDesktop: { getConnection } })
