 ...
 *
 * This store is the only writer of anakot-intro-reveal-seen-v1. First-run eligibility is guest onboarding
...
const SEEN_KEY = 'anakot-intro-reveal-seen-v1'
...
/** The film will never play for this identity (ANAKOT_SKIP_INTRO). Record it
...
export function isIntroRevealEnabled(): boolean {
  return isOnboardingEnabled() && window.anakotDesktop?.skipIntro !== true
}

/** The guided flow is on but its intro film was turned off at launch
 * (ANAKOT_SKIP_INTRO). Distinct from isIntroRevealEnabled(): false there also
...
export function startIntroReveal(): void {
  ...
  void window.anakotDesktop?.introReveal?.open({ hideMain: true }).catch(finishIntroReveal)
}
...
export function finishIntroReveal(): void {
  ...
  void window.anakotDesktop?.introReveal?.close({ showMain: true }).catch(() => undefined)
}

export function installIntroRevealBridgeListeners(): () => void {
  const bridge = window.anakotDesktop?.introReveal
