...
// reaches the gateway); everything else routes through window.anakotDesktop.git,
...
function file(path: string, over: Partial<AnakotReviewFile> = {}): AnakotReviewFile {
  return { path, status: 'modified', staged: false, added: 1, removed: 0, ...over } as AnakotReviewFile
}

type ReviewStub = Record<string, ReturnType<typeof vi.fn>>

// Install a review bridge on window.anakotDesktop. Any op not supplied defaults
...
describe('ship flow', () => {
  ...
  it('createOrOpenPr opens the existing PR without creating a new one', async () => {
    const review = stubReview()
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'https://example.com/pr/9' } } as AnakotReviewShipInfo)

    await createOrOpenPr()

    expect(review.createPr).not.toHaveBeenCalled()
    expect(
      (window.anakotDesktop as unknown as { openExternal: ReturnType<typeof vi.fn> }).openExternal
...
describe('refreshShipInfo', () => {
  it('populates ship info from the bridge', async () => {
    const info: AnakotReviewShipInfo = {
      ghReady: true,
      pr: { url: 'https://example.com/pr/3' }
    } as AnakotReviewShipInfo
  ...
  it('resets ship info when there is no bridge', async () => {
    delete (window as unknown as { anakotDesktop?: unknown }).anakotDesktop
    $reviewShipInfo.set({ ghReady: true, pr: { url: 'x' } } as AnakotReviewShipInfo)
