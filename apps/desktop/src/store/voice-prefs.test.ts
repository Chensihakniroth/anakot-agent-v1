...
vi.mock('@/anakot', () => ({
  getAnakotConfigRecord: vi.fn(async () => ({})),
  saveAnakotConfig: vi.fn(async () => undefined)
}))
...
it('keeps the desktop toggle local across config refreshes', async () => {
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      vi.mocked(saveAnakotConfig).mockClear()
      ...
      try {
        ...
        expect(saveAnakotConfig).not.toHaveBeenCalled()
        expect(localStorage.getItem('anakot.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
      } finally {
        write.mockRestore()
      }
    }
  ...
  for (const fails of [false, true]) {
    for (const enabled of [false, true]) {
      ...
      try {
        ...
        expect(localStorage.getItem('anakot.desktop.autoSpeakReplies')).toBeNull()
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: enabled } })
        prefs.applyAutoSpeakFromConfig({ voice: { auto_tts: !enabled } })
        expect(prefs.$autoSpeakReplies.get()).toBe(enabled)
        expect(localStorage.getItem('anakot.desktop.autoSpeakReplies')).toBe(fails ? null : String(enabled))
...
describe('applyVoiceStopPhraseFromConfig', () => {
  ...
  it('uses the first configured phrase so a custom phrase renders correctly', () => {
    applyVoiceStopPhraseFromConfig({ voice: { stop_phrases: ['goodbye anakot', 'stop'] } })
    expect($voiceStopPhrase.get()).toBe('goodbye anakot')
  })
