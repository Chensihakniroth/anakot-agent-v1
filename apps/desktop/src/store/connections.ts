...
const LAST_PROFILE_STORAGE_KEY = 'anakot.desktop.lastProfileByConnection'
...
async function rememberConnection(connectionId: string): Promise<void> {
  const setLastUsed = window.anakotDesktop?.connections?.setLastUsed
...
export async function selectConnection(connectionId: string, options: SelectConnectionOptions = {}): Promise<void> {
  ...
  // The primary local descriptor (startAnakot) historically publishes without
