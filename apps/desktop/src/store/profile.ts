...
// /api/profiles/active `current`). "default" is the root ~/.anakot. This is the
...
function profileListSource(connection: AnakotConnection | null): null | string {
  if (!connection) {
    return null
  }
...
const PROFILE_ORDER_STORAGE_KEY = 'anakot.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'anakot.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  ...
  try {
    const res = await anakotApi<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new ANAKOT_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.anakotDesktop.profile.set(name)
}
...
export function prewarmProfileBackend(name: string, connectionId: null | string = null): void {
  ...
  // tunnel and spawns `anakot -p <profile> serve --isolated` on the remote
...
async function resolveConnectionForProfile(profile: string): Promise<AnakotConnection | null> {
  const getConnection = window.anakotDesktop?.getConnection
...
export async function ensureGatewayProfile(
  ...
  // #89206 split-brain — observed live as atom 'default' over a anakot-setup
...
// previous descriptor.
async function resolveConnectionForAgent(connectionId: string, profile: string): Promise<AnakotConnection | null> {
  const getConnectionFor = window.anakotDesktop?.getConnectionFor
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'anakot.desktop.showAllProfiles'
...
export function selectProfile(name: string): void {
  ...
  void Promise.all([activateOnCurrentSource(target), shouldRememberStartupProfile])
    .then(([, shouldRemember]) => {
      if (shouldRemember) {
        return window.anakotDesktop?.profile?.remember(target)
      }
...
async function isLocalDesktopProfile(target: string): Promise<boolean> {
  const getConnectionConfig = window.anakotDesktop?.getConnectionConfig
...
// Switch to the default (root ~/.anakot) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.anakotDesktop?.touchBackend?.(target).catch(() => undefined)
}
