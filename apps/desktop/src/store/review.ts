...
import type { AnakotReviewFile, AnakotReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Anakot' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'anakot.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'anakot.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'anakot.desktop.reviewTreeMode'
const SELECTED_KEY = 'anakot.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<AnakotReviewFile[]>([])
...
export const $reviewShipInfo = atom<AnakotReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['anakotDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: AnakotReviewFile): Promise<void> {
...
function matchReviewFile(files: readonly AnakotReviewFile[], path: string): AnakotReviewFile | undefined {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.anakotDesktop?.openExternal?.(existing.url)
