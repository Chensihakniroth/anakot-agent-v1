...
export function setZoomPercent(percent: number): void {
  window.anakotDesktop?.zoom?.setPercent(percent)
}

if (typeof window !== 'undefined' && window.anakotDesktop?.zoom) {
  void window.anakotDesktop.zoom.get().then(({ percent }) => $zoomPercent.set(percent))
  window.anakotDesktop.zoom.onChanged(({ percent }) => $zoomPercent.set(percent))
}
