...
import type { AnakotGitWorktree, AnakotRepoStatus } from '@/global'
...
const EMPTY_WORKTREES: AnakotGitWorktree[] = []
...
export const $repoWorktreesByCwd = atom<Record<string, AnakotGitWorktree[]>>({})
...
export const $repoStatus: ReadableAtom<AnakotRepoStatus | null> = computed(
...
export const $repoWorktrees: ReadableAtom<AnakotGitWorktree[]> = computed(
...
const statusAtomByCwd = new Map<string, ReadableAtom<AnakotRepoStatus | null>>()
const worktreesAtomByCwd = new Map<string, ReadableAtom<AnakotGitWorktree[]>>()
const $noRepoStatus = atom<AnakotRepoStatus | null>(null)
const $noWorktrees = atom<AnakotGitWorktree[]>(EMPTY_WORKTREES)

/** Reactive status for one repo cwd (a tile's worktree). Stable per cwd. */
export function repoStatusForCwd(cwd?: null | string): ReadableAtom<AnakotRepoStatus | null> {
...
export function repoWorktreesForCwd(cwd?: null | string): ReadableAtom<AnakotGitWorktree[]> {
  const key = normalizeCwd(cwd)
...
function setRepoStatusEntry(target: string, status: AnakotRepoStatus | null): void {
...
function setRepoWorktreesEntry(target: string, worktrees: AnakotGitWorktree[]): void {
...
interface RepoStatusRefreshRequest {
  probe: (cwd: string) => Promise<AnakotRepoStatus | null>
  seq: number
  target: string
}
