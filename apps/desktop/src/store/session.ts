import type { ConnectionState } from '@anakot/shared'
...
import type { SessionInfo, UsageStats } from '@/types/anakot'
...
const COMPOSER_MODEL_KEY = 'anakot.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'anakot.desktop.composer.provider'
const COMPOSER_MODEL_SOURCE_KEY = 'anakot.desktop.composer.model-source'
const COMPOSER_EFFORT_KEY = 'anakot.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'anakot.desktop.composer.fast'
...
function composerScopeForConnection(connection: AnakotConnection | null): string | null {
  if (!connection) {
    return null
  }
...
const LAST_SESSION_KEY = 'anakot.desktop.lastSessionId'
const LAST_ROUTE_KEY = 'anakot.desktop.lastRoute'
...
function workspaceCwdKey(connection: AnakotConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(shouldPublish: () => boolean = () => true): Promise<string> {
  const settings = window.anakotDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(shouldPublish: () => boolean = () => true): Promise<void> {
  const sanitize = window.anakotDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<AnakotConnection | null>(null)
...
const SESSION_OWNER_HINTS_KEY = 'anakot.desktop.sessionOwnerHints.v1'
 ...
 * AnakotConnection lookup is unavailable. */
...
export const setConnection = (next: Updater<AnakotConnection | null>) => {
...
// until config loads, and re-seeded on every profile switch by useAnakotConfig.
