 ...
 *
 * Local models ship on main behind `--local` (either `anakot desktop --local`
 * or the flag on Anakot.exe itself). The flag is strict: without it the GUI
...
export const $localModelsEnabled = atom<boolean>(
  typeof window !== 'undefined' && window.anakotDesktop?.localModelsEnabled === true
)
