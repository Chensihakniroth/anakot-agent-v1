...
export const $pullRequestsByBranch = atom<Record<string, AnakotBranchPullRequest>>({})
...
export const $prBranchBySession = persistentAtom<Record<string, string>>(
  'anakot.desktop.prBranchBySession',
  {},
  Codecs.stringRecord
)
...
const $prScannedSessions = persistentAtom<string[]>('anakot.desktop.prScannedSessions', [], Codecs.stringArray)
...
export function pullRequestBucket(pr: AnakotBranchPullRequest | undefined): PullRequestBucket {
  if (!pr) {
    return 'none'
  }
