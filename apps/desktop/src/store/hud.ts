 ...
 * composer with the reply scrolling above it, so Anakot can be driven while
...
export const canUseHud = (): boolean =>
  typeof window !== 'undefined' && typeof window.anakotDesktop?.hud?.open === 'function'

export function openHud(sessionId?: null | string): void {
  const api = window.anakotDesktop?.hud
...
export function resetHudLayout(): void {
  void window.anakotDesktop?.hud?.resetLayout?.()
}
...
export const reportHudSession = (sessionId: null | string): void => window.anakotDesktop?.hud?.setSession?.(sessionId)
...
export function watchHudState(onClosed?: (sessionId: null | string) => void): () => void {
  const off = window.anakotDesktop?.hud?.onChanged?.(({ open, sessionId }) => {
    $hudActive.set(open)
