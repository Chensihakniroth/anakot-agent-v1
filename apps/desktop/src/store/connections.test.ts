...
beforeEach(() => {
  ...
  ensureGatewayAgent.mockImplementation(async (connectionId, profile, options) => {
    ...
    $connection.set({
      ...
      // The primary local descriptor from startAnakot() historically carried
  ...
  vi.stubGlobal('window', { anakotDesktop: { api, connections: { list, setLastUsed } }, localStorage, location: window.location })
})
...
describe('selectConnection', () => {
  ...
  it('restores the last-used profile on switch-back even when the primary local descriptor is profile-less', async () => {
    ...
    expect(JSON.parse(localStorage.getItem('anakot.desktop.lastProfileByConnection') || '{}')).toEqual({
      local: 'mac'
    })

    // A later resync republishes a profile-less primary descriptor (the
    // startAnakot shape). The remembered pair is the authority for "what was
