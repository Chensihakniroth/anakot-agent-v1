...
const ErrorRecoveryActions: FC = () => {
  ...
  // Open Logs reveals the LOCAL Electron profile's ANAKOT_HOME/logs. On a
  ...
  // ANAKOT_HOME/logs, and its parent is the Anakot data folder itself (what
  ...
  const openLocalDir = useCallback(async (resolve: (logsRoot: string) => string, failedMessage: string) => {
    try {
      const root = await window.anakotDesktop?.logsRoot?.()
      ...
      const result = await window.anakotDesktop?.openDir?.(resolve(root))
  ...
  const openAnakotFolder = useCallback(
    () => openLocalDir(root => root.replace(/[\\/]+logs[\\/]*$/, ''), copy.errorOpenAnakotFolderFailed),
    [copy.errorOpenAnakotFolderFailed, openLocalDir]
  )
  ...
  const localFolders = Boolean(window.anakotDesktop?.logsRoot)
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      {plan.openAnakotFolder && localFolders && (
        <button className="aui-error-action" onClick={() => void openAnakotFolder()} type="button">
          {copy.errorOpenAnakotFolder}
        </button>
      )}
      {plan.retry && (
