...
const ANAKOT_REF_TYPES = WIRE_REFERENCE_KINDS
type AnakotRefType = (typeof ANAKOT_REF_TYPES)[number]
...
const ANAKOT_DIRECTIVE_RE = referenceRe()
...
export const anakotDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(ANAKOT_DIRECTIVE_RE)).map(match => {
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...anakotDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Anakot directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.anakotDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.anakotDesktop?.readFileDataUrl(id)
