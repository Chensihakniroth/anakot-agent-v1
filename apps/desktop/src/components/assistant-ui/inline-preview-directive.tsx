 ...
 * own design.
 *
 * WIDGETS TALK BACK OFF-SCREEN. `window.anakot.send(prompt)` (or declarative
 * `data-anakot-send` on any clickable element) routes the prompt through the
...
const SIZE_MESSAGE_TYPE = 'anakot-inline-preview-size'
const INTENT_MESSAGE_TYPE = 'anakot-inline-preview-intent'
...
/** The script that gives the widget its ONE voice: `anakot.send(prompt)`.
 ...
 *  wires `data-anakot-send` so declarative HTML works with zero script:
 *  `<button data-anakot-send="get-price eth">ETH</button>`. */
export function intentScript(token: string): string {
  return (
    ...
    ')},"*");return true}' +
    'window.anakot={send:send};' +
    'addEventListener("click",function(e){var el=e.target&&e.target.closest?' +
    'e.target.closest("[data-anakot-send]"):null;' +
    'if(el)send(el.getAttribute("data-anakot-send")||"")},true)})()</script>'
  )
}
...
export function InlinePreviewDirective({
  ...
  // URL connection — including a same-machine `anakot serve` — renders live.)
