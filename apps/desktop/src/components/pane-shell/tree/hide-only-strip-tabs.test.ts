...
function sessionsBotsTree() {
  ...
  registerPane('anakot-bots:pane', { placement: 'left', hideOnly: true })
  registerPane('workspace', { placement: 'main', uncloseable: true })
  $layoutTree.set(
    split('row', [
      group(['sessions', 'anakot-bots:pane'], { active: 'sessions', id: 'g-side' }),
...
describe('hide-only strip tabs', () => {
  it('hides and shows a chrome tab, keeping the pane in the tree', () => {
    ...
    expect(setStripTabHidden('anakot-bots:pane', true)).toBe(true)
    expect($hiddenTreePanes.get()).toContain('anakot-bots:pane')
    expect($hiddenStripTabs.get()).toContain('anakot-bots:pane')
    // Hidden, not dismissed: the pane stays in the layout tree.
    expect(allPaneIds($layoutTree.get()!)).toContain('anakot-bots:pane')

    expect(setStripTabHidden('anakot-bots:pane', false)).toBe(true)
    expect($hiddenTreePanes.get()).not.toContain('anakot-bots:pane')
    expect($hiddenStripTabs.get()).not.toContain('anakot-bots:pane')
  })

  it('refuses to hide the zone last visible tab', () => {
    sessionsBotsTree()
    setStripTabHidden('anakot-bots:pane', true)
  ...
  it('persists hides and clears them on reveal', () => {
    ...
    const persisted = JSON.parse(window.localStorage.getItem('anakot.desktop.hiddenStripTabs.v1') ?? '[]')

    expect(persisted).toContain('anakot-bots:pane')
    ...
    revealTreePane('anakot-bots:pane')
    expect($hiddenTreePanes.get()).not.toContain('anakot-bots:pane')
    expect(window.localStorage.getItem('anakot.desktop.hiddenStripTabs.v1')).toBeNull()
  })
  ...
  it('lists the zone hide-only tabs with live hidden state for the menu', () => {
    ...
    expect(hideOnlyZoneTabs('g-side')).toEqual([
      ...
      { hidden: true, id: 'anakot-bots:pane', title: 'anakot-bots:pane' }
    ])
  ...
  it('excludes hide-only tabs from every close verb', () => {
    ...
    $layoutTree.set(group(['sessions', 'anakot-bots:pane', 'session-tile:x'], { active: 'sessions', id: 'g-mixed' }))
