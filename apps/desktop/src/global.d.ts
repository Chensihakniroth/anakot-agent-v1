import type { GatewayWsUrlResult } from '@anakot/shared'
import type { TranslucencyState } from '@anakot/shared/translucency'

import type { ScreenshotApi } from '../electron/command-screenshot-types'
import type { AnakotNotification } from '../electron/notification-types'
...
declare global {
  interface Window {
    anakotDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`anakot --tui
      ...
      // over anakot:window-control; Electron/OS chrome owns them elsewhere.
      windowControls: {
        custom: boolean
        minimize: () => void
        toggleMaximize: () => void
      ...
      // Anakot Cloud: one portal login powers discovery + silent per-agent
      ...
      readPluginSource?: (filePath: string) => Promise<AnakotReadFileTextResult>
      ...
      watchDirectory?: (dir: string) => Promise<AnakotPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setActiveWork?: (payload: AnakotActiveWork) => void
       ...
       *  (ANAKOT_GUEST_ONBOARDING=1 or --guest-onboarding). Read-only fact the
       *  main process also stamps onto every backend it spawns. */
      guestOnboardingEnabled?: boolean
      /** Launch flag: skip the first-run film (ANAKOT_SKIP_INTRO=1 or
       *  --skip-intro) so a fresh ANAKOT_HOME lands on the guided chat. */
      skipIntro?: boolean
      ...
      // Local Desktop runtime-plugin root (<ANAKOT_HOME>/desktop-plugins),
      ...
      /** LOCAL `<ANAKOT_HOME>/logs` (profile-aware) — error card "Open Logs". */
      ...
      git?: {
        ...
        repoStatus: (repoPath: string) => Promise<AnakotRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: AnakotReviewScope, baseRef?: null | string) => Promise<AnakotReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: AnakotReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<AnakotReviewShipInfo>
          ...
          prList: (repoPath: string, branches: string[], numbers?: number[]) => Promise<AnakotRepoPullRequests>
...
export interface AnakotActiveWork {
  count: number
  titles: string[]
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Anakot Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Anakot Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Anakot Cloud org (slug or id) to
...
// A discovered Anakot Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface AnakotRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
// turn began.
export type AnakotReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface AnakotReviewFile {
  path: string
  added: number
  removed: number
...
export interface AnakotReviewList {
  files: AnakotReviewFile[]
...
export interface AnakotReviewPr {
  url: string
  state: string
  number: number
}
...
export interface AnakotBranchPullRequest {
  branch: string
  draft: boolean
  number: number
...
export interface AnakotRepoPullRequests {
  ghReady: boolean
  prs: AnakotBranchPullRequest[]
}
...
export interface AnakotReviewShipInfo {
  ghReady: boolean
  pr: AnakotReviewPr | null
}
