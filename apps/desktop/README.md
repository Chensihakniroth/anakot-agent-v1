...
**The native desktop app for [Anakot Agent](../../README.md) — the self-improving AI agent from [Nous Research](https://nousresearch.com).** Same agent, same skills, same memory as the CLI and gateway, in a polished native window — chat with streaming tool output, side-by-side previews, a file browser, voice, and settings, no terminal required. Available for **macOS, Windows, and Linux**.

<table>
<tr><td><b>Chat with the full agent</b></td><td>Streaming responses, live tool activity, structured tool summaries, and the same conversation history as every other Anakot surface.</td></tr>
<tr><td><b>Side-by-side previews</b></td><td>Render web pages, files, and tool outputs in a right-hand pane while you keep chatting.</td></tr>
<tr><td><b>File browser</b></td><td>Explore and preview the working directory without leaving the app.</td></tr>
<tr><td><b>Voice</b></td><td>Talk to Anakot and hear it back.</td></tr>
...
### Install with Anakot (recommended)

Already have the Anakot CLI? Just run:

```bash
anakot desktop
```

It builds and launches the GUI against your existing install — same config, keys, sessions, and skills. If Desktop cannot find a usable runtime or saved remote connection, first launch lets you connect to an existing Anakot gateway or install Anakot locally. Local onboarding then walks you through choosing a provider and model.
...
left and right Command keys together in any app. Anakot captures that app's
frontmost window and attaches the image to the last-active Anakot composer,
...
```bash
# throwaway ANAKOT_HOME, separate Electron userData, distinct app name to avoid the single-instance lock
...
first launch it can install the Anakot Agent runtime into `ANAKOT_HOME`
(`~/.anakot`, or `%LOCALAPPDATA%\anakot` on Windows), using the same layout as a
CLI install.
...
- **Anakot Agent** runs as a headless `anakot serve` process and exposes the
...
1. `ANAKOT_DESKTOP_ANAKOT_ROOT`
2. the current source checkout during development
3. a completed managed install
4. `ANAKOT_DESKTOP_ANAKOT`, or `anakot` on `PATH`
5. a system Python that can import the Anakot runtime
...
Desktop supports a managed local backend, explicit remote gateways, and Anakot
...
screen offers **Connect to existing Anakot** before starting the local installer.
...
terminal commands, and file operations run against the remote Anakot host, not
...
transport- or Anakot-managed header names such as `Authorization`, `Cookie`,
`Host`, `Origin`, `Referer`, and `X-Anakot-Session-Token`.
...
Boot logs land in `ANAKOT_HOME/logs/desktop.log` (includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure.
...
```powershell
# Force a clean first-launch setup
Remove-Item "$env:LOCALAPPDATA\anakot\anakot-agent.anakot-bootstrap-complete"
# Rebuild a broken Python venv
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\anakot\anakot-agent\venv"
```

> The default Anakot home on Windows is `%LOCALAPPDATA%\anakot`. Set the `ANAKOT_HOME` env var if you've relocated it.
