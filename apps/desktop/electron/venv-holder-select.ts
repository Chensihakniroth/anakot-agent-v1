 ...
 *
 * The pre-update handoff kills Anakot-OWNED venv daemons (the memory plugin's
 * hindsight daemon) so the updater never races a mapped shim. External
 * holders (a user terminal running `anakot`, unrelated scripts) must NOT be
...
/**
 * True when a process is a Anakot-owned venv daemon: its exe lives under
 ...
 * spawns DETACHED — it outlives Anakot and holds venv shims mapped).
 */
export function isAnakotOwnedVenvDaemon(
