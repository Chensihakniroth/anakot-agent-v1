...
// orphan (ppid=1) — a hung remote CLI (e.g. a wedged `anakot --version`)
...
const _REDACTIONS: Array<[RegExp, string]> = [
  [/(ANAKOT_DASHBOARD_SESSION_TOKEN=)(\S+)/g, '$1<redacted>'],
  [/(X-Anakot-Session-Token["']?\s*[:=]\s*["']?)([^\s"'&]+)/gi, '$1<redacted>'],
...
// root under a short per-user base (`~/.anakot/desktop-ssh`) so even worst case
...
function defaultControlDir() {
  ...
  if (process.platform === 'win32') {
    return path.join(os.tmpdir(), 'anakot-desktop-ssh')
  }

  return path.join(os.homedir(), '.anakot', 'desktop-ssh')
}
