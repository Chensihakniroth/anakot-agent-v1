...
export const ZOOM_STORAGE_KEY = 'anakot:desktop:zoomLevel'
 ...
 * but forgot to emit 'anakot:zoom:changed', leaving the control stuck at 100%.
...
export function applyZoomLevel(webContents, level) {
  ...
  webContents.send('anakot:zoom:changed', { level: clamped, percent: zoomLevelToPercent(clamped) })
 ...
 * only learns of changes through `anakot:zoom:changed`, which never fired —
