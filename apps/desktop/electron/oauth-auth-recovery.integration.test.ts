...
async function fixture(beforeRefreshResponse?: () => Promise<void>) {
  const home = mkdtempSync(join(tmpdir(), 'anakot-auth-recovery-'))
...
test('HTTP outage, dead-refresh and cookie coexistence reach the correct ticket/readiness/media verdicts', async () => {
  ...
  try {
    ...
    try {
      await waitForAnakotReady(f.baseUrl, {
    ...
    const mediaRequest = { url: 'anakot-media://remote/%2Ftmp%2Fclip.mp4', headers: new Headers(), method: 'GET' }
