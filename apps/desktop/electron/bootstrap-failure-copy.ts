...
const BOOTSTRAP_FAILURE_REMEDY =
  'Common causes: no internet connection, antivirus blocking the installer, or another copy of Anakot running. ' +
  'Close other Anakot windows and choose Reload and retry; if it fails again, open the logs and send them to support.'
...
export function describeBootstrapFailure(failedStage: string | null | undefined, rawError: unknown): string {
  ...
  const lead = label
    ? `Setting up Anakot stopped during the '${label}' step.`
    : 'Setting up Anakot stopped before it could finish.'
...
/**
 * Error.message for an installed Anakot with a piece missing (source tree,
 * Python environment). The renderer's install overlay offers the Repair install
 * button ('anakot:bootstrap:repair'), so the copy points there. `whatIsMissing`
...
export function missingInstallPartMessage(whatIsMissing: string): string {
  return (
    "Part of Anakot' installation is missing (it may have been deleted or quarantined by antivirus). " +
