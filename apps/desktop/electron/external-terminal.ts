// Launching the Anakot TUI in the user's OWN terminal emulator.
//
...
// `anakot --tui --resume <id>` there. Two problems have to be solved for that
// to work anywhere:
//
//  1. WHAT to run. The desktop's Anakot runtime is often a venv Python invoked
//     as `python -m anakot_cli.main`, not a `anakot` on PATH — so the command
 ...
 * headless child) is the wrong answer for an interactive terminal. The Anakot
