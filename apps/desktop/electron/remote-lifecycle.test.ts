...
function fakeSsh(rules: any[] = []) {
  ...
  return {
    ...
    async exec(cmd) {
      ...
      if (cmd.includes('.anakot-update-in-progress') && !cmd.includes('marker_clear()') && !/setsid|nohup/.test(cmd)) {
        return 'CLEAR'
      }
...
test('POSIX relaunch gate refuses live and uncertain install markers without executing Anakot', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = {
      async exec(command) {
        ...
        if (command.includes('ANAKOT_HOME')) {
          return '/home/alice/.anakot\n'
        }

        if (command.includes('.anakot-update-in-progress')) {
          return observation
        }
...
test('POSIX relaunch gate permits absent/dead markers and normalizes named-profile homes install-wide', async () => {
  ...
  await assertRemoteInstallUpdateClear(ssh, '/home/alice/.anakot/profiles/research')
  assert.match(commands[0], /home\.parent\.name/)
  assert.match(commands[0], /profiles/)
  assert.match(commands[0], /.anakot-update-in-progress/)
})
...
test('listRemoteAnakotProfiles inventories Mini-style profile dirs without spawning a dashboard', async () => {
  const ssh = fakeSsh([
    [/ANAKOT_HOME/, '/Users/zillajr/.anakot\n'],
    [/ls -1/, 'bob\ndixie\ngoose\nrambo\nbob.rollback-old\n']
  ])

  assert.deepEqual(await listRemoteAnakotProfiles(ssh), ['default', 'bob', 'dixie', 'goose', 'rambo'])
  assert.equal(
...
test('listRemoteAnakotProfiles rejects a hostile ANAKOT_HOME', async () => {
  const ssh = fakeSsh([[/ANAKOT_HOME/, '/tmp/x; echo pwned\n']])

  await assert.rejects(
    () => listRemoteAnakotProfiles(ssh),
    (err: any) => {
...
test('locateAnakot prefers the explicit profile path when executable', async () => {
  const ssh = fakeSsh([[/\[ -x .*\/opt\/anakot/, 'OK']])
  assert.equal(await locateAnakot(ssh, '/opt/anakot'), '/opt/anakot')
})

test('locateAnakot throws (no silent fallback) when an EXPLICIT path is not executable', async () => {
  // command -v WOULD find a different install, but an explicit path must not
  // silently fall back to it — that is the "connected to the wrong anakot" bug.
  const ssh = fakeSsh([
    [/command -v anakot/, '/home/u/.local/bin/anakot\n'],
    [/\[ -x .*\.local\/bin\/anakot/, 'OK']
  ])

  await assert.rejects(
    () => locateAnakot(ssh, '/bad/path/anakot'),
    (err: any) => {
      assert.equal(err.kind, 'anakot-not-found')
      assert.match(err.message, /\/bad\/path\/anakot/)
...
test('locateAnakot falls back to the login-shell command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v anakot/, '/home/u/.local/bin/anakot\n'],
    [/\[ -x .*\.local\/bin\/anakot/, 'OK']
  ])

  assert.equal(await locateAnakot(ssh, ''), '/home/u/.local/bin/anakot')
})

test('locateAnakot preserves an installer wrapper instead of resolving its interpreter', async () => {
  // install.sh venv mode writes: exec "$ANAKOT_BIN" "$ANAKOT_ENTRYPOINT" "$@",
  // where $ANAKOT_BIN is the venv python. The old canonicalization returned
  ...
  const ssh = fakeSsh([
    ...
    [/python3 -c/, '/home/u/.anakot/anakot-agent/venv/bin/python\n']
  ])

  assert.equal(await locateAnakot(ssh, ''), '/home/u/.local/bin/anakot')
  assert.ok(
    !ssh.calls.some(cmd => cmd.includes('python3 -c')),
    'locateAnakot must not shell out to a python3 parser to rewrite the launcher'
  )
})

test('locateAnakot returns an explicit remoteAnakotPath unchanged', async () => {
  // The override half of #74411: an explicit remoteAnakotPath pointing at a
  // wrapper was also canonicalized to its interpreter, so overriding to
  // ~/.local/bin/anakot changed nothing for affected users.
  const ssh = fakeSsh([
    [/\[ -x .*\.local\/bin\/anakot/, 'OK'],
    [/python3 -c/, '/home/u/.anakot/anakot-agent/venv/bin/python\n']
  ])

  assert.equal(await locateAnakot(ssh, '~/.local/bin/anakot'), '~/.local/bin/anakot')
  assert.ok(!ssh.calls.some(cmd => cmd.includes('python3 -c')), 'an explicit remoteAnakotPath must never be rewritten')
})

test('locateAnakot falls back to ~/.local/bin/anakot when the login-shell probe misses', async () => {
  ...
  assert.equal(await locateAnakot(ssh, ''), '~/.local/bin/anakot')
})

test('locateAnakot tries the conventional venv path last', async () => {
  const ssh = fakeSsh([[/\[ -x .*venv\/bin\/anakot/, 'OK']])
  assert.equal(await locateAnakot(ssh, ''), '~/.anakot/anakot-agent/venv/bin/anakot')
})

test('locateAnakot throws a anakot-not-found error with an install hint', async () => {
...
test('locateAnakot uses a login shell for the command -v probe', async () => {
  const ssh = fakeSsh([
    [/command -v anakot/, '/x/anakot'],
    [/\[ -x/, 'OK']
  ])

  await locateAnakot(ssh, '')
  assert.ok(
...
test('ownership paths are isolated by ownership ID and spawn nonce', () => {
  assert.equal(ownershipDirectory(OWNERSHIP_ID), `~/.anakot/desktop-ssh/${OWNERSHIP_ID}`)
  assert.equal(lockfilePath(OWNERSHIP_ID), `~/.anakot/desktop-ssh/${OWNERSHIP_ID}/backend.lock.json`)
  assert.equal(spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE), `~/.anakot/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.log`)
})
...
test('metadata and process proof transport failures remain indeterminate', async () => {
  ...
  await assert.rejects(
    () => pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, failure]]), 5, SPAWN_NONCE, '/x/anakot'),
    (error: any) => error.kind === 'transient-transport-error'
  )
})

test('pidIsOurDashboard requires the exact serve ownership nonce', async () => {
  const ours = `/x/anakot serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'OWNED\n']]), 5, SPAWN_NONCE, '/x/anakot'), true)
  assert.equal(
...
test('pidIsOurDashboard accepts the venv entrypoint an installer wrapper execs into', async () => {
  ...
  assert.equal(
    await pidIsOurDashboard(ssh, 5, SPAWN_NONCE, '~/.local/bin/anakot', '/Users/cd9c/.anakot', OWNERSHIP_ID, 'ops'),
    true
  )
  assert.match(ownershipProbe, /anakot-agent.*venv.*bin.*anakot/)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    const temp = await mkdtemp(path.join(os.tmpdir(), 'anakot wrapper ownership '))
    ...
    const entrypoint = path.join(installDir, 'anakot')
    const launcher = path.join(temp, 'anakot launcher')
    ...
    try {
      ...
      assert.equal(
        await pidIsOurDashboard(ssh, child.pid, SPAWN_NONCE, launcher, '/unrelated/anakot-home', OWNERSHIP_ID, 'ops'),
        true
      )
      assert.equal(
        await pidIsOurDashboard(
...
test('buildSpawnCommand is headless serve, detached, token not in argv', () => {
  const cmd = buildSpawnCommand('/x/anakot', 'work', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  ...
  assert.ok(!cmd.includes('ANAKOT_DASHBOARD_SESSION_TOKEN'), 'token env var must not appear')
})
...
test('buildSpawnCommand atomically reserves the ownership slot through spawn and lock publication', () => {
  const cmd = buildSpawnCommand('/x/anakot', 'work', {
    anakotHome: '~/.anakot',
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    ownershipId: OWNERSHIP_ID,
  ...
  assert.ok(cmd.includes('.anakot-update-in-progress.mutex'))
  assert.match(cmd, /fcntl\.flock\(fd,fcntl\.LOCK_EX\)/)
  assert.match(cmd, /os\.O_CLOEXEC/)
  assert.match(
    cmd,
    /subprocess\.run\(\["sh","-c",payload,"anakot-update-mutex",str\(fd\)\],pass_fds=\(fd,\),check=False\)/
  )
  assert.doesNotMatch(cmd, /os\.set_inheritable\(fd,True\)/)
  assert.match(cmd, /anakot-update-child "\$1"/)
...
test.skipIf(process.platform === 'win32')('detached backend does not inherit the update mutex descriptor', async () => {
  const directory = await mkdtemp(path.join(os.tmpdir(), 'anakot-update-mutex-'))
  const anakotPath = path.join(directory, 'anakot')
...
for fd in /proc/$$/fd/*; do
  ...
  case "$target" in
    *anakot-update-in-progress.mutex) printf '%s\\n' "$target" >> ${reportPath} ;;
  esac
done
`,
      { mode: 0o700 }
    ...
    const command = buildSpawnCommand(anakotPath, '', {
      anakotHome: path.join(directory, 'home'),
      logPath
    })
...
test('spawnRemoteDashboard always spawns serve (legacy dashboard path removed)', async () => {
  ...
  await spawnRemoteDashboard(ssh, { anakotPath: '/x/anakot', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID })
...
test('READY_RE accepts both serve and dashboard sentinels', () => {
  assert.equal(READY_RE.exec('ANAKOT_BACKEND_READY port=4321')?.[1], '4321')
  assert.equal(READY_RE.exec('ANAKOT_DASHBOARD_READY port=8765')?.[1], '8765')
  // The remote log is `>> log 2>&1`, so a stderr chunk without a newline can be spliced onto the sentinel.
  assert.equal(READY_RE.exec('INFO  Started server process [4711]ANAKOT_BACKEND_READY port=65238')?.[1], '65238')
})
...
test('spawnRemoteDashboard rejects when no pid is returned', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { anakotPath: '/x/anakot', profile: '', token: 't', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('scrapeReadyPort reads only the named spawn log', async () => {
  ...
  const ssh = fakeSsh([[/cat/, 'some noise\nANAKOT_DASHBOARD_READY port=51234\n']])
...
test('connect() spawns fresh when there is no lockfile, adopts the served token', async () => {
  const ssh = fakeSsh([
    ...
    [/setsid/, '777\n'],
    [/kill -0 777/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=51999\n']
  ])
...
test('managed SSH maps a local scope to a different non-default remote profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '778\n'],
    [/kill -0 778/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_BACKEND_READY port=52000\n']
  ])
  ...
  assert.match(spawn, /.anakot\/desktop-ssh\/[0-9a-f]{32}\/[0-9a-f]{16}\.token/)
...
test('connect() respawns when the requested remote profile differs from the lockfile profile', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/kill 333/, ''],
    [/--version/, 'Anakot Agent v0.18.2\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/setsid/, '890\n'],
    [/kill -0 890/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=52050\n']
  ])
...
test('connect() respawns when the lockfile anakotPath differs from the resolved path', async () => {
  const reuseToken = 'stored-token'
  const lock = ownedLock({ anakotPath: '/old/stale/anakot', tokenFingerprint: fingerprintToken(reuseToken) })
  ...
  const result = await connect(
    connectDeps(ssh, { reuseToken, remoteAnakotPath: '/new/anakot', adoptServedToken: async () => 'fresh' })
  )
...
test('connect() respawns when the lockfile protocolVersion is incompatible', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '901\n'],
    [/kill -0 901/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=44100\n']
  ])
...
test('connect() fresh spawn writes anakotHome + protocolVersion into the lockfile', async () => {
  const writes: string[] = []
  ...
  const ssh = fakeSsh([
    ...
    [/\[ -x/, 'OK'],
    [/cat .*lock\.json/, ''], // no lockfile
    [/ANAKOT_HOME/, '/home/alice/.anakot\n'],
    [/grep -q ssh-session-token-file/, 'YES\n'],
    [/python3 -c/, ''],
    [/printf '%s\\n'/, ''],
    [/setsid/, '700\n'],
    [/kill -0 700/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=45500\n'],
    [
      /printf '%s' '/,
      c => {
        writes.push(c)
  ...
  assert.match(lockWrite, /"anakotHome":"\/home\/alice\/.anakot"/)
})
...
test('connect() respawns when the lockfile pid is dead (killed dashboard)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '888\n'],
    [/kill -0 888/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=42000\n']
  ])
...
test('connect() respawns when the dashboard is wedged (alive pid, probe fails)', async () => {
  ...
  const ssh = fakeSsh([
    ...
    [/setsid/, '999\n'],
    [/kill -0 999/, 'ALIVE'],
    [/cat .*\.log/, 'ANAKOT_DASHBOARD_READY port=43000\n']
  ])
...
test('validateRemotePath accepts absolute POSIX paths', () => {
  assert.doesNotThrow(() => validateRemotePath('/usr/bin/anakot'))
  assert.doesNotThrow(() => validateRemotePath('/home/user/.anakot/anakot-agent/venv/bin/anakot'))
})

test('validateRemotePath accepts ~/ prefix paths', () => {
  assert.doesNotThrow(() => validateRemotePath('~/bin/anakot'))
  assert.doesNotThrow(() => validateRemotePath('~/.anakot/logs/desktop-ssh.log'))
  assert.doesNotThrow(() => validateRemotePath('~'))
})

test('validateRemotePath accepts paths with spaces and quotes', () => {
  assert.doesNotThrow(() => validateRemotePath('/home/user/my project/anakot'))
...
test('validateRemotePath rejects relative paths', () => {
  assert.throws(() => validateRemotePath('anakot'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('./bin/anakot'), /absolute|relative/i)
  assert.throws(() => validateRemotePath('../etc/passwd'), /absolute|relative/i)
})

test('validateRemotePath rejects NUL and newline', () => {
  assert.throws(() => validateRemotePath('/usr/bin/anakot\x00'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/anakot\n'), /unsafe/i)
  assert.throws(() => validateRemotePath('/usr/bin/anakot\r'), /unsafe/i)
})

test('validateRemotePath preserves shell metacharacters as path data', () => {
  for (const p of ['/usr/$(whoami)/anakot', '/usr/`id`/anakot', '/usr/a;b|c&d<e>f']) {
...
test('expandRemotePath expands ~/ to "$HOME"/', () => {
  const result = expandRemotePath('~/.anakot/logs/desktop-ssh.log')
...
test('expandRemotePath returns quoted absolute paths unchanged', () => {
  const result = expandRemotePath('/usr/local/bin/anakot')
  assert.ok(result.includes('/usr/local/bin/anakot'))
  assert.ok(!result.includes('eval'))
})

test('expandRemotePath preserves spaces as data', () => {
  const result = expandRemotePath('/home/user/my project/anakot')
...
test('buildSpawnCommand does not embed the token in the command string', () => {
  ...
  assert.ok(!cmd.includes('ANAKOT_DASHBOARD_SESSION_TOKEN'), 'env var name must not appear')
})

test('buildSpawnCommand includes --ssh-session-token-file when tokenFilePath is provided', () => {
  const cmd = buildSpawnCommand('/x/anakot', 'work', {
    tokenFilePath: `~/.anakot/desktop-ssh/${OWNERSHIP_ID}/${SPAWN_NONCE}.token`,
    logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE),
    spawnNonce: SPAWN_NONCE
  })

  assert.match(cmd, /--ssh-session-token-file/)
  assert.match(cmd, /.anakot\/desktop-ssh\//)
})

test('buildSpawnCommand always uses serve, never dashboard', () => {
  const cmd = buildSpawnCommand('/x/anakot', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /serve --isolated/)
  assert.doesNotMatch(cmd, /\bdashboard\b/)
  assert.doesNotMatch(cmd, /--skip-build/)
...
test('buildSpawnCommand raises the SSH child file limit before execing Anakot', () => {
  const cmd = buildSpawnCommand('/x/anakot', '', { logPath: spawnLogPath(OWNERSHIP_ID, SPAWN_NONCE) })
  assert.match(cmd, /ulimit -n 65536 2>\/dev\/null \|\| true; exec env ANAKOT_DESKTOP=1/)
...
test('spawnRemoteDashboard removes a token file when upload reporting fails', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { anakotPath: '/x/anakot', profile: '', token: 'tok', ownershipId: OWNERSHIP_ID }),
    /channel closed/
  )
...
test('spawnRemoteDashboard fails with update-required when remote lacks --ssh-session-token-file', async () => {
  ...
  await assert.rejects(
    () => spawnRemoteDashboard(ssh, { anakotPath: '/x/anakot', profile: '', token: 'tk', ownershipId: OWNERSHIP_ID }),
    (err: any) => {
...
test('readLockfile treats a log path outside the exact ownership and spawn path as skew', async () => {
  const lock = ownedLock({ logPath: '~/.anakot/desktop-ssh/other.log' })
...
test('cleanupStale never deletes a lock-supplied unexpected log path', async () => {
  ...
  await cleanupStale(ssh, OWNERSHIP_ID, ownedLock({ logPath: '~/.anakot/unrelated.log' }))
...
test('pidIsOurDashboard requires an exact nonce option value', async () => {
  const prefix = `/x/anakot serve --isolated --ssh-owner-nonce ${SPAWN_NONCE}ff`
  const suffix = `/x/anakot serve --isolated --ssh-owner-nonce xx${SPAWN_NONCE}`
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/anakot'), false)
  assert.equal(await pidIsOurDashboard(fakeSsh([[/print\("OWNED"/, 'FOREIGN\n']]), 5, SPAWN_NONCE, '/x/anakot'), false)
})
...
test('remote SSH ownership capability requires both secure bootstrap flags', async () => {
  ...
  assert.equal(await remoteSupportsSshOwnership(supported, '/x/anakot'), true)
  ...
  assert.equal(await remoteSupportsSshOwnership(unsupported, '/x/anakot'), false)
})
...
test.skipIf(process.platform === 'win32')('capability probe survives a zsh login shell on the remote (#111949)', async t => {
  ...
  const dir = await mkdtemp(path.join(os.tmpdir(), 'anakot-zsh-probe-'))

  try {
    const anakot = path.join(dir, 'anakot')
    await writeFile(anakot, '#!/bin/sh\necho "--ssh-session-token-file --ssh-owner-nonce"\n', { mode: 0o700 })

    const ssh = { exec: async (command: string) => (await exec(command, { shell: zsh })).stdout }

    assert.equal(await remoteSupportsSshOwnership(ssh, anakot), true)
  } finally {
...
test('probes run under the remote watchdog so a hung CLI cannot orphan (#110478)', async () => {
  ...
  const versionSsh = fakeSsh([
    [
      ...
      (cmd: string) => {
        ...
        return 'Anakot Agent v0.18.2 (abc123)\n'
      }
    ]
  ])

  assert.equal(await probeAnakotVersion(versionSsh, '/x/anakot'), 'Anakot Agent v0.18.2 (abc123)')
  ...
  assert.equal(await remoteSupportsSshOwnership(helpSsh, '/x/anakot'), true)
...
test.skipIf(process.platform === 'win32')(
  ...
  async () => {
    ...
    const root = await mkdtemp(path.join(os.tmpdir(), 'anakot-argv-shim-'))
    ...
    try {
      ...
      assert.equal(
        argv[2],
        `${fakeHome}/.anakot/.anakot-update-in-progress.mutex`,
      ...
      const base = `${fakeHome}/.anakot/desktop-ssh/${OWNERSHIP_ID}`
...
test('connect() post-spawn cleanup that cannot prove ownership keeps the original boot error', async () => {
  ...
  await assert.rejects(
    connect(connectDeps(ssh, { platform: { os: 'Linux', arch: 'x86_64' }, waitForAnakot: async () => { throw boot } })),
