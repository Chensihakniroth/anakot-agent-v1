 ...
 *
 * The staged Tauri `anakot-setup.exe` has no self-update path, so every
 ...
 * checkout instead: every `anakot update` refreshes the code that drives the
 ...
 * and QUITS, the script waits it out, runs `anakot update`, swaps/relaunches
 * the app, and writes .anakot-update-result.json. With the app gone before
 * the update starts, the ANAKOT_DESKTOP_CHILD_PID reaper-exclusion dance is
 ...
 * `anakot update` card — old checkouts pull the script on their next update).
 */
 ...
 *
 * The Tauri installer self-copies into ANAKOT_HOME on *every* platform
 * (`anakot-setup.exe` on Windows, `anakot-setup` elsewhere — see
 * apps/bootstrap-installer `paths::installer_dest` and
 * `bootstrap::copy_self_to_anakot_home`), so finding that binary on macOS or
 ...
 * than the hand-off protocol holds the update marker, spawns `anakot update`,
...
export function resolveStagedUpdaterBinary(
  ...
  const candidate = path.join(anakotHome, 'anakot-setup.exe')
 ...
 *
 * `copy_self_to_anakot_home` deliberately no-ops during `--update`
 ...
 * "Another Anakot update is already running (PID <itself>, started 1s ago)" —
 * the observed infinite "Install didn't finish" loop. Skipping the pre-write
 * for those binaries lets them acquire cleanly and run `anakot update`, which
 ...
 * the installer is written to ANAKOT_HOME at install/repair time, making mtime
 ...
 * The lead sentence is plain: nothing changed and Anakot keeps running. The raw
...
export function describeUpdaterHandoffFailure(outcome: Pick<UpdaterHandoffOutcome, 'message'>): string {
  const lead =
    "The updater couldn't start, so nothing was changed and Anakot keeps running as before. " +
