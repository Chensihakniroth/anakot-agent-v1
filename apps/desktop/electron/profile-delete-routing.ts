// Profile-delete routing logic for the `anakot:api` IPC handler.
//
...
// be rejected: spawning a fresh backend would call ensure_anakot_home() and
...
/** Parse a `anakot:api` request into the profile name a DELETE targets. */
 ...
 * still removing the profile and recreate its ANAKOT_HOME.
 *
...
/**
 * Route the next `anakot:api` request away from the primary/window backend
 ...
 * ensure_anakot_home() recreates the directory the delete just removed.
 */
