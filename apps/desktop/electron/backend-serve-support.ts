...
// spawns `anakot serve`; runtimes older than serve only have `dashboard`, so
...
// (covers a bare `anakot` resolved from PATH with no known source root). Result
...
export function createBackendServeSupportResolver(anakotHome: string, rememberLog: (message: string) => void) {
    ...
    const pending = (async () => {
      ...
      if (backend.root) {
        try {
          const src = await fs.promises.readFile(
            path.join(backend.root, 'anakot_cli', 'subcommands', 'dashboard.py'),
            'utf8'
          )
      ...
      if (supported === null) {
        try {
          ...
          // `anakot --version` (~10.5s measured cold), and a false negative here
          ...
          await execProbe(backend.command, [...prefix, 'serve', '--help'], {
            ...
            env: { ...process.env, ANAKOT_HOME: anakotHome, ...(backend.env || {}) },
            timeout: PROBE_TIMEOUT_MS,
            stdio: 'ignore',
            // `.cmd`/`.bat` shim backends carry shell: true in their descriptor
            // (see resolveAnakotBackend step 4); execFileSync of a .cmd without
