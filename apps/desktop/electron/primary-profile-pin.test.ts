...
test('a live primary keeps answering for its booted profile after the preference moves', () => {
  ...
  // startAnakot() boots the primary as the preference at that moment.
  ...
  // anakot:profile:remember rewrites active-profile.json without re-homing.
  preference = 'claude'
  ...
  // spawned for the same ANAKOT_HOME.
  assert.equal(
    pin.resolve(() => preference),
    'default'
  )
