...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'anakot serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/anakot serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m anakot_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/anakot --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Anakot Runtime\\anakot.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('anakot chat --query serve'), false)
