 ...
 *   1. ANAKOT_GIT_BASH_PATH env var override
 *   2. PortableGit under %LOCALAPPDATA%\anakot\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  // Respect ANAKOT_GIT_BASH_PATH if set (mirrors tools/environments/local.py:_find_bash).
  const gitBashPath = env.ANAKOT_GIT_BASH_PATH
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'anakot', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'anakot', 'git', 'usr', 'bin', 'bash.exe'))
  }
