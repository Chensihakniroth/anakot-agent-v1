...
//   ANAKOT_WIN_SSH_HOST   ssh alias/host of the Windows box
//   ANAKOT_WIN_SSH_USER   remote user
//   ANAKOT_WIN_SSH_ANAKOT absolute path to the remote anakot.exe under test
const liveHost = process.env.ANAKOT_WIN_SSH_HOST || ''
const liveUser = process.env.ANAKOT_WIN_SSH_USER || ''
const configuredAnakot = process.env.ANAKOT_WIN_SSH_ANAKOT || ''
const ownershipId = '89abcdef0123456789abcdef01234567'

function fetchJson(url, token, path) {
  return fetch(`${url}${path}`, { headers: { 'X-Anakot-Session-Token': token } }).then(async response => {
    if (!response.ok) {
...
test.skipIf(!liveHost || !liveUser || !configuredAnakot)(
  ...
  async () => {
    ...
    try {
      ...
      const runtimeScript = `& '${configuredAnakot.replace('anakot.exe', 'python.exe')}' -m anakot_cli.windows_ssh_runtime read-lock '${ownershipId}'`
      ...
      if (lock) {
        const python = configuredAnakot.replace('anakot.exe', 'python.exe')
        const terminate = `& '${python}' -m anakot_cli.windows_ssh_runtime terminate '${lock.pid}' '${lock.creationTimeNs}' '${lock.anakotPath}' '${lock.spawnNonce}'`
        await ssh.exec(`powershell.exe -NoProfile -NonInteractive -Command "${terminate}"`)
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m anakot_cli.windows_ssh_runtime remove-lock '${ownershipId}'"`
        )
        await ssh.exec(
          `powershell.exe -NoProfile -NonInteractive -Command "& '${python}' -m anakot_cli.windows_ssh_runtime remove-log '${ownershipId}' '${lock.spawnNonce}'"`
        )
      }
