...
import { canImportAnakotCli, PROBE_TIMEOUT_MS, shouldTrustAnakotOverride, verifyAnakotCli } from './backend-probes'
...
import { isAnakotOwnedVenvDaemon } from './venv-holder-select'
...
const USER_DATA_OVERRIDE = process.env.ANAKOT_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.ANAKOT_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.ANAKOT_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with ANAKOT_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[anakot] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (DEV_CDP.port) {
  ...
  console.log(
    `[anakot] renderer debugging on http://127.0.0.1:${DEV_CDP.port} — anything that can reach it ` +
      'can run code in the renderer. ANAKOT_DESKTOP_CDP_PORT=off to disable.'
  )
} else {
  const why = describeDevCdpDecision(DEV_CDP)

  if (why) {
    console.warn(`[anakot] ${why}`)
  }
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[anakot] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}
...
// it). The value arrives via ANAKOT_DESKTOP_PASSWORD_STORE, bridged by the
// `anakot desktop` launcher from detection or `desktop.password_store` in
...
if (PASSWORD_STORE.warning) {
  console.warn(`[anakot] ${PASSWORD_STORE.warning}`)
}

if (PASSWORD_STORE.store) {
  app.commandLine.appendSwitch('password-store', PASSWORD_STORE.store)
  console.log(`[anakot] using password-store backend: ${PASSWORD_STORE.store}`)
}
...
// never go through `anakot desktop`; it is version-scoped so an app update
...
if (IS_WINDOWS) {
  ...
  // expose Anakot sessions/config to every packaged app on the machine.
  if (shouldAttemptAclRepair(priorMarker)) {
    const exeDir = path.dirname(process.execPath)
    const acl = grantAllApplicationPackagesAcl(exeDir, { execFileSync })

    if (acl.ok) {
      console.log(`[anakot] granted ALL APPLICATION PACKAGES RX on ${exeDir} (#38216)`)
    } else if (acl.error && acl.error !== 'missing-target-or-exec') {
      console.warn(`[anakot] AppContainer ACL grant failed on ${exeDir}: ${acl.error}`)
    }
  }
  ...
  if (sandboxDecision.enable && sandboxDecision.reason !== 'already-enabled') {
    ...
    console.log(
      `[anakot] Windows sandbox fallback enabled (${sandboxDecision.reason}); launching with --no-sandbox (#38216)`
    )
  }
  ...
  app.on('child-process-gone', (_event, details) => {
    ...
    console.warn(
      `[anakot] Windows GPU sandbox crashed (exit=${details?.exitCode}); relaunching once with --no-sandbox (#38216)`
    )
    ...
    try {
      ...
      console.error(`[anakot] --no-sandbox relaunch failed: ${error?.message || error}`)
    }
  })
}

ipcMain.handle('anakot:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
...
// always-on timers. A completely idle, minimized Anakot burned ~20% CPU
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[anakot] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[anakot] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[anakot] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[anakot] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// ANAKOT_HOME — the user-facing root for everything Anakot-related. Mirrors
// scripts/install.ps1's $AnakotHome and scripts/install.sh's $ANAKOT_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\anakot (matches install.ps1)
//   macOS / Linux: ~/.anakot (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.anakot directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\anakot yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// ANAKOT_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// ANAKOT_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.anakot / %LOCALAPPDATA%\anakot.
function resolveAnakotHome() {
  if (process.env.ANAKOT_HOME) {
    return normalizeAnakotHomeRoot(process.env.ANAKOT_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'anakot-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a ANAKOT_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\anakot and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('ANAKOT_HOME')

    if (fromRegistry) {
      return normalizeAnakotHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'anakot')
    const legacy = path.join(app.getPath('home'), '.anakot')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.anakot setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.anakot')
}

const ANAKOT_HOME = resolveAnakotHome()

function pathWithAnakotManagedNode(...entries) {
  const managed = anakotManagedNodePathEntries(ANAKOT_HOME).filter(directoryExists)

  return [...managed, ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_ANAKOT_ROOT — the canonical mutable Anakot install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_ANAKOT_ROOT = path.join(ANAKOT_HOME, 'anakot-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_ANAKOT_ROOT, 'venv')
...
// boots, going straight to `resolveAnakotBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_ANAKOT_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_ANAKOT_ROOT, '.anakot-bootstrap-complete')
...
// _apply_profile_override in anakot_cli/main.py) and bypasses the sticky
// ~/.anakot/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors anakot_cli.profiles._PROFILE_ID_RE so we never hand the backend a
...
const DESKTOP_LOG_PATH = path.join(ANAKOT_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (anakot_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.ANAKOT_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_ERROR = process.env.ANAKOT_DESKTOP_BOOT_FAKE_ERROR || ''
...
const SKIP_QUIT_CONFIRM = process.env.ANAKOT_DESKTOP_SKIP_QUIT_CONFIRM === '1'
// Nous free tier gate, decided ONCE here and stamped onto every backend spawn
// (desktopBackendSpawnEnv) and the renderer (anakot:launch-flags).
const GUEST_ONBOARDING = guestOnboardingEnabled()
const SKIP_INTRO = skipIntroEnabled()

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.ANAKOT_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.ANAKOT_DESKTOP_APP_NAME || 'Anakot'
...
// 'anakot:native-theme' ('dark' | 'light' | 'system'); we pin
...
// anakot:window-control IPC channel. See titleBarOverlayOptions.
...
// electron-builder derives from the build `appId` (com.nousresearch.anakot) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.anakot')
}
...
app.setAboutPanelOptions({
  ...
  applicationVersion: resolveAnakotVersion(),
...
// counts startAnakot() calls that have not settled; `primaryRecoverySuppressed`
// is set by every intentional invalidate of the slot and cleared by the next
// startAnakot(), so the dying child's stale exit never respawns behind a
...
// startAnakot(); this pool only holds EXTRA profile
...
// The legacy ANAKOT_DESKTOP_POOL_* env vars remain the initial-value fallback
...
function readPersistedPoolLimits() {
  try {
    ...
    const fromEnv = clampPoolLimits({
      maxBackends: Number(process.env.ANAKOT_DESKTOP_POOL_MAX) || undefined,
      idleMs: Number(process.env.ANAKOT_DESKTOP_POOL_IDLE_MS) || undefined
    })
...
function logPoolSpawnFailure(label: string, error: unknown): void {
  ...
  if (isBackgroundSlotWaitTimeout(error)) {
    ...
    rememberLog(
      `Anakot backend for profile ${label} failed to start: ${error instanceof Error ? error.message : String(error)}`
    )
  }
}
...
//   * WSL2 IPC stall  = the renderer's `anakot:backend:touch` roundtrips
...
const POOL_KEEPALIVE_FRESH_MS = Math.max(
  120_000,
  Number(process.env.ANAKOT_DESKTOP_POOL_KEEPALIVE_FRESH_MS) || 4 * 60_000
)
...
// onto the error so subsequent startAnakot() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning anakot serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Anakot backend',
  phase: 'idle',
  progress: 0,
  retryable: false,
  running: false,
...
function rememberLog(chunk) {
  ...
  anakotLog.push(...lines)

  if (anakotLog.length > 300) {
    anakotLog.splice(0, anakotLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-anakot-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('anakot:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the anakot:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('anakot:bootstrap:event', ev)
}
...
function promptFirstRunSetupChoice(backend) {
  broadcastBootstrapEvent({
    ...
    activeRoot: backend.activeRoot || ACTIVE_ANAKOT_ROOT
  })
}
...
// The Tauri updater writes ANAKOT_HOME/.anakot-update-in-progress for the whole
...
// (`force_kill_other_anakot`, taskkill /IM anakot.exe) kills it, the launch
...
function updateGateDeps() {
  return {
    hasLiveMarker: () => Boolean(readLiveUpdateMarker(ANAKOT_HOME)),
...
const BUNDLE_SWAP_RELAUNCH_FLAG = '--anakot-bundle-swap-relaunched'
...
// The detached updater swaps the packaged bundle on disk AFTER `anakot update`
...
async function waitForUpdateToFinish() {
  ...
  // "nothing happened").
  try {
    const result = readAndConsumeHandoffResult(ANAKOT_HOME)
    ...
    if (result && result.ok && result.manual) {
      ...
      const handoffLogPath = path.join(ANAKOT_HOME, 'logs', 'desktop-update-handoff.log')
      ...
      // and the same reveal primitive as 'anakot:logs:reveal'.
      void dialog
        .showMessageBox({
          type: 'error',
          title: 'Anakot update',
          message: "Anakot couldn't finish updating",
          detail:
  ...
  if (outcome === 'timeout') {
    ...
    await advanceBootProgress('backend.update-restart', 'Restarting Anakot to load the updated app…', 14)
...
function findOnPath(command) {
  ...
  // shell-script shim named `anakot` — must not shadow `anakot.cmd`/`anakot.exe`.
...
async function unwrapWindowsVenvAnakotCommand(command, backendArgs) {
  return resolveVenvAnakotCommand(command, backendArgs, {
    isWindows: IS_WINDOWS,
    isCommandScript,
    fileExists,
    directoryExists,
...
// spawns `anakot serve`; runtimes older than serve only have `dashboard`. We
...
const backendSupportsServe = createBackendServeSupportResolver(ANAKOT_HOME, rememberLog)
...
function isAnakotSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'anakot_cli', 'main.py'))
}

async function findPythonForRoot(root) {
  const override = process.env.ANAKOT_DESKTOP_PYTHON
...
async function findSystemPython() {
  ...
  // CPython but several Anakot deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Resolves ANAKOT_GIT_BASH_PATH
...
// normal ANAKOT_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\anakot\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'anakot', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'anakot', 'git', 'bin', 'git.exe'))
  }
...
function recentAnakotLog() {
  return anakotLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's anakot root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_ANAKOT_ROOT.
// ANAKOT_DESKTOP_ANAKOT_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.ANAKOT_DESKTOP_ANAKOT_ROOT && path.resolve(process.env.ANAKOT_DESKTOP_ANAKOT_ROOT),
    !IS_PACKAGED && isAnakotSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isAnakotSourceRoot(ACTIVE_ANAKOT_ROOT) ? ACTIVE_ANAKOT_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_ANAKOT_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('anakot:updates:progress', merged)
  }
}
...
async function checkUpdates({ force = false }: { force?: boolean } = {}) {
  ...
  if (!directoryExists(gitDir)) {
    return {
      ...
      message:
        "This copy of Anakot can't update itself from inside the app. Download the latest version from the Anakot website, " +
        `or reinstall Anakot to enable in-app updates. Details: ${updateRoot} has no version-control metadata.`,
        ...
        headers: githubApiHeaders(
          {
            ...
            'User-Agent': 'anakot-desktop-update-check'
          },
          token
        ),
        timeout: 10_000
...
// Windows that binary owns ALL repo mutation — running `anakot update` +
...
function resolveUpdaterBinary() {
  return resolveStagedUpdaterBinary(ANAKOT_HOME, { fileExists, isWindows: IS_WINDOWS })
}
...
// Path to the venv shim whose lock decides whether `anakot update` can write
// fresh entry points. On Windows this is the file the running backend
// `anakot.exe` holds open; on POSIX it's never mandatory-locked.
function venvAnakotShimPath(updateRoot) {
  const venvDir = resolveVenvDir(updateRoot)

  return IS_WINDOWS
    ? path.join(venvDir, 'Scripts', 'anakot.exe')
    : path.join(venvDir, 'bin', 'anakot')
}
...
// Kill only Anakot-OWNED venv daemons (the memory plugin's hindsight daemon:
...
// venv files mapped. External holders (a user terminal running `anakot`,
...
function killAnakotOwnedVenvDaemons(updateRoot) {
  if (!IS_WINDOWS) {
    return
  }
  ...
  try {
    ...
    holders = (Array.isArray(parsed) ? parsed : [parsed]).filter(p =>
      isAnakotOwnedVenvDaemon(p?.ExecutablePath, p?.CommandLine, scriptsDir)
    )
  } catch {
  ...
  for (const holder of holders) {
    ...
    if (Number.isInteger(pid) && pid > 0) {
      rememberLog(`[updates] stopping Anakot-owned venv daemon (hindsight) PID ${pid} before hand-off`)
...
// only signals the direct child, so on Windows a backend `anakot.exe` that
// spawned its own grandchildren (a `anakot` REPL, a pty terminal session, the
...
async function claimBackendChild(child, command, profile, nonce, outputTail: BackendOutputTail | null = null) {
  ...
  if (decision.action === 'fail') {
    ...
    throw new Error(
      `Anakot backend (PID ${child.pid}) died before its identity could be recorded: ${decision.reason}${outputTail?.describe() ?? ''}`
    )
  }
  ...
  if (decision.action === 'degrade') {
    ...
    rememberLog(
      `WARNING: process start marker probe failed for live Anakot backend PID ${child.pid}; ` +
  ...
  try {
    ...
    child.anakotBackendIdentity = identity
    ...
    await waitForBackendExit(child)
    throw new Error(
      `Could not persist ownership for the Anakot backend: ${error.message}${outputTail?.describe() ?? ''}`
    )
  }
}

function releaseBackendChild(child) {
  const identity = child?.anakotBackendIdentity
...
// `anakotProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `anakot.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a anakot
// REPL, a pty terminal, the gateway) keep `anakot.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const anakotProcess = backendConnectionState.getProcess()
  ...
  // unlock — the shim probe only covers venv\Scripts\anakot.exe, but the
  // backend is `python.exe -m anakot_cli.main serve`, which need not hold
  // the shim at all (#74805 first-attempt race).
  const initialPids = []

  if (anakotProcess && Number.isInteger(anakotProcess.pid)) {
    initialPids.push(anakotProcess.pid)
  }
  ...
  stopBackendTreesForUpdate(anakotProcess, {
  ...
  // `anakot gateway stop --all`: the CLI discovers every profile's gateway
  ...
  stopGatewayBeforeUpdate(venvAnakotShimPath(updateRoot), ANAKOT_HOME)

  // Reap Anakot-OWNED venv daemons the tree-kill above cannot reach: the
  ...
  killAnakotOwnedVenvDaemons(updateRoot)

  const shim = venvAnakotShimPath(updateRoot)
  ...
  const gate = await waitForBackendRelease(
    initialPids,
    {
      ...
      collectStragglerPids: () => {
        ...
        const currentAnakotProcess = backendConnectionState.getProcess()

        if (currentAnakotProcess && Number.isInteger(currentAnakotProcess.pid)) {
          stragglers.push(currentAnakotProcess.pid)
        }
...
// `anakot update`). Instead we spawn the staged Anakot-Setup binary with
// --update and quit, so it can run `anakot update` (which refuses while we
...
async function applyUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      ...
      // Windows (quit → detached orchestrator → `anakot update` → relaunch),
      ...
      // in: the ANAKOT_DESKTOP_CHILD_PID reaper-exclusion dance (#37532),
      ...
      // get the manual `anakot update` card once; their next update pulls it.
      return await applyUpdatesPosixHandoff(opts)
    }

    if (!updater) {
      // No staged updater binary — this is a CLI-installed user (they ran
      // `anakot desktop`, never the Tauri installer that self-copies
      // anakot-setup.exe into ANAKOT_HOME). On Windows the repo hand-off
      ...
      if (!resolveUpdateScriptHandoff(updateRoot)) {
        // They DO have a working `anakot` on PATH / in the venv, so the
        ...
        try {
          ...
          if (head.code === 0 && current && current !== 'HEAD') {
            ...
            if (branch !== 'main') {
              command = `anakot update --branch ${branch}`
            }
          }
        } catch {
          // Best-effort: fall back to bare `anakot update` if branch detection fails.
        }

        rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
        emitUpdateProgress({ stage: 'manual', message: command, percent: null })

        return { ok: true, manual: true, command, anakotRoot: updateRoot }
      }

      rememberLog('[updates] no staged updater; using repo hand-off script for CLI install')
    }

    const handoffConflict = updateHandoffConflict(ANAKOT_HOME)
    ...
    // anakot.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running anakot, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Anakot install open ' +
        '(a second Anakot window or a terminal running anakot?). Close it and retry.'
      ...
      if (IS_WINDOWS) {
        ...
        startGatewaysAfterUpdateAbort(venvAnakotShimPath(updateRoot))
      }
    ...
    // Anakot processes running from this venv.  The updater normally refuses
    ...
    // `anakot update` will run (the venv shim is locked while we live).
    //
    ...
    if (scriptHandoff) {
      ...
      child = spawnUpdaterProcess(wrapped.command, wrapped.args, {
        ...
        env: {
          ...process.env,
          ANAKOT_HOME,
          ANAKOT_UPDATE_STARTED_AT: String(updateStartedAt),
          PATH: pathWithAnakotManagedNode(venvBin)
        },
        detached: true,
        stdio: 'ignore'
      })
      ...
      // The `anakot update` child adopts the SCRIPT's claim via
      // update_lock.py's process-ancestry rule; no mtime heuristics needed.
      if (Number.isInteger(child.pid)) {
        writeUpdateMarker(ANAKOT_HOME, child.pid, { startedAt: updateStartedAt })
      }
      ...
      // abort with "Another Anakot update is already running (PID <itself>)" —
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(resolveVenvDir(updateRoot), IS_WINDOWS ? 'Scripts' : 'bin')
  const venvAnakot = path.join(venvBin, IS_WINDOWS ? 'anakot.exe' : 'anakot')
  const venvPython = path.join(venvBin, IS_WINDOWS ? 'python.exe' : 'python')

  // The updater invokes the venv's Anakot launcher, which in turn requires the
  ...
  // marker-only install through --update dead-ends at "Could not find the anakot
  ...
  const updaterArgs = chooseUpdaterArgs(
    {
      hasBootstrapMarker: fileExists(path.join(updateRoot, '.anakot-bootstrap-complete')),
      hasVenvAnakot: fileExists(venvAnakot),
...
// Python-level pre-update snapshot inside `anakot update`.
function preflightStateDb(anakotHome, rememberLog) {
  const stateDbPath = path.join(anakotHome, 'state.db')
  ...
  try {
    ...
    if (stat.size > 100) {
      ...
      const emergencyPath = path.join(anakotHome, `state.db.pre-update-emergency-${ts}.bak`)
      ...
      try {
        ...
        try {
          const homeDir = fs.readdirSync(anakotHome)
          ...
          for (const old of backups.slice(2)) {
            try {
              fs.unlinkSync(path.join(anakotHome, old))
            } catch {
              void 0
            }
          }
...
// out, runs `anakot update`, swaps/relaunches the app bundle, and writes
// .anakot-update-result.json for the relaunched Desktop to surface. It shows
...
async function applyUpdatesPosixHandoff(opts: any) {
  ...
  if (!handoff) {
    emitUpdateProgress({ stage: 'manual', message: 'anakot update', percent: null })

    return { ok: true, manual: true, command: 'anakot update', anakotRoot: updateRoot }
  }
  ...
  const child = spawnUpdaterProcess(handoff.command, args, {
    ...
    env: {
      ...
      ANAKOT_HOME,
      ANAKOT_UPDATE_STARTED_AT: String(updateStartedAt),
      PATH: pathWithAnakotManagedNode(path.join(resolveVenvDir(updateRoot), 'bin'))
    },
    detached: true,
    stdio: 'ignore'
  })
  ...
  emitUpdateProgress({
    stage: 'restart',
    message:
      'Updating Anakot — this window will close. Don’t reopen Anakot yourself; it restarts automatically when the update finishes.',
    percent: 100
  })
...
// Marker-independent: is the canonical install at ACTIVE_ANAKOT_ROOT actually
...
async function isActiveRuntimeUsable() {
  ...
  return (
    isAnakotSourceRoot(ACTIVE_ANAKOT_ROOT) &&
    ...
    (await canImportAnakotCli(venvPython, {
      env: {
...
async function activeRuntimeState() {
  ...
  // pinned commit -- users update via the in-app update path or `anakot
...
function resolveWebDist() {
  const override = process.env.ANAKOT_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set ANAKOT_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndexWithMissing(): { index: string; missing: string[] } {
  ...
  if (present.length > 0) {
    ...
    rememberLog(
      `[renderer] every renderer bundle is incomplete (${present.join(', ')}). ` +
        ...
        `Repair with: anakot desktop --force-build`
    )
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: anakot desktop --force-build`
  )
...
function resolveAnakotCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Anakot.app/Contents/...`
  ...
  const candidates = [
    ...
    process.env.ANAKOT_DESKTOP_CWD,
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveAnakotCwd(), sanitized: Boolean(trimmed) }
  }
...
async function createPythonBackend(root, label, backendArgs, options: any = {}) {
  ...
  return {
    ...
    label,
    command,
    args: ['-m', 'anakot_cli.main', ...backendArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_ANAKOT_ROOT, the
...
async function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Anakot at ${ACTIVE_ANAKOT_ROOT}`,
    command,
    args: ['-m', 'anakot_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      anakotHome: ANAKOT_HOME,
      pythonPathEntries: [ACTIVE_ANAKOT_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_ANAKOT_ROOT,
    bootstrap: true,
    shell: false
  }
}

async function resolveAnakotBackend(backendArgs) {
  // 1. Explicit override -- ANAKOT_DESKTOP_ANAKOT_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.ANAKOT_DESKTOP_ANAKOT_ROOT && path.resolve(process.env.ANAKOT_DESKTOP_ANAKOT_ROOT)

  if (overrideRoot && isAnakotSourceRoot(overrideRoot)) {
    const backend = await createPythonBackend(overrideRoot, `Anakot source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `anakot` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isAnakotSourceRoot.)
  if (!IS_PACKAGED && isAnakotSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = await createPythonBackend(SOURCE_REPO_ROOT, `Anakot source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. ACTIVE_ANAKOT_ROOT — the canonical install at
  //    %LOCALAPPDATA%\\anakot\\anakot-agent (Windows) or ~/.anakot/anakot-agent.
  ...
  if (activeRuntime.shouldUseActiveRuntime && !bootstrapRepairRequested) {
    if (!activeRuntime.hasValidMarker) {
      rememberLog(
        `[bootstrap] Active Anakot runtime at ${ACTIVE_ANAKOT_ROOT} is usable but the bootstrap marker is missing or stale; skipping first-run bootstrap.`
      )
    }
  ...
  //    ANAKOT_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.ANAKOT_DESKTOP_IGNORE_EXISTING !== '1') {
    let anakotCommand = null
    const anakotOverride = process.env.ANAKOT_DESKTOP_ANAKOT

    if (anakotOverride) {
      const resolvedOverride = findOnPath(anakotOverride)

      if (resolvedOverride) {
        anakotCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(anakotOverride, { isWsl: IS_WSL })) {
        anakotCommand = anakotOverride
      } else {
        rememberLog(`Ignoring Windows Anakot override under WSL: ${anakotOverride}`)
      }
    } else {
      anakotCommand = findOnPath('anakot')
    }

    if (anakotCommand) {
      if (looksLikeDesktopAppBinary(anakotCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Anakot CLI: ${anakotCommand}`)
        anakotCommand = null
      }
    }

    if (anakotCommand) {
      const unwrapped = await unwrapWindowsVenvAnakotCommand(anakotCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `anakot` shim
      ...
      const shellForProbe = isCommandScript(anakotCommand)

      // ANAKOT_DESKTOP_ANAKOT is an explicit deployment override (used by
      ...
      if (
        shouldTrustAnakotOverride(anakotOverride) ||
        (await verifyAnakotCli(anakotCommand, { shell: shellForProbe }))
      ) {
        ...
        // unwrapWindowsVenvAnakotCommand here: the second call repeats the
        // same un-memoized import probe, costing up to another full probe
        // timeout on the boot path for an answer we already have.
        return {
          label: `existing Anakot CLI at ${anakotCommand}`,
          command: anakotCommand,
          args: backendArgs,
          bootstrap: false,
          env: {},
          kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Anakot CLI at ${anakotCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed anakot_cli module via system Python.
  ...
  if (python) {
    ...
    // having anakot_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\anakot\anakot-agent\venv.
    if (await canImportAnakotCli(python)) {
      return {
        kind: 'python',
        label: `installed anakot_cli module via ${python}`,
        command: python,
        args: ['-m', 'anakot_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: anakot_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveAnakotBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Anakot Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function runEnsureRuntime(backend: any, assertStillOwned: () => void): Promise<any> {
  ...
  // backend.kind === 'bootstrap-needed' means resolveAnakotBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Anakot install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Anakot recovery was handed off to Anakot Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      anakotHome: ANAKOT_HOME,
      logRoot: path.join(ANAKOT_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Anakot install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      ...
      // Latch the failure so subsequent startAnakot() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // anakot:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(await resolveAnakotBackend(backend.args), assertStillOwned)
  }
  ...
  if (!isAnakotSourceRoot(ACTIVE_ANAKOT_ROOT)) {
    throw new Error(missingInstallPartMessage(`Anakot source files are missing or incomplete at ${ACTIVE_ANAKOT_ROOT}`))
  }

  // On Windows, preflight Git Bash. Anakot' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\anakot\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `anakot` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      "Anakot needs a helper called Git for Windows, which isn't installed. " +
        'Choose Repair install to add it automatically, or install it yourself from git-scm.com and reopen Anakot.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    // normal flow because activeRuntimeState() requires isAnakotSourceRoot()
    // plus an importable anakot_cli before it hands back the active runtime.
  ...
  backend.label = `Anakot at ${ACTIVE_ANAKOT_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Anakot runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function multipartBody(upload) {
  const boundary = `----anakot-${crypto.randomBytes(12).toString('hex')}`
    ...
    (requestState: any) =>
      new Promise((resolve, reject) => {
        ...
        req.setTimeout(timeoutMs, () => {
          req.destroy(new Error(`Timed out connecting to Anakot backend after ${timeoutMs}ms`))
        })
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = client.request(
      parsed,
      {
        agent,
        method: 'GET',
        headers: options.bearer ? { Authorization: `Bearer ${options.bearer}` } : { 'X-Anakot-Session-Token': token }
      },
      res => {
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Anakot-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('anakot:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveAnakotCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('anakot:preview-file-changed', payload)
}
...
// waitForAnakotReady how to read a 401 — rejected session vs gated route.
...
async function waitForAnakot(baseUrl, token, signal?, authMode?, headers = {}) {
  const { probeHealth, probeIsCredentialed } = await buildReadinessHealthProbe(baseUrl, authMode, token)

  return waitForAnakotReady(baseUrl, {
    token,
    signal,
    fetchPublicJson,
...
function sendBackendExit(payload) {
  ...
  webContents.send('anakot:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('anakot:close-preview-requested')
}
...
function sendPreviewNavCommand(command: 'back' | 'forward' | 'reload') {
  ...
  webContents.send('anakot:preview-nav', command)
}
...
function sendOpenFolderRequested() {
  ...
  webContents.send('anakot:open-folder-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Anakot...".
function sendPowerResume() {
  ...
  webContents.send('anakot:power-resume')
}
...
// 'anakot:power-battery' push below.
ipcMain.handle('anakot:power-battery:get', () => onBatteryPower === true)
...
function broadcastBatteryState(next: boolean) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('anakot:power-battery', next)
    }
  }
}
...
async function showPluginCompatNoticeOnce() {
  ...
  try {
    notice = pendingPluginCompatNotice(ANAKOT_HOME, app.getPath('userData'))
  } catch (err) {
  ...
  try {
    ...
    // 'Open Plugins' rides the existing deep-link channel (anakot://open/…),
    ...
    if (response === 0) {
      handleDeepLink(`${ANAKOT_PROTOCOL}://open/capabilities?tab=plugins`)
    }
  } finally {
    try {
...
function sendOpenUpdatesRequested() {
  ...
  // before the window is up) queue the request; 'anakot:deep-link-ready' flushes it.
  ...
  webContents.send('anakot:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean, target = mainWindow) {
  ...
  webContents.send('anakot:window-state-changed', state)
}
...
function installContextMenuBridge(window: BrowserWindow) {
  window.webContents.on('context-menu', (_event, params) => {
    ...
    if (params.isEditable && params.misspelledWord) {
      window.webContents.send('anakot:context-menu-spellcheck', {
...
//
// Hosted Anakot gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``anakot_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / anakot #37247).
//     Both are set as HttpOnly cookies (``anakot_session_at`` ~15 min,
//     ``anakot_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
// "not signed in", which on the initial boot path (startAnakot → the renderer's
// single-shot boot() with no retry) surfaced as the "Anakot couldn't start"
...
async function hasLiveOauthSession(baseUrl) {
  ...
  // for a signed-in user — the exact race that produced the transient "Anakot
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Anakot Cloud agent…' : 'Sign in to Anakot gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Anakot backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// feature; the server half lives in anakot_cli/dashboard_auth/native_flow.py.
// ---------------------------------------------------------------------------
...
// --- Anakot Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:anakot-remote-oauth`:
...
// convention (anakot_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Anakot surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.ANAKOT_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// Discover the hosted (Anakot Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Anakot Cloud. Open Settings → Gateway, choose Anakot Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (body === undefined) {
      ...
      if (error && error.statusCode === 401) {
        const err = new Error(
          'Your Anakot Cloud session has expired. Open Settings → Gateway and sign in again.'
        ) as any
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Anakot Cloud session has expired. Sign in to Anakot Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Anakot Cloud org tag on cloud-mode entries so Settings can
...
function readDesktopConnectionsRegistry() {
  ...
  if (mtime === null) {
    ...
    try {
      ...
      // hard-failing every anakot:connections:* call.
 ...
 * editor doesn't carry — cloud `org`, ssh `remoteAnakotPath`/`remoteProfile` —
  ...
  onDefaultChanged: route => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.webContents.isDestroyed()) {
        win.webContents.send('anakot:profile:default:changed', route)
      }
    }
  }
})
...
// contains "anakot", avoiding false positives from stale gateway.pid files
// whose PID was recycled by the OS to an unrelated process.
function isAnakotProcess(pid) {
  try {
  ...
  try {
    ...
    return cmdline.includes('anakot')
  } catch {
...
//   1. Legacy ~/.anakot/active_profile (explicit CLI choice via anakot profile use)
//   2. Running gateway (gateway.pid with verified liveness + anakot identity)
...
function migrateActiveProfileIfMissing() {
  migrateActiveProfileIfMissingPure(DESKTOP_PROFILE_CONFIG_PATH, {
    legacyActivePath: path.join(ANAKOT_HOME, 'active_profile'),
    anakotHome: ANAKOT_HOME,
    profilesRoot: path.join(ANAKOT_HOME, 'profiles'),
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.ANAKOT_DESKTOP_REMOTE_URL)
  ...
  const remoteUrl = envOverride ? String(process.env.ANAKOT_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    remoteUrl,
    // The persisted Anakot Cloud org (slug/id) for a cloud connection, or '' for
    ...
    sshRemoteAnakotPath: (ssh || savedSsh)?.remoteAnakotPath || '',
    sshRemoteProfile: (ssh || savedSsh)?.remoteProfile || '',
    // The env override only forces the global/primary connection; a per-profile
    // scope is never overridden by ANAKOT_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Anakot Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Anakot Cloud instance, NOT a user-owned remote gateway,
...
function buildSshBlock(input: any, existingBlock: any = {}) {
  ...
  const merged = normalizeSshConfig({
    ...
    remoteAnakotPath: input.sshRemoteAnakotPath ?? existingBlock.remoteAnakotPath,
...
function activeSshTerminalTarget(webContentsId?: number) {
  ...
  const route = resolveDesktopRemoteRoute({
    config,
    env: {
      token: process.env.ANAKOT_DESKTOP_REMOTE_TOKEN,
      url: process.env.ANAKOT_DESKTOP_REMOTE_URL
    },
    profile,
    registry: readDesktopConnectionsRegistry()
  })
...
async function rollbackSshBootstrapResult(ssh, result, profile, sshConfig, boundaryError) {
  ...
  try {
    const expected = {
      ...
      anakotPath: result.anakotPath,
      anakotHome: result.anakotHome,
    ...
    if (result.platform?.os === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        ssh,
        { anakotPath: result.anakotPath, anakotHome: result.anakotHome, python: result.pythonPath },
        expected
      )
...
async function bootstrapSshConnectionInner(profile, sshConfig, reuseToken, source, metadata, fingerprint, lease) {
  ...
  try {
    ...
    const platform = await detectRemotePlatform(ssh, sshConfig.remoteAnakotPath || '')
    const lifecycle = platform.os === 'Windows' ? connectWindowsRemote : remoteLifecycle.connect
    result = await lifecycle({
      ssh,
      platform,
      profile: resolveRemoteSshDashboardProfile(sshConfig.remoteProfile, profile),
      remoteAnakotPath: sshConfig.remoteAnakotPath || '',
      ownershipId: sshOwnershipKey(profile),
      ...
      pickLocalPort,
      waitForAnakot: (baseUrl, token) => waitForAnakot(baseUrl, token, lease.signal, 'token'),
      ...
      // spawn command adds ANAKOT_GUEST_ONBOARDING=1 only when this is on.
  ...
  await fenceManagedSshBootstrapPublication({
    ...
    publish: () => {
      ...
      sshConnections.set(scope, {
        ...
        host: sshConfig.host,
        hostLabel,
        anakotVersion: result.anakotVersion || '',
        ...
        // per-profile pools also use bare keys. Only startAnakot' explicit call
  ...
  sshRememberLog(
    `[ssh] connection ${result.reused ? 'REUSED' : 'spawned'} dashboard: ` +
      `${result.anakotVersion || 'anakot (version unknown)'} at ${result.anakotPath || '?'}`
  )
  ...
  return {
    ...connection,
    remoteAnakotVersion: result.anakotVersion || '',
    ssh: {
      effectiveConfigFingerprint: sshConfig.effectiveConfigFingerprint,
      host: sshConfig.host,
...
//   2. env override (ANAKOT_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
function globalRemoteActive() {
  if (process.env.ANAKOT_DESKTOP_REMOTE_URL) {
    return true
  }
...
// env → global) so a startAnakot() failure can be classified as remote (never
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Anakot
...
async function testDesktopConnectionConfig(input: any = {}) {
  if (input.mode === 'ssh') {
    const sshConfig = normalizeSshConfig({
      ...
      remoteAnakotPath: input.sshRemoteAnakotPath
    })
    ...
    try {
      ...
      for (;;) {
        try {
          await ssh.open()
          const platform: any = await detectRemotePlatform(ssh, sshConfig.remoteAnakotPath || '')
          let anakotPath
          let anakotVersion
          let supported
          ...
          if (platform.os === 'Windows') {
            ...
            anakotPath = runtime.anakotPath
            const inspection = await helper(ssh, runtime, 'inspect', [runtime.anakotPath])
            anakotVersion = inspection.version
            supported = inspection.supported
          } else {
            anakotPath = await remoteLifecycle.locateAnakot(ssh, sshConfig.remoteAnakotPath || '')
            anakotVersion = await remoteLifecycle.probeAnakotVersion(ssh, anakotPath)
            supported = await remoteLifecycle.remoteSupportsSshOwnership(ssh, anakotPath)
          }
          ...
          if (!supported) {
            return {
              ...
              error: 'Update Anakot on the remote host before connecting with Desktop SSH.'
            }
          }
          ...
          return {
            ...
            remoteAnakotPath: anakotPath,
            remoteAnakotVersion: anakotVersion,
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startAnakot())
  ...
  // connect to Anakot gateway". Mirror the renderer's connect here so the test
...
function resetAnakotConnection({ soft = false } = {}) {
  backendStartFailure = null
  remoteReauthFailure = null
  remoteLiveness.clear()
  // The next startAnakot() re-reads active-profile.json for its launch profile.
  primaryProfilePin.clear()
  const anakotProcess = invalidatePrimaryConnection()
  stopBackendChild(anakotProcess)
...
// startAnakot() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetAnakotConnection() invalidates it.
  const anakotProcess = backendConnectionState.getProcess()
  const dying = anakotProcess && !anakotProcess.killed ? anakotProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('anakot:connection:applied')
}
...
function broadcastConnectionsChanged(payload: { connectionId: string; reason: 'removed' | 'saved' | 'updated' }) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('anakot:connections:changed', payload)
    }
  }
}
...
// startAnakot() and cleared when the primary is torn down; while a primary is
...
async function ensureBackend(profile, opts: { passive?: boolean; spawnPriority?: LocalBackendSpawnPriority } = {}) {
  ...
  if (route.backend === 'primary') {
    const connection = await startAnakot()
  ...
  // spawning, so two children never share one profile's ANAKOT_HOME.
  ...
  const resolveRegistrySshConfig = () => {
    ...
    if (!resolvedRegistrySshConfig) {
      resolvedRegistrySshConfig = normalizeSshConfig({
        ...
        remoteAnakotPath: source.remoteAnakotPath,
...
async function connectRegistryBackend(
  ...
  if (source.kind === 'ssh') {
    ...
    return {
      ...
      // is only the routing label. anakot:api uses it to translate explicit
      // self-profile query filters into the backend's namespace.
      remoteProfile: sshConfig.remoteProfile || '',
      logs: anakotLog.slice(-80),
      ...getWindowState()
    }
  }
  ...
  await waitForAnakot(connection.baseUrl, connection.token, undefined, connection.authMode, connection.headers)
  poolEntry.remoteBaseUrl = connection.baseUrl
...
function remoteUpdateTargetFromState(state): RemoteUpdateTarget {
  if (!state?.ssh || !state?.anakotPath || !state?.anakotHome) {
  ...
  return {
    ...
    anakotPath: state.anakotPath,
    anakotHome: state.anakotHome,
...
async function openManagedSshUpdateTransport(
  ...
  try {
    const platform: any = await detectRemotePlatform(ssh, config.remoteAnakotPath || '')

    if (platform.os === 'Windows') {
      const runtime = platform.anakotPath ? platform : await probeWindowsRemote(ssh, config.remoteAnakotPath || '')
      ...
      return {
        ...
        target: {
          ...
          platform: 'Windows',
          anakotPath: runtime.anakotPath,
          anakotHome: runtime.anakotHome,
          pythonPath: runtime.python
        }
      }
    }

    const anakotPath = await remoteLifecycle.locateAnakot(ssh, config.remoteAnakotPath || '')
    const anakotHome = await remoteLifecycle.probeRemoteAnakotHome(ssh)

    return {
      close: () => ssh.close(),
      target: { ssh, platform: platform.os, anakotPath, anakotHome }
    }
  } catch (error) {
    await ssh.close()
    throw error
...
async function drainManagedSshScope(scope) {
  ...
  try {
    ...
    if (state.remotePlatform === 'Windows') {
      await terminateOwnedWindowsDashboardForUpdate(
        state.ssh,
        { anakotPath: state.anakotPath, anakotHome: state.anakotHome, python: state.pythonPath },
        expected
      )
    ...
    if (state && state.localPort && state.remotePort && scope.reuseToken) {
      try {
        ...
        if (!forwardClosed) {
          await waitForAnakot(`http://127.0.0.1:${state.localPort}`, scope.reuseToken, undefined, 'token')
        } else {
...
// local-spawn portion of startAnakot() but without the boot-progress UI,
...
async function runPoolBackendStart(profile, entry, opts: { forceLocal?: boolean; poolKey?: string } = {}) {
  ...
  if (remote) {
    await waitForAnakot(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
  ...
  // --profile wins over the inherited ANAKOT_HOME env (see _apply_profile_override
  // step 3 in anakot_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']

  const backend = await ensureRuntime(await resolveAnakotBackend(backendArgs), () =>
    assertPoolEntryStillOwned(poolKey, entry)
  )
  ...
  assertPoolEntryStillOwned(poolKey, entry)
  const anakotCwd = resolveAnakotCwd()
  ...
  assertLocalProfileCanStart(profile, profileDeletionGate, key =>
    directoryExists(path.join(ANAKOT_HOME, 'profiles', key))
  )
  rememberLog(`Starting Anakot backend for profile "${profile}" via ${backend.label}`)
      ...
      env: desktopBackendSpawnEnv(
        {
          ...
          ...(readyFile ? { ANAKOT_DESKTOP_READY_FILE: readyFile } : {})
        },
        GUEST_ONBOARDING
      ),
      shell: backend.shell,
  ...
  child.once('error', error => {
    rememberLog(`Anakot backend for profile "${profile}" failed to start: ${error.message}`)
    void teardownFailedLocalBackend(poolKey, entry).catch(cleanupError => {
      rememberLog(
        `Anakot backend for profile "${profile}" cleanup failed: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`
      )
    })
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(formatBackendExitLine(`Anakot backend for profile "${profile}" exited`, code, signal, outputTail))
    releaseBackendChild(child)

    if (!ready) {
      rejectStart?.(
        new Error(
          `Anakot backend for profile "${profile}" exited before it became ready (${signal || code}).${outputTail.describe()}`
        )
      )
    }
  })
  ...
  await Promise.race([waitForAnakot(baseUrl, token), startFailed])
  assertPoolEntryStillOwned(poolKey, entry, { releaseSlot: false })
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Anakot backend for profile "${profile}"`,
    rememberLog
  })
  ...
  if (!wsProbe.ok) {
    throw new Error(
      `Anakot backend for profile "${profile}" is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
    )
  }
...
function broadcastPoolBackendRetiring(poolKey: string) {
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    if (webContents && !webContents.isDestroyed()) {
      webContents.send('anakot:pool:retiring', { poolKey })
    }
  }
}
...
// backend whose ensure_anakot_home() recreates the deleted profile directory.
//
...
function startAnakot() {
  primaryRecoverySuppressed = false
  primaryStartsInFlight += 1

  const start = localBackendLifecycle.start(runAnakotStart)
...
function scheduleUnexpectedPrimaryRecovery({ code = null, signal = null, error = null, ready = false } = {}) {
  ...
  startAnakot().catch(respawnError => rememberLog(`[supervisor] backend respawn failed: ${respawnError.message}`))

  return true
}

async function runAnakotStart() {
  ...
  if (!isPrimaryInstance) {
    ...
    throw new Error('Anakot Desktop is already running in another window.')
  }
  ...
  // process, every subsequent startAnakot() call re-throws the same error
  ...
  if (BOOT_FAKE_ERROR) {
    await advanceBootProgress('backend.resolve', 'Resolving Anakot backend', 8)
  ...
  // later anakot:profile:remember must not retarget requests mid-life.
  ...
  const connectionPromise = (async () => {
    const connectRemote = async remote => {
      ...
      await advanceBootProgress('backend.remote', `Connecting to remote Anakot backend at ${remote.baseUrl}`, 24)
      await waitForAnakot(remote.baseUrl, remote.token, undefined, remote.authMode, remote.headers)
      ...
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Anakot backend is ready',
        progress: 94,
        running: true,
        error: null
      })

      return createPrimaryRemoteConnection(remote, anakotLog.slice(-80), getWindowState())
    }
    ...
    // deterministic (it wins over the sticky ~/.anakot/active_profile file) and
    // resolves ANAKOT_HOME the same way `anakot -p <name>` does on the CLI. An
    ...
    const setup = await runPrimaryBackendStartup({
      ...
      prepareLocalBackend: async () => {
        await advanceBootProgress('backend.runtime', 'Resolving Anakot runtime', 28)

        return resolveAnakotBackend(backendArgs)
      },
      resolveRemote: () => {
    ...
    await advanceBootProgress('backend.spawn', `Starting Anakot backend via ${backend.label}`, 84)
    rememberLog(`Starting Anakot backend via ${backend.label}`)
    ...
    const anakotProcess = spawnOwnedBackend(
      backend.command,
      backend.args,
      hiddenWindowsChildOptions({
        cwd: anakotCwd,
        env: desktopBackendSpawnEnv(
          {
            ...process.env,
            // Explicitly pin ANAKOT_HOME for the child so Python's get_anakot_home()
            // resolves to the SAME location our resolveAnakotHome() picked. Without
            // this pin, Python falls back to ~/.anakot on every platform — fine on
            // mac/linux (where our default matches), but on Windows our default is
            // %LOCALAPPDATA%\anakot, which differs from C:\Users\<u>.anakot.
    ...
    primaryOutputTail.attach(anakotProcess)
    ...
    const portAnnouncement = waitForDashboardPortAnnouncement(anakotProcess, {
    ...
    const processOwner = backendConnectionState.attachProcess(connectionAttempt, anakotProcess)

    if (!processOwner) {
      stopBackendChild(anakotProcess)
      await waitForBackendExit(anakotProcess)
      releaseBackendChild(anakotProcess)
      throw new Error('Anakot backend start was superseded by a newer connection attempt.')
    }

    anakotProcess.stdout.on('data', rememberLog)
    anakotProcess.stderr.on('data', rememberLog)
    ...
    anakotProcess.once('error', error => {
      releaseBackendChild(anakotProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(`Ignoring stale Anakot backend error: ${error.message}`)
        scheduleUnexpectedPrimaryRecovery({ error: error.message, ready: backendReady })
        rejectBackendStart?.(new Error('Anakot backend start was superseded by a newer connection attempt.'))

        return
      }

      rememberLog(`Anakot backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Anakot backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    anakotProcess.once('exit', (code, signal) => {
      releaseBackendChild(anakotProcess)

      if (!backendConnectionState.clearForCurrentProcess(processOwner)) {
        rememberLog(formatBackendExitLine('Ignoring stale Anakot backend exit', code, signal, primaryOutputTail))
      ...
      rememberLog(formatBackendExitLine('Anakot backend exited', code, signal, primaryOutputTail))
      ...
      if (!backendReady) {
        const message = `Anakot backend exited before it became ready (${signal || code}).${primaryOutputTail.describe()}`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Anakot backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentAnakotLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Anakot backend to launch', 86)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Anakot backend to become ready', 90)
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    await Promise.race([waitForAnakot(baseUrl, token), backendStartFailed])
    backendConnectionState.assertCurrentAttempt(connectionAttempt)
    backendReady = true
    ...
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      childAlive: () => anakotProcess.exitCode === null && !anakotProcess.killed,
      rememberLog
    })
    ...
    if (!wsProbe.ok) {
      throw new Error(
        `Local Anakot backend is HTTP-reachable but the WebSocket (/api/ws) rejected the session token: ${wsProbe.reason}`
      )
    }

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Anakot backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
    ...
    // The backend's plugin discovery just ran and refreshed ANAKOT_HOME/.plugin-compat-report.json.
    ...
    // getConnection()/startAnakot() caller arriving during the exit wait would
...
function wireCommonWindowHandlers(win, { zoom = true }: { zoom?: boolean } = {}) {
  ...
  // links arrive via `anakot:openExternal`, not here. See window-open-policy.ts.
 ...
 * always carries the `persist:anakot-preview` partition, so the partition is
 ...
 * audited `anakot:openExternal` channel. Popup requests themselves stay
...
function installPreviewGuestPreload() {
  app.on('web-contents-created', (_event, contents) => {
    ...
    contents.on('will-attach-webview', (_attachEvent, webPreferences, params) => {
      if (params.partition !== 'persist:anakot-preview') {
        return
      }
...
function notifyBrowserPopoutClosed(tabId) {
  ...
  for (const other of BrowserWindow.getAllWindows()) {
    if (!other.isDestroyed()) {
      other.webContents.send('anakot:browser-popout:closed', tabId)
    }
  }
}
...
// here so it can leave the app's bounds and stay visible while Anakot is
...
// window showing only the composer and its scrollback, so Anakot can be driven
...
// left the user with NO Anakot window after the second toggle (#88513).
...
function startHudCursorFeed(win: BrowserWindow) {
  ...
  const timer = setInterval(() => {
    ...
    last = key
    win.webContents.send('anakot:hud:cursor', point)
  }, HUD_CURSOR_POLL_MS)
...
function startHudGameOverlayFeed(win: BrowserWindow) {
  ...
  const push = (state: { active: boolean; app: string }) => {
    if (!win.isDestroyed()) {
      win.webContents.send('anakot:hud:game-overlay', state)
    }
  }
  ...
  const enumerate = async () => {
    ...
    if (!reported) {
      reported = true
      console.warn(`[anakot] HUD cannot enumerate windows: ${windows.reason}`)
    }
...
function broadcastHudState(open) {
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) {
      win.webContents.send('anakot:hud:changed', payload)
    }
  }
}
...
function spawnHudWindow(sessionId, profile) {
  const win = new BrowserWindow({
    ...
    // `anakot:hud:set-bounds`, which flips resizable on for the call — the
  ...
  // grab, anakot:hud:workspace-transfer temporarily makes the X11 window
...
function openHudWindow(sessionId, profile) {
  ...
  if (hudWindow && !hudWindow.isDestroyed()) {
    ...
    if (sessionId && sessionId !== hudSessionId) {
      ...
      hudWindow.webContents.send('anakot:hud:goto', sessionId)
...
function createWindow() {
    ...
    callbacks: {
      ...
      onFailedLoadBudgetExhausted: details => {
        ...
        void loadRendererLoadErrorPage(mainWindow, {
          ...
          repairHint: 'anakot desktop --force-build',
  ...
  const startup = defaultRoute ? connectDesktopProfileRoute(defaultRoute) : startAnakot()
...
ipcMain.handle('anakot:connection', async (event, profile, extra) => {
...
ipcMain.handle('anakot:connection:for', async (_event, payload) => {
...
ipcMain.on('anakot:connection:active-route', (event, route) => recordWindowConnectionRoute(event.sender, route))
...
// "Starting Anakot…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('anakot:connection:revalidate', async () => {
  ...
  return remoteRevalidation.run(connectionPromise, async () => {
    const [result] = await Promise.all([
      revalidateRemoteConnection({
        ...
        resetConnection: () => resetAnakotConnection({ soft: true }),
...
ipcMain.handle('anakot:backend:touch', async (_event, profile, options) => {
...
ipcMain.handle('anakot:pool-limits:get', async () => ({ ...poolLimits }))
ipcMain.handle('anakot:pool-limits:set', async (_event, raw) => {
...
ipcMain.handle('anakot:gateway:ws-url', async (_event, profile) => {
  return gatewayWsUrlIpcResult(() => freshGatewayWsUrl(profile))
})
ipcMain.handle('anakot:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('anakot:window:openInstance', async (event, options) => {
...
ipcMain.handle('anakot:window:openBrowser', async (_event, tabId) => {
...
ipcMain.handle('anakot:window:openInTerminal', async (_event, sessionId, opts) => {
  ...
  try {
    ...
    const backend = await resolveAnakotBackend(tuiResumeArgs(sessionId.trim(), profile || undefined))

    if (!backend.command) {
      return { ok: false, error: 'Anakot is not installed yet' }
    }
    ...
    const scriptPath = path.join(
      scriptDir,
      `anakot-${crypto.randomBytes(6).toString('hex')}${terminalScriptExtension()}`
    )
    ...
    fs.writeFileSync(
      ...
      buildTerminalScript({
        ...
        cwd,
        env: terminalScriptEnv(backend.env, ANAKOT_HOME)
      }),
      { mode: 0o700 }
    )
...
ipcMain.handle('anakot:wake-indicator:get', () => wakeIndicatorController.getState())
ipcMain.on('anakot:wake-indicator:set', (_event, state) => {
...
ipcMain.handle('anakot:zoom:get', event => {
...
ipcMain.on('anakot:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anakot:backend:recycle', async (_event, profile) => {
...
ipcMain.handle('anakot:bootstrap:reset', async () => {
  ...
  // reset connection state so the next startAnakot() call restarts the
...
ipcMain.handle('anakot:bootstrap:repair', async () => {
  // Forceful repair: force the next startAnakot() through the full installer
  ...
  // "bypass active runtime" path inside startAnakot() and fall through
...
ipcMain.handle('anakot:boot-progress:get', async () => bootProgressState)
ipcMain.handle('anakot:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('anakot:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('anakot:plugin-profile-routes', async (_event, rawProfileNames) => {
...
ipcMain.handle('anakot:ssh-config:hosts', async () => ({ hosts: collectSshConfigHosts() }))
ipcMain.handle('anakot:ssh-config:resolve', async (_event, host) => {
...
ipcMain.handle('anakot:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
...
ipcMain.handle('anakot:secret-storage:get', async () => ({ on: secretStoragePolicy().on }))
ipcMain.handle('anakot:secret-storage:set', async (_event: any, on: any) => applySecretStorageEncryption(on === true))
...
ipcMain.handle('anakot:connections:list', async () => sanitizeConnectionsRegistry())
ipcMain.handle('anakot:connections:save', async (_event, payload) => {
  const saved = await saveRegistryConnection(payload)

  return { ok: true, connection: saved, registry: sanitizeConnectionsRegistry() }
})
ipcMain.handle('anakot:connections:remove', async (_event, id) => {
  const key = String(id || '')
...
ipcMain.handle('anakot:connections:set-primary', async (_event, id) => {
...
ipcMain.handle('anakot:connections:set-launch-mode', async (_event, mode) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anakot:connections:set-last-used', async (_event, id) => {
  const registry = setLastUsedConnection(readDesktopConnectionsRegistry(), String(id || ''))
  writeDesktopConnectionsRegistry(registry)

  return { ok: true, registry: sanitizeConnectionsRegistry(registry) }
})
ipcMain.handle('anakot:connections:test', async (_event, id) => {
  const registry = readDesktopConnectionsRegistry()
  ...
  if (entry.kind === 'ssh') {
    const result = await testDesktopConnectionConfig({
      ...
      sshRemoteAnakotPath: entry.remoteAnakotPath
    })
...
async function probeSshProfileInventory(connection) {
  ...
  const sshConfig = normalizeSshConfig({
    ...
    remoteAnakotPath: connection.remoteAnakotPath
  })
  ...
  try {
    await ssh.open()
    const profiles = await remoteLifecycle.listRemoteAnakotProfiles(ssh)
    ...
    registry.connections.map(async connection => {
      ...
      try {
        ...
        if (connection.kind === 'ssh') {
          ...
          // The root ANAKOT_HOME is an agent too; enumerations that omit it
...
ipcMain.handle('anakot:agents:roster', async () => {
  const registry = readDesktopConnectionsRegistry()
  const enumerations = await enumerateRegistryAgentSources(registry)
...
// anakot:gateway:ws-url. Same single-use-ticket discipline for OAuth sources.
...
ipcMain.handle('anakot:gateway:ws-url-for', async (_event, payload) => {
...
ipcMain.handle('anakot:connections:update-managed', async (_event, rawId) => requestManagedSshUpdate(rawId))

// Fan out `anakot update` to every eligible registered connection at once.
...
ipcMain.handle('anakot:connections:update-all', async (_event, payload) => {
  const registry = readDesktopConnectionsRegistry()
    ...
    registry.connections
      ...
      .map(async connection => {
        ...
        try {
          ...
          const body: any = await postJsonForBackend(descriptor, '/api/anakot/update', {}, { timeoutMs: 15_000 })
...
// analogue of the anakot:api handler's own auth split: OAuth backends prefer a
...
ipcMain.handle('anakot:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('anakot:connection-config:oauth-login', async (_event, rawUrl) => {
  ...
  if (strategy === 'native') {
    try {
      ...
      // startAnakot() re-dials instead of replaying the stale rejection.
      remoteReauthFailure = null
...
ipcMain.handle('anakot:connection-config:oauth-logout', async (_event, rawUrl) => {
  const baseUrl = normalizeRemoteBaseUrl(rawUrl)
...
// --- Anakot Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('anakot:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('anakot:connection-config:save', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anakot:connection-config:apply', async (_event, payload) => {
  assertCanMutateManagedPrimaryRouting()
...
ipcMain.handle('anakot:profile:default:get', async () => desktopProfilePreferences.getDefault())
ipcMain.handle('anakot:profile:default:set', async (_event, route) => desktopProfilePreferences.setDefault(route))
ipcMain.handle('anakot:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
// Persistence-only sibling of anakot:profile:set: records the profile the
...
ipcMain.handle('anakot:profile:remember', async (_event, name) => ({
  profile: writeActiveDesktopProfile(name)
}))
ipcMain.handle('anakot:profile:set', async (_event, name) => {
  assertCanMutateManagedPrimaryRouting()
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new ANAKOT_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('anakot:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('anakot:requestMicrophoneAccess', async () => {
...
ipcMain.handle('anakot:window:readBelow', async event => {
...
async function handleAnakotApiRequest(request) {
  ...
  // backend calls ensure_anakot_home() which recreates the profile directory,
...
ipcMain.handle('anakot:api', async (_event, request) => {
  ...
  // respawn the old-name backend and recreate its ANAKOT_HOME (#45474).
  ...
  if (!mutatingProfile) {
    return handleAnakotApiRequest(request)
  }

  const releaseProfileDeletion = profileDeletionGate.acquire(mutatingProfile)

  return handleAnakotApiRequest(request).finally(releaseProfileDeletion)
})
...
ipcMain.handle('anakot:ambient:claim', (_event, key) => ownsAmbientCue(String(key ?? '')))
...
ipcMain.handle('anakot:data-url-read-max:get', () => ({
...
ipcMain.handle('anakot:data-url-read-max:set', (_event, maxMb) => {
...
ipcMain.handle('anakot:readFileDataUrlForAttach', async (_event, filePath) => {
  return readFileDataUrlForIpc(filePath, {
...
ipcMain.handle('anakot:readFileText', async (_event, filePath) => {
...
// `anakot:readFileText` is the *preview* read and silently truncates at
...
ipcMain.handle('anakot:readPluginSource', async (_event: unknown, filePath: unknown) => {
  const { resolvedPath, stat } = await resolveReadableFileForIpc(filePath, {
...
ipcMain.handle('anakot:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('anakot:writeClipboard', (_event, text) => {
...
ipcMain.handle('anakot:selectSavePath', async (_event, options: any = {}) => {
  const result = await dialog.showSaveDialog(mainWindow, {
...
ipcMain.handle('anakot:readClipboard', () => clipboard.readText())

ipcMain.handle('anakot:saveGatewayFile', (_event, payload) => saveGatewayFile(payload))
...
ipcMain.handle('anakot:context-menu:edit', (event, command) => {
...
ipcMain.handle('anakot:context-menu:copy-image', event => {
...
ipcMain.handle('anakot:context-menu:spellcheck', (event, action) => {
...
ipcMain.handle('anakot:context-menu:guest-add-word', (_event, payload) => {
...
ipcMain.handle('anakot:capturePreview', async (_event, payload) => {
  const guest = electronWebContents.fromId(Number(payload?.webContentsId))
...
ipcMain.handle('anakot:savePastedText', async (_event, payload) => {
...
ipcMain.handle('anakot:saveClipboardImage', async () => {
...
ipcMain.handle('anakot:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('anakot:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('anakot:watchDirectory', (_event, dir) => watchDirectory(String(dir || '')))

ipcMain.handle('anakot:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))
...
ipcMain.on('anakot:active-work', (event, payload) => {
...
ipcMain.on('anakot:titlebar-theme', (_event, payload) => {
...
ipcMain.on('anakot:native-theme', (_event, mode) => {
...
ipcMain.on('anakot:translucency:support', event => {
...
// it arrives from `anakot desktop --local` or directly on Anakot.exe (a
// shortcut edit), and survives self-relaunches because collectRelaunchArgs
// only strips internal flags.
ipcMain.on('anakot:launch-flags', event => {
  event.returnValue = {
...
ipcMain.on('anakot:translucency', (_event, payload) => {
...
ipcMain.on('anakot:keep-awake', (_event, on) => {
...
ipcMain.handle('anakot:quick-entry:settings:get', async () => {
...
ipcMain.handle('anakot:quick-entry:settings:set', async (_event, patch) => {
...
ipcMain.on('anakot:quick-entry:submit', (_event, payload) => {
  ...
  mainWindow.webContents.send('anakot:quick-entry:submit', {
...
ipcMain.on('anakot:quick-entry:dismiss', () => hideQuickEntryWindow())
...
ipcMain.on('anakot:devtools:disable-f12', (_event, on) => {
...
ipcMain.handle('anakot:openExternal', (_event, url) => {
...
// per-session secondary windows spawned via `anakot:window:openSession`).
...
ipcMain.handle('anakot:find-in-page', async (event, query, options) => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anakot:stop-find-in-page', event => {
  const win = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('anakot:preview:reach', async (event, url) => reachablePreviewUrl(event.sender.id, String(url || '')))

ipcMain.handle('anakot:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveAnakotCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('anakot:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveAnakotCwd()
}))

ipcMain.handle('anakot:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('anakot:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('anakot:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('anakot:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('anakot:resolveFavicon', (_event, url) => resolveFaviconCached(url))

ipcMain.handle('anakot:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('anakot:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: anakotLog.slice(-200) }))
...
ipcMain.on('anakot:logs:renderer-error', (_event, report) => {
...
// Embedded terminal PTY host (anakot:terminal:*) — see terminal-ipc.ts.
...
ipcMain.handle('anakot:updates:check', async (_event, opts) =>
...
ipcMain.handle('anakot:updates:apply', async (_event, payload) =>
...
ipcMain.handle('anakot:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('anakot:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Anakot version (the one `release.py` bumps in
// anakot_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Anakot version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveAnakotVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'anakot_cli', '__init__.py')
...
// Renderer-bundle skew: `anakot update` moves the SOURCE TREE, but the UI
...
// Re-resolve the live Anakot version and push it into the native About panel
// just before showing it, so an in-place `anakot update` is reflected without
  ...
  void detectRendererSkew().then(skew => {
    app.setAboutPanelOptions({
      ...
      applicationVersion: skew.outOfSync
        ? `${resolveAnakotVersion()} — app build out of date, update the desktop app`
        : resolveAnakotVersion(),
      copyright: 'Copyright © 2026 Nous Research'
    })
    app.showAboutPanel()
  })
...
ipcMain.handle('anakot:version', async () => {
  const skew = await detectRendererSkew()

  return {
    appVersion: resolveAnakotVersion(),
...
// The About page's "Restart Anakot" button (shown when bundleSwapPending):
...
ipcMain.handle('anakot:app:relaunch', async () => {
...
ipcMain.handle('anakot:machine:profile', async () => {
...
// the actual removal via `anakot uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (anakot_cli/uninstall.py + anakot_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_ANAKOT_ROOT
  ...
  const fallback = () => ({
    anakot_home: ANAKOT_HOME,
    agent_installed: isAnakotSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'anakot_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, ANAKOT_HOME, NO_COLOR: '1' },
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no anakot agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import anakot_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_ANAKOT_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `anakot-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `anakot-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('anakot:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('anakot:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('anakot:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('anakot:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// anakot:// deep links (e.g. anakot://blueprint/morning-brief?time=08:00,
// anakot://mcp/install?name=NAME&config=B64 — the vendor "Add to Anakot"
// button, or anakot://plugin/install?repo=owner/repo). Dev
// (`ANAKOT_DESKTOP_DEV_SERVER`) registers anakot-dev:// instead — bare
// Electron or a stale OS handler often owns anakot:// on dev machines.
...
// ---------------------------------------------------------------------------
const ANAKOT_PROTOCOL = DEV_SERVER ? 'anakot-dev' : 'anakot'
/** Schemes accepted when parsing inbound URLs (dev accepts both). */
const DEEPLINK_SCHEMES = DEV_SERVER ? ['anakot-dev', 'anakot'] : ['anakot']
...
function handleDeepLink(url) {
  ...
  // anakot://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('anakot:deep-link', payload)
...
ipcMain.handle('anakot:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${ANAKOT_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(ANAKOT_PROTOCOL, process.execPath, [entry])
    } else {
      app.setAsDefaultProtocolClient(ANAKOT_PROTOCOL)
    }

    rememberLog(`[deeplink] registered ${ANAKOT_PROTOCOL}:// handler`)
  } catch (err) {
...
// second-instance argv. Without the lock a second `anakot://` launch spawns a
...
if (!isPrimaryInstance) {
  ...
  // startup (shortcut registration, createWindow → startAnakot), whose
...
app.whenReady().then(() => {
  ...
  // startAnakot() finishes resolving the configured backend.
  const primaryProfile = primaryProfileKey()
  ...
  // Win/Linux cold start: the launching anakot:// URL is in our own argv.
...
app.on('before-quit', event => {
  ...
  // quitting Anakot never keeps another app's chord hostage.
