 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `anakot update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.anakotHome, 'Anakot home')
  const anakotPath = validateRemoteValue(target.anakotPath, 'launcher path')
  ...
  const launcherWord = expandRemotePath(anakotPath)

  const updateCommand =
    `env ANAKOT_HOME=${homeWord} ` +
    `ANAKOT_UPDATE_CORRELATION_ID=${shq(correlation)} ` +
    'ANAKOT_UPDATE_ORIGIN_PROFILE=default ' +
    `ANAKOT_UPDATE_ORIGIN_HOME=${homeWord} ` +
    `ANAKOT_UPDATE_OUTPUT_PATH=${outputWord} ` +
...
function buildWindowsManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const wrapper = [
    ...
    `$env:ANAKOT_HOME=${psLiteral(home)}`,
    `$env:ANAKOT_UPDATE_CORRELATION_ID=${psLiteral(correlation)}`,
    '$env:ANAKOT_UPDATE_ORIGIN_PROFILE="default"',
    `$env:ANAKOT_UPDATE_ORIGIN_HOME=${psLiteral(home)}`,
    `$env:ANAKOT_UPDATE_OUTPUT_PATH=${psLiteral(outputPath)}`,
    ...
    `$env:ANAKOT_UPDATE_WINDOWS_DETACHED=${psLiteral(correlation)}`,
    `$env:ANAKOT_UPDATE_TAURI_OUTCOME_PATH=${psLiteral(statusPath)}`,
    `$env:ANAKOT_UPDATE_TAURI_READY_PATH=${psLiteral(readyPath)}`,
    ...
    `& ${psLiteral(anakotPath)} update --yes *>> ${psLiteral(outputPath)}`,
    '$rc=$LASTEXITCODE',
...
marker_path=install_root/'.anakot-update-in-progress'
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Anakot updated and every managed SSH profile is ready.'
        : restoreOk
