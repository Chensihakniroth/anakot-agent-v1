 ...
 * remote-lifecycle.ts
 *
 * Pure, electron-free remote Anakot dashboard lifecycle over SSH for Desktop
 ...
 *
 *   - locate the Anakot install on the remote (login-shell probe),
 ...
 *     `ANAKOT_DASHBOARD_READY port=<n>` readiness line,
 ...
 * the real SshConnection, fetch, adoptServedDashboardToken, and waitForAnakot in.
 *
 * The minted ANAKOT_DASHBOARD_SESSION_TOKEN is the SPAWN credential. After
...
const REMOTE_LOCK_DIR = '~/.anakot/desktop-ssh'
...
// Resolve the remote anakot executable. An EXPLICIT path is honored strictly
...
async function locateAnakot(ssh, remoteAnakotPath) {
  const resolveLauncher = async (candidate: string) => {
    // Return the candidate path directly. The anakot binary or wrapper script
    ...
    //     the Anakot version, and
    //   - capability probing: `<python> serve --help` failed entirely.
    // See https://github.com/Chensihakniroth/anakot-agent-v1/issues/74411
    return candidate
  }
  ...
  if (remoteAnakotPath) {
    if (await isExecutable(remoteAnakotPath)) {
      return resolveLauncher(remoteAnakotPath)
    }

    const err: any = new Error(
      `The Anakot path you set is not an executable on the remote host: "${remoteAnakotPath}". ` +
        'Check the path (it must be the full path to the `anakot` binary on the remote, e.g. ' +
        '~/anakot-agent/.venv/bin/anakot), or clear it to auto-detect.'
    )

    err.kind = 'anakot-not-found'
    throw err
  }
  ...
  try {
    const found = (await ssh.exec(`bash -lc ${shq('command -v anakot')}`)).trim()
  ...
  candidates.push('~/.local/bin/anakot')
  candidates.push('/usr/local/bin/anakot')
  candidates.push('~/.anakot/anakot-agent/venv/bin/anakot')
...
// Probe the resolved binary's version string (first line of `<anakot> --version`,
// e.g. "Anakot Agent v0.18.2 ..."), or '' on failure. Surfaces WHICH anakot a
// connection uses, so a stale/unexpected install is visible.
async function probeAnakotVersion(ssh, anakotPath) {
  try {
    // Watchdogged: a hung remote CLI must die remotely instead of orphaning
    // when the local ssh child is SIGKILLed (#110478).
    const out = (await ssh.exec(withRemoteTimeout(`${expandRemotePath(anakotPath)} --version 2>&1`))).trim()
...
async function probeRemotePlatform(ssh) {
  ...
  if (!SUPPORTED_REMOTE_OS.has(osName)) {
    const err: any = new Error(
      `Unsupported remote platform "${osName || 'unknown'}". Anakot Desktop SSH mode supports Linux, macOS, and Windows remote hosts.`
    )
...
// The ANAKOT_HOME the remote dashboard will use (explicit env wins, else
// ~/.anakot). Recorded in the lockfile so a future reuse can tell it's the same
// state store; best-effort.
async function probeRemoteAnakotHome(ssh) {
  try {
    const out = (await ssh.exec('echo "${ANAKOT_HOME:-$HOME/.anakot}"')).trim().split('\n').pop()

    return out || '~/.anakot'
  } catch (cause) {
    const error: any = new Error('Could not resolve the remote Anakot home.')
...
marker=home/'.anakot-update-in-progress'
try:
 ...
 * bytes; it never imports or executes code from the changing Anakot checkout.
 ...
 */
async function assertRemoteInstallUpdateClear(ssh, anakotHome) {
  const home = assertSafeRemoteHome(anakotHome)
  let observation = ''
  ...
  try {
    ...
    const error: any = new Error('Could not prove that the remote Anakot install is clear for SSH startup.')
  ...
  const error: any = new Error(
    live
      ? `Remote Anakot update process ${live[1]} is still running; SSH startup is paused.`
      : 'The remote Anakot update marker is unreadable or malformed; refusing SSH startup.'
  )
...
async function listRemoteAnakotProfiles(ssh) {
  const home = assertSafeRemoteHome(await probeRemoteAnakotHome(ssh))
  ...
  try {
    ...
    const error: any = new Error('Could not list remote Anakot profiles.')
    error.kind = 'transient-transport-error'
    error.cause = cause
    throw error
  }
...
function assertSafeRemoteHome(home) {
  ...
  if (!/^(\/|~\/)[A-Za-z0-9._/+-]+$/.test(value) || value.includes('..')) {
    const error: any = new Error('Unsafe remote Anakot home.')
...
async function readLockfile(ssh, ownershipId) {
  ...
  for (const field of ['profile', 'anakotPath', 'anakotHome', 'logPath', 'startedAt']) {
...
async function pidIsOurDashboard(
  ...
  if (!pid || !/^[0-9a-f]{16}$/.test(String(spawnNonce || '')) || !anakotPath) {
    return false
  }
  ...
  const script =
      ...
      `pid=${Number(pid)}\n` +
      `expected=os.path.expanduser(${shq(anakotPath)})\n` +
      // The installer-facing launcher is intentionally preserved for invocation
      // (#74411), but it may `exec python <install-dir>/anakot`, leaving neither
      // launcher nor ANAKOT_HOME-derived entrypoint in argv. The ownership-scoped
      // token path + random nonce + exact profile below are the alternative proof.
      `anakot_home=os.path.expanduser(${shq(anakotHome)}) if ${shq(anakotHome)} else ""\n` +
      'expected_entries={expected}\n' +
      'if anakot_home:\n' +
      ' expected_entries.add(os.path.join(anakot_home,"anakot-agent","venv","bin","anakot"))\n' +
...
function buildOwnedStaleTerminationCommand(lock, ownershipId) {
  ...
  const expectedPath = expandRemotePath(lock.anakotPath)
  const expectedHome = lock.anakotHome ? expandRemotePath(lock.anakotHome) : "''"
  ...
  const executableMatch = lock.anakotHome
...
function lockMatchesManagedUpdateScope(lock, expected) {
  return Boolean(
    ...
    lock.anakotPath === expected.anakotPath &&
    lock.anakotHome === expected.anakotHome
  )
}
...
expected_path=os.path.expanduser(${py(lock.anakotPath)})
anakot_home=os.path.expanduser(${py(lock.anakotHome)})
expected_entries={expected_path,os.path.join(anakot_home,"anakot-agent","venv","bin","anakot")}
...
// execing Anakot. mutexPath is expandRemotePath() output — a complete shell
...
result=None
try:
 result=subprocess.run(["sh","-c",payload,"anakot-update-mutex",str(fd)],pass_fds=(fd,),check=False)
finally:
 os.close(fd)
...
function buildSpawnCommand(anakotPath, profile, opts: any = {}) {
  const anakot = expandRemotePath(anakotPath)
  ...
  const marker = expandRemotePath(`${remoteInstallRoot(opts.anakotHome || '~/.anakot')}/.anakot-update-in-progress`)

  const updateMutex = expandRemotePath(
    `${remoteInstallRoot(opts.anakotHome || '~/.anakot')}/.anakot-update-in-progress.mutex`
  )
  ...
  const dashCmd =
    ...
    `exec env ANAKOT_DESKTOP=1${opts.guestOnboarding === true ? ' ANAKOT_GUEST_ONBOARDING=1' : ''} ${anakot} ${profileArgs}${subCmd}`

  const detachedShell = `eval "exec $1>&-"; ${dashCmd} </dev/null >> ${logPath} 2>&1 & echo $!`
  const detachedSpawn = `child=$("$(command -v setsid || echo nohup)" sh -c ${shq(detachedShell)} anakot-update-child "$1" & echo $!)`
...
async function remoteSupportsSshOwnership(ssh, anakotPath) {
  const anakot = expandRemotePath(anakotPath)
  ...
  const out = await ssh.exec(
    `help="$( ${withRemoteTimeout(`${anakot} serve --help 2>&1`)} )"; ` +
...
async function spawnRemoteDashboard(
  ...
  if (!(await remoteSupportsSshOwnership(ssh, anakotPath))) {
    const err: any = new Error(
      'The remote Anakot install does not support --ssh-session-token-file and --ssh-owner-nonce. ' +
        'Update Anakot on the remote host to continue using Desktop SSH mode.'
    )
  ...
  try {
    ...
    out = await ssh.exec(
      buildSpawnCommand(anakotPath, profile, {
        spawnNonce,
        tokenFilePath,
        logPath,
        anakotHome,
 ...
 * opened SshConnection, forward/pickLocalPort/waitForAnakot, a token-gated
...
async function connect(deps) {
  ...
  const anakotHome = await probeRemoteAnakotHome(ssh)
  await assertRemoteInstallUpdateClear(ssh, anakotHome)
  const anakotPath = await locateAnakot(ssh, remoteAnakotPath)
  log(`located anakot at ${anakotPath}`)
  const anakotVersion = await probeAnakotVersion(ssh, anakotPath)

  if (anakotVersion) {
    log(`remote anakot version: ${anakotVersion}`)
  }
  ...
  if (isLockfileSkew(lock)) {
    ...
    const error: any = new Error(
      `The remote ownership record ${lpath} does not match this Anakot Desktop build (${lock.reason}). ` +
  ...
  if (lock) {
    ...
    const reusable =
      ...
      lock.anakotPath === anakotPath &&
      lock.anakotHome === anakotHome
  ...
  const spawned = await spawnRemoteDashboard(ssh, {
    ...
    anakotHome,
    guestOnboarding,
    assertInstallClear: () => assertRemoteInstallUpdateClear(ssh, anakotHome)
  })
  ...
  try {
    ...
    assertBootstrapNotSuperseded(signal)
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAnakot(baseUrl, spawnToken)
    assertBootstrapNotSuperseded(signal)
