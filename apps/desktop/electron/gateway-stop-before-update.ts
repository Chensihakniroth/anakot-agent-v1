 ...
 *
 * So we delegate to `anakot gateway stop --all`: the CLI discovers every
 ...
 * force-kills survivors — the same logic `anakot update`'s
...
export function stopGatewayBeforeUpdate(
  anakotCliPath: string,
  anakotHome: string,
  deps: StopGatewayBeforeUpdateDeps = {}
): boolean {
  return runGatewayLifecycleCommand(anakotCliPath, ['gateway', 'stop', '--all'], deps)
}
 ...
 */
export function startGatewaysAfterUpdateAbort(anakotCliPath: string, deps: StopGatewayBeforeUpdateDeps = {}): boolean {
  return runGatewayLifecycleCommand(anakotCliPath, ['gateway', 'start', '--all'], deps)
}

function runGatewayLifecycleCommand(anakotCliPath: string, args: string[], deps: StopGatewayBeforeUpdateDeps): boolean {
  ...
  if (deps.spy) {
    deps.spy(anakotCliPath, args)
  }

  if (!existsSync(anakotCliPath)) {
    return false
  }

  try {
    exec(anakotCliPath, args, {
