...
export function registerHudIpc({
  ...
  ipcMain.on('anakot:hud:native-drag', event => {
    event.returnValue = hudWindowing().move === 'native-drag'
  })

  ipcMain.on('anakot:hud:windowing', event => {
  ...
  ipcMain.on('anakot:hud:workspace-transfer', (event, transferring) => {
  ...
  ipcMain.handle('anakot:hud:open', async (_event, request) => {
    openHudWindow(
  ...
  ipcMain.handle('anakot:hud:frost', (_event, showing) => {
  ...
  // reaches the bar.
  ipcMain.on('anakot:hud:ignore-mouse', (_event, ignore) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anakot:hud:begin-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anakot:hud:end-move', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anakot:hud:move-by', (event, delta) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anakot:hud:set-bounds', (event, bounds) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('anakot:hud:reset-layout', event => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.on('anakot:hud:session', (event, sessionId) => {
    const hudWindow = getHudWindow()
  ...
  ipcMain.handle('anakot:hud:close', async () => {
    closeHudWindow()
