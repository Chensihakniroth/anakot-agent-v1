...
test('buildNativeAuthorizeUrl omits provider when not given and preserves prefix', () => {
  const url = buildNativeAuthorizeUrl('https://gw.example.com/anakot', {
    challenge: 'C',
  ...
  assert.equal(parsed.pathname, '/anakot/auth/native/authorize')
...
test('nativeTokenUrl / nativeRefreshUrl build the right endpoints', () => {
  ...
  assert.equal(nativeRefreshUrl('https://gw.example.com/anakot'), 'https://gw.example.com/anakot/auth/native/refresh')
})
