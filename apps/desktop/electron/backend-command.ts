// Backend subcommand routing for the desktop-managed Anakot process.
//
// The desktop app launches its own headless backend via `anakot serve` — it
...
// app hasn't updated yet, or an older `anakot` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Anakot profile to pin via `--profile`.
 */
 ...
 * `-m anakot_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `anakot_cli/subcommands/dashboard.py` source registers
