 ...
 *     `X-Anakot-Session-Token` header; WS uses `?token=`.
...
// anakot_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - anakot_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - anakot_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / anakot #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-anakot_session_at', '__Secure-anakot_session_at', 'anakot_session_at']
const RT_COOKIE_VARIANTS = ['__Host-anakot_session_rt', '__Secure-anakot_session_rt', 'anakot_session_rt']

// Keep this aligned with anakot_cli.profiles.validate_profile_name(). `default`
// is the built-in root alias; these names cannot be created as profiles.
const RESERVED_REMOTE_PROFILES = new Set(['anakot', 'test', 'tmp', 'root', 'sudo'])
...
function gatewayTicketFailure(error, authMessage, transportMessage) {
  ...
  if (needsOauthLogin) {
    ...
    // change until the user signs in, so tag it the way startAnakot latches
...
/** Which Anakot profile the remote SSH dashboard should actually run as.
...
// Anakot Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  const remoteAnakotPath = String(entry.remoteAnakotPath || '').trim()

  if (remoteAnakotPath) {
    out.remoteAnakotPath = remoteAnakotPath
  }

  // A Desktop profile can be a local routing label rather than the profile
  // name used by the remote Anakot installation. Preserve an explicit mapping
  // when it is a valid Anakot profile identifier; otherwise fall back to the
...
function localPrimaryRequestScope(opts: ProfileRouteOptions): boolean | null {
  ...
  // spawned them: `_spawn_anakot_action` registers the (often dynamic, e.g.
 ...
 *     `--profile`, so its `ANAKOT_HOME` scopes it.
 *
...
function resolveProfileBackendRoute(profile, opts: ProfileRouteOptions = {}): ProfileBackendRoute {
  ...
  if (scopedProfile === primaryProfile) {
    ...
    // the wire: the dashboard's process ANAKOT_HOME can belong to a different
 ...
 * `anakot:api` handler — both routes reach a backend whose namespace is the
...
function translateSelfProfileQuery(path, profile, backendProfile) {
  ...
  try {
    parsed = new URL(rawPath, 'http://anakot.local')
  } catch {
    return path
  }
...
/**
 * Resolve the two decisions made by the `anakot:api` IPC handler from the same
...
/**
 * True if any cookie in `cookies` is a anakot session ACCESS-token cookie
