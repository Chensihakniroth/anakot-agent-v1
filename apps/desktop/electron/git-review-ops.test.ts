...
function makeRepo() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'anakot-desktop-git-status-'))

  tempDirs.push(dir)
  execFileSync('git', ['init', '-q'], { cwd: dir })
  execFileSync('git', ['config', 'user.email', 'anakot-test@example.com'], { cwd: dir })
  execFileSync('git', ['config', 'user.name', 'Anakot Test'], { cwd: dir })
