...
export function markerPath(anakotHome) {
  return path.join(anakotHome, '.anakot-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(anakotHome)
  let raw
 ...
 *
 * The Tauri-based anakot-setup.exe takes several seconds to initialise its
...
export function writeUpdateMarker(
  ...
  const file = markerPath(anakotHome)
  const nowMs = now()
  const owner = readLiveUpdateMarker(anakotHome, { kill, maxAgeMs, now: () => nowMs })
 ...
 * Anakot to exit…") clobbers that still-running updater's claim: the
...
export function updateHandoffConflict(
  ...
  const owner = readLiveUpdateMarker(anakotHome, opts)
