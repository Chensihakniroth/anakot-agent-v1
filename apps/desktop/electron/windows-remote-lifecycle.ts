...
const READY_RE = /^ANAKOT_(?:BACKEND|DASHBOARD)_READY port=(\d+)/gm
...
async function probeWindowsRemote(ssh, explicitAnakotPath = '') {
  const explicit = psLiteral(explicitAnakotPath)
  ...
  const script = [
    ...
    '$anakotHome=$env:ANAKOT_HOME',
    'if(-not $anakotHome){$anakotHome=Join-Path $env:LOCALAPPDATA "anakot"}',
    'Assert-NoReparse $anakotHome $true',
    '$candidate=[IO.Path]::Combine($anakotHome, "anakot-agent\\venv\\Scripts\\anakot.exe")',
    ...
    '$profileCandidate=[IO.Path]::Combine($HOME, "anakot-agent\\.venv\\Scripts\\anakot.exe")',
    ...
    '$fallbackHomeCandidate=Join-Path $anakotHome "anakot-agent\\venv\\Scripts\\anakot.exe"',
    '$fallbackProfileCandidate=Join-Path $HOME "anakot-agent\\.venv\\Scripts\\anakot.exe"',
    '$candidates=@()',
    'if($explicit){$candidates+=$explicit}',
    '$cmd=Get-Command anakot.exe -ErrorAction SilentlyContinue',
    ...
    '$anakot=$null',
    'foreach($candidate in $candidates){Assert-NoReparse $candidate $true;$candidatePython=[IO.Path]::Combine([IO.Path]::GetDirectoryName($candidate), "python.exe");Assert-NoReparse $candidatePython $true;try{$item=Get-Item -LiteralPath $candidate -Force -ErrorAction Stop;if(($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -eq 0 -and -not $item.PSIsContainer){$anakot=$item.FullName;break}}catch [Management.Automation.ItemNotFoundException]{continue}}',
    'if(-not $anakot){throw "Anakot is not installed on the remote Windows host."}',
    'Assert-NoReparse $anakot $false',
    'if($explicit -and $anakot -ne $explicit){throw "The configured Anakot path is not an executable file."}',
    '$python=[IO.Path]::Combine([IO.Path]::GetDirectoryName($anakot), "python.exe")',
    'Assert-NoReparse $python $false',
    '[ordered]@{os="Windows";arch=$env:PROCESSOR_ARCHITECTURE;anakotHome=$anakotHome;anakotPath=$anakot;python=$python}|ConvertTo-Json -Compress'
  ].join(';')

  return JSON.parse((await ssh.exec(powerShellCommand(script))).trim())
}

function windowsUpdateMarkerProbeCommand(anakotHome) {
  const script = [
    '$ErrorActionPreference="Stop"',
...
public static class AnakotMarkerNoFollow {
...
`,
    ...
    '}',
    '}',
    `$anakotHome=${psLiteral(anakotHome)}`,
    '$installRoot=$anakotHome',
    '$parent=Split-Path -Parent $anakotHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".anakot-update-in-progress"',
    '$result="UNCERTAIN"',
    '$stream=$null;$memory=$null',
    'try{',
    'Assert-NoReparse $marker $true',
    'if(-not (Test-Path -LiteralPath $marker -PathType Leaf)){$result="CLEAR"}else{$stream=[AnakotMarkerNoFollow]::OpenRead($marker)',
 ...
 */
async function assertWindowsRemoteInstallUpdateClear(ssh, anakotHome) {
  let observation = ''

  try {
    observation =
      String(await ssh.exec(windowsUpdateMarkerProbeCommand(anakotHome)))
        .replace(/^\uFEFF/, '')
        .trim()
        .split(/\r?\n/)
        .pop() || ''
...
async function detectRemotePlatform(ssh, explicitAnakotPath = '') {
  try {
  ...
  try {
    return await probeWindowsRemote(ssh, explicitAnakotPath)
  } catch (cause: any) {
...
function helperCommand(runtime, operation, args = []) {
  const argv = [runtime.python, '-m', 'anakot_cli.windows_ssh_runtime', operation, ...args]
...
function atomicWindowsSpawnCommand(runtime, reservation: any = {}) {
  const argv = [runtime.python, '-m', 'anakot_cli.windows_ssh_runtime', 'spawn']
  const helper = operation => [runtime.python, '-m', 'anakot_cli.windows_ssh_runtime', operation]

  const script = [
    '$ErrorActionPreference="Stop"',
    `$anakotHome=${psLiteral(runtime.anakotHome)}`,
    '$installRoot=$anakotHome',
    '$parent=Split-Path -Parent $anakotHome',
    'if((Split-Path -Leaf $parent) -ieq "profiles"){$installRoot=Split-Path -Parent $parent}',
    '$marker=Join-Path $installRoot ".anakot-update-in-progress"',
    ...
    reservation.ownershipId
      ...
      : '  if($LASTEXITCODE -ne 0){exit $LASTEXITCODE}',
    reservation.ownershipId
      ? `  $spawned=$spawnLines[-1]|ConvertFrom-Json; $lock=[ordered]@{schemaVersion=2;protocolVersion=1;ownershipId=${psLiteral(reservation.ownershipId)};spawnNonce=${psLiteral(reservation.spawnNonce)};pid=[int]$spawned.pid;creationTimeNs=[string]$spawned.creationTimeNs;port=0;profile=${psLiteral(reservation.profile)};anakotPath=${psLiteral(reservation.anakotPath)};anakotHome=${psLiteral(reservation.anakotHome)};tokenFingerprint=${psLiteral(reservation.tokenFingerprint)};startedAt=${psLiteral(reservation.startedAt)}}|ConvertTo-Json -Compress; ` +
...
function validLock(lock, ownershipId) {
  ...
  return Boolean(
    ...
    typeof lock.anakotPath === 'string' &&
    typeof lock.anakotHome === 'string'
  )
}
...
function reusableWindowsLock(lock, state, profile, reuseToken, runtime) {
  return Boolean(
    ...
    lock.anakotPath === runtime.anakotPath &&
    lock.anakotHome === runtime.anakotHome
  )
}
...
async function connectWindowsRemote(deps) {
  ...
  assertBootstrapNotSuperseded(signal)
  const runtime = await probeWindowsRemote(ssh, remoteAnakotPath)
  await assertWindowsRemoteInstallUpdateClear(ssh, runtime.anakotHome)
  const inspection = await helper(ssh, runtime, 'inspect', [runtime.anakotPath])

  if (!inspection.supported) {
    const error: any = new Error('Update Anakot on the remote Windows host before connecting with Desktop SSH.')
    error.kind = 'update-required'
    throw error
  }

  runtime.anakotPath = inspection.path
  const anakotVersion = inspection.version || ''
  rememberLog(`[ssh-lifecycle] remote platform Windows/${runtime.arch}`)
  rememberLog(`[ssh-lifecycle] located anakot at ${runtime.anakotPath}`)
  ...
  try {
    ...
    spawned = await atomicWindowsSpawn(
      ssh,
      runtime,
      JSON.stringify({ ownershipId, spawnNonce, profile, anakotPath: runtime.anakotPath }),
      {
        ownershipId,
        spawnNonce,
        profile,
  ...
  try {
    ...
    const baseUrl = `http://127.0.0.1:${localPort}`
    await waitForAnakot(baseUrl, token)
    assertBootstrapNotSuperseded(signal)
...
function buildWindowsInteractiveCommand(remoteCwd = '') {
  ...
  script.push('$host.UI.RawUI.WindowTitle="Anakot SSH"', 'powershell.exe -NoLogo')
