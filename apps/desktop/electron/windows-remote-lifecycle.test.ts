...
test('Windows spawn holds the update mutex across marker check and helper spawn', () => {
  const command = atomicWindowsSpawnCommand({
    anakotHome: 'C:\\Users\\andre\.anakot',
    python: 'C:\\Users\\andre\.anakot\\python.exe'
  })

  const encoded = command.match(/-EncodedCommand\s+([^\s]+)$/)?.[1]
  const script = encoded ? Buffer.from(encoded, 'base64').toString('utf16le') : ''
  assert.match(script, /.anakot-update-in-progress/)
...
test('Windows spawn publishes the initial ownership record before releasing the mutex', () => {
  const command = atomicWindowsSpawnCommand(
    ...
    },
    {
      ...
      profile: 'default',
      anakotPath: 'C:\\Anakot\\anakot.exe',
      anakotHome: 'C:\\Users\\andre\.anakot',
...
test('every emitted PowerShell script keeps try blocks attached to their catch/finally handlers', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(
    ...
    'C:\\Users\\alice\.anakot'
  )
  scripts.push(
    decode(atomicWindowsSpawnCommand({ anakotHome: 'C:\\Users\\alice\.anakot', python: 'C:\\py\\python.exe' })),
...
test('Windows relaunch gate refuses live and uncertain markers before executing the remote runtime', async () => {
  for (const observation of ['LIVE:4242', 'UNCERTAIN']) {
    ...
    const ssh = sshWith(async command => {
      ...
      if (script.includes('Get-Command anakot.exe')) {
        return JSON.stringify({
          os: 'Windows',
          arch: 'AMD64',
          anakotHome: 'C:\\Users\\alice\.anakot',
          anakotPath: 'C:\\Anakot\\anakot.exe',
          python: 'C:\\Anakot\\python.exe'
        })
      }

      if (script.includes('.anakot-update-in-progress')) {
        return observation
      }
    ...
    assert.equal(
      scripts.some(script => script.includes('anakot_cli.windows_ssh_runtime')),
      false
    )
  }
})
...
test('Windows relaunch gate uses strict install-wide marker parsing and fail-closed PID probing', async () => {
  ...
  await assertWindowsRemoteInstallUpdateClear(ssh, 'C:\\Users\\alice\.anakot\\profiles\\research')
  assert.match(script, /.anakot-update-in-progress/)
...
test('Windows probe validates Anakot and Python topology before selection', async () => {
  let script = ''
  await probeWindowsRemote(
    sshWith(async command => {
      script = Buffer.from(command.split(' ').at(-1) || '', 'base64').toString('utf16le')
  ...
  const fallbackJoin = script.indexOf('Join-Path $anakotHome')
  ...
  await assert.rejects(
    detectRemotePlatform(
      sshWith(async command => {
        ...
        throw new Error('Anakot is not installed on the remote Windows host.')
      })
    ),
    (err: any) => err.kind === 'unsupported-platform' && /Anakot is not installed/.test(err.message)
  )
})

test('helper command uses the fixed remote Python entry point and quotes path data', () => {
  const command = helperCommand({ python: "C:\\Program Files\\Anakot's\\python.exe" }, 'inspect', [
    'C:\\x y\\anakot.exe'
  ])

  const encoded = command.split(' ').pop()!
  const script = Buffer.from(encoded, 'base64').toString('utf16le')
  assert.match(script, /-m' 'anakot_cli\.windows_ssh_runtime' 'inspect'/)
  assert.match(script, /Anakot''s/)
  assert.match(script, /C:\\x y\\anakot\.exe/)
})
...
test('Windows SSH reuse requires the requested remote profile to match the lock', () => {
  ...
  const runtime = { anakotPath: lock.anakotPath, anakotHome: lock.anakotHome }
