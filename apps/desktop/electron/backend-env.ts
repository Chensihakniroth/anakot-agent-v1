 ...
 * `iter_anakot_node_dirs()` in anakot_constants.py, which the Electron main
 * process cannot import.
 */
function anakotManagedNodePathEntries(
  anakotHome,
  ...
  const root = pathModule.join(anakotHome, 'node')
...
function buildDesktopBackendPath({
  ...
  const anakotNodeDirs = anakotManagedNodePathEntries(anakotHome, { platform, pathModule })
  const venvBin = venvRoot ? pathModule.join(venvRoot, platform === 'win32' ? 'Scripts' : 'bin') : null
  const saneEntries = platform === 'win32' ? [] : POSIX_SANE_PATH_ENTRIES

  return appendUniquePathEntries([anakotNodeDirs, venvBin, currentPath, saneEntries], { delimiter })
}

function normalizeAnakotHomeRoot(
  anakotHome,
  ...
  // would pin it under cwd and the Python backend inherits that absolute path via ANAKOT_HOME.
  let raw = String(anakotHome)
...
function buildDesktopBackendEnv({
  ...
  return {
    ...
    // cp1252, ...). anakot_bootstrap sets this inside the child too, but only
