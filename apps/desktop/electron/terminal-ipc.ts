...
// registry, and the anakot:terminal:* IPC surface. Extracted from main.ts; the
...
export function registerTerminalIpc({
  ...
  function terminalShellCommand() {
    // ANAKOT_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
    ...
    const override = (process.env.ANAKOT_DESKTOP_SHELL || (isWindows ? '' : process.env.SHELL) || '').trim()
  ...
  function terminalShellEnv() {
    ...
    // /TERM=dumb; some terminals set COLORFGBG which would flip Anakot' TUI into
    ...
    env.TERM_PROGRAM = 'Anakot'
    env.TERM_PROGRAM_VERSION = app.getVersion()

    // Let a anakot/--tui launched in this pane know it's embedded in the desktop
    // GUI (build_environment_hints surfaces this). Distinct from ANAKOT_DESKTOP,
    // which marks the agent *backend* and gates cron/gateway behavior.
    env.ANAKOT_DESKTOP_TERMINAL = '1'

    return env
  }

  function terminalChannel(id, suffix) {
    return `anakot:terminal:${id}:${suffix}`
  }
  ...
  ipcMain.handle('anakot:terminal:attach', (event, id) => {
  ...
  ipcMain.handle('anakot:terminal:write', (_event, id, data) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('anakot:terminal:resize', (_event, id, size = {}) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
  ...
  ipcMain.handle('anakot:terminal:cwd', async (_event, id) => {
    const sessionInfo = terminalSessions.get(String(id || ''))
