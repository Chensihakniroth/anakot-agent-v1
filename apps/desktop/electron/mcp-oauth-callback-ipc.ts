 ...
 *   - the browser sees only a minimal "return to Anakot" page.
 */
...
const DONE_HTML =
  ...
  '<p>You can close this window and return to Anakot.</p>' +
...
export function registerMcpOauthCallbackIpc() {
  ...
  ipcMain.handle('anakot:mcp-oauth:listen', async () => {
  ...
  ipcMain.handle('anakot:mcp-oauth:wait', async (_event, id, timeoutMs) => {
  ...
  ipcMain.handle('anakot:mcp-oauth:cancel', (_event, id) => {
    dispose(String(id || ''))
