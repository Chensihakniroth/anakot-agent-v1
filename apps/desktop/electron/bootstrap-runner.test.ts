...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'anakot-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/anakot-runner-test',
    sourceRepoRoot: null,
    anakotHome: '/tmp/anakot-runner-test',
    logRoot: '/tmp/anakot-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'anakot-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'anakot-agent')
...
test('fresh bootstrap args include the packaged commit pin', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp,
      activeRoot: '/tmp/anakot-agent',
      anakotHome: '/tmp/anakot'
    }),
    ['--dir', '/tmp/anakot-agent', '--anakot-home', '/tmp/anakot', '--branch', 'main', '--commit', installStamp.commit]
  )
})
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/anakot-agent', '--anakot-home', '/tmp/anakot', '--branch', 'main']
  )
})
...
test('fallback install stamps use an unpinned branch ref', () => {
  ...
  assert.deepEqual(
    buildPosixPinArgs({
      installStamp: stamp,
      activeRoot: '/tmp/anakot',
      anakotHome: '/tmp/home'
    }),
    ['--dir', '/tmp/anakot', '--anakot-home', '/tmp/home', '--branch', 'main']
  )
})
...
test('installer log lines reach the emitter without escape sequences; \\r redraws keep the last frame', () => {
  ...
  assert.equal(cleanInstallerLogLine('\u001b]0;anakot\u0007Installing Anakot'), 'Installing Anakot')
