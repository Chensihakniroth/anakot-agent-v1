/**
 * windows-anakot-path.ts
 *
 * Pure, dependency-injected pieces of Windows `anakot` resolution pulled out
 * of main.ts's findOnPath(), handOffWindowsBootstrapRecovery(), and
 * unwrapWindowsVenvAnakotCommand(). Each of the three functions here pins one
 ...
 *      FIRST, so an extensionless Git-Bash `anakot` shim shadowed the real
 *      anakot.cmd/anakot.exe; the shim then failed the --version probe and
 ...
 *      and Anakot launcher. Marker-only or partial runtimes must use --repair;
 *      only a runnable pair can use --update.
 *   3. resolveVenvAnakotCommand() — unwrapWindowsVenvAnakotCommand() returned
 ...
 * extensionless file (e.g. a Git-Bash shell-script shim named `anakot`) must
 * not shadow `anakot.cmd`/`anakot.exe`. The empty entry is kept LAST so
 ...
 * venv Python interpreter and the Anakot launcher that drives `anakot update`.
...
export interface ResolveVenvAnakotCommandDeps {
  isWindows: boolean
  isCommandScript: (command: string) => boolean
  fileExists: (filePath: string) => boolean
  directoryExists: (filePath: string) => boolean
  canImportAnakotCli: (python: string, opts?: { env?: Record<string, string> }) => Promise<boolean>
...
/**
 * If `command` is a Windows venv `anakot`/`anakot.exe` console-script shim
 * (i.e. `<venvRoot>/Scripts/anakot(.exe)`), resolve it to the underlying
 * venv python invoked as `python -m anakot_cli.main <backendArgs>` — but
 * ONLY after smoke-testing that interpreter with canImportAnakotCli(). A
 * venv whose update died mid-`pip install` still has python.exe + anakot.exe
 ...
 *
 * Returns null when `command` is not a venv anakot shim, the underlying
 * python doesn't exist, or the import probe fails. Otherwise returns the
 * resolved backend descriptor.
 */
export async function resolveVenvAnakotCommand(
  command: string,
  backendArgs: string[],
  deps: ResolveVenvAnakotCommandDeps
): Promise<{
  label: string
  command: string
  args: string[]
  ...
  if (!/^anakot(?:\.exe)?$/i.test(basename(resolved))) {
    return null
  }
  ...
  if (
    !(await canImportAnakotCli(python, {
      env: {
    ...
    rememberLog?.(
      `Ignoring venv Anakot at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Anakot Python at ${python}`,
    command: python,
