 ...
 * the three user-facing uninstall modes to the `anakot uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m anakot_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `anakot_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'anakot_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Anakot.app/Contents/MacOS/Anakot  → …/Anakot.app
 *   Windows: …\Anakot\Anakot.exe                 → …\Anakot  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Anakot.app/Contents/MacOS/Anakot → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Anakot.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Anakot.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Anakot$/i.test(dir) || /[\\/]anakot-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/anakot
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import anakot_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, anakotHome }) {
  ...
  const lines = [
    ...
    '  done',
    'fi',
    `export ANAKOT_HOME=${q(anakotHome)}`
  ]
 ...
 * modes + `pythonPath`=agentRoot so `import anakot_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Anakot install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "ANAKOT_HOME=${String(anakotHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]
