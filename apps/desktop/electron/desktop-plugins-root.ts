// The standalone desktop-plugin root (`<ANAKOT_HOME>/desktop-plugins`) and the
...
//      is COPIED out as `<root>/<name>/` with a `.anakot-package.json` marker
...
export const PACKAGE_MARKER = '.anakot-package.json'
...
async function packageOrigin(packageDir: string): Promise<Pick<DesktopHalfMarker, 'catalogName' | 'repo' | 'sha'>> {
  try {
    const sidecar = JSON.parse(await fs.promises.readFile(path.join(packageDir, '.anakot-catalog.json'), 'utf8')) as {
      catalog_name?: string
      repo?: string
      sha?: string
    }
...
/** Every anakot home the app knows about locally: the default plus each profile. */
export async function localHomes(anakotHome: string): Promise<string[]> {
  const profiles = await listDirs(path.join(anakotHome, 'profiles'))

  return [anakotHome, ...profiles.map(name => path.join(anakotHome, 'profiles', name))]
}
...
export async function migrateProfileScopedDesktopPlugins(anakotHome: string, appRoot: string): Promise<string[]> {
  const moved: string[] = []

  for (const profile of await listDirs(path.join(anakotHome, 'profiles'))) {
    const scopedRoot = path.join(anakotHome, 'profiles', profile, DESKTOP_PLUGINS_DIR)
...
export async function reconcileUnifiedDesktopHalves(anakotHome: string, appRoot: string): Promise<string[]> {
  const touched: string[] = []
  const seen = new Set<string>()

  for (const home of await localHomes(anakotHome)) {
