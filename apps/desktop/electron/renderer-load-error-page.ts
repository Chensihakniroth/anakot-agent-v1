 ...
 *
 * 1. TORN BUNDLE after an update (#95575): `anakot update` replaces the app
...
export interface RendererLoadErrorDetails {
  ...
  /** Repair command hint, e.g. `anakot desktop --force-build`. */
  repairHint?: string
  /**
...
export function buildRendererLoadErrorPage(details: RendererLoadErrorDetails = {}): string {
  ...
  const title = 'Anakot couldn\u2019t start the desktop UI'
  ...
  const repair = details.repairHint ? `<p>Repair with: <code>anakot desktop --force-build</code></p>` : ''
...
<main>
  ...
  <code>anakot desktop --force-build</code>, then restart the app.</p>
