...
export function installCommandScreenshot({ rendererUrl }: { rendererUrl: string }): () => void {
  ...
  const publish = () => {
    for (const win of BrowserWindow.getAllWindows()) {
      if (!win.isDestroyed()) {
        win.webContents.send('anakot:screenshot:status', status())
      }
    }
  }
  ...
  const start = (requestPermission = false) => {
    ...
    monitor.start(window => {
      ...
      // Retain the last Anakot chat window when another application takes focus.
      ...
      if (requestId) {
        recipient.webContents.send('anakot:screenshot:request', requestId)
      }
    }, result => {
  ...
  const handle = (name: string, callback: (event: IpcMainInvokeEvent, value: unknown) => unknown) => {
    const channel = `anakot:screenshot:${name}`
  ...
  ipcMain.on('anakot:screenshot:subscribe', onSubscribe)
  ...
  const dispose = () => {
    ...
    ipcMain.removeListener('anakot:screenshot:subscribe', onSubscribe)
