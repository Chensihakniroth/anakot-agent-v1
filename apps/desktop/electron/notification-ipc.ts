...
export function registerNativeNotifications({ getMainWindow, focusWindow }: NotificationHost): void {
  ...
  ipcMain.handle('anakot:notify', (event, payload: AnakotNotification) => {
    ...
    notification.on('click', () => {
      ...
      if (focusSessionId) {
        window.webContents.send('anakot:focus-session', focusSessionId)
      }

      if (payload?.activate || payload?.notifyId) {
        window.webContents.send('anakot:notification-activate', {
    ...
    notification.on('action', (actionEvent, index) => {
      ...
      if (payload?.sessionId && !payload?.notifyId && !payload?.activate) {
        ...
        window.webContents.send('anakot:notification-action', { sessionId: payload.sessionId, actionId: action.id })
