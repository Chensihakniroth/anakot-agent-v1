 ...
 * runtime, remote gateways, Anakot Cloud instances, SSH hosts) that are all
 ...
 * apps/shared/src/backend-scope.ts (`@anakot/shared`) — tsconfig project
...
export async function reuseMatchingPrimarySshBackend({
  ...
  if (
    ...
    String(source.remoteAnakotPath || '').trim() !== String(activeSsh.remoteAnakotPath || '').trim() ||
...
/** Turn `ls ~/.anakot/profiles` output into roster names. Always includes
...
/**
 * Whether "Update all instances" may drive this connection. Anakot Cloud
 * instances are platform-managed — we never run `anakot update` against them.
...
export function normalizeConnectionInput(input: ConnectionInput, registry: ConnectionRegistry): RegistryConnection {
  ...
  if (kind === 'ssh') {
    const ssh = normalizeSshConfig({
      ...
      remoteAnakotPath: input.remoteAnakotPath,
  ...
  if (kind === 'remote' || kind === 'cloud') {
    ...
    // transport-/Anakot-managed names; an empty result stores nothing.
 ...
 * `remoteAnakotPath`/`remoteProfile`. Only fields the payload explicitly
...
export function migrateV1ToRegistry(v1: unknown): ConnectionRegistry {
  ...
  const addRemoteLike = (block: Record<string, any>, kind: 'cloud' | 'remote'): null | RegistryConnection => {
    ...
    const label = uniqueLabel(
      hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Anakot Cloud' : 'Remote gateway'),
...
export function reconcileAppliedGlobalConnection(
  ...
  const hostLabel = hostLabelFromBaseUrl(url) || (kind === 'cloud' ? 'Anakot Cloud' : 'Remote gateway')
