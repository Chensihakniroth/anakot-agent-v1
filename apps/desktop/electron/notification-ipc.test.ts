...
import type { AnakotNotification } from './notification-types'
...
it('returns native clicks and approval actions to the emitting window, not the primary', () => {
  ...
  const notify = host.handle.mock.calls[0][1] as (event: IpcMainInvokeEvent, payload: AnakotNotification) => boolean
  ...
  expect(source.webContents.send).toHaveBeenCalledWith('anakot:focus-session', payload.focusSessionId)
  host.shown[0].emit('action', { actionIndex: 1 }, undefined)
  expect(source.webContents.send).toHaveBeenCalledWith('anakot:notification-action', {
  ...
  expect(primary.webContents.send).not.toHaveBeenCalled()
  host.shown[0].emit('click')
  expect(primary.webContents.send).toHaveBeenCalledWith('anakot:focus-session', payload.focusSessionId)
})
