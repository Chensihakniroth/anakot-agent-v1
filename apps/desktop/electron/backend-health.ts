...
export interface AnakotReadyOptions {
   ...
   * two very different meanings of a 401 (see `waitForAnakotReady`).
   */
 ...
 *  - readiness-probe exhaustion in waitForAnakotReady().
 *
...
/**
 * True when the backend URL points at a Nous-managed Anakot Cloud instance
 ...
 * `isReauthRequired` so startAnakot latches; a bare `needsOauthLogin` (the
...
export async function waitForAnakotReady(baseUrl: string, options: AnakotReadyOptions): Promise<void> {
  ...
  throw new Error(`Anakot backend did not become ready: ${detail}`)
}
