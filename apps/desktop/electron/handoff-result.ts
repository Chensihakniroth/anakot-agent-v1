 ...
 * console. It writes ANAKOT_HOME/.anakot-update-result.json on every exit
 ...
 * ever surfaces, and the user may not reopen Anakot within 30 minutes.
...
export function handoffResultPath(anakotHome: string): string {
  return path.join(anakotHome, '.anakot-update-result.json')
}
...
export function readAndConsumeHandoffResult(
  ...
  const file = handoffResultPath(anakotHome)
  let raw: string
