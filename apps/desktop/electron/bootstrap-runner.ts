 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Anakot Agent by spawning
...
// Relative, not `@anakot/shared/ansi`: the electron bundle is built by esbuild
...
function readExistingPinnedCommit(activeRoot: string | null | undefined): string | null {
  ...
  try {
    const raw = fs.readFileSync(path.join(activeRoot, '.anakot-bootstrap-complete'), 'utf8')
...
function bootstrapCacheDir(anakotHome) {
  return path.join(anakotHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.anakot/anakot-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(anakotHome) {
  if (!anakotHome) {
    return null
  }

  const candidate = path.join(anakotHome, 'anakot-agent', 'scripts', installScriptName())
...
function cachedScriptPath(anakotHome, commit) {
  return path.join(bootstrapCacheDir(anakotHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(ref, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/Chensihakniroth/anakot-agent-v1/${ref}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(anakotHome, installRef.cacheKey)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(anakotHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, anakotHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass ANAKOT_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          ANAKOT_HOME: anakotHome || process.env.ANAKOT_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, anakotHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, anakotHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--anakot-home', anakotHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, anakotHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, anakotHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(anakotHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, anakotHome, emit })
    abortSignal?.throwIfAborted()
