...
How to build Anakot Desktop well. This is a judgment guide, not an inventory —
...
- The **backend** is authoritative for anything another Anakot surface can also
...
- A **runtime home change** (switching the underlying `ANAKOT_HOME` profile) is
...
previews) and the preview pane's `<webview>` (`persist:anakot-preview`). Neither
...
- `main.ts` installs `electron/preview-guest-preload-entry.ts` via
  `will-attach-webview`, keyed on the `persist:anakot-preview` partition only.
...
- `PreviewPane` admits `http:`/`https:` only (`src/lib/preview-external.ts`)
  and hands the URL to the existing `anakot:openExternal` IPC, which applies
...
its shape. "Plugin" means several unrelated things across Anakot — do not assume
...
`env -u NODE_ENV ANAKOT_GUEST_ONBOARDING=1 ANAKOT_HOME=<tmp>/.anakot ANAKOT_DESKTOP_USER_DATA_DIR=<tmp>/electron-user-data npm run dev`
(replace `<tmp>` with that directory). To use the portal stand-in, add
`ANAKOT_PORTAL_BASE_URL=http://127.0.0.1:8765 ANAKOT_ANON_API_SECRET=test-secret ANAKOT_SHARED_AUTH_DIR=<tmp>/.anakot/shared`
...
The free tier (a Nous identity with no account, `anakot_cli/anon_auth.py`) reaches the renderer
