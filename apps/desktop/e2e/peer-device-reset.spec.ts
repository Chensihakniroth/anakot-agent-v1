 ...
 * Requires the built desktop and a repo venv (or ANAKOT_DESKTOP_PYTHON).
 */
...
function pythonBinary(): string {
  if (process.env.ANAKOT_DESKTOP_PYTHON) {
    return process.env.ANAKOT_DESKTOP_PYTHON
  }
  ...
  if (!candidate) {
    throw new Error('Create the repo Python venv or set ANAKOT_DESKTOP_PYTHON before running this spec')
  }
...
function isolatedEnv(sandbox: Sandbox): Record<string, string> {
  ...
  for (const key of ['ANAKOT_DESKTOP_USER_DATA_DIR', 'ANAKOT_DESKTOP_IGNORE_EXISTING', 'ANAKOT_DESKTOP_ANAKOT_ROOT', 'ANAKOT_DESKTOP_APP_NAME', 'ANAKOT_DESKTOP_SKIP_QUIT_CONFIRM']) {
    env[key] = defaults[key]
  }
  ...
  return {
    ...
    ANAKOT_DESKTOP_PYTHON: pythonBinary(),
...
const peerTest = test.extend<{ gateways: { app: ElectronApplication; source: Page } }>({
  ...
  gateways: async ({}, runTest, info) => {
    ...
    try {
      ...
      child = spawn(pythonBinary(), ['-m', 'anakot_cli.main', 'serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'], {
        cwd: REPO_ROOT,
        detached: process.platform !== 'win32',
        env: { ...isolatedEnv(remote), ANAKOT_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN },
