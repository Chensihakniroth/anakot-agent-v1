 ...
 * packaged binary) with a real `anakot serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → anakot serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Anakot title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Anakot')
  })
  ...
  test('the preload bridge reaches the renderer', async () => {
    const bridge = await fixture!.page.evaluate(() => {
      const desktop = (window as unknown as { anakotDesktop?: Record<string, unknown> }).anakotDesktop
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → anakot serve is spawned → WS connects → config
