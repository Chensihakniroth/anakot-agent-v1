...
async function seedBot(anakotHome: string, mockUrl: string, name: string): Promise<void> {
...
test.beforeAll(async () => {
  ...
  await seedBot(sandbox.anakotHome, mock.url, 'alpha')
  ...
  for (const stray of ['sessions', 'logs']) {
    fs.mkdirSync(path.join(sandbox.anakotHome, 'profiles', stray, 'cron'), { recursive: true })
  }

  // A deleted profile: identity file present, tombstone in profiles/.deleted/<name>.
  const ghost = path.join(sandbox.anakotHome, 'profiles', 'ghost')
  fs.mkdirSync(ghost, { recursive: true })
  fs.writeFileSync(path.join(ghost, 'profile.yaml'), 'ui_meta:\n  anakot-bots: {}\n')
  fs.mkdirSync(path.join(sandbox.anakotHome, 'profiles', '.deleted'), { recursive: true })
  fs.writeFileSync(path.join(sandbox.anakotHome, 'profiles', '.deleted', 'ghost'), '')
