...
// Hosted Group Chat rooms live in the gateway, and the Desktop's own `anakot serve` backend runs
...
const PYTHON = process.env.ANAKOT_DESKTOP_PYTHON ?? path.join(REPO_ROOT, 'venv', 'bin', 'python')

let fixture: MockBackendFixture | null = null
let rpc: Rpc | null = null

function seedProfile(anakotHome: string, mockUrl: string, name: string): void {
  const dir = path.join(anakotHome, 'profiles', name)
...
function seedLegacyRoom(anakotHome: string): void {
  const script = `
import sys
...
`
  const out = execFileSync(PYTHON, ['-c', script, anakotHome], {
    cwd: REPO_ROOT,
...
/** Locate the Electron-spawned `anakot serve` backend for this sandbox: its port and session token. */
function findBackend(anakotHome: string): { port: number; token: string; pid: number } {
  ...
  for (const pidText of pids) {
    ...
    if (vars.get('ANAKOT_HOME') !== anakotHome) {
      continue
    }
    const token = vars.get('ANAKOT_DASHBOARD_SESSION_TOKEN') ?? ''
    // The backend inherits Electron's remote-debugging socket fd too; the serve port is the listener
    // that only the anakot process holds.
  ...
  throw new Error(`no anakot serve backend found for ${anakotHome} (pids: ${pids.join(',')})`)
}
...
test.beforeAll(async () => {
  ...
  writeMockProviderConfig(sandbox.anakotHome, mock.url)
  writeEnvFile(sandbox.anakotHome)
  seedProfile(sandbox.anakotHome, mock.url, 'sentinel')
  seedProfile(sandbox.anakotHome, mock.url, 'friday')
  seedLegacyRoom(sandbox.anakotHome)
  expect(fs.existsSync(path.join(sandbox.anakotHome, 'shared-state.db'))).toBe(false)
  ...
  const backend = findBackend(sandbox.anakotHome)
...
test('rooms from before the shared-state.db split are still reachable from the Desktop backend (#109775)', async () => {
  ...
  expect(fs.existsSync(path.join(fixture!.sandbox.anakotHome, 'shared-state.db'))).toBe(true)
  expect(fs.existsSync(path.join(fixture!.sandbox.anakotHome, 'state.db'))).toBe(true)
})
