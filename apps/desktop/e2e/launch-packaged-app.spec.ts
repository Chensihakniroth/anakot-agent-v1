...
/**
 * E2E smoke tests for the packaged Anakot desktop app.
 *
 ...
 * (credential stripping, isolated ANAKOT_HOME + userData, unique app name).
 *
...
test('window opens with the Anakot title', async () => {
...
test('HUD composer remains fully inside the transparent window', async () => {
  ...
  await fixture!.page.evaluate(() =>
    (window as typeof window & {
      anakotDesktop?: { hud?: { open: (options: { sessionId: null }) => Promise<void> } }
    }).anakotDesktop?.hud?.open({ sessionId: null })
  )
