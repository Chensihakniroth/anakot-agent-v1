...
// #100406: in a Bot Mode group room a teammate's `@anakot` handoff must give
// the primary profile (internal name `default`) its turn, exactly like
// `@anakot → @code-farmer` already does. The live roster stamps the primary
// row's handle as the bare profile id ("default"), and the mention parser let
// that stamped handle shadow the `@anakot` alias — so the room settled with
// Anakot never driven. The mock inference server scripts each member's line
...
/** The plugin's persisted room log (`anakot.plugin.anakot-bots.group-chats`). */
async function roomLog(page: Page, group: string): Promise<RoomLogEntry[]> {
  return page.evaluate(name => {
    const raw = window.localStorage.getItem('anakot.plugin.anakot-bots.group-chats')
...
test.afterEach(async ({}, info) => {
  ...
  await info.attach('room-log', {
    body: JSON.stringify(await roomLog(fixture.page, 'Anakot, Code Farmer'), null, 2),
  ...
  await info.attach('runtime-source', {
    body: JSON.stringify(
      await fixture.app.evaluate(() => ({
        cwd: process.cwd(),
        argv: process.argv,
        root: process.env.ANAKOT_DESKTOP_ANAKOT_ROOT,
        home: process.env.ANAKOT_HOME
      })),
      null,
      2
    ),
  ...
  await info.attach('desktop-log', {
    body: readFileSync(join(fixture.sandbox.anakotHome, 'logs/desktop.log')),
...
test('a teammate handing off with @anakot drives the primary profile', async () => {
  test.setTimeout(420_000)
  const page = fixture!.page
  const group = 'Anakot, Code Farmer'
  ...
  for (const title of ['Anakot', 'Code Farmer']) {
  ...
  // to @anakot; Anakot' scripted reply is "B". Neither script token carries
  // a literal `@`, so the user send itself never mentions Anakot.
  await composer.fill(
    '@code-farmer Please reply with one line only. ' +
      'E2E_SAY(code-farmer)[{at}anakot Please reply with the letter B.] E2E_SAY(anakot)[B]'
  )
  await composer.press('Enter')

  // Code Farmer's handoff line lands first (the reverse direction is not in
  // question); then the room must NOT settle without Anakot' turn.
  await expect
    .poll(
      async () =>
        (await roomLog(page, group)).some(e => e.from?.name === 'code-farmer' && /@anakot/.test(e.text || '')),
      {
        timeout: 180_000
      }
    )
  ...
  await expect
    .poll(
      ...
      {
        timeout: 180_000,
        message: 'the primary profile (default / @anakot) never took its turn after being @mentioned by a teammate'
      }
    )
    .toBe(true)
  ...
  await composer.fill(
    '@anakot Begin the reverse handoff. E2E_SAY(anakot)[{at}code-farmer Reply with D.] E2E_SAY(code-farmer)[D]'
  )
  await composer.press('Enter')
  await expect
