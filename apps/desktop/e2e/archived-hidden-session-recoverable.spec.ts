 ...
 * host load; the desktop still reads them through the real `anakot serve`.
 */
function seedArchivedRows(anakotHome: string, hiddenId: string, controlId: string): void {
  execFileSync(
    RUNTIME_PYTHON,
    [
      '-c',
    ...
    { cwd: REPO_ROOT, env: { ...process.env, ANAKOT_HOME: anakotHome }, stdio: 'pipe' },
  )
  expect(fs.existsSync(path.join(anakotHome, 'state.db'))).toBe(true)
}
...
async function setupSeededMockBackend(): Promise<MockBackendFixture & { controlId: string; hiddenId: string }> {
  ...
  seedArchivedRows(sandbox.anakotHome, hiddenId, controlId)
