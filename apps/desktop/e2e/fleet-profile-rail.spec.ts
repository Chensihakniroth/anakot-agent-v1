 ...
 * second gateway, "Homelab", is a REAL second `anakot serve` this spec spawns
 * with its own ANAKOT_HOME, profiles and session token, registered in the v2
...
function findAnakotBinary(): string {
  const venv = path.join(REPO_ROOT, '.venv', 'bin', 'anakot')
  ...
  throw new Error('anakot binary not found: create the repo venv (uv sync) or put anakot on PATH')
}
...
/**
 * Spawn a second, fully real `anakot serve` as the remote gateway. Its
 * session token is pinned through ANAKOT_DASHBOARD_SESSION_TOKEN so the
  ...
  const child: ChildProcess = spawn(
    ...
    ['serve', '--host', '127.0.0.1', '--port', String(port), '--skip-build'],
    {
      ...
      env: {
        ...process.env,
        ANAKOT_HOME: home,
        ANAKOT_DASHBOARD_SESSION_TOKEN: REMOTE_TOKEN,
      },
  ...
  while (Date.now() < deadline) {
    ...
    try {
      const response = await fetch(`${url}/api/status`, {
        headers: { 'X-Anakot-Session-Token': REMOTE_TOKEN },
      })
...
test.describe('fleet profile rail — two registered gateways', () => {
  ...
  test.beforeAll(async () => {
    ...
    seedProfiles(sandbox.anakotHome, ['research'])
