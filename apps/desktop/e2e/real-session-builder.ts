 ...
 * AIAgent loop, using the E2E mock provider configured in `anakotHome`.
 *
...
export class RealSessionBuilder {
  ...
  private constructor(anakotHome: string) {
  ...
  static async start(anakotHome: string): Promise<RealSessionBuilder> {
    const builder = new RealSessionBuilder(anakotHome)
