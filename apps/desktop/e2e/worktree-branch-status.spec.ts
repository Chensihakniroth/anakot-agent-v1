...
function createGitRepo(root: string): string {
  ...
  execFileSync('git', ['config', 'user.name', 'Anakot E2E'], { cwd: repo })
...
function configureRepoCwd(anakotHome: string, mockUrl: string, repo: string): void {
  writeMockProviderConfig(anakotHome, mockUrl)
  fs.appendFileSync(path.join(anakotHome, 'config.yaml'), `\nterminal:\n  cwd: ${repo}\n`, 'utf8')
  writeEnvFile(anakotHome)
}
...
test.beforeAll(async () => {
  ...
  configureRepoCwd(sandbox.anakotHome, mock.url, repo)
