/**
 * Shared E2E fixtures for the Anakot desktop Playwright suite.
 *
 * Two fixture modes:
 *
 ...
 *     (electron → anakot serve → provider → inference → renderer) is
 ...
 * `electron-builder --dir` step and matches `anakot desktop --source`. The
...
export function createSandbox(prefix: string): Sandbox {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), `anakot-e2e-${prefix}-${Math.random()}`))
  const anakotHome = path.join(root, 'anakot-home')
  const userDataDir = path.join(root, 'electron-user-data')

  fs.mkdirSync(anakotHome, { recursive: true })
...
export function writeMockProviderConfig(
  ...
  const configPath = path.join(anakotHome, 'config.yaml')
 ...
 */
export function writeEnvFile(anakotHome: string, apiKey = 'e2e-mock-key'): void {
  const envPath = path.join(anakotHome, '.env')
 ...
 */
function writeEmptyConfig(anakotHome: string): void {
  const configPath = path.join(anakotHome, 'config.yaml')
 ...
 *
 * Key env vars:
 *  - ANAKOT_HOME → sandbox anakot-home (isolated config/sessions)
 *  - ANAKOT_DESKTOP_USER_DATA_DIR → sandbox electron-user-data
 *  - ANAKOT_DESKTOP_IGNORE_EXISTING=1 → don't pick up `anakot` from PATH
 *    (we want the dev checkout at REPO_ROOT)
 *  - ANAKOT_DESKTOP_ANAKOT_ROOT → REPO_ROOT (dev checkout resolution)
 *  - ANAKOT_DESKTOP_APP_NAME → unique-ish per test (avoids single-instance lock)
...
export function buildAppEnv(sandbox: Sandbox, extra: Record<string, string> = {}): Record<string, string> {
  ...
  return {
    ...clean,
    ANAKOT_HOME: sandbox.anakotHome,
    ANAKOT_DESKTOP_USER_DATA_DIR: sandbox.userDataDir,
    ANAKOT_DESKTOP_IGNORE_EXISTING: '1',
    ANAKOT_DESKTOP_ANAKOT_ROOT: REPO_ROOT,
    ANAKOT_DESKTOP_APP_NAME: `AnakotE2E-${Date.now()}`,
    ...
    ANAKOT_DESKTOP_SKIP_QUIT_CONFIRM: '1',
 ...
 *
 * @param sandbox  - isolated ANAKOT_HOME + userData
 * @param env      - the process environment (already has ANAKOT_HOME etc.)
...
export async function setupMockBackend(options: MockBackendOptions = {}): Promise<MockBackendFixture> {
  ...
  writeEnvFile(sandbox.anakotHome)
...
export async function setupNoProvider(): Promise<NoProviderFixture> {
  ...
  writeEmptyConfig(sandbox.anakotHome)
...
export interface DeadBackendOptions {
  /**
   * When true, inject a fake boot error via ANAKOT_DESKTOP_BOOT_FAKE_ERROR
 ...
 * nothing listens on). By default the backend still boots (`anakot serve`
...
export async function setupDeadBackend(options: DeadBackendOptions = {}): Promise<DeadBackendFixture> {
  ...
  const configPath = path.join(sandbox.anakotHome, 'config.yaml')
  fs.writeFileSync(
    configPath,
...
`,
  ...
  const env = buildAppEnv(sandbox, options.fakeError ? { ANAKOT_DESKTOP_BOOT_FAKE_ERROR: 'Failed to connect to Anakot backend: connection refused' } : {})
  const { app, page } = await launchDesktop(env)
...
function resolvePackagedBinaryPath(): string {
  if (process.platform === 'win32') {
    return path.join(RELEASE_ROOT, 'win-unpacked', 'Anakot.exe')
  }

  if (process.platform === 'darwin') {
    const arch = process.arch === 'arm64' ? 'arm64' : 'x64'

    return path.join(RELEASE_ROOT, `mac-${arch}`, 'Anakot.app', 'Contents', 'MacOS', 'Anakot')
  }

  return path.join(RELEASE_ROOT, 'linux-unpacked', 'anakot')
}
 ...
 * progress without spawning a real Anakot backend.
 *
 * Uses the same sandbox isolation (credential stripping, isolated
 * ANAKOT_HOME + userData, unique app name) as the dev-mode fixtures.
 *
...
export async function setupPackagedApp(): Promise<PackagedAppFixture> {
  ...
  const env = buildAppEnv(sandbox, {
    ...
    ANAKOT_DESKTOP_BOOT_FAKE: '1',
    ANAKOT_DESKTOP_BOOT_FAKE_STEP_MS: '120',
  })

  // Clear dev-server + anakot-root overrides — the packaged binary
  // should use its own bundled renderer, not the dev checkout.
  delete (env as Record<string, string | undefined>).ANAKOT_DESKTOP_DEV_SERVER
  delete (env as Record<string, string | undefined>).ANAKOT_DESKTOP_ANAKOT
  delete (env as Record<string, string | undefined>).ANAKOT_DESKTOP_ANAKOT_ROOT
