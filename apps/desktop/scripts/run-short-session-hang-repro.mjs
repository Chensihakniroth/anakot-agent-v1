...
const UPSTREAM_URL = 'https://github.com/Chensihakniroth/anakot-agent-v1.git'
...
const LEGACY_STORAGE_KEY = 'anakot.desktop.inflightTurnJournal.v1'
const LEGACY_MIGRATION_KEY = 'anakot.desktop.inflightTurnJournal.v2.migrated'
...
async function verifyInteractiveSurfaces(cdp, timed, measure, label) {
  ...
  const version = await timed(`version.ipc.${label}`, () => cdp.eval('window.anakotDesktop.getVersion()'))
...
async function withTemporarySandbox(label, body) {
  const sandbox = mkdtempSync(join(tmpdir(), `anakot-short-session-${label}-`))
...
async function executeRunInSandboxAttempt(target, index, warmup, mock, output, sandbox, attempt) {
  ...
  const anakotHome = join(sandbox, 'anakot-home')
  const userData = join(sandbox, 'electron-user-data')
  const desktopLog = join(anakotHome, 'logs', 'desktop.log')
  ...
  writeSandboxConfig(anakotHome, mock.url)
  ...
  const child = spawn(
    ...
    [target.targetDesktop, `--user-data-dir=${userData}`, `--remote-debugging-port=${port}`],
    {
      ...
      env: sanitizedEnv({
        ANAKOT_DESKTOP_APP_NAME: `AnakotShortSession-${target.label}-${process.pid}-${index}-${warmup ? 'w' : 'm'}-${attempt}`,
        ANAKOT_DESKTOP_ANAKOT_ROOT: target.targetRoot,
        ANAKOT_DESKTOP_IGNORE_EXISTING: '1',
        ANAKOT_DESKTOP_USER_DATA_DIR: userData,
...
async function main() {
  ...
  const ephemeralRoot = mkdtempSync(join(tmpdir(), 'anakot-short-session-ab-'))
  const prepared = []
