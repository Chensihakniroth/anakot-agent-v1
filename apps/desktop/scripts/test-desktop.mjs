...
// no longer bundles the Anakot Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Anakot.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Anakot'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Anakot.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Anakot'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default ANAKOT_HOME for non-sandboxed runs -- matches main.ts's
// resolveAnakotHome(). On Windows it's %LOCALAPPDATA%\anakot; elsewhere
// it's ~/.anakot. The fresh-install sandbox launchFresh() sets its own
// ANAKOT_HOME and never touches this.
const DEFAULT_ANAKOT_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'anakot')
  }
  return path.join(os.homedir(), '.anakot')
})()
const VENV_ROOT = path.join(DEFAULT_ANAKOT_HOME, 'anakot-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'anakot-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.ANAKOT_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Anakot-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Anakot-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Anakot-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Anakot-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.ANAKOT_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.ANAKOT_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const anakotHome = path.join(sandbox, 'anakot-home')
  ...
  env.ANAKOT_DESKTOP_CWD = cwd
  env.ANAKOT_DESKTOP_IGNORE_EXISTING = '1'
  env.ANAKOT_DESKTOP_TEST_MODE = 'fresh-install'
  env.ANAKOT_DESKTOP_USER_DATA_DIR = userDataDir
  env.ANAKOT_HOME = anakotHome
  delete env.ANAKOT_DESKTOP_ANAKOT
  delete env.ANAKOT_DESKTOP_ANAKOT_ROOT
  ...
  console.log(`  ANAKOT_HOME: ${anakotHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(anakotHome, 'anakot-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Anakot Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of anakot_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'anakot-agent', 'anakot_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Anakot
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + ANAKOT_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  ANAKOT_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
