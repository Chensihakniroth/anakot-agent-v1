 ...
 * product name (`Anakot`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Anakot'
 *
 * This is a hard failure with no obvious cause for the user — `anakot desktop`
 ...
 * (the mac-unpacked Anakot.app bundle) and Windows (win-unpacked), so we
 ...
 * Anakot.exe that Windows can't load (truncated PE from a corrupt cached
 * Electron zip, wrong arch), the updater's integrity gate in
 * `anakot desktop --build-only` (anakot_cli/main.py
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Anakot.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Anakot'}.exe`
