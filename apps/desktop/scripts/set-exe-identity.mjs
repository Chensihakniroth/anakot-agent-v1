#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Anakot icon + version metadata onto the
// built Anakot.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Anakot.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `anakot desktop`, the installer's
...
// path (which rebuilds via `anakot desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterExtract closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.mjs <path-to-Anakot.exe>
//
...
// Stamp the Anakot icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(
  ...
  console.log('[set-exe-identity] done — Anakot icon + identity stamped')
}
