...
//   D=$(mktemp -d) && env -u ANAKOT_SHARED_AUTH_DIR ANAKOT_GUEST_ONBOARDING=1 ANAKOT_SKIP_INTRO=1 \
//     ANAKOT_HOME="$D/home" ANAKOT_DESKTOP_USER_DATA_DIR="$D/userdata" anakot desktop
...
const sandbox = process.argv[2] ? path.resolve(process.argv[2]) : fs.mkdtempSync(path.join(os.tmpdir(), 'anakot-connectors-'))
const port = 5194
const url = `http://127.0.0.1:${port}`
const python = process.env.ANAKOT_DESKTOP_PYTHON
if (!python || !fs.existsSync(python)) throw new Error('Set ANAKOT_DESKTOP_PYTHON to your installed Anakot Python executable.')
...
for (const dir of ['.anakot/shared', 'electron', 'work']) fs.mkdirSync(path.join(sandbox, dir), { recursive: true })
const env = Object.fromEntries(['PATH', 'TMPDIR', 'LANG', 'SHELL'].filter(key => process.env[key]).map(key => [key, process.env[key]]))
Object.assign(env, {
  HOME: sandbox, ANAKOT_HOME: path.join(sandbox, '.anakot'), ANAKOT_SHARED_AUTH_DIR: path.join(sandbox, '.anakot/shared'),
  ANAKOT_DESKTOP_USER_DATA_DIR: path.join(sandbox, 'electron'), ANAKOT_DESKTOP_CWD: path.join(sandbox, 'work'),
  ANAKOT_DESKTOP_ANAKOT_ROOT: root, ANAKOT_DESKTOP_PYTHON: python,
  ANAKOT_DESKTOP_IGNORE_EXISTING: '1', ANAKOT_DESKTOP_APP_NAME: 'Anakot Connector Rehearsal',
  ANAKOT_GUEST_ONBOARDING: '1', ANAKOT_SKIP_INTRO: '1', ANAKOT_DESKTOP_DEV_SERVER: url, ANAKOT_DESKTOP_CDP_PORT: '9344'
})
