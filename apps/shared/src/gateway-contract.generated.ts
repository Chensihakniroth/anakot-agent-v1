...
/** One persisted reaction row (``anakot_state_messages.set_message_reaction``); ``seen`` is stamped once announced. */
...
/** One ``anakot_cli/inventory.py::build_models_payload`` provider row (the union of every field the builder sets; ``pricing_pending`` / ``free_tier_pending`` mark the cached-only path). */
...
/** ``anakot_cli/inventory.py::_apply_pricing`` — formatted $/Mtok strings (``""`` unknown, ``"free"``); the sale fields are Nous Portal-only. */
...
/** ``anakot_cli/foreign_sessions_browser.py::list_foreign_sessions`` — ``id`` is an opaque handle, never a path. */
...
/** ``tui_gateway/change_watcher.py::resolve_skin`` — the resolved active skin (``AnakotSkin``). ``{}`` when the skin engine failed to load. Colour maps are token → colour string. */
...
export interface RpcMethods {
  ...
  /** Run ``anakot <argv>`` non-interactively and capture its output; ``blocked`` explains a refusal. */
  ...
  /** Re-read ~/.anakot/.env (CLI /reload parity); built agents keep their pool until /new. */
  ...
  /** Export the transcript to ~/.anakot/sessions/saved (classic /save). */
