...
_ANAKOT_WEBHOOK_SAFE_TOOLS = ["web_search", "web_extract", "vision_analyze", "clarify"]
...
def _bundle(description, extras=()):
    """A `anakot-*` platform bundle: the shared core tools plus optional platform extras."""
    return _ts(description, _ANAKOT_CORE_TOOLS + list(extras))


def _core_without(*excluded, kanban=True):
    """_ANAKOT_CORE_TOOLS minus *excluded* (and, unless kanban=True, every kanban_* tool); order preserved."""
    return [t for t in _ANAKOT_CORE_TOOLS if t not in excluded and (kanban or not t.startswith("kanban_"))]
...
TOOLSETS = {
    ...
    "x_search": _ts(
        ...
        "(SuperGrok OAuth or XAI_API_KEY). Off by default; enable in `anakot tools` → "
        "X (Twitter) Search.",
        ["x_search"],
    ),
    ...
    "video_gen": _ts(
        ...
        "separate tools. Configure via ``anakot tools`` → Video Generation.",
    ...
    "browser": _ts(
        ...
        "iframes, hold-click)",
        [t for t in _ANAKOT_CORE_TOOLS if t.startswith("browser_")],
    ),
    "cronjob": _ts(
    ...
    "kanban": _ts(
        ...
        "the kanban dispatcher (ANAKOT_KANBAN_TASK env set). The dispatcher runs "
        ...
        [t for t in _ANAKOT_CORE_TOOLS if t.startswith("kanban_")],
    ),
    ...
    # config (see the non-configurable-toolset recovery loop in anakot_cli/tools_config.py).
    "coding": _ts(
    ...
    # Full Anakot toolsets (CLI + messaging platforms). All share the core tools;
    # there is deliberately no agent-callable send_message tool. anakot-acp is the
    ...
    "anakot-cli": _bundle("Full interactive CLI toolset - all default tools plus cronjob management"),

    # Mirrors anakot-cli; `anakot tools` platform config filters it down and
    # _get_platform_tools() drops _DEFAULT_OFF_TOOLSETS unless user-enabled.
    "anakot-cron": _bundle("Default cron toolset - same core tools as anakot-cli; gated by `anakot tools`"),
    "anakot-telegram": _bundle("Telegram bot toolset - full access for personal use (terminal has safety checks)"),
    "anakot-discord": _bundle(
        "Discord bot toolset - full access (terminal has safety checks via dangerous "
        "command approval)",
        ["discord", "discord_admin"],
    ),
    "anakot-whatsapp": _bundle("WhatsApp bot toolset - similar to Telegram (personal messaging, more trusted)"),
    "anakot-slack": _bundle("Slack bot toolset - full access for workspace use (terminal has safety checks)"),
    "anakot-signal": _bundle("Signal bot toolset - encrypted messaging platform (full access)"),
    "anakot-bluebubbles": _bundle("BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server"),
    "anakot-homeassistant": _bundle("Home Assistant bot toolset - smart home event monitoring and control"),
    "anakot-email": _bundle("Email bot toolset - interact with Anakot via email (IMAP/SMTP)"),
    "anakot-mattermost": _bundle("Mattermost bot toolset - self-hosted team messaging (full access)"),
    "anakot-matrix": _bundle("Matrix bot toolset - decentralized encrypted messaging (full access)"),
    "anakot-dingtalk": _bundle("DingTalk bot toolset - enterprise messaging platform (full access)"),
    "anakot-feishu": _bundle("Feishu/Lark bot toolset - enterprise messaging via Feishu/Lark (full access)", _FEISHU_TOOLS),
    "anakot-weixin": _bundle("Weixin bot toolset - personal WeChat messaging via iLink (full access)"),
    "anakot-qqbot": _bundle("QQBot toolset - QQ messaging via Official Bot API v2 (full access)"),
    "anakot-wecom": _bundle("WeCom bot toolset - enterprise WeChat messaging (full access)"),
    "anakot-wecom-callback": _bundle("WeCom callback toolset - enterprise self-built app messaging (full access)"),
    "anakot-yuanbao": {
        "description": "Yuanbao Bot 元宝消息平台工具集 - 群信息、成员查询、私聊、贴纸表情",
        "tools": _ANAKOT_CORE_TOOLS + _YUANBAO_TOOLS,
        "module": "tools.yuanbao_tools",
        "includes": [],
    },
    "anakot-sms": _bundle("SMS bot toolset - interact with Anakot via SMS (Twilio)"),
    "anakot-webhook": _ts("Webhook toolset - receive and process external webhook events", _ANAKOT_WEBHOOK_SAFE_TOOLS),
    "anakot-gateway": _ts(
        "Gateway toolset - union of all messaging platform tools",
        [],
        includes=[
            "anakot-telegram", "anakot-discord", "anakot-whatsapp", "anakot-slack",
            "anakot-signal", "anakot-bluebubbles", "anakot-homeassistant", "anakot-email",
            "anakot-sms", "anakot-mattermost", "anakot-matrix", "anakot-dingtalk",
            "anakot-feishu", "anakot-wecom", "anakot-wecom-callback", "anakot-weixin",
            "anakot-qqbot", "anakot-webhook", "anakot-yuanbao",
        ],
    ),
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """A bundle's tools minus _ANAKOT_CORE_TOOLS (one level of includes).

    Disabling a `core + extras` bundle must not strip the core tools every other
    toolset shares. One `includes` pass suffices (only anakot-gateway nests
    bundles). Unknown names: full resolution minus core.
    """
    core = set(_ANAKOT_CORE_TOOLS)
...
def _plugin_platform_bundle(name: str) -> List[str]:
    """Implicit `anakot-<platform>` bundle for a registered plugin platform: core
    tools plus whatever the plugin registered under the platform name. [] otherwise."""
    if not name.startswith("anakot-"):
        return []
    platform_name = name[len("anakot-"):]
    try:
