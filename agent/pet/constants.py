...
inferred from the atlas shape so Anakot renders both legacy 8-row sheets and
...
class PetState(str, Enum):
    """Animation state a pet can be shown in (Anakot names; Codex rows say ``jumping``/``running`` for ``jump``/``run``)."""
...
# Legacy Anakot/petdex row order (top -> bottom) for the older 8-row, 9-column atlas.
...
# Canonical Anakot names -> accepted row-name aliases in descending preference.
