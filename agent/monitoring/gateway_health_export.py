...
_OBSERVABLE_METRIC_NAMES = (
    "anakot.gateway.up", "anakot.gateway.state", "anakot.gateway.active_agents", "anakot.gateway.busy",
    "anakot.gateway.drainable", "anakot.gateway.restart_requested", "anakot.gateway.background_work",
    "anakot.gateway.background_delegations", "anakot.platform.up", "anakot.platform.degraded",
    "anakot.cron.scheduler.heartbeat_age_seconds", "anakot.cron.scheduler.last_success_age_seconds",
    "anakot.cron.scheduler.catch_up_occurrences", "anakot.cron.jobs.enabled", "anakot.cron.jobs.running",
    "anakot.cron.jobs.overdue",
)
...
class GatewayHealthExportRuntime:
    ...
    def shutdown(self) -> None:
        ...
        if closeables:
            worker = threading.Thread(target=_close, name="anakot-gateway-health-export-shutdown", daemon=True)
            worker.start()
...
def _read_runtime_snapshot(config: Dict[str, Any]):
    ...
    try:
        ...
        for name, read in (
            ("anakot.gateway.background_work", _read_background_work_count),
            ("anakot.gateway.background_delegations", _read_background_delegations_count),
        ):
...
def _start_metric_provider(config: Dict[str, Any], sdk: Dict[str, Any]) -> Any:
    ...
    meter = provider.get_meter("anakot.gateway.health")
...
class GatewayDiagnosticLogStreamer(EmitterStreamer):
    ...
    def __init__(self, config: Dict[str, Any], sdk: Dict[str, Any]):
        ...
        self._logger = self._provider.get_logger("anakot.gateway.diagnostics")
        self._sdk = sdk
        self.exported = 0
...
def start_gateway_health_export(config: Dict[str, Any]) -> GatewayHealthExportRuntime:
    ...
    if metrics_on or diagnostics_on:
        try:
            ...
            logger.warning("monitoring.gateway_health_export.enabled but OTLP SDK is unavailable; install 'anakot-agent[otlp]'", exc_info=True)
    ...
    if diagnostics_on:
        try:
            ...
            thread = threading.Thread(target=_run, name="anakot-gateway-health-export", daemon=True)
            thread.start()
