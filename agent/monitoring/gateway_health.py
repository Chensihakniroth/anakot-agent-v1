...
def build_gateway_health_snapshot(
    ...
    base = {
        ...
        "anakot.supervision_mode": mode if mode in _SUPERVISION_MODES else "unknown",
    }
    ...
    metrics: list[GatewayMetric] = [
        metric("anakot.gateway.up", int(bool(gateway_running))),
        metric("anakot.gateway.active_agents", active_agents),
        metric("anakot.gateway.busy", int(bool(busy))),
        metric("anakot.gateway.drainable", int(bool(drainable))),
        metric("anakot.gateway.restart_requested", int(bool(runtime.get("restart_requested")))),
        metric("anakot.gateway.state", 1, **{"anakot.gateway.state": gateway_state}),
    ]
    fatal_count = 0
    events: list[GatewayHealthEvent | GatewayDiagnosticEvent] = []
    ...
    for platform, pdata in platforms.items():
        ...
        pattrs = {"anakot.platform": str(platform), "anakot.platform.state": state}
        metrics.append(metric("anakot.platform.up", int(state in _RUNNING_PLATFORM_STATES), **pattrs))
        metrics.append(metric("anakot.platform.degraded", int(is_degraded), **pattrs, **{"anakot.error_code": error_code}))
        if is_degraded:
