...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("anakot-verify-", "anakot-ad-hoc-")
...
def _db_path() -> Path:
    return get_anakot_home() / "verification_evidence.db"
...
def record_verify_run(
    *, root: str | Path, session_id: str | None = None, ok: bool, command: str = "anakot verify",
    scope: str = "full", output: str = "",
) -> Optional[dict[str, Any]]:
    """Record a completed ``anakot verify`` run as verification evidence.
    ...
    return _insert_evidence(VerificationEvidence(
        command=command, canonical_command="anakot verify", kind="verify",
