...
from anakot_cli.timeouts import get_provider_request_timeout
...
def _warn_memory_provider_unavailable(name: str, reason: str = "") -> None:
    ...
    logger.warning(
        ...
        "provider's credentials/config with 'anakot memory status'. Note: "
        "systemd/gateway services do not inherit ~/.anakot/.env automatically; set "
...
def _provider_default_routes(provider: str) -> set[str]:
    ...
    with suppress(Exception):
        from anakot_cli.providers import ANAKOT_OVERLAYS, get_provider
        overlay = ANAKOT_OVERLAYS.get(provider)
    ...
    with suppress(Exception):
        ...
        from anakot_cli.models import normalize_provider as normalize_model_provider
        from anakot_cli.providers import normalize_provider as normalize_registry_provider
...
def _build_codex_gpt5_autoraise_notice(
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: anakot config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    """Per-profile marker path (``$ANAKOT_HOME`` is profile-scoped; not a config key)."""
    return get_anakot_home() / ".codex_gpt55_autoraise_notice"
...
def _resolve_api_mode(agent, api_mode, provider_name, base_url):
    ...
    if is_actual_route(agent.provider, base_url):
        ...
        try:
            ...
            from anakot_cli.providers import host_mandated_api_mode as _host_mandated_api_mode
...
def _finalize_routing(agent, api_mode, credential_pool):
    ...
    with suppress(Exception):
        from anakot_cli.model_normalize import (
    ...
    from anakot_cli.anon_auth import pin_model_for_route
...
_TURN_STATE: Dict[str, Any] = {
    ...
    # Credits tracking (dev-only, ANAKOT_DEV_CREDITS) from x-nous-credits-* headers; session
...
def _init_prompt_cache_config(agent):
    ...
    agent._cache_ttl = "5m"
    with suppress(Exception):
        from anakot_cli.config import load_config_readonly as _load_pc_cfg
...
def _setup_logging(agent):
    ...
    from anakot_logging import setup_logging, setup_verbose_logging
    setup_logging(anakot_home=_ra()._anakot_home)
    ...
    # would starve the root file handlers. Noise reduction belongs in anakot_logging.
...
def _init_anthropic_client(agent, api_key, base_url, _provider_timeout):
    ...
    if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
        try:
            from anakot_cli.auth import build_minimax_oauth_token_provider
...
def _routed_client_kwargs(agent, fallback_model, _provider_timeout) -> Optional[Dict[str, Any]]:
    ...
    if _routed_client is not None:
        from anakot_cli.providers import is_actual_route, normalize_provider
        effective_provider = getattr(_routed_client, "_anakot_aux_effective_provider", "")
    ...
    raise RuntimeError(
        f"No LLM provider configured. Run `anakot {_sel}model` to "
        f"select a provider, or run `anakot {_sel}setup` for first-time "
        "configuration."
    )
...
_HOST_DEFAULT_HEADERS: List[tuple[str, Callable[[Any, str], Dict[str, str]]]] = [
    ...
    ("githubcopilot.com", _lazy_headers("anakot_cli.models", "copilot_default_headers")),
    ...
    ("x.ai", _lazy_headers("tools.xai_http", "anakot_xai_default_headers")),
]
...
def _load_tools(agent, enabled_toolsets, disabled_toolsets):
    # A multiplexed gateway may have switched ANAKOT_HOME since model_tools was imported;
...
def _publish_session_id(session_id: str) -> None:
    ...
    try:
        ...
        if not delegated_child:
            os.environ["ANAKOT_SESSION_ID"] = session_id
...
def _init_session_state(agent, session_id, session_db, parent_session_id, reasoning_config, max_tokens,
    ...
    # ~/.anakot/sessions/ — kept unconditionally for request_dump_*.json debug breadcrumbs.
    agent.logs_dir = get_anakot_home() / "sessions"
    ...
    # Process-scoped --yolo is persisted so `anakot --resume` restores the bypass
        ...
        try:
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if _mp is None:
                    ...
                    from anakot_cli.memory_provider_migration import recover_at_startup
...
def _compression_codex_settings(cfg: Dict[str, Any]) -> tuple[str, bool, Optional[int]]:
    ...
    if app_server_auto not in {"native", "anakot", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, anakot, off.",
            app_server_auto,
        )
...
def _configured_default_base_url(_agent_cfg, _model_cfg, _custom_providers) -> str:
    ...
    if _norm in _RUNTIME_FIRST_PROVIDER_IDS:
        ...
        with suppress(Exception):
            from anakot_cli.auth import resolve_provider as resolve_auth_provider
...
def _select_context_engine(_agent_cfg):
    ...
    if _selected_engine is None:
        try:
            from anakot_cli.plugins import get_plugin_context_engine
...
def _enforce_minimum_context(agent):
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH and not _allow_lmstudio_explicit_below_floor:
        ...
        raise ValueError(
            ...
            f"by Anakot Agent.  {remedy}"
        )


def _warn_nonagentic_anakot_model(agent):
    # Nous Anakot 3/4 are chat models, not tool-call-tuned. cli.py show_banner() already
    ...
    if agent.quiet_mode or (agent.platform or "cli") == "cli":
        return
    with suppress(Exception):
        from anakot_cli.model_switch import _check_anakot_model_warning
        _anakot_warn = _check_anakot_model_warning(agent.model or "")
        if _anakot_warn:
            _user_msg = (
                "⚠ Nous Research Anakot 3 & 4 models are NOT agentic — they "
            ...
            _ra().logger.warning(_anakot_warn)
    ...
    if agent.context_compressor:
        try:
            agent.context_compressor.on_session_start(
                agent.session_id, anakot_home=str(get_anakot_home()),
...
def init_agent(
    ...
    Non-obvious parameters:
      ...
      skip_context_files: skip SOUL.md/.anakot.md/AGENTS.md/CLAUDE.md/.cursorrules injection;
        load_soul_identity keeps ~/.anakot/SOUL.md as identity regardless.
    """
    _install_safe_stdio()
    ...
    try:
        from anakot_cli.config import load_config_readonly as _load_agent_config
    ...
    _warn_nonagentic_anakot_model(agent)
