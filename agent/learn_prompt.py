...
``skill_manage`` per the Anakot authoring standards; large prose sources get the knowledge-base layout (lean SKILL.md
...
Follow the Anakot skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Anakot`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Anakot.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.anakot.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Anakot tools.
...
Anakot-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Anakot tools by name in backticks: `terminal`, `read_file`,
