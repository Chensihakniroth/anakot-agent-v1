...
Installs go to a Anakot-owned staging dir, ``<ANAKOT_HOME>/lsp/bin/``, so the
...
missing binary skips the server and ``anakot lsp status`` reports it).
...
# ``bin`` in ``<ANAKOT_HOME>/lsp/bin/`` first, then on PATH.  ``extra_pkgs``
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # the host so `anakot lsp status` reports its presence.
...
def anakot_lsp_bin_dir() -> Path:
    """Return the Anakot-owned bin staging dir for LSP servers."""
    from anakot_constants import get_anakot_home

    p = get_anakot_home() / "lsp" / "bin"
...
def _npm_bin_dir() -> Path:
    ...
    return anakot_lsp_bin_dir().parent / "node_modules" / ".bin"
...
def _existing_binary(name: str, *, is_windows: Optional[bool] = None) -> Optional[str]:
    ...
    bases = [anakot_lsp_bin_dir() / name] + ([_npm_bin_dir() / name] if win else [])
...
def _link_into_bin(target: Path) -> str:
    ...
    link = anakot_lsp_bin_dir() / target.name
    if not link.exists():
        try:
...
def _install_npm(pkg: str, bin_name: str, extra_pkgs: Optional[list] = None) -> Optional[str]:
    ...
    if pm is None:
        return None
    # Managed Node first: $ANAKOT_HOME/node isn't on an arbitrary process's
    # PATH, so a bare which() would miss the Node that Anakot installed.
    ...
    if pm_bin is None:
        ...
        return None
    staging = anakot_lsp_bin_dir().parent  # <ANAKOT_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    ...
    pip_target = anakot_lsp_bin_dir().parent / "python-packages"
...
def detect_status(pkg: str) -> str:
    """Return ``installed``, ``missing``, or ``manual-only`` (for ``anakot lsp status``; spawns nothing)."""
    recipe = INSTALL_RECIPES.get(pkg)
...
__all__ = ["INSTALL_RECIPES", "try_install", "detect_status", "anakot_lsp_bin_dir"]
