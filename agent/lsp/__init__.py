"""Language Server Protocol (LSP) integration for Anakot Agent.
...
# Routed multiplex profiles (ANAKOT_HOME override) each get their own service: ``lsp.*`` config
...
def get_service() -> Optional[LSPService]:
    ...
    without it every ``anakot chat`` exit leaks pyright processes for a few seconds
