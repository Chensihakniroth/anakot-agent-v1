...
YAML files in ``<ANAKOT_HOME>/skill-bundles/`` (``name``, ``description``,
...
def _bundles_dir() -> Path:
    """Bundles directory: ``ANAKOT_BUNDLES_DIR`` override (tests) or ``<ANAKOT_HOME>/skill-bundles``."""
    override = os.environ.get("ANAKOT_BUNDLES_DIR")
    return Path(override).expanduser() if override else get_anakot_home() / "skill-bundles"
...
# File-level CRUD — used by `anakot bundles`.
