...
a cache problem must never block Anakot startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """``home_path`` as resolved by ``load_anakot_dotenv()``, else ``$ANAKOT_HOME``/``~/.anakot``."""
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend at ``<anakot_home>/cache/<basename>``::
