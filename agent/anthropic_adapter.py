"""Anthropic Messages API adapter: client construction + the Messages call for Anakot's
...
# Anakot effort -> Anthropic adaptive-thinking effort (output_config.effort). 4.7+ exposes
...
# Anthropic's OAuth billing classifier fingerprints certain Anakot tool schemas/prose as a
...
def _base_client_kwargs(base_url, timeout) -> tuple[str, Dict[str, Any]]:
    ...
    anakot's outer loop (``max_retries=0``): the SDK default of 2 uses its own backoff that ignores
...
def _new_sdk_client(sdk, kwargs: Dict[str, Any], headers: Dict[str, str], route: str = None):
    ...
    ANTHROPIC_AUTH_TOKEN in the environment (both loaded from ~/.anakot/.env) and then sends dual
...
def _custom_provider_extra_headers(base_url) -> Dict[str, str]:
    ...
    if not base_url:
        return {}
    try:
        from anakot_cli.config import get_custom_provider_extra_headers
...
def build_anthropic_bedrock_client(region: str):
    ...
    return sdk.AnthropicBedrock(
        ...
        max_retries=0,  # retry belongs to anakot's outer loop (honors Retry-After)
...
def _normalize_to_mcp_wire(name: str) -> str:
    ...
    Anakot tools (``read_file``) and native MCP tools registered as ``mcp_<server>_<tool>`` must
...
_OAUTH_SLUG_PATTERN = re.compile(r"""(?<![\w./:@'"`-])anakot-agent(?![\w/@-]|\.\w)""")
...
def _thinking_kwargs(reasoning_config: Dict[str, Any], model: str, effective_max_tokens: int) -> Dict[str, Any]:
    ...
    4.7+ ``thinking.display`` defaults to "omitted", hiding the reasoning Anakot shows in its CLI,
...
def build_anthropic_kwargs(
    ...
    # which silently hides reasoning text that Anakot surfaces in its CLI. We request "summarized" so the
...
_PLUGIN_COMPAT_LAZY = {
    ...
    'get_anakot_home': ('anakot_constants', 'get_anakot_home'),
    ...
    'read_anakot_oauth_credentials': ('agent.anthropic_credentials', 'read_anakot_oauth_credentials'),
    ...
    'run_anakot_oauth_login_pure': ('agent.anthropic_credentials', 'run_anakot_oauth_login_pure'),
