"""``$ANAKOT_HOME/cache/<kind>/`` materialisation helpers for the image/video
...
def cache_dir(kind: str) -> Path:
    """Return ``$ANAKOT_HOME/cache/<kind>/``, creating parents as needed."""
    from anakot_constants import get_anakot_home
    path = get_anakot_home() / "cache" / kind
