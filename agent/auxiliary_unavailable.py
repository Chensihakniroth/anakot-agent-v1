...
def _quarantined_nous_error(exc: BaseException) -> BaseException:
    ...
    """
    from anakot_cli.auth import AuthError, get_provider_auth_state
    from anakot_cli.auth_nous import _terminal_quarantine_marker
...
def pool_cooldown_message(provider_id: str) -> Optional[str]:
    ...
    were found. Run anakot auth add …" — wrong on both counts for a valid OAuth grant that is
    ...
    return (f"Provider '{provider_id}' is set in config.yaml but {which} cooling down after a "
            ...
            f"add another credential with `anakot auth add {provider_id}`, or switch to a "
            "different provider with `anakot model`.")
...
def missing_provider_credentials_message(provider_id: str) -> str:
    ...
    if env_vars:
        ...
        problem, remedy = "no credentials were found", f"Run `anakot auth add {provider_id}` to sign in"
    else:
        problem = "no credentials were found"
    switch = "switch to a different provider with `anakot model`."
...
def record_nous_credential_failure(exc: BaseException) -> str:
    ...
    Logged once per distinct message: WARNING when a real credential failed, DEBUG when Anakot was
