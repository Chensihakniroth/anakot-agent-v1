"""OpenAI-shape bridge shared by Anakot' ACP clients.

ACP has no OpenAI-style ``tools``/``tool_calls`` channel, so Anakot' tool schemas travel INTO the
...
read/edit/execute tools forwards only Anakot' agent-level tools, since re-offering overlapping ones
makes Anakot redo finished work.
"""
...
class StreamChunks(list):
    """Chunk list that also carries response-level attributes (e.g. ``anakot_projected_messages``)
    Anakot reads off the ``create`` result; a plain list would drop them on the stream path."""
