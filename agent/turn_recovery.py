...
def _print_nous_401_diagnostics(agent: Any, api_error: Exception) -> None:
    ...
    try:
        from anakot_cli.anon_auth import is_anonymous_agent
        if is_anonymous_agent(agent):
            ...
            _plines(agent, "   Your session ended and Anakot couldn't start a new one.",
    ...
    _plines(
        agent,
        "   Troubleshooting:",
        "     • Re-authenticate: anakot auth add nous",
        "     • Check credits / billing: https://portal.nousresearch.com",
        f"     • Verify stored credentials: {display_anakot_home()}/auth.json",
...
def _print_anthropic_401_diagnostics(agent: Any, key: Any) -> None:
    ...
    if is_token_provider(key):
        ...
        _plines(
            ...
            "   Run `anakot doctor` for credential-chain diagnostics, or",
    ...
    _dhh = display_anakot_home()
    _plines(
        agent,
        "   Troubleshooting:",
        f"     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Anakot-managed OAuth/setup tokens",
        ...
        "     • Anakot login (OAuth): run 'anakot auth add anthropic' to sign in again, then retry",
        "     • Inspect what Anakot holds: anakot auth list anthropic",
        "     • Legacy cleanup: anakot config set ANTHROPIC_TOKEN \"\"",
        "     • Clear stale keys: anakot config set ANTHROPIC_API_KEY \"\"",
    )
...
def recover_after_classification(
    ...
    if (
        ...
        try:
            from anakot_cli.models_reasoning_caps import refresh_reasoning_caps_async
...
def _print_nonretryable_auth_guidance(
    ...
    if provider in {"openai-codex", "xai-oauth", "nous"} and status_code == 401:
        if provider == "openai-codex":
            ...
            _vlines(
                ...
                "      refreshed by another client (Codex CLI, VS Code) or another Anakot profile.",
            ...
            _vlines(
                ...
                "      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `anakot model`.",
            )
        else:  # nous
            _vlines(
                agent,
                ...
                "      1. Re-authenticate: anakot portal",
    ...
    _vlines(
        ...
        "      • Is the key valid? Run: anakot setup",
...
def _welcome_tier_guidance(classified: Any, *, model: Any, in_chat: bool, door: bool = True) -> str:
    ...
    if not refusal and not route:
        return ""
    from anakot_cli.anon_auth import welcome_refusal_copy, welcome_route_refusal_copy
    if refusal:
...
def _welcome_outage_copy(base_url: Any, classified: Any, *, anonymous: bool = False) -> str:
    ...
    try:
        from anakot_cli.anon_auth import FREE_TIER_OUTAGE_COPY, route_is_welcome_host
...
def _missing_vendor_prefix_suggestion(api_error: Exception, provider: Any, model: Any) -> Optional[str]:
    ...
    if getattr(api_error, "status_code", None) != 404:
        return None
    try:
        from anakot_cli.model_normalize import suggest_prefixed_model_id
...
def nonretryable_client_error_result(
    ...
    if _welcome_hint:
        ...
        _vlines(agent, f"   💡 Fix: pick another model (/model), or check `{display_anakot_home()}/logs/agent.log`.")
    ...
    if classified.reason == FailoverReason.upstream_blocked:
        _vlines(
            ...
            "      set `extra_headers: {User-Agent: AnakotAgent/1.0}` on the custom_providers entry,",
    ...
    if classified.reason == FailoverReason.content_policy_blocked:
        _vlines(
            ...
            "      To route future blocks to another provider automatically: anakot fallback add",
        )
    ...
    if classified.reason == FailoverReason.ssl_cert_verification:
        _vlines(
            agent,
            "   💡 Anakot couldn't verify the provider's security certificate. This fails the same",
            ...
            "      • Missing/stale system CA store? Refresh it (in Anakot's venv: `uv pip install",
