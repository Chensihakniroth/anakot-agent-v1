"""Shared SKILL.md preprocessing helpers: ``${ANAKOT_*}`` template tokens and
...
from anakot_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger(__name__)

# ${ANAKOT_SKILL_DIR} / ${ANAKOT_SESSION_ID} tokens. Unresolvable ones (e.g. no
# session) are left as-is so the author can spot them.
_SKILL_TEMPLATE_RE = re.compile(r"\$\{(ANAKOT_SKILL_DIR|ANAKOT_SESSION_ID)\}")
...
def substitute_template_vars(content: str, skill_dir: Path | None, session_id: str | None) -> str:
    """Replace ${ANAKOT_SKILL_DIR} / ${ANAKOT_SESSION_ID}; tokens without a value stay in place."""
    if not content:
        return content
    values = {
        "ANAKOT_SKILL_DIR": str(skill_dir) if skill_dir else None,
        "ANAKOT_SESSION_ID": str(session_id) if session_id else None,
    }
