...
_XAI_CLIENT_WEB_SEARCH_ALIAS = "anakot_web_search"
...
# anakot_<name> and map them back before local dispatch.
...
# (anakot_<name>), map back in normalize_response so Anakot dispatch is unaffected. See #85589.
...
_RESERVED_TOOL_ALIAS_PREFIX = "anakot_"
...
def _alias_wire_tools(response_tools: Any, params: dict[str, Any], is_xai_responses: bool) -> tuple[Any, dict[str, str]]:
    ...
    swaps it 1:1 for the built-in, client mode keeps Anakot dispatch under an alias.
    """
    ...
    # Keep Anakot's client-side functions available under wire aliases.
    ...
    # xAI server-side web search vs Anakot web providers. grok models on xAI's /v1/responses surface have a
    ...
    # or resolved): keep Anakot dispatch so ``web.backend`` / ``web.search_backend`` is honored, but rename
    # the wire tool to ``anakot_web_search`` so Grok cannot hijack the name. The alias is mapped back to
...
class ResponsesApiTransport(ProviderTransport):
    ...
    def build_kwargs(
        ...
        kwargs = {
            # ``-900k`` picker variants are Anakot-side aliases; the backend knows only the base slug.
            "model": wire_model,
