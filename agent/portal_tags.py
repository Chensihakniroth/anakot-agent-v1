...
Every Anakot request to the Nous Portal (main loop, auxiliary client, fallback
paths) must carry the same product-attribution tags, sent in OpenAI-compatible
``extra_body['tags']``: ``["product=anakot-agent", "client=anakot-client-v<__version__>"]``.
The version is read live from ``anakot_cli.__version__`` — do NOT pre-compute it
...
_affinity_scope: ContextVar[Optional[str]] = ContextVar("anakot_affinity_scope", default=None)
...
def anakot_client_tag() -> str:
    """``client=anakot-client-v<MAJOR>.<MINOR>.<PATCH>`` ("unknown" if anakot_cli is unimportable)."""
    try:
        from anakot_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"client=anakot-client-v{__version__}"
...
def nous_portal_tags(session_id: str | None = None) -> List[str]:
    ...
    """
    tags = ["product=anakot-agent", anakot_client_tag()]
