"""Unified removal contract for every credential source Anakot reads from.

Readers live in ``agent.credential_pool``; what is unified here is **removal**:
``anakot auth remove <provider> <N>`` must make the entry stay gone across
...
EXTERNAL_LOGINS_NOT_ADOPTED_NOTICE = (
    ...
    "Anakot uses only its own logins; run `anakot auth add <provider>` to add one."
)
...
def adopt_external_logins_enabled() -> bool:
    ...
    Codex and Claude OAuth refresh tokens are single-use and rotate, so once Anakot borrows a CLI's
    token pair the two programs hold one token family and whichever refreshes first logs the other
    out. When the user opts out, Anakot never reads or refreshes those files and says so once per
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    """env:<VAR> — clear from ~/.anakot/.env; hint when the shell exports it."""
    from anakot_cli.config import get_env_path, remove_env_value
    ...
    # .env as utf-8-sig like anakot_cli/config.py: a BOM-sensitive read would
    ...
    if env_in_process and not env_in_dotenv:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Anakot.",
            f"  The pool entry is now suppressed — Anakot will ignore "
            f"{env_var} until you run `anakot auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_anakot_pkce(provider: str, removed) -> RemovalResult:
    """~/.anakot/.anthropic_oauth.json is ours — delete it outright."""
    from anakot_constants import get_anakot_home

    result = RemovalResult()
    oauth_file = get_anakot_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Anakot Anthropic OAuth credentials")
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.append(
        "Run `anakot model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    result.hints.extend([
        ...
        "Run `anakot auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `anakot auth add copilot` to re-enable if needed.",
    ])
...
_REGISTRY: List[RemovalStep] = [
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `anakot auth add anthropic` to re-enable if needed.",
        ),
        description="~/.claude/.credentials.json",
    ),
    RemovalStep(
        provider="anthropic", source_id="anakot_pkce",
        remove_fn=_remove_anakot_pkce,
        description="~/.anakot/.anthropic_oauth.json",
    ),
    RemovalStep(
    ...
    RemovalStep(
        ...
        remove_fn=_suppress_only(
            ...
            "Run `anakot auth add qwen-oauth` to re-enable if needed.",
        ),
