...
from anakot_constants import (
    get_anakot_home, get_skills_dir, is_wsl, reset_anakot_home_override, set_anakot_home_override,
)
...
def _scan_context_content(content: str, filename: str, *, user_authored: bool = False) -> str:
    ...
    *user_authored* (SOUL.md in the user's own ANAKOT_HOME): a hit is WARNED and the file still loads.
    ...
    (repo AGENTS.md / .cursorrules / .anakot.md) arrive with the checkout and keep blocking, and so does
    a SOUL.md owned by a profile distribution (``anakot profile install <git-url>`` copies it in unscanned;
    ...
    if user_authored:
        logger.warning("Context file %s matched injection pattern(s) %s; loaded anyway because it is the "
                       "user's own file in ANAKOT_HOME — review it if you did not write that text",
...
def _find_anakot_md(cwd: Path) -> Optional[Path]:
    """Nearest ``.anakot.md`` / ``ANAKOT.md`` from *cwd* up to the git root, else None."""
    ...
    for directory in [current, *current.parents] if stop_at else [current]:
        found = next((directory / n for n in (".anakot.md", "ANAKOT.md") if _is_file_or_denied(directory / n)), None)
...
DEFAULT_AGENT_IDENTITY = (
    ...
    "You are Anakot Agent, built by Nous Research. Be direct: match the length of your reply to the weight of the ask "
...
ANAKOT_AGENT_HELP_GUIDANCE = (
    # Injected only when skill_view exists AND the anakot-agent skill is installed (system_prompt.py slot
    # resolution). No "when the two differ" clause: docs-are-authoritative already carries the precedence.
    "You run on Anakot Agent (by Nous Research). When the user needs help with Anakot itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://anakot-agent.nousresearch.com/docs is your "
    "authoritative reference and always holds the latest, most up-to-date information. The `anakot-agent` "
    "skill has the actual commands and proven workflows — load it with skill_view(name='anakot-agent') "
    "before configuring, modifying, or troubleshooting Anakot so you don't guess or invent workarounds."
)

# Variant for sessions without the skills toolset (e.g. Blank Slate): naming skill_view() there would dangle.
ANAKOT_AGENT_HELP_GUIDANCE_NO_SKILLS = (
    "You run on Anakot Agent (by Nous Research). When the user needs help with Anakot itself — configuring, "
    "setting up, using, extending, or troubleshooting it — or when you need to understand your own features, "
    "tools, or capabilities, the documentation at https://anakot-agent.nousresearch.com/docs is the "
...
KANBAN_GUIDANCE = (
    ...
    "You have been assigned ONE task from the shared board at `~/.anakot/kanban.db`. Your task id is in "
    "`$ANAKOT_KANBAN_TASK`; your workspace is `$ANAKOT_KANBAN_WORKSPACE`. The `kanban_*` tools in your schema are your "
    ...
    "2. **Work inside the workspace.** `cd $ANAKOT_KANBAN_WORKSPACE` before any file operations. The workspace is "
    ...
    "- **Workspace.** `cd $ANAKOT_KANBAN_WORKSPACE` first. For a `worktree` kind with no `.git`, `git worktree add "
    "<path> ${ANAKOT_KANBAN_BRANCH:-wt/$ANAKOT_KANBAN_TASK}` from the main repo, then cd there. For a project-linked "
    "task the workspace is a fresh `<repo>/.worktrees/<task-id>` and `$ANAKOT_KANBAN_BRANCH` a deterministic "
    ...
    "sits in `ready` forever). Ground every assignee in a real profile (`anakot profile list`, or ask the user), and "
    "express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n\n"
    "- Do not shell out to `anakot kanban <verb>` for board operations. Use the `kanban_*` tools — they work across "
...
# cost that compounds over a long conversation. The anakot-agent runtime already executes a batch of tool
...
# to anakot-agent's Python prompt-assembly architecture.
...
# Text after the "[" that opens one of Anakot' own control frames (the steer marker above, the compaction
...
# apps/desktop/src/plugins/anakot-bots/group-round-prompt.ts byte-equivalent to this list.
...
STEER_CHANNEL_NOTE = (
    ...
    "Mid-turn, the user can steer you: Anakot delivers their message as a standalone user message right after "
...
def hud_surface_note(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    gated = (
        (True,
         "[Note: this message came from HUD mode — a small floating Anakot "
         ...
         "rather than anything inside Anakot. read_window_below identifies that app."),
        (True,
...
PLATFORM_HINTS = {
    ...
    "tui": (
        ...
        "You are in the Anakot terminal UI (TUI). Files: there is no attachment channel and MEDIA:/path tags "
    ...
    "desktop": (
        ...
        "You are chatting inside the Anakot desktop app, a graphical chat surface. Markdown renders with full GitHub "
        ...
        "or it measures full-bleed. Widgets talk back: data-anakot-send=\"prompt\" on any clickable element (or "
        "window.anakot.send(\"prompt\")) sends that prompt as a hidden user turn — answer it by updating the widget's "
        "file, not with prose."
    ),
    "sms": (
...
def _run_backend_probe(env_type: str, terminal_tool) -> str:
    ...
    env = _create_environment(
        ...
        # snapshot. A normal SSHEnvironment would upload the whole ~/.anakot tree just to run `uname`,
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    cache_key = (anakot_home_key(), env_type, _tenv_read("TERMINAL_CWD", ""))
...
def _remote_backend_hint(backend: str) -> str:
    ...
    if probe:
        return lead + (
            f"this {backend} environment — NOT on the machine where Anakot itself is running. The host OS, "
            f"home, and cwd of the Anakot process are irrelevant; only the following backend state matters:\n{probe}"
        )
    description = (
    ...
    return lead + (
        f"{description} — NOT on the machine where Anakot itself runs. The backend probe didn't respond at "
...
def _embedder_environment_hint() -> str:
    """Embedder-supplied environment description: ANAKOT_ENVIRONMENT_HINT (container ENV)
    wins over config.yaml ``agent.environment_hint``. Read once at prompt-build time."""
    return (os.getenv("ANAKOT_ENVIRONMENT_HINT") or "").strip() or str(
...
RUNTIME_ENVIRONMENT_HEADING = "# Anakot runtime environment"
RUNTIME_ENVIRONMENT_END = "<!-- End Anakot runtime environment -->"
...
def _skills_prompt_snapshot_path() -> Path:
    return get_anakot_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("ANAKOT_PLATFORM") or os.environ.get("ANAKOT_SESSION_PLATFORM")
    if platform:
        return platform
    get_session_env = getattr(sys.modules.get("gateway.session_context"), "get_session_env", None)
    try:
        return (get_session_env("ANAKOT_SESSION_PLATFORM") if get_session_env else "") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    ``skills_dir_override`` makes home resolution EXPLICIT: a build thread that never bound the ANAKOT_HOME
    ...
    if skills_dir_override is not None:
        ...
        _home_token = set_anakot_home_override(str(skills_dir.parent))
    else:
    ...
    try:
        ...
        if _home_token is not None:
            reset_anakot_home_override(_home_token)
...
def load_soul_md(context_length: Optional[int] = None, home_override: "Path | None" = None) -> Optional[str]:
    """SOUL.md from ANAKOT_HOME (identity slot #1), or None.

    Callers must pass ``skip_soul=True`` to ``build_context_files_prompt`` so it isn't injected twice.
    ``home_override`` pins the profile home (a thread that lost the ANAKOT_HOME ContextVar reads the wrong one).

    ``home_override`` scopes the read to an explicit profile home (the agent knows its own home from its
    session_db path). Without it, resolution is ambient — which on a thread that lost the ANAKOT_HOME
    ...
    """
    try:
        from anakot_cli.config import ensure_anakot_home
        ensure_anakot_home()
    except Exception as e:
        logger.debug("Could not ensure ANAKOT_HOME before loading SOUL.md: %s", e)
    soul_path = (Path(home_override) if home_override is not None else get_anakot_home()) / "SOUL.md"
    ...
    try:
        ...
        if not content:
            return None
        # `anakot profile install <git-url>` / `profile update` plant a third-party SOUL.md into a
        # distribution profile (anakot_cli/profile_distribution.py, DEFAULT_DIST_OWNED) with no scan and no
        ...
        from anakot_cli.profile_distribution import read_manifest
        try:
...
def _anakot_md_candidates(cwd_path: Path) -> list[tuple[str, Path, str]]:
    """.anakot.md / ANAKOT.md — nearest match walking up to the git root."""
    path = _find_anakot_md(cwd_path)
    if path is None:
        return []
...
_CONTEXT_FILE_CANDIDATES = {
    "anakot_md": _anakot_md_candidates,
...
def _project_context_suppressed(cwd: Optional[str], cwd_path: Path, allow_install_tree_fallback: bool) -> bool:
    """A FALLBACK-picked cwd inside the Anakot install tree must not gain system-prompt authority (the desktop
    default would load this repo's contributor AGENTS.md). An explicitly configured cwd is honored verbatim —
    the Anakot tree is a legitimate workspace when the user deliberately points a session at it — and
    CLI-style surfaces pass allow_install_tree_fallback=True because their launch dir IS the user's shell cwd
    (developing Anakot in-tree). See #64590."""
    from agent.runtime_cwd import _is_install_tree
    return cwd is None and not allow_install_tree_fallback and _is_install_tree(cwd_path)


def _load_anakot_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.anakot.md / ANAKOT.md — nearest match walking up to the git root."""
    for label, path, content in _anakot_md_candidates(cwd_path):
        if content:
            return _context_section(_strip_yaml_frontmatter(content), label, ".anakot.md", path, context_length)
    return ""
...
def build_context_files_prompt(
    ...
    Only ONE project context type loads, first found wins: .anakot.md/ANAKOT.md (walk to git root) →
    AGENTS.md chain (git root → cwd) → CLAUDE.md (cwd) → .cursorrules + .cursor/rules/*.mdc (cwd). SOUL.md
    from ANAKOT_HOME is independent and always included unless *skip_soul* (already the identity slot).
    """
    ...
    if _project_context_suppressed(cwd, cwd_path, allow_install_tree_fallback):
        logger.warning(
            "skipping project-context discovery: working-directory resolution fell back to the Anakot "
            "install tree (%s) — set terminal.cwd to your project directory", cwd_path,
        )
        sections = []
    else:
        sections = [_load_anakot_md(cwd_path, context_length) or _load_agents_md(cwd_path, context_length)
