...
def _detect_kanban() -> bool:
    ...
    if os.getenv("ANAKOT_KANBAN_TASK") or os.getenv("ANAKOT_KANBAN_BOARD"):
        try:
...
def _load_raw_config() -> Dict[str, Any]:
    """Read config.yaml with an mtime+size keyed cache (no anakot_cli.config import)."""
...
def _home_relative(p: Path) -> Path:
    """Anchor a relative config path at ANAKOT_HOME; absolute paths pass through."""
    from anakot_constants import get_anakot_home
    return p if p.is_absolute() else get_anakot_home() / p


# Never disableable: `anakot-agent` is the agent's own operating manual and the
# system prompt points at it unconditionally.
ESSENTIAL_SKILLS: frozenset = frozenset({"anakot-agent"})


def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    """Disabled skill names from config.yaml: global list ∪ platform list
    (*platform* defaults to ``ANAKOT_PLATFORM`` / ``ANAKOT_SESSION_PLATFORM``)."""
    skills_cfg = _skills_cfg()
    if skills_cfg is None:
        return set()
    from gateway.session_context import get_session_env
    resolved_platform = platform or os.getenv("ANAKOT_PLATFORM") or get_session_env("ANAKOT_SESSION_PLATFORM")
...
def parse_config_string_list(value) -> List[str]:
    ...
    ``anakot config set`` stores lists as quoted JSON/Python-literal strings;
...
def get_external_skills_dirs() -> List[Path]:
    ...
    are ``~``/``${VAR}`` expanded, relative to ANAKOT_HOME; the local skills dir is skipped."""
    config_path = get_config_path()
    if not config_path.exists():
        return []
...
def get_skill_create_dir() -> Optional[Path]:
    ...
    relative to ANAKOT_HOME; a value equal to the local skills dir counts as unset."""
...
def display_skill_create_dir() -> str:
    ...
    if create_dir is None:
        return f"{display_anakot_home()}/skills/"
...
def get_all_skills_dirs() -> List[Path]:
    """Skill dirs: local ``~/.anakot/skills/`` first, then create_dir, then external.
...
# Project-local skills (<root>/.anakot/skills, <root>/.agents/skills; root = nearest
...
PROJECT_SKILLS_SUBDIRS = (os.path.join(".anakot", "skills"), os.path.join(".agents", "skills"))
...
def _candidate_project_skills_dirs(root: Path) -> List[Path]:
    ...
    (ANAKOT_HOME itself may live inside a git checkout)."""
    local_skills = get_skills_dir().resolve()
    dirs: List[Path] = []
...
# cached under ANAKOT_HOME, never inside the repo); "dangerous" excludes the
# skill from index, list, view and slash commands ("caution" loads, as on the hub).

# ── Project skill quarantine (scan-time injection defense) ──────────────── Trust (`anakot skills trust`)
...
# high-confidence findings only. The scan cache lives under ANAKOT_HOME, never inside the repo (we don't
...
def is_quarantined_project_skill(skill_md) -> bool:
    ...
    try:
        ...
        from anakot_constants import get_anakot_home
        cache_dir = get_anakot_home() / "cache" / "project_skill_scans"
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    # (which follows the live profile-scoped ANAKOT_HOME), so normalization
    ...
    # ~/.anakot/skills/<name> may be a symlink to a checkout elsewhere, and
...
def _anakot_metadata(frontmatter: Dict[str, Any]) -> Dict[str, Any]:
    """``metadata.anakot`` mapping from frontmatter, or ``{}`` when malformed."""
    metadata = frontmatter.get("metadata")
    anakot = metadata.get("anakot") if isinstance(metadata, dict) else None
    return anakot if isinstance(anakot, dict) else {}
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    anakot = _anakot_metadata(frontmatter)
    return {key: anakot.get(key, []) for key in _CONDITION_KEYS}


def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Extract ``metadata.anakot.config`` declarations (key/description/default/prompt).
    Entries missing ``key`` or ``description`` are skipped; ``prompt`` defaults to the description."""
    raw = _anakot_metadata(frontmatter).get("config")
...
def _expand_skill_config_path(value: str) -> str:
    """Expand ``~`` / ``$HOME`` against the HOME Anakot injects into tool subprocesses.

    Skill config defaults describe paths the agent hands to tools, so in a container where the
    control process HOME (``/opt/data``) differs from the tool HOME (``{ANAKOT_HOME}/home``) a
