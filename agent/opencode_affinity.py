...
same way as the other conversation-affinity hints Anakot already sends
...
def opencode_transport(provider: Optional[str], model: Optional[str], base_url: Optional[str]) -> tuple[Optional[str], str]:
    ...
    The main runtime (``anakot_cli/runtime_provider.py``) always re-derives from the effective model;
    ...
    hosts all count.
    """
    from anakot_cli.models import normalize_opencode_base_url, normalize_opencode_model_id, opencode_model_api_mode
    from anakot_cli.runtime_provider_custom import _get_named_custom_provider, _opencode_family_for_custom
