...
try:
    import anakot_cli as _anakot_cli

    _ANAKOT_VERSION = str(_anakot_cli.__version__)
except Exception:
    _ANAKOT_VERSION = "0.0.0"
_API_CLIENT = f"anakot-agent/{_ANAKOT_VERSION}"  # client context per Gemini's partner-integration guidance
...
_FREE_TIER_GUIDANCE = (
    ...
    "Anakot typically makes 3-10 API calls per user turn, so the free tier is exhausted in a handful of "
...
_STANDARD_KEY_GUIDANCE = (
    ...
    "generativelanguage.googleapis.com). Then update GEMINI_API_KEY / GOOGLE_API_KEY in ~/.anakot/.env, "
...
_MISSING_KEY_ERROR = (
    ...
    "environment / ~/.anakot/.env (get one at https://aistudio.google.com/app/apikey), or run `anakot setup` to "
...
class GeminiAPIError(Exception):
    """Error shape compatible with Anakot retry/error classification."""
...
class GeminiNativeClient:
    ...
    # (No ANAKOT_SKIP_ASYNC_WRAP — the async path has a real conversion, AsyncGeminiNativeClient.)
