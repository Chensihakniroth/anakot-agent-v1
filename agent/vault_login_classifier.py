...
INSPECTION_STAMP_ATTR = "data-anakot-vault-slot"
...
_LOGIN_CONTROL_INSPECTION_JS_TEMPLATE = """(() => {
  ...
  elements.forEach((element, index) => element.setAttribute("data-anakot-vault-slot", nonce + ":" + index));
...
_FILL_JS_TEMPLATE = """(() => {
  ...
  for (const f of fills) {
    const el = document.querySelector('[data-anakot-vault-slot="' + nonce + ':' + f.index + '"]');
  ...
  document.querySelectorAll("[data-anakot-vault-slot]").forEach((n) => n.removeAttribute("data-anakot-vault-slot"));
