...
``anakot pause`` writes a sentinel at ``$ANAKOT_HOME/ESTOP``; ``anakot resume``
...
(fail safe, e.g. ``touch ~/.anakot/ESTOP``). Ported from gastownhall/gastown estop.go (MIT).
"""
...
# Same profile-aware / fleet-root resolvers the file-safety guards use (fail-open to ~/.anakot).
from agent.file_safety import _anakot_home_path as _anakot_home, _anakot_root_path as _canonical_root
...
def sentinel_path() -> Path:
    """Path of the ESTOP sentinel this process would write on `anakot pause`."""
    return _anakot_home() / SENTINEL_NAME


def _candidate_sentinel_paths() -> list:
    """Profile home first, then the fleet root if it is a different directory: a profile
    gateway (ANAKOT_HOME=~/.anakot/profiles/<n>) must still honor an operator's ~/.anakot/ESTOP."""
...
def paused_reply() -> Optional[str]:
    ...
    return f"⏸️ Anakot is paused{tag}. New work is on hold; run `anakot resume` to pick things back up."
...
def check_paused(component: str, logger: logging.Logger) -> bool:
    ...
    if first:
        ...
        logger.info(
            "%s dispatch paused by global emergency stop%s — remove with `anakot resume` (%s)",
