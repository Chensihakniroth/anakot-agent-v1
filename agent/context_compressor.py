...
_SUMMARY_ROUTE_PIN: contextvars.ContextVar[Optional[Dict[str, Any]]] = (
    contextvars.ContextVar("anakot_summary_route_pin", default=None)
)
...
# (anakot_state._rows_to_conversation), so any code that mutates a loaded or flushed dict's content in place
...
_COMPRESSION_MARKER_PREFIX = "⟪ANAKOT-CONTEXT-COMPRESSION:"
_COMPRESSION_MARKER_TEMPLATE = (
    _COMPRESSION_MARKER_PREFIX
    + " {omitted:,} of {total:,} chars omitted here by Anakot's context compressor. "
...
def _today_for_prompt() -> str:
    ...
    try:
        ...
        # via anakot_time.now(). The compaction summary is a mid-conversation message that is NOT part of
        ...
        return _anakot_now().strftime("%Y-%m-%d")
...
class ContextCompressor(SummaryDispatchMixin, MicroCompactionMixin, ContextEngine):
    ...
    def _serialize_records_for_summary(self, turns: List[Dict[str, Any]]) -> List[str]:
        ...
        Records are kept STRUCTURAL (a list, not one "\n\n"-joined string) so lean sampling
        can never split a message whose body contains blank lines into label-less fragments.
        """
    ...
    def _serialize_for_summary(self, turns: List[Dict[str, Any]]) -> str:
        ...
        Byte-identical to the pre-record form: legacy tail_mode and micro-compaction's
        ``_micro_summarize_one`` consume this and never sample, so a record-level rstrip
        here would silently change the bytes they reproduce in the prompt.
        """
    ...
    def _generate_summary(
        ...
        if getattr(self, "tail_mode", "lean") == "lean":
            content_to_summarize, coverage = self._sample_summary_records(
                self._serialize_records_for_summary(turns_to_summarize)
            )
