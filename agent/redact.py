...
# `export ANAKOT_REDACT_SECRETS=false`) cannot disable redaction mid-session.
...
# to this env var in anakot_cli/main.py, gateway/run.py, and cli.py) or `ANAKOT_REDACT_SECRETS=false` in
# ~/.anakot/.env. An opt-out warning is logged at gateway and CLI startup so operators see the downgrade —
# see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("ANAKOT_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}

# Routed multiplex profiles: the import-time snapshot above is the LAUNCH profile's policy. A profile
# served under a ANAKOT_HOME override resolves its own ``security.redact_secrets`` (its ``.env``
# value first, like the standalone bridge in anakot_cli/main.py), cached per home so the hot path
...
def _redact_enabled() -> bool:
    ...
    try:
        ...
        raw = scope.get("ANAKOT_REDACT_SECRETS") if scope else None
        if raw is None:
...
def mask_secret(value: str, *, head: int = 4, tail: int = 4, floor: int = 12,
    ...
    """Mask a secret for display (``anakot config`` / ``status`` / ``dump``):
...
# a shell rc/profile, Anakot' own ``config.yaml`` where ``anakot mcp add --env`` writes
...
_ANAKOT_HOME_PREFIXES = ("$ANAKOT_HOME/", "${ANAKOT_HOME}/")
# ``$HOME/.anakot/config.yaml`` keeps the ``.anakot`` segment, so stripping the prefix is
...
def _is_under_anakot_home(path: str) -> bool:
    """True when an absolute ``config.yaml`` path sits under the active Anakot home or root.

    The default home's basename is an installation detail — ``.anakot`` on POSIX, ``anakot``
    under ``AppData/Local`` on Windows — and a resolved path never spells ``$ANAKOT_HOME``,
...
def _is_secret_file_arg(arg: str) -> bool:
    ...
    ``.anakot`` directory, ``$ANAKOT_HOME``, or the resolved Anakot home (never arbitrary
    YAML). The resolved-home arm is what covers native Windows, where the home directory
    is ``%LOCALAPPDATA%\\anakot`` and carries no ``.anakot`` segment."""
    path = arg.strip("\"'").replace("\\", "/")
    anakot_home = False
    for prefix in _ANAKOT_HOME_PREFIXES:
    ...
    if parts[-1] in _ENV_FILE_BASENAMES or parts[-1] in _SHELL_RC_BASENAMES:
        return True
    # ``config.yaml`` plus the ``config.yaml.good.<stamp>`` / ``.corrupt.<stamp>`` copies Anakot
    ...
    if parts[-1] != "config.yaml" and not parts[-1].startswith(("config.yaml.good.", "config.yaml.corrupt.")):
        return False
    return anakot_home or ".anakot" in parts[:-1] or _is_under_anakot_home(path)
...
def redact_terminal_output(output: str, command: str | None = None, *, force: bool = False) -> str:
    ...
    shell rc, Anakot ``config.yaml``); otherwise code_file=True avoids false positives on
    source/config dumps."""
    if not output:
        return output
