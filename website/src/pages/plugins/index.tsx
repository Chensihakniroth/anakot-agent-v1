...
// anakot_cli/plugin_catalog.py and website/scripts/extract-plugins.py.
const CATEGORY_CONFIG: Record<string, { label: string; icon: string; blurb: string }> = {
  desktop: { label: "Desktop", icon: "\u{1F5A5}\u{FE0F}", blurb: "Panes, tabs and views for Anakot Desktop" },
      ...
      <div className={styles.cardInner}>
        ...
        {expanded && (
          <div className={styles.cardDetail}>
            ...
            {plugin.requiresAnakot && (
              <div className={styles.metaRow}>
                <span className={styles.metaLabel}>Requires</span>
                <span className={styles.metaValue}>
                  <code>anakot {plugin.requiresAnakot}</code>
                </span>
              </div>
            )}
            <div className={styles.metaRow}>
...
export default function PluginCatalogPage() {
  ...
  // (Anakot desktop's Capabilities > Plugins tab) as a one-click catalog
  // picker. Site chrome is hidden via CSS and every card gains an
  // "+ Add to this Agent" button that posts
  //   { type: 'anakot-plugin-pick', name, repo, sha, subdir, tier,
  //     installCmd }
    ...
    window.parent.postMessage(
      {
        ...
        tier: plugin.tier,
        installCmd: plugin.installCommand || `anakot plugins install ${plugin.name}`,
      },
      "*"
    );
  }, []);
  ...
  return (
    <Layout
      ...
      description="Give Anakot new powers: reviewed plugins you can install in one click"
    >
      <div className={`${styles.page} ${pickerMode ? styles.pickerMode : ""}`}>
        <header className={styles.hero}>
          <div className={styles.heroGlow} />
          <div className={styles.heroContent}>
            <p className={styles.heroEyebrow}>Anakot Agent</p>
            ...
            <p className={styles.heroSub}>
              Give Anakot new powers. Memory, voice, messaging, browsing, Desktop panes and more,
              built by the community and reviewed by the Anakot team before it reaches you.
              {loadError && (
            ...
            {!catalogEmpty && (
              <ul className={styles.trustRow} aria-label="What every listing gets you">
                <TrustChip icon="check" text="Reviewed by the Anakot team" />
                <TrustChip icon="lock" text="Installs exactly the version we reviewed" />
                <TrustChip icon="download" text="One click from Anakot Desktop" />
              </ul>
            )}
          </div>
        </header>
          ...
          <div className={styles.controlsBar}>
            <div className={styles.searchWrap}>
              ...
              <input
                ref={searchRef}
                type="text"
                placeholder="Search plugins by name or by what you want Anakot to do"
          ...
          {!data && !loadError ? (
            ...
            <div className={styles.empty}>
              ...
              <p className={styles.emptyDesc}>
                The plugin catalog is a curated, human-reviewed list of Anakot
