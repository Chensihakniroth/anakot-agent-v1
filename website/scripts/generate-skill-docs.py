...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    anakot_meta = (fm.get("metadata") or {}).get("anakot") or {}
    tags = anakot_meta.get("tags") or []
    related = anakot_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `anakot skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Anakot Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Anakot ships with a large built-in skill library copied into `~/.anakot/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Anakot also syncs bundled skills on `anakot update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.anakot/skills/` tree, it is still shipped with Anakot; restore it with `anakot skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with anakot-agent — install via anakot skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with anakot-agent under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
        "anakot skills install official/<category>/<skill>",
        "```",
        "",
        "For example:",
        "",
        "```bash",
        "anakot skills install official/blockchain/solana",
        "anakot skills install official/mlops/flash-attention",
        "```",
        "",
        ...
        "",
        "```bash",
        "anakot skills uninstall <skill-name>",
        "```",
        "",
    ]
    for category in sorted(by_cat):
