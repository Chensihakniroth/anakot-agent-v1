...
2. The unified Anakot Skills Index at ``website/static/api/skills-index.json``,
    ...
    for base_dir, source_label in LOCAL_SKILL_DIRS:
        ...
        for root, _dirs, files in os.walk(base_path):
            ...
            if isinstance(metadata, dict):
                anakot_meta = metadata.get("anakot", {})
                if isinstance(anakot_meta, dict):
                    tags = anakot_meta.get("tags", [])
            if not tags:
                tags = fm.get("tags", [])
