...
description: "Run Anakot Agent directly on an Android phone with Termux"
---

# Anakot on Android with Termux
...
Anakot Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Anakot from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
APT-managed Anakot installs are marked with install method `apt`. Anakot therefore does not run its Git self-updater against package-owned files; use the package manager instead:

```bash
pkg update
pkg upgrade anakot-agent
```

Packaging/repository/signing problems for this option should be reported to the community packaging repositories above. Anakot runtime bugs can still be reported here, keeping in mind that Android/Termux support is best-effort.
...
Anakot requires **Python >=3.11,&lt;3.14**. Current Termux ships `python`
...
### 5. Put `anakot` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/anakot" "$PREFIX/bin/anakot"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `anakot` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.anakot/.env`.
...
### `anakot doctor` says ripgrep or Node is missing
