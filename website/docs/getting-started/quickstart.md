---
sidebar_position: 1
title: "Anakot Agent Quickstart"
description: "Your first conversation with Anakot Agent — from install to chatting in under 5 minutes"
---

# Anakot Agent Quickstart

This guide gets you from zero to a working Anakot setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Anakot Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Anakot Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Anakot for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Anakot working on my machine | `anakot setup` | Run a real chat and verify it responds |
| I already know my provider | `anakot model` | Save the config, then start chatting |
| I want a bot or always-on setup | `anakot gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `anakot model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `anakot model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Anakot cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Anakot Agent
### With the Anakot Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Anakot Desktop installer](https://anakot-agent.nousresearch.com/) from our website and run it.
...
The single most important setup step. Use `anakot model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `anakot setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `anakot update`. Re-enable anything later with `anakot tools`, seed skills with `anakot skills opt-in --sync`, or tune settings with `anakot setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `anakot model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `anakot model` → **ChatGPT or Codex Subscription** |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `anakot model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `anakot model` (requires `copilot` CLI + `copilot login`) |
...
Anakot Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `anakot model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Anakot separates secrets from normal config:

- **Secrets and tokens** → `~/.anakot/.env`
- **Non-secret settings** → `~/.anakot/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
anakot config set model anthropic/claude-opus-4.6
...
```bash
anakot            # classic CLI
anakot --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Anakot ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `anakot` vs `anakot --tui`.
:::
...
```bash
anakot --continue    # Resume the most recent session
anakot -c            # Short form
```
...
```bash
anakot gateway setup    # Interactive platform configuration
```
...
- `anakot tools` — tune tool access per platform
- `anakot skills` — browse and install reusable workflows
...
```bash
anakot config set terminal.backend docker    # Docker isolation
anakot config set terminal.backend ssh       # Remote server
```

For Docker sandboxes, you can also enable the **egress credential-injection proxy** so the sandbox never sees your real API keys — only opaque proxy tokens that work exclusively from behind a local TLS-intercepting daemon. See [Egress proxy](../user-guide/egress/iron-proxy.md). Setup is `anakot egress setup && anakot egress start`; `anakot setup terminal` also points Docker users at it. Modal, SSH, Daytona, and Singularity are not wired yet.

### Voice mode

```bash
# From the Anakot install directory (the curl installer placed it at
# ~/.anakot/anakot-agent on Linux/macOS or %LOCALAPPDATA%\anakot\anakot-agent on Windows):
cd ~/.anakot/anakot-agent
...
Skills are on-demand instruction documents that teach Anakot how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Anakot ships with a catalog of bundled skills already installed in `~/.anakot/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
anakot skills browse                      # list everything available
anakot skills search kubernetes           # find skills by keyword
anakot skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `anakot skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Anakot ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.anakot/anakot-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Anakot opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `anakot model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `anakot gateway setup` and check `anakot gateway status` |
| `anakot --continue` can't find old session | Switched profiles or session never saved | Check `anakot sessions list` and confirm you're in the right profile |
...
| `anakot doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `anakot model`
3. `anakot setup`
4. `anakot sessions list`
5. `anakot --continue`
6. `anakot gateway status`
...
| `anakot tools` | Configure which tools are enabled per platform |
| `anakot setup` | Full setup wizard (configures everything at once) |
| `anakot doctor` | Diagnose issues |
| `anakot update` | Update to latest version |
| `anakot gateway` | Start the messaging gateway |
| `anakot --continue` | Resume last session |
...
- **[Moving to another machine](../reference/faq.md#exporting-anakot-to-another-machine)** — `anakot backup` migrates your whole setup (or [a single profile](../reference/faq.md#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
