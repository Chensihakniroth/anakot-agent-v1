...
description: "One-command import of a Claude Code (~/.claude) or OpenAI Codex CLI (~/.codex) setup into Anakot — instructions, allowlists, MCP servers, skills, and memories."
---

# Import from Other Agents

`anakot import-agent` imports your existing **Claude Code** or **OpenAI Codex CLI** setup into Anakot with one command. It follows the same preview-first pattern as [`anakot claw migrate`](../guides/migrate-from-openclaw.md): you always see a per-item plan before anything is written, and `--dry-run` never touches disk.

```bash
anakot import-agent                    # auto-detect ~/.claude or ~/.codex
anakot import-agent claude-code        # import from ~/.claude
anakot import-agent codex              # import from ~/.codex
anakot import-agent claude-code --dry-run          # preview only
anakot import-agent codex --source /path/to/.codex # custom location
anakot import-agent claude-code --overwrite --yes  # replace conflicts, skip prompts
```
...
| Claude Code | Anakot |
|---|---|
| `CLAUDE.md` (global instructions) | Memory entries in `~/.anakot/memories/MEMORY.md` |
...
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.anakot/skills/claude-code-imports/<name>/` |
...
| Codex CLI | Anakot |
|---|---|
| `AGENTS.md` (global instructions) | Memory entries in `~/.anakot/memories/MEMORY.md` |
| `config.toml` → `[mcp_servers.*]` | `mcp_servers` in `config.yaml` |
| `memories/*.md` | Memory entries in `~/.anakot/memories/MEMORY.md` |
| `skills/<name>/` (dirs with `SKILL.md`) | `~/.anakot/skills/codex-imports/<name>/` |

## What is never imported

**API keys and credentials.** Credential files (`~/.claude/.credentials.json`, `~/.codex/auth.json`) are never read, and MCP server environment variables or headers with secret-looking names (`*_TOKEN`, `*_API_KEY`, `Authorization`, ...) are stripped and listed in the report so you can re-add them deliberately. Run `anakot setup` to configure providers, or add secrets to `~/.anakot/.env`.
...
- **Conflicts are skipped by default.** An MCP server or skill that already exists in Anakot is reported as a conflict; pass `--overwrite` to replace it.
...
- Coming from OpenClaw instead? Use [`anakot claw migrate`](../guides/migrate-from-openclaw.md).

## Keeping imports in sync

Every successful import registers its source (and the digest of everything it read) in `~/.anakot/import-sync.json`. When the other agent's setup changes later — new skills, edited `CLAUDE.md`/`AGENTS.md`, added MCP servers — pull the changes in with:

```bash
anakot import-agent --sync            # re-import every changed source
...
The manifest is per profile. A profile created with `anakot profile create <name> --clone --sync-imports` carries it over, so `anakot -p <name> import-agent --sync` keeps pulling from the same external trees (see [Profiles](./profiles.md#keep-a-clones-imported-agent-setups-synced---sync-imports)).
