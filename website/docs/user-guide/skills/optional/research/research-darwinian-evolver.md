...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research\darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Anakot Agent |
| License | MIT |
...
import upstream classes into Anakot itself.
...
```bash
mkdir -p ~/.anakot/cache/darwinian-evolver && cd ~/.anakot/cache/darwinian-evolver
...
```bash
cd ~/.anakot/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.anakot/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.anakot/skills/research/darwinian-evolver
DE_DIR=~/.anakot/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Anakot core.
   Custom driver scripts under `~/.anakot/skills/...` are user-side and fine.
