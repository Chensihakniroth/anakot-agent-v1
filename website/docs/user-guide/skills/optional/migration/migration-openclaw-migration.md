---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Anakot"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Anakot"
---
...
Import an OpenClaw setup (memories, skills) into Anakot.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `anakot skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Anakot Agent (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Anakot`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Anakot Migration

Use this skill when a user wants to move their OpenClaw setup into Anakot Agent with minimal manual cleanup.
...
```bash
anakot claw migrate              # Full interactive migration
...
anakot claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `anakot setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_anakot.py` to:

- import `SOUL.md` into the Anakot home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Anakot memory entries
- merge OpenClaw command approval patterns into Anakot `command_allowlist`
- migrate Anakot-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Anakot working-directory configuration
- copy OpenClaw skills into `~/.anakot/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Anakot workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.anakot/tts/`
- archive non-secret docs that do not have a direct Anakot destination
...
- `scripts/openclaw_to_anakot.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py`

Do not guess a shorter path like `~/.anakot/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.anakot/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Anakot-compatible secrets, currently:
...
Anakot CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Anakot. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Anakot workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Anakot skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.anakot/skills/migration/openclaw-migration/scripts/openclaw_to_anakot.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Anakot unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Anakot targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Anakot destination. Unsupported auth blobs must still be reported as skipped.
...
- Anakot persona state imported
- Anakot memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.anakot/skills/openclaw-imports/`
