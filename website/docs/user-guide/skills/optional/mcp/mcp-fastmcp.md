...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/mcp/fastmcp` |
...
| Related skills | [`anakot-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent.md), [`mcporter`](./mcp-mcporter.md) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Anakot or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Anakot. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.anakot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.anakot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Anakot integration, either:

- configure the server in `~/.anakot/config.yaml` using the `native-mcp` skill, or
...
### Anakot cannot see the deployed server

The server-building part may be correct while the Anakot config is not. Load the `native-mcp` skill and configure the server in `~/.anakot/config.yaml`, then restart Anakot.
