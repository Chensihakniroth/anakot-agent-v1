---
title: "Honcho — Configure and troubleshoot Honcho memory for Anakot"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Anakot"
---
...
Configure and troubleshoot Honcho memory for Anakot.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `anakot skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Anakot

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Anakot profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/anakot#running-honcho-locally-with-anakot

### Verify

```bash
anakot honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Anakot profile's AI peer
...
Honcho models conversations as interactions between **peers**. Anakot creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Anakot instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Anakot run |
| `global` | Single session across all directories |

Manual override: `anakot honcho map my-project-name`
...
Each Anakot profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block anakot.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `anakot.coder` host block in `honcho.json`
...
```bash
anakot honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Anakot when Honcho memory is active.
...
Config file: `$ANAKOT_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `anakot honcho setup`. Ensure `memory.provider: honcho` is in `~/.anakot/config.yaml`.

### Memory not persisting across sessions
Check `anakot honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `anakot profile create <name> --clone`. For existing profiles: `anakot honcho sync`.
...
| Command | Description |
|---------|-------------|
| `anakot honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `anakot honcho status` | Show resolved config, connection test, peer info for active profile |
| `anakot honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `anakot honcho disable` | Disable Honcho for the active profile |
| `anakot honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `anakot honcho peers` | Show peer identities across all profiles |
| `anakot honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `anakot honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `anakot honcho sessions` | List known directory-to-session-name mappings |
| `anakot honcho map <name>` | Map current working directory to a Honcho session name |
| `anakot honcho identity` | Seed AI peer identity or show both peer representations |
| `anakot honcho sync` | Create host blocks for all Anakot profiles that don't have one yet |
| `anakot honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Anakot + Honcho |
| `anakot memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `anakot memory status` | Show active memory provider and config |
| `anakot memory off` | Disable external memory provider |
