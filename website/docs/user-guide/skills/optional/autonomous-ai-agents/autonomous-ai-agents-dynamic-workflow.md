...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/autonomous-ai-agents/dynamic-workflow` |
| Path | `optional-skills/autonomous-ai-agents/dynamic-workflow` |
| Version | `2.0.0` |
| Author | Teknium + Anakot Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `orchestration`, `fan-out`, `subagents`, `delegation`, `verification`, `migration`, `audit`, `research`, `campaign` |
| Related skills | [`anakot-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent.md), [`simplify-code`](../../bundled/software-development/software-development-simplify-code.md) |
...
for serial chains. For a refactor or fix campaign on anakot-agent itself, load
`anakot-agent` (the dev workflow) alongside; this skill owns the fan-out shape.
...
10. Live QA is its own wave: one squad per surface, isolated `ANAKOT_HOME`,
