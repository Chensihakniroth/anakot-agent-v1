...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/autonomous-ai-agents/openhands` |
| Path | `optional-skills/autonomous-ai-agents\openhands` |
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), Anakot Agent |
| License | MIT |
| Platforms | linux, macos |
| Tags | `Coding-Agent`, `OpenHands`, `Model-Agnostic`, `LiteLLM` |
| Related skills | [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`opencode`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-opencode.md), [`anakot-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent.md) |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Anakot.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Anakot-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `anakot-agent` (Anakot subagents via `delegate_task`).
