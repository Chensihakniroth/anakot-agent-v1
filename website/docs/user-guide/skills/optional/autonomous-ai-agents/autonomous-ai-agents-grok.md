...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents\grok` |
| Version | `0.1.1` |
| Author | Matt Maximo (MattMaximo), Anakot Agent |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Coding-Agent`, `Grok`, `xAI`, `Code-Review`, `Refactoring`, `Automation` |
| Related skills | [`codex`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-codex.md), [`claude-code`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code.md), [`anakot-agent`](../../bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent.md) |
...
# Grok Build CLI — Anakot Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Anakot terminal. Grok
...
1. Prepare stable input files first with Anakot tools (`read_file`,
...
2. **Don't conflate Anakot' xAI auth with the `grok` CLI's auth.** Anakot'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Anakot' xAI auth carries
   over.
