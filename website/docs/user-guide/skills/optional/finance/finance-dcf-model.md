...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/finance/dcf-model` |
...
| Related skills | [`excel-author`](./finance-excel-author.md), [`pptx-author`](./finance-pptx-author.md), [`comps-analysis`](./finance-comps-analysis.md), [`lbo-model`](./finance-lbo-model.md), [`3-statement-model`](./finance-3-statement-model.md) |
...
**If you encounter errors or unreasonable results, read [TROUBLESHOOTING.md](https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md) for detailed debugging guidance.**
...
3. **Check output**:
   ...
   - If `status` is `"errors_found"` → Check `error_summary` and read [TROUBLESHOOTING.md](https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/optional-skills/finance\dcf-model/TROUBLESHOOTING.md) for debugging guidance
...
Many passages below say "use the S&P Kensho MCP / Daloopa MCP / FactSet MCP". Those are commercial financial-data MCPs from the original Cowork plugin context. In Anakot:

- **If you have any structured financial-data MCP configured** (Anakot supports MCP — see `native-mcp` skill), prefer it for point-in-time comps, precedent transactions, and filings.
