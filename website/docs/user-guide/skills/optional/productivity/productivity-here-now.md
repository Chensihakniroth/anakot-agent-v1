...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${ANAKOT_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${ANAKOT_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${ANAKOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client anakot
```
...
```bash
PUBLISH="${ANAKOT_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client anakot
```
...
```bash
DRIVE="${ANAKOT_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `anakot`)    |
