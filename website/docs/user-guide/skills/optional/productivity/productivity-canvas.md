...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Anakot Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${ANAKOT_HOME:-~/.anakot}/.env`:
...
```bash
CANVAS="python $ANAKOT_HOME/skills/productivity/canvas/scripts/canvas_api.py"
