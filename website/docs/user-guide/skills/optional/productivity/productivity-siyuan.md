...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/productivity/siyuan` |
...
3. Store it in `${ANAKOT_HOME:-~/.anakot}/.env`:
   ```
...
```yaml
# In ~/.anakot/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
