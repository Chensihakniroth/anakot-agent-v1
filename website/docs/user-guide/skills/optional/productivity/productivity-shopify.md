...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/productivity/shopify` |
...
4. Save to `${ANAKOT_HOME:-~/.anakot}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Anakot","productType":"Apparel","tags":["test"]}}'
```
