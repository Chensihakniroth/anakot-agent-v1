...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/creative/meme-generation` |
...
| Related skills | [`ascii-art`](./creative-ascii-art.md) |
...
4. Find the skill's script directory:
   ```
   SKILL_DIR=$(dirname "$(find ~/.anakot/skills -path '*/meme-generation/SKILL.md' 2>/dev/null | head -1)")
   ```
5. Run the generator:
   ```bash
