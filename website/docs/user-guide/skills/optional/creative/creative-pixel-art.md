...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.anakot/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.anakot/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Anakot installs package this)
