...
| | |
|---|---|
| Source | Optional — install with `anakot skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Anakot MCP catalog. The
...
Two halves, in this order: the editor side must be up before Anakot connects.
...
### One-time, Anakot side

    anakot mcp install unreal-engine
...
`~/.anakot/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Anakot — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Anakot connects
...
2. Start the Anakot session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Anakot
they appear as:
...
then happens at `anakot mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Anakot runs batched calls
...
- **Start order matters.** Editor + server up first, then the Anakot
...
- **Timeouts on long operations.** Anakot' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.anakot/config.yaml` for
