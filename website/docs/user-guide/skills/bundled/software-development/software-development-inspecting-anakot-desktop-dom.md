---
title: "Inspecting Anakot Desktop Dom — Read the live Anakot desktop DOM/CSS over CDP"
sidebar_label: "Inspecting Anakot Desktop Dom"
description: "Read the live Anakot desktop DOM/CSS over CDP"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Inspecting Anakot Desktop Dom

Read the live Anakot desktop DOM/CSS over CDP.
...
| Path | `skills/software-development\inspecting-anakot-desktop-dom` |
| Version | `1.0.0` |
| Author | Anakot Agent |
| License | MIT |
...
# Inspecting the live Anakot desktop DOM
...
`debugging-anakot-desktop`), or anything where the real question is "does this
look right".
...
- **no `ANAKOT_DESKTOP_DEV_SERVER`** — an unpackaged `electron .` against
  `dist/` is how the packaged app gets smoke tested, so it behaves like one.

`ANAKOT_DESKTOP_CDP_PORT` moves the port (`=9333`) or disables it (`=off`).

Check before doing anything else:

```bash
curl -s --max-time 3 http://127.0.0.1:${ANAKOT_DESKTOP_CDP_PORT:-9222}/json/version
```
...
```bash
cd apps/desktop
ANAKOT_HOME=/tmp/cdp-probe-home \
ANAKOT_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
ANAKOT_DESKTOP_CDP_PORT=9333 \
  npx electron . --user-data-dir=/tmp/cdp-probe-userdata
```

The separate `--user-data-dir` dodges Electron's single-instance lock, so it
cannot collide with a running `hgui`; the separate `ANAKOT_HOME` keeps it away
from real sessions. Pick a port other than 9222 for the same reason. Run it in
the background and kill it when done.

`npm run perf:serve` does the same with a temp `ANAKOT_HOME` baked in, if you
...
- **A throwaway `ANAKOT_HOME` has no backend.** The app logs `ECONNREFUSED` for
  `anakot:api` and may exit on its own. The renderer still mounts and the DOM is
