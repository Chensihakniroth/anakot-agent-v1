...
This wrapper skill teaches the Anakot `computer_use` workflow and action
...
skill installed by `cua-driver skills install`. Anakot autodetection is a
planned cua-driver follow-up, so currently point Anakot at the resulting
...
Anakot autodetection is a planned follow-up in trycua/cua. For now, the command
installs the pack under `~/.cua-driver/skills/cua-driver`; point Anakot at that
