...
| Related skills | [`claude-code`](./autonomous-ai-agents-claude-code.md), [`anakot-agent`](./autonomous-ai-agents-anakot-agent.md) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Anakot terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Anakot itself, `model.provider: openai-codex` uses Anakot-managed Codex
OAuth from `~/.anakot/auth.json` after `anakot auth add openai-codex`. For the
...
## Anakot Gateway Caveat

When invoking the Codex CLI from a Anakot gateway/service context (for example,
