...
| Related skills | [`anakot-agent`](./autonomous-ai-agents-anakot-agent.md) |
...
- Both sides' intent sources: kanban completion summaries (`terminal` running
  `anakot kanban show <task-id>`), PR bodies, or at minimum each branch's
  commit messages.
...
- Collect each side's intent: `anakot kanban show <task-id>` for completion
