...
| Related skills | [`arxiv`](./research-arxiv.md), [`pdf`](../productivity/productivity-pdf.md), [`reddit-reading`](../../optional/social-media/social-media-reddit-reading.md), [`rss-feeds`](../../optional/research/research-rss-feeds.md), [`youtube-content`](../media/media-youtube-content.md) |
...
Ledger location: `$ANAKOT_HOME/cache/citations/ledger.json` (profile-aware).
Override per task with `--ledger <path>` or `ANAKOT_CITATION_LEDGER`.

## How to Run

```bash
S=~/.anakot/skills/research/grounded-citations/scripts/sources.py
...
`anakot skills install official/social-media/reddit-reading` or
`anakot skills install official/research/rss-feeds` before using them.
...
- **Parallel subagents.** Each subagent has its own working directory; point
  them all at one ledger with `--ledger` (or `ANAKOT_CITATION_LEDGER`) if their
