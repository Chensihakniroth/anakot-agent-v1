...
| List/filter by metadata (age, source, cost, tokens, workspace) | `anakot sessions list` / `stats` via terminal |
| Rename | `anakot sessions rename <session_id> <title...>` |
| Bulk soft-hide (reversible) | `anakot sessions archive <filters>` |
| Delete (destructive) | `anakot sessions delete` / `anakot sessions prune <filters>` |
| Export before deleting anything valuable | `anakot sessions export --session-id <id> --format md` |
...
`anakot sessions list --source telegram --limit 50` instead.
...
- Before deleting anything with meaningful content, offer
  `anakot sessions export --format md` as a backup.
...
After a cleanup pass, re-run the discovery query and `anakot sessions list`
