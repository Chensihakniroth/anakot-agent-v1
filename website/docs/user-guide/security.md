...
Anakot Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Anakot checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.anakot/config.yaml`:
...
| `timeout` | `300` | Seconds Anakot waits for an approval reply before timing out. |
...
| `destructive_slash_confirm` | `true` | When true, destructive session slash commands (`/clear`, `/new`, `/reset`, `/undo`) prompt before discarding conversation state. Three-option dialog (Approve Once / Always Approve / Cancel) routed through native yes/no buttons on Telegram, Discord, and Slack; text fallback elsewhere. Users who click **Always Approve** flip this key to `false`. The TUI also honors this setting for its `/clear`, `/new`, and `/reset` modal; `ANAKOT_TUI_NO_CONFIRM=1` force-skips that modal regardless of the configured value. |
...
1. **CLI flag**: Start a session with `anakot --yolo` or `anakot chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `ANAKOT_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `ANAKOT_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Anakot shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
`ANAKOT_SUPERVISED_CHILD` marker they export.
...
the job uses KeepAlive or controls Anakot.
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Anakot refuses to run them **regardless** of:
...
Configure the timeout in `~/.anakot/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.anakot/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.anakot/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` anakot/gateway | Self-termination prevention |
...
The `ANAKOT_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.anakot/config.yaml`:
...
unattended ones: a cron job, `anakot chat -q` run or webhook session under
...
re-save it with `anakot config edit`. Other malformed values are ignored with
a warning; they never become per-character approvals. Loading does not rewrite
your configuration file.

:::tip
Use `anakot config edit` to review or remove patterns from your permanent allowlist.
:::

:::caution
The list is read when Anakot starts. A pattern you remove while a session is
already running stays approved in that session until it next writes the file
(the next time you answer `always` to a prompt) or you restart Anakot. If you
removed it for safety reasons, restart.
:::

### Mining Approval History (`anakot approvals suggest`)
...
```bash
anakot approvals suggest            # dry run — prints a numbered proposal
anakot approvals suggest --apply 1,3  # merge picks into command_allowlist
anakot approvals suggest --json     # machine-readable output
```

The command scans the session database (`~/.anakot/state.db`) for
...
Before `write_file` or `patch` touches disk, Anakot checks the target path against a denylist and an optional sandbox. Blocked writes return an error to the agent immediately — **there is no approval prompt** and no way to override from the chat UI. The model may still claim the edit succeeded; when `display.file_mutation_verifier` is on (default), trust the [file-mutation verifier footer](./configuration.md#file-mutation-verifier) over the assistant's closing summary.

### Protected paths (always blocked)

These categories are always denied, even when `ANAKOT_WRITE_SAFE_ROOT` is unset:

| Category | Examples |
|----------|----------|
| OS credential stores | `~/.ssh/` (keys, `authorized_keys`), `~/.aws/`, `~/.kube/`, `/etc/sudoers`, `~/.netrc` |
| Anakot secret stores | `.env`, `.anthropic_oauth.json`, `auth/google_oauth.json`, Bitwarden cache (`cache/bws_cache.json`, `cache/bws_cache.enc.json`), `vault/`, `browser-profile/`, `mcp-tokens/`, `pairing/` under ANAKOT_HOME (active profile and global root). Control files (`auth.json`, `config.yaml`, `webhook_subscriptions.json`) are read-denied but stay writable. |
...
Sensitive paths inside the safe root are still blocked — pointing `ANAKOT_WRITE_SAFE_ROOT` at `$HOME` does not allow writing `~/.ssh/id_rsa`.

The `~` in the OS-credential rows means *every* home a write can land in, not just the process `HOME`: the OS user's real home, the profile home (`{ANAKOT_HOME}/home` under `TERMINAL_HOME_MODE=profile`, containers and spawned workers, where the process `HOME` is pinned), and named accounts (`~root/.ssh/authorized_keys`). An absolute path to the real home's `~/.aws/credentials` is denied even when the agent process runs with `HOME` pointed elsewhere.

Safe-root violations return `Write denied: '…' is outside ANAKOT_WRITE_SAFE_ROOT (…)`. Credential-path blocks use `Write denied: '…' is a protected system/credential file.`
...
### ANAKOT_WRITE_SAFE_ROOT (optional sandbox)
...
- Set automatically in the [official Docker image](https://github.com/Chensihakniroth/anakot-agent-v1) (`ANAKOT_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.anakot/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.anakot/cron/jobs.json`, profile skills, or other Anakot state outside that prefix

To allow both a workspace and Anakot home:

```bash
export ANAKOT_WRITE_SAFE_ROOT=/path/to/project:/home/you/.anakot
```

Unset the variable to restore unrestricted writes (subject to the protected-path denylist). Full reference: [ANAKOT_WRITE_SAFE_ROOT](../reference/environment-variables.md#anakot_write_safe_root).

### Cron and other Anakot state

Do not ask the agent to `patch` `~/.anakot/cron/jobs.json` directly. Use the `cronjob` tool, [`anakot cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Anakot control files when write safety blocks direct edits.
...
When running the messaging gateway, Anakot controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.anakot/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.anakot/.env to allow open access,
...
For more flexible authorization, Anakot includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `anakot pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.anakot/config.yaml`:
...
anakot pairing approve telegram ABC12DEF

# Revoke a user's access
anakot pairing revoke telegram 123456789

# Clear all pending codes
anakot pairing clear-pending
```

:::tip Docker users: run pairing commands as the `anakot` user
The official Docker image runs the gateway as the unprivileged `anakot` user
...
Always pass `-u anakot`:

```bash
docker exec -u anakot anakot-agent anakot pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/Chensihakniroth/anakot-agent-v1/issues/10270
:::

**Storage:** Pairing data is stored in `~/.anakot/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Anakot applies strict security hardening to every container.
...
Container resources are configurable in `~/.anakot/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.anakot/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `ANAKOT_HOME`. Skills declare these in frontmatter:
...
When loaded, Anakot checks if these files exist in the active profile's `ANAKOT_HOME` and registers them for mounting:
...
With the switch off Anakot never reads or refreshes those files: the `claude_code` credential-pool row disappears, `anakot auth list` prints one line saying so, and the log carries one INFO line per process. Only automatic adoption is affected — `anakot auth add openai-codex` still asks before importing an existing Codex CLI login. Add your own logins with `anakot auth add anthropic` / `anakot auth add openai-codex`.
...
| **terminal** (local) | Blocks explicit Anakot infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Anakot infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Anakot integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Anakot under WSL.
...
Your own `SOUL.md` in `ANAKOT_HOME` is treated differently: it is a file you wrote (file-tool writes to it
need your approval, and project checkouts never supply it), so a scanner hit there **does not block the
file**. Anakot logs a warning naming the matched pattern, loads the file as usual, and `/context` lists it as
...
the file. The exception does not extend to a `SOUL.md` shipped by a profile distribution: `anakot profile
install <git-url>` and `anakot profile update` copy a third party's `SOUL.md` into the profile home without
...
4. **Store secrets securely** — keep API keys in `~/.anakot/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.anakot/logs/` for unauthorized access attempts
10. **Keep updated** — run `anakot update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.anakot/.env`:
...
TERMINAL_SSH_USER=anakot
TERMINAL_SSH_KEY=~/.ssh/anakot_agent_key
```
...
Anakot ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `anakot_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `anakot doctor` for the full remediation.
- **`anakot doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Anakot installs these **lazily** on first use rather than eagerly under `anakot-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `anakot tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `anakot tools`.
