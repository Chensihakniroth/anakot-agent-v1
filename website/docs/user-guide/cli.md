...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Anakot Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Anakot Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `anakot setup --portal` — and you're ready to `anakot chat`. See [Nous Portal](../integrations/nous-portal.md).
:::
...
anakot chat --query-file prompt.txt
anakot chat --query-file - < prompt.txt
...
anakot chat --provider nous        # Use Nous Portal
anakot chat --provider openrouter  # Force OpenRouter

# With specific toolsets
anakot chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
anakot -s anakot-agent-dev,github-auth
anakot chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
anakot --continue             # Resume the most recent CLI session (-c)
anakot --resume <session_id>  # Resume a specific session by ID (-r)
anakot --resume latest        # Resume the most recent session (same as -c)
anakot --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
anakot chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
anakot -w                         # Interactive mode in worktree
anakot -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`anakot -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
```bash
anakot worktree list              # audit: age, size, verdict, reason per tree
anakot worktree list --json       # machine-readable audit (trees, external trees, branches)
anakot worktree prune             # remove safe trees + delete merged branches
anakot worktree prune --dry-run   # show the plan without changing anything
anakot worktree prune --older-than 7   # only reap trees idle for 7+ days
anakot worktree prune --trees-only     # leave local branches alone
anakot worktree prune --branches-only  # leave worktrees alone
```
...
- Trees **in use by a running anakot session** are never touched.
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.anakot/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `anakot -w` for days — no longer accumulate merged scratch trees
...
The `anakot plugins` commands manage native Anakot plugins and portable Agent
...
anakot plugins disable <plugin-name>
anakot plugins update <plugin-name>
anakot plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Anakot currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Anakot CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Anakot CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `ANAKOT_YOLO_MODE` is on (either `anakot --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Anakot to background (Unix only). Run `fg` in the shell to resume. |
...
- **Runs on your machine, in the session working directory.** With the default local terminal backend `!pwd` matches what the agent would see. A remote or sandboxed `terminal.backend` (`ssh`, `docker`, …) is **not** used for `!` commands — they always run on the host where Anakot itself runs, so `!hostname` names your machine while the agent's `terminal` tool names the backend. Ask the agent (or open a shell on the target) to run something *inside* the backend. Path completion in the composer, by contrast, does follow the configured backend and lists the target's filesystem.
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.anakot/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Anakot supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Anakot replies |
...
```bash
anakot -s anakot-agent-dev,github-auth
anakot chat -s github-pr-workflow -s github-auth
```

Anakot loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.
...
`--ignore-rules` (equivalently `ANAKOT_IGNORE_RULES=1`) skips auto-load together with AGENTS.md, SOUL.md, `.cursorrules` and memory injection; explicit `-s` skills still load. The setting is profile-scoped: each profile's `config.yaml` controls its own list.

## Skill Slash Commands

Every installed skill in `~/.anakot/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.anakot/config.yaml`:
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Anakot also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Anakot recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Anakot — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Anakot is working, Anakot prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Anakot to the background — just like any terminal process. The shell prints a confirmation:

```
Anakot Agent has been suspended. Run `fg` to bring Anakot Agent back.
```
...
Resume this session with:
  anakot --resume 20260225_143052_a1b2c3
...
anakot -c                                  # Short form
anakot -c "my project"                     # Resume a named session (latest in lineage)
anakot --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
anakot --resume "refactoring auth"         # Resume by title
anakot --resume latest                     # Resume the most recent session (same as -c)
anakot --resume latest --in ./my-project   # Latest session for ./my-project's workspace
anakot -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `anakot sessions rename <id> <title>` from the command line. Use `anakot sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Anakot's SQLite state database under `~/.anakot/state.db`. The database keeps:
...
Anakot immediately confirms the task and gives you back the prompt:
