...
If you only run one anakot agent, you don't need this page — see
...
[Connecting Desktop to Many Anakot Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more anakot agents that should all
...
(`ai.anakot.gateway-<name>.plist`), a systemd user service
(`anakot-gateway-<name>.service`), a systemd **system** service when installed with
`sudo anakot gateway install --system` (runs as the invoking user via `User=`), a
Windows Scheduled Task, or an s6/Docker service — and the Desktop app spawns its own
per-profile `anakot serve` backend. This guide adds the patterns for managing them
collectively.
...
anakot profile create coder
anakot profile create personal-bot
anakot profile create research
...
[`anakot gateway migrate --multiplex`](#migrating-from-per-profile-gateways) and
...
`anakot gateway migrate --multiplex` one-liner. Nothing is changed on disk.
...
Other processes (`anakot -p <name> gateway start`, the dashboard, `anakot gateway
...
```bash
anakot config set gateway.multiplex_profiles true    # or false
anakot gateway restart
```

Equivalently, in the default profile's `~/.anakot/config.yaml`:
...
You do **not** run `anakot gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `anakot gateway run`, `start`,
...
loop. `anakot -p coder gateway stop` refuses the same way (exit 78) when coder has no
gateway of its own — there is nothing to stop but the multiplexer, which
`anakot gateway stop` on the default profile takes down for every served profile.
...
The setup flows follow the same rule: `anakot -p coder setup gateway`, `anakot -p coder setup`,
`anakot -p coder gateway setup` and `anakot -p coder import` configure the profile's bots but
...
- **WhatsApp (bridge) and Relay are shared ingress owned by the default profile.**
  ...
  is not being served; `anakot gateway status --profile work` shows
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.anakot/profiles/coder/.env`; the default listener key is rejected. Under
...
- Webhook routes without `profile` remain default-profile routes and are not
  ...
  way: `anakot webhook subscribe <name> --route-profile coder` writes
  `profile: coder` into the default gateway's `webhook_subscriptions.json` and
  prints the `/p/coder/webhooks/<name>` URL (`anakot webhook ls` shows the
...
A standalone `anakot -p coder gateway run` binds coder's Twilio, LINE, Teams,
...
```
$ anakot -p coder gateway status
✓ Gateway is running via the default-profile multiplexer
  Manage it from the default profile: anakot gateway status
...
`anakot gateway status` and `anakot status` on the default profile list the same
...
There is a single process-level PID and lock (the multiplexer, under the default home). `anakot status` on the default profile reports the multiplexer and lists the profiles it serves (`Serves: coder, research`). `anakot -p coder status` and `anakot -p coder gateway status` report "running via the default-profile multiplexer" instead of "stopped". The dashboard's `/api/status?profile=coder` / Channels page report the multiplexer as coder's running gateway, with coder's own adapters as its platforms. The single `gateway_state.json` lives under the default home: secondary adapters appear there as `<profile>:<platform>` entries beside `served_profiles`; no per-profile gateway status file is written.

`anakot -p coder cron status` prints `Scheduler host: default-profile multiplexer`, then checks coder's own ticker heartbeat and last successful tick. A missing or stale heartbeat produces a warning rather than an unconditional running verdict; the restart hint targets `anakot --profile default gateway restart`. `cron list` and `cron create` also warn when a served profile has no fresh heartbeat. `cron status` adds tick-failure details that those lightweight checks do not read.
...
`HINDSIGHT_BANK_ID`, `ANAKOT_HONCHO_HOST`) are read from the routed profile's
...
`ANAKOT_LANGUAGE` and `display.language`, and `hooks.outbound[].secret_env` are
...
`ANAKOT_HOME`, without the default profile's secrets in their environment, and
...
| Provider logins and the credential pool (`auth.json`: OAuth tokens, `anakot auth add` keys) | The profile's own `auth.json` | "Not connected to any AI provider" with the `anakot -p <name> model` hint — never the default profile's login, and a refresh never writes to the root store |
...
| Dashboard actions (`anakot -p <name> …` spawned by the Desktop/dashboard) | A scrubbed child env pinned to that profile's `ANAKOT_HOME` | The child loads its own `.env`; the dashboard profile's tokens and ports are not inherited |
...
archived or deleted instead — `anakot profile delete <name>`, or move the
...
multiplexer started as `anakot -p <name> gateway run` always ticks its own
profile's cron store as well.

The served set is **live**. A profile created while the multiplexer is running
(`anakot profile create`, the dashboard, Desktop or the TUI) is served at once:
...
default profile's `gateway_state.json` is updated, and `anakot -p <name> gateway
...
adapters the same way, and `anakot profile rename` unroutes the old name before
...
`~/.local/bin/anakot-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: anakot-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    anakot gateway "$action"
  else
    anakot -p "$profile" gateway "$action"
  fi
}
...
```bash
anakot-gateways start      # start every configured profile
anakot-gateways stop       # stop every configured profile
anakot-gateways restart    # restart all
anakot-gateways status     # status across all
anakot-gateways list       # delegates to `anakot gateway list`
```

:::tip
The `default` profile is targeted with `anakot gateway <action>` (no `-p`),
not `anakot -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `anakot -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.anakot.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/anakot-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.anakot.gateway.plist` /
`anakot-gateway.service`.
...
```bash
# Default profile
tail -f ~/.anakot/logs/gateway.log
tail -f ~/.anakot/logs/gateway.error.log

# Named profile
tail -f ~/.anakot/profiles/<name>/logs/gateway.log
tail -f ~/.anakot/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.anakot/logs/gateway.log ~/.anakot/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
anakot logs -f                  # follow default profile
anakot -p coder logs -f         # follow one profile
anakot logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
anakot profile list             # profiles + model + gateway state
anakot-gateways status          # full status across every profile
launchctl list | grep anakot    # macOS — PIDs and labels
systemctl --user list-units 'anakot-gateway-*'   # Linux — units
```
...
```
~/.anakot/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.anakot/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
anakot config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.anakot/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=anakot --why="gateways running" \
  sleep infinity &
...
`anakot-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.anakot/.env ~/.anakot/profiles/*/.env
```
...
```bash
anakot gateway migrate --multiplex --dry-run   # print the plan and any blockers; changes nothing
anakot gateway migrate --multiplex             # apply (asks for confirmation on a TTY; -y skips)
anakot gateway migrate --standalone            # roll back to per-profile gateways
```

### What `anakot update` does
...
service) and `gateway.multiplex_profiles` is off, `anakot update` runs the same
preflight:

- **Nothing blocks it** → the migration runs automatically (the same code path
  as `anakot gateway migrate --multiplex --yes`) and prints what it did. This
...
install that is already multiplexing is left alone. `anakot update` also does
nothing when no secondary profile runs its own gateway — it never flips modes
on an install where nothing was running.

### Boundaries `anakot update` never crosses on its own

The unattended hook only folds profiles that share **one UNIX user, one service
domain and one `profiles/` tree** — the shape `anakot profile create` produces.
...
| `ANAKOT_HOME` outside `<default home>/profiles/` | a unit pinning `ANAKOT_HOME=/opt/anakot/profiles/emma` |

In that case `anakot update` prints the boundary it found plus
`anakot gateway migrate --multiplex`, and changes nothing — no unit is removed
...
findings appear as **notices** in `anakot gateway migrate --multiplex --dry-run`
...
```bash
anakot config set gateway.auto_multiplex_migration false
```

`anakot update` then leaves per-profile gateways exactly as they are, with no
...
other setting, so a value pinned in the managed scope (`/etc/anakot/config.yaml`)
wins over the profile's own file. It governs the **automatic** path only:
`anakot gateway migrate --multiplex` is an explicit request and still migrates
(and is the supported way to opt back in). Absent or `true` keeps the default
behaviour described above.

The explicit command is different: `anakot gateway migrate --multiplex` with
...
`anakot profile create --clone` leaves the source's bot tokens and allowlists
...
Older clones that still carry them are flagged by `anakot profile list`.
:::
...
1. Stops each secondary profile's standalone gateway and uninstalls its
   ...
   recorded in `~/.anakot/gateway_migration.json` for rollback.
...
above). `anakot profile create` confirms this when the live multiplexer picked the
profile up; it prints the `anakot gateway restart` reminder only when it could not
reach the multiplexer (for example, a gateway started from an older build).

### Rollback

```bash
anakot gateway migrate --standalone
```
...
window, the next `anakot gateway migrate --multiplex` sees the flag on, the
...
with `anakot config set gateway.multiplex_profiles false && anakot gateway restart`
...
flag, stop the per-profile tasks, `anakot gateway restart`). The dashboard's
System page offers the same migration as a button when the preflight finds an
eligible install.

## Updating the code

`anakot update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `anakot gateway start` after a previous `anakot gateway stop`. The
...
```bash
ps -ef | grep "anakot_cli.*-p <profile>"
cat ~/.anakot/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.anakot.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.anakot.gateway-<profile>.plist

# Linux
systemctl --user restart anakot-gateway-<profile>.service
```

### Health check

```bash
anakot doctor                  # default profile
anakot -p <profile> doctor     # one profile
```
