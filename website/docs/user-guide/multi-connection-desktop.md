...
# Connecting Desktop to Many Anakot Instances

Register every Anakot backend you own — the local runtime, remote gateways on
your LAN or VPS, SSH hosts, and Anakot Cloud instances — in one desktop app,
...
- **The sidebar profile rail** — the plug button at the right end of the rail
  (tooltip: **"Connect another Anakot gateway…"**) deep-links straight to
...
list of Anakot gateways. Its intro says it plainly: *"Manage this device and
every Anakot gateway it can reach through remote, SSH, or Cloud connections."*
Each entry is a *connection*:

| Kind | What it is | Auth |
|---|---|---|
| **Local** | "The Anakot runtime managed by this app." | automatic |
| **Remote gateway** | "A Anakot gateway reachable over HTTP(S) — LAN, Tailscale, or the internet." | session token or OAuth |
| **SSH** | "A Anakot install reached over SSH." The app opens the tunnel and starts the dashboard for you | SSH key + adopted token |
| **Anakot Cloud** | "A hosted instance discovered through your Anakot Cloud account." | portal sign-in |
...
- **Duplicates are rejected when you save**: there is only ever one **local**
  ...
  **SSH** entries are deduplicated on the normalized `user@host:port` plus
  remote profile.
- Cloud entries normally come from the Anakot Cloud sign-in/discovery flow at
  the top of the Gateways page — the **Anakot Cloud** kind in the add-connection
...
The Anakot Cloud panel also lists **Saved Cloud gateways** when portal discovery
...
3. Pick the kind: **Local**, **Anakot Cloud**, **Remote gateway**, or **SSH**.
   (**Local** is disabled while the app-managed local entry exists — which is
   almost always; **Anakot Cloud** directs you to the cloud sign-in/discovery
   flow above.)
4. Fill the fields:
   - **Name** — required, unique; the "device name" shown everywhere this
     instance appears (placeholder: `Homelab`). Max 64 characters.
   - *Remote gateway only:*
     - **Gateway URL** — the base URL of a running `anakot serve` backend,
   ...
   - *SSH only:*
     ...
     - **Anakot path (optional)** — full path to the `anakot` executable on
       the remote (for example `/opt/anakot/bin/anakot`). Leave blank to
       auto-detect. Set it when the remote's non-interactive shell does not
       have `anakot` on its `PATH` and **Test** reports *"Anakot is not
...
Gateways are machines or hosted backends; profiles are isolated anakot agents
...
connection is registered) dispatches `anakot update` to every eligible
...
- **Anakot Cloud** instances are skipped with a *"Managed by Anakot Cloud"*
...
panel, ⌘K **Update Anakot**, the update-ready toast) run the same fan-out
...
- **"Connection test failed"** — the backend isn't reachable at that URL from
  this machine. Check that `anakot serve` is running on the remote host, the
...
- **"Update Anakot Desktop to chat with agents on other connections"** — the
