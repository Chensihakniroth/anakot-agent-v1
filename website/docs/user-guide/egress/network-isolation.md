...
When running Anakot inside Docker, the default `network_mode: host` gives the
...
The Anakot [SECURITY.md](https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/SECURITY.md) §2 defines the trust model. The
...
#
# Usage:
#   ANAKOT_UID=$(id -u) ANAKOT_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=anakot,anakot-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://anakot-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"
