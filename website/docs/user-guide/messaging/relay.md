...
sidebar_position: 30
title: "Anakot Relay"
description: "Connect Anakot to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Anakot Relay (Connector)
...
Anakot Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.anakot/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.anakot/.env`:
...
`anakot gateway enroll` refuses to run in managed/hosted installs — there the
...
`anakot gateway enroll` remains available while runtime relay is disabled.

| Setting | Where | Meaning |
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.anakot/.env`) | Connector relay WebSocket URL. Enables relay unless explicitly disabled in platform configuration. |
...
token. Re-login with `anakot auth add nous` (or `anakot setup`) and retry.
...
startup. Restart it (`anakot gateway restart`).
