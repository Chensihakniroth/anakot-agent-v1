...
description: "Chat with Anakot from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---
...
For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](../features/voice-mode.md) and [Use Voice Mode with Anakot](../../guides/use-voice-mode-with-anakot.md).
...
saved by `anakot gateway setup` can enable a credential-based platform without a
...
```mermaid
flowchart TB
    subgraph Gateway["Anakot Gateway"]
...
For group chats, hooks, and automation flows, Anakot supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Anakot keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Anakot does not hide failures just because the text resembles a silence token.
...
sudo anakot gateway install --system   # Linux only: install a boot-time system service
...
anakot gateway status --system         # Linux only: inspect the system service explicitly
```

### Stack dump on demand (`SIGUSR2`)

On Linux and macOS, `kill -USR2 <gateway pid>` appends a dump of every thread's
stack to `~/.anakot/logs/gateway_faulthandler.log` and the gateway keeps
...
supervisor restarts the process. `anakot gateway status` renders that record as
...
stamp is more than 120 s old, `anakot gateway status` prints
...
```bash
anakot gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Anakot sends heartbeats
...
| `/update` | Update Anakot Agent to the latest version |
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.anakot/config.yaml`:
...
The first time you message a busy agent on any platform, Anakot appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.anakot/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.anakot/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.

### Configurable status phrases

Long-running gateway status lines ("still working…"-style heartbeats) draw from a phrase catalog. Built-in defaults ship in `gateway/assets/status_phrases.yaml`; you can add your own with profile-portable files under `ANAKOT_HOME`:

- `~/.anakot/status_phrases.yaml` or any `*.yaml` in `~/.anakot/status_phrases/` (conventional paths, auto-loaded), or
- point config at a relative path:

```yaml
display:
  status_phrases:
    path: status_phrases/whatsapp.yaml  # relative to ANAKOT_HOME
...
Off by default. When enabled, Anakot prepends a human-readable timestamp
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.anakot/config.yaml`:
...
```bash
ANAKOT_BACKGROUND_NOTIFICATIONS=result
```
...
anakot gateway start                 # Start the service
anakot gateway stop                  # Stop the service
anakot gateway status                # Check status
journalctl --user -u anakot-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo anakot gateway install --system
sudo anakot gateway start --system
sudo anakot gateway status --system
journalctl -u anakot-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Anakot installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit anakot-gateway` (or `sudo systemctl edit anakot-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `anakot update`. When `anakot update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart anakot-gateway` command (it never blocks on an interactive password prompt).
...
```bash
anakot gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `anakot update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo anakot update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/anakot-gateway`:

```
anakot ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed anakot-gateway*, /usr/bin/systemctl --no-ask-password start anakot-gateway*, /usr/bin/systemctl --no-ask-password restart anakot-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Anakot will warn if it detects both because start/stop/status behavior gets ambiguous.

:::note Inside a container, only the system scope is offered
`anakot gateway install` (and the `anakot gateway setup` wizard) refuse to install a **user** service when Anakot detects it is running inside a container. A user unit lands in `~/.config/systemd/user`, and when that home is bind-mounted from the host (podman/distrobox), the host's own `systemd --user` enables and starts the same unit — a second gateway polling the same bot token. Run the gateway as the container's main process (`anakot gateway run`, with a container restart policy), or in a systemd container (systemd as PID 1) install the isolated system scope: `sudo anakot gateway install --system --run-as-user <user>`.
:::

:::info Multiple installations
If you run multiple Anakot installations on the same machine (with different `ANAKOT_HOME` directories), each gets its own systemd service name. The default `~/.anakot` uses `anakot-gateway`; other installations use `anakot-gateway-<hash>`. The `anakot gateway` commands automatically target the correct service for your current `ANAKOT_HOME`.
:::

### macOS (launchd)

```bash
anakot gateway install               # Install as launchd agent
anakot gateway start                 # Start the service
anakot gateway stop                  # Stop the service
anakot gateway status                # Check status
tail -f ~/.anakot/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.anakot.gateway.plist`. It includes three environment variables:
...
- **ANAKOT_HOME** — scopes the gateway to your Anakot installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `anakot gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `ANAKOT_HOME` directory gets its own launchd label. The default `~/.anakot` uses `ai.anakot.gateway`; other installations use `ai.anakot.gateway-<suffix>`.
:::

### Windows (Task Scheduler)

```powershell
anakot gateway install               # Register the Anakot_Gateway Scheduled Task (runs at logon)
anakot gateway start                 # Start the gateway hidden, without a console window
anakot gateway stop                  # Drain and stop the service
anakot gateway status                # Check status, including registration drift
```

The Scheduled Task runs `wscript.exe` on a generated `.vbs` launcher under `%USERPROFILE%.anakot\gateway-service\`. The launcher starts `python.exe -m anakot_cli.main gateway run` with a hidden window and **exits immediately** — by design: `wscript.exe` has no console, so at logon it never receives the `CTRL_CLOSE_EVENT` that kills a `cmd.exe`-hosted gateway, and the gateway inherits one hidden console instead of every subprocess flashing its own (see `anakot_cli/gateway_windows.py::_build_gateway_vbs_script`).

:::warning RestartOnFailure covers the launcher, not the gateway
Because the launcher returns as soon as the gateway is spawned, Task Scheduler only ever sees the launcher's exit code. The `<RestartOnFailure>` policy in the registered task therefore fires only when `wscript.exe` itself fails to start the gateway — it does **not** restart a gateway that crashes or is killed later. Gateway auto-restart on Windows relies on the gateway's own in-process restart path (`/restart`, updates, and the `anakot gateway restart` command); a gateway killed from outside stays down until `anakot gateway start` or `schtasks /Run /TN <task>`.
:::

`anakot gateway install` writes the task from the current template; a task registered by an older build would otherwise keep its old settings (no `RestartOnFailure`, no logon `Delay`, an older launcher command line) indefinitely. `anakot gateway status` compares the registered task with the current template and warns when it predates it:

```
⚠ Scheduled Task registration predates the current template (missing: RestartOnFailure, LogonTrigger Delay; version 1.3 vs 1.4)
  Repair: anakot gateway start  (or: anakot gateway install)
```

`anakot gateway start` and `anakot update` run the same comparison and re-register a drifted task from the current template automatically (like the systemd unit refresh on Linux); when `schtasks` refuses without elevation, re-run `anakot gateway install`, which can request administrator approval. The check is silent when the task cannot be queried, and it only inspects a few settings Anakot owns (task version, `RestartOnFailure`, the logon trigger delay and the launcher arguments), so deliberate local edits elsewhere in the task are not flagged.
...
|----------|---------|--------------|
| CLI | `anakot-cli` | Full access |
| Telegram | `anakot-telegram` | Full tools including terminal |
| Discord | `anakot-discord` | Full tools including terminal |
| WhatsApp | `anakot-whatsapp` | Full tools including terminal |
| WhatsApp Cloud API | `anakot-whatsapp` | Full tools including terminal (shares toolset with the Baileys bridge) |
| Slack | `anakot-slack` | Full tools including terminal |
| Google Chat | `anakot-google_chat` | Full tools including terminal |
| Signal | `anakot-signal` | Full tools including terminal |
| SMS | `anakot-sms` | Full tools including terminal |
| Email | `anakot-email` | Full tools including terminal |
| Home Assistant | `anakot-homeassistant` | Full tools + HA device control (ha_list_entities, ha_get_state, ha_call_service, ha_list_services) |
| Mattermost | `anakot-mattermost` | Full tools including terminal |
| Matrix | `anakot-matrix` | Full tools including terminal |
| DingTalk | `anakot-dingtalk` | Full tools including terminal |
| Feishu/Lark | `anakot-feishu` | Full tools including terminal |
| WeCom | `anakot-wecom` | Full tools including terminal |
| WeCom Callback | `anakot-wecom-callback` | Full tools including terminal |
| Weixin | `anakot-weixin` | Full tools including terminal |
| BlueBubbles | `anakot-bluebubbles` | Full tools including terminal |
| QQBot | `anakot-qqbot` | Full tools including terminal |
| Yuanbao | `anakot-yuanbao` | Full tools including terminal |
| Microsoft Teams | `anakot-teams` | Full tools including terminal |
| API Server | `anakot-api-server` | Full tools (drops `clarify`, `text_to_speech` — programmatic access doesn't have an interactive user) |
| Webhooks | `anakot-webhook` | Full tools including terminal |
| Raft | `anakot-raft` | Wake-only channel; agent uses Raft CLI for message I/O |
...
`platforms.<name>.enabled: false` in `~/.anakot/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.anakot/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
