...
description: "Set up Anakot Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Anakot Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `anakot gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Anakot strips automatically before processing.
...
```bash
# devtunnel (Microsoft)
devtunnel create anakot-bot --allow-anonymous
devtunnel port create anakot-bot -p 3978 --protocol http  # replace 3978 with TEAMS_PORT if changed
devtunnel host anakot-bot
...
The public tunnel URL uses HTTPS, but Anakot' local webhook listener uses plain HTTP. The tunnel terminates TLS and forwards HTTP to port `3978`; do not configure the local tunnel port as HTTPS.
...
**Docker** (must run from the directory that contains `docker-compose.yml` — usually your cloned `anakot-agent` repo, not `~`):

```bash
cd /path/to/anakot-agent
ANAKOT_UID=$(id -u) ANAKOT_GID=$(id -g) docker compose up -d gateway
```

**Native / systemd install** (typical `anakot` one-liner installer under `~/.anakot/anakot-agent`):

```bash
anakot gateway restart
# or foreground: anakot gateway run
```

The Teams SDK is optional; when Teams is enabled, the gateway lazy-installs it into Anakot' own venv on first start (do **not** use system `pip install` on Ubuntu 24.04 — that hits PEP 668 `externally-managed-environment`). To install manually into the Anakot venv:

```bash
~/.anakot/anakot-agent/venv/bin/pip install microsoft-teams-apps aiohttp
...
# Docker:
docker logs -f anakot
# Native:
anakot gateway status -l
```
...
Alternatively, configure via `~/.anakot/config.yaml`:
...
When the [Teams meeting pipeline plugin](./msgraph-webhook.md) is enabled, this adapter also handles outbound delivery of meeting summaries — one Teams integration surface, not two. After a meeting's transcript is summarized, the writer posts the summary into your chosen Teams target.
...
| `graph` | Threaded channel posts or 1:1/group chat posts under the bot's identity via Microsoft Graph. | Requires the [Graph app registration](../../guides/microsoft-graph-app-registration.md) with `ChannelMessage.Send` (channel) or `Chat.ReadWrite.All` (chat) application permissions. |
...
For a permanent server, terminate TLS at a reverse proxy and forward requests to the plain HTTP Anakot listener, normally `http://127.0.0.1:3978`. Register the proxy's public HTTPS endpoint with Teams:
...
Make sure the public HTTPS endpoint is reachable from the internet and uses a valid TLS certificate. Teams rejects self-signed certificates. Keep the Anakot listener behind the proxy; port `3978` does not serve HTTPS itself.
...
| Problem | Solution |
|---------|----------|
| `Can't find a suitable configuration file` from `docker compose` | You are not in the repo that has `docker-compose.yml`, or you are on a native install — use `anakot gateway restart` instead, or `cd` into the clone first |
| `requirements not met` / `Teams SDK missing` / `No adapter available for teams` | Restart gateway so lazy-install can run, or install into the **Anakot venv**: `~/.anakot/anakot-agent/venv/bin/pip install microsoft-teams-apps aiohttp`. System `pip` fails on Ubuntu 24.04 (PEP 668) and would not affect the service anyway |
...
| Logs show `"UNKNOWN / HTTP/1.0" 400` when Teams sends a message | The tunnel or reverse proxy is forwarding HTTPS to Anakot' plain HTTP listener. Terminate TLS at the proxy and forward HTTP to port `3978` |
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.anakot/.env` |
...
| Tunnel URL changes on restart | devtunnel URLs are persistent if you use a named tunnel (`devtunnel create anakot-bot`). ngrok and cloudflared generate a new URL each run unless you have a paid plan — update the bot endpoint with `teams app update` when it changes |
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs anakot` / `anakot gateway status -l` for tracebacks |
...
- Store credentials in `~/.anakot/.env` with permissions `600` (`chmod 600 ~/.anakot/.env`)
