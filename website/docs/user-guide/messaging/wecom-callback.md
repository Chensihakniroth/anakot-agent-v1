...
Connect Anakot to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.
...
> Run `anakot gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Anakot decrypts the message, queues it for the agent
...
(Use `anakot gateway start` only after `anakot gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Anakot and the token the
...
in `~/.anakot/.env` values around `=` will also break signature checks. After
fixing, restart `anakot gateway run`.
...
Check `anakot gateway run` logs for the bound host/port. If the adapter bound to
