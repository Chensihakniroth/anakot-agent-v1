...
Connect Anakot to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Anakot — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Anakot runs it in a
...
anakot photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Anakot Agent` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.anakot/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.anakot/.env`
...
(device token, dashboard project id) lives in `~/.anakot/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Anakot
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Anakot replies with a pairing code. Approve it with:

```bash
anakot pairing approve photon <CODE>
```

Use `anakot pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.anakot/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.anakot/.env`):
...
By default Anakot responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Anakot` and
`@anakot agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Anakot will reply.

## Status & troubleshooting

```bash
anakot photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Anakot uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run anakot photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `anakot photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `anakot photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `anakot photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Anakot sends images, voice
  ...
  media.
- **Native polls are supported.** Anakot sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Anakot, so the sender sees `Read` without
  waiting for a model/tool turn. Inbound receipts for Anakot-sent messages
...
- **Message effects are supported.** Anakot sends text with native iMessage
...
- **Cron and standalone sends need the gateway running.** Out-of-process
  senders (cron jobs, `anakot send`, the dashboard) reuse the sidecar the
  gateway spawned — they read its port/token from
  `<anakot-home>/runtime/photon-sidecar.json`, written once the sidecar
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Anakot is set
...
| `PHOTON_MENTION_PATTERNS` | Anakot wake words  | JSON list / comma / newline regex patterns for group mentions |
