...
sidebar_position: 12
title: "Google Chat"
description: "Set up Anakot Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Anakot Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Anakot
...
> Run `anakot gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Anakot can read (e.g.,
`~/.anakot/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a anakot restart)
...
- Principal: `anakot-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Anakot calls
...
- **App name**: whatever you want users to see ("Anakot" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/anakot-chat-events`.
...
`ADDED_TO_SPACE` event that Anakot uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Anakot

Add the Google Chat section to `~/.anakot/.env`:
...
On Docker / hosted images `/opt/anakot/.venv` is read-only. That installer
routes through `tools.lazy_deps` into `ANAKOT_LAZY_INSTALL_TARGET`
...
Send "hola" in the test DM. The bot posts a "Anakot is thinking…" marker, then
...
`~/.anakot/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Anakot is thinking…").
...
Thread support: when a user replies inside a thread, Anakot detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Anakot session.
...
3. Download the JSON. Move it onto the host that runs Anakot.
4. Register the client with Anakot (run under the profile you want it scoped to):
...
anakot -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Anakot home (e.g.
`~/.anakot/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.anakot/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.anakot/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Anakot isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `anakot gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Anakot will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.anakot/google_chat_user_tokens/<sanitized_email>.json` (filesystem
