...
title: "Git Worktrees"
description: "Run multiple anakot agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Anakot Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Anakot so each session has a clean, isolated working directory.

## Why Use Worktrees with Anakot?

Anakot treats the **current working directory** as the project root:

- CLI: the directory where you run `anakot` or `anakot chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.anakot/config.yaml`
...
Anakot creates `.worktrees/my-experiment/` inside the repo (branch
`anakot/my-experiment`, based on the freshly-fetched remote tip unless
`worktree_sync: false`), and retargets the session's terminal and file tools
into it — no restart needed. Omit the name to get a random `anakot-<id>`
...
git worktree add ../repo-feature feature/anakot-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/anakot-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Anakot there:

```bash
cd ../repo-feature

# Start Anakot in the worktree
anakot
```
...
git worktree add ../repo-experiment-a feature/anakot-a
git worktree add ../repo-experiment-b feature/anakot-b
```
...
- Works on its own branch (`feature/anakot-a` vs `feature/anakot-b`).
...
- Anakot checkpoint data under `~/.anakot/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Anakot experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/anakot-checkpoints-docs`, `feature/anakot-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Anakot from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `anakot -w` (Automatic Worktree Mode)

Anakot has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `anakot/anakot-<hash>`).
...
```bash
anakot -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `anakot -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Anakot session its own clean checkout.
