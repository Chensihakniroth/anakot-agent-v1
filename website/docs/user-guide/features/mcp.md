...
description: "Connect Anakot Agent to external tool servers via MCP — and control exactly which MCP tools Anakot loads"
---

# MCP (Model Context Protocol)

MCP lets Anakot Agent connect to external tool servers so the agent can use tools that live outside Anakot itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Anakot to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Anakot' `config.yaml` — and `anakot import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Anakot tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Anakot to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.anakot/config.yaml`:
...
4. Ask Anakot to use the MCP-backed capability.
...
Anakot will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Anakot ships a curated catalog of MCP servers that Nous staff has reviewed
...
```bash
anakot mcp                   # interactive picker (default)
anakot mcp catalog           # plain-text list, scriptable
anakot mcp install deepwiki  # install a catalog entry by name
```
...
`optional-mcps/` in the anakot-agent repo — presence in that directory means
...
- **API key** — Anakot prompts at install time and writes the value to
  `~/.anakot/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Anakot points you at
  `anakot auth <provider>` if you haven't authenticated already.
...
3. Run `anakot mcp install n8n-official` and enter that URL when prompted.
4. Complete browser OAuth. If needed, run `anakot mcp login n8n-official`
   or use **Authorize** on the configured server in Desktop or the dashboard.
5. Review tools with `anakot mcp configure n8n-official`, then start a new
   session or use `/reload-mcp`.

The Anakot backend must be able to reach the URL. n8n controls permissions
...
After credentials are configured, Anakot probes the MCP server to list every
...
written (if not). Re-run `anakot mcp configure <name>` once the server is
reachable to refine.
...
the anakot-agent repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/Chensihakniroth/anakot-agent-v1/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Anakot
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Anakot`) for that entry instead of silently hiding it. Run `anakot update`
to install the latest Anakot when you see that.
...
from environment variables (which include everything in `~/.anakot/.env`).
...
to register, then run `anakot mcp login <name>` and restart (or
...
pre-registered with a pinned `redirect_port`, Anakot keeps the registered
...
same machine as the Anakot process. For a remote host, use `anakot mcp login`
over SSH port-forwarding.

### Updating tool selection later

```bash
anakot mcp configure linear
```
...
MCPs are never auto-updated. Re-run `anakot mcp install <name>` to refresh
after a Anakot update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/Chensihakniroth/anakot-agent-v1/tree/main/optional-mcps).
...
rejected), and Anakot's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Anakot connects to directly.
...
- you do not want Anakot spawning a local subprocess for that integration
...
Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Anakot handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Anakot identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Anakot Agent"` 403s, while `"Claude Code"` and `"Codex"` succeed. Anakot auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
Or: `anakot mcp install figma`, then `anakot mcp login figma`.
:::
...
On first connect, Anakot prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.anakot/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.
...
The redirect back from the authorization server is checked against RFC 9207: when the server's metadata advertises `authorization_response_iss_parameter_supported`, a redirect without a matching `iss` is rejected. Figma's authorization server (`https://api.figma.com`) advertises that support and then omits `iss`; Anakot fills the missing value from the discovered issuer for that one issuer and logs a warning, so `anakot mcp login figma` completes. A present-but-different `iss` is still rejected, and no other server gets the exemption.

**Remote / headless hosts.** When Anakot runs on a different machine than your browser, the loopback callback can't reach your laptop. Ways to complete the flow:

- **Anakot Desktop (automatic):** when you run the OAuth sign-in from the Desktop app's MCP setup UI against a remote backend, Desktop hosts the callback listener on *your* machine and relays the authorization back to the gateway automatically — no tunnel, paste, or proxy needed. Requires both the Desktop app and the backend to be up to date.
- **Paste-back (no setup):** on an interactive terminal Anakot prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **Device-code login (no callback at all):** if the server's authorization server advertises a device authorization endpoint, run `anakot mcp login <server> --flow device` on the machine running Anakot. It prints a verification URL and a short code; open the URL on any device, enter the code, and Anakot polls for approval. No browser is launched on the host and no callback listener is needed. Set `oauth.flow: device` on the server to make `login` and `reauth` use it by default. Details: [Device-code login](../../reference/mcp-config-reference.md#device-code-login-rfc-8628).
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Anakot on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Anakot expects them.
...
See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `anakot mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `anakot mcp login` can list the tools and look like it worked, but every real tool call later times out. `anakot mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `anakot mcp login googledrive` — with the pre-registered client, Anakot skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.anakot/config.yaml` from inside a running Anakot session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `anakot mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Anakot passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Anakot profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Anakot reads MCP config from `~/.anakot/config.yaml` under `mcp_servers`.
...
| `cwd` | string | Working directory for the stdio server process. Default: the session working directory when one is pinned (ACP/gateway sessions, `terminal.cwd`), else the Anakot process directory |
...
| `enabled` | bool | If `false`, Anakot skips the server entirely |
...
For well-known MCP servers, `anakot mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
anakot mcp add codex --preset codex
```
...
You can pick any local name (`anakot mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Anakot registers MCP tools

Anakot prefixes MCP tools so they do not collide with built-in names:
...
- Anakot only registers resource utilities if the MCP session actually supports resource operations
- Anakot only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Anakot, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Anakot skips the server completely and does not even attempt a connection.
...
You can also separately disable Anakot-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Anakot does not create an empty runtime MCP toolset for that server.
...
Anakot discovers MCP servers at startup and registers their tools into the normal tool registry.
...
MCP servers can notify Anakot when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Anakot receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
A running messaging gateway (`anakot gateway run`) also watches `config.yaml` on its own: within about a minute of you removing an `mcp_servers` entry or setting `enabled: false`, that server's connection is torn down; a newly added entry is connected. A server whose first connect failed (an unreachable host, or an OAuth server on a headless box that had no token yet) is retried automatically on its connect cooldown schedule (30 s, doubling up to 10 min) once you fix the cause. No restart or `/reload-mcp` needed for the edit to take effect.

**Expired OAuth tokens in the background.** The gateway, `/reload-mcp`, and the periodic self-probe of a parked server never open a browser — nobody is there to complete the flow. When a refresh token dies, the server parks with a warning in `gateway.log` and you re-authorize once with `anakot mcp login <server>` (or the Desktop/dashboard *Authorize* button); the parked server picks the new token up on its next probe.
...
For stdio servers, Anakot does not blindly pass your full shell environment.
...
cd ~/.anakot/anakot-agent && uv pip install -e ".[mcp]"

node --version
npx --version
```

Then verify your config and restart Anakot.

### Remote (HTTP) server rejects the connection

`anakot mcp test <name>` reports what the server actually answered. When the MCP SDK can only say
`Server returned an error response` (a 4xx/5xx whose body is not a JSON-RPC error), Anakot appends
...
endpoint. `anakot logs --level debug` additionally shows the exact endpoint each connect attempt used.
...
Because Anakot now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Anakot may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Anakot via the `sampling/createMessage` protocol. This allows an MCP server to ask Anakot to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Anakot as an MCP server

In addition to connecting **to** MCP servers, Anakot can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Anakot's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Anakot
- You want a single MCP server that bridges to all of Anakot's connected messaging platforms at once
- You already have a running Anakot gateway with connected platforms
...
Add Anakot to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Anakot in a specific location:

```json
{
  "mcpServers": {
    "anakot": {
      "command": "/home/user/.anakot/anakot-agent/venv/bin/anakot",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Anakot-specific channel browser:
...
The MCP server includes a live event bridge that polls Anakot's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
anakot mcp serve              # Normal mode
anakot mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Anakot's session store — `~/.anakot/state.db` is the primary source, with `sessions.json` kept only as a legacy fallback. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `anakot send` CLI.
...
- The embedded `anakot mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Anakot, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
...
- [Use MCP with Anakot](../../guides/use-mcp-with-anakot.md)
