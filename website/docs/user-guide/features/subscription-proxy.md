...
chat completions — use your Anakot-managed provider subscription as their
...
| Use case | "Use Anakot as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Anakot stores
the refresh token in `~/.anakot/auth.json` — the same place all Anakot
...
Model:      Anakot-4-70B    # or Anakot-4.3-36B, Anakot-4-405B
```
...
interface in `anakot_cli/proxy/adapters/`.
...
If you see `not logged in`, run `anakot portal`. If you see
...
`anakot_cli/proxy/adapters/<provider>.py` and registering it in
