...
description: "Give Anakot new powers with reviewed plugins you can install in one click"
---

# Plugin Catalog

The plugin catalog is a curated, human-reviewed directory of Anakot plugins you
can install by name with a single command:

```bash
anakot plugins install <name>
```
...
[`plugin-catalog/`](https://github.com/Chensihakniroth/anakot-agent-v1/tree/main/plugin-catalog)
directory of the anakot-agent repository, declaring:

| Field | Meaning |
|---|---|
| `name` | The catalog key you pass to `anakot plugins install` |
...
| `requires_anakot` | Minimum Anakot version, e.g. `>=0.19` (optional) |
...
- **Scanned at admission, trusted at install.** Admission CI runs the same
  security scanner the installer runs (`anakot plugins validate` includes a
...
The catalog name and the plugin's own manifest name can differ; `anakot
...
```bash
anakot plugins install touchdesigner
anakot plugins enable td
```
...
skill, so one install gives Anakot code, dependency, container and IaC scanning
plus the workflow for using it; the catalog name and manifest name match:

```bash
anakot plugins install snyk
anakot plugins enable snyk
```

### Updating a catalog install

`anakot plugins update <name>` never runs `git pull` for catalog installs —
...
enabled/disabled state is preserved. `anakot plugins list` shows catalog
...
`anakot plugins install <git-url>` still works for any repository, but it
...
[plugin-catalog README](https://github.com/Chensihakniroth/anakot-agent-v1/tree/main/plugin-catalog);
...
`anakot plugins list --json` reports `update_available`, the Desktop Plugins
tab shows an **Update to 1.4.0** button, and `anakot plugins update <name>`
