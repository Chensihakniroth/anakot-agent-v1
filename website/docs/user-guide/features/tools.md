...
description: "Overview of Anakot Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
Anakot ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, scheduled tasks, Home Assistant, and more.
...
| **X Search** | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool — gated on xAI credentials (SuperGrok OAuth or `XAI_API_KEY`); off by default, opt in via `anakot tools` → 🐦 X (Twitter) Search. |
...
| **Automation** | `cronjob` | Scheduled tasks with create/list/update/pause/resume/run/remove actions. Outbound delivery is handled by cron's own delivery, the `anakot send` CLI, and the gateway notifier — not by an agent-callable tool. |
...
See [Toolsets Reference](../../reference/toolsets-reference.md) for the full set, including platform presets such as `anakot-cli`, `anakot-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
**One persistent container, shared across the whole process.** Anakot starts a single long-lived container on first use (`docker run -d ... sleep infinity`) and routes every terminal, file, and `execute_code` call through `docker exec` into that same container. Working-directory changes, installed packages, environment tweaks, and files written to `/workspace` all carry over from one tool call to the next, across `/new`, `/reset`, and `delegate_task` subagents, for the lifetime of the Anakot process. The container is stopped and removed on shutdown.
...
# Configure
anakot config set terminal.backend singularity
anakot config set terminal.singularity_image ~/python.sif
```
...
modal setup
anakot config set terminal.backend modal
```
...
anakot config set terminal.backend vercel_sandbox
anakot config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running Anakot processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; Anakot defaults to `/vercel/sandbox` as the remote workspace root.
...
With `container_persistent: true`, Anakot uses Vercel snapshots to preserve filesystem state across sandbox recreation for the same task. This can include Anakot-synced credentials, skills, and cache files inside the sandbox. Snapshots do not preserve live processes, PID space, or the same live sandbox identity.

Background terminal commands use Anakot' generic non-local process flow: spawn, poll, wait, log, and kill work through the normal process tool while the sandbox is alive, but Anakot does not provide native Vercel detached-process recovery after cleanup or restart.
...
Anakot keeps the newest **64 completed results**, for up to **7 days after
completion**, under `logs/process-results/` in the profile's Anakot home. Each
...
:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.anakot/.env`.
:::
