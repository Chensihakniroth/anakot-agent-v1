...
Anakot can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<ANAKOT_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Anakot renders the sprite at full fidelity when your
...
anakot pets list
anakot pets list cat

# Install a pet and make it active in one step
anakot pets install boba --select
...
## `anakot pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `anakot pets list [query] [--limit N]` |
| List installed pets | `anakot pets list --installed` |
| Install a pet | `anakot pets install <slug> [--select] [--force]` |
| Set the active pet | `anakot pets select [slug]` (omit slug for a picker) |
| Resize the pet everywhere | `anakot pets scale <factor>` (e.g. `0.5`, clamped 0.1–3.0) |
| Preview/animate | `anakot pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `anakot pets off` |
| Remove an installed pet | `anakot pets remove <slug>` |
| Diagnose setup | `anakot pets doctor` |

`anakot pets show` flags:
...
Beyond installing pre-made pets from the gallery, Anakot can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `anakot pets` → the generate flow.
...
2. **Hatch** — the chosen base is used as a reference image to generate one grounded animation row per Anakot state (idle, thinking, tool use, etc.), which are deterministically sliced into frames and packed into a standard petdex/Codex atlas (8×9 grid of 192×208 cells). The result is a valid spritesheet you keep — and could `petdex submit`.

### Image backend

Generation uses the active [image-generation provider](./image-generation.md), but it requires **reference-image grounding** so each animation row stays the same character as the base. Reference-capable backends: **Nous Portal**, **OpenRouter**, **OpenAI** (`gpt-image-2`), and **Krea**. OpenRouter/Nous run a quality-first model chain by default.

- Resolution order prefers Nous Portal → OpenAI → OpenRouter.
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `anakot tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `ANAKOT_PET_IMAGE_PROVIDER` env var (e.g. `ANAKOT_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Anakot is
...
Run `anakot pets doctor` — it reports:
...
- The petdex npm CLI installs to `~/.codex/pets`; Anakot uses its own
  profile-scoped `<ANAKOT_HOME>/pets/` instead — install through `anakot pets`.

## See also

- The [`anakot-agent` skill](../skills/bundled/autonomous-ai-agents/autonomous-ai-agents-anakot-agent.md)
