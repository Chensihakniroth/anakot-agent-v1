...
fills the password through Anakot. The tool result it sees is
...
- **Authenticator key saved with the login** (the "setup key" or `otpauth://`
  ...
  hold a TOTP seed count too): Anakot generates the current code and enters
  it. Nobody is asked. Add the key in **Settings → Passwords & Logins → Add**
  or `anakot vault add`; the item shows a *2FA auto* badge.
- **Code sent to your phone or email**: a small prompt appears in your
  surface ("Verification code for github.com"), you type the code, Anakot
...
in, Anakot picks it up automatically and its website logins become fillable
...
per session, 30 minutes idle). Anakot hands the master password to the manager's
...
Prefer not to use a detected manager? `anakot vault sources --disable bitwarden`,
...
Passwords & Logins → Add**, or `anakot vault add`), bound to the checkout site,
...
- **CLI**: `anakot vault list`, `anakot vault add`, `anakot vault rm <handle>`,
  `anakot vault sources`.

Items live encrypted under `~/.anakot/vault/` (Fernet key + vault file, both
...
Cron jobs, webhooks, the API server and `anakot chat -q` have nobody to answer a
...
**Does:** the password never enters the model's context through Anakot: not in
