...
Anakot runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Anakot
...
1. Anakot captures a baseline of current diagnostics for the file.
...
…). Anakot auto-detects the binary on PATH or in
`<ANAKOT_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Anakot at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<ANAKOT_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`anakot lsp status` reports `installed` once `pwsh` is found; if the
...
Anakot launches it as `laravel-lsp lsp` (stdio). There is no
auto-install recipe; `anakot lsp status` shows `manual-only` until the
binary is found.
...
tree — Anakot installs `typescript@6` (the last JavaScript-based line;
TypeScript 7 is the Go port and ships no `tsserver.js`) together with
the server when you run `anakot lsp install typescript` /
`anakot lsp install vue-language-server` or auto-install fires on first use.
...
Neovim setup) that Anakot's generic client does not run, so it never
publishes diagnostics. If an earlier Anakot installed 3.x, the log shows a
one-time `vue-language-server: ... 3.x` warning; delete
`<ANAKOT_HOME>/lsp/node_modules/@vue` and `<ANAKOT_HOME>/lsp/bin/vue-language-server*`,
then run `anakot lsp install vue-language-server` (the recipe co-installs the
TypeScript SDK).

## CLI

```
anakot lsp status          # service state + per-server install status
anakot lsp list            # registry, optionally --installed-only
anakot lsp install <id>    # eagerly install one server
anakot lsp install-all     # try every server with a known recipe
anakot lsp restart         # tear down running clients
anakot lsp which <id>      # print resolved binary path
```

`anakot lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <ANAKOT_HOME>/lsp/bin
  #   manual  — only use binaries already on PATH
  install_strategy: auto

  # Node package manager for the npm-based servers: npm (default), pnpm
  # or yarn. Installs still land in <ANAKOT_HOME>/lsp/node_modules; a
  ...
  # in <ANAKOT_HOME>/lsp/.yarnrc.yml. pnpm 11 blocks git-hosted transitive
  ...
  # <ANAKOT_HOME>/lsp/.npmrc if your policy allows it.
  package_manager: npm
...
built-ins, so they can also take over an extension Anakot already
handles.
...
give an absolute path) and `anakot lsp status` lists it as
`installed`. A malformed entry is logged and skipped without
affecting the other servers.

## Installation locations

When `install_strategy: auto`, Anakot installs binaries into
`<ANAKOT_HOME>/lsp/bin/`. NPM packages land in
`<ANAKOT_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Anakot-owned and is
...
are not idle: removing a Anakot-managed worktree (`anakot -w` session
...
exists on disk (deleted outside Anakot). The sweep is part of the idle
...
**single process** per Anakot process: the first Python project spawns
...
**`anakot lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<ANAKOT_HOME>/lsp/bin/`. Run
`anakot lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `anakot lsp status`**
...
When `anakot lsp status` shows a `Backend warnings` section, install
...
`~/.anakot/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.anakot/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `anakot lsp restart` to clear the set;
