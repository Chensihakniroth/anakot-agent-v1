...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Anakot's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Anakot instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
anakot memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `anakot` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Automatically generated Anakot titles (`derived` or `llm`), including lineage titles for Desktop branches, are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions without a manual mapping.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Anakot syncs them back at session init.
...
The `anakot honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `anakot memory setup honcho` (or run `anakot memory setup` and pick it from the list); the `anakot honcho` subcommand then appears on the next invocation.

```bash
anakot memory setup honcho    # Configure Honcho directly (works before activation)
anakot honcho status          # Connection status, config, and key settings
anakot honcho setup           # Redirects to `anakot memory setup` (post-activation alias)
anakot honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
anakot honcho peer            # Show or update peer names + dialectic reasoning level
anakot honcho mode            # Show or set recall mode (hybrid/context/tools)
anakot honcho tokens          # Show or set token budget for context and dialectic
anakot honcho identity        # Seed or show the AI peer's Honcho identity
anakot honcho sync            # Sync Honcho config to all existing profiles
anakot honcho peers           # Show peer identities across all profiles
anakot honcho sessions        # List known Honcho session mappings
anakot honcho map             # Map current directory to a Honcho session name
anakot honcho enable          # Enable Honcho for the active profile
anakot honcho disable         # Disable Honcho for the active profile
anakot honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `anakot honcho`

If you previously used the standalone `anakot honcho setup`:
...
No re-login or re-setup needed. Run `anakot memory setup` and select "honcho" — the wizard detects your existing config.
