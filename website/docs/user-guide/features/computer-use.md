...
Anakot Agent can drive your desktop — clicking, typing, scrolling,
...
The built-in `computer_use` toolset is the recommended Anakot integration. It
...
**Fresh installs already have the driver.** The Anakot installer
...
- **`anakot tools`** → pick `🖱️  Computer Use` — installs the driver
...
```
anakot computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `anakot computer-use
status` to verify the install.

Already have cua-driver? Anakot reuses it when it supports the 0.20 runtime
contract. During setup, toolset enablement, `anakot update`, and the first
`computer_use` call of a session, Anakot checks the local version and
...
selected with `ANAKOT_CUA_DRIVER_CMD` stays
under your control, so Anakot reports the incompatibility and leaves it
unchanged.

If you install Cua Driver first, `cua-driver skills install` installs Cua's
skill pack under `~/.cua-driver/skills/cua-driver`. Anakot autodetection is a
planned cua-driver follow-up, so currently point Anakot at that directory or
...
interface. The built-in toolset provides Anakot actions, configuration,
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording**. Grant the identity named by `anakot computer-use doctor`. Standard mode uses CuaDriver.app; bounded and unrestricted modes use the Anakot host identity. |
...
```
anakot -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.anakot/config.yaml`.

## Permission modes and logged-in browser profiles

Anakot maps its existing approval UX onto cua-driver's immutable runtime
modes. Permission mode and capability manifest approval are launch settings.
They cannot change after the runtime starts:

| Anakot session | cua-driver mode | Human intervention |
|---|---|---|
| Manual or smart approvals (default) | `standard` | Normal Anakot approvals; Cua stops at its protected boundary |
...
| `--yolo`, `/yolo`, or `approvals.mode: off` | private `unrestricted` daemon | One explicit Anakot risk acceptance; no runtime Cua prompts |
...
computer_use:
  ...
  capability_manifest: ~/.anakot/cua-manifest.yaml
```
...
for the format). Anakot launches a private runtime with
...
identity instead of resetting with every Anakot build), and Anakot verifies
...
resetting or closing the Anakot session, cancellation cleanup, or process exit
closes that transport session. Anakot also stops private runtimes that it
launched for bounded or unrestricted access. One Anakot
conversation cannot change another runtime's mode or grants. Bounded and
unrestricted modes use a private
embedded service under the Anakot host identity.
...
## `anakot computer-use doctor` — your first triage stop

`anakot computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ anakot computer-use doctor
...
Anakot run declares a public cua-driver **session name** (something like
`anakot-3a7b9c14d2e8`). The name labels cursor identity and related state, so
...
without it. Anakot disables it automatically where it is a known failure
...
Anakot keeps its wrapper skill (`skills/autonomous-ai-agents/computer-use/SKILL.md`)
focused on the Anakot-side `computer_use` workflow and action vocabulary. For
...
The command links the pack into `~/.anakot/skills/cua-driver` (Anakot is one
...
These are **platform deep dives, not duplicates of the Anakot skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Anakot
...
image capture also saves a bounded, shareable copy under Anakot' image cache
...
Anakot applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.anakot/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Anakot applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  anakot agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the anakot agent itself at High integrity (launch from an
  elevated terminal); otherwise target non-elevated windows.
- **Windows: `anakot computer-use doctor` fails with "Access is denied"
  while the tool works.** A cua-driver installed under
  `C:\Program Files\WindowsApps` cannot be executed by the Anakot venv
...
On Linux, native Wayland support remains an explicit opt-in. Anakot passes the
...
```
ANAKOT_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
ANAKOT_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Anakot disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Anakot sets
...
When it's on, `anakot computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Anakot at a binary you built from source instead
of the published release. Anakot resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`ANAKOT_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Anakot testing (see notes).
...
### Option B — point Anakot straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`ANAKOT_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
ANAKOT_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
ANAKOT_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Anakot is using your build

- `anakot computer-use status` prints the resolved binary path and
  version.
- `anakot computer-use doctor` confirms the binary is reachable and
...
- **Anakot spawns a `cua-driver mcp` stdio proxy.** In a normal session the
  proxy connects to (and may start) the standard machine daemon. In explicit
  Anakot YOLO, Anakot instead owns a private `cua-driver serve --embedded`
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Anakot
  change needed either way.
- **Local builds skip the version check.** Anakot warns when the
...
**First action when anything's off: run `anakot computer-use doctor`.**
...
Run `anakot computer-use install` to fetch the cua-driver binary, or
run `anakot tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `anakot computer-use doctor` will
...
- **Anakot-side skill** — `skills/autonomous-ai-agents/computer-use/SKILL.md` — teaches the
  Anakot `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  `LINUX.md` / `RECORDING.md` / `WEB_APPS.md`. Anakot autodetection is a
  planned follow-up; currently point Anakot at the installed pack directory
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Anakot among them)
