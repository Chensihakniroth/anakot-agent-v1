...
Anakot Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
anakot memory setup      # interactive picker + configuration
anakot memory status     # check what's active
anakot memory off        # disable external provider
```

You can also select the active memory provider via `anakot plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.anakot/config.yaml`:
...
When a memory provider is active, Anakot automatically:
...
**Setup Wizard:**
```bash
anakot memory setup        # select "honcho" — runs the Honcho-specific post-setup
```
...
:::tip Migrating from `anakot honcho`
If you previously used `anakot honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Anakot profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Anakot profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Anakot profile. Host key `anakot` → default; `anakot.<profile>` for others. |
...
```bash
anakot profile create coder --clone
```

`--clone` creates a `anakot.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Anakot profile, creates host blocks for any profile without one, inherits settings from the default `anakot` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `anakot memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/anakot).
...
anakot memory setup    # select "openviking"
# Or manually:
anakot config set memory.provider openviking
```

`anakot memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.anakot/.env`, and for named profiles use
`~/.anakot/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Anakot sends no peer ID and writes
...
Set `memory.openviking.agent: anakot` to restore the old peer-scoped writes.
...
Anakot sends `User-Agent: openviking-memory-anakot/<version>` on OpenViking
...
**Setup (Platform):**
```bash
anakot memory setup    # select "mem0" → "Platform"
# Or manually:
anakot config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.anakot/.env
```

**Setup (OSS):**
```bash
anakot memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
anakot memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
anakot memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
anakot memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
anakot memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.anakot/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.anakot/.env
```
...
**Config:** `$ANAKOT_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.anakot/.env`.
...
| `user_id` | `anakot-user` | User identifier |
| `agent_id` | `anakot` | Agent identifier |
...
**Switching modes:** Re-run `anakot memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
anakot memory setup    # select "hindsight"
# Or manually:
anakot config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.anakot/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p anakot ui start`

**Config:** `$ANAKOT_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `anakot` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Anakot Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Chensihakniroth/anakot-agent-v1/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Config:** `config.yaml` under `plugins.anakot-memory-store`
...
**Setup:**
```bash
anakot memory setup    # select "retaindb"
# Or manually:
anakot config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.anakot/.env
```
...
# Then configure Anakot
anakot memory setup    # select "byterover"
# Or manually:
anakot config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$ANAKOT_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [cloud API key](http://app.supermemory.ai/integrations?connect=anakot), or a [self-hosted server](https://supermemory.ai/docs/self-hosting/overview) |
...
**Setup:**
```bash
anakot memory setup    # select "supermemory"
# Or manually:
anakot config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.anakot/.env
```
...
Before running `anakot memory setup`, set `base_url` in
`$ANAKOT_HOME/supermemory.json`:
...
Then run `anakot memory setup` and enter the API key printed by the local
server. Configuring the endpoint first ensures the setup connection probe also
stays local.

**Config:** `$ANAKOT_HOME/supermemory.json`
...
| `container_tag` | `anakot` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `anakot-{identity}` → `anakot-coder`) to isolate memories per Anakot profile
...
| **Requires** | `pip install anakot-memori` + `anakot-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install anakot-memori
anakot-memori install
anakot config set memory.provider memori
anakot memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `anakot-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](../profiles.md):

- **Local storage providers** (Holographic, ByteRover) use `$ANAKOT_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$ANAKOT_HOME/` so each profile has its own credentials
- **Cloud providers** (RetainDB) auto-derive profile-scoped project names
- **Env var providers** (OpenViking) are configured via each profile's `.env` file

## Providers Moving to the Plugin Catalog

Memory providers are moving out of the Anakot tree into their maintainers' own repositories,
...
When a provider you have configured stops shipping with Anakot, `anakot update` installs its
...
`false`, in which case it prints the `anakot plugins install <name>` one-liner instead).
