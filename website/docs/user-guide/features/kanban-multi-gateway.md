...
Anakot supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.anakot/config.yaml`:
...
Or set the env var: `ANAKOT_KANBAN_DISPATCH_IN_GATEWAY=false`
