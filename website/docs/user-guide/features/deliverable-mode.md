...
When Anakot Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.anakot/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.anakot/config.yaml` (switchable per session
via `/personality`).
...
If you use Anakot' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.anakot/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Anakot Agent's deliverable
