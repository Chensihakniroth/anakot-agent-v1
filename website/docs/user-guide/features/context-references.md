...
Type `@` followed by a reference to inject content directly into your message. Anakot expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Anakot env: `$ANAKOT_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$ANAKOT_HOME/skills/.hub/`
