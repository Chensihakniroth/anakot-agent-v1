...
This tutorial walks you through setting up a Telegram bot powered by Anakot Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Anakot Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](../getting-started/installation.md) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.anakot/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Anakot itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Anakot Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_anakot_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Anakot Agent. DM me for help with code, research, debugging, and more.
   ```
...
```
[Gateway] Starting Anakot Gateway...
...
# View live logs
journalctl --user -u anakot-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `anakot gateway install` to update the plist.
:::
...
```bash
anakot gateway stop && anakot gateway start
```
...
anakot pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.anakot/.env`:
...
Control how much detail the bot shows when using tools. In `~/.anakot/config.yaml`:
...
Customize how the bot communicates by editing `~/.anakot/SOUL.md`:

For a full guide, see [Use SOUL.md with Anakot](./use-soul-with-anakot.md).
...
```bash
# From the CLI
anakot cron list          # View all scheduled jobs
anakot cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u anakot-gateway` (Linux) or `~/.anakot/logs/gateway.log` (macOS) |
| Cron job output | `~/.anakot/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.anakot/cron/jobs.json` |
| Pairing data | `~/.anakot/pairing/` |
| Session history | `~/.anakot/sessions/` |
