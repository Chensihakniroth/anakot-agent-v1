...
```bash
anakot teams-pipeline validate
```
...
```bash
anakot teams-pipeline token-health
anakot teams-pipeline token-health --force-refresh
```
...
```bash
anakot teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
anakot teams-pipeline maintain-subscriptions
anakot teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Anakot cron (recommended if you already run the Anakot gateway)

Anakot ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.anakot/scripts/`. First create the script:

```bash
mkdir -p ~/.anakot/scripts
cat > ~/.anakot/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec anakot teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.anakot/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
anakot cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/anakot-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Anakot Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=anakot
EnvironmentFile=/etc/anakot/env
ExecStart=/usr/local/bin/anakot teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/anakot-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Anakot Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now anakot-teams-pipeline-maintain.timer
systemctl list-timers anakot-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/anakot teams-pipeline maintain-subscriptions >> /var/log/anakot/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.anakot/.env` at the top of a wrapper script that crontab calls.
...
```bash
anakot teams-pipeline subscriptions   # should show expirationDateTime advanced
anakot teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
anakot teams-pipeline list
anakot teams-pipeline list --status failed
anakot teams-pipeline show <job-id>
```

### Replay a stored job

```bash
anakot teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
anakot teams-pipeline fetch --meeting-id <meeting-id>
anakot teams-pipeline fetch --join-web-url "<join-url>"
anakot teams-pipeline fetch --join-web-url "<join-url>" --organizer-user-id <entra-user-id>
```
...
- run `anakot teams-pipeline maintain-subscriptions --dry-run`
- inspect `anakot teams-pipeline list --status failed`
...
- run `anakot teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `anakot teams-pipeline run`
...
- [ ] `anakot teams-pipeline validate` returns an OK snapshot
- [ ] `anakot teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Anakot cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
