...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Anakot Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Anakot instantly when PRs are opened or updated.
:::
...
- **Anakot Agent installed** — see the [Installation guide](../getting-started/installation.md)
- **Gateway running** for cron jobs:
  ```bash
  anakot gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.anakot/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Anakot can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Chensihakniroth/anakot-agent-v1 --state open --limit 3
```
...
Still in the chat, ask Anakot to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Chensihakniroth/anakot-agent-v1
```
...
A skill gives Anakot consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.anakot/skills/code-review
```

Create `~/.anakot/skills/code-review/SKILL.md`:
...
Verify it loaded — start `anakot` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Anakot your team's standards:
...
2. Teach Anakot your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
anakot gateway status    # Is the gateway running?
anakot cron list         # Is the job enabled?
```
