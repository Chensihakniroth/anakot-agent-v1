---
sidebar_position: 8
title: "Use Voice Mode with Anakot"
description: "A practical guide to setting up and using Anakot voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Anakot
...
- you want Anakot sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Anakot.
...
## Step 1: make sure normal Anakot works first
...
Anakot supports both local and cloud speech stacks.
...
### If you use `anakot setup`

If you choose NeuTTS in the setup wizard, Anakot checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Anakot transcribes and responds
...
- asking Anakot to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Anakot loop.

## Tuning CLI behavior

### Silence threshold

If Anakot starts/stops too aggressively, tune:
...
Anakot stays a normal chat bot, but can speak replies.
...
- you want Anakot to function like a portable research or ops assistant
...
Anakot joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Anakot detects speech boundaries
- transcripts are posted in the associated text channel
- Anakot responds in text and audio
...
1. get text Anakot working
2. run `anakot setup tts` to enable voice support
