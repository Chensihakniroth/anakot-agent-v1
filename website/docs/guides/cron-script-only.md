...
Anakot calls this **no-agent mode**. It's the cron system minus the LLM.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Anakot writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Anakot:** *(writes `~/.anakot/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.anakot/scripts/memory-watchdog.sh",
...
>
> **Anakot:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `anakot cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Anakot:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.anakot/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.anakot/scripts/memory-watchdog.sh
...
# 3. Verify
anakot cron list
anakot cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.anakot/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
anakot cron create "every 5m"        # interval
anakot cron create "every 2h"
anakot cron create "0 9 * * *"       # standard cron: 9am daily
anakot cron create "30m"             # one-shot: run once in 30 minutes
```

See the [cron feature reference](../user-guide/features/cron.md) for the full syntax.
...
--deliver local                          # just save to ~/.anakot/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.anakot/.env` / `~/.anakot/config.yaml`.

## Editing and Lifecycle

```bash
anakot cron list                                    # see all jobs
anakot cron pause <job_id>                          # stop firing, keep definition
anakot cron resume <job_id>
anakot cron edit <job_id> --schedule "every 10m"    # adjust cadence
anakot cron edit <job_id> --agent                   # flip to LLM mode
anakot cron edit <job_id> --no-agent --script …     # flip back
anakot cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.anakot/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.anakot/scripts/disk-alert.sh

anakot cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Anakot' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](../user-guide/messaging/webhooks.md) | Your script on the OS schedule | When Anakot might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Anakot webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Anakot being up. The in-gateway scheduler is the right choice when the thing being monitored is external.

## Related

- [Automate Anything with Cron](./automate-with-cron.md) — LLM-driven cron patterns.
- [Scheduled Tasks (Cron) reference](../user-guide/features/cron.md) — full schedule syntax, lifecycle, delivery routing.
- [Webhook Subscriptions](../user-guide/messaging/webhooks.md) — fire-and-forget HTTP entry points for external schedulers.
- [Gateway Internals](../developer-guide/gateway-internals.md) — delivery-router internals.
