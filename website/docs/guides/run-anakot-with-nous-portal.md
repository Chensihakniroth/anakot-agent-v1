...
3. Sets `model.provider: nous` in `~/.anakot/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Anakot is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
anakot setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
anakot auth add nous --type oauth
# Then re-run `anakot setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Anakot Agent release notes" and summarize the top 3 hits.
```

You should see Anakot call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`anakot setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
anakot config set model.default anthropic/claude-sonnet-4.6
```

### Don't pick Anakot-4 for agent work

Anakot-4-70B and Anakot-4-405B are available on the Portal at deep discounts, but they're **chat/reasoning models**, not tool-call-tuned. They will struggle with multi-step agent loops. Use them for conversation/research work through the [subscription proxy](../user-guide/features/subscription-proxy.md) from non-agent tools. For Anakot Agent itself, stick to the frontier agentic models above.

The Portal's own [info page](https://portal.nousresearch.com/info) carries this warning too — it's the official Nous guidance, not just a Anakot-side opinion.
...
These rows appear in `anakot tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Anakot runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Anakot will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
anakot config set model.provider nous
```
...
Re-verify with `anakot portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Anakot doesn't replay it endlessly. Just log in again:

```bash
anakot auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/Chensihakniroth/anakot-agent-v1/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`anakot portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Anakot profiles where you're using the wrong one (check `anakot profile list`)

### Want to revoke and start clean

```bash
anakot auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](../user-guide/features/subscription-proxy.md)** — Use your Portal subscription from non-Anakot tools
...
- **[Profiles](../user-guide/profiles.md)** — Share one Portal login across multiple Anakot configurations
