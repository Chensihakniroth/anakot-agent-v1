...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `anakot send`."
---

# Pipe Script Output to Messaging Platforms

`anakot send` is a small, scriptable CLI that pushes a message to any
messaging platform Anakot is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | anakot send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `anakot
...
anakot send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
anakot send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
anakot send --list

# Filter by platform
anakot send --list telegram
```
...
Any platform Anakot ships adapters for works as a target:
...
`anakot send` resolves the message body in this order:

1. **Positional argument** — `anakot send --to telegram "hi"`
2. **`--file PATH`** — `anakot send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | anakot send --to telegram`

When stdin is a TTY (no pipe), Anakot does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  anakot send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `anakot send` reuses your Anakot config, the same script works on
any host where Anakot is installed — no need to export bot tokens into
...
`anakot send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  anakot send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | anakot send \
...
0 9 * * * /usr/local/bin/generate-metrics.sh \
  | /home/me/.anakot/bin/anakot send \
...
```bash
./train.py --epochs 200 && \
  anakot send --to telegram "training done" || \
  anakot send --to telegram "training failed (exit $?)"
```
...
anakot send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(anakot send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `anakot send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `anakot send` calls
the platform's REST endpoint directly using credentials from
`~/.anakot/.env` and `~/.anakot/config.yaml` (or the equivalent files under
your resolved Anakot home — `%LOCALAPPDATA%\anakot` on Windows, or the profile
directory when `ANAKOT_HOME` / `-p` is set). It's a standalone subprocess
...
`Looked in: C:\Users\me\AppData\Local\anakot\.env (no DISCORD_BOT_TOKEN),
C:\Users\me\AppData\Local\anakot\config.yaml (no platforms.discord block),
...
`.env` so `anakot send` can use it. When your shell is scoped to a profile home
(`ANAKOT_HOME=<root>/profiles/<name>`) but the connected gateway runs from the
default root, the error says so — the gateway never read the profile's `.env`,
and `anakot send --list` points at the root's `channel_directory.json`.
...
pointing at the gateway; start it with `anakot gateway start` and retry.
...
# Machine-readable
anakot send --list --json
```

The listing is built from `~/.anakot/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`anakot gateway
...
| Approach | Multi-platform | Reuses Anakot creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `anakot send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`anakot send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `anakot send`.
...
- [Gateway Internals](../developer-guide/gateway-internals.md) —
  the delivery router that `anakot send` shares with cron delivery.
