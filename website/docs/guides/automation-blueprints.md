...
Copy-paste blueprints for common automation patterns. Each blueprint uses Anakot's built-in [cron scheduler](../user-guide/features/cron.md) for time-based triggers and [webhook platform](../user-guide/messaging/webhooks.md) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`anakot webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `anakot webhook subscribe`) |
...
```bash
anakot cron create "0 2 * * *" \
  "You are a project manager triaging the Chensihakniroth/anakot-agent-v1 GitHub repo.

1. Run: gh issue list --repo Chensihakniroth/anakot-agent-v1 --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
anakot webhook subscribe github-pr-review \
  --events "pull_request" \
...
anakot cron create "0 9 * * 1" \
  "Scan the Chensihakniroth/anakot-agent-v1 repo for documentation drift.

1. Run: gh pr list --repo Chensihakniroth/anakot-agent-v1 --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (anakot_cli/commands.py, anakot_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (anakot_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
anakot cron create "0 6 * * *" \
  "Run a dependency security audit on the anakot-agent project.

1. cd ~/.anakot/anakot-agent && source .venv/bin/activate
...
```bash
anakot webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
anakot webhook subscribe alert-triage \
...
```python title="~/.anakot/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Anakot-Monitor/1.0"})
...
anakot cron create "every 30m" \
  ...
  --script ~/.anakot/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
anakot cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Anakot Agent development." \
...
```bash
anakot webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
anakot webhook subscribe stripe-payments \
...
```bash
anakot cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the anakot-agent codebase.
