...
Anakot includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Anakot and does not require a Anakot observability plugin. Anakot remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Anakot execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `anakot-agent[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Anakot `observability/nemo_relay` plugin. Existing users
...
> `plugins.toml` selected with `ANAKOT_NEMO_RELAY_PLUGINS_TOML`. The legacy
> `ANAKOT_NEMO_RELAY_ATOF_*` and `ANAKOT_NEMO_RELAY_ATIF_*` variables no
> longer activate exporters. Without the new variable, Anakot does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Anakot requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Anakot
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Anakot installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Anakot process so configured interceptors can
...
Collection remains off unless Anakot policy enables it:
...
opt-in. Set `ANAKOT_NEMO_RELAY_PLUGINS_TOML` to a selected `plugins.toml` to
activate configured middleware, exporters, or dynamic plugins. When the
variable is unset, Anakot does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Anakot reports the error and does not invoke
...
retain the same `session_id` and add `anakot.session.segment` plus
`anakot.session.segment_reason` (`compaction` or `max_turns`).

## Process-Wide Plugin Policy and Profile Isolation

Relay plugin configuration is a process-level deployment choice, not a Anakot
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Anakot process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Anakot logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Anakot
process and share the same policy boundary.

Run profiles in separate Anakot processes when they require different trust
...
Anakot core owns one Relay host and one isolated Relay session scope per Anakot
...
marks do not require Anakot plugin registration. Shared-metrics marks must
...
```text
Anakot turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Anakot shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Anakot sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Anakot used for the logical call, such as
...
New calls use `anakot.model_route.count`. The previous
`anakot.model_call.count` contract remains readable only so pending local
counters created by older builds can be exported without losing data.

The first consented session start emits an empty `anakot.client.active` Relay
...
counter in any rolling 24-hour window. The metric has no dimensions; Anakot
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Anakot processes share the SQLite latch, so simultaneous
...
Each task run is a Relay `Function` scope named `anakot.task_run`, parented to
the owning Anakot session. The start counter contains only bounded execution
...
provider attempts for the same Anakot API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Anakot tool-call
...
cancellations. Active task ownership follows the task ID if Anakot rotates its
conversation session during context compression.

Each tool invocation is represented by a Relay tool lifecycle named
`anakot.tool_call`. The terminal counter contains only bounded tool category,
outcome, approval outcome, latency, and explicit retry-count buckets. Anakot
...
tool or plugin names. Anakot does not infer retries from repeated tool names or
adjacent calls; when the
hook does not provide an explicit retry relationship, the retry bucket is
`unknown`. Approval decisions are emitted as `anakot.tool_approval` marks and
...
Successful skill mutations emit `anakot.skill.lifecycle` marks with only a
bounded action and provenance. Successful loads emit `anakot.skill.load`
marks with bounded provenance, first-use or reuse state, reuse-after-patch
state, and a use-count bucket. Anakot derives reuse and patch-generation
...
```text
$ANAKOT_HOME/telemetry/shared_metrics/metrics.sqlite3
$ANAKOT_HOME/telemetry/shared_metrics/outbox/*.json
```
...
are written with atomic replacement. Each package records the Anakot version,
...
schema field name, its current scope is one `ANAKOT_HOME`, so it is more
...
Deleting `$ANAKOT_HOME/telemetry/shared_metrics` resets the identifier together
...
The install identity is scoped to one `ANAKOT_HOME`. To reset it, stop Anakot
processes and remove `$ANAKOT_HOME/telemetry/shared_metrics`. This deliberately
...
Run a real Anakot CLI turn against the deterministic local model server:
...
`$ANAKOT_HOME/telemetry/shared_metrics/outbox/` to the Anakot telemetry ingest
...
Removing `$ANAKOT_HOME/telemetry/shared_metrics` still resets local identity,
