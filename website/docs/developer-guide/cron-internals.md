...
description: "How Anakot stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `anakot_cli/cron.py` | CLI `anakot cron` subcommands |
...
Jobs are stored in `~/.anakot/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
renderer (`anakot cron list`/`doctor`, the `cronjob` tool, the web dashboard
...
The same store fields drive every topology: a standalone `anakot -p X gateway
...
In CLI mode, cron jobs only fire when `anakot cron` commands are run or during active CLI sessions.
...
```python
# ~/.anakot/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.anakot/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `anakot cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `anakot cron` CLI provides direct job management:

```bash
anakot cron list                    # Show all jobs
anakot cron create                  # Interactive job creation (alias: add)
anakot cron edit <job_id>           # Edit job configuration
anakot cron pause <job_id>          # Pause a running job
anakot cron resume <job_id>         # Resume a paused job
anakot cron run <job_id>            # Trigger immediate execution
anakot cron remove <job_id>         # Delete a job
```
