...
Plugins can launch and supervise fresh Anakot child sessions without importing
...
active agent turn fails closed with `No active Anakot parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Anakot can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Anakot's existing unsafe-tool block remains enforced.
