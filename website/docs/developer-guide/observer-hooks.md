...
# Anakot Observer Hooks

Anakot observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Anakot' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "anakot.observer.v1"
```

Hook callbacks are fail-open. Anakot catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Anakot only builds
...
The native NeMo Relay SDK integration maps Anakot session, turn, LLM, and tool
