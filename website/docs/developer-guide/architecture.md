...
sidebar_position: 1
title: "Architecture"
description: "Anakot Agent internals — major subsystems, execution paths, data flow, and where to read next"
---
...
├── cli.py                    # AnakotCLI facade — mixins in anakot_cli/cli_*_mixin.py
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── anakot_state.py           # SQLite session/state database facade (+ anakot_state_*.py siblings)
├── anakot_constants.py       # ANAKOT_HOME, profile-aware paths
...
│   ├── main.py               # Entry point — `anakot` subcommands (parsers in subcommands/, main_*.py)
...
│   ├── skills_config.py      # anakot skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # anakot tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # anakot gateway start/stop
│
...
Three discovery sources: `~/.anakot/plugins/` (user), `.anakot/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `anakot plugins` or `config.yaml`.

→ [Plugin Guide](./plugins/index.md), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes Anakot as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`anakot -p <name>`) gets its own ANAKOT_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
