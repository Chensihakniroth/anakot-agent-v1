...
description: "How to build an image-generation backend plugin for Anakot Agent"
---
...
:::tip
Image-gen is one of several **backend plugins** Anakot supports. The others (with more specialized ABCs) are [Memory Provider Plugins](./memory-provider-plugin.md), [Context Engine Plugins](./context-engine-plugin.md), and [Model Provider Plugins](./model-provider-plugin.md). General tool/hook/CLI plugins live in [Build a Anakot Plugin](./plugins/index.md).
:::

## How discovery works

Anakot scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.anakot/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `anakot_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `anakot tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `anakot tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.anakot/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `anakot plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `anakot tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `anakot tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `anakot tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            #   - base64 data: save under $ANAKOT_HOME/cache/images/ via save_b64_image()
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `anakot plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `anakot tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `anakot tools` model picker |
...
Some backends return image URLs (fal, Replicate); others return base64 payloads (OpenAI gpt-image-2). For the base64 case, use `save_b64_image()` — it writes to `$ANAKOT_HOME/cache/images/<prefix>_<timestamp>_<uuid>.<ext>` and returns the absolute `Path`. Pass that path (as `str`) as `image=` in `success_response()`. Gateway delivery (Telegram photo bubble, Discord attachment) recognizes both URLs and absolute paths.

## User overrides

Drop a user plugin at `~/.anakot/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `anakot plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.

## Testing

```bash
export ANAKOT_HOME=/tmp/anakot-imggen-test
mkdir -p $ANAKOT_HOME/plugins/image_gen/my-backend
# …copy __init__.py + plugin.yaml into that dir…

export MY_BACKEND_API_KEY=your-test-key
anakot plugins enable my-backend

# Pick it as the active provider
echo "image_gen:" >> $ANAKOT_HOME/config.yaml
echo "  provider: my-backend" >> $ANAKOT_HOME/config.yaml

# Exercise it
anakot -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `anakot tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
...
- [Build a Anakot Plugin](./plugins/index.md) — general tools/hooks/slash commands guide
