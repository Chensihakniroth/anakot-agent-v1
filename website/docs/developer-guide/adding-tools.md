...
sidebar_position: 2
title: "Adding Tools"
description: "How to add a new tool to Anakot Agent — schemas, handlers, registration, and toolsets"
---
...
modifying Anakot core, use the plugin route instead:

- [Plugins](../user-guide/features/plugins.md)
- [Build a Anakot Plugin](./plugins/index.md)
...
2. **`toolsets.py`** — add tool name to `_ANAKOT_CORE_TOOLS` (or a specific toolset)
...
If your tool requires an API key, add it to `anakot_cli/config.py`:
...
- [ ] Optional: API key added to `OPTIONAL_ENV_VARS` in `anakot_cli/config.py`
- [ ] Optional: Added to `toolset_distributions.py` for batch processing
- [ ] Tested with `anakot chat -q "Use the weather tool for London"`
