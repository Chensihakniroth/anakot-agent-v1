...
Anakot Agent uses a SQLite database (`~/.anakot/state.db`) to persist session
metadata, full message history, and model configuration across CLI and gateway
sessions. This replaces the earlier per-session JSONL file approach.

Source files: `anakot_state.py` (facade) plus the `anakot_state_*.py` siblings (schema, fts, search, compression, portability, gateway, ...)

## Anakot home and profile isolation

`get_anakot_home()` is the authoritative filesystem resolver for state and
configuration. It uses a context-local override first, then the `ANAKOT_HOME`
environment variable, and finally the platform default (`~/.anakot` on macOS
and Linux; `%LOCALAPPDATA%/anakot` on Windows). Consequently, the default
database is always `get_anakot_home() / "state.db"`, not a path that callers
should hard-code as `~/.anakot/state.db`.

Named profiles are isolated directories: a profile named `coder`, for example,
uses `<default Anakot root>/profiles/coder/` and therefore has its own
...
profile must retain or pass that profile's `ANAKOT_HOME`; falling back to the
...
of Anakot. An explicit `--profile`/`-p` resolves that profile and writes the
resolved directory to `ANAKOT_HOME`. Without an explicit selector, a
profile-specific `ANAKOT_HOME` is preserved; otherwise the bootstrap can use
the default root's active-profile selection. `HOME` only determines the
platform default used when no context override or `ANAKOT_HOME` is available.
Changing `HOME` is not a safe way to select a named profile. In particular, a
subprocess that drops `ANAKOT_HOME` can fall back to the default profile even
when another profile is active, so subprocess spawners should pass
`ANAKOT_HOME` explicitly.

Use `display_anakot_home()` only for user-facing text. It formats the resolved
home relative to the user's home directory when possible (for example,
`~/.anakot/profiles/coder`); it does not provide a separate resolution rule.

### Test isolation guard

Tests must use a temporary `ANAKOT_HOME` or an explicit temporary database
path. The live-system guard raises before a test-context process opens a
production `state.db` under the real default Anakot root or a real named
profile, preventing fixture data or SQLite side effects from reaching a live
installation.

`ANAKOT_STATE_DB_GUARD_BYPASS=1` is a test-only escape hatch for a spawned
...
either bypass in normal Anakot commands, development shells, or application
...
Each named profile stores its transcript in its own `$ANAKOT_HOME/state.db`,
including when one `anakot serve` process serves several profiles. In-session
...
splice boundary, Anakot excludes only that leading item when it exactly matches
...
```
~/.anakot/state.db (SQLite, WAL mode)
...
`anakot sessions recover` copies the row-bearing tables above into the
...
Abridged — see `SCHEMA_SQL` in `anakot_state_common.py` (applied by `anakot_state_schema.py`) for the full current column list
...
an authenticated `anakot serve` (OAuth or the basic username/password provider)
...
indexed columns — see `SCHEMA_SQL` in `anakot_state_common.py` for the exact SQL.
...
Multiple anakot processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.anakot/state.db
...
Default path: `get_anakot_home() / "state.db"` — `~/.anakot/state.db` for the
default profile, `~/.anakot/profiles/<name>/state.db` for a named profile, or
wherever `ANAKOT_HOME` points (see [Anakot home and profile isolation](#anakot-home-and-profile-isolation)).
