...
title: "Desktop Plugin SDK (@anakot/plugin-sdk)"
description: "Extend the native Anakot Desktop app — panes, pages, sidebar nav, status bar, palette commands, keybinds, themes, and a scoped backend namespace, with one import and no build step."
---

# Desktop Plugin SDK

The native [Anakot Desktop](../user-guide/desktop.md) app is contribution-driven: every
...
A **desktop plugin** is a single ESM file that default-exports a `AnakotPlugin`.
It imports one module — `@anakot/plugin-sdk` — and gets everything: the app's
...
`$ANAKOT_HOME/desktop-plugins/<id>/plugin.js` and the app loads it within seconds
and hot-reloads every save.

:::warning This is not the web-dashboard plugin SDK
"Plugin" means several unrelated things across Anakot. This page is the **native
desktop app** (`anakot desktop`) SDK — the `@anakot/plugin-sdk` module and
`$ANAKOT_HOME/desktop-plugins/`. The **web dashboard** (`anakot dashboard`) has
its own, unrelated plugin system on `window.__ANAKOT_PLUGIN_SDK__` with a
`manifest.json` — documented at
[Extending the Dashboard](../user-guide/features/extending-the-dashboard.md). Python
CLI/gateway plugins are documented at [Build a Anakot Plugin](./plugins/index.md).
...
|------|-------|-----|------------|
| **Disk** (recommended) | `$ANAKOT_HOME/desktop-plugins/<id>/plugin.js` | users, agents | none — plain ESM, loaded uncompiled |
| **Unified package** | `$ANAKOT_HOME/plugins/<id>/desktop/plugin.js` | plugins that also ship agent-side code | none — same disk pipeline |
| **Bundled** | `apps/desktop/src/plugins/<id>/plugin.tsx` | in-tree, shipped with the app | the app's own Vite build |

All three take the same `AnakotPlugin` contract, appear in **Capabilities → Plugins**,
...
Create `$ANAKOT_HOME/desktop-plugins/hello/plugin.js` (that's `~/.anakot/...`
...
```javascript
// ~/.anakot/desktop-plugins/hello/plugin.js
import { host, haptic, useValue } from '@anakot/plugin-sdk'
...
function HelloPane() {
  ...
  return jsxs('div', {
    ...
    children: [
      jsx('div', { className: 'font-medium', children: 'Hello, Anakot' }),
      jsx('div', {
...
The only importable specifiers are `@anakot/plugin-sdk`, `react`, and
`react/jsx-runtime` — everything else fails to resolve, on purpose.
:::

## The plugin contract

A plugin default-exports a `AnakotPlugin`:

```ts
interface AnakotPlugin {
...
interface PluginContext {
  ...
  os: PluginOs
  /** Plugin-scoped JSON persistence (keys live under `anakot.plugin.<id>.`). */
...
```javascript
import { ROUTES_AREA, SIDEBAR_NAV_AREA } from '@anakot/plugin-sdk'
...
```javascript
import { STATUSBAR_AREAS, TITLEBAR_AREAS } from '@anakot/plugin-sdk'
...
```javascript
import { PALETTE_AREA, KEYBINDS_AREA } from '@anakot/plugin-sdk'
...
```javascript
import { THEMES_AREA } from '@anakot/plugin-sdk'
...
```javascript
import { Button, useTheme } from '@anakot/plugin-sdk'
...
```javascript
import { host, requestTheme } from '@anakot/plugin-sdk'
...
```javascript
import { TRANSCRIPT_DIRECTIVE_AREA } from '@anakot/plugin-sdk'
...
`window.anakot.send('get-price eth')` (or a declarative
`<button data-anakot-send="get-price eth">` — no script needed) hands that
...
```javascript
import { Contribute, WORKSPACE_PAGE_HEADER_AREA } from '@anakot/plugin-sdk'
...
for requests; `targetProfile` is the backend Anakot profile served by that route.
...
approval/turn alerts use. It fires only while the user is away from Anakot
...
ctx.os.notify({
  ...
  // Body click → focus Anakot + navigate. Same vocabulary as OS deep links:
  activate: 'anakot://index-network/intent/1',
  ...
  actions: [
    { id: 'open', label: 'Open', activate: 'anakot://index-network/intent/1' },
    { id: 'dismiss', label: 'Dismiss', onAction: () => dismiss('1') },
  ],
})
```

`activate` is deeplink-compatible: `anakot://index-network/intent/1` and the
hash path `/index-network/intent/1` resolve to the same in-app route (and the
same `anakot://…` URL works as an OS deep link). Action buttons only render on
...
```javascript
import { useQuery, useMutation, useQueryClient, atom, computed, useValue } from '@anakot/plugin-sdk'
...
```javascript
import { queryClient } from '@anakot/plugin-sdk'
...
copies that half into `$ANAKOT_HOME/desktop-plugins/<id>/` beside a
`.anakot-package.json` marker, and the renderer loads it through the exact same
pipeline as the standalone disk door (hot reload included):

```
~/.anakot/plugins/<id>/           # ONE installable folder
...
is refreshed when the source `plugin.js` changes (`anakot plugins update`, or
...
a package into `~/.anakot/plugins` is inert on every surface until the user
...
backend, the remote box's `~/.anakot/plugins` is not reachable as a filesystem —
...
the `anakot://` scheme — a plain anchor on your website or README:
...
`anakot-dev://`. Full link reference:
...
`dashboard/` subfolder of a regular Anakot plugin and declare it in a
`manifest.json`:
...
anakot-agent codebase directly (`anakot_state`, `anakot_cli.config`, …). See
...
`config.yaml` (and not in `plugins.disabled`). Project plugins (`./.anakot/`)
...
(`anakot.plugin.<id>.*`) so plugins can't read or clobber each other:
...
export a `AnakotPlugin`). It's discovered by `discoverBundledPlugins()` at boot —
...
1. It goes through the app's Vite build, so you can write **real JSX** and import
   the SDK by its `@anakot/plugin-sdk` alias.
2. It's still lint-fenced to `@anakot/plugin-sdk` + `react` only — no `@/…` app
   internals.
...
- **Only three specifiers resolve:** `@anakot/plugin-sdk`, `react`,
...
| Plugin contract | `AnakotPlugin`, `PluginContext`, `PluginContribution`, `PluginStorage`, `PluginOs`, `PluginRestOptions`, `PluginNativeNotificationInput`, `PluginNotificationAction`, `AnakotOpenTarget`, `Contribution` |
...
### Agents: the `anakot-desktop-plugins` skill

When an agent writes a desktop plugin, it should load the bundled
**`anakot-desktop-plugins`** skill — it carries the same contract as this page in
...
`$ANAKOT_HOME/desktop-plugins/<id>/plugin.js` and the folder name matches the
export `id`. Run ⌘K → **Reload desktop plugins**. Check the app for an error
toast naming the failure, and tail `anakot logs gui -f`.

**"unsupported import" on load.** A disk plugin may only import
`@anakot/plugin-sdk`, `react`, and `react/jsx-runtime`. Remove any other import.
...
`~/.anakot/plugins/<id>/dashboard/manifest.json` has `"api": "plugin_api.py"`,
that the plugin is in `plugins.enabled` in `config.yaml`, and restart the gateway
(backend routes mount at startup). Tail `~/.anakot/logs/errors.log` for
