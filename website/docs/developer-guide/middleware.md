...
Anakot middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Anakot'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Anakot calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Anakot retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Anakot guardrails,
...
- `telemetry_schema_version`: currently `anakot.observer.v1`
- `middleware_schema_version`: currently `anakot.middleware.v1`
...
Anakot stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Anakot runs
them as a nested chain in registration order. Middleware failures are fail-open:
Anakot logs a warning and continues with the next middleware or the base
...
For each provider request, Anakot applies middleware in this order:
...
For each tool call, Anakot applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Anakot pre-execution path against the effective arguments:
...
For isolated local testing, use one `ANAKOT_HOME` for plugin enablement and the
agent run:

```bash
export ANAKOT_HOME=/tmp/anakot-middleware-test
mkdir -p "$ANAKOT_HOME"
anakot plugins enable <plugin-name>
anakot chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run anakot plugins enable <plugin-name>
uv run anakot chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["anakot_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/anakot-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Anakot expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Anakot treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Anakot preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Anakot does not convert downstream
