...
Anakot Agent saves conversation trajectories in ShareGPT-compatible JSONL format
...
It follows the Anakot function-calling prompt template with:
...
Trajectory saving is a `run_agent.py` / library-level switch — the `anakot` CLI
