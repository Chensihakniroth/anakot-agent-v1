...
Anakot Agent uses a dual compression system and Anthropic prompt caching to
...
- **Failed probes use the current table without persisting it.** Failures have a
  five-minute in-memory cooldown scoped to Anakot home, endpoint, model, and
  region. Expiry or explicit cache invalidation permits another attempt.

The cache remains at `context_length_cache.yaml` under the active Anakot home.
...
Configure via `anakot plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](./context-engine-plugin.md).

## Dual Compression System

Anakot has two separate compression layers that operate independently:
...
value is kept per `model@host` in `~/.anakot/cache/image_token_costs.json` and
...
compression:
  ...
  codex_app_server_auto: native  # native|anakot|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `anakot`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
excluded because they already unlock the wider window), Anakot raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$ANAKOT_HOME`
...
```bash
anakot config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
anakot config set compression.codex_gpt55_autoraise_notice false
```
...
for ChatGPT-subscription accounts (live-verified Aug 2026). Anakot keeps the
...
`gpt-5.4-900k`). These are Anakot-side aliases: the suffix is stripped before
...
thread context, so Anakot' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Anakot
  records the resulting compaction events (compression counters, session
  events). Set `anakot` to let Anakot' compression threshold initiate
  app-server compaction, or `off` to disable Anakot-initiated automatic
  compaction entirely (codex may still compact natively).

Anakot' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Anakot' summary compressor.
...
encrypted `compaction` output item. Anakot captures that item into the
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Anakot
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Anakot can avoid; user-facing docs for `/model`, fallback providers, and
