...
description: "How Anakot recovers state.db when the FTS index or the file itself is corrupt"
---
...
`anakot doctor` reports the explicit repair command.
...
repaired or restored file. Do not run `anakot doctor --fix` while the gateway
is still up. Next steps:

```bash
anakot gateway stop
ANAKOT_HOME="$HOME/.anakot" anakot sessions recover --source "$HOME/.anakot/state.db" --inspect-only
# if recoverable:
ANAKOT_HOME="$HOME/.anakot" anakot sessions recover --source "$HOME/.anakot/state.db" --output "$HOME/recovered-state.db"
```
...
```bash
anakot gateway stop
ANAKOT_HOME="$HOME/.anakot" anakot sessions repair --check-only
ANAKOT_HOME="$HOME/.anakot" anakot sessions repair
```
