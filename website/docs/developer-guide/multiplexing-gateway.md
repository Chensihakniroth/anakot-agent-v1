...
by `anakot_cli/gateway_multiplex_mode.py::resolve_multiplex_mode`, which runs
the `anakot gateway migrate` preflight and keeps the gateway standalone when a
...
_profile_runtime_scope(profile_home)           (gateway/run.py)
   ├── set_anakot_home_override(home)          config / state.db / skills /
...
- `get_secret(name)` resolves: global allowlist → active scope → fallback.
  ...
  - multiplexing **on**, no scope installed: **raises `UnscopedSecretError`**
    ...
    value.
- A small allowlist (`ANAKOT_HOME`, `ANAKOT_PROFILE`, proxy settings,
...
reload helper: `anakot_cli.env_loader.load_anakot_dotenv` skips the
...
## The ANAKOT_HOME override

`anakot_constants.py` holds a context-local override consulted by
`get_anakot_home()` before the `ANAKOT_HOME` env var. Everything that resolves
...
automatically. `get_process_anakot_home()` exists for the few machine-level
assets that must not follow the override. `anakot_home_key()` gives
...
`profiles_to_serve(multiplex, profile_allowlist)` in `anakot_cli/profiles.py`
...
DB handles are resolved at call time through the active ANAKOT_HOME override —
...
ANAKOT_HOME override. Asset writes are atomic, type- and size-capped.
...
| Built-in tool registry | Built-ins are process-global; plugin-registered tools are overlaid per profile via `anakot_home_key()`. |
...
- `agent/secret_scope.py`, `anakot_constants.py`, `gateway/profile_routing.py`,
  `gateway/run.py` (`_profile_runtime_scope`), `anakot_cli/profiles.py`
