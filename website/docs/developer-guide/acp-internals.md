...
The ACP adapter wraps Anakot' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
anakot acp / anakot-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.anakot/.env
  -> configure stderr logging
  -> construct AnakotACPAgent
...
- `allow_once` -> Anakot `once`
- `allow_always` -> Anakot `always`
- reject options -> Anakot `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Anakot tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["anakot-acp"])
...
retries, interrupt before any reply) is closed by the core loop with a Anakot-authored
...
Instead it reuses Anakot' runtime resolver:

- `acp_adapter/auth.py`
- `anakot_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Anakot provider/credentials. It also always advertises a terminal setup auth method (`anakot-setup`, args `--setup`) so first-run ACP clients can open Anakot' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.anakot/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `anakot-acp` toolset definition
- `anakot_cli/main.py` — `anakot acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `anakot-acp` script
