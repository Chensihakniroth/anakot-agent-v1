...
description: "Wire contract between the Anakot gateway relay adapter and external connectors (experimental)"
---
...
This document is the formal interface between the **Anakot gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `anakot gateway enroll --wake-url <url>` (or
...
the inbound (`scope_id`, `user_id`) and, on a multiplexed gateway, the Anakot
...
**per-gateway secret** (`anakot gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `anakot` CLI.
