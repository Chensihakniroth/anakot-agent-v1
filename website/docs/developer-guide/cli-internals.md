...
sidebar_position: 15
title: "CLI Internals"
description: "How anakot_cli is shaped: slash dispatch, config loaders, the skin engine, the transactional update pipeline, and process-identity rules"
---

# CLI Internals

Companion to `anakot_cli/AGENTS.md` (the rules) — this page holds the longer explanations.

## Update pipeline

The stage-by-stage contract (`plan → snapshot → apply → restart-per-kind → verify → report`) and the
field failure each stage guards are documented in `anakot_cli/AGENTS.md`; user-facing behaviour
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line` (gateway run),
  `anakot_cli.update_cmd._anakot_holder_subcommand` (top-level subcommand of any Anakot argv). Never
...
Built-in skins (`_BUILTIN_SKINS` in `anakot_cli/skin_engine.py`): `default` (classic gold/kawaii),
...
User skins are `~/.anakot/skins/<name>.yaml` with the same keys, activated with `/skin <name>` or
`display.skin: <name>`; the full YAML template is in the
[Skins & Themes](../user-guide/features/skins.md) user guide.

## Profiles: multi-instance support

Anakot supports profiles — fully isolated instances, each with its own `ANAKOT_HOME` (config, API
keys, memory, sessions, skills, gateway). For single-profile commands (`anakot -p x <cmd>`),
`_apply_profile_override()` in `anakot_cli/main.py` sets `ANAKOT_HOME` before any module imports, so
every `get_anakot_home()` reference scopes to the active profile. The multiplex gateway and the
Desktop/dashboard `serve` backend serve several profiles from one process instead: the active
profile is a contextvar override bound per activity, `os.environ["ANAKOT_HOME"]` stays the launch
...
`Path.home() / ".anakot" / "profiles"`, not `get_anakot_home() / "profiles"`) so
`anakot -p coder profile list` sees all profiles regardless of which one is active — intentional.
