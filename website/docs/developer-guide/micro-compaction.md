...
has to be thrown away or summarized. Anakot has always done this in one batch:
...
Anakot folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Anakot's normal session flush is
...
With `micro_compact` unset or `false` Anakot behaves exactly as it always has:
...
Defaults to `$ANAKOT_HOME/logs/agent.log`. It reports passes, outcome mix, net
