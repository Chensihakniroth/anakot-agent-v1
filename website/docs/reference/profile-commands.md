...
This page covers all commands related to [Anakot profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `anakot profile` without a subcommand shows help.
...
```bash
anakot profile use <name>
```

Sets `<name>` as the active profile. All subsequent `anakot` commands (without `-p`) will use this profile.
...
anakot profile use default
```

## `anakot profile create`

```bash
anakot profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `anakot profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `anakot update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.anakot`), use `anakot skills opt-out` / `anakot skills opt-in`. |
...
anakot profile create work2 --clone-from work

# Clone everything from a specific profile
anakot profile create work2-backup --clone-from work --clone-all
```

## `anakot profile describe`

```bash
anakot profile describe [<name>] [options]
```
...
anakot profile describe researcher

# Set it explicitly
anakot profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
anakot profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
anakot profile describe --all --auto
```

## `anakot profile delete`

```bash
anakot profile delete <name> [options]
```
...
```bash
anakot profile delete mybot
anakot profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.anakot`) cannot be deleted — use `anakot uninstall` to remove everything.
:::

## `anakot profile show`

```bash
anakot profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

The skills count here (and in `anakot profile list`) is counted on the spot. The Desktop and dashboard profile lists are polled every few seconds, so they show the last known count instead and refresh it in the background — a freshly started backend may briefly show `0` skills for a profile until the first background count lands, and a skill you just installed appears in those lists within about a minute.

This shows the profile's Anakot home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ anakot profile show work
Profile: work
Path:    ~/.anakot/profiles/work
...
## `anakot profile alias`

```bash
anakot profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Anakot installation.
...
```bash
anakot profile alias work
# Creates/updates ~/.local/bin/work

anakot profile alias work --name mywork
# Creates ~/.local/bin/mywork

anakot profile alias work --remove
# Removes the wrapper script
```

## `anakot profile rename`

```bash
anakot profile rename <old-name> <new-name>
```
...
```bash
anakot profile rename mybot assistant
# ~/.anakot/profiles/mybot → ~/.anakot/profiles/assistant
...
after the rename; `anakot profile migrate-identity` retries that step too if it was reported as
failed.

## `anakot profile migrate-identity`

```bash
anakot profile migrate-identity <old-name> <new-name>
```

Retries the identity migration of a rename that already completed. Run it if `anakot profile
...
#     anakot profile migrate-identity mybot assistant

anakot profile migrate-identity mybot assistant
# ✓ Session/routing identity migrated: mybot → assistant
```

## `anakot profile purge-identity`

```bash
anakot profile purge-identity <name>
```

Retries the identity purge of a delete that already completed. Run it if `anakot profile delete`
...
is decided by `anakot profile delete`, which removes the profile's own `profiles/<name>/`, its
...
#   Restart the gateway, then run:
#     anakot profile purge-identity mybot

anakot profile purge-identity mybot
# ✓ Session/routing identity purged: mybot
```

## `anakot profile export`

```bash
anakot profile export <name> [options]
```
...
```bash
anakot profile export work
# Creates work.tar.gz in the current directory

anakot profile export work -o ./work-2026-03-29.tar.gz
```
...
## `anakot profile import`

```bash
anakot profile import <archive> [options]
```
...
```bash
anakot profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

anakot profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `anakot -p telemetry`). |
...
anakot profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
anakot profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
anakot profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
anakot profile install ./telemetry/
```

### `anakot profile update`

```bash
anakot profile update <name> [--force-config] [--yes]
```
...
```bash
anakot profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Anakot version, author, env var requirements, the source URL/path, and
...
`anakot profile list` also shows the distribution name and version in a
`Distribution` column, and `anakot profile show <name>` / `delete <name>`
...
anakot profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
anakot profile install https://github.com/your-org/internal-assistant.git
```
...
`anakot_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Anakot version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Anakot can clone from.
3. Tell recipients to run `anakot profile install <your-repo-url>`.
...
## `anakot -p` / `anakot --profile`

```bash
anakot -p <name> <command> [options]
anakot --profile <name> <command> [options]
```

Global flag to run any Anakot command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
anakot -p work chat -q "Check the server status"
anakot --profile dev gateway start
anakot -p personal skills list
anakot -p work config edit
```

## `anakot completion`

```bash
anakot completion <shell>
```
...
- `anakot profile <TAB>` — subcommands (list, use, create, etc.)
- `anakot profile use <TAB>` — profile names
- `anakot -p <TAB>` — profile names
