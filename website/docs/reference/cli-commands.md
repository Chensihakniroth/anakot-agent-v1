...
description: "Authoritative reference for Anakot terminal commands and command families"
---
...
```bash
anakot [global-options] <command> [subcommand/options]
```
...
| `--profile <name>`, `-p <name>` | Select which Anakot profile to use for this invocation. Overrides the sticky default set by `anakot profile use`. |
...
| `--ignore-user-config` | Ignore `~/.anakot/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, memory, and preloaded skills. |
| `--tui` | Launch the [TUI](../user-guide/tui.md) instead of the classic CLI. Equivalent to `ANAKOT_TUI=1`. Always wins over `display.interface`. |
...
| Command | Purpose |
|---------|---------|
| `anakot chat` | Interactive or one-shot chat with the agent. |
| `anakot model` | Interactively choose the default provider and model. |
| `anakot moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `anakot fallback` | Manage fallback providers tried when the primary model errors. |
| `anakot gateway` | Run or manage the messaging gateway service. |
| `anakot proxy` | Local OpenAI-compatible proxy that attaches OAuth provider credentials. See [Subscription Proxy](../user-guide/features/subscription-proxy.md). |
| `anakot egress` | Outbound credential-injection firewall for remote terminal sandboxes (iron-proxy). Disabled by default. See [Egress proxy](../user-guide/egress/iron-proxy.md). |
| `anakot lsp` | Manage Language Server Protocol integration (semantic diagnostics for write_file/patch). |
| `anakot setup` | Interactive setup wizard for all or part of the configuration. |
| `anakot whatsapp` | Configure and pair the WhatsApp bridge. |
| `anakot whatsapp-cloud` | Configure the official Meta WhatsApp Business Cloud API adapter (Business account + public webhook required). Distinct from `anakot whatsapp` (Baileys personal-account bridge). |
| `anakot slack` | Slack helpers (currently: generate the app manifest with every command as a native slash). |
| `anakot auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `anakot login` / `logout` | **Deprecated** — use `anakot auth` instead. |
| `anakot send` | Send a one-shot message to a configured messaging platform (Telegram, Discord, Slack, Signal, SMS, …). Useful from shell scripts, cron jobs, CI hooks, and monitoring daemons — no agent loop, no LLM. |
| `anakot peer` | Register peer Anakot gateways on other machines and DM their agents' canonical Bot Chats (`anakot peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `anakot secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.anakot/.env`. |
| `anakot migrate` | Diagnose and (optionally) rewrite `config.yaml` to replace references to retired models or deprecated settings (e.g. `migrate xai`). |
| `anakot codex-runtime` | Noninteractive counterpart of `/codex-runtime`: `migrate [--dry-run] [--json]` regenerates the Anakot-managed block in `~/.codex/config.toml` for the selected profile. See [Codex app-server runtime](../user-guide/features/codex-app-server-runtime.md#running-the-migration-from-a-script). |
| `anakot status` | Show agent, auth, and platform status. |
| `anakot cron` | Inspect and tick the cron scheduler. |
| `anakot pause` / `anakot resume` | Global emergency stop: no new cron fires (built-in ticker, managed-cron webhook, misfire catch-up), kanban dispatch or gateway turns start until resumed; in-flight work is never killed. |
| `anakot kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
| `anakot project` | Manage named, multi-folder workspaces (projects). Anchors desktop session grouping and, when bound to a kanban board, gives tasks a deterministic worktree + branch convention. State is per-profile. |
| `anakot webhook` | Manage dynamic webhook subscriptions for event-driven activation. |
| `anakot hooks` | Inspect, approve, or remove shell-script hooks declared in `config.yaml`. |
| `anakot doctor` | Diagnose config and dependency issues. |
| `anakot security audit` | On-demand supply-chain audit (OSV.dev) for the venv, plugin requirements, and pinned MCP servers. |
| `anakot approvals` | Approval-prompt tools — mine approval history into allowlist proposals. |
| `anakot dump` | Copy-pasteable setup summary for support/debugging. |
| `anakot prompt-size` | Show a byte breakdown of the system prompt + tool schemas (skills index, memory, profile). Runs offline. |
| `anakot debug` | Debug tools — upload logs and system info for support. |
| `anakot backup` | Back up Anakot home directory to a zip file. |
| `anakot checkpoints` | Inspect / prune / clear `~/.anakot/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `anakot import` | Restore a Anakot backup from a zip file. |
| `anakot logs` | View, tail, and filter agent/gateway/error log files. |
| `anakot config` | Show, edit, migrate, and query configuration files. |
| `anakot skin` | List, switch, and tweak display skins. |
| `anakot console` | Open the safe Anakot command console. |
| `anakot pairing` | Approve or revoke messaging pairing codes. |
| `anakot skills` | Browse, install, publish, audit, and configure skills. |
| `anakot bundles` | Group several skills under a single `/<name>` slash command. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles). |
| `anakot curator` | Background skill maintenance — status, run, pause, pin. See [Curator](../user-guide/features/curator.md). |
| `anakot journey` (aliases `learning`, `memory-graph`) | Timeline of learned skills + memories over time. |
| `anakot memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `anakot honcho`) register automatically when their provider is active. |
| `anakot acp` | Run Anakot as an ACP server for editor integration. |
| `anakot mcp` | Manage MCP server configurations and run Anakot as an MCP server. |
| `anakot plugins` | Manage Anakot Agent plugins (install, enable, disable, remove). |
| `anakot portal` | Nous Portal status, subscription link, and Tool Gateway routing. See [Tool Gateway](../user-guide/features/tool-gateway.md). |
| `anakot tools` | Configure enabled tools per platform. |
| `anakot computer-use` | Install or check the Computer Use (cua-driver) backend (macOS/Windows/Linux). |
| `anakot pets` | Browse, install, and select [petdex](../user-guide/features/pets.md) animated pets shown across the CLI, TUI, and desktop app. Subcommands: `list`, `install`, `select`, `show`, `off`, `scale`, `remove`, `doctor`. |
| `anakot sessions` | Browse, export, prune, rename, and delete sessions. |
| `anakot insights` | Show token/cost/activity analytics. |
| `anakot claw` | OpenClaw migration helpers. |
| `anakot import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `anakot dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `anakot serve` | Start the Anakot backend server (headless; powers the desktop app and remote backends). |
| `anakot desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `anakot profile` | Manage profiles — multiple isolated Anakot instances. |
| `anakot completion` | Print shell completion scripts (bash/zsh/fish). |
| `anakot --version` | Show version information. |
| `anakot update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `ANAKOT_HOME` snapshot. |
| `anakot uninstall` | Remove Anakot from the system. |
...
| `--ignore-user-config` | Ignore `~/.anakot/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Anakot itself. |
...
```bash
anakot
anakot chat -q "Summarize the latest PRs"          # seeds an interactive session
anakot chat --oneshot -q "Summarize the latest PRs"  # answer and exit
anakot chat --provider openrouter --model anthropic/claude-sonnet-4.6
anakot chat --toolsets web,terminal,skills
anakot chat --quiet -q "Return only JSON"
anakot chat -q "Inspect this repository" --format stream-json
anakot chat --worktree -q "Review this repo and open a PR"
anakot chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
anakot chat --safe-mode -q "Is this bug mine or Anakot'?"
```
...
```bash
anakot chat -q "Summarize this repository" --format stream-json
```
...
(`ANAKOT_KANBAN_TASK` set) whose turn failed only because the provider was
...
### `anakot -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `anakot -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
anakot -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(anakot -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.anakot/config.yaml`):

| Flag | Equivalent env var | Purpose |
|---|---|---|
| `-m` / `--model <model>` | `ANAKOT_INFERENCE_MODEL` | Override the model for this run |
...
```bash
anakot -z "…" --provider openrouter --model openai/gpt-5.5
# or:
ANAKOT_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 anakot -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `anakot chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".
...
`anakot -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, the `completed` / `failed` / `partial` / `interrupted` flags and `turn_exit_reason` (why `completed` is false, e.g. `max_iterations_reached(3/3)`). Those top-level counters cover the **main agent loop** only. Auxiliary LLM calls made on the same run (title generation, vision, context compression, `web_extract`, background review, …) are reported separately under `auxiliary` — the same totals plus a per-task `by_task` map — and `total_including_auxiliary` (`estimated_cost_usd`, `total_tokens`, `api_calls`) is the grand total to bill on. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
anakot -z "summarize this repo" --usage-file /tmp/usage.json
...
Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Anakot chat session.
...
:::warning anakot model vs /model — know the difference
**`anakot model`** (run from your terminal, outside any Anakot session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Anakot chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Anakot session first (`Ctrl+C` or `/quit`), then run `anakot model` from your terminal prompt.
:::
...
| `migrate-legacy` | Remove legacy `anakot.service` units left over from pre-rename installs. Profile units (`anakot-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Anakot Relay](../user-guide/messaging/relay.md). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `ANAKOT_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `anakot update`. |
| `--no-supervise` | On `run`: inside the s6-overlay Docker image, opt out of auto-supervision and use pre-s6 foreground semantics — gateway runs as the container's main process with no auto-restart. No-op outside the s6 image. Equivalent to setting `ANAKOT_GATEWAY_NO_SUPERVISE=1`. |
...
`anakot gateway restart`, or service-restart update exits with status `75`
...
`anakot gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `anakot gateway run` instead of `anakot gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s anakot 'anakot gateway run'`. See [WSL FAQ](./faq.md#wsl-gateway-keeps-disconnecting-or-anakot-gateway-start-fails) for details.
:::

## `anakot lsp`

```bash
anakot lsp <subcommand>
```
...
```bash
anakot setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `anakot setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `anakot setup` on an existing install now does this by default. |
...
```bash
anakot portal [status|open|tools]
```
...
For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `anakot setup --portal` above.
...
```bash
anakot slack manifest              # print manifest to stdout
anakot slack manifest --write      # write to ~/.anakot/slack-manifest.json
anakot slack manifest --long-description-file AGENTS.md --write
anakot slack manifest --slashes-only  # just the features.slash_commands array
```
...
|------|---------|---------|
| `--write [PATH]` | stdout | Write to a file instead of stdout. Bare `--write` writes `$ANAKOT_HOME/slack-manifest.json`. |
| `--name NAME` | `Anakot` | Bot display name in Slack. |
...
Run `anakot slack manifest --write` again after `anakot update` to pick
up any new commands.
...
```bash
anakot send --to <target> "message text"
anakot send --to <target> --file <path>
echo "message" | anakot send --to <target>
anakot send --list [platform]
```

Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.anakot/.env` + `~/.anakot/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.

For bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp-CloudAPI) no running gateway is required — `anakot send` talks directly to the platform's REST endpoint. Plugin platforms that need a persistent adapter still require a live gateway.
...
If neither a positional `message` argument nor `--file` is provided, `anakot send` reads from stdin when it is not a TTY. Exit codes: `0` on success, `1` on delivery/backend failure, `2` on usage errors.
...
```bash
anakot send --to telegram "MEDIA:/tmp/screenshot.png"
anakot send --to telegram "Build chart for today MEDIA:/tmp/chart.png"   # with caption
anakot send --to discord:#ops "MEDIA:/tmp/report.pdf"
```
...
```bash
anakot send --to telegram "[[as_document]] MEDIA:/tmp/screenshot.png"
```

Examples:

```bash
anakot send --to telegram "deploy finished"
echo "RAM 92%" | anakot send --to telegram:-1001234567890
anakot send --to discord:#ops --file /tmp/report.md
anakot send --to slack:#eng --subject "[CI]" --file build.log
anakot send --list                  # all platforms
anakot send --list telegram         # filter by platform
```
...
```bash
anakot peer add <name> --url http://host:port --key <API_SERVER_KEY>
anakot peer list
anakot peer dm <peer>[/<agent>] "message"
anakot peer run <peer>[/<agent>] --idempotency-key <key> "message"
anakot peer status <peer>[/<agent>] <run_id>
anakot peer stop <peer>[/<agent>] <run_id>
anakot peer remove <name>
```

Bot-to-bot DMs across machines. Register another Anakot gateway (any machine
running the `api_server` platform) as a *peer*, then message its agents:
`anakot peer dm` resolves the remote agent's canonical **Bot Chat** session
over the peer's API server, runs one agent turn there, and prints the reply
on stdout — the cross-machine twin of the local
`anakot -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `ANAKOT_PEER_<NAME>_KEY` in `~/.anakot/.env`. |
...
includes the peer roster and the `anakot peer dm` pattern, so agents discover
...
```bash
anakot secrets bitwarden <subcommand>
anakot secrets bw <subcommand>          # short alias
```

Pull API keys from an external secret manager at process startup instead of storing them in `~/.anakot/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `anakot claw migrate` (one-shot import of OpenClaw configuration into Anakot) — `anakot migrate` is the top-level config-rewrite command.


## `anakot codex-runtime`

```bash
anakot codex-runtime migrate [--dry-run] [--json]
```

Runs the `~/.codex/config.toml` migration that `/codex-runtime codex_app_server` triggers, without a chat session: Anakot' `mcp_servers` (plus installed codex plugins and the `default_permissions` default) are projected into the managed block for the selected profile (`anakot -p <name> codex-runtime migrate`). User text outside the block is kept verbatim; a user-owned `[mcp_servers.<name>]` with the same name as a Anakot server is preserved and the Anakot projection for that name skipped (reported as `preserved_user_servers`). The result is validated as TOML before an atomic write; exit code is 1 when the report contains errors.
...
```bash
anakot security <subcommand>
```

On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Anakot venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.anakot/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Anakot Python venv. |
| `--skip-plugins` | off | Skip scanning plugin requirements files. |
| `--skip-mcp` | off | Skip scanning pinned MCP servers in `config.yaml`. |


## `anakot login` / `anakot logout` *(Deprecated)*

:::caution
`anakot login` has been removed. Use `anakot auth` to manage OAuth credentials, `anakot model` to select a provider, or `anakot setup` for full interactive setup.
:::
...
```bash
anakot auth                                              # Interactive wizard
anakot auth list                                         # Show all pools
anakot auth list openrouter                              # Show specific provider
anakot auth add openrouter --api-key sk-or-v1-xxx        # Add API key
anakot auth add openrouter --type oauth                  # Browser login (OpenRouter PKCE) mints a key for you
anakot auth add anthropic --type oauth                   # Add OAuth credential
anakot auth add openai-codex --type oauth --priority 0   # Add an account and try it first
anakot auth remove openrouter 2                          # Remove by index
anakot auth priority openrouter backup-key 0             # Move a credential to the front of fill_first order
anakot auth reset openrouter                             # Clear cooldowns
anakot auth reset openrouter 2                           # Clear the cooldown on one credential
anakot auth refresh openai-codex work                    # Refresh one OAuth credential and clear its cooldown
anakot auth status anthropic                             # Show auth status for a provider
anakot auth logout anthropic                             # Log out and clear stored auth state
anakot auth spotify                                      # Authenticate Anakot with Spotify via PKCE
```
...
```bash
anakot cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
`$ANAKOT_HOME/plugins/<name>/`. An unknown or unavailable provider falls back to
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.anakot/kanban/current`). |
...
anakot kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
anakot kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
anakot kanban boards switch atm10-server
anakot kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
anakot kanban boards rm atm10-server
anakot kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `ANAKOT_KANBAN_BOARD` env var → `~/.anakot/kanban/current` file → `default`.
...
```bash
anakot egress install                  # download the pinned iron-proxy binary
anakot egress install --force          # re-download even if already installed

anakot egress setup                    # interactive wizard: CA, mappings, config
anakot egress setup --tunnel-port N    # override the tunnel listener port (default 9090)
anakot egress setup --from-bitwarden   # use Bitwarden Secrets Manager as credential source
anakot egress setup --no-bitwarden     # explicitly switch back to env-based credentials
anakot egress setup --rotate-tokens    # mint fresh proxy tokens (default preserves existing)

anakot egress start                    # spawn the managed proxy daemon
anakot egress stop                     # SIGTERM (then SIGKILL after 5s grace)
anakot egress restart                  # stop (if running) then start — needed for secret changes
anakot egress reload                   # hot-reload the ruleset in-place (no restart, no dropped
                                       #   connections) via the loopback management API

anakot egress status                   # binary + config + pid + listening + mappings
anakot egress status --show-tokens     # print proxy tokens in full (default: redacted)

anakot egress disable                  # flip proxy.enabled = false (does not stop a running proxy)
anakot egress config                   # print the path to proxy.yaml for inspection
```
...
anakot egress setup && anakot egress start
anakot config set terminal.backend docker   # if not already

# Switching credential source after the fact
anakot egress setup --from-bitwarden       # env → bitwarden
anakot egress setup --no-bitwarden         # bitwarden → env
# (just `setup` without either flag preserves the existing mode)

# Rotating all tokens (e.g. after a suspected token leak)
anakot egress setup --rotate-tokens    # setup offers to restart the running daemon for you
# (running sandboxes still hold old tokens; restart them too)

# Adding a new upstream
# Edit ~/.anakot/config.yaml proxy.extra_allowed_hosts: [api.example.com]
anakot egress setup
anakot egress restart                  # one-command apply (stop + start)
```

### Diagnostic shortcuts

```bash
anakot egress status                     # current state in one view
cat ~/.anakot/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.anakot/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.anakot/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
```bash
anakot project <create|list|show|add-folder|remove-folder|rename|set-primary|use|archive|restore|bind-board>
```
...
```bash
anakot webhook <subscribe|list|remove|test>
```
...
### `anakot webhook subscribe`

```bash
anakot webhook subscribe <name> [options]
```
...
| `--script` | Filter/transform script under `~/.anakot/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |
...
Subscriptions persist to `~/.anakot/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart. Re-running `subscribe` for an existing name keeps its secret and profile binding unless you pass `--secret` / `--route-profile`.
...
```bash
anakot dump [--show-keys]
```

Outputs a compact, plain-text summary of your entire Anakot setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Anakot version, release date, git commit hash |
| **Environment** | OS, Python version, OpenAI SDK version |
| **Identity** | Active profile name, ANAKOT_HOME path |
...
:::tip
`anakot dump` is specifically designed for sharing. For interactive diagnostics, use `anakot doctor`. For a visual overview, use `anakot status`.
:::

## `anakot debug`

```bash
anakot debug share [options]
```
...
The report includes system info (OS, Python version, Anakot version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
anakot debug share              # Upload debug report, print URL
anakot debug share --lines 500  # Include more log lines
anakot debug share --expire 30  # Keep paste for 30 days
anakot debug share --nous       # Upload a private diagnostics bundle for Nous support
anakot debug share --local      # Print report to terminal (no upload)
```

## `anakot backup`

```bash
anakot backup [options]
```

Create a zip archive of your Anakot configuration, skills, sessions, and data. The backup excludes the anakot-agent codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.

| Option | Description |
|--------|-------------|
| `-o`, `--output <path>` | Output path for the zip file (default: `~/anakot-backup-<timestamp>.zip`). |
| `-q`, `--quick` | Quick snapshot: only critical state files (config.yaml, state.db, .env, auth, cron jobs). Much faster than a full backup. |
| `-l`, `--label <name>` | Label for the snapshot (only used with `--quick`). |
| `-k`, `--keep <N>` | After a full backup, delete older `anakot-backup-*.zip` files in the output directory beyond the newest N (default 3; `0` keeps everything). Custom-named zips are never touched. |

The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Anakot is running (WAL-mode safe).
...
- `models/`, `runtimes/`, `node/` at the root of `~/.anakot` (and of each `profiles/<name>/`) — regenerable runtime downloads, often tens of GB. Deeper directories with the same names (a skill's `models/`) are kept.
...
- The `anakot-agent` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
anakot backup                           # Full backup to ~/anakot-backup-*.zip
anakot backup -o /tmp/anakot.zip        # Full backup to specific path
anakot backup --quick                   # Quick state-only snapshot
anakot backup --quick --label "pre-upgrade"  # Quick snapshot with label
```

## `anakot checkpoints`

```bash
anakot checkpoints [COMMAND]
```

Inspect and manage the shadow git store at `~/.anakot/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.

| Subcommand | Description |
|------------|-------------|
| `status` (default) | Show total size, project count, and per-project breakdown. Bare `anakot checkpoints` is equivalent. |
...
```bash
anakot checkpoints                                  # status overview
anakot checkpoints prune --retention-days 3         # aggressive cleanup
anakot checkpoints prune --max-size-mb 200          # tighten size cap once
anakot checkpoints clear-legacy -f                  # drop v1 archive dirs
anakot checkpoints clear -f                         # wipe everything
```
...
```bash
anakot import <zipfile> [options]
```

Restore a previously created Anakot backup into your Anakot home directory. All files in the archive overwrite existing files in your Anakot home; `--force` only skips the confirmation prompt that fires when the target already has a Anakot installation.
...
```
  ⚠ Session data replaced by older backup contents:
    ...
    Recover from a newer backup or snapshot: anakot snapshot list
```

### Examples
```bash
anakot import ~/anakot-backup-20260423.zip           # Prompts before overwriting existing config
anakot import ~/anakot-backup-20260423.zip --force   # Overwrite without prompting
```

## `anakot logs`

```bash
anakot logs [log_name] [options]
```

View, tail, and filter Anakot log files. All logs are stored in `~/.anakot/logs/` (or `<profile>/logs/` for non-default profiles).
...
anakot logs gateway -n 100

# Show only warnings and errors from the last hour
anakot logs --level WARNING --since 1h

# Filter by a specific session
anakot logs --session abc123

# Follow errors.log, starting from 30 minutes ago
anakot logs errors --since 30m -f
...
anakot logs --level WARNING --since 2h --session tg-12345
```
...
Anakot uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `anakot logs list` subcommand shows all log files including rotated ones.


## `anakot prompt-size`

```bash
anakot prompt-size [--platform <name>] [--json]
```
...
- **Prompt tiers** — stable / context / volatile, matching how Anakot layers
...
anakot prompt-size --platform telegram

# Machine-readable output for scripts
anakot prompt-size --json
```

:::tip
The skills index and tool schemas scale with how many skills and tools you have
enabled. To shrink the prompt, disable unused toolsets (`anakot tools`) or
uninstall skills you don't need (`anakot skills`). Context files (AGENTS.md,
...
`anakot config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `anakot config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `anakot config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
anakot pairing <list|approve|revoke|clear-pending>
```
...
```bash
anakot skills browse
anakot skills browse --source official
anakot skills search react --source skills-sh
anakot skills search https://mintlify.com/docs --source well-known
anakot skills inspect official/security/1password
anakot skills inspect skills-sh/vercel-labs/json-render/json-render-react
anakot skills install official/migration/openclaw-migration
anakot skills install skills-sh/anthropics/skills/pdf --force
anakot skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
anakot skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
anakot skills check
anakot skills update
anakot skills config
anakot skills reset google-workspace
anakot skills reset google-workspace --restore --yes
anakot skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
anakot skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
anakot skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Anakot at a site exposing `/.well-known/skills/index.json`.
...
```bash
anakot bundles <subcommand>
```

Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.anakot/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.anakot/skill-bundles/` and report added/removed bundles |

Examples:

```bash
anakot bundles create backend-dev \
...
anakot bundles list
anakot bundles show backend-dev
anakot bundles delete backend-dev
```
...
| `backup` | Take a manual tar.gz snapshot of `~/.anakot/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.anakot/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `anakot update`. Use `anakot curator run --dry-run` to preview before that happens.
...
```bash
anakot moa list
anakot moa configure [name]
anakot moa delete <name>
```

`anakot moa configure` reuses Anakot' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.

## `anakot fallback`

```bash
anakot fallback <subcommand>
```
...
| `add` | Pick a provider + model (same picker as `anakot model`) and append to the chain |
...
```bash
anakot hooks <subcommand>
```

Inspect shell-script hooks declared in `~/.anakot/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.anakot/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `anakot <provider>` command for provider-specific management (e.g. `anakot honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `anakot --help` to see what's currently wired in.
:::
...
Starts Anakot as an ACP (Agent Client Protocol) stdio server for editor integration.
...
```bash
cd ~/.anakot/anakot-agent && uv pip install -e '.[acp]'
```
...
```bash
anakot mcp <subcommand>
```

Manage MCP (Model Context Protocol) server configurations and run Anakot as an MCP server.
...
| `install <name>` | Install a catalog entry (e.g. `anakot mcp install deepwiki`). |
| `serve [-v\|--verbose]` | Run Anakot as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Anakot](../guides/use-mcp-with-anakot.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-anakot-as-an-mcp-server).

## `anakot plugins`

```bash
anakot plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `anakot plugins` with no subcommand opens a composite interactive screen with two sections:
...
| `install <identifier> [--force] [--ref COMMIT_SHA] [--allow-removed]` | Install a plugin from the Anakot plugin catalog (bare entry name), a Git URL, or `owner/repo` shorthand. Catalog names resolve to the entry's repo at its pinned 40-hex commit SHA, show the declared capability summary, and record catalog provenance in a `.anakot-catalog.json` sidecar. Raw URLs are flagged as custom (unreviewed) sources; `--ref` (full 40-character commit SHA) pins them. `--allow-removed` (DANGEROUS) bypasses the removed-plugin blocklist. |
| `search [term] [--json]` | Search the Anakot plugin catalog (matches entry names, descriptions, and declared tools; omit `term` to list everything). The catalog is curated in-repo (`plugin-catalog/`), refreshed from the live repo with a 6-hour cache, and falls back to the in-tree copy offline. Cataloged ≠ audited — admission reviews the entry, not the code. |
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Anakot Plugin](../developer-guide/plugins/index.md).

## `anakot tools`

```bash
anakot tools [--summary]
```
...
```bash
anakot computer-use <subcommand>
```
...
`anakot computer-use install` is the stable entry point for installing the
[cua-driver](https://github.com/trycua/cua) binary used by the
`computer_use` toolset. It runs the same upstream installer that
`anakot tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Anakot checks its version and runtime
...
installer. Anakot never replaces a custom binary selected through
`ANAKOT_CUA_DRIVER_CMD`; update that binary directly or remove the override.
`anakot computer-use status` reports when repair is required.

The built-in `computer_use` toolset is the recommended Anakot integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Anakot and links Cua's
skill pack into the Anakot skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Anakot passes Cua's canonical
...
`anakot update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Anakot update.

## `anakot pets`

```bash
anakot pets <list|install|select|show|off|scale|remove|doctor>
```

[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Anakot shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
```bash
anakot insights [--days N] [--source platform]
```
...
```bash
anakot claw migrate [options]
```

Migrate your OpenClaw setup to Anakot. Reads from `~/.openclaw` (or a custom path) and writes to `~/.anakot`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Anakot files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.anakot/` (by default a single restore-point archive is written to `~/.anakot/backups/pre-migration-*.zip` before apply; restorable with `anakot import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Anakot equivalents or **archived** for manual review.
...
anakot claw migrate --dry-run

# Full migration (all compatible settings, no secrets)
anakot claw migrate --preset full

# Full migration including API keys
anakot claw migrate --preset full --migrate-secrets

# Migrate user data only (no secrets), overwrite conflicts
anakot claw migrate --preset user-data --overwrite

# Migrate from a custom OpenClaw path
anakot claw migrate --source /home/user/old-openclaw
```

## `anakot import-agent`

```bash
anakot import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Anakot. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.anakot/skills/`. Always previews before applying; API keys and credentials are never imported.
...
Every successful import registers its source in `~/.anakot/import-sync.json`; `anakot import-agent --sync` then re-imports any registered source whose files changed (a cron-friendly way to keep an imported Claude Code / Codex setup current). See the **[import guide](../user-guide/import-from-other-agents.md)** for the full mapping tables.

## `anakot serve`

```bash
anakot serve [options]
```

Start the Anakot **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](../user-guide/desktop.md) and remote clients connect to. It is the same server `anakot dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `anakot serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `anakot dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `anakot serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `ANAKOT_BACKEND_READY port=<port>`).

## `anakot dashboard`

```bash
anakot dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`anakot serve`](#anakot-serve) above.) Requires `cd ~/.anakot/anakot-agent && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.anakot/anakot-agent && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](../user-guide/features/web-dashboard.md) for full documentation.
...
| `--stop` | — | Stop the running `anakot dashboard` / `anakot serve` backend **of this Anakot home** and exit (`-p <profile>` / `ANAKOT_HOME` select which; backends of other profiles, other installs on the machine, and the shell you typed the command into are never touched — a backend whose owner cannot be read is left alone). SIGTERM, a 10s grace, then SIGKILL; a hosted Chat TUI that outlives its backend is stopped too (it would otherwise keep the deleted `state.db-wal` open and block the next start). Messaging-gateway bots started from the dashboard are not touched. |
| `--status` | — | List running `anakot dashboard` processes and exit. |

### `anakot dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `ANAKOT_DASHBOARD_OAUTH_CLIENT_ID` into `~/.anakot/.env`, and prints how to engage the login gate. Requires being logged in (`anakot setup`).

| Option | Description |
|--------|-------------|
| `--name` | Human-readable label for the dashboard (default: auto-generated). |
| `--redirect-uri` | Public HTTPS OAuth redirect URI (e.g. `https://anakot.example.com/auth/callback`). Omit for localhost-only use. |
| `--portal-url` | Override the Nous Portal base URL for registration (default: the portal you logged into). Also settable via `ANAKOT_DASHBOARD_PORTAL_URL`. |
...
anakot dashboard --port 8080 --no-open
...
```bash
anakot profile <subcommand>
```

Manage profiles — multiple isolated Anakot instances, each with its own config, sessions, skills, and home directory.
...
```bash
anakot profile list
anakot profile create work --clone
anakot profile create work --clone --sync-imports   # also carry over the import-agent sync manifest
anakot profile use work
anakot profile alias work --name h-work
anakot profile export work -o work-backup.tar.gz
anakot profile import work-backup.tar.gz --name restored
anakot profile install github.com/user/my-distro --alias
anakot profile update work
anakot -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Anakot commands, subcommands, and profile names.

Examples:

```bash
# Bash
anakot completion bash >> ~/.bashrc

# Zsh
anakot completion zsh >> ~/.zshrc

# Fish
anakot completion fish > ~/.config/fish/completions/anakot.fish
```

## `anakot update`

```bash
anakot update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `anakot-agent` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`anakot update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Anakot may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--backup` | Force a **full** pre-update backup for this run: the quick state snapshot plus a complete zip of `ANAKOT_HOME` (config, auth, sessions, skills, pairing data). The default mode is `quick` — a lightweight state snapshot only. Set the permanent mode via `updates.pre_update_backup: quick | full | off` in `config.yaml`. |
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `anakot config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Anakot attempts to restart all running gateway profiles automatically so they pick up the new code. Use `anakot gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.anakot/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Anakot makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `anakot update`.
- **Pairing data snapshot.** Even when `--backup` is off, `anakot update` takes a lightweight snapshot of `~/.anakot/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `anakot backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `anakot.service` warning.** If Anakot detects a pre-rename `anakot.service` systemd unit (instead of the current `anakot-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `anakot --version` | Print version information. |
| `anakot update` | Pull latest changes and reinstall dependencies. |

| `anakot uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Anakot, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
