...
Let the anakot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where anakot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `anakot meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `anakot meet` CLI |
...
| `cli.py` | `anakot meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$ANAKOT_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `anakot meet node {run,list,approve,remove,status,ping}` |
...
anakot meet install                                      # pip + Chromium
anakot meet setup                                        # preflight
anakot meet auth                                         # optional
anakot meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
anakot meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.anakot/.env
anakot meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
anakot meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
anakot meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.anakot/.env
anakot meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, anakot will **not** switch your system audio input automatically — the
...
admission the bot unmutes itself if Meet seated it muted; `status.json` / `anakot meet
...
anakot plugins enable google_meet
anakot meet node run --display-name my-mac --host 0.0.0.0 --port 18789
