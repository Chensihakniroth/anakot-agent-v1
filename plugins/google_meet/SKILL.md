...
anakot meet install                 # pip deps + Chromium (transcribe only)
anakot meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
anakot meet auth                    # optional; skips guest-lobby wait
anakot meet setup                   # preflight checks
```

`anakot meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or ANAKOT_MEET_REALTIME_KEY in ~/.anakot/.env
```
...
pip install playwright websockets && python -m playwright install chromium
anakot plugins enable google_meet
anakot meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
anakot meet node approve my-mac ws://<mac-ip>:18789 <token>
anakot meet node ping my-mac                   # confirm reachable
```

Run `anakot meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A anakot agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `anakot meet auth` avoids this.
- **Lobby timeout**: if the host doesn't admit the bot within 5 minutes (configurable via `ANAKOT_MEET_LOBBY_TIMEOUT` env), the bot leaves and `meet_status` reports `leaveReason: "lobby_timeout"`.
...
| `realtimeDevice` | Audio device name the bot is feeding (e.g. `anakot_meet_src`). |
...
Local:
```
$ANAKOT_HOME/workspace/meetings/<meeting-id>/transcript.txt
```
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$ANAKOT_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `anakot meet node approve`.
