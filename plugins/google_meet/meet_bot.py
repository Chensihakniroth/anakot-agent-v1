...
Standalone subprocess spawned by ``process_manager.py``; configured via ``ANAKOT_MEET_*`` env,
status + transcript written under ``$ANAKOT_MEET_OUT_DIR`` (filesystem is the only IPC).
...
Debug: ``ANAKOT_MEET_URL=... ANAKOT_MEET_OUT_DIR=/tmp/x ANAKOT_MEET_HEADED=1 \\
...
# collects {speaker, text}; ``window.__anakotMeetDrain()`` pulls new entries.
_CAPTION_OBSERVER_JS = r"""
(() => {
  if (window.__anakotMeetInstalled) return;
  window.__anakotMeetInstalled = true;
  window.__anakotMeetQueue = [];
  ...
  window.__anakotMeetDrain = () => {
    const out = window.__anakotMeetQueue.slice();
    window.__anakotMeetQueue = [];
    return out;
  };
})();
...
_ADMISSION_PROBE_JS = r"""
    (() => {
      ...
      if (window.__anakotMeetInstalled && document.querySelector(
...
def _start_pcm_pump(rt: dict, bridge_info: dict, pcm_path: Path, state: "_BotState",
    ...
    if platform_tag == "linux":
        cmd = ["paplay", "--raw", "--rate=24000", "--format=s16le", "--channels=1",
               f"--device={target or 'anakot_meet_sink'}", "-"]
...
def _setup_realtime(rt: dict, api_key: str, state: _BotState) -> None:
    ...
    if not api_key:
        state.set(error="realtime mode requested but no API key in ANAKOT_MEET_REALTIME_KEY/OPENAI_API_KEY — falling back to transcribe")
        rt["enabled"] = False
        return
    try:
...
_BotConfig = SimpleNamespace  # everything the bot reads from ``ANAKOT_MEET_*`` env vars


def _config_from_env() -> _BotConfig:
    env = os.environ.get
    out_raw = env("ANAKOT_MEET_OUT_DIR", "").strip()
    return _BotConfig(
        url=env("ANAKOT_MEET_URL", "").strip(),
        out_dir=Path(out_raw) if out_raw else None,
        headed=env("ANAKOT_MEET_HEADED", "").lower() in {"1", "true", "yes"},
        auth_state=env("ANAKOT_MEET_AUTH_STATE", "").strip(),
        guest_name=env("ANAKOT_MEET_GUEST_NAME", "Anakot Agent"),
        duration_s=_parse_duration(env("ANAKOT_MEET_DURATION", "")),
        realtime=env("ANAKOT_MEET_MODE", "transcribe").strip().lower() == "realtime",
        # ANAKOT_MEET_REALTIME_KEY is resolved by process_manager.start() via the parent's
        # profile secret scope; OPENAI_API_KEY only serves standalone `python -m` runs.
        realtime_api_key=env("ANAKOT_MEET_REALTIME_KEY") or env("OPENAI_API_KEY", ""),
        realtime_model=env("ANAKOT_MEET_REALTIME_MODEL", "gpt-realtime"),
        realtime_voice=env("ANAKOT_MEET_REALTIME_VOICE", "alloy"),
        realtime_instructions=env("ANAKOT_MEET_REALTIME_INSTRUCTIONS", ""),
        lobby_timeout=float(env("ANAKOT_MEET_LOBBY_TIMEOUT", "300")))
    ...
    while not stop_flag["stop"]:
        ...
        if not state.in_call and (now - last_admission_check) > 3.0:
            ...
            if _probe(page, _ADMISSION_PROBE_JS):
                ...
                return
        try:
            queued = page.evaluate("window.__anakotMeetDrain && window.__anakotMeetDrain()")
...
def run_bot() -> int:
    ...
    if not _is_safe_meet_url(cfg.url):
        sys.stderr.write("google_meet bot: refusing to launch — ANAKOT_MEET_URL must be a "
                         "meet.google.com URL. got: %r\n" % cfg.url)
        return 2
    if cfg.out_dir is None:
        sys.stderr.write("google_meet bot: ANAKOT_MEET_OUT_DIR is required\n")
        return 2
