"""CLI commands for the google_meet plugin (``anakot meet <subcommand>``).
...
def _auth_state_path() -> Path:
    return Path(get_anakot_home()) / "workspace" / "meetings" / "auth.json"


# ``anakot meet <sub>`` in help order.
_SUBCOMMAND_HELP = (
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    """Build the ``anakot meet`` argparse tree (called at plugin load time)."""
    ...
    p["join"].add_argument("--guest-name", default="Anakot Agent")
...
_DISPATCH = {
    ...
    "stop": lambda a: _print_result(pm.stop(reason="anakot meet stop")),
...
def meet_command(args: argparse.Namespace) -> int:
    ...
    if not sub:
        print("usage: anakot meet {setup,auth,join,status,transcript,say,stop,node}")
        return 2
...
def _cmd_setup() -> int:
    ...
    print("  google auth    : " + (f"ok ({auth_path})" if auth_path.is_file() else "not saved — run: anakot meet auth"))
    print()
    all_ok = system_ok and pw_ok and chromium_ok
    print("ready. Join a meeting:  anakot meet join https://meet.google.com/abc-defg-hij" if all_ok
    ...
    if not realtime:
        ...
        if system == "Linux":
            ...
            print("\n  NOTE: macOS does not auto-route audio. Open\n    System Settings → Sound → "
                  ...
                  "anakot will not switch your default input for you.")
    print("\ndone. verify with: anakot meet setup")
    return 0
...
def _cmd_auth() -> int:
    ...
    try:
        ...
        return 1
    print("saved. you can now run: anakot meet join <url>")
    return 0
...
if __name__ == "__main__":  # pragma: no cover
    parser = argparse.ArgumentParser(prog="anakot meet")
    register_cli(parser)
