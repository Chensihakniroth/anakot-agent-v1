"""disk_cleanup — ephemeral file cleanup for Anakot Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under ANAKOT_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly ANAKOT_HOME and /tmp/anakot-*
Never touches: ~/.anakot/logs/ or any system directory.
"""
...
try:
    ...
    def get_anakot_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("ANAKOT_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".anakot").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$ANAKOT_HOME/logs/``."""
    return get_anakot_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$ANAKOT_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under ANAKOT_HOME or ``/tmp/anakot-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    anakot_home = get_anakot_home()
    try:
        path.resolve().relative_to(anakot_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/anakot-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("anakot-"):
        return True
    return False
...
# Paths under $ANAKOT_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    """
    # Lazily build the set once per process so ANAKOT_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        anakot_home = get_anakot_home()
        for parent in ("cron", "cronjobs"):
            base = anakot_home / parent
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under ANAKOT_HOME (but leave ANAKOT_HOME itself and
    ...
    empty_removed = 0
    try:
        for dirpath in sorted(anakot_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == anakot_home:
                continue
            try:
                rel_parts = dirpath.relative_to(anakot_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    anakot_home = get_anakot_home()
    try:
        rel = path.resolve().relative_to(anakot_home)
        ...
        # Path isn't under ANAKOT_HOME (e.g. /tmp/anakot-*) — fall through.
        pass
