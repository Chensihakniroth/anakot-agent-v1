...
Auto-tracks and cleans up ephemeral files created during Anakot Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$ANAKOT_HOME` and `/tmp/anakot-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `ANAKOT_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `ANAKOT_HOME` or `/tmp/anakot-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$ANAKOT_HOME/disk-cleanup/` is itself excluded
- `$ANAKOT_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
