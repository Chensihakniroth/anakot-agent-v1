...
models from :func:`anakot_cli.models._fetch_deepinfra_models_by_tag`; no ids hardcoded).
...
class DeepInfraImageGenProvider(StaticImageGenProvider):
    ...
    def generate(self, prompt: str, aspect_ratio: str = DEFAULT_ASPECT_RATIO, **kwargs: Any) -> Dict[str, Any]:
        ...
        if not api_key:
            return fail(
                "DEEPINFRA_API_KEY not set. Run `anakot tools` → Image "
                "Generation → DeepInfra to configure, or `anakot setup` "
                "to add the key.",
                "auth_required")
        ...
        from anakot_cli.models import deepinfra_base_url
