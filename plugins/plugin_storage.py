"""Per-plugin persistent storage: ``<anakot home>/plugin-data/<name>/``.

Plugins must NOT park state in ``<anakot home>/plugins/<name>/`` (the install dir, deleted by
...
# Mirrors the plugin-name shape `anakot plugins install` accepts (no separators/traversal).
...
def plugin_data_dir(name: str) -> Path:
    """Return (and create) ``<anakot home>/plugin-data/<name>/``; resolves ``get_anakot_home()`` on
    ...
    root = get_anakot_home() / "plugin-data" / _validate_name(name)
