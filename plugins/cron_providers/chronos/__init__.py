...
class ChronosCronScheduler(CronScheduler):
    ...
    def _note_identity_rejected(self) -> None:
        ...
        ``agent:*`` client or the hosted bootstrap session, and a plain ``anakot auth`` login is
        ...
        logger.warning(
            ...
            "or the hosted bootstrap session), so no job can be armed. A normal `anakot auth` re-login "
        ...
        if self._stop_event is None:
            return  # start() never ran (e.g. a CLI `anakot cron add`); nothing to tick here
