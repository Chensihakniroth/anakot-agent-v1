...
Config: $ANAKOT_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/
...
_RETAIN_CONTEXT_DEFAULT = "conversation between Anakot Agent and the User"
...
def _load_config() -> dict:
    """$ANAKOT_HOME/hindsight/config.json (profile-scoped), else ~/.hindsight/config.json
    (legacy, shared), else environment variables."""
    for path in (get_anakot_home() / "hindsight" / "config.json", Path.home() / ".hindsight" / "config.json"):
    ...
    return {
        ...
        "banks": {"anakot": {"bankId": get_secret("HINDSIGHT_BANK_ID", "") or "anakot",
                             "budget": os.environ.get("HINDSIGHT_BUDGET", "mid"), "enabled": True}},
    }


def _event_timestamp() -> str:
    """Configured Anakot event time with an explicit UTC offset."""
    event_time = _anakot_now()
    # anakot_time.now() is aware; guard a replacement clock emitting offset-less dates.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._bank_id, self._budget, self._bank_id_template = "anakot", "mid", ""
    ...
    def save_config(self, values, anakot_home):
        """Merge *values* into $ANAKOT_HOME/hindsight/config.json."""
        from utils import atomic_json_write
        config_path = Path(anakot_home) / "hindsight" / "config.json"
    ...
    def post_setup(self, anakot_home: str, config: dict) -> None:
        ...
        run_setup(self, anakot_home, config)
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "anakot"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: anakot-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Anakot Agent and the User"},
    ...
    def _new_embedded_client(self):
        ...
        logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                     cfg.get("profile", "anakot"), llm_provider)
        ...
        kwargs = dict(profile=cfg.get("profile", "anakot"), llm_provider=llm_provider,
    ...
    def _apply_connection_settings(self, cfg: dict) -> None:
        ...
        banks = cfg_get(cfg, "banks", "anakot", default={})
        self._bank_id_template = cfg.get("bank_id_template", "") or ""
        self._bank_id = _resolve_bank_id_template(
            self._bank_id_template,
            fallback=cfg.get("bank_id") or banks.get("bankId", "anakot"),
    ...
    def _start_embedded_daemon(self) -> None:
        ...
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            msg = ("Hindsight local_embedded mode cannot run as root "
                   ...
                   "memory daemon. Run Anakot as a non-root user, or switch "
                   "to cloud / local_external mode via 'anakot memory setup'.")
            logger.warning(msg)
            # Surface to the terminal too — a daemon that never starts would otherwise fail silently and
            # the user would only see Anakot get sluggish (issue #13125). This is an automatic
    ...
    def _daemon_start_worker(self) -> None:
        import traceback
        log_path = get_anakot_home() / "logs" / "hindsight-embed.log"
        ...
        try:
            ...
            profile = self._config.get("profile", "anakot")
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        # See Chensihakniroth/anakot-agent-v1#5820.
    ...
    def on_session_switch(self, new_session_id: str, *, parent_session_id: str = "",
        ...
        document. See anakot-agent#6672.
