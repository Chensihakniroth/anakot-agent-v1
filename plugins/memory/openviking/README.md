...
- OpenViking server running and reachable from Anakot

OpenViking 0.2.10 or newer is recommended. For backward compatibility,
Anakot can identify older servers that expose the legacy status-only health
...
connection values into Anakot, or create a minimal `ovcli.conf` when one does
not exist.
...
default profile that is `~/.anakot/.env`; for a named profile use
...
OpenViking's server config is separate from Anakot:
...
Anakot-side provider config is read from environment variables in the active
profile's `.env`:
...
When `OPENVIKING_API_KEY` is set, Anakot lets OpenViking derive account/user
identity from the key. In local or trusted deployments without an API key,
Anakot sends `OPENVIKING_ACCOUNT` and `OPENVIKING_USER` as identity headers.
Anakot also sends `User-Agent: openviking-memory-anakot/<version>` on
OpenViking requests. This standard harness identifier contains the Anakot
...
does not ask for a peer ID. Without a configured peer, Anakot sends neither
...
order remains environment, linked OpenViking config, then Anakot YAML. To use
...
on the old implicit `anakot` peer now use user memory for new writes. Without
...
Set `agent: anakot` to restore peer-scoped writes. Memories written at user
...
`viking_remember` creates a one-shot `anakot-remember-<random>` OpenViking
...
not advertised or used. The one-shot session is separate from the live Anakot
...
Use the same OpenViking profile and credentials as Anakot for manual recovery.
...
Anakot built-in `memory` tool additions are mirrored to OpenViking after the
local memory operation succeeds:

| Anakot action | OpenViking operation |
|---------------|----------------------|
| `add` | `content/write` with `mode=create` under user memory, or the configured peer memory directory |

Built-in `replace` and `remove` operations are not mirrored because Anakot
...
file URIs, such as
`viking://user/default/peers/anakot/memories/preferences/mem_abc123.md` (any
