...
- Hosted: API key from [app.supermemory.ai/integrations?connect=anakot](http://app.supermemory.ai/integrations?connect=anakot)
...
Before running `anakot memory setup`, add the local endpoint to
...
Then run `anakot memory setup` and enter the local server's API key. Configuring
the endpoint first ensures the setup connection probe also stays local.

## Config

Config file: `$ANAKOT_HOME/supermemory.json`
...
| `container_tag` | `anakot` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `anakot-{identity}` → `anakot-coder`). |
...
`https://api.supermemory.ai`. Anakot resolves it once and uses the same endpoint
...
All Supermemory API calls send `x-sm-source: anakot`, and document writes stamp
`metadata.sm_source: anakot`. This is a **functional routing key, not telemetry**:
it groups Anakot-written memories into a dedicated "Anakot" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Anakot profile:

```json
{
  "container_tag": "anakot-{identity}"
}
```

For a profile named `coder`, this resolves to `anakot-coder`. The default profile resolves to `anakot-default`. Without `{identity}`, all profiles share the same container.
