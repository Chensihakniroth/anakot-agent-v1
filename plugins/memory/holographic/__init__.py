"""anakot-memory-store — holographic memory plugin (MemoryProvider): structured fact storage with entity
resolution, trust scoring, and HRR-based compositional retrieval. Original plugin by dusterbloom (PR #2351).
Config in $ANAKOT_HOME/config.yaml under plugins.anakot-memory-store: db_path ($ANAKOT_HOME/memory_store.db),
...
def _load_plugin_config() -> dict:
    try:
        from anakot_cli.config import load_config_readonly  # canonical: managed-scope overlay + ${VAR} expansion
        return cfg_get(load_config_readonly(), "plugins", "anakot-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, anakot_home):
        """Write config to config.yaml under plugins.anakot-memory-store."""
        # The canonical writer: config lock, managed-mode refusal, default stripping, atomic replace.
        # ``merge_existing`` keeps every other section; *anakot_home* is the active profile already.
        from anakot_cli.config import save_config
        save_config({"plugins": {"anakot-memory-store": dict(values)}}, merge_existing=True)

    def get_config_schema(self):
        from anakot_constants import display_anakot_home
        return [
            {"key": "db_path", "description": "SQLite database path", "default": f"{display_anakot_home()}/memory_store.db"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        _anakot_home = str(get_anakot_home())
        db_path = self._config.get("db_path", _anakot_home + "/memory_store.db")
        if isinstance(db_path, str):  # expand $ANAKOT_HOME so paths resolve to the active profile
            db_path = db_path.replace("$ANAKOT_HOME", _anakot_home).replace("${ANAKOT_HOME}", _anakot_home)
