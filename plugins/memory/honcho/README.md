...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/anakot>
...
```bash
anakot memory setup honcho   # configure Honcho directly (works on a fresh install)
anakot memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
anakot config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.anakot/.env
```

> `anakot honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `anakot memory setup honcho`.
...
Set `"recallSync": true` in `$ANAKOT_HOME/honcho.json` (at the root or under
`hosts.anakot`), or enable **Current-query recall** in the memory settings or
`anakot honcho setup`. An explicit host-level `false` overrides root-level `true`.
...
cycle (not per pass). Select a fast, inexpensive model under `anakot model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$ANAKOT_HOME/honcho.json` | Profile-local (isolated Anakot instances) |
| 2 | `~/.anakot/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Anakot profile: `anakot` (default) or `anakot_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `anakot honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 2 | `per-session` strategy | Anakot session ID (`20260415_a3f2b1`) |
| 3 | Manual map (`sessions` config) | `"myproject-main"` |
| 4 | Explicit `/title` command (non-automatic title) | `"refactor-auth"` |
| 5 | `per-repo` strategy | Git root directory name (`anakot-agent`) |
...
Automatically generated Anakot titles (`derived` or `llm`) are display metadata and do not override `sessionStrategy`. An explicit user title remains an intentional session-name override for non-gateway, non-`per-session` sessions.
...
If `sessionPeerPrefix` is `true`, the user peer name is prepended: `alice-anakot-agent`.
...
- **`per-session`** — Anakot session ID (timestamp + hex). Every `anakot` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Anakot profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`anakot`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Anakot profile: `anakot` (default) or `anakot_<profile>` (e.g. `anakot -p coder` -> host key `anakot_coder`). Older `anakot.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `ANAKOT_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `anakot-agent`) |
...
|---------|-------------|
| `anakot memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `anakot honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `anakot memory setup`) |
| `anakot honcho status` | Show resolved config for active profile |
| `anakot honcho enable` / `disable` | Toggle Honcho for active profile |
| `anakot honcho mode <mode>` | Change recall or observation mode |
| `anakot honcho peer --user <name>` | Update user peer name |
| `anakot honcho peer --ai <name>` | Update AI peer name |
| `anakot honcho tokens --context <N>` | Set context token budget |
| `anakot honcho tokens --dialectic <N>` | Set dialectic max chars |
| `anakot honcho map <name>` | Map current directory to a session name |
| `anakot honcho sync` | Create host blocks for all Anakot profiles |
