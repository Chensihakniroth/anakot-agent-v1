...
```bash
anakot memory setup    # select "mem0"
```
...
Behavioral settings live in `$ANAKOT_HOME/mem0.json` (set them via `anakot memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.anakot/.env`.
...
| `user_id` | `anakot-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Anakot session and call `mem0_search` — it connects to your server.
...
```bash
anakot memory setup mem0 --mode oss \
...
```bash
anakot memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$ANAKOT_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.anakot/mem0_qdrant"}}
  }
}
```
...
```bash
anakot memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$ANAKOT_HOME/mem0.json`).
