...
def _normalize_matrix_bang_command(text: str) -> str:
    """Convert Matrix ``!command`` aliases to normal Anakot ``/command`` text."""
...
from anakot_constants import get_anakot_dir as _get_anakot_dir
...
_TEX_TOKEN_RE = re.compile(r"ANAKOTTEX(?:DISPLAY|INLINE)(\d+)ANAKOTTEXEND")
_TEX_DISPLAY_TOKEN = "ANAKOTTEXDISPLAY%dANAKOTTEXEND"
_TEX_INLINE_TOKEN = "ANAKOTTEXINLINE%dANAKOTTEXEND"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    typed_command_prefix = "!"  # clients reserve typed "/" for local commands; "!command" always reaches Anakot
    ...
    def _resolve_store_dir(self) -> Path:
        ...
        self._store_dir = _get_anakot_dir("platforms/matrix/store", "matrix/store")
        return self._store_dir

    @property
    def _crypto_db_path(self) -> Path:
        return (self._store_dir or _get_anakot_dir("platforms/matrix/store", "matrix/store")) / "crypto.db"
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("ANAKOT_MATRIX_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("ANAKOT_MATRIX_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
        ...
        if self._access_token:
            ...
            try:
                resp = await client.login(
                    identifier=self._user_id, password=self._password, device_name="Anakot Agent",
    ...
    async def _connect_setup_e2ee(self, client: Any, api: Any, state_store: Any) -> bool:
        ...
        try:
            ...
            _acct_id = self._user_id or "anakot"
    ...
    async def create_handoff_thread(self, parent_chat_id: str, name: str) -> Optional[str]:
        ...
        if self._client is None:
            return None
        result = await self.send(parent_chat_id, (name or "").strip() or "Anakot session")
        root = result.message_id if result.success else None
        if not root:
            return None
        await self._threads.mark_async(str(root))  # replies here bypass require_mention, like inbound roots
        return str(root)
    ...
    def _strip_mention(self, body: str) -> str:
        ...
        words, or "Anakot Agent" would become "Agent"."""
        if not body:
            return ""
        if self._user_id:
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    try:
        ...
        txn_id = f"anakot_{int(time.time() * 1000)}_{os.urandom(4).hex()}"
...
def interactive_setup() -> None:
    ...
    from anakot_cli.cli_output import prompt, prompt_yes_no, print_header, print_info, print_success, print_warning
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        for line in ("📬 Home Room: where Anakot delivers cron job results and notifications.",
...
def _is_connected(config) -> bool:
    """Connected = homeserver + token (or password). Reads via anakot_cli.gateway.get_env_value so
