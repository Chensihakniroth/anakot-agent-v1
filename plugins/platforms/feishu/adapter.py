...
# Anakot monkey-patches ``websockets.connect`` on the shared module to inject ping settings.
...
# (``loop``), and Anakot also monkey-patches ``websockets.connect`` on the shared ``websockets`` module to
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._dedup_state_path = get_anakot_home() / "feishu_seen_message_ids.json"
    ...
    def _load_settings(extra: Dict[str, Any]) -> FeishuAdapterSettings:
        ...
        return FeishuAdapterSettings(
            ...
            dedup_cache_size=max(32, env_int("ANAKOT_FEISHU_DEDUP_CACHE_SIZE", _DEFAULT_DEDUP_CACHE_SIZE)),
            text_batch_delay_seconds=env_float("ANAKOT_FEISHU_TEXT_BATCH_DELAY_SECONDS", _DEFAULT_TEXT_BATCH_DELAY_SECONDS),
            text_batch_split_delay_seconds=env_float("ANAKOT_FEISHU_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0),
            text_batch_max_messages=max(1, env_int("ANAKOT_FEISHU_TEXT_BATCH_MAX_MESSAGES", _DEFAULT_TEXT_BATCH_MAX_MESSAGES)),
            text_batch_max_chars=max(1, env_int("ANAKOT_FEISHU_TEXT_BATCH_MAX_CHARS", _DEFAULT_TEXT_BATCH_MAX_CHARS)),
            media_batch_delay_seconds=env_float("ANAKOT_FEISHU_MEDIA_BATCH_DELAY_SECONDS", _DEFAULT_MEDIA_BATCH_DELAY_SECONDS),
    ...
    def _get_sdk_executor(self) -> concurrent.futures.ThreadPoolExecutor:
        ...
        with lock:
            ...
            if executor is None or getattr(executor, "_shutdown", False):
                executor = concurrent.futures.ThreadPoolExecutor(max_workers=10, thread_name_prefix="anakot-feishu-sdk")
    ...
    async def _run_blocking(self, func, *args):
        ...
        profile ANAKOT_HOME override / secret scope (multiplexed dedup flush, thread lookup).
        """
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Anakot gateway is already using this Feishu app_id"
    ...
    async def _send_exec_approval_prompt(self, prompt: ExecApprovalPrompt) -> SendResult:
        """Approval-button card; ``anakot_action`` in each button value lets the click callback
        ...
        try:
            ...
            actions = [
                _card_button(label, style or "default",
                             {"anakot_action": self._EA_CARD_ACTIONS[choice], "approval_id": approval_id})
    ...
    def _build_update_prompt_card(*, prompt: str, default: str, prompt_id: int) -> Dict[str, Any]:
        ...
        def _btn(label: str, answer: str, btn_type: str) -> dict:
            return _card_button(label, btn_type, {"anakot_update_prompt_action": answer, "update_prompt_id": prompt_id})
    ...
    def _on_message_read_event(self, data: P2ImMessageMessageReadV1) -> None:
        """Ignore read-receipt events that Anakot does not act on."""
    ...
    def _on_card_action_trigger(self, data: Any) -> Any:
        ...
        if isinstance(action_value, dict):
            if action_value.get("anakot_action"):
                return self._handle_approval_card_action(event=event, action_value=action_value, loop=loop)
            if action_value.get("anakot_update_prompt_action"):
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        choice = _APPROVAL_CHOICE_MAP.get(action_value.get("anakot_action"), "deny")
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if not state:
            ...
            return self._card_response()
        answer = str(action_value.get("anakot_update_prompt_action", "") or "").strip().lower()
    ...
    async def _download_remote_document(self, file_url: str, *, default_ext: str, preferred_name: str) -> tuple[str, str]:
        ...
        async with create_ssrf_safe_async_client(
            ...
            response = await client.get(
                file_url, headers={"User-Agent": "Mozilla/5.0 (compatible; AnakotAgent/1.0)", "Accept": "*/*"},
            )
    ...
    async def _handle_webhook_request(self, request: Any) -> Any:
        ...
        if payload.get("encrypt"):
            logger.error("[Feishu] Encrypted webhook payloads are not supported by Anakot webhook mode")
    ...
    def _persist_seen_message_ids(self) -> None:
        try:
            ...
            mkdir_under_anakot_home(self._dedup_state_path.parent)
...
def _begin_registration(domain: str = "feishu") -> dict:
    ...
    qr_url += ("&" if "?" in qr_url else "?") + "from=anakot&tp=anakot"
    return {
...
# static dict in anakot_cli/gateway.py, and the _send_feishu dispatch in tools/send_message_tool.py).
# ──────────────────────────────────────────────────────────────────────────
...
async def _standalone_send(pconfig, chat_id, message, *, thread_id=None, media_files=None, force_document=False):
    ...
    if not await asyncio.to_thread(_load_lark_oapi):
        return send_error("Feishu dependencies not installed. Run `anakot setup` to install Feishu support.")
    try:
...
def interactive_setup() -> None:
    ...
    from anakot_cli.setup import prompt_choice
    from anakot_cli.cli_output import prompt, print_header, print_info, print_success, print_warning
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `anakot setup` to install Feishu support.", setup_fn=interactive_setup,
