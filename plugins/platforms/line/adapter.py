...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        path is rechecked against allowed roots (tempdir, ``/tmp``→``/private/tmp`` on macOS, ANAKOT_HOME).
        ...
        try:
            ...
            anakot_home = Path(get_anakot_home()).resolve()
        except Exception:
            anakot_home = Path.home().joinpath(".anakot").resolve()
        resolved = path.resolve()
        if not any(resolved.is_relative_to(r) for r in (Path(tempfile.gettempdir()).resolve(), Path("/tmp").resolve(), anakot_home)):
...
def is_connected(config) -> bool:
    """Surface in ``anakot status`` even before the adapter is instantiated."""
    return validate_config(config)


def _env_enablement() -> Optional[Dict[str, Any]]:
    """``env_enablement_fn``: seed ``PlatformConfig.extra`` from env-only setups so ``anakot status`` sees them."""
    if not _env_credentials_present():
        return None
...
def interactive_setup() -> None:
    """``anakot setup line`` wizard (writes ``~/.anakot/.env``); CLI helpers are lazy-imported."""
    from anakot_cli.config import get_env_value, save_env_value
    from anakot_cli.cli_output import print_header, print_info, prompt
