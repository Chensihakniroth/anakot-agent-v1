# A2A — Agent-to-Agent protocol for Anakot
...
peer (another Anakot, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
anakot gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Anakot serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.anakot/a2a_audit.jsonl`.
- Conversations persist to `~/.anakot/a2a_conversations/` — they survive context
