...
_DEFAULT_DESCRIPTION = "Anakot Agent — a general-purpose agent reachable over A2A."
...
def _default_agent_name() -> str:
    ...
    try:
        import socket
        return f"anakot-{socket.gethostname()}"
    except Exception:
        return "anakot-agent"
...
def _active_profile_name() -> str:
    try:
        ...
        return os.getenv("ANAKOT_PROFILE", "default") or "default"
...
def _profile_home(profile: str) -> Optional[str]:
    ...
    if profile and profile != "default":
        return os.path.expanduser(f"~/.anakot/profiles/{profile}")
    with contextlib.suppress(Exception):
...
class A2AAdapter(BasePlatformAdapter):
    ...
    def _load_served_agents(self, extra: dict) -> dict[str, dict]:
        ...
        for key, val in items:
            ...
            agents[slug] = {
                ...
                "name": str(val.get("name") or f"Anakot {slug}"),
                "description": str(val.get("description") or f"Anakot profile '{profile or slug}' exposed over A2A."),
    ...
    def _forward_to_profile(self, agent: dict, peer: str, context_id: str, framed_text: str) -> tuple[str, str]:
        """Forward a routed task to another local profile via ``anakot chat``. First contact creates a
        ...
        with self._forward_lock(key):
            ...
            cmd = ["anakot", "chat", "-q", framed_text, "-Q", "--source", "a2a"] + (["--resume", session_id] if session_id else [])
