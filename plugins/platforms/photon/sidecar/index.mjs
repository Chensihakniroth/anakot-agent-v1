// Anakot Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Anakot): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Anakot -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Anakot-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `anakot photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Anakot can restart the Photon adapter"
    );
    process.exit(75);
...
const PROBE_MSG_PREFIX = "anakot-liveness-probe-";
...
// instead of a cryptic module-resolution error during import. Apply Anakot'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Anakot allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Anakot, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-anakot-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Anakot supplied them so a known-good
