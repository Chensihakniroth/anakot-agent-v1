...
Small Node helper that bridges Anakot Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Anakot is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Anakot-Sidecar-Token`)
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Anakot can receive inbound messages
...
The Anakot plugin's `anakot photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Anakot is Python, so replies go through this sidecar
