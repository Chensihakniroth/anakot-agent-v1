...
// `message.content.text` before Anakot can see it. We rewrite the two inbound
...
const MARKER = "Anakot patch: Preserve mixed text + attachment iMessage payloads";
...
export function patchSpectrumTs(root = scriptDir()) {
  ...
  for (const file of files) {
    ...
    // Anakot to patch; keep the legacy v8 path below for older pinned installs.
    if (
