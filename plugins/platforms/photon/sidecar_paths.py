...
``$ANAKOT_HOME/photon/sidecar``. The mirror is refreshed by content compare; ``node_modules``
...
# Written by `anakot photon install-sidecar` on npm failure so check_requirements() can
...
def resolve_sidecar_dir(source_dir: Optional[Path] = None) -> Path:
    ...
    mirror = get_anakot_home() / "photon" / "sidecar"
    try:
