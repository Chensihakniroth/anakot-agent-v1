...
Generation observations include the Anakot system prompt when the provider
uses a separate `system` param (Anthropic Messages API). Open an **LLM call**
child span to inspect `role: system` (truncated via `ANAKOT_LANGFUSE_MAX_CHARS`).
...
ANAKOT_LANGFUSE_CAPTURE=sanitized    # content capture mode (see below)
ANAKOT_LANGFUSE_DEBUG=true           # verbose plugin logging
```

## Capture modes

`ANAKOT_LANGFUSE_CAPTURE` controls how much *content* (prompts, responses,
