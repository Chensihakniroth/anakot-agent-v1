...
const lockedLog = [
  'error: failed to remove file `C:/install/venv/Lib/site-packages/../../Scripts/anakot.exe`: Access is denied. (os error 5)',
  'File "C:/install/venv/Scripts/anakot.exe/__main__.py", line 10, in <module>',
...
const base = {
  ...
  installMethod: 'installer-script', updateMethod: 'anakot-update',
  error: 'E2E ASSERTION FAILED: anakot update exited 1 (expected 0)', logs: { update: lockedLog },
}
...
describe('known install failures', () => {
  it('recognizes the released launcher self-lock, not generic access denied', () => {
    ...
    expect(matchKnownFailure({ ...base, logs: { update: lockedLog.replaceAll('anakot.exe', 'other.exe') } })).toBeNull()
  ...
  it('matches manual-only app updates only for the three proven July script cases', () => {
    const sample = {
      ...
      updateMethod: 'anakot-desktop-app-update',
      error: 'E2E ASSERTION FAILED: app driven via captured anakot desktop spec; update completed',
      logs: { desktop: '[anakot] [updates] no staged updater; surfacing manual `anakot update` for CLI install at C:/install\n[anakot] [updates] manual: anakot update\n' },
    }
    ...
    expect(matchKnownFailure({ ...sample, logs: { desktop: '[updates] manual: anakot update' } })).toBeNull()
  })
...
it('renders known receipts as footnotes, without suppressing a red job', async () => {
  ...
  const name = 'windows: installer-script -> anakot-update (v2026.3.12 -> HEAD)'
