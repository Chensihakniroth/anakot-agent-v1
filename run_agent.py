...
try:
    ...
    pass  # partial `anakot update` — only skips the Windows UTF-8 stdio setup
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    """cwd to stamp on a new session row (``anakot -c`` / ``--resume``), or None.
...
# Sources that label the human conversation an interactive UI transport hosts. A finite ``anakot chat -q`` /
# one-shot child spawned from such a session inherits ANAKOT_SESSION_SOURCE (the terminal tool bridges the
# session env into child processes) but is NOT that conversation: labelling it ``tui``/``desktop`` lists it
# in the TUI/WebUI pickers as a resumable chat and lets ``anakot -c`` in the TUI continue it (#112550).
# Automation sources (kanban, tool, cron, a2a, ...) are inherited on purpose.
_UI_TRANSPORT_SOURCES = frozenset({"tui", "desktop"})

# Finite non-interactive CLI runs (``anakot chat -q``/``--oneshot``, ``anakot -z``) get their own source so human
# pickers hide them without title/cwd heuristics; ``anakot -c`` still treats them as CLI history.
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    ...
    source = str(get_session_env("ANAKOT_SESSION_SOURCE", "") or "").strip()
    single_query = get_session_env("ANAKOT_SINGLE_QUERY_SESSION", "") == "1"
    explicit = get_session_env("ANAKOT_SESSION_SOURCE_EXPLICIT", "") == "1"
...
_anakot_home = get_anakot_home()  # read by agent_init via _ra()._anakot_home
_loaded_env_paths = load_anakot_dotenv(anakot_home=_anakot_home, project_env=Path(__file__).parent / '.env')
...
class AIAgent(
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[anakot-agent: tool call arguments were corrupted in this session and "
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> Any:
        ...
        if (getattr(self, "lmstudio_load_mode", "explicit") or "explicit").strip().lower() == "jit":
            ...
            return None
        from anakot_cli.models_local import ensure_lmstudio_model_loaded
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        ``ANAKOT_API_TIMEOUT`` > 1800s."""
        cfg = get_provider_request_timeout(self.provider, self.model)
        return cfg if cfg is not None else env_float("ANAKOT_API_TIMEOUT", 1800.0)

    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        """Base non-stream stale timeout: per-model ``stale_timeout_seconds`` > provider-wide >
        ``ANAKOT_API_CALL_STALE_TIMEOUT`` > reasoning floor > 90s.
        ...
        if cfg is not None:
            return cfg, False
        env_timeout = os.getenv("ANAKOT_API_CALL_STALE_TIMEOUT")
    ...
    def _stale_timeout_is_explicit(self) -> bool:
        ...
        return (get_provider_stale_timeout(self.provider, self.model) is not None
                or os.getenv("ANAKOT_API_CALL_STALE_TIMEOUT") is not None)
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        return (
            ...
            "See anakot-agent#21444 for symptom history."
        )
