#!/usr/bin/env python3
"""SQLite state store for Anakot Agent: session metadata, message history, model
...
from anakot_constants import get_anakot_home, mkdir_under_anakot_home
from typing import Any, Callable, Dict, Iterator, List, Optional, Tuple, TypeVar, cast

from anakot_state_common import (
...
from anakot_state_holders import read_only_db_uri
from anakot_state_errors import (
...
from anakot_state_guard import (
...
from anakot_state_readpool import _READ_POOL_MAX, _proc_fd_targets, _read_budget_for
from anakot_state_sessions import SessionSessionsMixin
from anakot_state_fts import SessionFtsSetupMixin, load_fts5_cjk_extension
from anakot_state_portability import SessionPortabilityMixin
from anakot_state_telegram import SessionTelegramTopicsMixin
from anakot_state_profile_repair import SessionProfileRepairMixin
from anakot_state_schema import SessionSchemaMixin
import anakot_state_holders as _state_holders
import anakot_state_lockguard as _lockguard
from anakot_state_dbfile import (
...
from anakot_state_messages import SessionMessagesMixin
from anakot_state_rewind import SessionRewindMixin
from anakot_state_wal import (
    _WAL_INCOMPAT_MARKERS, _on_disk_journal_mode, apply_database_pragmas, apply_wal_with_fallback,
)
from anakot_state_repair import _claim_repair_attempt, preflight_db_writability, repair_state_db_schema
from anakot_state_titles import SessionTitlesMixin
from anakot_state_usage import SessionUsageMixin
from anakot_state_maintenance import SessionMaintenanceMixin
from anakot_state_gateway import SessionGatewayMixin
from anakot_state_compression import SessionCompressionMixin
from anakot_state_search import SessionSearchMixin
...
class SessionResumeTooLargeError(ValueError):
    def __init__(
        ...
        super().__init__(
            ...
            "Start a fresh chat and use `anakot sessions export` to keep a copy, or raise the limit "
            "with `anakot config set sessions.max_resume_messages 0`."
        )
...
DEFAULT_DB_PATH = _IMPORT_DEFAULT_DB_PATH = get_anakot_home() / "state.db"
...
def _default_db_path() -> Path:
    ...
    ``get_anakot_home()`` is resolved fresh (a runtime ANAKOT_HOME redirect works regardless of import)."""
    return DEFAULT_DB_PATH if DEFAULT_DB_PATH != _IMPORT_DEFAULT_DB_PATH else get_anakot_home() / "state.db"


# Live-DB guard knobs live HERE (not in anakot_state_guard): the hermetic conftest monkeypatches
# ``anakot_state._STATE_DB_GUARD_BYPASS`` (``@pytest.mark.live_system_guard_bypass`` escape hatch)
# and ``_EXTRA_DENY_ROOTS`` (the pre-sandbox root, so custom-ANAKOT_HOME deployments are covered).
...
def _ensure_test_isolation(db_path: Path) -> None:
    ...
    ``ANAKOT_HOME`` together, which is precisely the state in which it writes to production (#82770).
    """
    ...
    for root in roots:
        if _is_production_state_db(resolved, root):
            raise RuntimeError(
                ...
                f"state.db at {resolved} (under real Anakot root {root}). "
                "Tests must run against a temporary ANAKOT_HOME — pass an "
                "explicit tmp db_path or let the hermetic conftest redirect "
                "ANAKOT_HOME. If this test genuinely needs the live database, mark it with "
...
def _secure_state_db_files(db_path: Path, *, create_main: bool = False) -> None:
    ...
    (gateway, desktop ``anakot serve``) keep using the deleted inodes.
    """
    if os.name == "nt":
        return
...
_NETWORK_DRIVE_ACTION = (
    "Move it to a local disk (`anakot {profile_arg}doctor` shows where it is), then start Anakot again."
)


def format_session_db_unavailable(
    prefix: str = "Anakot can't open its session history right now",
    *,
    details: bool = False,
) -> str:
    """User-facing one-liner: ``<prefix>: <gloss>. <consequence> <action>[ network hint]``.

    The cause table lives in ``anakot_state_user_copy`` so CLI, gateway and TUI agree. Chat
    ...
    message names the network-drive suspicion, because ``anakot doctor --fix`` cannot repair a
    ...
    if not cause:
        return (
            f"{prefix}. {_SESSION_DB_CONSEQUENCE} Run `anakot {profile_arg}doctor` to check the "
            "storage location."
        )
    from anakot_state_user_copy import describe_storage_failure
    ...
    if details:
        from anakot_state_user_copy import storage_failure_details
...
def divert_session_transcript_jsonl(session_id: str, messages) -> "Optional[Path]":
    """Append pending messages to ANAKOT_HOME/sessions/<id>.jsonl (state.db was replaced under a
...
# connection per resolved path via anakot_state_registry.acquire(); one-shots use SessionDB() + close().
...
# ── Process-wide shared SessionDB registry (#90837) ── lives in anakot_state_registry.py (acquire /
# release / close_all / release_or_close). Long-lived in-process callers (gateway, tui_gateway, cron,
# in-process tools) share ONE writer connection per resolved path via anakot_state_registry.acquire(); CLI
...
class SessionDB(
    ...
    # See #60609.  `recovered` = placeholders `anakot sessions recover` synthesizes for
    ...
    # SQLite's deterministic busy handler convoys under many anakot processes: keep its
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        self._wal_lock_guard: dict = {}  # anakot_state_lockguard.hold() record, see _open_writer
        ...
        # Opened via anakot_state_registry.acquire(): close() releases a refcount instead.
        # Set True when this instance is opened via anakot_state_registry.acquire(). Makes close() a no-op so the
    ...
    def _open_writer(self) -> None:
        ...
        mkdir_under_anakot_home(self.db_path.parent)
        ...
        try:
            ...
            if not repair_state_db_schema(self.db_path).get("repaired"):
                raise
            self._connect_and_init_with_lock_patience()
        # FTS optimization is OPT-IN (`anakot db optimize`); no background worker races session lifecycle.
    ...
    def _handle_quarantine_if_invalid(self, already_locked: bool = False) -> None:
        ...
        msg = (
            f"state.db was empty or damaged ({zsize} bytes) and has been {where}; Anakot started with a "
            "fresh, empty session database. To bring old sessions back, run "
            f"`anakot sessions recover --source {qpath or self.db_path} --inspect-only`, or restore a "
            "snapshot with `/snapshot list` then `/snapshot restore <id>` (terminal `anakot` chat only)."
        )
        logger.error(msg)
            ...
            try:
                ...
                if isinstance(exc, sqlite3.OperationalError):
                    if "locked" in err_msg or "busy" in err_msg:
                        ...
                        raise sqlite3.OperationalError(
                            f"database is locked (another Anakot process held the "
    ...
    def _halt_db_corrupt(self, exc: BaseException) -> None:
        ...
        logger.error(
            ...
            "gateway and run `anakot sessions recover --source %s --inspect-only`.", self.db_path, exc,
            self.db_path,
        )
    ...
    def _settle_lost_generation_locked(self) -> bool:
        ...
        through anakot_state_registry.release_or_close, which swallows the error, so an interpreter
    ...
    def _foreign_state_db_holders(self) -> List[Tuple[int, str]]:
        """Foreign processes holding this DB or its WAL sidecars (see anakot_state_holders)."""
    ...
    def close(self):
        ...
        When this instance is shared (opened via ``anakot_state_registry.acquire``), ``close()`` RELEASES one
        ...
        if self._shared_registry_owned:
            from anakot_state_registry import release
            release(self)
            return
            ...
            if self._conn:
                ...
                if quarantine_reason is not None:
                    logger.warning(
                        ...
                        "before restarting, then run `anakot sessions recover --source %s --inspect-only`.",
...
_PLUGIN_COMPAT_LAZY = {
    'AUTO_VACUUM_MIN_FREELIST_RATIO': ('anakot_state_common', 'AUTO_VACUUM_MIN_FREELIST_RATIO'),
    'ActivityProvenance': ('agent.session_activity', 'ActivityProvenance'),
    'CompressionSessionBusyError': ('anakot_state_errors', 'CompressionSessionBusyError'),
    'CompressionSessionClosedError': ('anakot_state_errors', 'CompressionSessionClosedError'),
    'DEFERRED_INDEX_SQL': ('anakot_state_common', 'DEFERRED_INDEX_SQL'),
    'FTS_CJK_STALE_KEY': ('anakot_state_common', 'FTS_CJK_STALE_KEY'),
    'FTS_CJK_TABLE_SQL': ('anakot_state_fts', 'FTS_CJK_TABLE_SQL'),
    'FTS_CJK_TRIGGER_SQL': ('anakot_state_fts', 'FTS_CJK_TRIGGER_SQL'),
    'FTS_REBUILD_DEFERRAL_KEY': ('anakot_state_common', 'FTS_REBUILD_DEFERRAL_KEY'),
    'FTS_SQL': ('anakot_state_common', 'FTS_SQL'),
    'FTS_STALE_KEY': ('anakot_state_common', 'FTS_STALE_KEY'),
    'FTS_STORAGE_VERSION': ('anakot_state_common', 'FTS_STORAGE_VERSION'),
    'FTS_TRIGRAM_SQL': ('anakot_state_common', 'FTS_TRIGRAM_SQL'),
    'LEGACY_FTS_SQL': ('anakot_state_common', 'LEGACY_FTS_SQL'),
    'LEGACY_FTS_TRIGRAM_SQL': ('anakot_state_common', 'LEGACY_FTS_TRIGRAM_SQL'),
    'MAX_FTS5_QUERY_CHARS': ('anakot_state_common', 'MAX_FTS5_QUERY_CHARS'),
    'PERSISTENCE_ERROR_CAUSES': ('anakot_state_errors', 'PERSISTENCE_ERROR_CAUSES'),
    'SCHEMA_SQL': ('anakot_state_common', 'SCHEMA_SQL'),
    'SCHEMA_VERSION': ('anakot_state_common', 'SCHEMA_VERSION'),
    'SESSION_STATUS_COMPLETE': ('anakot_state_sessions', 'SESSION_STATUS_COMPLETE'),
    'SESSION_STATUS_EMPTY': ('anakot_state_sessions', 'SESSION_STATUS_EMPTY'),
    'SESSION_STATUS_ERROR': ('anakot_state_sessions', 'SESSION_STATUS_ERROR'),
    'SESSION_STATUS_INTERRUPTED': ('anakot_state_sessions', 'SESSION_STATUS_INTERRUPTED'),
    ...
    'SessionTurnLeaseLostError': ('anakot_state_errors', 'SessionTurnLeaseLostError'),
    'WalUnsupportedError': ('anakot_state_wal', 'WalUnsupportedError'),
    'apply_durability_barriers': ('anakot_state_repair', 'apply_durability_barriers'),
    'classify_session_status': ('anakot_state_sessions', 'classify_session_status'),
    'collect_state_db_stats': ('anakot_state_dbfile', 'collect_state_db_stats'),
    'count_db_holders': ('anakot_state_dbfile', 'count_db_holders'),
    'describe_skill_invocation': ('agent.skill_commands', 'describe_skill_invocation'),
    'fts5_cjk_so_path': ('anakot_state_fts', 'fts5_cjk_so_path'),
    'is_advisory_lock_contention': ('anakot_state_common', 'is_advisory_lock_contention'),
    'is_automatic_end_reason': ('anakot_state_common', 'is_automatic_end_reason'),
    'is_disk_full_error': ('anakot_state_errors', 'is_disk_full_error'),
    'is_sqlite_wal_reset_vulnerable': ('anakot_state_wal', 'is_sqlite_wal_reset_vulnerable'),
    'is_transient_sqlite_error': ('anakot_state_errors', 'is_transient_sqlite_error'),
    'iter_deleted_sqlite_sidecar_holders': ('anakot_state_dbfile', 'iter_deleted_sqlite_sidecar_holders'),
    'release_or_close': ('anakot_state_registry', 'release_or_close'),
    'report_startup_progress': ('anakot_startup_watchdog', 'report_startup_progress'),
    'resolve_journal_mode': ('anakot_state_wal', 'resolve_journal_mode'),
    'resolve_synchronous_level': ('anakot_state_wal', 'resolve_synchronous_level'),
    'sanitize_context': ('agent.memory_manager', 'sanitize_context'),
    'sqlite_source_id': ('anakot_state_wal', 'sqlite_source_id'),
    'workspace_key': ('anakot_state_sessions', 'workspace_key'),
}
