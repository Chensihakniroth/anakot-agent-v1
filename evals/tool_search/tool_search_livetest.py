#!/usr/bin/env python3
"""Live test harness for Anakot Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any anakot imports.
ORIGINAL_HOME = os.environ.get("ANAKOT_HOME")
ORIGINAL_AUTH = Path.home() / ".anakot" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.anakot/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.anakot/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="anakot_ts_live_"))
    anakot_home = home_dir / ".anakot"
    anakot_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, anakot_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".anakot" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, anakot_home / ".env")
        ...
        load_anakot_dotenv(anakot_home=str(Path.home() / ".anakot"))
    ...
    (anakot_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return anakot_home
...
def reset_module_state():
    """Drop cached modules so the new ANAKOT_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "anakot_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original ANAKOT_HOME
    if ORIGINAL_HOME is not None:
        os.environ["ANAKOT_HOME"] = ORIGINAL_HOME
    else:
