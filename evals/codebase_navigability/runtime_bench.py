...
Each probe runs in a FRESH subprocess with an isolated ANAKOT_HOME so nothing is cached across reps.
...
ENV = {**os.environ, "ANAKOT_HOME": HOME, "PYTHONPATH": TREE, "PYTHONDONTWRITEBYTECODE": "0", "OPENROUTER_API_KEY": "sk-bench-placeholder",
       "ANAKOT_SKIP_UPDATE_CHECK": "1", "NO_COLOR": "1", "TERM": "dumb", "COLUMNS": "120"}
for k in list(ENV):
    if k.startswith(("ANAKOT_SESSION", "ANAKOT_PROFILE")): ENV.pop(k)
...
IMPORT_TARGETS = ["run_agent", "cli", "anakot_cli.main", "gateway.run", "tools.registry", "anakot_state", "tui_gateway.server", "anakot_cli.web_server", "model_tools", "agent.prompt_builder"]
probe = r'''
...
tree=os.getcwd(); foreign=[m for m,mod in list(sys.modules.items()) if getattr(mod,"__file__",None) and "anakot-agent" in mod.__file__ and not mod.__file__.startswith(tree) and "site-packages" not in mod.__file__]
...
# 2. CLI end-to-end startup: `anakot --version`, `anakot --help`, `anakot doctor --help`, `anakot config get model` (no network)
CLI = {"version": ["anakot_cli/main.py", "--version"], "help": ["anakot_cli/main.py", "--help"], "config_get": ["anakot_cli/main.py", "config", "get", "model"], "tools_list": ["anakot_cli/main.py", "tools", "--help"], "skills_help": ["anakot_cli/main.py", "skills", "--help"]}
cli = {}
...
try:
    ...
    T("resolve_toolset_anakot_default_ms", lambda: resolve_toolset("anakot-default"), 200)
...
try:
    import anakot_state, tempfile, uuid
    from pathlib import Path
    db=anakot_state.SessionDB(Path(tempfile.mkdtemp())/"s.db") if hasattr(anakot_state,"SessionDB") else None
    if db:
        sid=str(uuid.uuid4())
