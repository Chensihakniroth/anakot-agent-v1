...
with tempfile.TemporaryDirectory(prefix="delegate-schema-") as home:
    os.environ.clear()
    os.environ.update(HOME=home, ANAKOT_HOME=home, PATH="/usr/bin:/bin", ANAKOT_PLATFORM="cli")
    ...
    definitions = get_tool_definitions(enabled_toolsets=["anakot-cli"], quiet_mode=True)
    serialized = json.dumps(definitions, separators=(",", ":"))
    repeated = get_tool_definitions(enabled_toolsets=["anakot-cli"], quiet_mode=True)
