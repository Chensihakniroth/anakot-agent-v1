...
def run_probe(repo, receipt):
    ...
    owner = Path(os.environ["ANAKOT_HOME"])
    ...
    with serving(Provider()) as provider, serving(CallbackServer(
        ...
        reloaded = asyncio.run(AnakotTokenStorage("positive", anakot_home=owner).get_tokens())
        ...
        override = set_anakot_home_override(other)
        try:
...
def main():
    ...
    if args.cold_probe:
        sys.path.insert(0, str(repo))
        logging.disable(logging.CRITICAL)
        from anakot_cli.mcp_config import _get_mcp_servers, _probe_single_server
    ...
    if not args.isolated_worker:
        with tempfile.TemporaryDirectory(prefix="anakot-oauth-http-") as temp:
            home = Path(temp)
            (home / ".anakot").mkdir()
            env = {"PATH": os.environ.get("PATH", ""), "HOME": temp,
                   "ANAKOT_HOME": str(home / ".anakot"), "LANG": "C.UTF-8", "TZ": "UTC",
                   "PYTHONNOUSERSITE": "1"}
