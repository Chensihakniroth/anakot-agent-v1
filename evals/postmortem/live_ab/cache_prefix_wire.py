...
message prefix between call N and N+1. If Anakot strips prior-turn thinking, call N+1's messages[:k]
...
#   python cache_prefix_wire.py <repo_root> <A|B> [--anakot-home DIR]   (default ANAKOT_HOME: the real one, for credentials)
sys.path.insert(0, sys.argv[1])
os.environ.setdefault("ANAKOT_HOME", os.path.expanduser("~/.anakot"))
if "--anakot-home" in sys.argv:
    os.environ["ANAKOT_HOME"] = sys.argv[sys.argv.index("--anakot-home") + 1]
...
time.sleep(1)
log = subprocess.run(f"grep -h '\\[{sid}\\]' ~/.anakot/logs/agent.log | grep 'API call #'", shell=True, capture_output=True).stdout.decode("utf-8", "replace")
