...
os.environ.update(HOME=str(home), ANAKOT_HOME=str(home/'anakot'), XDG_CONFIG_HOME=str(home/'config'), CODEX_HOME=str(home/'codex'))
for k in list(os.environ):
    if any(x in k for x in ('API_KEY','TOKEN','NOUS_','SECRET')): os.environ.pop(k, None)
(home/'anakot').mkdir()
(home/'anakot'/'config.yaml').write_text('nous:\n  keepalive_interval_seconds: 0\nmemory:\n  memory_enabled: false\n', encoding='utf-8')
...
class Handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        data={'id':'local-probe','object':'chat.completion','created':int(time.time()),'model':'anakot-test','choices':[{'index':0,'message':{'role':'assistant','content':'local-only'},'finish_reason':'stop'}],'usage':{'prompt_tokens':1,'completion_tokens':1,'total_tokens':2}}
...
os.environ['ANAKOT_SHARED_AUTH_DIR']=str(home/'shared')
...
def store(token):
    ...
    (home/'anakot'/'auth.json').write_text(json.dumps({'version':1,'active_provider':'nous','providers':{'nous':state}}), encoding='utf-8')
results=[]
for case, own_sub, store_sub, ttl in [('same-account','account-A','account-A',30),('explicit-account','account-A','account-B',30),('far-from-expiry','account-A','account-B',3000)]:
    ...
    agent=AIAgent(api_key=own,base_url=url,provider='nous',model='anakot-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    reply=client.chat.completions.create(model='anakot-test',messages=messages)
...
def worker(_):
    a=AIAgent(api_key=expired,base_url=url,provider='nous',model='anakot-test',quiet_mode=True,skip_context_files=True,skip_memory=True,enabled_toolsets=[])
    ...
    try:
        c.chat.completions.create(model='anakot-test',messages=messages)
        status=200
...
if MODE != 'main':
    ...
    state_file=home/'anakot'/'auth.json'
