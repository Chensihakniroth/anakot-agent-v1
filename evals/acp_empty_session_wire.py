...
def main():
    ...
    home = Path(tempfile.mkdtemp(prefix='anakot-acp-empty-'))
    anakot = home / '.anakot'
    anakot.mkdir()
    (anakot / 'config.yaml').write_text(
    ...
    env.update(HOME=str(home), ANAKOT_HOME=str(anakot), PYTHONPATH=os.pathsep.join([str(repo), os.environ.get('PYTHONPATH', '')]),
               ANAKOT_ACP_SKIP_CONFIGURED_MCP='1', OPENAI_API_KEY='local-fixture-key',
    ...
    def rows():
        with sqlite3.connect(anakot / 'state.db') as db:
