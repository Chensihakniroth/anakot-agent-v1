...
env = {k: v for k, v in os.environ.items() if not (k.startswith('ANAKOT_') or k.endswith(('_API_KEY', '_TOKEN')))}
env.update(HOME=str(out / 'os-home'), ANAKOT_HOME=str(home), ANAKOT_DASHBOARD_SESSION_TOKEN='hooks-fixture-token', ANAKOT_IGNORE_RULES='1', PYTHONPATH=str(a.repo))
cmd = [sys.executable, '-m', 'anakot_cli.main', 'serve', '--host', '127.0.0.1', '--port', str(a.port), '--skip-build']
