...
def main():
    ...
    env = {"PATH": "/usr/bin:/bin", "HOME": str(root / "home"),
           "ANAKOT_HOME": str(root / "home/.anakot"),
           "XDG_DATA_HOME": str(root / "xdg"), "LANG": "C.UTF-8",
           "ANAKOT_NONINTERACTIVE": "1"}
    Path(env["ANAKOT_HOME"]).mkdir(parents=True)
    venv_dir = root / "venv"
    ...
    install = (
        ...
        "from anakot_cli.linux_desktop_entry import install_desktop_entry; "
        f"sys.argv[0]={str(repo / 'anakot')!r}; "
    ...
    rows["venv_import"] = run([str(python), "-I", "-c", "import yaml,anakot_cli.main; print(yaml.__version__)"], env, "/")
    rows["base_import_negative"] = run([str(python.resolve()), "-I", "-c", "import yaml,anakot_cli.main"], env, "/")
    ...
    custom = original.replace(b"Name=Anakot\n", b"Name=Anakot custom\n").replace(b"Terminal=false", b"Terminal=true")
    entry.write_bytes(custom)
    config = Path(env["ANAKOT_HOME"]) / "config.yaml"
