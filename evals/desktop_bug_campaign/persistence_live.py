...
def main():
    ...
    env = {k: v for k, v in os.environ.items() if not (k.startswith('ANAKOT_') or 'API_KEY' in k or 'TOKEN' in k or k in ('PYTHONPATH', 'PYTEST_PLUGINS'))}
    env.update(ANAKOT_HOME=str(home), HOME=str(home / 'os-home'), ANAKOT_DASHBOARD_SESSION_TOKEN='persistence-fixture-token', ANAKOT_IGNORE_RULES='1', PYTHONPATH=str(args.repo), OPENAI_API_KEY='local-fixture')
    log = (args.out / 'serve.log').open('w')
    cmd = [sys.executable, '-m', 'anakot_cli.main', 'serve', '--isolated', '--port', str(args.port)]
    if observe:
