"""Drive the real ``anakot fallback add`` under a Linux PTY into an ordinary picker error.
...
def _persisted_model(root: Path, env: dict) -> dict:
    ...
    out = subprocess.run(
        [sys.executable, "-c", "import json; from anakot_cli.config import load_config; "
...
def run(root: Path, output: Path) -> dict:
    with tempfile.TemporaryDirectory(prefix="anakot_test_fallback_") as home:
        hh = Path(home) / ".anakot"
        hh.mkdir()
        ...
        env = {"PATH": os.environ["PATH"], "HOME": home, "ANAKOT_HOME": str(hh),
        ...
        proc = subprocess.Popen([sys.executable, "-m", "anakot_cli.main", "fallback", "add"],
