...
def run_case(root, output, name, behind, early=False, cancel=False):
    with tempfile.TemporaryDirectory(prefix="anakot_test_notice_") as home:
        hh = Path(home) / ".anakot"
        hh.mkdir()
        ...
        from anakot_cli.banner import VERSION
        ...
        bootstrap = ("import anakot_cli.main as m; import anakot_cli.banner as b; "
        ...
        try:
            ...
            result = {"case": name, "ready": ready, "exited": exited,
                      ...
                      "loaded_worktree": str(root / "anakot_cli/banner.py") in text,
