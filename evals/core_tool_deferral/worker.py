...
# --- hermetic env BEFORE any anakot import -------------------------------
for var in list(os.environ):
    if var.endswith(("_API_KEY", "_TOKEN")) and var != "OPENROUTER_API_KEY":
        os.environ.pop(var, None)
os.environ.pop("FAL_KEY", None)
os.environ.pop("ANAKOT_PROFILE", None)

tmp_root = tempfile.mkdtemp(prefix=f"ab-{ARM}-{TASK_ID}-")
anakot_home = os.path.join(tmp_root, ".anakot")
workspace = os.path.join(tmp_root, "ws")
os.makedirs(anakot_home)
os.makedirs(workspace)
with open(os.path.join(anakot_home, "config.yaml"), "w", encoding="utf-8") as f:
    f.write("model:\n  provider: openrouter\n  model: %s\n" % MODEL)
...
PREAMBLE = ("You are running inside the Anakot desktop app on the user's machine. "
...
ctx = {
    "workspace": workspace, "anakot_home": anakot_home,
