"""Run the read-tool eval through the REAL Anakot AIAgent.

For each task: fresh temp ANAKOT_HOME, fresh fixture workspace, real
...
def run_task(task, model: str, provider: str, timeout_mult: float,
    ...
    anakot_home = Path(tempfile.mkdtemp(prefix="readtool-home-")) / ".anakot"
    ...
    try:
        ...
        shutil.rmtree(anakot_home.parent, ignore_errors=True)
    return result
...
def main() -> int:
    ...
    if not os.environ.get("OPENROUTER_API_KEY"):
        raise SystemExit(
            ...
            "source ~/.anakot/.env; set +a  — then relaunch."
        )
