...
batch (tracker: [#77056](https://github.com/Chensihakniroth/anakot-agent-v1/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Anakot home, same model, same tasks, same reps.
...
1. Create a dedicated Anakot home with credentials for the models under test:
   ...
   integration at it; no Anakot observability plugin needs to be enabled.
