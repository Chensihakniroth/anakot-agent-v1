...
measured in the production session DB) through `anakot chat` twice — once per
...
Environment:
  ...
  ABEVAL_HOME    ANAKOT_HOME for runs   (default: $ABEVAL_ROOT/home)
                 Must be a configured Anakot home with credentials for the
...
(tracker: Chensihakniroth/anakot-agent-v1#77056).
"""
import json
import os
import re
...
""".strip(),
            ...
            env.update({
                ...
                "ANAKOT_NEMO_RELAY_PLUGINS_TOML": str(relay_config),
            })
            q = TASKS[name].replace("{WORK}", str(work))
            t0 = time.time()
            try:
                p = subprocess.run(
                    [sys.executable, "-m", "anakot_cli.main", "chat", "--query", q,
