...
os.environ.clear()
os.environ.update(allowed, HOME=str(home), ANAKOT_HOME=str(home / "anakot"))
...
def scoped_systemctl(argv, *, timeout):
    if "list-units" in argv:
        ...
        argv = [arg for arg in argv if arg not in ("anakot-gateway*", "anakot-serve*")] + [unit + ".service"]
...
unit = f"anakot-serve-audit-089c35aa-{tag}"
cmd = ["systemctl", "--user"]
def run(args):
