...
Run with the Anakot venv Python and --repo CHECKOUT --output RECEIPT_DIR.
No user environment/config is inherited by Anakot children. Unix PTY required.
"""
import argparse
import http.server
import json
...
def cli_pty(repo, env):
    ...
    proc = subprocess.Popen([sys.executable, "-m", "anakot_cli.main", "model"],
    ...
    try:
        for schema in ("providers", "custom_providers"):
            for surface in ("cli", "rows"):
                ...
                with tempfile.TemporaryDirectory(prefix="anakot-keycmd-live-") as tmp:
                    home = Path(tmp)
                    state = home / ".anakot"
                    state.mkdir()
                    ...
                    env = {"HOME": str(home), "ANAKOT_HOME": str(state),
